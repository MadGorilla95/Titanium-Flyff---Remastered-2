...
void	CAction::OnEndAttackState( DWORD dwState )
{
	...
	case OBJSTA_AEG_ATK:
		CAegonBossMechanicsMng::GetInstance()->OnActEndSpecialAtk(m_pMover);
		break;
#endif
	}
}
...
#ifdef __MOUNT
BOOL	CAction::IsUseMount()
{
	return m_pMover && m_pMover->GetMountId() != NULL_ID && m_pMover->GetMountId() > 0;
}
#endif // __MOUNT
