	...
	if (IsValidObj(pTarget)) {
		...
		if (m_MovementStage == MovementStage)
		{
			if (m_tsMovementUpdate < TIMEGETTIME) {
				...
				g_UserMng.FF_MoveToVarSpot(pMover, dwIdTarget, fRange, bKeepRange, vDst);
			}
		}
		else
		{
		...
		if( pProp->m_nScanJob != -1 )
		{
			...
			if( pTarget )
			{
				if( IsValidObj( (CObj*)pTarget ) )
				{
					...
					if (pMover->IsFlyingNPC() || ( pTarget->m_pActMover->IsFly() == FALSE && pTarget->m_pActMover->IsUseMount() == FALSE ) || pProp->dwClass == RANK_GUARD )	// 가드는 무조건 공격.
#else
...
BOOL CAIMonster::SubAttackChance( const AIMSG & msg, CMover *pTarget )
{
	...
	if( bAttack )
	{
		...
		if (pTarget->IsPlayer() && CAegonBossMechanicsMng::GetInstance()->OnChanceSpecialAtk(pMover, pTarget))
		{
			CUser* pUserTarget = (CUser*)pTarget;
			if (IsValidObj(pUserTarget)) 
			{
				int nError = pMover->SendActMsg(OBJMSG_AEG_ATK, pTarget->GetId(), 0, 0);
				if (nError > 0)
					g_UserMng.AddMeleeAttack(pMover, OBJMSG_AEG_ATK, pTarget->GetId(), 0, MAKELONG(0, (WORD)nError));

				pMover->m_dwLastSpecialAttack = GetTickCount();
...
void CAIMonster::SubSummonProcess( CMover *pTarget )
{
	...
	{
#ifdef __SYS_SANDBOX
		if (pWorld && pWorld->GetID() == WI_WORLD_SANDBOX)
		{
			return;
		}
#endif //__SYS_SANDBOX
...
#if defined( __MOUNT) && !defined(__AEGON_MOB_ATTACK_MOUNTED)
		|| pTarget->m_pActMover->IsUseMount()
#endif // __MOUNT	
#ifdef __AEGON_STAFF_MOB_IGNORE_MODE
		|| ((CMover*)pTarget)->IsMode(AEGON_MOB_IGNORE_MODE)
#endif
		)		// ?고 있던 타겟이 투명화 상태가 되면 
	{
#if __VER >= 14 // __INSTANCE_DUNGEON
...
#if defined( __MOUNT) && !defined(__AEGON_MOB_ATTACK_MOUNTED)
		|| ((CMover*)pTarget)->m_pActMover->IsUseMount()
#endif // __MOUNT	
		)	// this가 비행형이 아니고 상대가 비행중이라면
#endif	// __VER >= 8  
	{
		DoReturnToBegin();
		SendAIMsg( AIMSG_SETSTATE, dwAIState );
...
#ifdef __HEAL_SHOW
				pMover->IncHitPoint(pMover->GetHPRecovery(), (pMover->GetHitPoint() < pMover->GetMaxHitPoint()));
#else // __HEAL_SHOW
