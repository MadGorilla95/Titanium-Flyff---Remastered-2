...
//
#ifdef _WIN64
int CActionMover::ProcessActMsg1(CMover* pMover, OBJMSG dwMsg, INT_PTR nParam1, INT_PTR nParam2, INT_PTR nParam3, INT_PTR nParam4, INT_PTR nParam5)
#else
...
#ifdef __SECURITY_FIXES
		if (pMover->GetPos().y > (m_fCurrentHeight + 0.2f + (pMover->GetParam(DST_JUMPING, 0) / 1000.0f)) * 1.15f)
			return 0;
#endif // __SECURITY_FIXES
...
#endif	//__LEGEND	//	10차 전승시스템	Neuz, World, Trans
#ifdef __SECURITY_FIXES
		if (!pMover->m_pActMover->m_bGround) return 0;
		if (pMover->GetPos().y > (m_fCurrentHeight + 0.2f + (pMover->GetParam(DST_JUMPING, 0) / 1000.0f)) * 1.15f) return 0;
#endif // __SECURITY_FIXES
#ifdef __CLIENT
		if( pMover->IsActiveMover() )
		{
...
#ifdef __GAMEGUARD
		if (!pMover->m_pActMover->m_bGround)
			return 0;
...
//			SendActMsg( OBJMSG_STAND );
			...
			pMover->OnAttackStart( pHitObj, dwMsg );			// 공격시작 핸들러.
			pMover->m_nAtkCnt = 1;		// 카운트 시작.
#ifdef __PSEVDORANGERFIX
			SendActMsg(OBJMSG_STOP_TURN);
			SendActMsg(OBJMSG_STOP_RUN);
			SendActMsg(OBJMSG_ASTOP);
			SendActMsg(OBJMSG_STOP);
#endif // __PSEVDORANGERFIX
		}
		break;
	...
	case OBJMSG_ATK4:	// 공격 4
		{
#if defined(__WORLDSERVER) && defined(__4TH_HIT_HACK)
			if (dwMsg == OBJMSG_ATK4 && pMover->last_atkmsg == OBJMSG_ATK4 && pMover->IsPlayer()) {
				CUser* user = (CUser*)pMover;
				if (user && ++(user->attackhack_count) >= 3) {
					Error("User %s [%07d] used 4th Hit Hack", user->GetName(), user->m_idPlayer);
					g_DPSrvr.QueryDestroyPlayer(user->m_Snapshot.dpidCache, user->m_Snapshot.dpidUser, user->m_dwSerial, user->m_idPlayer);
					return -2;
				}
			}
			else if (pMover->IsPlayer()) {
				((CUser*)pMover)->attackhack_count = 0;
			}
			pMover->last_atkmsg = dwMsg;
#endif //__4TH_HIT_HACK
			if( IsAction() )
				return 0;
...
#ifdef _WIN64
			m_dwAtkFlags = (DWORD)LOWORD((DWORD_PTR)nParam3);
#else
...
#ifdef __SLOW_WALK_FIX
		if (GetWorld() && (GetWorld()->GetID() == WI_WORLD_ARENA || GetWorld()->GetID() == WI_WORLD_GUILDWAR || GetWorld()->GetID() == WI_WORLD_FWC)
#ifdef __AEGON_TOURNAMENT
			|| GetWorld()->GetID() == AT_TOURNAMENT_MAP
#endif // __AEGON_TOURNAMENT
			)
			return 0;
#endif // __SLOW_WALK_FIX
...
#ifdef __MOUNT
		if (pItemProp && pItemProp->dwItemKind3 == IK3_MOUNT)
		{
			if (!pMover->GetMountModel())
				m_pMover->SetMount(pModel, (DWORD)nParam1);
		}
		else
#endif // __MOUNT
...
#ifdef __SECURITY_FIXES
#ifdef __WORLDSERVER
		if (pMover->IsDie() == FALSE && pMover->IsPlayer())
		{
			return 0;
		}
#endif // __WORLDSERVER
#ifdef __CLIENT
		if (CMover::GetActiveMover()->IsDie() == FALSE && CMover::GetActiveMover()->IsPlayer())
			return 0;
#endif // __CLIENT
#endif // __SECURITY_FIXES
		ClearState();
...
#ifdef __MOUNT
	case OBJMSG_MOUNT_GROUND_STAND:
	{
		if (nParam1 <= 0)
			return FALSE;
		...
		if (nParam3 == 0)
		{
...
#endif	//__LEGEND	//	10차 전승시스템	Neuz, World, Trans
			{
				...
				{
					int nAttr = pMover->GetWorld()->GetHeightAttribute(pMover->GetPos().x, pMover->GetPos().z);		// 이동할 위치의 속성 읽음.
					if (nAttr == HATTR_NOWALK)		// 비행 금지 지역에서 타려고 하면 못탄다.
						return 0;
				}
			}
		}

		AddStateFlag(OBJSTAF_MOUNT_GROUND_STAND);
		RemoveStateFlag(OBJSTAF_FLY);
		RemoveStateFlag(OBJSTA_STAND);
		ItemProp* pItemProp = prj.GetItemProp(nParam1);	// 빗자루 프로퍼티.
		...
		{
			m_pMover->GetWorld()->SetObjFocus(NULL);	// 비행직전에 기존타겟팅을 클리어해준다.
			//g_Neuz.m_camera.Lock();//Kamera Fix
			g_WndMng.m_pWndTaskBar->OnCancelSkill();	// 비행직전엔 스킬바 사용 취소.
		}
#endif // __CLIENT

		CModel* pMountModel = prj.m_modelMng.LoadModel(D3DDEVICE, OT_ITEM, (DWORD)nParam1);

		CModelObject* pModelObject = (CModelObject*)pMountModel;
		if (pModelObject->m_pBone)
		{
			CString strMotion;
			if (pModelObject)
			{
				strMotion = pModelObject->GetMotionFileName(_T("stand"));
				assert(strMotion != _T(""));
				pModelObject->LoadMotion(strMotion);
			}
...
#ifdef __CLIENT
		m_pMover->m_eMountState = pMover->GROUND_STAND;
#endif // __CLIENT
		if (!m_pMover->GetMountModel())
			m_pMover->SetMount(pMountModel, (DWORD)nParam1);



		m_pMover->SetMotion(prj.GetMountAnimation((DWORD)nParam1)->dwPlayerAni, ANILOOP_LOOP);
		break;
	}
#endif // __MOUNT
