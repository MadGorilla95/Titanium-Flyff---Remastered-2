...
void CActionMover::PresupposePos2( D3DXVECTOR3* pv, D3DXVECTOR3* pvd, float* pf, float* pfAngleX, float* pfAccPower, u_long uTickCount )
{
	...
	if (IsUseMount())
		fAcc = prj.GetMountAnimation(pMover->GetMountId())->fFlightSpeed;
	else
		fAcc = prj.GetItemProp(pMover->m_dwRideItemIdx)->fFlightSpeed;
#else // __MOUNT
...
#ifdef __TRAILS
		if (g_Option.m_bHideTrails == FALSE)
		{
			if (pMover->HasBuffByIk3(IK3_TRAIL))
			{
				if (nType >= 1 && nType <= 200 && nType != NULL)
				{
					g_ParticleMng.CreateParticle(nType, vPos, vVel, CMover::GetActiveMover()->GetPos().y + 0.5f); break;
				}
			}
		}
#else // __TRAILS
...
void CActionMover::ProcessFlyParticle( float fLenSq )
{
	...
	if( (pMover->IsMode( TRANSPARENT_MODE ) ) == 0 )
	{
		...
		{
#ifdef __TRAILS
			if (pMover->IsActiveMover() || pMover->IsActiveMover() == FALSE)
#else // __TRAILS
...
#endif // __TRAILS
			{
				...
				if (g_Option.m_bHideTrails == FALSE)
				{
					if (pMover->HasBuffByIk3(IK3_TRAIL))
					{
						IBuff* pBuff = pMover->m_buffs.GetBuffByIk3(IK3_TRAIL);
						if (pBuff != NULL)
						{
							ItemProp* pItemProp = pBuff->GetProp();
							if (pItemProp)
							{
								int nType = pItemProp->nEffectValue;
								if (nType >= 1 && nType <= 200 && nType != NULL)
...
void	CActionMover::ProcessFlyMove( void )
{
	...
	MoverProp* moverProp = pMover->GetProp();
...
void	CActionMover::ProcessState2( CMover* pMover, DWORD dwState, float fSpeed )
{
	...
	if (IsUseMount())
	{
		pItemProp = prj.GetItemProp(m_pMover->GetMountId());
		if (!pItemProp)
		{
			Error("ProcessState2(): %s, m_dwMountItemId == %d", m_pMover->m_szName, m_pMover->m_dwRideItemIdx);
			return;
		}
	}
	else
#endif // __MOUNT
	{
		pItemProp = prj.GetItemProp(m_pMover->m_dwRideItemIdx);
...
#ifdef __MOUNT
		if (IsUseMount())
			m_fAccPower = prj.GetMountAnimation(m_pMover->GetMountId())->fFlightSpeed;
		else
		...
		else
		{
			if( pModel->IsAttrHit() )
			{
				...
				if( pItemProp )
				{
					D3DXVECTOR3 v = pMover->GetPos();
#ifdef __fmod
					SoundSystemClass::playSound(pItemProp->dwSndAttack1, { v.x, v.y,v.x });
#else
