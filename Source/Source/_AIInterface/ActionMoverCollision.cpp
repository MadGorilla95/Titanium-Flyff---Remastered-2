...
#ifdef __FIX_EXPLOIT_PET
		if (pMover->IsPlayer())
		{
			pMover->DoDie(NULL);
		}
#else // __FIX_EXPLOIT_PET
		pMover->DoDie(NULL);
#endif // __FIX_EXPLOIT_PET
		break;
		...
		if (vDelta.y < 0.f)
		{
			...
			if (bLanding)			// 착지? 
			{
				...
				else
				{
					...
					m_bLand = FALSE;
#ifdef __CLIENT
					m_pMover->m_eMountState = m_pMover->FLY_START_WALK;
#endif // __CLIENT
					CString strMotion;
					if (m_pMover->GetMountModel())
					{
						strMotion = m_pMover->GetMountModel()->GetMotionFileName(prj.GetMountAnimation(m_pMover->GetMountId())->m_aMonsterAnimation.find(MTI_FLY_GROUND)->second);
						assert(strMotion != _T(""));
						m_pMover->GetMountModel()->LoadMotion(strMotion);
						m_pMover->GetMountModel()->SetMotionBlending(TRUE);
						m_pMover->GetMountModel()->SetLoop(ANILOOP_1PLAY);
					}
...
#ifdef __GAMEGUARD
		float COLL_TEST_DIR = 2.0f;
#else // __GAMEGUARD
		float COLL_TEST_DIR = 6.5f;
#endif // __GAMEGUARD

#ifdef __COLLISION_FIX_PLAYER
		if (pMover->IsPlayer())		//주인공일경우 
		{
			COLL_TEST_DIR = 1.0f;
