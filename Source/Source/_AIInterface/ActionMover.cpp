...
//	m_vDelta.x	= m_vDelta.z = m_vDelta.y	= 0.0f;		
		...
		)	
	{
		...
		if( ( !bFly
			...
			|| m_bLand
#endif // __MOUNT
			) && GetDmgState() != OBJSTA_DISAPPEAR )		// 죽어 사라져가는 중엔 중력처리 안함
		{
			if( m_bGround != 1 
#ifdef __MOUNT
				|| m_bLand != FALSE
#endif // __MOUNT
				)		// 땅에 서있는 경우가 아닐때만, 중력 처리
			{
				m_vDelta.y -= 0.005f;
				if( m_vDelta.y < -0.50f )		
					m_vDelta.y = -0.50f;
...
#ifdef _WIN64
int CActionMover::SendActMsg(OBJMSG dwMsg, INT_PTR nParam1, INT_PTR nParam2, INT_PTR nParam3, INT_PTR nParam4, INT_PTR nParam5)
#else
