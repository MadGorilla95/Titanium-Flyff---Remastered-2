...
BOOL CAIMeteonyker::MoveProcessRage( void )
{
	...
	*/
	...
	if (pTarget->GetType() == OT_MOVER && (((CMover*)pTarget)->IsMode(TRANSPARENT_MODE) || ((CMover*)pTarget)->IsMode(AEGON_MOB_IGNORE_MODE)))
#else
...
//	{
		if( g_tmCurrent > (DWORD)( m_tmRecovery ) )	// 회복 틱 2초가 경과한 상태라면,
		{
			...
			CMover* pMover	= GetMover();
#ifdef __HEAL_SHOW
			pMover->IncHitPoint(50, (pMover->GetHitPoint() < pMover->GetMaxHitPoint()));
#else // __HEAL_SHOW
