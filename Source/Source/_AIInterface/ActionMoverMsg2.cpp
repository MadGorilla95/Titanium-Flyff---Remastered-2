...
//
#ifdef _WIN64
int     CActionMover::ProcessActMsg2(CMover* pMover, OBJMSG dwMsg, INT_PTR nParam1, INT_PTR nParam2, INT_PTR nParam3, INT_PTR nParam4, INT_PTR nParam5)
#else
...
#endif
{
	...
	DWORD dwMountID = m_pMover->GetMountId();
	if (dwMountID != NULL_ID && dwMountID > 0)
		pItemProp = prj.GetItemProp(dwMountID);
	else
		pItemProp = prj.GetItemProp(m_pMover->GetRideItemIdx());
#else
	ItemProp* pItemProp = NULL;
	if (m_pMover->GetMountId() > 0)
