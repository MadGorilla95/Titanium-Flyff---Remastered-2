...
int CAttackArbiter::OnDamageMsgW()
{
...
#ifdef __SYS_SANDBOX
	if (m_pAttacker->IsNPC() && m_pAttacker->IsWorldSandBox())
		if (!((CSandboxMover*)m_pAttacker)->m_bDamage)
			return 0;
#endif //__SYS_SANDBOX
	...
	{
		if (m_dwAtkFlags & (AF_MAGIC | AF_MAGICSKILL))
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_TAKE_PVP_DMG_MAGIC_RATE, 0) / 100);
		}
		else
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_TAKE_PVP_DMG_PHYSICAL_RATE, 0) / 100);
		}
	}
	if (!m_pDefender->IsPlayer())
	{
		if (m_dwAtkFlags & (AF_MAGIC | AF_MAGICSKILL))
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_TAKE_PVE_DMG_MAGIC_RATE, 0) / 100);
		}
		else
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_TAKE_PVE_DMG_PHYSICAL_RATE, 0) / 100);
		}

		SAI79::ePropType type = SAI79::NO_PROP;
		type = m_pDefender->GetPropA()->eElementType;

		switch (type)
		{
		case SAI79::FIRE:
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_FIRE_RATE, 0) / 100);
			break;
		case SAI79::WATER:
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_WATER_RATE, 0) / 100);
			break;
		case SAI79::ELECTRICITY:
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_ELECT_RATE, 0) / 100);
			break;
		case SAI79::WIND:
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_WIND_RATE, 0) / 100);
			break;
		case SAI79::EARTH:
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_EARTH_RATE, 0) / 100);
			break;
		}

		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_STUN)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_STUN, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_DARK)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_DARK, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_POISON)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_POISON, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_SLOW)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SLOW, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_BLEEDING)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_BLEEDING, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_SILENT)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SILENT, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_LOOT)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_LOOT, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_SETSTONE)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SETSTONE, 0) / 100);
		}
		if (m_pDefender->GetAdjParam(DST_CHRSTATE) & CHS_SLEEPING)
		{
			nDamage += (nDamage * m_pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SLEEPING, 0) / 100);
		}
	}
#endif //__V19_FORMATS
...
BOOL CAttackArbiter::CheckValidDamageMsg()
{
	...
	// Check if user attacking himself
...
//#endif	// __PET_0410
	...
	if (pItemElem)
	{
		...
		{
			nATK = (int)(nATK * nWeaponRarity->m_fAttackRate);
		}
	}
...
{
#ifdef __GAMEGUARD
	if (m_pAttacker->m_dwMode & ONEKILL_MODE && m_pAttacker->IsAuthHigher(AUTH_GAMEMASTER2))	// 원킬 모드
#else // __GAMEGUARD
...
int CAttackArbiter::MinusHP( int *pnDamage )
{
	...
	{
#ifdef __GAMEGUARD
		if (m_pDefender->m_dwMode & MATCHLESS2_MODE && m_pDefender->IsAuthHigher(AUTH_GAMEMASTER2))	// HP가 0미만으로 떨어졌는데 반무적 상태냐
#else // __GAMEGUARD
...
{
#ifdef __SYS_WORLDBOSS
	if (m_pDefender->IsNPC() && m_pAttacker->IsPlayer())
		if (m_pDefender->m_bIsWorldBoss == TRUE) {
			if (!easy_strcmp_vec(m_pDefender->_worldbossHWID, ((CUser*)m_pAttacker)->HWID)) 
			{
				m_pDefender->_worldbossHWID.push_back(((CUser*)m_pAttacker)->HWID);
				m_pDefender->m_wbAttackers.push_back(((CUser*)m_pAttacker)->m_idPlayer);
			}
		}
#endif //__SYS_WORLDBOSS
...
#if defined(__MOUNT) && defined(__AEGON_MOB_ATTACK_MOUNTED)
	if (m_pDefender->IsPlayer() && m_pDefender->m_pActMover->IsUseMount() && !m_pDefender->IsFly())
		m_pDefender->UnequipRide();
#endif
...
void CAttackArbiter::OnDied()
{
	...
	if (m_pDefender->m_bIsWorldBoss == TRUE)
	{
		for (auto hwid : m_pDefender->m_wbAttackers)
		{
			CUser* player = (CUser*)prj.GetUserByID(hwid);
			if (IsValidObj(player) && m_pDefender->IsNearPC(player))
			{
				WBLOOT data = CWorldBossController::Instance()->GetLoot(m_pDefender->GetIndex());
				for (auto loot_it = data.begin(); loot_it != data.end(); ++loot_it)
				{
					CItemElem pItemElem;
					pItemElem.m_dwItemId = loot_it->first;
					pItemElem.m_bCharged = 0;
					pItemElem.m_nItemNum = loot_it->second;
					pItemElem.SetSerialNumber();

					if (player->CreateItem(&pItemElem))
					{
						LogItemInfo aLogItem;
						aLogItem.Action = "C";
						aLogItem.SendName = "WorldBoss";
						strcpy(aLogItem.szItemName, pItemElem.GetName());
						aLogItem.RecvName = player->GetName();
						aLogItem.itemNumber = loot_it->second;
						aLogItem.adwItemId[0] = pItemElem.m_dwItemId;
						g_DPSrvr.OnLogItem(aLogItem, &pItemElem, pItemElem.m_nItemNum);
					}
				}
			}
		}
		CWorldBossController::Instance()->BossDeath(m_pDefender);
	}
#endif // __SYS_WORLDBOSS
...
#ifdef __SYS_SANDBOX
	if (m_pDefender && m_pDefender->IsNPC() && !m_pDefender->IsWorldSandBox())
	{
#endif //__SYS_SANDBOX

#ifdef __DST_PET_EXPERIENCE
	CPet* pPet = m_pAttacker->GetPet();
	if (pPet && m_pDefender->IsPlayer() == FALSE)
	{
		{
			int nDst = m_pAttacker->GetParam(DST_PET_EXPERIENCE, 0);
			if (pPet->GetLevel() == PL_EGG && m_pDefender->GetLevel() >= (m_pAttacker->GetLevel() - 5))
			{
				DWORD dwExp = (DWORD)(500);

				if (nDst > 0)
					dwExp = (int)(dwExp + (dwExp * nDst / 100.0f));

				pPet->SetExp(pPet->GetExp() + dwExp);
				((CUser*)m_pAttacker)->AddPetSetExp(pPet->GetExp());
			}
			else if (pPet->GetLevel() != PL_S && pPet->GetExp() < MAX_PET_EXP
				&& m_pDefender->GetLevel() >= (m_pAttacker->GetLevel() - 5))
			{
				DWORD dwExp = (DWORD)(500 / pPet->GetLevel());
	...
	else if (m_pDefender->IsPlayer() == FALSE)
	{
		CSandbox::GetInstance()->DeleteMover(m_pDefender->GetLayer(), m_pDefender);
	}
#endif //__SYS_SANDBOX
...
void CAttackArbiter::OnDiedPVP()
{
	...
	if (IsValidObj(m_pDefender) && IsValidObj(m_pAttacker) && m_pDefender->IsPlayer() && m_pAttacker->IsPlayer())
	{
		CAegonSeasonPassMng::GetInstance()->OnPvPKill(m_pDefender, m_pAttacker);
	}
