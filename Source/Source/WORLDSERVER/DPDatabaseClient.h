...
//	Operations
	...
	void	SendUpdatePlayerData(CUser* pUser, DWORD worldid = 0);
...
#ifdef __VENDING_NPC
private:
	void    OnAddNPCVendor(CAr& ar, DPID, DPID);
	void    OnGetNPCVendor(CAr& ar, DPID, DPID);
	void    SendAddNPCVendorLog(CUser* pUser, CMover* pVendor, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, DWORD dwItemID);
	void    SendRemoveNPCVendorLog(CMover* pVendor);
public:
#ifdef __AEGON_IMPROVED_SHOPDESIGN
	void    SendAddNPCVendor(CUser* pUser, CMover* pVendor, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, DWORD dwItemID, DWORD dwShopDesign);
#else
	void    SendAddNPCVendor(CUser* pUser, CMover* pVendor, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, DWORD dwItemID);
#endif
	void    SendRemoveNPCVendor(CMover* pVendor);
	void    SendUpdateNPCVendor(CMover* pVendor);
	void    SendGetNPCVendor();
	void	SendShopCloseLog(u_long idPlayer, CItemBase* apItem_VT[MAX_VENDITEM]);

#endif // __VENDING_NPC

#ifdef __AEGON_INGAME_DP
	private:
		void    OnDPCommand(CAr& ar, DPID, DPID);
	public:
		void	SendDPCommand(CUser* pUser, __DP_COMMAND_INFO info);
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LAST_ONLINE

		void    OnLastOnlineInit(CAr& ar, DPID, DPID);
		void	SendInitLoadLastLogin();
#endif // __AEGON_LAST_ONLINE

#ifdef __BAN_SYSTEM
		void	SendBan(LPCTSTR szAccount, int nDay);
#endif // __BAN_SYSTEM

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
		void	SendUpdateAchievementOffline(u_long playerID, int achievementID, int additionalProg);
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_PVP_RANKING
		void SendInitLoadPvPRanking();
		void OnPvPRankingInit(CAr& ar, DPID, DPID);
		void SendSavePvPRanking(__PVPRANKING_PROGRESS_UNIT* unit);
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_INGAME_SHOP
		void SendInitLoadIngameShop();
		void OnIngameShopInit(CAr& ar, DPID, DPID);
#endif //__AEGON_INGAME_SHOP

#ifdef __AEGON_TOURNAMENT
		void SendLoadTournaments();
		void OnTournamentLoad(CAr& ar, DPID, DPID);
		void SendSaveTournaments();
#endif //__AEGON_TOURNAMENT

#ifdef __ROYAL_RUMBLE
		void SaveWarPlayers();
		void OnLoadSiegeRanking(CAr& ar, DPID, DPID);
		void LoadSiegeRanking();
