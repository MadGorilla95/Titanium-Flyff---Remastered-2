...
void CDPSrvr::OnUseSkill( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	int nActionSlot = 0;
	ar >> nActionSlot;
	if (nActionSlot < 0 || nActionSlot >= MAX_ACTION_SLOT)
		return;
...
#ifdef __RANGE_FIX
		if (pSkillProp->dwAttackRange != dwRange)
			return;

		if (pUser->GetAttackRange(pSkillProp->dwAttackRange) != fRange)
			return;
#endif //__RANGE_FIX
...
#ifdef __RANGE_FIX
		BOOL fSuccess;
		CObj* objTarget = (CObj*)prj.GetCtrl(objid);
		if (objTarget && objTarget->GetType() == OT_MOVER)
		{
			CMover* pTarget = prj.GetMover(objid);
			if (pTarget && pUser->IsRangeObj(pTarget, pUser->GetAttackRange(pSkillProp->dwAttackRange) + 6.15f))
			{
				fSuccess = pUser->DoUseSkill(wType, nIdx, objid, (SKILLUSETYPE)nUseType, bControl);
#ifdef __ACTION_SLOT_PAGES
				pUser->SetCurrentActionSlot(nActionSlot);
#endif // __ACTION_SLOT_PAGES
			}
			else
				fSuccess = FALSE;
