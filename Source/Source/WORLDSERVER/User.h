...
#ifdef __CHECKLOAD
	void	DoCheckLoad(const unsigned long worldId, const int layer);
#endif //__CHECKLOAD
...
private:
	int		m_nCurrentActionSlotIndex;
public:
	int		GetCurrentActionSlot() const { return m_nCurrentActionSlotIndex; }
	void	SetCurrentActionSlot(const int nIndex) { m_nCurrentActionSlotIndex = nIndex; }
...
#ifdef __AFK_MONITORING
public:
	DWORD			m_dwLastWalkTick[2];
	DWORD			m_dwLastSay;
#endif //__AFK_MONITORING

public:
#if defined(__4TH_HIT_HACK)
	size_t	attackhack_count;
...
private:
	DWORD savedEatPetId;
public:
	DWORD GetSavedEatPetId() { return this->savedEatPetId; }
	void  SetSavedEatPetId(const DWORD savedEatPetId) { this->savedEatPetId = savedEatPetId; }
#endif //_LOOTPET_TELEPORT
...
//	DWORD			m_tmCollect;						/// 자원채집 타이머 - 1초에 한번씩
	...
	DWORD	m_dwLastBuff;
	DWORD	m_dwLastBuffClass;
#endif // __FIX_BUFFPANG_SPAM

#ifdef __GAMEGUARD
	int		m_nNbrOpenPet;
	DWORD	m_tLastOpenPet;
#endif // __GAMEGUARD

#ifdef __TELEPORT_DELAY
	DWORD	m_dwTickLastTeleport;
...
#ifdef __AEGON_FIXES
	DWORD m_dwEqUseCount;
	DWORD m_dwEqUseLastReset;
	DWORD m_dwPetfilterLastSave;
#endif
...
//	void			Flush()	{	Notify();	m_Snapshot.Flush();		}
	...
	void			OnDoUseItemBag(DWORD dwData, OBJID objid, int nBag);
	...
	void			AddCreateItem(CItemBase* pItemBase, BYTE* pnId, short* pnNum, BYTE nCount, BOOL bAdd = FALSE);
#else //__FL_FARM_STAT
...
//	void			AddTradeClearGold( OBJID objid );
...
#ifdef __FL_BAG_UPDATE
	void			AddUpdateItem(CHAR cType, BYTE nId, CHAR cParam, DWORD dwValue, DWORD dwTime, int nBag = -1);
#else //__FL_BAG_UPDATE
	...
	void			AddPocketAddItem(int nPocket, CItemElem* pItem, BOOL bAddDropLog = FALSE);
#else
	...
	void			AddGoldText(int nPlus, BOOL bAddCount = FALSE);
#else //__FL_FARM_STAT
...
#endif	// __SYS_PLAYER_DATA
#ifdef __GAMEGUARD
	void			AddSetState(LONG nStr, LONG nSta, LONG nDex, LONG nInt, LONG nGP, BOOL bRegen);
#else // __GAMEGUARD
	...
	void			AddTag(vector<TAG_ENTRY>* tags);
#else
	void			AddTag( short nTagCount, const TAG_ENTRY* tags );
#endif
#ifdef __ANARCHY_SYSTEM
	void			AddSetSkillState(CMover* pMover, DWORD dwSkill, DWORD dwLevel, DWORD dwTime, bool bOverwrite = false);
	...
	void			AddRegisterPVendorItem(BYTE iIndex, BYTE nType, BYTE nId, short nNum, int nCost, BYTE nBuyMode);
...
#ifdef __VENDING_NPC
	void			AddPVendorRemove(OBJID objidVendor);
#endif // __VENDING_NPC
...
#ifdef __AEGON_FIXES
	void			AddPartyMapInfo(int nIndex, D3DXVECTOR3 vPos, DWORD dwWorldID);	// __S1005_PARTY_MAP
#else
...
public:
	...
	void AddAssistClient(BOOL send);
#endif // __PARTY_ASSISTANT

#if __VER >= 15 // __PETVIS
#ifdef __AEGON_FIXES
	void AddActivateVisPet(OBJID objIdVisPetItem, OBJID objIdVisPetId, BOOL bRegularEatPet = FALSE);
#else
...
#ifdef __VENDING_NPC
public:
	void    AddNpcVendorLink(DWORD dwVendorTime, DWORD dwVendorItem);
	void    AddNpcVendorUnlink(DWORD dwVendorItem);
#endif // __VENDING_NPC
...
public:
	void	AutoConvertPerin();
	BOOL	ConvertSinglePerin();
#endif //  __AEGON_CONVERT_PERIN

#ifdef __MIKSIK_EQUALIZED_MODE
public:
	void AddInvAndBuffsSerialize();
...
#ifdef __AEGON_MONSTER_HUNT
public:
	void AddMHLoadProgress();
	void FinishedMHQuest(DWORD questID);
#endif // __AEGON_MONSTER_HUNT

#ifdef __LEEAEG_SHOP_FINDER
	void AddShopFinderRefresh();
#endif // __LEEAEG_SHOP_FINDER

#ifdef __GIFTBOX_LIST_ITEMS
	void	AddGiftboxPreview(vector<pair<DWORD, pair<DWORD, DWORD>>> map);
#endif // __GIFTBOX_LIST_ITEMS

#ifdef __DROP_LOG
public:
#ifdef __FL_FARM_STAT
	void	AddReapGold(const unsigned int& nGold, BOOL bAddCount);
#else //__FL_FARM_STAT
...
public:
	void	AddWeaponRarity(DWORD dwID, int nRarity);
...
#ifdef __AEGON_LOOT_BOX
	void	AddLootboxResult(__LOOTBOX_RESULT result);
#endif // __AEGON_LOOT_BOX

#ifdef __AEGON_VIEW_INV
public:
	void	AddViewInv(CUser* pViewUser);
	void	AddViewBag(CUser* pViewUser);
	void	AddViewBank(CUser* pViewUser);
#endif // __AEGON_VIEW_INV

#ifdef __BADGE
	void	AddBadgeListUpdate();
#endif //__BADGE
...
#ifdef __GUILDFINDER
	void	AddSetGuildFinder(u_long idGuild, BOOL bGuildFinder);
	void	AddGuildList(vector<GUILDFINDER_LIST> vectmp, int nCount);
	void	SendGuildApplication(CUser* pUser);
#endif //__GUILDFINDER

#ifdef __SYS_MONSTER_CLASH
	void	AddMonsterClashRegister(DWORD dwGuildId, CString szGuild);
	void	AddMonsterClashRegisterMember(u_long idPlayer, CString szPlayer);
	void	AddMonsterClashUnregisterMember(u_long idPlayer, CString szPlayer);
	void	AddMonsterClashRegisterWindow();
	void	AddMOnsterClashRegisterMemberWindow();
	void	AddMonsterClashActive(map<DWORD, CString>& aGuildAct, int nMaxKill, int nMaxState);
	void	AddMonsterClashKillCount(DWORD dwGuild, int nKillCount, time_t tTime, int nState);
	void	AddMonsterClashInactive();
	void	AddMonsterClashState(int nState, time_t tTime);
	void	AddMonsterClashGuilds();
...
public:
	...
	void	AddAchievementLoadProgress();
	void	AddFinishedAchievement(int achievementID);
	void	AddFavoriteAchievementRefresh();
	void	AddCollectingTimeRefresh();
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __UPGRADE_SUCCESS
	void	SendUpgradeSuccessMap();
#endif // __UPGRADE_SUCCESS

#ifdef __AEGON_LAST_ONLINE
	void	AddLastOnlineRefresh();
	void	AddSingleLastOnlineRefresh(u_long playerID);
	void	AddOwnLastOnlineRefresh();
#endif // __AEGON_LAST_ONLINE

#ifdef __AEGON_INGAME_TIME
	void	CalculateNewOnlineTime();
	void	AddOnlineTimeRefresh();
#endif // __AEGON_INGAME_TIME

#ifdef __PLAYER_LIST
public:
	void	SendPlayerList(BOOL isFakePlayer, PlayerList player, BOOL bSort = TRUE, BOOL bClear = TRUE);
	void	SendPlayerList(BOOL isFakePlayer);
#endif // __PLAYER_LIST

#ifdef __AEGON_CRAFTING_SYSTEM
	void	AddCraftingSuccess();
	void	AddCraftingExpRefresh();
#endif // __AEGON_CRAFTING_SYSTEM

#ifdef __WEAPON_MERGE
	void	AddMergedWeapon(CItemElem* pItem);
	void	AddMergedWeaponCancel(DWORD dwID);
...
public:
	void	UpdatePlayerCount(const long& count);
#endif //__CLIENT_ONLINE_COUNT

#ifdef __AEGON_SEASON_PASS
	void	AddSeasonPassLoadProgress();
	void	AddSeasonPassUpdateProgress(__SEASONPASS_PROGRESS_UNIT* progUnit);
	void	AddSeasonPassCountdown(time_t pveCountdown, time_t pvpCountdown);
...
public:
	...
	void	ProcessPermanentBuffs();
	void	ActivatePermanentBuffs();
	BOOL	DoUsePermBuff(ItemProp* pItemProp);
	void	AddPermBuffRefresh();
#endif // __AEGON_PERMANENT_BUFF

#ifdef __AEGON_PREM_MOTIONS
	BOOL	DoUnlockMotion(ItemProp* pItemProp);
	BOOL	DoUnlockMotion(DWORD dwMotionID);
	void	AddUnlockedMotionsRefresh();
#endif // __AEGON_PREM_MOTIONS

#ifdef __AEGON_EMOTE_SYSTEM
	BOOL	DoUnlockEmote(ItemProp* pItemProp);
	BOOL	DoUnlockEmote(DWORD dwEmoteID);
	void	AddUnlockedEmotesRefresh();
#endif //__AEGON_EMOTE_SYSTEM

#ifdef __LEEAEG_SYSTEM_INDIVIDUAL
	void	AddUserSystemShout(LPCSTR szMessage, int nImage, int nFont);
#endif //__LEEAEG_SYSTEM_INDIVIDUAL

#ifdef __GAMEGUARD
public:
	void	ClearObj(float fRange);
#endif //__GAMEGUARD

#ifdef __CRYSTAL
public:
	void	AddCrystal(DWORD dwID, DWORD dwCrystal);
#endif // __CRYSTAL

#ifdef __AEGON_INGAME_SHOP
public:
	void	AddIngameShopInfo();
	BOOL	GiveWebshopItem(__AEGIGNSHP_ITEM_UNIT* pShopUnit);

#endif //__AEGON_INGAME_SHOP

#ifdef __AEGON_TOURNAMENT
	void	AddTournamentMap();
	void	AddTournamentsChanged();
#endif //__AEGON_TOURNAMENT
...
public:
	BOOL	IsSiegeUser();
	void	AddWarTime(DWORD dwTime, int nState);
	void	SetRegister(BOOL bApply) { m_bApply = bApply; };
	void	SetFightState(BOOL bFightStage);
	void	AddSiegeEnterWindow(BOOL bFightState, DWORD dwTime);
	void	AddRankingList();
	void	UpdatePlayerList();
#endif // __ROYAL_RUMBLE

#ifdef __AEGON_AUTO_STATS
	void AddAutoStatRefresh();
	void DistributeAutoStat();
#endif

#ifdef __AEGON_F_TOOL
	void AddFToolRefresh();
#endif

#ifdef __SYS_REBIRTH
	bool	ProcessRebirthLevelUp();
#endif //__SYS_REBIRTH

#ifdef __AEGON_TIME_DIFF
	void AddUpdateTimeDiff();
#endif //__AEGON_TIME_DIFF

#ifdef __FL_BAG_LOOT
public:
	void	AddSelectedBag(int nSelectedBag);
#endif //__FL_BAG_LOOT

#ifdef __AEGON_PVP_MINIGAMES
	void AddPvPMinigameInvitation(u_long uChallengerId, int nGameID, LPCTSTR challengerName);
	void AddPvPMinigameRefresh();
	void AddPvPMinigameChat(CString* strMessage, BOOL bSystem = FALSE);
	void AddPvPMinigameList();
#endif

#ifdef __AEGON_IMPROVED_SHOPDESIGN
	void AddVendorShopDesign(CMover* pMover);
#endif

#ifdef __AEGON_MESSENGER
	void AddMessengerUpdate(vector<int>* vStatus);
	void AddMessengerChatUnit(BOOL bExists, char* szName);
#endif

#ifdef __SYS_PETFILTER
	BOOL ShouldPickUp(CItem* pItem);
#endif
#ifdef __SYS_LOCK_ITEM
	void AddItemLock(OBJID objid, bool bIsLocked);
#endif

#ifdef __MAX_COLLECT_BY_IP
	void AddCollectingStatus(int nStatus);
#endif

#ifdef __STOP_EXP
	void AddExpStopState(BOOL bState);
#endif

#ifdef __AUTO_MAX_SKILL
	void SetAutoMaxSkills();
#endif

#ifdef __AEGON_BAG_FREE_MOVE
	void AddMovePocketItem(int nPocket, int nSrcIndex, int nDestIndex);
#endif
};
...
public:
	void			AddSetNameColor(CUser* pUser, int r, int g, int b);
#endif //__SYS_NAMECOLOR

#ifdef __ANARCHY_SYSTEM
public:
	map < DWORD, CUser*> GetUsersMapRef() { return m_users; }
#endif //__ANARCHY_SYSTEM

#ifdef __TICKETS_DETAILS
public:
	void    		LayersDetail(CUser* pUser2);
#endif //__TICKETS_DETAILS
...
public:
...
#ifdef __WS_STAFF_COUNT
	LONG			GetStaffCount() { return m_sCount; }
	void			AddStaffCount();
#endif //__WS_STAFF_COUNT
	void			RemoveAllUsers();
	...
	CUser* GetUserByName(LPTSTR name);
#endif
...
#ifdef __HEAL_SHOW
	void			AddHealDamage(CMover* pMover, OBJID objidAttacker, DWORD dwHit);
#endif // __HEAL_SHOW
...
#ifdef __AEGON_FIXES
	void			AddSetPos(CCtrl* pCtrl, D3DXVECTOR3& vPos, BOOL bTeleportImage = FALSE);
#else
...
#ifdef __ANARCHY_SYSTEM
	void			AddSetSkillState(CMover* pCenter, CMover* pTarget, WORD wType, DWORD dwID, DWORD dwLevel, DWORD dwTime, bool bOverwrite = false);
#else // __ANARCHY_SYSTEM
	void			AddSetSkillState(CMover* pCenter, CMover* pTarget, WORD wType, WORD wID, DWORD dwLevel, DWORD dwTime);
#endif // __ANARCHY_SYSTEM
#else
#ifdef __ANARCHY_SYSTEM
	void			AddSetSkillState(CMover* pCenter, CMover* pTarget, WORD wType, WORD wID, DWORD dwLevel, DWORD dwTime, bool bOverwrite = false);
#else // __ANARCHY_SYSTEM
	void			AddSetSkillState(CMover* pCenter, CMover* pTarget, WORD wType, WORD wID, DWORD dwLevel, DWORD dwTime);
#endif // __ANARCHY_SYSTEM
#endif
...
#endif //__MIKSIK_EQUALIZED_MODE
#ifdef __AEGON_FIXES
	void			AddModifyMode(CMover* pMover, BOOL bShowMessage = TRUE, BOOL bForce = FALSE);
#else
	void			AddModifyMode( CMover* pMover );
#endif
...
#ifdef __VENDING_NPC
	void            AddPVendorOpen(CUser* pUser, CMover* pVendorNPC = NULL);
	void			AddPVendorClose(CUser* pUser, BOOL bShopClear = TRUE);
#else // __VENDING_NPC
...
//	void			AddDoCollect( CUser* pUser, OBJID idTarget );
	...
	void			AddDoApplySkill(CCtrl* pCtrl, OBJID idTarget, DWORD dwSkill, DWORD dwLevel, int nSfxMode = 0);
#else
	...
	void	UpdateTitle(CUser* pUser);
#endif // __SYS_CUSTOM_TITLE

#ifdef __PREMIUM
	void	UpdatePremium(CUser* pUser);
#endif // __PREMIUM

#ifdef __MOUNT
	void	UpdateMountState(CUser* pUser, DWORD dwID);
#endif // __MOUNT

#ifdef __BADGE
	void	AddBadgeChange(CMover* pMover, int nChange) const;
#endif //__BADGE

#ifdef __SHOPS_VISITED
	void	AddRemoveShop(OBJID id);
#endif //__SHOPS_VISITED

#ifdef __CHECKLOAD
	vector<CUser*> VecCheckLoad(DWORD getworld, DWORD layer);
#endif //__CHECKLOAD

#ifdef __AFK_MONITORING
	void	AddMessiState(CUser* pUser);
#endif //__AFK_MONITORING

#ifdef __PET_GLOW
	void    AddChangePetGlowSfxId(CMover* pMover);
#endif // __PET_GLOW

#ifdef __LEESE_STAFF_CONNECT
	void AddStaffConnect(CUser* pUser2);
...
#ifdef __ROYAL_RUMBLE
	void	AddClientUpdate(u_long idPlayer);
#endif // __ROYAL_RUMBLE

#ifdef __AEGON_NINJA_MODE
	void	AddUserNinjaMode(CUser* pUser);
#endif

#ifdef __SYS_REBIRTH
	void	AddRebirthLevelUp(CUser* const pUser) const;
#endif //__SYS_REBIRTH
	...
	void AddChangeWingSfx(CMover* pMover);
