...
#ifdef __GAMEGUARD
	if (pItemElem0->GetProp()->dwItemKind3 == IK3_PET || pItemElem1->GetProp()->dwItemKind3 == IK3_PET || (IsUsableItem(pItemElem2) && pItemElem2->GetProp()->dwItemKind3 == IK3_PET))
	{
...
BYTE	CItemUpgrade::SmeltSafetyGeneral( CUser* pUser, CItemElem* pItemMain, CItemElem* pItemMaterial, CItemElem* pItemProtScr, CItemElem* pItemSmeltScr )
{
	...
	bool bSuccess = false;
	bool bFind = false;
	auto iter = prj.m_mapUpgradeSuccess.find(pItemMain->GetProp()->dwReferStat1);
	if (iter != prj.m_mapUpgradeSuccess.end())
	{
		const map<int, int>& upgrade = iter->second;
		auto iterUpgrade = upgrade.find(pItemMain->GetAbilityOption());
		if (iterUpgrade != upgrade.end())
		{
			bFind = true;
			bSuccess = iterUpgrade->second > 0 && pItemMain->m_nUpgradeTries + 1 >= iterUpgrade->second;
		}
	}

	if ((int)(xRandom(10000)) > nPercent && !bSuccess)
#else // __UPGRADE_SUCCESS
...
#ifdef __UPGRADE_SUCCESS
		if (bFind)
			pUser->UpdateItem((BYTE)pItemMain->m_dwObjId, UI_UPGRADE_SUCCESS, pItemMain->m_nUpgradeTries + 1);
#endif // __UPGRADE_SUCCESS
...
#ifdef __UPGRADE_SUCCESS
		pUser->UpdateItem((BYTE)pItemMain->m_dwObjId, UI_UPGRADE_SUCCESS, 0);
#endif // __UPGRADE_SUCCESS
...
BYTE	CItemUpgrade::SmeltSafetyAccessory(CUser* pUser, CItemElem* pItemMain, CItemElem* pItemMaterial, CItemElem* pItemProtScr )
{
	...
	if (xRandom(10000) > dwProbability && !bSuccess)
#else // __UPGRADE_SUCCESS
...
void	CItemUpgrade::RefineAccessory( CUser* pUser, CItemElem* pItemMain, CItemElem* pItemMaterial )
{
	...
	if (iter != prj.m_mapUpgradeSuccess.end())
	{
		...
		{
			bFind = true;
			if (iterUpgrade->second > 0 && pItemMain->m_nUpgradeTries + 1 >= iterUpgrade->second)
			{
				bSuccess = true;
			}
		}
...
void CItemUpgrade::SetPetVisItem(CUser* pUser, OBJID objIdVis, int nBag)
...
#endif //__FL_BAG_UPDATE
{
	...
	if (nBag != -1)
		pItemElemVis = pUser->m_Pocket.GetAtId(nBag, objIdVis);
#else //__FL_BAG_UPDATE
	CItemElem* pItemElemVis = pUser->m_Inventory.GetAtId(objIdVis);
#endif //__FL_BAG_UPDATE
	...
	for( int i=0; i<pItemElemPet->GetPiercingSize(); i++ )
	{
		...
		{
#ifdef __SYS_STACKABLE_BEADS
			const time_t now = time(0);
			const time_t timeLeft = pItemElemPet->GetVisKeepTime(i);
			const long beadTimeLeft = (timeLeft - now) / 60;
			
			if (beadTimeLeft > (__SYS_STACKABLE_BEADS - pVisProp->dwAbilityMin))
			{
				pUser->AddText("You can not stack this bead any further!", COLOR_ERROR);
				return;
			}

			u_long timeTotal = beadTimeLeft + pVisProp->dwAbilityMin;
			if (timeTotal > __SYS_STACKABLE_BEADS)
				timeTotal = __SYS_STACKABLE_BEADS;

			pUser->UpdateItem((BYTE)pItemElemPet->m_dwObjId, UI_PETVIS_ITEM, i, timeTotal);
			pUser->RemoveItem((BYTE)objIdVis, 1);
			
			PutPetVisItemLog(pUser, "!", "VIS_MATERIAL", pItemElemPet, i);
			g_DPSrvr.PutItemLog(pUser, "$", "VIS_PIERCING", pItemElemPet);

			pUser->AddText("Your bead duration was increased!", COLOR_SUCCESS);
#else //__SYS_STACKABLE_BEADS
			pUser->AddDefinedText(TID_GAME_BUFFPET_EQUIPVIS);
#endif //__SYS_STACKABLE_BEADS
			return;
		}
...
#ifdef __FL_BAG_UPDATE
	if (nBag != -1)
		pUser->RemoveItem2((BYTE)(objIdVis), 1, nBag);
	else
		pUser->RemoveItem((BYTE)(objIdVis), 1);
#else //__FL_BAG_UPDATE
	pUser->RemoveItem((BYTE)(objIdVis), 1);
...
#ifdef __PET_GLOW
	if (pVisPet && pItemElemPet && IsValidObj(pUser) && IsValidObj(pVisPet))
#else // __PET_GLOW
