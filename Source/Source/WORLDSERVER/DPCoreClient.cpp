...
CDPCoreClient::CDPCoreClient()
{
	...
	ON_MSG(PACKETTYPE_SETGUILDCOLOR, &CDPCoreClient::OnGuildSetColor);
...
//	ON_MSG( PACKETTYPE_MODIFYMODE, &CDPCoreClient::OnModifyMode );
	...
	ON_MSG(PACKETTYPE_ASSISTANT, &CDPCoreClient::OnPartyAssistantCore);
...
#ifdef __VENDING_NPC
	ON_MSG(PACKETTYPE_REMOVEVENDORNPC, &CDPCoreClient::OnRemoveNPCVendor);
#endif // __VENDING_NPC

#ifdef __ANARCHY_SYSTEM
	ON_MSG(PACKETTYPE_ANARCHY_BUFFS, &CDPCoreClient::OnAnarchyBuff);
#endif // __ANARCHY_SYSTEM

#ifdef __GUILDFINDER
	ON_MSG(PACKETTYPE_CW_GUILDFINDER, &CDPCoreClient::OnGuildFinderACk);
#endif //__GUILDFINDER

#ifdef __SYS_MAP_BUFFS
	ON_MSG(PACKETTYPE_CHECK_MAPBUFFS, &CDPCoreClient::OnCheckMapBuffs);
#endif //__SYS_MAP_BUFFS
...
void CDPCoreClient::OnLoadWorld( CAr & ar, DPID, DPID, OBJID )
{
	...
	std::map<DWORD, CString> existingWorlds;
#else //__LEESE_WORLDSERVER_OPT
	char szMsg[1024] = "";
...
#ifdef __LEESE_WORLDSERVER_OPT
		existingWorlds[pJurisdiction->m_dwWorldID] = lpWorld->m_szFileName;
#else //__LEESE_WORLDSERVER_OPT
...
#ifdef __LEESE_WORLDSERVER_OPT
	CString worldDisplay = "  Worlds Loaded\n  ------------------------------\n", worldName;
	for (const auto& worldId : existingWorlds)
	{
		worldDisplay.AppendFormat("[%lu] %s - ", worldId, worldName.GetString());
	}

	SetServerList(std::string(worldDisplay.GetString(), worldDisplay.GetLength() - 1)); // -1 to remove trailing space (or \n)
#else //__LEESE_WORLDSERVER_OPT
...
void CDPCoreClient::SendSay(u_long idFrom, u_long idTo, const CHAR* lpString, char* szToName)
#else
...
void CDPCoreClient::SendGetDiffPlayer(u_long idOperator)
{
	BEFORESENDDUAL(ar, PACKETTYPE_GETDIFFPLAYER, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << idOperator;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
void CDPCoreClient::SendSystem(const CHAR* lpString, int nImage, int nFont)
...
void CDPCoreClient::OnAddPartyMember( CAr & ar, DPID, DPID, OBJID )
{
...
#ifdef __NO_PARTY_GW_JOIN
	CUser* ptmpUser = (CUser*)prj.GetUserByID(idMember);
	if (ptmpUser)
	{
		if (ptmpUser->GetWorld()->GetID() == WI_WORLD_GUILDWAR1TO1_0 || ptmpUser->GetWorld()->GetID() == WI_WORLD_GUILDWAR1TO1_1 
			|| ptmpUser->GetWorld()->GetID() == WI_WORLD_GUILDWAR1TO1_2 || ptmpUser->GetWorld()->GetID() == WI_WORLD_GUILDWAR1TO1_3
			|| ptmpUser->GetWorld()->GetID() == WI_WORLD_GUILDWAR || ptmpUser->m_nGuildCombatState == 1)
		{
			return;
		}
	}
#endif // __NO_PARTY_GW_JOIN
	...
	if( pParty )
	{
		...
		if (pParty->GetLeader())
		{
			...
			{
				CUser* pUser = g_UserMng.GetUserByPlayerID(idMember);
				if (IsValidObj(pUser))
				{
					pUser->AddText("The leader is refusing partyfinder joins.", COLOR_WARNING);
				}
			}
	}
...
#endif	// __SYS_PLAYER_DATA
		{
			...
			if (IsValidObj((CMover*)pUser))
			{
				if (((CMover*)pUser)->GetAssist() == TRUE)
				{
			...
			for( int i = 0; i < pParty->m_nSizeofMember; i++ )
			{
				...
				{
#ifdef __PARTY_ASSISTANT
					g_dpDBClient.SendUpdatePlayerData(pMember);
#endif //__PARTY_ASSISTANT
#if __VER >= 11 // __SYS_PLAYER_DATA
...
void CDPCoreClient::OnSetPartyExp( CAr & ar, DPID, DPID, OBJID )
{
	...
	if( pParty )
	{
		...
		int nPartyLevelProg = 0;
		if (pParty->m_nLevel < nLevel)
		{
			nPartyLevelProg = nLevel - pParty->m_nLevel;
		}
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
				if(nPartyLevelProg > 0)
					CAegonAchievementMng::GetInstance()->AddAchievementProgressForType(pUser, AEGACH_PARTYLEVELUP, nPartyLevelProg);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
			}
...
void CDPCoreClient::OnAddFriend( CAr & ar, DPID, DPID, OBJID )
{
	...
	if( IsValidObj( (CObj*)pSender ) )
	{
		...
		else
		{
			...
			CAegonLastOnlineMng::GetInstance()->DistributeRefreshs(pSender);
			CAegonLastOnlineMng::GetInstance()->DistributeRefreshs(pFriend);
	...
	if( pGuild )
	{
		...
		i != pGuild->m_mapPMember.end(); ++i )
		{
			...
			if( IsValidObj( pUsertmp ) )
			{
				...
				pUsertmp->RemoveIk3Buffs(IK3_GUILD_BUFF);
		...
		*/
		...
		for( int cou = 0 ; cou < nSize ; ++cou )
		{
			if( IsValidObj( pUserIk3[cou] ) )
			{
				...
				pUserIk3[cou]->m_tGuildMember += CTimeSpan(0, 0, 10, 0);
...
#ifdef __AEGON_LAST_ONLINE
				CAegonLastOnlineMng::GetInstance()->DistributeRefreshs(pUser);
#endif // __AEGON_LAST_ONLINE
...
#ifdef __10MIN_CD_REGUILD			
			pUser->m_tGuildMember += CTimeSpan(0, 0, 10, 0);
#else // __10MIN_CD_REGUILD
...
// 길드 스탯변경 요청 
#ifdef _WIN64
BOOL CDPCoreClient::SendGuildStat(CUser* pUser, GUILD_STAT stat, ULONG_PTR data)
#else
...
#ifdef _WIN64
			ar << pUser->m_idGuild << pUser->m_idPlayer << (DWORD)data;
#else
...
void CDPCoreClient::OnSetFriendState( CAr & ar, DPID, DPID, OBJID )
{
	...
	if (IsValidObj(pUser))
	{
		...
		pUser->m_dwAFKState = dwState;
		if (dwState == FRS_ABSENT)
		{
...
#ifdef __SECURITY_FIXES
			if (pFriend->dwState < FRS_ONLINE || pFriend->dwState > MAX_FRIENDSTAT)
				pFriend->dwState = FRS_OFFLINE;
#endif // __SECURITY_FIXES
...
#ifdef __PARTY_ASSISTANT
void CDPCoreClient::OnPartyAssistantCore(CAr& ar, DPID, DPID, OBJID)
{
	u_long uPartyId, uAssist;
	ar >> uPartyId >> uAssist;
	...
	if (pParty)
	{
		CUser* pAssist = (CUser*)prj.GetUserByID(uAssist);
		if (IsValidObj((CMover*)pAssist))
		{
			CString str;
			if (((CMover*)pAssist)->GetAssist() == FALSE)
			{
				str.Format("%s is assistant now and can use party skills!", pAssist->GetName());
			}
			else
			{
				str.Format("%s is no longer an assistant and can not use party skills!", pAssist->GetName());
			}
			...
			for (int i = 0; i < pParty->m_nSizeofMember; i++)
			{
				...
				if (IsValidObj(pMember) && IsValidObj(pAssist))
				{
					if (pMember == pAssist)
					{
						if (((CMover*)pAssist)->GetAssist() == FALSE)
						{
							((CMover*)pAssist)->m_bAssistMember = TRUE;
							pAssist->AddAssistClient(TRUE);
							pAssist->AddText("You are a party assistant now!", COLOR_SUCCESS);
						}
						else
						{
							((CMover*)pAssist)->m_bAssistMember = FALSE;
							pAssist->AddAssistClient(FALSE);
							pAssist->AddText("You are no longer a party assistant...", COLOR_ERROR);
						}
					}
					else
					{
			...
			g_dpDBClient.SendUpdatePlayerData(pAssist);
		}
	}
}
#endif //__PARTY_ASSISTANT
...
#ifdef __VENDING_NPC
void CDPCoreClient::SendRemoveNPCVendor(CMover* pVendor)
{
	BEFORESENDDUAL(ar, PACKETTYPE_REMOVEVENDORNPC, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pVendor->m_vtInfo.GetOwner();
	SEND(ar, this, DPID_SERVERPLAYER);
}

#if _WIN64
void CDPCoreClient::OnRemoveNPCVendor(CAr& ar, DPID, DPID, OBJID)
#else
void CDPCoreClient::OnRemoveNPCVendor(CAr& ar, DPID, DPID, DPID)
#endif
{
	u_long idPlayer;
	ar >> idPlayer;
...
#ifdef __ANARCHY_SYSTEM
void CDPCoreClient::SendAnarchyBuff(DWORD dwId)
{
	BEFORESENDDUAL(ar, PACKETTYPE_ANARCHY_BUFFS, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << dwId;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#include <iostream>

void CDPCoreClient::ReloadAnarchyBuff(CUser* pUser)
{
	for (int i = 0; i < prj.m_vAnarchyBuffProp.size(); i++)
	{
		ItemProp* pBuffProp = prj.GetItemProp(prj.m_vAnarchyBuffProp[i].nId);

		if (!pBuffProp)
			continue;

		if (pBuffProp->dwItemKind3 != IK3_ANARCHY)
			continue;

		if (m_mapAnarchy.find(pBuffProp->dwID)->second <= GetTickCount())
		{
			pUser->RemoveBuff(BUFF_ITEM, pBuffProp->dwID);
			continue;
		}

		//pBuffProp->dwSkillTime = m_mapAnarchy.find(pBuffProp->dwID)->second - GetTickCount();
		//pUser->DoApplySkill(pUser, pBuffProp, nullptr);
		ItemProp* tmpProp = new ItemProp(*pBuffProp);
		if (tmpProp)
		{
			tmpProp->dwSkillTime = m_mapAnarchy.find(pBuffProp->dwID)->second - GetTickCount();
			pUser->DoApplySkill(pUser, tmpProp, nullptr);
			delete(tmpProp);
		}
	}
}

void CDPCoreClient::OnAnarchyBuff(CAr& ar, DPID, DPID, OBJID)
{
	DWORD dwId;
	ar >> dwId;

	const map<DWORD, CUser*>& mapUsers = g_UserMng.GetUsersMapRef();

	ItemProp* pBuffProp = prj.GetItemProp(dwId);
	if (!pBuffProp)
		return;
	...
	__time32_t time = GetTickCount() + pBuffProp->dwSkillTime;

	if (m_mapAnarchy.find(dwId) != m_mapAnarchy.end())
		m_mapAnarchy.erase(dwId);

	m_mapAnarchy.insert(make_pair(dwId, time));

	for (const auto& it : mapUsers)
	{
...
#ifdef __SYS_GUILDCOLOR
void CDPCoreClient::SendSetGuildColor(DWORD guildid, int r, int g, int b)
{
	BEFORESENDDUAL(ar, PACKETTYPE_SETGUILDCOLOR, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << guildid << r << g << b;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPCoreClient::OnGuildSetColor(CAr& ar, DPID, DPID, OBJID)
{
	DWORD guildid;
	int r, g, b;
	ar >> guildid;
	...
	CGuild* pGuild = g_GuildMng.GetGuild(guildid);
	if (!pGuild)
		Error("CDPCoreClient:: OnSetGuildColor Fucked up");
...
#ifdef __GUILDFINDER
void CDPCoreClient::OnGuildFinderACk(CAr& ar, DPID, DPID, OBJID)
{
	u_long idGuild;
	BOOL	bGuildFinder;
	...
	if (pGuild)
	{
		...
		for (map<u_long, CGuildMember*>::iterator i = pGuild->m_mapPMember.begin(); i != pGuild->m_mapPMember.end(); ++i)
		{
			...
			pUser = (CUser*)prj.GetUserByID(pMember->m_idPlayer);
			if (IsValidObj(pUser))
				pUser->AddSetGuildFinder(idGuild, bGuildFinder);
		}
	}
}
void CDPCoreClient::SendGuildFinder(CUser* pUser, BOOL bGuildFinder)
{
	BEFORESENDDUAL(ar, PACKETTYPE_WC_GUILDFINDER, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pUser->m_idGuild << pUser->m_idPlayer << bGuildFinder;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__GUILDFINDER
...
#ifdef __SYS_MAP_BUFFS
#if _WIN64
void CDPCoreClient::OnCheckMapBuffs(CAr& ar, DPID, DPID, OBJID)
#else
void CDPCoreClient::OnCheckMapBuffs(CAr& ar, DPID, DPID, DPID) 
#endif
{
	CMapBuffManager::GetInstance()->Reload(false);
}

void CDPCoreClient::SendCommandCheckMapBuffs() 
{
	BEFORESENDDUAL(ar, PACKETTYPE_CHECK_MAPBUFFS, DPID_UNKNOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__SYS_MAP_BUFFS