...
public:
	...
	map < DWORD, __time32_t> m_mapAnarchy;
...
//	Operator commands
	...
	void	SendSay( u_long idFrom, u_long idTo, const CHAR* lpString, char* szToName = NULL );
...
#ifdef __GUILDFINDER
	void 	OnGuildFinderACk(CAr& ar, DPID, DPID, OBJID);
	void	SendGuildFinder(CUser* pUser, BOOL bGuildFinder);
#endif //__GUILDFINDER
	...
	void	SendGetDiffPlayer(u_long idOperator);
	...
	void	SendSystem( const CHAR* lpString, int nImage = 0, int nFont = 0 );
...
#ifdef _WIN64
	BOOL	SendGuildStat(CUser* pUser, GUILD_STAT stat, ULONG_PTR data);
#else
...
#ifdef __ANARCHY_SYSTEM
	void	SendAnarchyBuff(DWORD dwId);
	void	ReloadAnarchyBuff(CUser* pUser);
	void	OnAnarchyBuff(CAr& ar, DPID, DPID, OBJID);
#endif // __ANARCHY_SYSTEM

#ifdef __SYS_GUILDCOLOR
	void	SendSetGuildColor(DWORD guildid, int r, int g, int b);
	void	OnGuildSetColor(CAr& ar, DPID, DPID, OBJID);
...
protected:
	...
	void	OnPartyAssistantCore(CAr& ar, DPID, DPID, OBJID);
...
#if _WIN64
#ifdef __EVENT0913
	void	OnEvent0913(CAr& ar, DPID, DPID, OBJID);
#endif	// __EVENT0913
#ifdef __EVENT1206
	void	OnEvent1206(CAr& ar, DPID, DPID, OBJID);
#endif	// __EVENT1206
...
#if _WIN64
		void    OnRemoveNPCVendor(CAr& ar, DPID, DPID, OBJID);
#else
		void    OnRemoveNPCVendor(CAr& ar, DPID, DPID, DPID);
#endif
#endif // __VENDING_NPC

#ifdef __SYS_MAP_BUFFS
#if _WIN64
		void    OnCheckMapBuffs(CAr& ar, DPID, DPID, OBJID);

#else
		void    OnCheckMapBuffs(CAr& ar, DPID, DPID, DPID);
#endif
		void	SendCommandCheckMapBuffs();
