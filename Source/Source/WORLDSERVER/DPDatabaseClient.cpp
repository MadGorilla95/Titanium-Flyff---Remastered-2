...
#ifdef __VENDING_NPC
	ON_MSG(PACKETTYPE_ADDVENDORNPC, &CDPDatabaseClient::OnAddNPCVendor);
	ON_MSG(PACKETTYPE_GETVENDORNPC, &CDPDatabaseClient::OnGetNPCVendor);
#endif // __VENDING_NPC

#ifdef __AEGON_INGAME_DP
	ON_MSG(PACKETTYPE_DP_COMMAND, &CDPDatabaseClient::OnDPCommand);
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LAST_ONLINE
	ON_MSG(PACKETTYPE_LASTONLINE_INIT_LOAD, &CDPDatabaseClient::OnLastOnlineInit);
#endif // __AEGON_LAST_ONLINE

#ifdef __AEGON_PVP_RANKING
	ON_MSG(PACKETTYPE_PVPRANKING_INIT_LOAD, &CDPDatabaseClient::OnPvPRankingInit);
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_INGAME_SHOP
	ON_MSG(PACKETTYPE_INGAMESHOP_INIT_LOAD, &CDPDatabaseClient::OnIngameShopInit);
#endif //__AEGON_INGAME_SHOP

#ifdef __AEGON_TOURNAMENT
	ON_MSG(PACKETTYPE_TOURNAMENT_DB_LOAD, &CDPDatabaseClient::OnTournamentLoad);
#endif //__AEGON_TOURNAMENT

#ifdef __ROYAL_RUMBLE
	ON_MSG(PACKETTYPE_PLAYERWAR_LOAD, &CDPDatabaseClient::OnLoadSiegeRanking);
...
void CDPDatabaseClient::OnJoin( CAr & ar, DPID dpidCache, DPID dpidUser )
{
	...
	if( pUser )
	{
...
#ifdef __ROYAL_RUMBLE
			|| pWorld && pWorld->GetID() == WI_WORLD_FWC && g_War.GetState() == -1
#endif // __ROYAL_RUMBLE
			)
		{
			dwWorldId = 1;
		...
		vector<TAG_ENTRY> tags;
		//TAG_ENTRY tags[MAX_TAGS];
		for (unsigned int i = 0; i < nTagCount; ++i)
		{
			TAG_ENTRY tmpTag;
			ar >> tmpTag.idFrom;					// 누구에게서 
			ar >> tmpTag.dwDate;					// 언제 
			ar.ReadString(tmpTag.szString, 256);		// 쪽지 내용 
...
*/
...
#ifdef __GAMEGUARD	// Authorization.ini
		...
		if (s.Load("..\\ImmortalProgram\\Config.ini"))
		{
			pUser->m_nAuth = 0;
			s.GetToken();
					...
					if (s.Token == "{")
					{
						...
						while (s.Token != "}")
						{
							...
							if (s.Token == "Character" || s.Token == "character" || s.Token == "CHARACTER")
							{
								...
								{
									if (s.Token == "AUTH_HELPER")
										pUser->m_dwAuthorization = AUTH_HELPER;
									else if (s.Token == "AUTH_BALANCE")
										pUser->m_dwAuthorization = AUTH_BALANCE;
...
//		pUser->m_dwAuthorization = AUTH_ADMINISTRATOR;
...
#ifdef __SYS_SANDBOX
			&& dwWorldId != WORLD_SANDBOX
#endif //__SYS_SANDBOX
#ifdef __ROYAL_RUMBLE
			&& dwWorldId != WI_WORLD_FWC
#endif // __ROYAL_RUMBLE
			)
		{
		...
		if( !pUser->IsAuthHigher( AUTH_GAMEMASTER ) )
		{
			...
			if( pUser->m_dwMode & ( MATCHLESS_MODE | TRANSPARENT_MODE | ONEKILL_MODE | MATCHLESS2_MODE )
				...
				|| pUser->IsMode(AEGON_MOB_IGNORE_MODE)
#endif
#ifdef __GAMEGUARD
				|| pUser->m_dwMode & (EVENT_OLDBOY_MODE | QUERYSETPLAYERNAME_MODE)
#endif
				)
			{
...
#ifdef __GAMEGUARD
				pUser->SetNotMode(EVENT_OLDBOY_MODE | QUERYSETPLAYERNAME_MODE);
#endif
			}
		}
		...
		{
			if (pUser->m_dwMode & (MATCHLESS_MODE | ONEKILL_MODE | MATCHLESS2_MODE))
			{
				Error("Abnormal User!! - %s(%07d), m_dwMode:%08x", pUser->GetName(), pUser->m_idPlayer, pUser->m_dwMode);
				pUser->SetNotMode(MATCHLESS_MODE | ONEKILL_MODE | MATCHLESS2_MODE);
			}
		}

		if (!pUser->IsAuthHigher(AUTH_ADMINISTRATOR))
		{
			if (pUser->m_dwMode & (FREE_MONEY_MODE))
			{
				Error("Abnormal User!! - %s(%07d), m_dwMode:%08x", pUser->GetName(), pUser->m_idPlayer, pUser->m_dwMode);
				pUser->SetNotMode(FREE_MONEY_MODE);
			}
		}
		...
		g_DPCoreClient.ReloadAnarchyBuff(pUser);
...
#ifdef __LEESE_PARTY_REFRESH
			for (int i = 0; i < MAX_PARTYMODE; ++i)
			{
				if (pParty->m_nModeTime[i] && pParty->GetLeader())
				{
					g_DPCoreClient.SendUserPartySkill(pParty->GetLeader()->m_idPlayer, i, (DWORD)(pParty->GetPartyModeTime(i)), 0, 1);
				}
			}
#endif //__LEESE_PARTY_REFRESH
		}
		else
		{
			pUser->m_idparty	= 0;
#ifdef __PARTY_ASSISTANT
			((CMover*)pUser)->m_bAssistMember = FALSE;
			pUser->AddAssistClient(FALSE);
		}
		...
		pUser->RemoveBuff(BUFF_ITEM, II_SYS_SYS_LS_WINNERDIGNITY);
		pUser->RemoveBuff(BUFF_ITEM, II_SYS_SYS_LS_SPLENDIDGLORY);
		...
		if (pUser->m_idGuild == g_GuildCombatMng.m_uWinGuildId && g_GuildCombatMng.m_nState == CGuildCombat::CLOSE_STATE)
		{
			if (g_GuildCombatMng.m_nWinGuildCount == 1)
				pUser->AddBuff(BUFF_ITEM, II_SYS_SYS_LS_WINNERDIGNITY, 0, d * 1000, 0);
			else if (g_GuildCombatMng.m_nWinGuildCount == 2)
				pUser->AddBuff(BUFF_ITEM, II_SYS_SYS_LS_SPLENDIDGLORY, 0, d * 1000, 0);
		}
...
// 			pUser->Replace( g_uIdofMulti, WI_WORLD_MADRIGAL, D3DXVECTOR3( 6983.0f, 0.0f, 3330.0f ) );
	...
	if( pUser->IsCollecting() )
...
#ifdef __VENDING_NPC
		if (pUser->m_idVendorNPC)
		{
			CMover* pVendor = prj.GetVendorNpcByID(pUser->m_idVendorNPC);
			if (pVendor != NULL)
				pUser->AddNpcVendorLink(pVendor->m_dwVendorTime, pVendor->m_dwVendorItemID);
		}
#endif // __VENDING_NPC
		...
		CMonsterHuntMng::GetInstance()->MonsterHuntOnJoin(pUser);
#endif // __AEGON_MONSTER_HUNT

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
		CAegonAchievementMng::GetInstance()->AddAllProgUnits(pUser);
		...
		{
			CMover* tmpVendor = prj.GetVendorNpcByID(pUser->m_idVendorNPC);
			if(IsValidObj(tmpVendor))
				CAegonAchievementMng::GetInstance()->CollectOfflineVendorSellCount(tmpVendor);
		}
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_LAST_ONLINE
		CAegonLastOnlineMng::GetInstance()->OnUserLogin(pUser);
		pUser->AddLastOnlineRefresh();
#endif // __AEGON_LAST_ONLINE

#ifdef __CLIENT_ONLINE_COUNT
		pUser->UpdatePlayerCount(INT_MAX);
#endif //__CLIENT_ONLINE_COUNT

#ifdef __AEGON_SEASON_PASS
		CAegonSeasonPassMng::GetInstance()->SeasonPassOnJoin(pUser);
#endif // __AEGON_SEASON_PASS

#ifdef __SYS_MAP_BUFFS
		CMapBuffManager::GetInstance()->Handle(pUser, pUser->GetWorld()->GetID());
#endif //__SYS_MAP_BUFFS

#ifdef __AEGON_PVP_RANKING
		CAegonPvPRankingMng::GetInstance()->PvPRankingOnJoin(pUser);
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_PERMANENT_BUFF
		pUser->ActivatePermanentBuffs();
		...
		if (pUser->m_dwAuthorization >= AUTH_GAMEMASTER) // Gamemaster+
		{
			g_UserMng.AddStaffConnect(pUser);
		}
#endif //__LEESE_STAFF_CONNECT

#ifdef __AEGON_INGAME_SHOP
		CAegonIngameShopMng::GetInstance()->IngameShopOnJoin(pUser);
#endif //__AEGON_INGAME_SHOP
...
void CDPDatabaseClient::SendUpdatePlayerData(CUser* pUser, DWORD worldid)
#else //__LEESE_PARTY_LOCATION
...
#ifdef __PARTY_ASSISTANT
	data.nAssist = pUser->GetAssist();
#endif //__PARTY_ASSISTANT
#ifdef __LEESE_PARTY_LOCATION
	if (worldid == 0)
		data.nLocation = pUser->GetWorld()->GetID();
	else
		data.nLocation = worldid;
#endif //__LEESE_PARTY_LOCATION
...
#ifdef __GAMEGUARD
			pUser->AddText(prj.GetText(TID_MAIL_SEND_OK));
#else // __GAMEGUARD
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(srTender.vecLineUpMember.size());
#else
...
void CDPDatabaseClient::OnHousingLoadInfo( CAr & ar, DPID, DPID )
{
	...
	if( !pHousing )
		...
		Error( "CDPDatabaseClient::OnHousingLoadInfo - playerID: %07d", dwPlayerId );
...
void CDPDatabaseClient::SendAddNPCVendor(CUser* pUser, CMover* pVendor, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, DWORD dwItemID, DWORD dwShopDesign)
#else
void CDPDatabaseClient::SendAddNPCVendor(CUser* pUser, CMover* pVendor, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, DWORD dwItemID)
#endif
{
	BEFORESENDDUAL(ar, PACKETTYPE_ADDVENDORNPC, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pUser->m_idPlayer << pVendor->GetId() << vPos << fAngle << dwSkillTime << fMonsterSize << dwItemID << g_uIdofMulti << pUser->GetWorld()->GetID();
	...
	char szTitle[64];
	strcpy_s(szTitle, 64, pVendor->m_vtInfo.GetTitle());
	szTitle[63] = 0;
	ar.WriteString(szTitle);
	pVendor->m_Inventory.Serialize(ar);
	SEND(ar, this, DPID_SERVERPLAYER);
	SendAddNPCVendorLog(pUser, pVendor, vPos, fAngle, dwSkillTime, fMonsterSize, dwItemID);
}

void CDPDatabaseClient::SendRemoveNPCVendor(CMover* pVendor)
{
	BEFORESENDDUAL(ar, PACKETTYPE_REMOVEVENDORNPC, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pVendor->m_idVendorNPC;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPDatabaseClient::SendAddNPCVendorLog(CUser* pUser, CMover* pVendor, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, DWORD dwItemID)
{
	BEFORESENDDUAL(ar, PACKETTYPE_ADDVENDORNPCLOG, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pUser->m_idPlayer << pVendor->GetId() << vPos << fAngle << dwSkillTime << fMonsterSize << dwItemID << g_uIdofMulti << pUser->GetWorld()->GetID();
...
void CDPDatabaseClient::SendRemoveNPCVendorLog(CMover* pVendor)
{
	BEFORESENDDUAL(ar, PACKETTYPE_REMOVEVENDORNPCLOG, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pVendor->m_idVendorNPC;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPDatabaseClient::SendUpdateNPCVendor(CMover* pVendor)
{
	BEFORESENDDUAL(ar, PACKETTYPE_UPDATEVENDORNPC, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pVendor->m_idVendorNPC << pVendor->m_dwVendorTime;

#ifdef __AEGON_IMPROVED_SHOPDESIGN
	ar << pVendor->m_dwShopDesign;
#endif
...
void CDPDatabaseClient::SendGetNPCVendor()
{
	BEFORESENDDUAL(ar, PACKETTYPE_GETVENDORNPC, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << g_uIdofMulti;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPDatabaseClient::OnAddNPCVendor(CAr& ar, DPID, DPID)
{
	u_long  idPlayer;
	OBJID   objidVendor;
	u_long  idVendor;

	ar >> idPlayer >> objidVendor >> idVendor;

	CMover* pMover = prj.GetMover(objidVendor);
	if (IsValidObj(pMover))
	{
		pMover->m_idVendorNPC = idVendor;
		CUser* pUser = g_UserMng.GetUserByPlayerID(idPlayer);
		if (IsValidObj(pUser))
		{
			pUser->m_idVendorNPC = idVendor;
			g_dpDBClient.SavePlayer(pUser, pUser->GetWorld()->GetID(), pUser->GetPos(), pUser->GetLayer());
		}
		prj.AddVendorNpcToGlobal(idVendor, pMover);
	}
}

void CDPDatabaseClient::OnGetNPCVendor(CAr& ar, DPID, DPID)
{
	int nCount;
	ar >> nCount;
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		ar >> idVendor >> idPlayer >> vPos >> fAngle >> dwSkillTime >> fMonsterSize >> dwItemID >> idChannel >> idWorld;
		...
		MoverProp* pMoverProp = prj.GetMoverProp(pItemProp->dwAbilityMin);
		if (pMoverProp == NULL)
			return;

		char szName[MAX_NAME];
		//sprintf(szName, "[%s's Vendor] %s", szPlayerName, pMoverProp->szName);
		sprintf(szName, "[%s's Vendor]", szPlayerName);

		CMover* pMover = (CMover*)CreateObj(D3DDEVICE, OT_MOVER, pMoverProp->dwID, FALSE);
		if (NULL != pMover)
		{
			pMover->m_dwVendorItemID = dwItemID;
			pMover->m_bNPCVendor = TRUE;
			pMover->InitProp(FALSE);

#ifdef __AEGON_IMPROVED_SHOPDESIGN
			pMover->m_dwShopDesign = dwShopDesign;
#endif
			...
			pMover->m_dwVendorTime = dwSkillTime;
			pMover->m_vtInfo.SetTitle(szShopName);
			pMover->m_vtInfo.SetOwnerId(idPlayer);

			pMover->m_Inventory.SetItemContainer(ITYPE_ITEM, MAX_VENDITEM);
			pMover->m_Inventory.Serialize(ar);

			//pMover->m_Inventory.Copy(tmpInventory);
			for (int i = 0; i < MAX_VENDITEM; i++)
			{
				CItemElem* pItemElem = pMover->m_Inventory.GetAt(i);
				if (pItemElem == NULL)
					continue;

#ifdef __LEEAEG_SHOP_FINDER
				CAegonShopFinderMng::GetInstance()->AddItem(pItemElem, pMover->GetName(), vPos, idWorld, CContinent::GetInstance()->GetContinentName(CContinent::GetInstance()->GetContinent(vPos)));
#endif // __LEEAEG_SHOP_FINDER

#ifdef __CHIP_PLAYERSHOP
				pMover->m_vtInfo.VendorSetItem((BYTE)i, (BYTE)i, pItemElem->m_nItemNum, pItemElem->m_nCost, pItemElem->m_nBuyMode);
#else //__CHIP_PLAYERSHOP
				pMover->m_vtInfo.VendorSetItem((BYTE)i, (BYTE)i, pItemElem->m_nItemNum, pItemElem->m_nCost);
#endif //__CHIP_PLAYERSHOP
			}

			pMover->m_idVendorNPC = idVendor;
			CWorld* pWorld = g_WorldMng.GetWorld(idWorld);
			pWorld->AddObj(pMover, TRUE, 0);
			prj.AddVendorNpcToGlobal(idVendor, pMover);
		}
	}
}

void CDPDatabaseClient::SendShopCloseLog(u_long idPlayer, CItemBase* apItem_VT[MAX_VENDITEM])
{
	BEFORESENDDUAL(ar, PACKETTYPE_SHOPCLOSELOG, DPID_UNKNOWN, DPID_UNKNOWN);
	int nCount = 0;
	for (int i = 0; i < MAX_VENDITEM; i++)
	{
		if (apItem_VT[i])
	...
	for (int i = 0; i < MAX_VENDITEM; i++)
	{
		if (apItem_VT[i])
		{
			apItem_VT[i]->Serialize(ar);
			ar << apItem_VT[i]->GetExtra();
		}
	}
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __AEGON_INGAME_DP
void CDPDatabaseClient::SendDPCommand(CUser* pUser, __DP_COMMAND_INFO info)
{
	if (IsInvalidObj(pUser) ||  pUser->m_bDPCommandInProgress)
		return;

	pUser->m_bDPCommandInProgress = TRUE;

	if (info.dpCost <= 0) 
	{
		Error("%s - User: %s - playerID: %d - dpCost: %d", __FUNCTION__, pUser->GetName(), pUser->m_idPlayer, info.dpCost);
		return;
	}


	BEFORESENDDUAL(ar, PACKETTYPE_DP_COMMAND, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << pUser->m_idPlayer;
	ar << info;
	ar.WriteString(pUser->m_playAccount.lpszAccount);
	SEND(ar, this, DPID_SERVERPLAYER);
...
#define LOG_DP_PURCHASE(a, b, c) { LogSuccessfullDPPurchase(b->IsAuthHigher(AUTH_HELPER), "DP PURCHASE: '%s' - User: '%s' - Info: command: '%d' dpCost: '%d' newDPBalance: '%d', dwAmount: '%d', nDPCommandVar1: '%d'", a, b->GetName(), c.command, c.dpCost, c.newDPBalance, c.dwAmount, c.nDPCommandVar1); }
void CDPDatabaseClient::OnDPCommand(CAr& ar, DPID, DPID)
{
	CString strOut;
	DWORD dwOutColor;
	...
	if (IsValidObj(pUser) && info.dwAmount > 0)
	{

		switch (info.command)
		{
		case INGAMEDP_CASH_ERROR:
		{
			dwOutColor = COLOR_ERROR;
			strOut.Format("Not enough DP. %d DP needed.", info.dpCost * info.dwAmount);
			break;
		}
		case INGAMEDP_BUYLOOTBOX:
		{
			dwOutColor = COLOR_SUCCESS;
			strOut.Format("Bought Lootbox for %d DP.", info.dpCost);

			if (info.lootboxResult.validResult && info.lootboxResult.receiveItem > 0 && info.lootboxResult.receiveNum > 0)
			{
				pUser->m_bLootBoxInProgress = TRUE;
				pUser->m_dwLootboxItem = info.lootboxResult.receiveItem;
				pUser->m_dwLootboxItemNum = info.lootboxResult.receiveNum;

				pUser->AddLootboxResult(info.lootboxResult);

#ifndef __LOCALSERVER
				LOG_DP_PURCHASE("LOOTBOX", pUser, info);
#endif
			}
			else
			{
				Error("OnDPCommand BUYLOOTBOX - User: %s Command: %d dpCost: %d newBalance: %d dwAmount: %d", pUser->GetName(), info.command, info.dpCost, info.newDPBalance, info.dwAmount);
			}
		...
		case INGAMEDP_BUYSEASONPASS:
		{
			...
			if (pUser->m_SeasonpassProgress.UnlockSeasonPassPremium(info.nDPCommandVar1)) 
			{
				dwOutColor = COLOR_SUCCESS;
				strOut.Format("Season pass successfully unlocked for %d DP.", info.dpCost);

				__SEASONPASS_PROGRESS_UNIT* progUnit = pUser->m_SeasonpassProgress.GetProgressUnit(info.nDPCommandVar1);
				if(progUnit)
					pUser->AddSeasonPassUpdateProgress(progUnit);

#ifndef __LOCALSERVER
				LOG_DP_PURCHASE("SEASONPASS", pUser, info);
#endif
			}
			else 
			{
				Error("OnDPCommand BUYSEASONPASS - User: %s Command: %d dpCost: %d newBalance: %d nDPCommandVar1: %d dwAmount: %d", pUser->GetName(), info.command, info.dpCost, info.newDPBalance, info.nDPCommandVar1, info.dwAmount);
				dwOutColor = COLOR_ERROR;
				strOut.Format("There was an error with the purchase, please contact a gm.");
			}
		...
		{
			MotionProp* pMotionProp = prj.GetMotionProp(info.nDPCommandVar1);
			if (pMotionProp && pUser->DoUnlockMotion(pMotionProp->dwID)) 
			{
				dwOutColor = COLOR_SUCCESS;
				strOut.Format("Bought Motion for %d DP.", info.dpCost);
				pUser->AddUnlockedMotionsRefresh();

#ifndef __LOCALSERVER
				LOG_DP_PURCHASE("MOTION", pUser, info);
#endif
			}
			else 
			{
				Error("OnDPCommand BUYMOTION - User: %s Command: %d dpCost: %d newBalance: %d nDPCommandVar1: %d dwAmount: %d", pUser->GetName(), info.command, info.dpCost, info.newDPBalance, info.nDPCommandVar1, info.dwAmount);
				dwOutColor = COLOR_ERROR;
				strOut.Format("There was an error with the purchase, please contact a gm.");
			}
		...
		{
			__AEGON_EMOTE_UNIT* pEmoteUnit = CAegonEmoticonMng::GetInstance()->GetUnitByID(info.nDPCommandVar1, 1);
			if (pEmoteUnit && pUser->DoUnlockEmote(pEmoteUnit->nEmoteID))
			{
				dwOutColor = COLOR_SUCCESS;
				strOut.Format("Bought Emote for %d DP.", info.dpCost);
				pUser->AddUnlockedEmotesRefresh();

#ifndef __LOCALSERVER
				LOG_DP_PURCHASE("EMOTE", pUser, info);
#endif
			}
			else
			{
				Error("OnDPCommand BUYEMOTE - User: %s Command: %d dpCost: %d newBalance: %d nDPCommandVar1: %d dwAmount: %d", pUser->GetName(), info.command, info.dpCost, info.newDPBalance, info.nDPCommandVar1, info.dwAmount);
				dwOutColor = COLOR_ERROR;
				strOut.Format("There was an error with the purchase, please contact a gm.");
			}
		...
		{
			__AEGIGNSHP_ITEM_UNIT* pShopUnit = CAegonIngameShopMng::GetInstance()->GetShopUnitByID(info.nDPCommandVar1);
			int nSuccess = 0;

			for (int i = 0; i < info.dwAmount; i++) 
			{
				if (pShopUnit && pUser->GiveWebshopItem(pShopUnit))
				{
					nSuccess++;
				}
			}
			if (info.dwAmount == nSuccess)
			{
				dwOutColor = COLOR_SUCCESS;
				strOut.Format("Webshop purchase completed for %d DP.", info.dpCost * info.dwAmount);

#ifndef __LOCALSERVER
				LOG_DP_PURCHASE("INGAME_WEBSHOP", pUser, info);
#endif
			}
			else
			{
				Error("OnDPCommand WEBSHOP - User: %s Command: %d dpCost: %d newBalance: %d nDPCommandVar1: %d dwAmount: %d - nSuccess: %d", pUser->GetName(), info.command, info.dpCost, info.newDPBalance, info.nDPCommandVar1, info.dwAmount, nSuccess);
				dwOutColor = COLOR_ERROR;
				strOut.Format("There was an error with the purchase, please contact a gm.");
			}
			break;
		...
		default:
		{
			dwOutColor = COLOR_ERROR;
			strOut.Format("There was an error with the purchase, please contact a gm.");
			Error("OnDPCommand Switch - User: %s Command: %d dpCost: %d newBalance: %d dwAmount: %d", pUser->GetName(), info.command, info.dpCost, info.newDPBalance, info.dwAmount);
			break;
		}
		}

		pUser->SetIngameDP(info.newDPBalance);
		pUser->AddDPRefresh();
		pUser->AddText(strOut.GetString(), dwOutColor);

		pUser->m_bDPCommandInProgress = FALSE;
	...
	{
		Error("OnDPCommand - UserID: %d Command: %d dpCost: %d newBalance: %d nDPCommandVar1: %d dwAmount: %d", playerID, info.command, info.dpCost, info.newDPBalance, info.nDPCommandVar1, info.dwAmount);
	}
...
#ifdef __AEGON_LAST_ONLINE
void CDPDatabaseClient::SendInitLoadLastLogin()
{
	BEFORESENDDUAL(ar, PACKETTYPE_LASTONLINE_INIT_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_SYSMSG);
}
void CDPDatabaseClient::OnLastOnlineInit(CAr& ar, DPID, DPID)
{
...
#ifdef __BAN_SYSTEM
void CDPDatabaseClient::SendBan(LPCTSTR szAccount, int nDay)
{
	BEFORESENDDUAL(ar, PACKETTYPE_BAN_ACCOUNT, DPID_UNKNOWN, DPID_UNKNOWN);
	ar.WriteString(szAccount);
	ar << nDay;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
void CDPDatabaseClient::SendUpdateAchievementOffline(u_long playerID, int achievementID, int additionalProg)
{
	BEFORESENDDUAL(ar, PACKETTYPE_UPDATEACHIEVEMENTOFFLINE, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << playerID;
	ar << achievementID;
	ar << additionalProg;
	SEND(ar, this, DPID_SYSMSG);
...
#ifdef __AEGON_PVP_RANKING
void CDPDatabaseClient::SendInitLoadPvPRanking()
{
	BEFORESENDDUAL(ar, PACKETTYPE_PVPRANKING_INIT_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_SYSMSG);
}
void CDPDatabaseClient::OnPvPRankingInit(CAr& ar, DPID, DPID)
{
	
	CAegonPvPRankingMng::GetInstance()->ReadMap(ar);
	CAegonPvPRankingMng::GetInstance()->CalculatePointsForAll();

}
void CDPDatabaseClient::SendSavePvPRanking(__PVPRANKING_PROGRESS_UNIT* unit)
{
	if (unit) 
	{
		BEFORESENDDUAL(ar, PACKETTYPE_PVPRANKING_SAVE, DPID_UNKNOWN, DPID_UNKNOWN);
		unit->Write(ar);
		SEND(ar, this, DPID_SYSMSG);
	}
}
...
#ifdef __AEGON_INGAME_SHOP
void CDPDatabaseClient::SendInitLoadIngameShop()
{
	BEFORESENDDUAL(ar, PACKETTYPE_INGAMESHOP_INIT_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_SYSMSG);
}
void CDPDatabaseClient::OnIngameShopInit(CAr& ar, DPID, DPID)
{
	CAegonIngameShopMng::GetInstance()->ReadCategoryMap(ar);
	CAegonIngameShopMng::GetInstance()->ReadItemMap(ar);
	CAegonIngameShopMng::GetInstance()->OnShopUpdate();

}
#endif //__AEGON_INGAME_SHOP

#ifdef __AEGON_TOURNAMENT
void CDPDatabaseClient::SendLoadTournaments()
{
	BEFORESENDDUAL(ar, PACKETTYPE_TOURNAMENT_DB_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_SYSMSG);
}
void CDPDatabaseClient::OnTournamentLoad(CAr& ar, DPID, DPID)
{
	CAegonTournamentMng::GetInstance()->ReadTournamentMap(ar);
	CAegonTournamentMng::GetInstance()->OnTournamentLoad();
}
void CDPDatabaseClient::SendSaveTournaments()
{
	BEFORESENDDUAL(ar, PACKETTYPE_TOURNAMENT_DB_SAVE, DPID_UNKNOWN, DPID_UNKNOWN);
	CAegonTournamentMng::GetInstance()->WriteTournamentMap(ar);
	SEND(ar, this, DPID_SYSMSG);
}
#endif //__AEGON_TOURNAMENT

#ifdef __ROYAL_RUMBLE
void CDPDatabaseClient::SaveWarPlayers()
{
	BEFORESENDDUAL(ar, PACKETTYPE_PLAYERWAR_SAVE, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << g_War.m_vecWarPlayers.size();
	for (u_int i = 0; i < g_War.m_vecWarPlayers.size(); i++)
	{
		CUser* pUser = g_UserMng.GetUserByPlayerID(g_War.m_vecWarPlayers[i].m_idPlayer);
		if (IsValidObj(pUser))
		{
			ar.WriteString(pUser->GetName());
			ar << pUser->m_idPlayer;
			ar << g_War.m_vecWarPlayers[i].m_nJob;

			// Check if exists then add core
			int nkills = 0, ndeaths = 0, npoints = 0;
			for (auto wp : g_War.m_vecRankSiege)
			{
				if (wp.uidPlayer == pUser->m_idPlayer)
				{
					nkills = wp.nKills;
					ndeaths = wp.nDeaths;
					npoints = wp.nPoints;
			...
			ar << (g_War.m_vecWarPlayers[i].m_nKills + nkills);
			ar << (g_War.m_vecWarPlayers[i].m_nDeaths + ndeaths);
			ar << (g_War.m_vecWarPlayers[i].m_nPoints + npoints);
		}
	}
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPDatabaseClient::LoadSiegeRanking()
{
	//g_War.m_vecRankSiege.clear();
	BEFORESENDDUAL(ar, PACKETTYPE_PLAYERWAR_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPDatabaseClient::OnLoadSiegeRanking(CAr& ar, DPID, DPID)
{
	u_int nCount;
	ar >> nCount;
	g_War.m_vecRankSiege.clear();
