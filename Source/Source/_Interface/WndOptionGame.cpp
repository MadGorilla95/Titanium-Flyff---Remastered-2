...
void CWndOptionGame::OnDraw( C2DRender* p2DRender ) 
{
	...
	pt = pt1;
	pt.x += GetStepPos( m_nStep[0], lpWndCtrl->rect.right - lpWndCtrl->rect.left, MAX_SLIDER).x;
	...
	m_Texture.Render( p2DRender, pt1, pt2 );
	pt = pt1;
	pt.x += GetStepPos( m_nStep[1], lpWndCtrl->rect.right - lpWndCtrl->rect.left, MAX_SLIDER).x;
#ifdef __AEGON_QOL_STUFF
	pt.y += 1;
#endif
	m_TexturePt.Render( p2DRender, pt, CPoint( 16, 16 ) );
...
{ 
	...
	CWndButton* pWndMuteSoundsInBG = (CWndButton*)GetDlgItem(WIDC_CHECK_MUTE_SOUND);
	if (pWndMuteSoundsInBG)
	{
		pWndMuteSoundsInBG->SetToolTip(_T("Mute the sound when you minimize your client."));
		pWndMuteSoundsInBG->SetCheck(g_Option.muteSoundsInBG);
	}

	CWndButton* pWndMuteMusicInBG = (CWndButton*)GetDlgItem(WIDC_CHECK_MUTE_MUSIC);
	if (pWndMuteMusicInBG)
	{
		pWndMuteMusicInBG->SetToolTip(_T("Mute the music when you minimize your client."));
		pWndMuteMusicInBG->SetCheck(g_Option.muteMusicInBG);
	}
	...
	if (pWndButton[0])
	{
			pWndButton[0]->SetCheck(!g_Option.m_bBlockGuildInvites);
	}
	if (pWndButton[1])
	{
		pWndButton[1]->SetCheck(g_Option.m_bBlockGuildInvites);
	}
...
{ 
	...
	if( nResult == 1 )
	{
		g_Option.m_fEffectVolume	= m_nStep[0]*0.05f;
#ifdef __fmod
	...
	if( nResult == 1 )
	{
		g_Option.m_fBGMVolume = m_nStep[1]*0.05f;
#ifdef __fmod
...
#ifdef __BLOCK_GUILD_INVITES
	case WIDC_RADIO9:
		g_Option.m_bBlockGuildInvites = FALSE;
		break;
	case WIDC_RADIO10:
		g_Option.m_bBlockGuildInvites = TRUE;
		break;
#endif //__BLOCK_GUILD_INVITES
...
#ifdef __AEGON_THEME_SWITCHER
void CWndOptionGame::OnThemeSelectionChanged()
{
