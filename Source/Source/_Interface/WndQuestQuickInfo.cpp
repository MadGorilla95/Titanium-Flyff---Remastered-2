...
void CWndQITreeCtrl::PaintTree( C2DRender* p2DRender, CPoint& pt, CPtrArray& ptrArray )
{
	...
	for(int i = 0; i < ptrArray.GetSize(); ++i )
	{
		...
		if (GetMemberCheckingMode() == TRUE && pTreeElem->m_pWndCheckBox)
		{
			...
			if (pWndCheckBox && pWndCheckBox->m_pTexture && pTreeElem->m_ptrArray.GetSize() == 0)
			{
...
#ifdef __AEGON_THEME_SWITCHER
		p2DRender->TextOut(10, 4 + m_nTitleDrawOffsetY, m_strTitle, ((m_dwColor & 0x00ffffff) | (m_nAlphaCount << 24)));
#else
