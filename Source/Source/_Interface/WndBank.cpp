...
{ 
	...
	m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_THEME, "WndCommonBankNotUse.tga" ), COLOR_ITEM_BG );
} 
...
BOOL CWndBank::OnChildNotify( UINT message, UINT nID, LRESULT* pLResult ) 
{
	...
	if (message == WNM_ITEMCLICK)
	{
		...
		BYTE slot = nID - 11;
		g_DPlay.SendGetItemBank(slot, (BYTE)pItem->m_dwObjId, pItem->m_nItemNum);
	}
	else
			...
			if( strlen( szLastPass ) == 4 && strlen( szNewPass ) == 4 && strlen( szConfirmPass ) == 4)
			{
				if( 0 == strcmp( szNewPass, szConfirmPass ) )
				{
					...
					if( bDigit )
					{
						...
						if (nNewPass <= 0 || nConfirmPass <= 0)
						{
							g_WndMng.CloseMessageBox();
							g_WndMng.OpenMessageBox(prj.GetText(TID_2ND_PASSWORD_INPUT_ERROR01));
							return FALSE;
						}
						/*short nWeakPassword[] = { 1111, 2222, 3333, 4444, 5555, 6666, 7777, 8888, 9999, 1234, 147, 258, 369, 7410, 8520, 9630 };
						for (int i = 0; i < sizeof(nWeakPassword); i++)
						{
							if (nNewPass == nWeakPassword[i] || nConfirmPass == nWeakPassword[i])
							{
								g_WndMng.CloseMessageBox();
								g_WndMng.OpenMessageBox("You've entered a weak password!");
								return FALSE;
							}
						}*/
#endif // __GAMEGUARD
