...
#ifdef __MODEL_NPC_VIEW
	if (pMover->GetModel() == NULL && m_pModel != NULL)
	{
		SAFE_DELETE(m_pModel);
	}
	else if (pMover->GetModel() != NULL && m_pModel == NULL)
	{
		m_pModel = new CModelObject;
		m_pModel->InitDeviceObjects(g_Neuz.m_pd3dDevice);

		CString textFile;
		textFile.Format("Mvr_%s.chr", pMover->GetModel()->m_pModelElem->m_szName);
		m_pModel->LoadBone(textFile);
		textFile.Format("Mvr_%s.o3d", pMover->GetModel()->m_pModelElem->m_szName);
		m_pModel->LoadElement(textFile, 0);

		m_pModel->m_pModelElem = pMover->GetModel()->m_pModelElem;

		textFile.Format("Mvr_%s_stand.ani", pMover->GetModel()->m_pModelElem->m_szName);

		m_pModel->LoadMotion(textFile);
	}
...
{ 
	...
	else if (strcmp(m_aKeyButton[m_nSelectKeyButton].szWord, prj.GetText(TID_GAME_NEW_QUEST)) == 0 && m_nNewQuestListNumber == 0) 
	{
		...
		p2DRender->TextOut(pt.x + 10, pt.y + 10, prj.GetText(TID_GAME_EMPTY_NEW_QUEST), CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		p2DRender->TextOut(pt.x + 10, pt.y + 10, prj.GetText(TID_GAME_EMPTY_CURRENT_QUEST), CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
	...
	if (m_pModel != NULL)
	{
		CObject3D* pObj3D = m_pModel->GetObject3D();
		if (!pObj3D)
			return;

		vecPos = pObj3D->m_vEvent[0];

		float fModelScale = m_pModel->m_pModelElem->m_fScale;
		...
		float m_fRadius = m_pModel->GetRadius();
...
#ifdef __AEGON_FIXES
		if (m_pModel->m_Element[0].m_pObject3D) 
#endif // __AEGON_FIXES
		{
...
#ifdef __MODEL_NPC_VIEW
HRESULT CWndDialog::RestoreDeviceObjects()
{
	CWndBase::RestoreDeviceObjects();
	return S_OK;
}
HRESULT CWndDialog::InvalidateDeviceObjects()
{
	CWndBase::InvalidateDeviceObjects();
	SAFE_DELETE(m_pModel);
	return S_OK;
}
HRESULT CWndDialog::DeleteDeviceObjects()
{
...
void CWndDialog::OnMouseWndSurface( CPoint point )
{
		...
		if( pKeyButton->rect.PtInRect( point ) )
		{
			...
			if( string.IsEmpty() == FALSE )
			{
				...
				switch( nLang )
				{
					...
					default:
#ifdef _WIN64
						g_toolTip.PutToolTip(reinterpret_cast<ULONG_PTR>(pKeyButton), string, &rect, point2);
#else
	...
	for( int i = 0; i < m_nContextButtonNum; i++ )
	{
		...
		{
#ifdef __NEWCUR
			g_Neuz.SetMouseCursor(CUR_HELP);
#else //__NEWCUR
...
{ 
	...
	static const int QUEST_LIST_SELECT_COLOR = COLOR_ERROR;
	...
	if( lpCharacter )
	{
		...
		m_texChar.LoadTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_CHAR ,lpCharacter->m_szChar ), COLOR_ITEM_BG, TRUE );
		if( lpCharacter->m_dwMusicId )
#ifdef __fmod
			SoundSystemClass::playMusic(lpCharacter->m_dwMusicId, false);
#else
...
void CWndDialog::ParsingString( LPCTSTR lpszString )
{
...
#ifdef __AEGON_THEME_SWITCHER
	editString.AddParsingString(lpszString, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
	...
	// ¿ÞÂÊÀÌ strWord, ¿À¸¥ÂÊÀÌ strOrigital
		...
		else
		{
			...
			else
			{
				...
				if( IsDBCSLeadByte( editString[ i ] ) )
				{
					m_aContextMark[ nPos ].Add( nMarkTemp );
#ifdef __AEGON_THEME_SWITCHER
					pEditString->AddString(editString[++i], CAegonThemeSwitcherMng::GetInstance()->GetTextColor(), dwStyle);
#else // __AEGON_THEME_SWITCHER
...
void CWndDialog::MakeAnswerButton()
{
	...
	if( m_nCurArray == m_strArray.GetSize() - 1 )
	{
		...
		for( int i = 0; i < m_nWordButtonNum; i++ )
		{
			WORDBUTTON* pWordButton = &m_aWordButton[ i ];
#if __VER >= 15 // __IMPROVE_QUEST_INTERFACE
			rect = CRect( x, lpWndCtrl->rect.bottom - 20, x + 70, lpWndCtrl->rect.bottom );
#else // __IMPROVE_QUEST_INTERFACE
...
#ifdef __AEGON_THEME_SWITCHER
				CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture(m_apWndAnswer[j], D3DDEVICE, strTexture, TRUE);
#else
