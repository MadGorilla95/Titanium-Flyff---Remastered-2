...
void CWndQuestDetail::PaintFrame( C2DRender* p2DRender )
{
	...
	if( m_pTexture )
	{
		...
		if( IsWndStyle( WBS_CAPTION ) )
		{
			...
			p2DRender->TextOut(10, 4 + m_nTitleDrawOffsetY, m_strTitle, m_dwColor);
#else
...
void CWndQuestDetail::UpdateQuestDetailText( DWORD dwQuestID, LPQUEST lpQuest, BOOL bComplete, BOOL bClick )
{
	...
	if( bComplete )
	{
		for( int i = QS_END; i >= 0; --i )
		{
			...
			{
#ifdef __AEGON_THEME_SWITCHER
				pTextDesc->SetString(pQuestState->m_szDesc, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
...
#ifdef __AEGON_FIXES
	int nSize = 0;
	CRect rectEditStr = pWndQuestConditionTree->GetWndRect();
	LPTREEELEM rootElem = pWndQuestConditionTree->GetRootElem();
	for (int i = 0; i < rootElem->m_ptrArray.GetSize(); i++)
	{
		LPTREEELEM lpTreeElem = (LPTREEELEM)rootElem->m_ptrArray.GetAt(i);
		CSize sizeTmp;
		GetFont()->GetTextExtent(lpTreeElem->m_strKeyword, &sizeTmp);
		if (sizeTmp.cx > nSize)
			nSize = sizeTmp.cx;
	}

	CRect rectNew = pWndQuestConditionTree->GetWndRect();
	int nRectWidth = rectNew.Width() - pWndQuestConditionTree->m_wndScrollBar.GetWndRect().Width() - 5;
	if (nSize != 0 && nRectWidth < nSize)
	{
		int nDiff = nSize - nRectWidth;
		nDiff += 15;
		...
		rectNew.right += nDiff;
		pWndQuestConditionTree->SetWndRect(rectNew);

		CRect rectNewDesc = pTextDesc->GetWndRect();
		rectNewDesc.right += nDiff;
		pTextDesc->SetWndRect(rectNewDesc);

		CRect rectNewReward = pTextReward->GetWndRect();
		rectNewReward.right += nDiff;
		pTextReward->SetWndRect(rectNewReward);

		CRect rectNewMain = GetWndRect();
		rectNewMain.right += nDiff;
		SetWndRect(rectNewMain);
	}
#endif
