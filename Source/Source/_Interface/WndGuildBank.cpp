...
{ 
	if( g_pPlayer && g_pPlayer->GetGuild() )
	{
		...
		CWndEdit* pSearchBox2 = (CWndEdit*)GetDlgItem(WIDC_EDIT2);
		pSearchBox2->SetPlaceholderString("Search...");
...
#ifdef __AEGON_FIXES
		CWndBase* pWndTmp = lpShortcut->m_pFromWnd->GetWndBase();

		if (pWndTmp == NULL)
			return FALSE;

		CWndBase* pWndFrame = pWndTmp->GetFrameWnd();
#else // __AEGON_FIXES
...
#ifdef __AEGON_FIXES
						g_WndMng.m_pWndGuildBankLog->Initialize(NULL);
#else
...
#ifdef __AEGON_THEME_SWITCHER
		p2DRender->TextOut(4, pt.y, *iter, dwColorText);
#else
...
#ifdef __AEGON_THEME_SWITCHER
		p2DRender->TextOut(10, pt.y, *iter, dwColorText);
#else
