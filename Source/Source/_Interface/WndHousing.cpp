...
{ 
	...
	m_texUp.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "LvUp.bmp" ), COLOR_ITEM_BG );
	m_texDown.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "LvDn.bmp" ), COLOR_ITEM_BG );
...
{ 
	if( !m_cWndItems.empty() )
	{
		...
		//		p2DRender->TextOut( pCustom->rect.left + 55, pCustom->rect.top + 20 + ( nIndex )*nListFontHeight, "1Lv", 0xffcc11cc );
			...
			CTexture* pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pProp->szIcon), COLOR_ITEM_BG );
			if( pTexture )
...
//		p2DRender->TextOut( rect.left + 2, rect.bottom - 2, str, COLOR_ERROR );
