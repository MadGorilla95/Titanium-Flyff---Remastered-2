...
void CCaption::Render( CPoint ptBegin, C2DRender* p2DRender )
{
	...
	for( int i = 0; i < nCount; i++ )
	{
		...
		if (lpCaption->m_pAegonFont) 
		{
			fXScale = 1.0f;
			fYScale = 1.0f;
			point.x -= lpCaption->m_pAegonFont->GetTextExtent(lpCaption->m_szCaption).cx / 2;
		}
		else 
		{
...
//		CWndWorld* pWndWorld = (CWndWorld*)g_WndMng.GetWndBase( APP_WORLD );
		...
		{
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
			if (lpCaption->m_bgTexture)
			{
				p2DRender->RenderTexture(CPoint((rect.Width() / 2) - (lpCaption->m_bgTexture->m_size.cx / 2), point.y), lpCaption->m_bgTexture, (int)(lpCaption->m_fXScale * 255) - (255 - lpCaption->m_nAlpha));
				point.y += lpCaption->m_bgTexture->m_size.cy / 2;
			}
#endif // __AEGON_ADVANCED_ANNOUNCEMENT
			...
			{
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
				if (!lpCaption->m_texture.m_pTexture && !lpCaption->m_pFont && !lpCaption->m_pAegonFont)
				{
#else // __AEGON_ADVANCED_ANNOUNCEMENT
...
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
				else if (lpCaption->m_pAegonFont)
				{
					lpCaption->m_pAegonFont->DrawText((FLOAT)(point.x), (FLOAT)(point.y), fXScale, fYScale, D3DCOLOR_ARGB((int)(lpCaption->m_fXScale * 255) - (255 - lpCaption->m_nAlpha), 250, 250, 255), lpCaption->m_szCaption);
				}
#endif // __AEGON_ADVANCED_ANNOUNCEMENT
				else
				{
...
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
		if(lpCaption->m_bgTexture != NULL)
			ptBegin.y += lpCaption->m_bgTexture->m_size.cy > size.cy ? lpCaption->m_bgTexture->m_size.cy : size.cy;
		else
			ptBegin.y += size.cy;
#else // __AEGON_ADVANCED_ANNOUNCEMENT
		ptBegin.y += size.cy;
...
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
void CCaption::AddCaption(LPCTSTR lpszCaption, CD3DFontAPI* pFont, BOOL bChatLog, DWORD dwColor, int nImage, int nFont)
#else // __AEGON_ADVANCED_ANNOUNCEMENT
...
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
	lpCaption->m_bgTexture = g_Caption1.GetSystemImage(nImage);
	lpCaption->m_pAegonFont = GetSystemFont(nFont);
...
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
CTexture* CCaption::GetSystemImage(int nImage)
{
	CTexture* textureOut = NULL;
	switch (nImage) 
	{
	case 0:
		return NULL;
	case 1:
		textureOut = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME_ANNOUNCEMENT, "zone_event.png"), COLOR_BLACK, TRUE);
		break;
	case 2:
		textureOut = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME_ANNOUNCEMENT, "winner_message.png"), COLOR_BLACK, TRUE);
		break;
	case 3:
		textureOut = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME_ANNOUNCEMENT, "world_event.png"), COLOR_BLACK, TRUE);
		break;
	case 4:
		textureOut = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME_ANNOUNCEMENT, "battlefield.png"), COLOR_BLACK, TRUE);
		break;
	case 5:
		textureOut = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME_ANNOUNCEMENT, "discovery.png"), COLOR_BLACK, TRUE);
		break;
	case 6:
		textureOut = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME_ANNOUNCEMENT, "levelup.png"), COLOR_BLACK, TRUE);
		break;
	default:
		break;
	}
...
CD3DFont* CCaption::GetSystemFont(int nFont)
{
	CD3DFont* fontOut = NULL;

	switch (nFont)
	{
	case 0:
		fontOut = CWndBase::m_Theme.m_pFontCaption;
		break;
	case 1:
		fontOut = CWndBase::m_Theme.m_pFontZoneText;
		break;
	case 2:
		fontOut = CWndBase::m_Theme.m_pFontZoneTextSub;
		break;
	case 3:
		fontOut = CWndBase::m_Theme.m_pFontAegonCaption1;
		break;
#ifdef __AEGON_TOURNAMENT
	case 4:
		fontOut = CWndBase::m_Theme.m_pFontAegonTournamentBracket;
		break;
...
void CCapTime::RenderClashTime(CPoint ptBegin, C2DRender* p2DRender)
{
	CEditString strTemp;
	CString strTime;

	CTimeSpan ct = CWndMonsterClashRank::GetInstance()->GetTime() - CTime::GetCurrentTime().GetTime();
	if (CWndMonsterClashRank::GetInstance()->GetState() == STATE_WAIT)
	{
		if (ct.GetHours() > 0)
			strTime.Format("Monster Clash - Wait: %02d:%02d:%02d", ct.GetHours(), ct.GetMinutes(), ct.GetSeconds());
		else if (ct.GetMinutes() > 0)
			strTime.Format("Monster Clash - Wait: %02d:%02d", ct.GetMinutes(), ct.GetSeconds());
		else
			strTime.Format("Monster Clash - Wait: 00:%02d", ct.GetSeconds() < 0 ? 0 : ct.GetSeconds());
	}
	else if (CWndMonsterClashRank::GetInstance()->GetState() == STATE_CLASH)
	{
		if (ct.GetHours() > 0)
			strTime.Format("Monster Clash: %02d:%02d:%02d", ct.GetHours(), ct.GetMinutes(), ct.GetSeconds());
		else if (ct.GetMinutes() > 0)
			strTime.Format("Monster Clash: %02d:%02d", ct.GetMinutes(), ct.GetSeconds());
		else
			strTime.Format("Monster Clash: 00:%02d", ct.GetSeconds() < 0 ? 0 : ct.GetSeconds());
	}
	else
		return;
	...
	CWndBase::m_Theme.m_pFontCaption->DrawText((FLOAT)(point.x), (FLOAT)(point.y), 1.f, 1.f, D3DCOLOR_ARGB(255, 250, 250, 255), strTime);
}
...
//#endif	//
...
#ifdef __AEGON_TOURNAMENT
	memset(m_szTournamentStr, 0, sizeof(char) * 256);
	m_tournamentCaptionTime = GetTickCount();
	m_tournamentCaptionState = 9999;
#endif // __AEGON_TOURNAMENT
	...
	m_bAchievementNoticeOpen = FALSE;
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __DPS_METER
	CWndDpsMeter::GetInstance()->InitDeviceObjects();
#endif //__DPS_METER
	...
	memset(m_szPlayerWarstr, 0, sizeof(char) * 64);
...
void CWndWorld::OnDraw( C2DRender* p2DRender )
{
	...
	if (g_pPlayer && g_pPlayer->IsMode(AEGON_NINJA_MODE)) 
	{
		CEditString tmpEditStr;
		tmpEditStr.SetParsingString("NINJA MODE", COLOR_ERROR, ESSTY_BOLD);

		CD3DFont* oldFont = p2DRender->GetFont();
		p2DRender->SetFont(CWndBase::m_pTheme->m_pFontCaption);

		p2DRender->TextOut_EditString(0, 130, tmpEditStr, 0, 0, 2);
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	CAegonAchievementMng::GetInstance()->ProcessAchievementNotice(p2DRender);
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __ZONE_AFFICHAGE
	g_ZoneText.Render(p2DRender);
	g_ZoneDesc.Render(p2DRender);
#endif // __ZONE_AFFICHAGE

#ifdef __TICKETS_DETAILS
	g_LayersDetail.DrawTicketsDetails(p2DRender);
#endif // __TICKETS_DETAILS
	...
	if( g_pPlayer && m_bRenderFPS )
	{
		if (g_pPlayer->IsAuthHigher(AUTH_GAMEMASTER)) {
			...
			p2DRender->RenderFillRect(CRect(370, 8, 225, PosY), dwRenderColor, dwRenderColor, dwRenderColor, dwRenderColor);
		}
		else {
			int PosY = 80;
			DWORD dwRenderColor = D3DCOLOR_TEMP(130, 30, 30, 30);
		...
		//Ping
		TCHAR strPing[32];
		std::sprintf(strPing, "Ping: %d ms", g_Neuz.m_dwPingTime);
		p2DRender->TextOut(235, 15, strPing, D3DCOLOR_ARGB(255, 238, 238, 238), D3DFONT_BOLD);

		//Fps
		TCHAR strFPS[32];
		std::sprintf(strFPS, "FPS: %.02f", g_Neuz.m_fFPS);
		p2DRender->TextOut(235, 35, strFPS, D3DCOLOR_ARGB(255, 238, 238, 238), D3DFONT_BOLD);

		//Position
		TCHAR strPOSI[64];
		std::sprintf(strPOSI, "You're at: %s", g_pPlayer->GetWorld()->m_szWorldName);
		p2DRender->TextOut(235, 55, strPOSI, D3DCOLOR_ARGB(255, 238, 238, 238), D3DFONT_BOLD);

#ifdef __CLIENT_ONLINE_COUNT
		// Players Online for Admin, Developer, Game Designer
		if (g_pPlayer->IsAuthorization(AUTH_ADMINISTRATOR) || g_pPlayer->IsAuthorization(AUTH_OPERATOR) || g_pPlayer->IsAuthorization(AUTH_GAMEMASTER3))
		{
			TCHAR strCount[64];
			std::sprintf(strCount, "Players Online: %d", OnlineCount);
			p2DRender->TextOut(235, 75, strCount, D3DCOLOR_ARGB(255, 238, 238, 238), D3DFONT_BOLD);
		}
...
#ifdef __DPS_METER
	CWndDpsMeter::GetInstance()->Render(p2DRender, rc.bottom, rc.left);
#endif //__DPS_METER

	g_DialogMsg.Render( p2DRender );

#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
	g_Caption1.Render(CPoint(0, 80), p2DRender);
#else // __AEGON_ADVANCED_ANNOUNCEMENT
...
#ifdef __SYS_MONSTER_CLASH
	if (CWndMonsterClashRank::GetInstance()->IsActive())
	{
		g_CapTime.RenderClashTime(CPoint(0, 50 * p2DRender->m_clipRect.Height() / 768 + 70), p2DRender);
		CWndMonsterClashRank::GetInstance()->DrawMonsterClashinfo(p2DRender);
	}
#endif //__SYS_MONSTER_CLASH
...
*/			
	...
	if( g_pPlayer->IsFly() 
#ifdef __MOUNT
		&& !g_pPlayer->m_pActMover->IsUseMount()
#endif // __MOUNT
		)
	{
...
#ifdef __HEAL_SHOW
	g_HealNumMng.m_matView = g_Neuz.m_camera.m_matView;
	g_HealNumMng.m_matProj = g_WorldMng.Get()->m_matProj;
	pd3dDevice->GetViewport(&g_HealNumMng.m_viewport);
#endif // __HEAL_SHOW
#endif
...
#ifdef __HEAL_SHOW
	if (g_Option.m_bHideHeal == FALSE)
	{
		g_HealNumMng.Render();
	}
#endif // __HEAL_SHOW
	if(g_WndMng.m_pWndTaskBar && g_WndMng.m_pWndTaskBar->m_nCurQueue >= 0 )
	{
		...
		if (g_Option.m_bHideHeal == FALSE)
		{
			g_HealNumMng.m_textPackNum.Render(p2DRender, CPoint(rectClient.Width() - 20, rectClient.Height() - 20), nCount);
		}
#endif // __HEAL_SHOW
	}
	...
	if( m_dwGuildCombatTime != 0xffffffff )
	{
		...
		if(m_bGuildCombat1to1Wait)
		{
			...
			std::sprintf( szMsg, strMsg );
		}
		else
			std::sprintf( szMsg, "%s %.2d:%.2d:%.2d", m_szGuildCombatStr, ct.GetHours(), ct.GetMinutes(), ct.GetSeconds() );
...
//		if(g_GuildCombat1to1Mng.m_nState == CGuildCombat1to1Mng::GC1TO1_ENTRANCE || g_GuildCombat1to1Mng.m_nState == CGuildCombat1to1Mng::GC1TO1_WAR)
	...
	if( dwSecretTime != 0 )
	{
		...
		std::sprintf( szMsg, "%s %.2d:%.2d:%.2d", m_szSecretRoomStr, ct.GetHours(), ct.GetMinutes(), ct.GetSeconds() );
...
#ifdef __AEGON_TOURNAMENT
	DWORD dwTournamentTime = m_tournamentCaptionTime;

	if (g_pPlayer && g_pPlayer->GetWorld() && g_pPlayer->GetWorld()->GetID() == AT_TOURNAMENT_MAP)
	{
		CD3DFont* pOldFont = g_Neuz.m_2DRender.GetFont();
		g_Neuz.m_2DRender.SetFont(CWndBase::m_Theme.m_pFontGuildCombatText);
		...
		if (strTmpComp.Compare(m_szTournamentStr) == 0)
			dwTournamentTime = GetTickCount();

		__int64 timeDiff = (dwTournamentTime - GetTickCount()) / 1000;
		if (timeDiff < 0)
			timeDiff = 0;

		CTimeSpan ct(timeDiff);

		char szMsg[256] = { 0 };



		if (ct.GetDays() <= 0 && ct.GetHours() <= 0 && ct.GetMinutes() <= 0 && ct.GetSeconds() <= 0)
		{
			dwTournamentTime = 0;
			m_tournamentCaptionTime = 0;
			std::sprintf(szMsg, "%s", m_szTournamentStr);
		}
		else if (ct.GetDays() <= 0 && ct.GetHours() <= 0)
		{
			std::sprintf(szMsg, "%s %.2d:%.2d", m_szTournamentStr, ct.GetMinutes(), ct.GetSeconds());
		}
		else if (ct.GetDays() <= 0)
		{
			std::sprintf(szMsg, "%s %.2d:%.2d:%.2d", m_szTournamentStr, ct.GetHours(), ct.GetMinutes(), ct.GetSeconds());
		}
		else
		{

			std::sprintf(szMsg, "%s %.2I64u:%.2d:%.2d:%.2d", m_szTournamentStr, ct.GetDays(), ct.GetHours(), ct.GetMinutes(), ct.GetSeconds());
	...
	if( IsValidObj(g_pPlayer) && g_pPlayer->GetWorld() && g_pPlayer->GetWorld()->GetID() == WI_WORLD_GUILDWAR )
	{
		int		nGap  = 16;
		int		nRate = 0;
		CString strFormat;
...
//			if( nPoint > 0 )
			{
				if(IsValidObj(g_pPlayer) && g_pPlayer->GetGuild() )
				{
	...
	
	{
		...
		for( vector<__GuildCombatJoin>::iterator  itv = m_vGuildCombatSort.begin() ; itv != m_vGuildCombatSort.end() ; ++itv )
		{
			...
			if( bFind )
			{
				...
				if( nCount < 1 )
				{
					p2DRender->TextOut( nX, nY, szMsgGuild, COLOR_ERROR, 0xFF000000 );
					p2DRender->TextOut( nX + 120, nY, szMsg, COLOR_ERROR, 0xFF000000 );
				}
				else if( nCount < 3 )
				{
...
#ifdef __NEWCUR
		g_Neuz.SetMouseCursor(CUR_BASE);
#else //__NEWCUR
		CWndNeuz::OnSetCursor( pWndBase, nHitTest, message );
#endif //__NEWCUR
	else
#ifdef __NEWCUR
		g_Neuz.SetMouseCursor(dwCursor);
#else //__NEWCUR
...
#ifdef __MOUNT
#ifdef __AEGON_FIXES
	if (((CMover*)pObj) && pObj->GetType() != OT_ITEM && ((CMover*)pObj)->GetMountId() > 0 && ((CMover*)pObj)->GetMountModel() && ((CMover*)pObj)->GetMountId() != NULL_ID)
#else
	if (((CMover*)pObj) && pObj->GetType() != OT_ITEM && ((CMover*)pObj)->GetMountId() > 0 && ((CMover*)pObj)->GetMountModel())
#endif
	{
		MOUNT_ANIMATION* pMountAnimation = prj.GetMountAnimation(((CMover*)pObj)->GetMountId());
		if (pMountAnimation)
		{
			D3DXMATRIX matEvent;
			...
			D3DXMatrixMultiply(&matEvent, &matRotation, &matEvent);
			D3DXMatrixMultiply(&matEvent, &matEvent, ((CMover*)pObj)->GetMountModel()->GetMatrixBone(pMountAnimation->dwBindBone));
...
//#endif //__CSC_VER8_1
				...
				m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rect, D3DCOLOR_ARGB(200, 255, 255, 255), m_pVBGauge, &m_texGauFillTest);
				m_Theme.RenderGauge(p2DRender->m_pd3dDevice, m_pVBGauge, &m_texGauFillTest);

				float currentPercent = pMover->GetHitPointPercent();
				if (currentPercent >= 90)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff14AC00, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 80 && currentPercent < 90)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff279A05, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 70 && currentPercent < 80)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff3A880B, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 60 && currentPercent < 70)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff4D7710, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 50 && currentPercent < 60)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff606515, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 40 && currentPercent < 50)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff74531B, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 30 && currentPercent < 40)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff874120, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 20 && currentPercent < 30)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xff9A3025, m_pVBGauge, &m_texGauFillTest);
				}
				else if (currentPercent >= 10 && currentPercent < 20)
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xffAD1E2B, m_pVBGauge, &m_texGauFillTest);
				}
				else
				{
					m_Theme.MakeGaugeVertex(p2DRender->m_pd3dDevice, &rectTemp, 0xffC00C30, m_pVBGauge, &m_texGauFillTest);
				}
				m_Theme.RenderGauge(p2DRender->m_pd3dDevice, m_pVBGauge, &m_texGauFillTest);
				...
				if (g_pPlayer->GetWorld()->GetID() == WI_WORLD_GUILDWAR && g_pPlayer->GetGuild() && pMover->GetGuild())
				{
					if (g_pPlayer->GetGuild()->GetGuildId() == pMover->GetGuild()->GetGuildId())
					{
						sprintf(szText, "%s", pMover->GetName());
					}
					else
					{
						sprintf(szText, "Enemy");
					}
				}
				else
				{
				...
				if (bViewLevel)
					sprintf(szText2, "Lv. %d", pMover->GetLevel());

				BOOL renderLevel = TRUE;
				CString TargetName = pMover->GetName();
				...
				if (pMover->IsPlayer())
				{
					...
					if (g_pPlayer->GetWorld()->GetID() == WI_WORLD_GUILDWAR && g_pPlayer->GetGuild() && pMover->GetGuild())
					{
						if (g_pPlayer->GetGuild()->GetGuildId() == pMover->GetGuild()->GetGuildId())
						{
							p2DRender->TextOut(point.x, point.y - 20, szText, COLOR_SUCCESS);
						}
						else
						{
							p2DRender->TextOut(point.x, point.y - 20, szText, COLOR_ERROR);
						}
					}
					else
					{
						p2DRender->TextOut(point.x, point.y - 20, szText, COLOR_PLAYER);
					}
#else //__ENEMY_SYSTEM
					p2DRender->TextOut(point.x, point.y - 20, szText, COLOR_PLAYER);
#endif //__ENEMY_SYSTEM

					if (pMover->GetLevel() <= 9)
					{
						p2DRender->TextOut(point2.x - 35, point.y - 20, szText2, COLOR_PLAYER);
					}
					else if (pMover->GetLevel() > 9 && pMover->GetLevel() <= 99)
					{
						p2DRender->TextOut(point2.x - 42, point.y - 20, szText2, COLOR_PLAYER);
					}
					else
					{
						p2DRender->TextOut(point2.x - 49, point.y - 20, szText2, COLOR_PLAYER);
					}
				}
				else
				{
					...
					if (pMover->GetProp())
					{
						if (pMover->GetProp()->eElementType)
						{
							p2DRender->TextOut(point.x + 25, point.y - 20, szText, dwC);
						}
						else
						{
							p2DRender->TextOut(point.x, point.y - 20, szText, dwC);
						}
					}
					...
					if (renderLevel == TRUE)
					{
						if (pMover->GetLevel() <= 9)
						{
							p2DRender->TextOut(point2.x - 35, point.y - 20, szText2, dwC);
						}
						else if (pMover->GetLevel() > 9 && pMover->GetLevel() <= 99)
						{
							p2DRender->TextOut(point2.x - 42, point.y - 20, szText2, dwC);
						}
						else
						{
							p2DRender->TextOut(point2.x - 49, point.y - 20, szText2, dwC);
						}
					}
					else
					{
						sprintf(szText2, "Lv. %d", pMover->GetLevel());
						p2DRender->TextOut(point.x, point.y + 15, szText2, dwC);
					}
					...
					if (pMover->IsPlayer() == FALSE)
					{
						...
						if (pMover->GetProp())
						{
							if (pMover->GetProp()->eElementType)
							{
								...
								m_texAttrIcon.Render(p2DRender, point, pMover->GetProp()->eElementType - 1, 255, 1.2f, 1.2f);
							}
						}
					}
				}
				...
				CString string;
				string.Format(("%.2f%%"), (float)pMover->GetHitPointPercent(10000) / 100.0f);
				strTemp += string;
				p2DRender->TextOut(rect.left + 76, rect.top - 2, strTemp);
				RenderMoverBuff(pMover, p2DRender);
				p2DRender->SetFont(pOldFont);
#endif //__CUSTOM_HP_BAR
			}
		}
	}
...
#endif //__CSC_VER11_2
{
	...
	else
	{
		if( g_pPlayer->m_pActMover->IsFly() )
		{
			...
			{
				m_texTargetFly.GetAt(8)->Render(&g_Neuz.m_2DRender, pt1);
				m_texTargetFly.GetAt(9)->Render(&g_Neuz.m_2DRender, pt2);
				m_texTargetFly.GetAt(10)->Render(&g_Neuz.m_2DRender, pt3);
				m_texTargetFly.GetAt(11)->Render(&g_Neuz.m_2DRender, pt4);
			}
...
//	FOR_LAND( &g_World, pLand, g_World.m_nVisibilityLand, FALSE )
	...
	if( pObj )
	{
		DWORD dwColor = dwColor = COLOR_ERROR;
		CMover* pMover = (CMover*)pObj;
		D3DXVECTOR3 vPos = pObj->GetPos();
		vPos.y -= fLow2;
		// ( fHigh - fLow ) : vPos.z = rect.Height() : z
...
void CWndWorld::OnInitialUpdate()
{
...
#ifdef __CUSTOM_HP_BAR
	m_texGauFillTest.LoadTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "GauEmptyTest.bmp"), COLOR_ITEM_BG, TRUE);
#endif // __CUSTOM_HP_BAR
	m_texGauFillNormal.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "GauEmptyNormal.bmp" ), COLOR_ITEM_BG, TRUE );
	m_texLvUp.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "LvUp.bmp" ), COLOR_ITEM_BG, TRUE );
	m_texLvDn.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "LvDn.bmp" ), COLOR_ITEM_BG, TRUE );
	m_texLvUp2.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "LvUp2.bmp" ), COLOR_ITEM_BG, TRUE );
	m_texLvDn2.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "LvDn2.bmp" ), COLOR_ITEM_BG, TRUE );
		...
		if( pAddSkill )
		{
			if( pAddSkill->dwSkillTime != -1 )
			{
				...
				if( pItem )
				{
					...
					buffskill.m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ICON, pItem->szIcon ), COLOR_ITEM_BG );
	...
	for( i=0; i<prj.m_aPartySkill.GetSize(); i++ )
	{
		...
		if( pItemProp )
		{
			if( pItemProp->dwSkillTime != -1 )
			{
				...
				buffskill.m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ICON, pItemProp->szIcon ), COLOR_ITEM_BG );
	...
	for( i=0; i<prj.m_aPropItem.GetSize(); i++ )
	{
		...
		if( pItemProp )
		{
...
#ifdef __DST_GIFTBOX
#ifdef __V19_FORMATS
				|| pItemProp->dwDestParam[0] == DST_GIFTBOX || pItemProp->dwDestParam[1] == DST_GIFTBOX || pItemProp->dwDestParam[2] == DST_GIFTBOX || pItemProp->dwDestParam[3] == DST_GIFTBOX || pItemProp->dwDestParam[4] == DST_GIFTBOX || pItemProp->dwDestParam[5] == DST_GIFTBOX
#else //__V19_FORMATS
			...
			)
			{
				...
				{
					strIcon.Replace( ".", "_00." );
					buffskill.m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ITEM, strIcon ), COLOR_ITEM_BG );
...
#ifdef __DST_GIFTBOX
#ifdef __V19_FORMATS
					if (pItemProp->dwDestParam[0] == DST_GIFTBOX || pItemProp->dwDestParam[1] == DST_GIFTBOX || pItemProp->dwDestParam[2] == DST_GIFTBOX || pItemProp->dwDestParam[3] == DST_GIFTBOX || pItemProp->dwDestParam[4] == DST_GIFTBOX || pItemProp->dwDestParam[5] == DST_GIFTBOX)
#else //__V19_FORMATS
					if (pItemProp->dwDestParam[0] == DST_GIFTBOX || pItemProp->dwDestParam[1] == DST_GIFTBOX || pItemProp->dwDestParam[2] == DST_GIFTBOX)
#endif //__V19_FORMATS
						buffskill.m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ICON, "Skill_TroGiftbox02.dds" ), COLOR_ITEM_BG );
					else
						buffskill.m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ITEM, pItemProp->szIcon ), COLOR_ITEM_BG );
#else //__DST_GIFTBOX
					buffskill.m_pTexture = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ITEM, pItemProp->szIcon ), COLOR_ITEM_BG );
#endif //__DST_GIFTBOX
	...
	int rectWidth = m_rectWindow.Width();
	if (rectWidth >= 2560) 
	{
		m_nLimitBuffCount = 25;
	}
	else if (rectWidth >= 1920)
	{
		m_nLimitBuffCount = 19;
	}
	else if (rectWidth >= 1680)
	{
		m_nLimitBuffCount = 18;
	}
	else if (rectWidth >= 1600)
	{
		m_nLimitBuffCount = 17;
	}
	else if (rectWidth >= 1400)
	{
		m_nLimitBuffCount = 15;
	}
	else if (rectWidth >= 1360)
	{
		m_nLimitBuffCount = 14;
	}
	else if (rectWidth >= 1280)
	{
		m_nLimitBuffCount = 13;
	}
	else if (rectWidth >= 1024)
	{
		m_nLimitBuffCount = 10;
	}
	else if(rectWidth >= 800)
	{
		m_nLimitBuffCount = 7;
	}
	else 
	...
	for( i=0; i<CUSTOM_LOGO_MAX; i++ )
	{
		...
		str.Format("Icon_CloakSLogo%02d.png", i + 1);

		if (!m_pTextureLogo[i].LoadTexture(g_Neuz.GetDevice(), MakePath(DIR_ICON, str), D3DCOLOR_XRGB(0, 0, 0), TRUE))
		{
			Error("Failed to load guild logo: %s", str);
		}
#else
	...
	m_texMsgIcon.LoadScript( m_pApp->m_pd3dDevice, MakePath(DIR_ICON, "icon_IconMessenger.inc") );
	m_texAttrIcon.LoadScript( m_pApp->m_pd3dDevice, MakePath(DIR_ICON, "Icon_MonElemantkind.inc") );
	
#if __VER >= 11 // __CSC_VER11_4
	m_texPlayerDataIcon.LoadScript( m_pApp->m_pd3dDevice, MakePath(DIR_ICON, "icon_PlayerData.inc") );
#endif //__CSC_VER11_4
	for( int j = 0 ; j < SM_MAX ; ++j )
	{
		if( j != SM_RESIST_ATTACK_LEFT && j != SM_RESIST_ATTACK_RIGHT && j != SM_RESIST_DEFENSE )
		{
			...
			if( pItem )
				m_dwSMItemTexture[j] = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ITEM, pItem->szIcon ), COLOR_ITEM_BG );
		}
	}
	...
	for( int kk = 0; kk < 10; kk++ )
	{
		...
		if( pItem )
			m_dwSMResistItemTexture[kk] = m_textureMng.AddTexture( m_pApp->m_pd3dDevice,  MakePath( DIR_ITEM, pItem->szIcon ), COLOR_ITEM_BG );	
	}
	...
	pWorldMap->Init(); 
	m_TexGuildWinner.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "TexGuildCombatWinner.bmp" ), COLOR_ITEM_BG );
	m_TexGuildBest.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "TexGuildCombatBest.bmp" ), COLOR_ITEM_BG );
...
BOOL CWndWorld::OnCommand( UINT nID, DWORD dwMessage, CWndBase* pWndBase )
{
	...
	if( pWndBase == &m_wndMenuMover && pFocusObj )
	{
...
#ifdef __SYS_MONSTER_CLASH
		case MMI_MONSER_CLASH_REGISTER:
		{
			g_DPlay.SendHdr(PACKETTYPE_MONSTER_CLASH_REGISTER_WINDOW); 
		}
		break;
		case MMI_MONSTER_CLASH_REGISTER_MEMBER:
		{
			g_DPlay.SendHdr(PACKETTYPE_MONSTER_CLASH_REGISTER_MEMBER_WINDOW);
		}
		break;
#endif //__SYS_MONSTER_CLASH

#ifdef __ROYAL_RUMBLE
		case MMI_WARPLAYERSJOIN:
		{
			g_DPlay.SendGetSiegeRanking(g_pPlayer->m_idPlayer, FALSE);

			if (g_WndMng.m_pJoinPlayerSiege)
				SAFE_DELETE(g_WndMng.m_pJoinPlayerSiege);

			g_WndMng.m_pJoinPlayerSiege = new CWndJoinPlayerSiege;
			g_WndMng.m_pJoinPlayerSiege->Initialize();
			break;
		}

		case MMI_WARPLAYERRANKING:
		{
			g_DPlay.SendGetSiegeRanking(g_pPlayer->m_idPlayer, TRUE);

			if (g_WndMng.m_pSiegeRanking)
				SAFE_DELETE(g_WndMng.m_pSiegeRanking);

			g_WndMng.m_pSiegeRanking = new CWndPlayerRanking;
			g_WndMng.m_pSiegeRanking->Initialize();
			break;
		}
		case MMI_WARPLAY_JOIN:
		{
			g_DPlay.SendJoinWarPlayer(FALSE, TRUE);
			break;
		}
#endif // __ROYAL_RUMBLE
		...
		case MMI_GUILDWAR_CANCEL:
			...
			SAFE_DELETE(g_WndMng.m_pWndGuildCombatCancelConfirm);
			g_WndMng.m_pWndGuildCombatCancelConfirm = new CWndGuildWarCancelConfirm(0);

			if (g_WndMng.m_pWndGuildCombatCancelConfirm)
				g_WndMng.m_pWndGuildCombatCancelConfirm->Initialize(NULL);
#else
		...
		case MMI_GUILDWAR_JOIN:
			{
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
			SAFE_DELETE(g_WndMng.m_pWndGuildCombatJoinConfirm);
			g_WndMng.m_pWndGuildCombatJoinConfirm = new CWndGuildWarJoinConfirm(0);

			if (g_WndMng.m_pWndGuildCombatJoinConfirm)
				g_WndMng.m_pWndGuildCombatJoinConfirm->Initialize(NULL);
#else
#if __VER >= 11 // __GUILD_COMBAT_1TO1
		...
		case MMI_GUILDCOMBAT_INFO_TEX:
			{
				...
				if( g_WndMng.m_pWndGuildCombatBoard )
				{
					...
					CScript scanner;
					if( scanner.Load( DIR_CLIENT +strnotice ) == FALSE )
					{
						SAFE_DELETE(g_WndMng.m_pWndGuildCombatBoard);
						break;
					}
		...
		case MMI_GUILDCOMBAT_1TO1_CANCEL:
			...
			g_WndMng.m_pWndGuildCombatCancelConfirm = new CWndGuildWarCancelConfirm(1);
...
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
			SAFE_DELETE(g_WndMng.m_pWndGuildCombatJoinConfirm);
			g_WndMng.m_pWndGuildCombatJoinConfirm = new CWndGuildWarJoinConfirm(1);
		...
		case MMI_LEGEND_SKILLUP:
			...
			case MMI_NPC_BUFF_CLASS:
			{
				g_DPlay.SendNPCBuffClass(pFocusMover->m_szCharacterKey);
			}
			break;
	...
	case MMI_RENAME_CANCEL:
...
#ifdef __MOUNT
				&& g_pPlayer->m_pActMover->IsUseMount() == FALSE
#endif // __MOUNT	
				)
			{
		...
		case MMI_RETURN_AWAKE:
		{
			SAFE_DELETE(g_WndMng.m_pWndReturnAwakening);
			g_WndMng.m_pWndReturnAwakening = new CWndReturnAwakening;
			g_WndMng.m_pWndReturnAwakening->Initialize();

			CWndBase* invBase = g_WndMng.GetApplet(APP_INVENTORY);
			if (!invBase)
				g_WndMng.CreateApplet(APP_INVENTORY);
			break;
		}
#endif // __AEGON_RETURN_AWAKENING

#ifdef __WEAPON_RARITY
		case MMI_WEAPON_RARITY:
		{
			if (!g_WndMng.GetWndBase(APP_WEAPON_RARITY))
			{
				g_WndMng.m_pWndWeaponRarity = new CWndWeaponRarity;
				g_WndMng.m_pWndWeaponRarity->Initialize();
			}
		}
		break;
		...
		case MMI_PVP_MINIGAMES:
		{
			if (g_WndMng.m_pWndAegonPvPGameMain)
				break;

			SAFE_DELETE(g_WndMng.m_pWndAegonPvPGameCreate);
			g_WndMng.m_pWndAegonPvPGameCreate = new CWndAegonPvPGameCreate;
			g_WndMng.m_pWndAegonPvPGameCreate->Initialize();
			break;
		}
		case MMI_PVP_MINIGAMES_FIND:
		{
			if (g_WndMng.m_pWndAegonPvPGameMain)
				break;

			if (g_WndMng.m_pWndAegonPvPGameFinder)
				break;

			g_WndMng.m_pWndAegonPvPGameFinder = new CWndAegonPvPGameFinder;
			g_WndMng.m_pWndAegonPvPGameFinder->Initialize();
			break;
		}
...
#ifdef __AEGON_TOURNAMENT
		case MMI_AEGON_TOURNAMENT:
		{
			SAFE_DELETE(g_WndMng.m_pWndAegonTournament);
			g_WndMng.m_pWndAegonTournament = new CWndAegonTournament;
			g_WndMng.m_pWndAegonTournament->Initialize();
			break;
		}
		case MMI_AEGON_TOURNAMENT_LOG:
		{

			break;
		}
		case MMI_AEGON_TOURNAMENT_TELE:
		{
		...
		case MMI_VIEW_BAG:
		{
			...
			if (nID == MMI_VIEW_INV)
				nContainer = 0;
			else if (nID == MMI_VIEW_BAG)
				nContainer = 1;

			if (g_pPlayer && g_pPlayer->IsAuthHigher(AUTH_GAMEMASTER2) && pFocusMover->IsPlayer())
			{
				if (g_WndMng.GetWndBase(APP_VIEW_INVENTORY))
				{
					g_WndMng.m_pWndViewInventoryMain->DoInventoryView(pFocusMover->GetName(), nContainer);
				}
				else if(nContainer != -1)
				{
					...
					g_WndMng.m_pWndViewInventoryMain->Initialize();

					if (g_WndMng.m_pWndViewInventoryMain)
						g_WndMng.m_pWndViewInventoryMain->DoInventoryView(pFocusMover->GetName(), nContainer);
				}
			}
		}
...
#ifdef __SYS_SANDBOX
		case MMI_SANDBOX_ENTER:
		case MMI_SANDBOX_LEAVE:
		{
			g_DPlay.SendSandboxTeleport();
		}
		break;
		case MMI_SANDBOX:
		{
			if (g_pPlayer && g_pPlayer->IsWorldSandBox())
			{
				if (!g_WndMng.m_pWndSandbox)
				{
					g_WndMng.m_pWndSandbox = new CWndSandbox;
					g_WndMng.m_pWndSandbox->Initialize(&g_WndMng);
				}
			}
		}
		break;
		case MMI_SANDBOX_CLEAR:
		{
			if (g_pPlayer && g_pPlayer->IsWorldSandBox())
			{
				g_DPlay.SendSandboxClear();
			}
		}
		break;
#endif //__SYS_SANDBOX
		...
		{
			if (!g_WndMng.m_pWndWeaponMerge)
			{
				g_WndMng.m_pWndWeaponMerge = new CWndWeaponMerge;
				g_WndMng.m_pWndWeaponMerge->Initialize();
			}
		}
		break;
		case MMI_TWOHANDEDWEAPONMERGE_CANCEL:
		{
			if (!g_WndMng.m_pWndWeaponMergeCancel)
			{
				g_WndMng.m_pWndWeaponMergeCancel = new CWndWeaponMergeCancel;
				g_WndMng.m_pWndWeaponMergeCancel->Initialize();
			}
		}
		break;
...
*/
		...
		case MMI_PET_GLOW:
		{
			if (!g_WndMng.m_pWndPetGlowChange)
			{
				g_WndMng.m_pWndPetGlowChange = new CWndPetGlowChange;
				g_WndMng.m_pWndPetGlowChange->Initialize();
			}
		}
		break;
#endif // __PET_GLOW
		...
		case MMI_CRYSTAL:
		{
			if (!g_WndMng.GetApplet(APP_CRYSTAL))
			{
				g_WndMng.m_pWndCrystal = new CWndCrystal;
				g_WndMng.m_pWndCrystal->Initialize();
			}
		}
		break;
#endif // __CRYSTAL
...
#ifdef __MOUNT
#ifdef __AEGON_FIXES
	if (g_pPlayer->GetMountId() > 0 && g_pPlayer->GetMountId() != NULL_ID)		return FALSE;
#else
	if (g_pPlayer->GetMountId() > 0)		return FALSE;
#endif
#endif // __MOUNT
...
#ifdef __ACTION_SLOT_PAGES
BOOL CWndWorld::UseSkillToFocusObj(CCtrl* pFocusObj, const int nIndex)
...
#ifdef __ACTION_SLOT_PAGES
		BOOL bRet = pTaskBar->UseSkillQueue((CCtrl*)pFocusObj, nIndex);
#else // __ACTION_SLOT_PAGES
...
void CWndWorld::SetNextTarget()
{
	...
	if(m_pNextTargetObj && pWorld)
	{
		if( m_pNextTargetObj->GetType() == OT_MOVER )
		{
			...
			if (((CMover*)m_pNextTargetObj)->IsMode(AEGON_MOB_IGNORE_MODE)) // 대상이 투명모드일땐 타겟 안됨.
			{
				return;
			}
#endif
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	if (m_bAchievementNoticeOpen && m_rectAchievementNotice.PtInRect(point)) 
	{
		if(g_pPlayer->m_AchievementNoticeID.size() > 0)
			CAegonAchievementMng::GetInstance()->OpenAchievementWnd(g_pPlayer->m_AchievementNoticeID[0]);
...
#if __VER >= 13 // __HOUSING
#ifdef __VENDING_NPC
	if (!CDeployManager::GetInstance()->IsReady() && !CVendorDeploy::GetInstance()->IsReady())
#else //__VENDING_NPC
...
void CWndWorld::ShowMoverMenu( CMover* pTarget )
{
	...
	{		
		...
		if( pTarget->IsPlayer() )
		{
			...
			if( pTarget->m_vtInfo.IsVendorOpen() )
			{
				m_wndMenuMover.AppendMenu( 0,	MMI_MESSAGE		, prj.GetText(TID_APP_MESSAGE));
...
/*
			...
			*/
			...
			{
				m_wndMenuMover.AppendMenu(0, MMI_VIEW_INV, _T("View Inventory"));
				m_wndMenuMover.AppendMenu(0, MMI_VIEW_BAG, _T("View Bag"));
...
#ifdef __AEGON_RETURN_AWAKE
							else if (i == MMI_RETURN_AWAKE)
							{
								m_wndMenuMover.AppendMenu(0, i, ("Return Awakening"));
								bView = TRUE;
							}
#endif // __AEGON_RETURN_AWAKE

#ifdef __WEAPON_RARITY
							else if (i == MMI_WEAPON_RARITY)
							{
								m_wndMenuMover.AppendMenu(0, i, prj.GetText(TID_WR_TEXT_001));
								bView = TRUE;
							}
#endif // __WEAPON_RARITY

#ifdef __ANARCHY_SYSTEM
							else if (i == MMI_ANARCHY)
							{
								m_wndMenuMover.AppendMenu(0, i, ("Anarchy Buffs"));
								bView = TRUE;
							}
#endif //__ANARCHY_SYSTEM

#ifdef __SYS_MONSTER_CLASH
							else if (i == MMI_MONSER_CLASH_REGISTER)
							{
								m_wndMenuMover.AppendMenu(0, i, ("Monster Clash"));
								bView = TRUE;
							}
							else if (i == MMI_MONSTER_CLASH_REGISTER_MEMBER)
							{
								m_wndMenuMover.AppendMenu(0, i, ("Monster Clash - Members"));
								bView = TRUE;
							}
#endif //__SYS_MONSTER_CLASH

#ifdef __AEGON_LOOT_BOX
							else if (i == MMI_LOOTBOX)
							{
								m_wndMenuMover.AppendMenu(0, i, ("Lootbox System"));
								bView = TRUE;
							}
#endif //__AEGON_LOOT_BOX

#ifdef __SYS_SANDBOX
							else if (i == MMI_SANDBOX)
							{
								m_wndMenuMover.AppendMenu(0, i, "Spawn Sandbox Monster");
								bView = TRUE;
							}
							else if (i == MMI_SANDBOX_CLEAR)
							{
								m_wndMenuMover.AppendMenu(0, i, "Clear Sandbox");
								bView = TRUE;
							}
							else if (i == MMI_SANDBOX_ENTER)
							{
								m_wndMenuMover.AppendMenu(0, i, "Enter Sandbox");
								bView = TRUE;
							}
							else if (i == MMI_SANDBOX_LEAVE)
							{
								m_wndMenuMover.AppendMenu(0, i, "Leave Sandbox");
								bView = TRUE;
							}
#endif // __SYS_SANDBOX

#ifdef __NPC_BUFF_CLASS
							else if (i == MMI_NPC_BUFF_CLASS)
							{
								m_wndMenuMover.AppendMenu(0, i, "Class Buffs");
								bView = TRUE;
							}
#endif //__NPC_BUFF_CLASS
...
/*
#ifdef __BUYBACK
							else if (i == MMI_BUYBACK)
							{
								m_wndMenuMover.AppendMenu(0, i, ("Buyback"));
								bView = TRUE;
							}
#endif //__BUYBACK
*/
...
#ifdef __PET_GLOW
							else if (i == MMI_PET_GLOW)
							{
								m_wndMenuMover.AppendMenu(0, i, prj.GetText(TID_PET_GLOW));
								bView = TRUE;
							}
#endif // __PET_GLOW

#ifdef __CRYSTAL
							else if (i == MMI_CRYSTAL)
							{
								m_wndMenuMover.AppendMenu(0, i, "Crystal Upgrade");
								bView = TRUE;
							}
#endif // __CRYSTAL
#ifdef __ROYAL_RUMBLE
							else if (i == MMI_WARPLAYERSJOIN)
							{
								m_wndMenuMover.AppendMenu(0, i, prj.GetText(TID_PLAYER_WAR_31));
								bView = TRUE;
							}
							else if (i == MMI_WARPLAYERRANKING)
							{
								m_wndMenuMover.AppendMenu(0, i, prj.GetText(TID_PLAYER_WAR_32));
								bView = TRUE;
							}
							else if (i == MMI_WARPLAY_JOIN)
							{
								m_wndMenuMover.AppendMenu(0, i, prj.GetText(TID_PLAYER_WAR_33));
								bView = TRUE;
							}
#endif // __ROYAL_RUMBLE
							else if (i == MMI_CHANGORI)
							{
								m_wndMenuMover.AppendMenu(0, i, prj.GetText(TID_MMI_CHANGORI_TEXT01));
								bView = TRUE;
							}
							else
							{
...
#ifdef __VENDING_NPC
	if (g_Neuz.m_pVendingNpcItem && VendorDeploy()->IsReady())
	{
		if ((VendorDeploy()->CheckCollision() || g_pPlayer->GetWorld()->GetID() != WI_WORLD_MADRIGAL) && !g_pPlayer->IsAuthHigher(AUTH_ADMINISTRATOR))
		{
			g_WndMng.PutString("You can't put the NPC here.", NULL, prj.GetTextColor(TID_GAME_HOUSING_INSTALL_FAIL01));
			return;
		}
		HOUSING_ITEM* pItem = VendorDeploy()->EndDeploy(FALSE);
		if (pItem)
		{
			g_DPlay.SendCreateVendor(MAKELONG(ITYPE_ITEM, g_Neuz.m_pVendingNpcItem->m_dwObjId), pItem->m_vPos, pItem->m_fAngle, g_Neuz.m_szVendingNpcTitle);
			g_Neuz.m_pVendingNpcItem->SetExtra(0);
			g_Neuz.m_pVendingNpcItem = NULL;
		}
		else
		{
			g_DPlay.SendPVendorClose(g_pPlayer->m_objid, TRUE);
			g_Neuz.m_pVendingNpcItem = NULL;
		}
		return;
	}
...
//			m_bAutoAttack = TRUE;
	...
	if( pTaskBar && pTaskBar->m_nExecute )
	...
	else 
	{
		if( pSelectObj->GetType() == OT_MOVER )
		{
			...
			if (!pMover->IsActiveMover() &&
				...
				//pMover->m_vtInfo.GetOwner() != CMover::GetActiveMover()->m_idPlayer &&
			...
			{
#ifdef __fmod
				SoundSystemClass::playSound(pMover->GetProp()->dwSndIdle1);
#else
...
#ifdef __DEV_FREECAM
	if (g_Neuz.m_camera.m_nCamMode == CM_ROTATE)
		return;
#endif //__DEV_FREECAM
...
#ifdef __VENDING_NPC
	if (m_dwNpcVendorItem == 0 && g_pPlayer->HasBuffByIk3(IK3_VENDING))
	{
		auto pBuff = g_pPlayer->m_buffs.GetBuffByIk3(IK3_VENDING);
		if (pBuff)
		{
			m_dwNpcVendorItem = pBuff->GetId();
		}
	}

	if (m_dwNpcVendorItem != 0 && GetBuffIconRect(m_dwNpcVendorItem, point))
	{
		SAFE_DELETE(g_WndMng.m_pWndVendorWarning);
		g_WndMng.m_pWndVendorWarning = new CWndVendorWarning;
		g_WndMng.m_pWndVendorWarning->Initialize();
#ifdef __AEGON_FIXES
		return;
#endif
	}
...
void CWndWorld::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
#ifdef __DPS_METER
	/*if (m_bCtrlPushed && nChar == 'D') // Ctrl & D
	{
	...
	{
#ifdef __MIKSIK_EQUALIZED_MODE
		if (nChar == 'C' && !g_WndMng.GetWndBase(APP_TMP_CLASS))
...
#ifdef __ACTION_SLOT_PAGES
				g_WndMng.m_pWndWorld->SetCurrentActionSlot(g_WndMng.m_pWndTaskBar->m_nActionSlotIndex/*GetActionSlot()*/);
#endif // __ACTION_SLOT_PAGES
			}
		}
	}
...
void CWndWorld::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == g_Neuz.Key.chUp || nChar == g_Neuz.Key.chBack || nChar == g_Neuz.Key.chLeft || nChar == g_Neuz.Key.chRight)
...
#ifdef __AEGON_FOLLOW_HOTKEY
	if(nChar == g_Neuz.Key.chTrace)
	{
		if (g_pPlayer) 
		{
			...
			if (pWorld) 
			{
				...
				{
					m_objidTracking = ((CMover*)pObj)->GetId();
					g_pPlayer->SetDestObj((CMover*)pObj);
				}
			}
...
BOOL CWndWorld::OnDropIcon( LPSHORTCUT pShortcut, CPoint point )
{
		...
		g_WndMng.GetWndBase( APP_TRADE ) )
	{
		...
		{
			g_WndMng.PutString( prj.GetText(TID_GAME_TRADELIMITITEM), NULL, prj.GetTextColor(TID_GAME_TRADELIMITITEM) );
			//g_WndMng.PutString( "거래중에 아이템을 버릴수 없어요.", NULL, COLOR_ERROR );
			SetForbid( TRUE );
			return FALSE;
		}
	}
	...
	if( g_WndMng.GetWndBase( APP_REPAIR ) )
	{
		if( pWndFrame && pWndFrame->GetWndId() == APP_INVENTORY )
		{
			...
			//g_WndMng.PutString( "수리중에 아이템을 버릴수 없어요.", NULL, COLOR_ERROR );
			SetForbid( TRUE );
			return FALSE;
		}
	}
	...
	if( pShortcut->m_dwShortcut == SHORTCUT_ITEM ) //&& pWndFrame->GetWndId() == APP_INVENTORY )
	{
		...
		{
#ifdef __fmod
			SoundSystemClass::playSound(SND_INF_GROUNDDROP);
#else
...
BOOL CWndWorld::Process()
{
	...
	if( g_pPlayer )
	{
		...
		{
			if (g_bKeyTable[g_Neuz.Key.chUp])
			{
				D3DXVECTOR3 v3CameraDir = g_Neuz.m_camera.m_vLookAt - g_Neuz.m_camera.m_vPos;
				D3DXVec3Normalize(&v3CameraDir, &v3CameraDir);
				v3CameraDir.y = 0.0f;
				g_Neuz.m_camera.m_vFree += v3CameraDir * 0.5f;
			}
			if (g_bKeyTable[g_Neuz.Key.chBack])
			{
				D3DXVECTOR3 v3CameraDir = g_Neuz.m_camera.m_vLookAt - g_Neuz.m_camera.m_vPos;
				D3DXVec3Normalize(&v3CameraDir, &v3CameraDir);
				v3CameraDir.y = 0.0f;
			...
			if (g_bKeyTable[g_Neuz.Key.chLeft])
			{
				D3DXVECTOR3 v3CameraDir = g_Neuz.m_camera.m_vLookAt - g_Neuz.m_camera.m_vPos;
				D3DXVec3Normalize(&v3CameraDir, &v3CameraDir);
				v3CameraDir.y = 0.0f;
				...
				D3DXMatrixRotationY(&mMat, D3DXToRadian(90));
				D3DXVec3TransformCoord(&v3CameraDir, &v3CameraDir, &mMat);
				g_Neuz.m_camera.m_vFree -= v3CameraDir * 0.5f;
			}
			if (g_bKeyTable[g_Neuz.Key.chRight])
			{
				D3DXVECTOR3 v3CameraDir = g_Neuz.m_camera.m_vLookAt - g_Neuz.m_camera.m_vPos;
				D3DXVec3Normalize(&v3CameraDir, &v3CameraDir);
				v3CameraDir.y = 0.0f;
			...
			if (g_bKeyTable[VK_SPACE])
			{
				g_Neuz.m_camera.m_vFree.y += 0.5f;
			}
			if (g_bKeyTable[VK_SHIFT])
			{
				g_Neuz.m_camera.m_vFree.y -= 0.5f;
			}
		}
		else if (g_Neuz.m_camera.m_nCamMode == CM_OBSERVE || g_Neuz.m_camera.m_nCamMode == CM_ROTATE)
		{
			if (g_bKeyTable[VK_SPACE])
			{
				CMover* pMover = NULL;

				if (n_nMoverSelectCount >= MAX_MOVEROBSERVE)
					n_nMoverSelectCount = 0;

				int i;
				for (i = 0; i < MAX_MOVEROBSERVE; i++)
				{
					if (n_nMoverSelectCount <= i)
						continue;

					pMover = CWorld::m_amvrSelectObserve[i];

					if (pMover && !pMover->IsMode(TRANSPARENT_MODE) && !pMover->IsDie())
						break;
				}

				if (pMover)
				{
					g_Neuz.m_camera.m_nCamMode = CM_OBSERVE;
					g_Neuz.m_camera.m_idMoverTarget = pMover->m_objid;
					g_Neuz.m_camera.m_idMoverTargetRot = NULL_ID;
				}
...
#ifdef __CHANGE_HOT_KEY
			(g_bKeyTable[g_Neuz.Key.chUp] || g_bKeyTable[g_Neuz.Key.chBack] || g_bKeyTable[g_Neuz.Key.chLeft] || g_bKeyTable[g_Neuz.Key.chRight]) && g_pPlayer->IsFly() == FALSE)
#else // __CHANGE_HOT_KEY
		...
		{
#ifdef __fmod
			if (SoundSystemClass::IsStopMusic())
			{
				SoundSystemClass::playMusic(m_dwIdBgmMusic);
#else
			if( IsStopMusic() == TRUE )
			{
...
//#endif	//
		...
		if( g_pPlayer->m_pActMover->IsState( OBJSTA_STAND ) == FALSE || g_pPlayer->m_pActMover->GetMoveState() == OBJSTA_BMOVE )
		{
...
#endif
#if __VER >= 11 // __SYS_IDENTIFY
#ifndef __NO_CLOSE_AWAKEWINDOW
			pWndBase	= g_WndMng.GetWndBase( APP_AWAKENING );
			if( pWndBase )
			{
				pWndBase->Destroy();
			}
#endif // __NO_CLOSE_AWAKEWINDOW
#ifdef __AEGON_RETURN_AWAKE
			pWndBase = g_WndMng.GetWndBase(APP_RETURN_AWAKENING);
			if (pWndBase)
			{
				pWndBase->Destroy();
			}
			...
			pWndBase = g_WndMng.GetApplet(APP_AEGON_INGAME_SHOP);
			if (pWndBase)
			{
				pWndBase->Destroy();
			}
			...
			pWndBase = g_WndMng.GetWndBase(APP_DP_CONFIRM);
			if (pWndBase)
			{
				pWndBase->Destroy();
			}
#endif
#ifdef __AEGON_TOURNAMENT
			pWndBase = g_WndMng.GetWndBase(APP_AEGON_TOURNAMENT);
			if (pWndBase)
			{
				pWndBase->Destroy();
			}
...
#ifdef __WEAPON_RARITY
			pWndBase = g_WndMng.GetWndBase(APP_WEAPON_RARITY);
			if (pWndBase)
				pWndBase->Destroy();
#endif // __WEAPON_RARITY

#ifdef __AEGON_LOOT_BOX
			pWndBase = g_WndMng.GetWndBase(APP_LOOTBOX);
			if (pWndBase)
				pWndBase->Destroy();
#endif // __AEGON_LOOT_BOX

#ifdef __WEAPON_MERGE
			pWndBase = g_WndMng.GetWndBase(APP_WEAPONMERGE);
			if (pWndBase)
				pWndBase->Destroy();

			pWndBase = g_WndMng.GetWndBase(APP_WEAPONMERGE_CANCEL);
			if (pWndBase)
				pWndBase->Destroy();
#endif //__WEAPON_MERGE
...
#ifdef __PET_GLOW
			pWndBase = g_WndMng.GetWndBase(APP_CHANGE_PETGLOW);
			if (pWndBase)
				pWndBase->Destroy();
#endif // __PET_GLOW

#ifdef __FASHION_COMBINE
			pWndBase = g_WndMng.GetWndBase(APP_COSTUME_MIXMENU);
			if (pWndBase)
				pWndBase->Destroy();
#endif // __FASHION_COMBINE

#ifdef __COLOSSEUM
			pWndBase = g_WndMng.GetWndBase(APP_NEWCOLOENTER);
			if (pWndBase)
				pWndBase->Destroy();
#endif //__COLOSSEUM

#ifdef __AEGON_CRAFTING_SYSTEM
			pWndBase = g_WndMng.GetWndBase(APP_AEGON_CRAFTING);
			if (pWndBase)
				pWndBase->Destroy();
#endif //__AEGON_CRAFTING_SYSTEM
		}
#ifdef __fmod
		SoundSystemClass::setListener(g_pPlayer->GetPos(), g_Neuz.m_camera.m_fCurRotx);
#else
		...
		if (g_pPlayer) 
		{
			...
			if(IsValidObj(g_pPlayer) && g_pPlayer->m_pActMover && g_pPlayer->m_pActMover->IsFly() == FALSE )
			{
				CObj* pObj = pWorld->GetObjFocus();
				if( m_dwNextSkill == NEXTSKILL_ACTIONSLOT )	// 걍 1이면 액션슬롯 사용 
				{
	#ifdef __ACTION_SLOT_PAGES
					UseSkillToFocusObj((CCtrl*)pObj, GetCurrentActionSlot());
	#else // __ACTION_SLOT_PAGES
					UseSkillToFocusObj((CCtrl*)pObj);		// ½ºA³A≫ ≫c¿eCO.
	#endif // __ACTION_SLOT_PAGES
				} 
			...
			for( int i = 0; i < nSize; i++ )
			{
				...
				{
	#ifdef __AEGON_ACHIEVEMENT_SYSTEM
					CAegonAchievementMng::GetInstance()->CheckPlayerRegionAchievement(lpRegionElem);
	#endif
	...
	//					if( IsStopMusic()  ) //|| lpRegionElem->m_bDirectMusic ) ) //&& !lpRegionElem->m_bPlayDirectMusic )
						{
	#ifdef __fmod
							SoundSystemClass::playMusic(lpRegionElem->m_dwIdMusic, true);
	#else
	...
	#else //__ZONE_AFFICHAGE								
												g_ZoneText.Add(szDesc);
	#endif //__ZONE_AFFICHAGE
											}
											else
											{
...
void CWndWorld::AegonOnSwitchFont()
{
	CString strFont = g_Option.m_strFont;
	if (!IsFontInstalled(strFont))
	{
		strFont = "Verdana";
	}

	PLANG_DATA	pData = CLangMan::GetInstance()->GetLangData(LANG_USA);
	lstrcpy(pData->font.lfCaption.szFontFirst, g_Option.m_strFont.GetString());
	...
	m_pFontAPICaption = new CD3DFontAPI(strFont, 9);
	m_pFontAPICaption->m_nOutLine = 2;
	m_pFontAPICaption->m_dwColor = D3DCOLOR_ARGB(255, 255, 255, 255);
	m_pFontAPICaption->m_dwBgColor = D3DCOLOR_ARGB(255, 40, 100, 220);
	m_pFontAPICaption->m_dwFlags = D3DFONT_FILTERED;
	...
	m_pFontAPITitle = new CD3DFontAPI(strFont, 9);
	m_pFontAPITitle->m_nOutLine = 2;
	m_pFontAPITitle->m_dwColor = D3DCOLOR_ARGB(255, 255, 255, 255);
	m_pFontAPITitle->m_dwBgColor = D3DCOLOR_ARGB(255, 40, 100, 220);
	m_pFontAPITitle->m_dwFlags = D3DFONT_FILTERED;
...
#ifdef __DPS_METER
	CWndDpsMeter::GetInstance()->DeleteDeviceObjects();
#endif //__DPS_METER
...
void CWndWorld::RenderBuffIcon( C2DRender *p2DRender, IBuff* pBuff, BOOL bPlayer, BUFFICON_INFO* pInfo, CPoint ptMouse )
{
	...
	{		
		...
		if (g_Option.m_nSmallPowerUps) {
			if (pItem->dwItemKind3 == IK3_VENDING)
				p2DRender->RenderTexture2(pInfo->pt, pp->second.m_pTexture, 0.75, 0.75, D3DCOLOR_ARGB(255, 255, 255, 255));
			else
				p2DRender->RenderTexture2(pInfo->pt, pp->second.m_pTexture, 0.75, 0.75, color);
		}
		else {
			if (pItem->dwItemKind3 == IK3_VENDING)
				p2DRender->RenderTexture2(pInfo->pt, pp->second.m_pTexture, 1, 1, D3DCOLOR_ARGB(255, 255, 255, 255));
			else
				p2DRender->RenderTexture2(pInfo->pt, pp->second.m_pTexture, 1, 1, color);
		}
#else //__RENDER_SMALL_POWERUPS
		if (pItem->dwItemKind3 == IK3_VENDING)
			p2DRender->RenderTexture2(pInfo->pt, pp->second.m_pTexture, 1, 1, D3DCOLOR_ARGB(255, 255, 255, 255));
		else
			p2DRender->RenderTexture2(pInfo->pt, pp->second.m_pTexture, 1, 1, color);
...
#ifdef __RENDER_SMALL_POWERUPS
		if (g_Option.m_nSmallPowerUps) {
			RenderSmallOptBuffTime(p2DRender, pInfo->pt, ct, D3DCOLOR_XRGB(240, 240, 0));
		}
		else {
...
#ifdef __RENDER_SMALL_POWERUPS
		if (g_Option.m_nSmallPowerUps) {
			RenderSmallOptBuffTime(p2DRender, pInfo->pt, ts, D3DCOLOR_XRGB(240, 240, 0));
		}
		else {
...
#ifdef __RENDER_SMALL_POWERUPS
	if (g_Option.m_nSmallPowerUps) {
		SetRect(&rectHittest, pInfo->pt.x, pInfo->pt.y, pInfo->pt.x + 24, pInfo->pt.y + 24);
	}
	else {	
	...
	if( rc.PtInRect( ptMouse ) )
	{
		...
		if( pItem->dwItemRare == 102 )
			...
			strEdit.AddString( pItem->szName, D3DCOLOR_XRGB( 46, 112, 169 ), ESSTY_BOLD );
#ifdef __PREMIUM
		else if (pItem->dwItemKind3 == IK3_PREMIUM)
			strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(189, 128, 51), ESSTY_BOLD);
#endif //__PREMIUM
#ifdef __ANARCHY_SYSTEM
		else if (pItem->dwItemKind3 == IK3_ANARCHY)
			strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
#endif //__ANARCHY_SYSTEM
		else
...
#ifdef __VENDING_NPC
		else if (pItem->dwItemKind3 == IK3_VENDING)
		{
			BUFFICONRECT_INFO info;
			CString strPercent;
			CopyRect(&info.rc, &rectHittest);
			info.dwID = pItem->dwID;
			m_dwNpcVendorItem = pItem->dwID;
			m_rcCheck.push_back(info);
		}
#endif // __VENDING_NPC
		...
		else if (pItem->dwItemKind3 == IK3_GUILD_BUFF)
			bItemKind3 = TRUE;
...
#ifdef __AEGON_THEME_SWITCHER
			strEdit.AddString(strTemp, dwColorText);
#else
...
#ifdef __AEGON_THEME_SWITCHER
			strEdit.AddString(str, dwColorHighlightBlue);
#else
		...
		if(pItem->dwID != II_SYS_SYS_SCR_PET_FEED_POCKET && pItem->dwID != II_PET_EGG)
#ifdef __AEGON_THEME_SWITCHER
			strEdit.AddString(str, dwColorText);
#else
...
//	int nTexture = 0;
	...
	strEdit.AddString("Scroll of Amplification", dwColorHighlightBlue, ESSTY_BOLD);
#else
	...
	CString strTemp;
#ifdef __AEGON_THEME_SWITCHER
	strEdit.AddString(prj.GetText(TID_GAME_EXPITEM_TOOLTIP), dwColorText);
#else
...
#ifdef __AEGON_THEME_SWITCHER
	strEdit.AddString(prj.GetText(TID_GAME_EXPITEM_TOOLTIP1), dwColorText);
#else
	...
	strEdit.AddString( '\n' );
#ifdef __AEGON_THEME_SWITCHER
	strEdit.AddString(prj.GetText(TID_GAME_EXP_COUTMSG0), dwColorText);//, 0xff99cc00 );
#else
	...
	for( int i = 0 ; i < nExpCount ; ++i )
	{
		...
		strEdit.AddString( "\n  " );
#ifdef __AEGON_THEME_SWITCHER
		strEdit.AddString(prj.GetText(dwMsg), dwColorText);
#else
...
#ifdef __AEGON_THEME_SWITCHER
		strEdit.AddString(strTime, dwColorText);
#else
...
void CWndWorld::RenderSMBuff( C2DRender *p2DRender, BUFFICON_INFO* pInfo, CPoint ptMouse )
{
	...
	for( int i = 0; i < SM_MAX ; ++i )
	{
		...
		if (g_Option.m_nHidePU && g_pPlayer->m_dwSMTime[i] >= 60)
			continue;
...
#ifdef __RENDER_SMALL_POWERUPS
						if (g_Option.m_nSmallPowerUps) {
							p2DRender->RenderTexture2( pInfo->pt, m_dwSMResistItemTexture[nResistTexture], 0.75, 0.75, D3DCOLOR_ARGB(m_nSMAlpha[i], 255, 255, 255));
						}
						else {
...
#ifdef __RENDER_SMALL_POWERUPS
						if (g_Option.m_nSmallPowerUps) {
							p2DRender->RenderTexture2(pInfo->pt, m_dwSMItemTexture[i], 0.75, 0.75, D3DCOLOR_ARGB(m_nSMAlpha[i], 255, 255, 255));
						}
						else {
...
#ifdef __RENDER_SMALL_POWERUPS
						if (g_Option.m_nSmallPowerUps) {
							p2DRender->RenderTexture2(pInfo->pt, m_dwSMResistItemTexture[nResistTexture], 0.75, 0.75, D3DCOLOR_ARGB(192, 255, 255, 255));
						}
						else {
...
#ifdef __RENDER_SMALL_POWERUPS
						if (g_Option.m_nSmallPowerUps) {
							p2DRender->RenderTexture2(pInfo->pt, m_dwSMItemTexture[i], 0.75, 0.75, D3DCOLOR_ARGB(192, 255, 255, 255));
						}
						else {
...
#ifdef __PREMIUM
				if (pItem->dwItemKind3 == IK3_PREMIUM)
				{
					g_WndMng.PutDestParam(pItem->dwDestParam[0], pItem->dwDestParam[1], pItem->nAdjParamVal[0], pItem->nAdjParamVal[1], strEdit);
					g_WndMng.PutDestParam(pItem->dwDestParam[2], 0, pItem->nAdjParamVal[2], 0, strEdit);
				}
#endif //__PREMIUM
#endif //__LEESE_SM_DESC
...
void CWndWorld::RenderBuff(C2DRender *p2DRender)
{
	...
	for( i=0; i<MAX_SKILLINFLUENCE; i++)
	{
		...
		ItemProp* pBuffProp = pBuff->GetProp();
		if (pBuffProp && pBuffProp->dwItemKind3 == IK3_GUILD_BUFF && g_Option.m_bHideGuildBuffs)
			continue;
#endif
		...
		if( wId  )	
		{
			...
			if( wType == BUFF_ITEM )
			{
				...
				if( pItemProp->dwItemKind3 == IK3_EXP_RATE )
				{
					BOOL bHideExpIcon = g_Option.m_nHidePU;
					if (bHideExpIcon && pBuff->GetTotal() > 0)
					{
						DWORD dwOddTime = pBuff->GetTotal() - (g_tmCurrent - pBuff->GetInst());
						bHideExpIcon = (dwOddTime < 60 * 1000) ? FALSE : TRUE;
					}

					if( bHasExpRender == TRUE || bHideExpIcon)
						continue;
					else
					{
...
#endif // __NEW_STACKABLE_AMPS
			{
				...
				if (wType != BUFF_SKILL)
				{
					...
					{
						dwOddTime = pBuff->GetTotal() - (g_tmCurrent - pBuff->GetInst());
						bFlash = (dwOddTime < 60 * 1000) ? TRUE : FALSE;
					}
					if (!g_Option.m_nHidePU || bFlash || pBuff->GetTotal() == 999999999 || pBuff->GetType() == BUFF_PET || (pItemProp->dwSkillTime == 999999999 && pItemProp->dwAbilityMin == (DWORD)-1)
#ifdef __VENDING_NPC
						|| (pItemProp && pItemProp->dwItemKind3 == IK3_VENDING)
#endif //__VENDING_NPC
						)
					{
...
#ifdef __RENDER_SMALL_POWERUPS
		if (g_Option.m_nSmallPowerUps) {
			RenderSmallOptBuffTime(p2DRender, pInfo->pt, timeSpan, D3DCOLOR_XRGB(240, 240, 0));
		}
		else {
...
#ifdef __CUSTOM_HP_BAR
	CPoint Lpoint = CPoint((GetClientRect().Width() - 200) / 2 - 20, 50);
#else // __CUSTOM_HP_BAR
...
#ifdef __PREMIUM
				else if (pItem->dwID == II_SCR_PREMIUM_01)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(189, 128, 51), ESSTY_BOLD);
				else if (pItem->dwID == II_SCR_PREMIUM_07)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(189, 128, 51), ESSTY_BOLD);
				else if (pItem->dwID == II_SCR_PREMIUM_15)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(189, 128, 51), ESSTY_BOLD);
				else if (pItem->dwID == II_SCR_PREMIUM_30)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(189, 128, 51), ESSTY_BOLD);
				else if (pItem->dwID == II_SCR_PREM_NOTUSE)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(189, 128, 51), ESSTY_BOLD);
#endif //__PREMIUM

#ifdef __ANARCHY_SYSTEM
				else if (pItem->dwID == II_ANARCHY_BUFF01)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF02)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF03)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF04)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF05)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF06)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF07)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF08)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF09)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF10)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF11)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
				else if (pItem->dwID == II_ANARCHY_BUFF12)
					strEdit.AddString(pItem->szName, D3DCOLOR_XRGB(31, 38, 147), ESSTY_BOLD);
#endif //__ANARCHY_SYSTEM
				else
					strEdit.AddString( pItem->szName, 0xff2fbe6d, ESSTY_BOLD );
...
#ifdef __AEGON_THEME_SWITCHER
				strEdit.AddParsingString(str.GetString(), dwColorText);
#else
				strEdit += str;
#endif
...
#ifdef __RENDER_SMALL_POWERUPS
void CWndWorld::RenderSmallOptBuffTime(C2DRender* p2DRender, CPoint& point, CTimeSpan& ct, DWORD dwColor)
{
	if (g_Option.m_bVisibleBuffTimeRender)
	{
		CString str;
		...
		if (ct.GetDays() >= 1)
		{
			str.Format("%.1dd", static_cast<int>(ct.GetDays()));
			strEditOut.SetParsingString(str);
			strEditOut.SetStyle(ESSTY_BOLD);
			strEditOut.SetColor(0xffffffff);
			p2DRender->TextOut_EditString(point.x + 3, point.y + 24, strEditOut, 0, 0, 2);
			strEditOut.ClearStyle(ESSTY_BOLD);
		}
		else
		if (ct.GetHours() >= 1)
		{
			str.Format("%.1dh", ct.GetHours());
			strEditOut.SetParsingString(str);
			strEditOut.SetStyle(ESSTY_BOLD);
			strEditOut.SetColor(0xffffffff);
			p2DRender->TextOut_EditString(point.x + 3, point.y + 24, strEditOut, 0, 0, 2);
...
CWndFarmCounter::CWndFarmCounter()
{
	LoadPresets();
	...
	const static CString str[] = { "cntrefresh.png", "cntclose.png", "killcount.png", "deathcount.png", "itm_GolGolSeed.dds", "Itm_SysSysScrPerin.png", "cnttimer.png",
		"cntwiki.png", "cntactivate.png", "cntpause.png", "farmtimer.png", "cntdamage.png" };

	for (int i = 0; i < sizeof(str) / sizeof(str[0]); ++i)
	{
		m_pTexture[i] = m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ITEM, str[i]), 0xffff00ff);
	}
...
CWndFarmCounter::~CWndFarmCounter()
{
	for (int i = 0; i < MAX_CNTTEXTURE; i++)
	{
		m_pTexture[i] = NULL;
	}
	for (int i = 0; i < prj.m_vecCountItems.size(); i++)
	{
		prj.m_vecCountItems[i].nCountItems = 0;
	}
}

void CWndFarmCounter::OnDraw(C2DRender* p2DRender)
{
	if (iTotalMoney >= PERIN_VALUE)
	{
		int nPerinNum = (int)(iTotalMoney / PERIN_VALUE);
		int nPenya = (int)(iTotalMoney - (nPerinNum * PERIN_VALUE));

		nInt[1] += nPerinNum;
		iTotalMoney = __int64(nPenya);
	}

	CRect crBoard;
	crBoard.right = WINDOWX;
	crBoard.bottom = WINDOWY + prj.m_vecCountItems.size() * 25;

#if __VER >= 19
	p2DRender->RenderFillRect(crBoard, D3DCOLOR_ARGB(200, 0, 0, 0));
	p2DRender->RenderRect(crBoard, D3DCOLOR_ARGB(255, 255, 194, 64));
#else
	p2DRender->RenderFillRect(crBoard, D3DCOLOR_ARGB(220, 250, 234, 234));
	p2DRender->RenderRect(crBoard, D3DCOLOR_ARGB(255, 200, 166, 148));
#endif

	// Buttons
	CPoint	pt;
	pt.SetPoint(12, 6);
	p2DRender->RenderTexture(pt, dwStartTime == NULL ? m_pTexture[8] : m_pTexture[9], 255, 0.5f, 0.5f);
	pt.SetPoint(35, 6);
	p2DRender->RenderTexture(pt, m_pTexture[7], 255, 0.5f, 0.5f);
	pt.SetPoint(58, 6);
	p2DRender->RenderTexture(pt, m_pTexture[0], 255, 0.5f, 0.5f);
	pt.SetPoint(81, 6);
	p2DRender->RenderTexture(pt, m_pTexture[1], 255, 0.5f, 0.5f);

	// Stats
	pt.SetPoint(12, 33);
	p2DRender->RenderTexture(pt, m_pTexture[10], 255, 0.6f, 0.6f);

	pt.SetPoint(12, 58);
	if (dwStartTime == NULL)
	{
		p2DRender->RenderTexture(pt, m_pTexture[6], 255, 0.6f, 0.6f);
	}
	else
	{
		m_pTexture[6]->RenderRotate(&g_Neuz.m_2DRender, pt, oldpos, TRUE, 255, 0.6f, 0.6f);
		oldpos += 0.02f;
		if (oldpos >= 7.0f)
			oldpos = 1.0f;
	}
	...
	pt.SetPoint(12, 84);
	p2DRender->RenderTexture(pt, m_pTexture[2], 255, 0.6f, 0.6f);
	pt.SetPoint(12, 109);
	p2DRender->RenderTexture(pt, m_pTexture[3], 255, 0.6f, 0.6f);
	pt.SetPoint(12, 132);
	p2DRender->RenderTexture(pt, m_pTexture[11], 255, 0.6f, 0.6f);
	pt.SetPoint(10, 156);
	p2DRender->RenderTexture(pt, m_pTexture[4], 255, 0.75f, 0.75f);
	pt.SetPoint(12, 183);
	p2DRender->RenderTexture(pt, m_pTexture[5], 255, 0.6f, 0.6f);

	DWORD dwColLine, dwColText, dwShadoww;

#if __VER >= 19
	dwColLine = 0x80ffffff;
	dwColText = 0xffffffff;
#else
	dwColLine = 0x80000000;
	dwColText = 0xff000000;
#endif

	int linepos = 30;

	g_Neuz.m_2DRender.RenderLine(CPoint(WINDOWX - 2, linepos - 1), CPoint(0, linepos - 1), dwColLine);
	for (int i = 0; i < 8; i++)
	{
		g_Neuz.m_2DRender.RenderLine(CPoint(WINDOWX - 2, linepos), CPoint(0, linepos), dwColLine);
		linepos += 25;
	}

	// Custom Items
	for (auto it : prj.m_vecCountItems)
	{
		ItemProp* pItem = it.pCountItems;
		if (pItem)
		{
			if (strlen(pItem->szIcon))
			{
				pt.SetPoint(12, linepos - 20);
				CString szIcon = pItem->szIcon;
				if (pItem->dwItemKind3 == IK3_EGG && pItem->dwID != II_PET_EGG)
					szIcon.Replace(".", "_00.");
				CTexture* pIcon = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, szIcon), 0xffff00ff);
				p2DRender->RenderTexture(pt, pIcon, 255, 0.6f, 0.6f);
				g_Neuz.m_2DRender.RenderLine(CPoint(WINDOWX - 2, linepos), CPoint(0, linepos), dwColLine);
				linepos += 25;
			}
		}
	...
	p2DRender->TextOut(40, y, str, dwColText);
	...
	{
		CTimeSpan ct(((GetTickCount() - dwStartTime) / 1000));
		CString szNumber;
		szNumber.Format("%.2d:%.2d:%.2d", ct.GetHours(), ct.GetMinutes(), ct.GetSeconds());
		str = szNumber;
	}
	p2DRender->TextOut(40, y += 25, str, dwColText);

	str = Separator(nInt[2], ',');
	p2DRender->TextOut(40, y += 25, str, dwColText);
	str = Separator(nInt[3], ',');
	p2DRender->TextOut(40, y += 25, str, dwColText);
	str = Separator64(iTotalDamage, ',');
	p2DRender->TextOut(40, y += 25, str, dwColText);
	str = Separator64(iTotalMoney, ',');
	p2DRender->TextOut(40, y += 25, str, dwColText);
	str = Separator(nInt[1], ',');
	p2DRender->TextOut(40, y += 25, str, dwColText);

	y = 210;
	for (auto it : prj.m_vecCountItems)
	{
		str = Separator(it.nCountItems, ',');
		p2DRender->TextOut(40, y, str, dwColText);
		y += 25;
	}
...
BOOL CWndFarmCounter::Process()
{
	return true;
}

void CWndFarmCounter::SetWndRect(CRect rectWnd, BOOL bOnSize)
{
	AdjustMinRect(&rectWnd, WINDOWX, WINDOWY);
	AdjustMaxRect(&rectWnd, WINDOWX, WINDOWY + prj.m_vecCountItems.size() * 25);
	CWndNeuz::SetWndRect(rectWnd, bOnSize);

}
void CWndFarmCounter::OnLButtonUp(UINT nFlags, CPoint point)
{
	CPoint ptMouse = GetMousePoint();

	if (CRect(12, 0, 34, 28).PtInRect(ptMouse))
	{
		if (dwStartTime == NULL)
		{
			dwStartTime = timeGetTime();
			time = CTime::GetCurrentTime();
			g_WndMng.PutString(("Farming stats activated"), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
		}
		else
		{
			for (int i = 0; i < 4; i++)
			...
			iTotalDamage = 0;
			iTotalMoney = 0;
			g_WndMng.PutString(("Farming stats resetted."), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
		}

	}
	else if (CRect(35, 0, 57, 28).PtInRect(ptMouse))
	{
		if (dwStartTime == NULL)
		{
			if (prj.m_vecCountItems.size() < MAX_CNTITEMS)
			{
				g_WndMng.CreateApplet(APP_WIKI_ITEMS);
				CWndWikiItems* pWndWIKI = (CWndWikiItems*)g_WndMng.GetWndBase(APP_WIKI_ITEMS);
				if (pWndWIKI)
				{
					pWndWIKI->bWikiSelectMode = TRUE;
					g_WndMng.PutString(("Select items which you want to count..."), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
				}
			}
			else
			{
				g_WndMng.PutString(("You can have only 10 items in this list!"), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
			}
		}
		else
		{
			g_WndMng.PutString(("You cant add new items in the list while farm counter activated"), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
		}
	}
	else if (CRect(58, 0, 80, 28).PtInRect(ptMouse))
	{
		for (int i = 0; i < 4; i++)
		{
			nInt[i] = 0;
		}
		prj.m_vecCountItems.clear();
		dwStartTime = NULL;
		time = NULL;
		oldpos = 0.0f;
		iTotalDamage = 0;
		iTotalMoney = 0;
		SavePresets();
		g_WndMng.PutString(("All parameters resetted!"), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));

		CRect rect = GetWindowRect(TRUE);
		rect.bottom = WINDOWY + prj.m_vecCountItems.size() * 25;
		SetWndRect(rect);
	}
	else if (CRect(81, 0, 104, 28).PtInRect(ptMouse))
	{
		//g_WndMng.PutString(("Window closed"), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
		Destroy();
	}
}

void CWndFarmCounter::OnInitialUpdate()
{
	CWndNeuz::OnInitialUpdate();
...
BOOL CWndFarmCounter::Initialize(CWndBase* pWndParent, DWORD)
{
	return InitDialog(g_Neuz.GetSafeHwnd(), APP_FARM_COUNTER);
}

BOOL CWndFarmCounter::OnCommand(UINT nID, DWORD dwMessage, CWndBase* pWndBase)
{
	return CWndNeuz::OnCommand(nID, dwMessage, pWndBase);
}

void CWndFarmCounter::OnSize(UINT nType, int cx, int cy)
{
	CWndNeuz::OnSize(nType, cx, cy);
}

void CWndFarmCounter::OnMouseWndSurface(CPoint point)
{

}

BOOL CWndFarmCounter::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	return CWndNeuz::OnChildNotify(message, nID, pLResult);
}
void CWndFarmCounter::DoSelectItem(ItemProp* pItemProp)
{
	if (pItemProp)
	{
		if (prj.m_vecCountItems.size() < MAX_CNTITEMS)
		{
			if (dwStartTime == NULL)
			{
				...
				if (!bAlreadyHere)
				{
					COUNT_ITEMS vec;
					vec.nCountItems = 0;
					vec.pCountItems = pItemProp;
					prj.m_vecCountItems.push_back(vec);

					str1.Format("Item added: %s", pItemProp->szName);
					g_WndMng.PutString((str1), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
					SavePresets();
				...
				{
					g_WndMng.PutString(("This item already added!"), NULL, prj.GetTextColor(TID_GAME_NAME_PET_S00));
				}
			}
			else
			{
...
void CWndFarmCounter::LoadPresets()
{
	if (g_pPlayer)
	{
		CreateDirectory("Data\\Client\\CountItems", NULL);
		//SetFileAttributes("Player\\Config\\CountItems", FILE_ATTRIBUTE_HIDDEN);
		CString str1;
		str1.Format("Data\\Client\\CountItems\\%d", g_pPlayer->m_idPlayer);
		prj.LoadCountItems(str1);
	}
}
void CWndFarmCounter::SavePresets()
{
	if (g_pPlayer)
	{
		CreateDirectory("Data\\Client\\CountItems", NULL);
