...
public:
	...
	void SetTextColor(BOOL bForce = FALSE);
...
#ifdef __AEGON_THEME_SWITCHER
	void FitTextureSizeY();
	void FitTextureSizeAndTextX();
#endif
...
#ifdef __AEGON_SCROLLBAR_STEP
	void SetScrollStep(int scrollStep) { m_nScrollStep = scrollStep; };
	int GetScrollStep() { return m_nScrollStep; };
	void SetButton1Texture(LPCTSTR fileName) { m_wndArrow1.SetTexture(m_pApp->m_pd3dDevice, fileName, TRUE); m_wndArrow1.FitTextureSize();  };
	void SetButton2Texture(LPCTSTR fileName) { m_wndArrow2.SetTexture(m_pApp->m_pd3dDevice, fileName, TRUE); m_wndArrow2.FitTextureSize();  };
	void OffsetButt1Pos(int nOffsetX, int nOffsetY);
	void OffsetButt2Pos(int nOffsetX, int nOffsetY);

	void OffsetBarSize(int nOffset);
#endif // __AEGON_SCROLLBAR_STEP
...
#ifdef __AEGON_THEME_SWITCHER
#ifdef __ITEMLINK
	void SetString(LPCTSTR pszString, DWORD dwColor = 0x00000000, DWORD dwPlayerId = 0, DWORD dwObjId = 0);
	void AddString(LPCTSTR pszString, DWORD dwColor = 0x00000000, DWORD dwPStyle = 0x00000001, DWORD dwPlayerId = 0, DWORD dwObjId = 0);
#else //__ITEMLINK
...
public:
	void SetPlaceholderString(LPCTSTR pszString, DWORD dwColor = COLOR_PLACEHOLDER);
private:
...
public:
	void ForceSetEditString(CEditString strEdit);
...
private:
	...
	int InterpriteScript(CScript& script, CPtrArray& ptrArray, BOOL bWiki = FALSE, DWORD dataNum = 0);
...
public:
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	CTexture* m_AchievementStatusIcon[4];
#endif // __AEGON_ACHIEVEMENT_SYSTEM
	...
	int   GetCount() const;
#ifdef _WIN64
	INT_PTR GetItemData(int nIndex) const;
#else
	DWORD GetItemData(int nIndex) const;
#endif
	void* GetItemDataPtr(int nIndex) const;
#ifdef _WIN64
	int   SetItemData(int nIndex, INT_PTR dwItemData);
#else
...
#if __VER >= 15 // __IMPROVE_QUEST_INTERFACE
#ifdef _WIN64
	INT_PTR GetItemData2(int nIndex) const;
#else
...
#ifdef _WIN64
	int SetItemData2(int nIndex, INT_PTR dwItemData);
#else
