...
void CWndMapEx::OnDraw( C2DRender* p2DRender )
{
	...
	for( int i = 0; i < g_Party.GetSizeofMember(); ++i )
	{
		if( g_pPlayer->m_idPlayer != g_Party.GetPlayerId( i ) )
		{
			if( g_Party.m_aMember[ i ].m_bRemove == FALSE )
			{
				...
				{
#ifdef __AEGON_FIXES
					DWORD dwWorldID = g_Party.m_aMember[i].m_dwWorldID;
#endif
	...
	RenderPlayerPosition(p2DRender, m_pPCArrowTexture, g_pPlayer->GetPos(), g_Neuz.m_camera.m_vPos, g_pPlayer->GetName(), 0xff7fff7f, dwWorldID);
#else
	...
	case NORMAL:
		{
			...
			{
#ifdef __CHECKLOAD
				byLocationID = CContinent::GetInstance()->getWorld(g_pPlayer);
#else //__CHECKLOAD
	...
	for( int i = 0; i < static_cast< int >( pMapMonsterInformationPack->GetNumber() ); ++i )
	{
		...
		if( rectRevisedMonsterIconPosition.PtInRect( pointMouse ) == TRUE )
		{
			...
			for( int j = 0; j < pMapMonsterInformation->GetMonsterIDNumber(); ++j )
			{
...
#ifdef __AEGON_THEME_SWITCHER
				strMonsterInformationToolTip.AddString(prj.GetText(TID_GAME_MONSTER_INFORMATION_LEVEL), dwColorHighlightBlue);
#else
...
#ifdef __AEGON_THEME_SWITCHER
				strMonsterInformationToolTip.AddString(strTemp, dwColorHighlightBlue);
#else
				...
				{
#ifdef __AEGON_THEME_SWITCHER
					strMonsterInformationToolTip.AddString(strTemp, dwColorHighlightRed);
#else
				...
				{
#ifdef __AEGON_THEME_SWITCHER
					strMonsterInformationToolTip.AddString(strTemp, dwColorText);
#else
			...
			{
#ifdef __AEGON_THEME_SWITCHER
				strMonsterInformationToolTip.AddString(prj.GetText(TID_GAME_DROP_NONE), dwColorText);
#else
...
//-----------------------------------------------------------------------------
#ifdef __AEGON_FIXES
void CWndMapEx::RenderPlayerPosition(C2DRender* p2DRender, CTexture* pArrowTexture, const D3DXVECTOR3& vPlayerPosition, const D3DXVECTOR3& vCameraPosition, const CString& strName, DWORD dwNameColor, DWORD dwWorldID)
#else
...
#ifdef __AEGON_FIXES
	BYTE byPlayerLocationID = GetMapArea(vPlayerPosition, dwWorldID);
#else
...
//-----------------------------------------------------------------------------
BYTE CWndMapEx::GetMapArea( const D3DXVECTOR3& vPlayerPosition, DWORD dwWorldID)
{

	CWndComboBox* pWndComboBoxMapCategory = static_cast<CWndComboBox*>(GetDlgItem(WIDC_COMBOBOX_MAP_CATEGORY));
	BOOL bTown = FALSE;
	DWORD byPlayerLocationID = CONT_NODATA;

	if (pWndComboBoxMapCategory && pWndComboBoxMapCategory->GetCurSel() == 2)
	{
		bTown = TRUE;
	}
...
#ifdef __AEGON_FIXES
		if (dwWorldID != WI_WORLD_NONE && dwWorldID != WI_WORLD_MADRIGAL)
		{
			byPlayerLocationID = CContinent::GetInstance()->worldIdToMCD(dwWorldID, false);
		}
#endif
	}
...
void CWndMapEx::addCheckLoad(std::vector<D3DXVECTOR3>&& vec, const unsigned long worldId)
{
	...
	if (pWndComboBoxNpcName)
		pWndComboBoxNpcName->SetVisible(false);
	if (pWndMonsterInfoCheckBox)
		pWndMonsterInfoCheckBox->SetVisible(false);

	MapComboBoxDataVector* pMapComboBoxDataVector = prj.m_MapInformationManager.GetMapNameVector();
	if (!pMapComboBoxDataVector)
		return;
	for (MapComboBoxDataVector::iterator Iterator = pMapComboBoxDataVector->begin(); Iterator != pMapComboBoxDataVector->end(); ++Iterator)
	{
