...
void CWndConfirmSell::OnDraw(C2DRender* p2DRender)
{
	...
	if (m_pItemElem->GetProp()->dwItemKind3 == IK3_TICKET && (m_pItemElem->m_dwKeepTime > 0 || m_pItemElem->IsFlag(CItemElem::expired)))
	{
		BuyGold = 1;
	}
	...
	if (m_pItemElem->IsFlag(CItemElem::binds) && (m_pItemElem->m_dwItemId != II_GEN_MAT_ORICHALCUM01_1 && m_pItemElem->m_dwItemId != II_GEN_MAT_MOONSTONE_1) && m_pItemElem->GetProp()->dwFlag != IP_FLAG_EQUIP_BIND)
	{
		BuyGold = 1;
	}
...
BOOL CWndConfirmSell::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	...
	{
#ifdef __fmod
		SoundSystemClass::playSound(SND_INF_TRADE);
#else
...
void CWndConfirmBuy::OnChangeBuyCount(DWORD dwBuy)
{
...
#endif // __VENDOR_UPDATE
	{
		...
		dwCost = m_pItemElem->GetCurrencyCost(m_nBuyType);
...
BOOL CWndConfirmBuy::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
...
#ifdef __VENDING_NPC
		if (m_bVendor && nBuyNum > m_pItemElem->GetExtra())
		{
			g_WndMng.PutString("NPC 先텄臥밞뼜㎴衲?");
			nBuyNum = m_pItemElem->GetExtra();
		}
#endif // __VENDING_NPC
	...
	case WIDC_MAX:
	{
		...
		if (m_bVendor)
		{
			OnChangeBuyCount(m_pItemElem->GetExtra());
			break;
		}
#endif // __VENDING_NPC
#ifdef __AEGON_MAX_BUY_CHANGE
		int nCost;
		if (m_nBuyType == 0) 
		{
			nCost = m_pItemElem->GetCost();
			int goldCount = g_pPlayer->GetGold();
			nBuyNum = floor(goldCount / nCost);
			
		}
		else 
		{
#ifdef __SQK_MULTIALTERNATEPRICE
			nCost = m_pItemElem->GetCurrencyCost(m_nBuyType);
#else //__SQK_MULTIALTERNATEPRICE
			nCost = m_pItemElem->GetChipCost();
#endif //__SQK_MULTIALTERNATEPRICE
			DWORD currencyCount = g_pPlayer->m_Inventory.GetItemCount(m_nBuyType);
			nBuyNum = floor(currencyCount / nCost);
		}
		if (nBuyNum > 9999)
			nBuyNum = 9999;
		if (nBuyNum <= 0)
...
void CWndConfirmBuy::OnOK()
{
...
#ifdef __VENDOR_UPDATE
			CString strTmp;
			strTmp.Format(prj.GetText(TID_GAME_CANNTBUY_CURRENCY), prj.GetItemProp(m_nBuyType)->szName );
			g_WndMng.OpenMessageBox(strTmp.GetString());
			//g_WndMng.OpenMessageBox(_T(prj.GetText(TID_GAME_CANNTBUY_CURRENCY), prj.GetItemProp(lpCharacter)->szName));	// 칩이 부족합니다.	
#else // __VENDOR_UPDATE
	...
	if (m_bVendor)
	{
		...
		if (nBuy < 1)
			return;
		if (nBuy > m_pItemElem->GetExtra())
			nBuy = m_pItemElem->GetExtra();
		if (pWndVendor && pWndVendor->m_pVendor)
			g_DPlay.SendBuyPVendorItem(pWndVendor->m_pVendor->GetId(), m_nVendorPos, m_dwItemId, nBuy);
		return;
	}
#endif // __VENDING_NPC
...
void CWndShop::OnDraw(C2DRender* p2DRender)
{
	...
	if (nIndex >= 0)
	{
		...
		if (pItemBase && m_pMover)
		{
			...
			if (lpCharacter)
			{
				...
				{
#ifdef __SQK_MULTIALTERNATEPRICE
					dwCost += pItemBase->GetCurrencyCost(lpCharacter->m_nVenderType);
#else //__SQK_MULTIALTERNATEPRICE
	...
	if (lpCharacter->m_nVenderType != 0)
		strCurrency = prj.GetItemProp(lpCharacter->m_nVenderType)->szName;

	if (lpCharacter && lpCharacter->m_nVenderType != 0)
		strName.Format(strCurrency + " Shop");
	else
		strName.Format("Penya Shop");
	...
	CTexture* pTexture = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, pItem->szIcon), COLOR_ITEM_BG);
	if (pTexture != NULL)
		pTexture->Render(p2DRender, CPoint(wndCtrl->rect.left, wndCtrl->rect.top));
#endif // __VENDOR_UPDATE

#ifdef __SHOP_SEARCH
	CWndEdit* m_pEditFilter = (CWndEdit*)GetDlgItem(WIDC_EDIT);
...
BOOL CWndShop::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	...
	if (message == WIN_ITEMDROP)
	{
		...
		if (lpShortcut->m_dwType == ITYPE_ITEM && lpShortcut->m_dwData != 0)
		{
			...
			{
				BOOL bResult = TRUE;
				if (lpShortcut->m_dwData == 0 || ((CItemElem*)lpShortcut->m_dwData)->IsQuest())
				{
					bResult = FALSE;
				}
				...
				if (bResult && pWndFrame->GetWndId() == APP_INVENTORY)
				{
...
#ifdef __NPC_QUICK_SELL
							if (pProp->dwReferStat1 != NULL_ID && pProp->dwReferStat1 >= WEAPON_UNIQUE && pProp->dwLimitLevel1 >= 120)
							{
								bWarning = TRUE;
							}
#endif // __NPC_QUICK_SELL
					...
					if (FALSE == g_pPlayer->m_Inventory.IsEquip(((CItemElem*)lpShortcut->m_dwData)->m_dwObjId))
					{
						...
						ItemProp* pProp = prj.GetItemProp(((CItemElem*)lpShortcut->m_dwData)->m_dwItemId);
						bool bInstant = false;
						if (pProp && ((CItemElem*)lpShortcut->m_dwData)->m_nAbilityOption == 0 && ((CItemElem*)lpShortcut->m_dwData)->IsBinds() == FALSE)
						{
							if (pProp->dwPackMax == 1 && ((CItemElem*)lpShortcut->m_dwData)->m_nItemNum == 1)
							{
								bInstant = true;

								//g_DPlay.SendSellItem(((CItemElem*)lpShortcut->m_dwData)->m_dwObjId, 1);	// Instant Sell 
	...
	else if (message == WNM_ITEMCLICK)
	{
		...
		{
			CWndBase* pWndInventory = g_WndMng.GetWndBase(APP_INVENTORY);
			if (pWndInventory)
			{
				SHORTCUT shortcut;
				shortcut.m_pFromWnd = this;
#if _WIN64
				shortcut.m_dwData = reinterpret_cast<INT_PTR>(pLResult);
#else
				shortcut.m_dwData = (DWORD)pLResult;
#endif
				pWndInventory->OnChildNotify(WIN_ITEMDROP, 11, (LRESULT*)&shortcut);
			}
		}
	}
#endif //__QUICK_MOVE_ITEMS
	...
	case WIDC_BUTTON:
	{
		SAFE_DELETE(m_pWndWarning);
		SAFE_DELETE(m_pWndBuyBack);
		m_pWndBuyBack = new CWndBuyBack;
		m_pWndBuyBack->Initialize(NULL);
	}
	break;
#endif // __BUYBACK
#ifdef __NPC_QUICK_SELL
	case WIDC_BUTTON1:
	{
		...
		SAFE_DELETE(m_pWndBuyBack);
#endif // __ENH_BUYBACK
		m_pWndQuickSell = new CWndQuickSell;
		m_pWndQuickSell->Initialize(this);
	} 
	break;
#endif // __NPC_QUICK_SELL
	}
...
{
	if (m_pWndConfirmSell == pWndChild)
	{
		SAFE_DELETE(m_pWndConfirmSell);
		pWndChild = NULL;
	}
	else if (m_pWndWarning == pWndChild)
	{
		SAFE_DELETE(m_pWndWarning);
		pWndChild = NULL;
	}
#ifdef __BUYBACK
	else if (m_pWndBuyBack == pWndChild)
	{
		SAFE_DELETE(m_pWndBuyBack);
		pWndChild = NULL;
	}
...
#ifdef __NPC_QUICK_SELL
	else if (m_pWndQuickSell == pWndChild)
	{
		SAFE_DELETE(m_pWndQuickSell);
		pWndChild = NULL;
	}
...
#ifdef __NPC_QUICK_SELL
CWndQuickSell::CWndQuickSell()
{
}
CWndQuickSell::~CWndQuickSell()
{
}
void CWndQuickSell::OnDraw(C2DRender* p2DRender)
{
	__int64 nTotalCost = 0;

	for (auto id : m_wndctrlMerit.GetItemList())
	{
		CItemElem* inv_item = g_pPlayer->m_Inventory.GetAtId(id);
		if (!inv_item) return;
		ItemProp* prop = prj.GetItemProp(inv_item->m_dwItemId);
		if (!prop) return;
		if (inv_item->IsBinds())
			nTotalCost += 1;
		else
			nTotalCost += (prop->dwCost / 4) * inv_item->m_nItemNum;
	}

	CString strTmp;
	strTmp.Format("%I64d", nTotalCost);

	CWndStatic* pWndStaticMoney = (CWndStatic*)GetDlgItem(WIDC_STATIC1);
	if (pWndStaticMoney) 
	{
		pWndStaticMoney->SetTitle(strTmp);
		pWndStaticMoney->AddWndStyle(WSS_MONEY);
	}
}
void CWndQuickSell::OnInitialUpdate()
{
	CWndNeuz::OnInitialUpdate();
	...
	if (g_pPlayer)
	{
		...
		for (int i = 0; i < g_pPlayer->m_Inventory.GetMax(); i++)
		{
			...
			if (IsUsableItem(pItemElem))
			{
				...
				if (pProp->dwItemKind1 != IK1_WEAPON && pProp->dwItemKind1 != IK1_ARMOR && pProp->dwItemKind2 != IK2_JEWELRY)
					continue;

				if (pProp->dwItemKind1 == IK1_WEAPON && pProp->dwReferStat1 != WEAPON_GENERAL)
					continue;

				if (pProp->dwItemKind1 == IK1_ARMOR)
				{
					if (pProp->dwReferStat1 == ARMOR_SET)
						continue;
					if (pProp->dwReferStat1 == WEAPON_UNIQUE)
						continue;
					if (pProp->dwReferStat1 == WEAPON_ULTIMATE)
						continue;
					if (pProp->dwItemKind3 != IK3_SUIT && pProp->dwItemKind3 != IK3_HELMET && pProp->dwItemKind3 != IK3_GAUNTLET 
						&& pProp->dwItemKind3 != IK3_BOOTS && pProp->dwItemKind3 != IK3_SHIELD && pProp->dwItemKind3 != IK3_NECKLACE 
						&& pProp->dwItemKind3 != IK3_RING && pProp->dwItemKind3 != IK3_EARRING)
						continue;
				}

				if (g_pPlayer->m_Inventory.IsEquip(i))
					continue;
				...
				if (CTax::GetInstance()->IsApplyTaxRate(g_pPlayer, pItemElem))
					nTax = (int)(nGold * CTax::GetInstance()->GetSalesTaxRate(g_pPlayer));
				nGold -= nTax;
...
BOOL CWndQuickSell::Initialize(CWndBase* pWndParent, DWORD)
{
	return CWndNeuz::InitDialog(g_Neuz.GetSafeHwnd(), APP_QUICK_SELL, 0, CPoint(0, 0), pWndParent);
}

BOOL CWndQuickSell::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	CWndButton* pWndUpped = (CWndButton*)GetDlgItem(WIDC_CHECK1);
	CWndButton* pWndStatted = (CWndButton*)GetDlgItem(WIDC_CHECK2);
	CWndButton* pWndAwaked = (CWndButton*)GetDlgItem(WIDC_CHECK3);
	CWndButton* pWndRarety = (CWndButton*)GetDlgItem(WIDC_CHECK4);
	CWndButton* pWndElement = (CWndButton*)GetDlgItem(WIDC_CHECK5);

	BOOL bUppedItems = FALSE;
	BOOL bStattedItems = FALSE;
	BOOL bAwakedItems = FALSE;
	BOOL bRaretyItems = FALSE;
	BOOL bElementItems = FALSE;


	if (pWndUpped) 
	{
		bUppedItems = pWndUpped->GetCheck() == 0 ? FALSE : TRUE;
	}
	if (pWndStatted)
	{
		bStattedItems = pWndStatted->GetCheck() == 0 ? FALSE : TRUE;
	}
	if (pWndAwaked)
	{
		bAwakedItems = pWndAwaked->GetCheck() == 0 ? FALSE : TRUE;
	}
	if (pWndRarety)
	{
		bRaretyItems = pWndRarety->GetCheck() == 0 ? FALSE : TRUE;
	}
	if (pWndElement)
	{
		bElementItems = pWndElement->GetCheck() == 0 ? FALSE : TRUE;
	}
	...
	case WIDC_BUTTON1:
	{
		g_DPlay.SendQuickSell(m_wndctrlMerit.GetItemList(), bUppedItems, bStattedItems, bAwakedItems, bRaretyItems, bElementItems);
	...
	case WIDC_CHECK4:
	case WIDC_CHECK5:
		m_wndctrlMerit.SetScrollBarPos(0);
		FillItemList();
		break;
#endif
...
#ifdef __AEGON_IMPROVED_QUICKSELL
void CWndQuickSell::SetFilters() 
{
	CWndComboBox* pWndItems = (CWndComboBox*)GetDlgItem(WIDC_COMBOBOX1);
	if (pWndItems) 
	{
		pWndItems->AddString("All");
		pWndItems->AddString("Weapons & Armor");
		pWndItems->AddString("Weapons");
		pWndItems->AddString("Armor");
		pWndItems->AddString("Accessories");
		pWndItems->AddString("Food");
		pWndItems->AddWndStyle(EBS_READONLY);
		pWndItems->SetCurSel(1);
	}

	CWndComboBox* pWndItemType = (CWndComboBox*)GetDlgItem(WIDC_COMBOBOX2);
	if (pWndItemType)
	{
		pWndItemType->AddString("Blue");
		pWndItemType->AddString("Green");
		pWndItemType->AddString("Ultimate");
		pWndItemType->AddWndStyle(EBS_READONLY);
		pWndItemType->SetCurSel(0);
	}

	CWndButton* pWndUpped = (CWndButton*)GetDlgItem(WIDC_CHECK1);
	if (pWndUpped) 
	{
		pWndUpped->SetCheck(0);
	}
	CWndButton* pWndStatted = (CWndButton*)GetDlgItem(WIDC_CHECK2);
	if (pWndStatted)
	{
		pWndStatted->SetCheck(0);
	}
	CWndButton* pWndAwaked = (CWndButton*)GetDlgItem(WIDC_CHECK3);
	if (pWndAwaked)
	{
		pWndAwaked->SetCheck(0);
	}
	CWndButton* pWndRarety = (CWndButton*)GetDlgItem(WIDC_CHECK4);
	if (pWndRarety)
	{
		pWndRarety->SetCheck(0);
	}
	CWndButton* pWndElement = (CWndButton*)GetDlgItem(WIDC_CHECK5);
	if (pWndElement)
	{
		pWndElement->SetCheck(0);
	}
}
void CWndQuickSell::FillItemList() 
{
	m_wndctrlMerit.RestoreItem();
	...
	if (!pWndItems || !pWndItemType || !pWndUpped)
		return;

	BOOL bUppedItems = pWndUpped->GetCheck() == 0 ? FALSE : TRUE;
	BOOL bStattedItems = pWndStatted->GetCheck() == 0 ? FALSE : TRUE;
	BOOL bAwakedItems = pWndAwaked->GetCheck() == 0 ? FALSE : TRUE;
	BOOL bRaretyItems = pWndRarety->GetCheck() == 0 ? FALSE : TRUE;
	BOOL bElementItems = pWndElement->GetCheck() == 0 ? FALSE : TRUE;


	int nItems = pWndItems->GetCurSel();
	int nItemType = pWndItemType->GetCurSel();
	...
	if (g_pPlayer)
	{
		...
		for (int i = 0; i < g_pPlayer->m_Inventory.GetMax(); i++)
		{
			...
			if (IsUsableItem(pItemElem))
			{
				...
				//item filter
				if (pProp->dwItemKind1 != IK1_WEAPON && pProp->dwItemKind1 != IK1_ARMOR && pProp->dwItemKind2 != IK2_JEWELRY && pProp->dwItemKind2 != IK2_FOOD)
					continue;


				if (pProp->dwItemKind1 == IK1_WEAPON && !bWeapons) //Weapon
					continue;

				if (pProp->dwItemKind1 == IK1_ARMOR && !bArmor) //Armor
					continue;

				if (pProp->dwItemKind2 == IK2_JEWELRY && !bAcce) //Accessories
					continue;

				if (pProp->dwItemKind2 == IK2_FOOD && !bFood) //Food
					continue;
				...
				if (pProp->dwItemKind1 == IK1_ARMOR)
				{
					...
					if (nItemType == 0 && pProp->dwReferStat1 == ARMOR_SET && pProp->dwReferStat1 != NULL_ID) //blue
						continue;

					if (nItemType == 1 && pProp->dwReferStat1 != ARMOR_SET && pProp->dwReferStat1 != NULL_ID)//green
						continue;

					if (nItemType == 2)//ultimate armor?
						continue;
				}

				if (pProp->dwItemKind1 == IK1_WEAPON && pProp->dwReferStat1 != NULL_ID)
				{
					if (nItemType == 0 && pProp->dwReferStat1 != WEAPON_GENERAL) //blue
						continue;

					if (nItemType == 1 && pProp->dwReferStat1 != WEAPON_UNIQUE)//green
						continue;

					if (nItemType == 2 && pProp->dwReferStat1 != WEAPON_ULTIMATE)//ultimate
						continue;
				}
				if (pProp->dwItemKind2 == IK2_JEWELRY)
				{
					if (nItemType == 2) //ultimate acce?
						continue;

					CSetItem* pSetItem = CSetItemFinder::GetInstance()->GetSetItemByItemId(pItemElem->m_dwItemId);
					if (pSetItem && nItemType != 1) //acce with set effect only with green filter
						continue;

					if (!pSetItem && nItemType != 0)
						continue;
				}
				if (pProp->dwItemKind2 == IK2_FOOD && pProp->dwItemKind3 != NULL_ID)
				{
					if (nItemType == 0 && pProp->dwItemKind3 != IK3_INSTANT && pProp->dwItemKind3 != IK3_ICECEARM)
						continue;

					if (nItemType == 1 && pProp->dwItemKind3 != IK3_PILL)
						continue;

					if (nItemType == 2 && pProp->dwItemKind3 != IK3_COOKING)
						continue;
				}
	...
	CString strTmp;
	strTmp.Format("%I64d", nTotalCost);
	strTmp = GetNumberFormatEx(strTmp);
...
CTextureMng	CWndQuickSellCtrl::m_textureMng;
CWndQuickSellCtrl::CWndQuickSellCtrl()
{
...
vector<DWORD> CWndQuickSellCtrl::GetItemList()
{
	vector<DWORD> rtvec;
	for (auto item : m_pItemContainer) {
		CItemElem* itm = (CItemElem*)item;
		if (itm)
			rtvec.push_back(itm->m_dwObjId);
	}

	return rtvec;
}

void CWndQuickSellCtrl::Create(DWORD dwListCtrlStyle, RECT& rect, CWndBase* pParentWnd, UINT nID)
{
	m_dwListCtrlStyle = dwListCtrlStyle;
	CWndBase::Create(m_dwListCtrlStyle | WBS_CHILD, rect, pParentWnd, nID);
}
void CWndQuickSellCtrl::OnInitialUpdate(void)
{
...
void CWndQuickSellCtrl::OnDraw(C2DRender* p2DRender)
{
	CRect rect = GetClientRect();
	int nWidth = (rect.Width() - m_wndScrollBar.GetWndRect().Width() - 5) / 32;
	...
	for (int i = pt.y; i < (int)(m_pItemContainer.size()); i++)
	{
		...
		if (pItemBase)
		{
			...
			if (rectHittest.PtInRect(point) && point.y < 140)
			{
				p2DRender->RenderRect(rectHittest, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
			...
			if (pItemElem) 
			{
				...
				if (pItemProp && pItemProp->dwPackMax > 1)
				{
					...
					p2DRender->TextOut(x * 32 + 31 - size.cx + 17, y * 32 + 31 - size.cy + 10, szTemp, 0xFFffb912);
#else //__LEESE_RENDER_ITEMCOUNT
					p2DRender->TextOut(x * 32 + 32 - size.cx + 17, y * 32 + 32 - size.cy + 10, szTemp, 0xff303030);
					p2DRender->TextOut(x * 32 + 31 - size.cx + 17, y * 32 + 31 - size.cy + 10, szTemp, 0xff68f442);
#endif //__LEESE_RENDER_ITEMCOUNT
...
void CWndQuickSellCtrl::SetWndRect(CRect rectWnd, BOOL bOnSize)
{
...
void CWndQuickSellCtrl::PaintFrame(C2DRender* p2DRender)
{
	CRect rect = GetWindowRect();
...
int CWndQuickSellCtrl::HitTest(CPoint point)
{
	CRect rect = GetClientRect();
	int nWidth = (rect.Width() - m_wndScrollBar.GetWndRect().Width() - 5) / 32;
	int nHeight = rect.Height() / 32;
	...
	for (int i = 0; i < nWidth * nHeight; i++)
	{
		...
		rect = CRect(x * 32 + 13, y * 32 + 7, x * 32 + 13 + 32, y * 32 + 32 + 7);
...
void CWndQuickSellCtrl::AddItem(CItemBase* pItemContainer)
{
	m_pItemContainer.push_back(pItemContainer);
}

void CWndQuickSellCtrl::ClearItem()
{
	m_pItemContainer.clear();
}

int CWndQuickSellCtrl::GetItemSize()
{
	return m_pItemContainer.size();
}

void CWndQuickSellCtrl::RestoreItem()
{
	CItemElem* pItemElem = NULL;
...
int CWndQuickSellCtrl::GetTotalCount()
{
	int nCount = 0;
	CItemElem* pItemElem = NULL;
...
void CWndQuickSellCtrl::OnRButtonDown(UINT Flags, CPoint pt)
{
	int index = HitTest(pt);
	if (index < 0 || index >= m_pItemContainer.size()) return;

	CItemElem* useritem = (CItemElem*)m_pItemContainer[index];
	if (useritem) {
		int i = 0;
		for (auto it = m_pItemContainer.begin(); it != m_pItemContainer.end(); it++) {
			if (index == i) {
				((CItemElem*)m_pItemContainer[i])->SetExtra(0);
				m_pItemContainer.erase(it);
				break;
			}
...
void CWndBeautyShop::OnDestroyChildWnd(CWndBase* pWndChild)
{
	...
	else if (m_pWndBeautyShopConfirm == pWndChild)
	{
		SAFE_DELETE(m_pWndBeautyShopConfirm);
		pWndChild = NULL;
	}
...
*/
	...
	if (m_TargetWndId != -1)
	{
		...
		else if (m_MainFlag == 1)
		{
			...
			pText->AddString("\n");
			pText->AddString(prj.GetText(TID_GAME_WARNNING_USE_COUPON), COLOR_ERROR);
		}
	}
