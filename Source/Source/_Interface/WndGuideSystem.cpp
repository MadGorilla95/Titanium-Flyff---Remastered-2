...
void CWndGuideSystem::ChangeModel( int nJob )
{
	...
	{
#ifdef __PANG_SYSTEM
		m_pModel->LoadBone("Mvr_Pinata.chr");
		m_pModel->LoadElement("Mvr_Pinata.o3d", 0);
#else // __PANG_SYSTEM
		switch( nJob )
		{
			default:
...
void CWndGuideSystem::SetAni(int nJob, int nAniKind)
{
	...
	CString filename;
#ifdef __PANG_SYSTEM
	char* Mover[3] = { "Mvr_Pinata_idle1.ani", "Mvr_Pinata_idle1.ani", "Mvr_Pinata_die1.ani" };
	filename = Mover[nAniKind];
#else // __PANG_SYSTEM
...
void CWndGuideSystem::OnDestroyChildWnd(CWndBase* pWndChild)
{
	if (m_pWndTutorialView == pWndChild)
	{
		SAFE_DELETE(m_pWndTutorialView);
		pWndChild = NULL;
	}
	else if (m_wndGuideSelection == pWndChild)
	{
		SAFE_DELETE(m_wndGuideSelection);
		pWndChild = NULL;
	}
	else if (m_wndGuideText == pWndChild)
	{
		SAFE_DELETE(m_wndGuideText);
		pWndChild = NULL;
	}
	...
	if(m_CurrentGuide.m_nVicCondition)
	{
		...
		if(CheckCompletion(m_CurrentGuide))
		{
			if(m_CurrentIter == m_mapGuide.end())
			{
				...
				if(m_pWndTutorialView && g_Option.m_nTutorialLv > 0)
...
{ 
	...
	m_pTextureBG = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(g_Neuz.m_pd3dDevice, "GuideBG.tga", 0, TRUE);
...
#ifdef __AEGON_THEME_SWITCHER
void CWndGuideTextMgr::OnThemeSelectionChanged()
{
