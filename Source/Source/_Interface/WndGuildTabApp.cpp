...
#ifdef _WIN64
	pWndWorld->m_texMsgIcon.Render(m_pApp->m_pd3dDevice, pVertex, ((INT_PTR)pVertices - (INT_PTR)pVertex) / sizeof(TEXTUREVERTEX2));
#else
...
BOOL CWndGuildTabApp::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	...
	if (nID == WIDC_CHECK252)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK252);
		pWndCheck->GetCheck() ? m_adwPower[GUD_KINGPIN] |= PF_GUILDCOMBAT : m_adwPower[GUD_KINGPIN] &= (~PF_GUILDCOMBAT);
	}
	if (nID == WIDC_CHECK257)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK257);
		pWndCheck->GetCheck() ? m_adwPower[GUD_KINGPIN] |= PF_SECRETROOM : m_adwPower[GUD_KINGPIN] &= (~PF_SECRETROOM);
	}
...
#ifdef __NEW_GUILD_PERM
	if (nID == WIDC_CHECK253)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK253);
		pWndCheck->GetCheck() ? m_adwPower[GUD_CAPTAIN] |= PF_GUILDCOMBAT : m_adwPower[GUD_CAPTAIN] &= (~PF_GUILDCOMBAT);
	}
	if (nID == WIDC_CHECK258)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK258);
		pWndCheck->GetCheck() ? m_adwPower[GUD_CAPTAIN] |= PF_SECRETROOM : m_adwPower[GUD_CAPTAIN] &= (~PF_SECRETROOM);
	}
#endif //__NEW_GUILD_PERM
...
#ifdef __NEW_GUILD_PERM
	if (nID == WIDC_CHECK254)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK254);
		pWndCheck->GetCheck() ? m_adwPower[GUD_SUPPORTER] |= PF_GUILDCOMBAT : m_adwPower[GUD_SUPPORTER] &= (~PF_GUILDCOMBAT);
	}
	if (nID == WIDC_CHECK259)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK259);
		pWndCheck->GetCheck() ? m_adwPower[GUD_SUPPORTER] |= PF_SECRETROOM : m_adwPower[GUD_SUPPORTER] &= (~PF_SECRETROOM);
	}
#endif //__NEW_GUILD_PERM
...
#ifdef __NEW_GUILD_PERM
	if (nID == WIDC_CHECK255)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK255);
		pWndCheck->GetCheck() ? m_adwPower[GUD_ROOKIE] |= PF_GUILDCOMBAT : m_adwPower[GUD_ROOKIE] &= (~PF_GUILDCOMBAT);
	}
	if (nID == WIDC_CHECK260)
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK260);
		pWndCheck->GetCheck() ? m_adwPower[GUD_ROOKIE] |= PF_SECRETROOM : m_adwPower[GUD_ROOKIE] &= (~PF_SECRETROOM);
	}
#endif //__NEW_GUILD_PERM
...
#ifdef __NEW_GUILD_PERM
	/*if (m_adwPower[GUD_MASTER] & PF_GUILDCOMBAT)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK251);
		pWndCheck->SetCheck(1);
	}

	if (m_adwPower[GUD_MASTER] & PF_SECRETROOM - 1)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK256);
		pWndCheck->SetCheck(1);
	}*/
	pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK251);
	pWndCheck->SetCheck(1);
...
#ifdef __NEW_GUILD_PERM
	if (m_adwPower[GUD_KINGPIN] & PF_GUILDCOMBAT)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK252);
		pWndCheck->SetCheck(1);
	}

	if (m_adwPower[GUD_KINGPIN] & PF_SECRETROOM)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK257);
		pWndCheck->SetCheck(1);
	}
#endif //__NEW_GUILD_PERM
...
#ifdef __NEW_GUILD_PERM
	if (m_adwPower[GUD_CAPTAIN] & PF_GUILDCOMBAT)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK253);
		pWndCheck->SetCheck(1);
	}

	if (m_adwPower[GUD_CAPTAIN] & PF_SECRETROOM)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK258);
		pWndCheck->SetCheck(1);
	}
#endif //__NEW_GUILD_PERM
...
#ifdef __NEW_GUILD_PERM
	if (m_adwPower[GUD_SUPPORTER] & PF_GUILDCOMBAT)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK254);
		pWndCheck->SetCheck(1);
	}

	if (m_adwPower[GUD_SUPPORTER] & PF_SECRETROOM)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK259);
		pWndCheck->SetCheck(1);
	}
#endif //__NEW_GUILD_PERM
...
#ifdef __NEW_GUILD_PERM
	if (m_adwPower[GUD_ROOKIE] & PF_GUILDCOMBAT)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK255);
		pWndCheck->SetCheck(1);
	}

	if (m_adwPower[GUD_ROOKIE] & PF_SECRETROOM)
	{
		pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK260);
		pWndCheck->SetCheck(1);
	}
#endif //__NEW_GUILD_PERM
}
...
void CWndGuildTabApp::OnDestroyChildWnd(CWndBase* pWndChild)
{
	if (m_pWndGuildPayConfirm == pWndChild)
	{
		SAFE_DELETE(m_pWndGuildPayConfirm);
		pWndChild = NULL;
	}
