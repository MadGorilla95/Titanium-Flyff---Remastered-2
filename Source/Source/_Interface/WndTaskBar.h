...
public:
	...
	int            m_nActionSlotIndex;
	SHORTCUT	   m_aSlotQueue[MAX_ACTION_SLOT][MAX_SLOT_QUEUE];
	LPSHORTCUT     m_paActionSlot;
...
#ifdef __ACTION_SLOT_PAGES
	SHORTCUT       m_aSlotSkill[MAX_ACTION_SLOT];
#else // __ACTION_SLOT_PAGES
	...
	CPoint m_pQueueIconOffsetPoint;

	CPoint m_pOffsetLoopIncrease;
	CPoint m_pOffsetLoopIncreaseQueue;
	...
	void PutTooTip(LPSHORTCUT pShortcut, CPoint point, CRect* pRect, BOOL bSkipClientToScreen = FALSE);
#else
...
#ifdef __ACTION_SLOT_PAGES
	int GetCurrentActionSlot() const;
	void SetActionSlot(const int nSlot);
	BOOL UseSkillQueue(CCtrl* pTargetObj, const int nIndex = -1);
#else // __ACTION_SLOT_PAGES
...
#ifdef __ACTION_SLOT_PAGES
	BOOL SetShortcut(int nIndex, DWORD dwShortcut, DWORD dwType, DWORD dwId, CTexture* pTexture = NULL, int nWhere = 0, DWORD dwUserId = 0);
#else //__ACTION_SLOT_PAGES
