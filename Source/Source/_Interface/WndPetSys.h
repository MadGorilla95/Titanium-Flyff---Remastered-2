...
class CWndPetMistakeCustom : public CWndNeuz
{
public:
	int m_nMiracleLv;
	int m_nCount;
	int m_nDelay;			//회전시키는 Delay값
	...
	BOOL m_bEnd;				//회전이 끝난 시점
	BOOL m_bLocked;			//상태창에 레벨고정을 하였었는지.

	DWORD m_dwObjId;

	CString m_strPetLevel;
	CString m_strPathLvImage[9];

	CWndText* m_pText;

public:
	CWndPetMistakeCustom();
	virtual ~CWndPetMistakeCustom();
	...
	void ReceiveResult(int nCurLevel);		//서버로 부터 결과 값을 받음
	void CurLevelImgProcess();
	void SetDescription(CHAR* szChar);
	void SetItem(DWORD dwObjId);
};
