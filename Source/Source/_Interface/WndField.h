...
#ifdef __MOUNT
	CWndMenu m_wndMenuMount;
#endif // __MOUNT

#ifdef __INVENTORY_SORT
	CWndMenu sortmenu;
#endif //__INVENTORY_SORT
...
public:
	...
	CRect		m_rectRenderChar;
	CRect		m_rectDeleteButt;
...
#ifdef __BADGE
class CWndBadgeList : public CWndListBox
{
protected:
	int m_ID;
...
#ifndef __LEESE_BADGE_HONOR_COMBINE
class CWndBadge : public CWndNeuz
{
private:
	CWndBadgeList m_wndBadgeList;

public:
	CWndBadge();
	virtual ~CWndBadge();
	virtual	void OnInitialUpdate();
	virtual BOOL Initialize(CWndBase* pWndParent = nullptr, DWORD dwWndId = 0);
	virtual BOOL OnChildNotify(UINT message, UINT nID, LRESULT* pLResult);

	CWndButton* m_pOK;
};
#endif //__LEESE_BADGE_HONOR_COMBINE
#endif //__BADGE
...
#ifdef __LEESE_BADGE_HONOR_COMBINE
	CWndBadgeList			m_wndBadgeList;
#endif //__LEESE_BADGE_HONOR_COMBINE
...
#ifdef __FL_SMOOTH_STATUS
	int nWidthOldHp;
	int nWidthOldMp;
	int nWidthOldFp;
	int nWidthOldExp;
#endif //__FL_SMOOTH_STATUS
...
public:
	...
	void SetJewel(CItemElem* pItemElem);
#ifdef __AEGON_PUT_JEWEL_WEAP
	void SetItem(CItemElem* pItemElem);
#endif // __AEGON_PUT_JEWEL_WEAP
};
...
#define CH_KEY_UP		(DWORD)0xffffff00
#define CH_KEY_DOWN		(DWORD)0xffffff01
#define CH_KEY_LEFT		(DWORD)0xffffff02
#define CH_KEY_RIGHT	(DWORD)0xffffff03
#define CH_KEY_TRACE	(DWORD)0xffffff04
#define CH_KEY_WALK		(DWORD)0xffffff05

#ifdef __DPS_METER
#define CH_KEY_DPS		(DWORD)0xffffff06
#endif

class CWndChangeHotkeyList : public CWndListBox
{
private:
	map<DWORD, char*> m_mapApplet;

public:
	CWndChangeHotkeyList();
	virtual ~CWndChangeHotkeyList();
	...
	void Init();
	void Clear();
	void OnKeyInput(int nVirtKey);
};

class CWndChangeHotkey : public CWndNeuz
{
private:
	CWndChangeHotkeyList m_wndHKList;
public:
	virtual void OnInitialUpdate();
	virtual BOOL Initialize(CWndBase* pWndParent = NULL, DWORD dwStyle = 0);
...
#ifdef __VENDING_NPC
class CWndVendorWarning : public CWndNeuz
{
public:
	CWndVendorWarning() {};
	~CWndVendorWarning() {};
...
class CWndHonorTitle : public CWndNeuz
{
private:
	CString str;
	CString str2;
	int	m_nSelected, nLine, nIndex, Slot, nListFontHeight;
	DWORD dwColor;
...
public:
	...
	void SetText(int id);
	void UpdateMap();
	int GetMapSize() { return m_mapAll.size(); };
};
#endif // __WIKIPEDIA_TITLE

#ifdef __WIKIPEDIA_JOB
class CWndWikipediaJobCreate;
class CWndWikipediaJob : public CWndNeuz
{
private:
	int	m_nJob;
	CWndWikipediaJobCreate* m_pCreate;
public:
	CWndWikipediaJob(void);
	~CWndWikipediaJob(void);
	...
	virtual void SetJob(int nJob);

#ifdef __AEGON_FULLSCREEN_AND_RESIZE
	virtual void OnDestroyChildWnd(CWndBase* pWndChild);
#endif
};
class CWndWikipediaJobCreate : public CWndNeuz
{
private:
	int	m_nJob;
	int m_nMaxLevel;
...
{
private:
	CWndButton* m_pWndButton1;
	CWndButton* m_pWndButton2;
	CWndButton* m_pWndButton3;
	CWndButton* m_pWndButton4;
	CWndButton* m_pWndButtonOK;
	CWndStatic* m_pWndStatic1;
	CWndStatic* m_pWndStatic2;
	CWndStatic* m_pWndStatic3;
	CWndStatic* m_pWndStatic4;
	CWndStatic* m_pWndStatic5;
	CWndText* m_pWndText1;
	CWndText* m_pWndText2;
	CWndText* m_pWndText3;
	CWndText* m_pWndText4;
	int m_nJob;
...
public:
	...
	virtual void OnSize(UINT nType, int cx, int cy);
	virtual BOOL OnChildNotify(UINT message, UINT nID, LRESULT* pLResult);
	virtual int GetNewJob(int nButton);
};
...
#ifdef __DROP_LOG
#define REMOVE_TIMEOUT	5000
#define GOLD_FADEOUT	5000
...
class CWndWeaponRarity : public CWndNeuz
{
private:
	CTexture* pCustom;
...
class CWndAnarchySkill : public CWndNeuz
{
	int				m_nCurrentPage;
	int				m_nMaxPage;
	CRect			m_pWndRect[4];
	CRect			m_pWndCurRect[4];
	ItemProp*		m_pItemProp[MAX_ANARCHY_BUFFS];
	ItemProp*		m_pCurItemProp[MAX_ANARCHY_BUFFS];
	CTexture*		m_pTexture[MAX_ANARCHY_BUFFS];
	CTexture*		m_pCurTexture[MAX_ANARCHY_BUFFS];
	CWndButton*		m_pWndButton[4];
	CWndStatic*		m_pWndStaticIcon[4];
	CWndStatic*		m_pWndStaticName[4];
	CWndStatic*		m_pWndStaticPrice[4];
	CWndButton*		m_pWndCheck;
...
class CWndAnarchySkillConfirm : public CWndNeuz
{
	int m_nId;
...
public:
	...
	CWndAnarchySkillConfirm() { m_nId = 0; m_nMode = 0; };
	~CWndAnarchySkillConfirm() {};
	CWndAnarchySkillConfirm(int nType) { m_nId = nType; m_nMode = 0; };
...
struct sItem
{
	...
	{
#ifdef __SYS_LOCK_ITEM
		dwId = dwKind2 = dwItemId = dwLocked = nIndex = 0;
#else //__SYS_LOCK_ITEM
...
class CWndRemoveCmm : public CWndNeuz
{
public:
	CWndRemoveCmm(DWORD dwItemID);
	~CWndRemoveCmm();
...
#ifdef __PLAYER_LIST
class CWndPlayerList : public CWndNeuz
{

private:
	CWndButton* pWndCheck[18];
	CWndEdit* pWndEdit[3];
	CWndListBox* pListBox;
	vector<PlayerList>	m_vecPlayerList;
	CWndMenu			m_wndMenu;
...
public:
	...
	virtual void OnLButtonUp(UINT nFlags, CPoint point);
	virtual void OnLButtonDown(UINT nFlags, CPoint point);
	virtual void OnGetPlayerList(CAr& ar);
};
#endif // __PLAYER_LIST

#ifdef __BUYBACK
class CWndBuyBack : public CWndNeuz
{
public:
	CWndBuyBack();
	virtual ~CWndBuyBack();
...
class CWndIngameDPPurchaseConfirm : public CWndNeuz
{
...
public:
	...
	//Motion || Emote || Webshop unitID
	DWORD dwID;

	vector<tuple<DWORD, int>> m_vItemQuantityTuple;

	CWndListBox* m_pWndItemPreview;

	DWORD m_dwPreviewSize;

	char m_szPackageName[128];
	int m_nPrice;
	BOOL m_bEnableAmount;

	CWndIngameDPPurchaseConfirm();
	virtual ~CWndIngameDPPurchaseConfirm();
	...
	void SetInfo(char* szItemName, int nPrice, BOOL bEnableAmount = FALSE);
	void SetText(int nAmount);
	void PurchaseConfirmed(int nAmount);
...
class CWndAzuriomProfileImage : public CWndNeuz
{

	CWndStatic* pWndStaticField;

public:

	CWndAzuriomProfileImage();
	virtual ~CWndAzuriomProfileImage();
	...
	void AdjustElementsToSize();
	void SendProfilePicture();
};
