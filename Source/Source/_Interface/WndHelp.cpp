...
{ 
	...
	pWndText->SetString(m_aString.GetAt(0), CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
	...
	case WIDC_NEXT:
		{
			...
			CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT );
#ifdef __AEGON_THEME_SWITCHER
			pWndText->SetString(m_aString.GetAt(m_nPosString), CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
