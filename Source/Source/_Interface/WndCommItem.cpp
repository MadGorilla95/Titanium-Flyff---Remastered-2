...
{ 
	...
	m_wndPermBuffs.Create(WBS_CHILD | WBS_NODRAWFRAME, rect, pWndTabCtrl, APP_PERM_BUFFS);
...
#ifdef __AEGON_PERMANENT_BUFF
	tabTabItem.mask = WTCIF_TEXT | WTCIF_PARAM;
	tabTabItem.pszText = _T("Perm Buffs");
	tabTabItem.pWndBase = &m_wndPermBuffs;
...
BOOL CWndCommItemCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
CString CWndCommItemCtrl::GetDstString(DWORD nDst, DWORD nDstNum)
{
	CString strOut;
	if (nDst == DST_NONE)
	{
...
void CWndCommItemCtrl::OnMouseWndSurface( CPoint point )
{
	...
	for( int i = 0; i < m_nMaxDraw; i++ ) 
	{
		...
		if( rectHittest.PtInRect( point ) )
		{
			...
			CString strEnter = '\n';
#ifdef __AEGON_THEME_SWITCHER
			str.AddString(pItem->szName, dwColorHighlightBlue, ESSTY_BOLD);
#else
			str.AddString( pItem->szName, 0xff0000ff, ESSTY_BOLD );
#endif
#ifdef __AEGON_THEME_SWITCHER
			str.AddString(strEnter, dwColorText);
#else
			str += strEnter;
#endif
			if( pItem->dwID == II_SYS_SYS_SCR_BLESSING )
			{
...
#ifdef __AEGON_THEME_SWITCHER
				str.AddString(strTemp, dwColorText);
#else
				str += strTemp;
#endif
			}
				...
				|| pItem->dwID == II_SYS_SCR_RPROTECT
...
#ifdef __AEGON_THEME_SWITCHER
					str.AddString(str2, dwColorText);
#else
					str += str2;
#endif
				}
...
//					if( pItemElem->m_dwKeepTime && !pItemElem->IsFlag( CItemElem::expired ) )
...
#ifdef __AEGON_PERMANENT_BUFF
			else if (pItem->dwItemKind3 == IK3_PERM_BUFF 
#ifdef __WBQT_GUILD_BUFFS
			|| pItem->dwItemKind3 == IK3_GUILD_BUFF
#endif
			)
			{
			strTemp.Format(prj.GetText(TID_TOOLTIP_ITEMTIME));	// 지속시간 : 
			...
			str += strTemp;
#endif
			strTemp.Format("Permanent Buff");	// 지속시간 : 
#ifdef __AEGON_THEME_SWITCHER
			str.AddString(strTemp, dwColorText);
#else
			str += strTemp;
			...
			IBuff* pBuff = g_pPlayer->m_buffs.GetBuff(BUFF_ITEM2, pItem->dwID);
#else
...
#ifdef __AEGON_PERMANENT_BUFF
			if (pItem->dwItemKind3 == IK3_PERM_BUFF
#ifdef __WBQT_GUILD_BUFFS
				|| pItem->dwItemKind3 == IK3_GUILD_BUFF
#endif
				)
			{
				CString strPermBuffs[3];
				for (int i = 0; i < 3; i++)
				{
					strPermBuffs[i] = GetDstString(pItem->dwDestParam[i], pItem->nAdjParamVal[i]);
				}
				CString permTmp = "";
				...
				for (int i = 0; i < 3; i++) 
				{
					if (strPermBuffs[i].GetLength() > 0) 
					{
						permTmp += "\n";
						permTmp += strPermBuffs[i].GetString();
					}
				}
				permTmp += "\n";
#ifdef __AEGON_THEME_SWITCHER
				str.AddString(permTmp.GetString(), dwColorText);
#else
				str += permTmp;
#endif
				//Description
				if (strlen(pItem->szCommand) > 0) 
				{
					CString strDescr;
					strDescr.Format("Description: %s", pItem->szCommand);
					str.AddString(strDescr.GetString(), dwColorText);
				}
...
void CWndCommItemCtrl::DrawSM( C2DRender* p2DRender, CPoint* pPoint, int x, int &nScroll )
{
	for( int i = 0 ; i < SM_MAX; ++i )
	{
...
#ifdef __AEGON_PERMANENT_BUFF
		CWndTabCtrl* pWndTabCtrl = (CWndTabCtrl*)GetParentWnd();
		if (pWndTabCtrl->GetSelectedTab()->pWndBase->GetWndId() == APP_PERM_BUFFS)
		{
			//Block SM Buffs from perm tab
			continue;
		}
#endif // __AEGON_PERMANENT_BUFF
		...
		CEditString strEdit;
		strEdit.SetParsingString(string, dwColorHighlightBlue);
#else
		...
		p2DRender->RenderLine(CPoint(0, pPoint->y + m_nFontHeight - 3), CPoint(rectClient.right - 17, pPoint->y + m_nFontHeight - 3), dwColor);
...
void CWndCommItemCtrl::DrawSkill( C2DRender* p2DRender, CPoint* pPoint, int x, int &nScroll )
{
	...
	for( MAPBUFF::iterator it1 = g_pPlayer->m_buffs.m_mapBuffs.begin(); it1 != g_pPlayer->m_buffs.m_mapBuffs.end(); ++it1 )
	{
...
#ifdef __WBQT_GUILD_BUFFS
		if ((pItem->dwItemKind3 == IK3_PERM_BUFF || pItem->dwItemKind3 == IK3_GUILD_BUFF) && pWndTabCtrl->GetSelectedTab()->pWndBase->GetWndId() != APP_PERM_BUFFS)
#else

		if (pItem->dwItemKind3 == IK3_PERM_BUFF && pWndTabCtrl->GetSelectedTab()->pWndBase->GetWndId() != APP_PERM_BUFFS)
#endif
		{
			//Block Perm Buffs in regular tab
			continue;
		}
		if (pItem->dwItemKind3 != IK3_PERM_BUFF && pWndTabCtrl->GetSelectedTab()->pWndBase->GetWndId() == APP_PERM_BUFFS
#ifdef __WBQT_GUILD_BUFFS
		...
		{
			//Block regular items/buffs in Perm tab
			continue;
		}
#endif // __AEGON_PERMANENT_BUFF
...
void CWndCommItemCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
	CPoint pt(3, 3);
	...
	for (int i = 0; i < m_nMaxDraw; i++)
	{
		int x = 0, nWidth = m_rectClient.Width();
		CRect rectHittest = CRect(x + 3, pt.y, x + 3 + nWidth, pt.y + 32);
		...
		if (rectHittest.PtInRect(point))
		{
			...
			if (g_WndMng.m_pWndRemoveCmm)
				SAFE_DELETE(g_WndMng.m_pWndRemoveCmm);

			g_WndMng.m_pWndRemoveCmm = new CWndRemoveCmm(pItem->dwID);
			g_WndMng.m_pWndRemoveCmm->Initialize(NULL);
		}
		pt.y += m_nFontHeight;
	}
}
