...
BOOL CWndVendorBuy::OnChildNotify( UINT message, UINT nID, LRESULT* pLResult )
{
	...
	if( (message == WNM_CLICKED && nID == WIDC_OK) || message == EN_RETURN )
	{
		...
		if (m_pItemBase->m_nBuyMode == 4 && g_pPlayer->m_Inventory.GetItemCount(II_SYS_REDPERIN) < ((CItemElem*)m_pItemBase)->m_nCost * nBufNum)
		{
			g_WndMng.OpenMessageBox(_T(prj.GetText(TID_GAME_LACKMONEY)), MB_OK, this);
			return TRUE;
		}
		else if (m_pItemBase->m_nBuyMode == 3 && g_pPlayer->m_Inventory.GetItemCount(II_CHP_RED) < ((CItemElem*)m_pItemBase)->m_nCost * nBufNum)
		{
			g_WndMng.OpenMessageBox(_T(prj.GetText(TID_GAME_LACKMONEY)), MB_OK, this);
			return TRUE;
		}
		else if (m_pItemBase->m_nBuyMode == 2 && g_pPlayer->m_Inventory.GetItemCount(II_SYS_SYS_SCR_PERIN) < ((CItemElem*)m_pItemBase)->m_nCost * nBufNum)
		{
			g_WndMng.OpenMessageBox(_T(prj.GetText(TID_GAME_LACKMONEY)), MB_OK, this);
			return TRUE;
		}
		else if (m_pItemBase->m_nBuyMode == 1 && g_pPlayer->GetGold() < ((CItemElem*)m_pItemBase)->m_nCost * nBufNum)
		{
			g_WndMng.OpenMessageBox(_T(prj.GetText(TID_GAME_LACKMONEY)), MB_OK, this);
			return TRUE;
		}
