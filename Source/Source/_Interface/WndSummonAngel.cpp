	...
	else if( nID == WIDC_START )
	{
		...
		{	
			if(!m_nowStarting)
			{
				...
				int nItemCount = 0;
				int nSuns = 0, nMoons = 0;

				for (int i = 0; i<MAX_MATDIE; i++)
				{
					if (m_MatDie[i].isUse && m_MatDie[i].pItemElem != NULL && m_MatDie[i].pItemElem->GetExtra() > 0)
					{
						nItemCount++;
						switch (m_MatDie[i].pItemElem->m_dwItemId)
						{
				...
				g_DPlay.SendCreateAngel(nSuns, nMoons);
				ReFreshAll(TRUE);
