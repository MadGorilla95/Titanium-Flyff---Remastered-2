					...
					&& g_Neuz.m_aSavedInven[i].m_dwItemId == pItemElem->m_dwItemId )
					{
						...
						if(g_Neuz.m_aSavedInven[i].m_nExtra > pItemElem->m_nItemNum)
						{
							...
							g_DPlay.SendRegisterPVendorItem(i, 0, (BYTE)( g_Neuz.m_aSavedInven[i].m_dwObjId ),
								...
								, g_Neuz.m_aSavedInven[i].m_nBuyMode
...
BOOL CWndVendor::Initialize( CWndBase* pWndParent, DWORD dwWndId ) 
{
...
#ifdef __VENDING_NPC
	if (g_Neuz.m_pVendingNpcItem != NULL)
		return FALSE;
#endif // __VENDING_NPC
	...
	if (UserContinent) // Town Check
	{
		...
		{
			g_WndMng.PutString("You're not allowed to create a private shop here.", NULL, COLOR_WARNING);
					...
					*/
					...
					if( 0 <= iIndex && iIndex < MAX_VENDITEM )						
					{
						...
						if( IsUsingItem( pItemBase ) == FALSE && m_pVendor->IsActiveMover() && !m_pVendor->m_vtInfo.IsVendorOpen() )
						{
							SAFE_DELETE( m_pWndRegVend );
#ifdef __VENDING_NPC
							m_pWndRegVend = new CWndRegVend(m_pVendingNpcItem != NULL);
#else // __VENDING_NPC
	...
	else
	{
		if( nID == WIDC_OK )
		{
			...
			if( !strVendor.IsEmpty()  )
			{
				...
				pWndEdit->m_string.SetColor(CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#endif
			}
			...
			if (m_pVendingNpcItem != NULL)
			{
				g_WndMng.PutString("Place your offline vendor.", NULL, 0xff1eff00);

				HOUSING_ITEM kItem;
				kItem.m_bDeploy = TRUE;
				kItem.dwItemId = m_pVendingNpcItem->m_dwItemId;
				kItem.m_nIndex = m_pVendingNpcItem->m_dwItemId;
				kItem.m_vPos = g_pPlayer->GetPos();
				kItem.m_fAngle = 0.0f;
				kItem.m_nSlotIndex = 1;
				VendorDeploy()->LoadToDeploy(m_pVendingNpcItem->m_dwItemId, kItem);
				g_Neuz.m_pVendingNpcItem = m_pVendingNpcItem;
#ifdef __AEGON_FIXES
				strcpy(g_Neuz.m_szVendingNpcTitle, strVendor.GetString());
#else
				strcpy(g_Neuz.m_szVendingNpcTitle, strFormat.GetString());
#endif
...
#ifdef __VENDING_NPC
	if (m_pVendingNpcItem)
	{
		if (m_pVendingNpcItem->GetExtra())
			m_pVendingNpcItem->SetExtra(0);
	}

	if (m_pVendingNpcItem == NULL || m_bUserClose)
		g_DPlay.SendPVendorClose(m_pVendor->GetId(), TRUE);
	else
		g_DPlay.SendPVendorClose(m_pVendor->GetId(), FALSE);
...
{ 
	...
	pWndEdit->SetPlaceholderString("Type your message ...");
...
void CWndVendorMessage::AddMessage( LPCTSTR lpszFrom, LPCTSTR lpszMessage )
{
	...
	if (g_Option.m_bTimeStamp) {
		m_wndChat.AddString(timeMessage + strMessage);
	}
	else {
...
#if __VER >= 11 // __MOD_VENDOR
#ifdef __CHIP_PLAYERSHOP
void CWndVendorMessage::WriteBuyInfo(char* pBuyerName, CString strItem, int nItemNum, int nTotalCost, CItemElem* pItemElem)
#else //#ifdef __CHIP_PLAYERSHOP
void CWndVendorMessage::WriteBuyInfo(char* pBuyerName, CString strItem, int nItemNum, int nTotalCost)
#endif //#ifdef __CHIP_PLAYERSHOP
{
#ifdef __CHIP_PLAYERSHOP
	if (m_nIsOwner)
	{
		if (pItemElem->m_nBuyMode == 1)
		{
			CString str;
		...
		else if (pItemElem->m_nBuyMode == 2)
		{
			CString str;
			str.Format(prj.GetText(TID_GAME_BUY_INFOPR), pBuyerName, (LPSTR)(LPCSTR)strItem, nItemNum, nTotalCost);
		...
		else if (pItemElem->m_nBuyMode == 3)
		{
			CString str;
			str.Format(prj.GetText(TID_GAME_BUY_INFORC), pBuyerName, (LPSTR)(LPCSTR)strItem, nItemNum, nTotalCost);
		...
		else if (pItemElem->m_nBuyMode == 4)
		{
			CString str;
			str.Format(prj.GetText(TID_GAME_BUY_INFORP), pBuyerName, (LPSTR)(LPCSTR)strItem, nItemNum, nTotalCost);
