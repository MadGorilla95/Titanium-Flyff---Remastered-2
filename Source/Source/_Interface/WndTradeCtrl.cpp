	...
	for( int i = 0; i < MAX_TRADE; i++ )
	{
		...
		if( pItemBase )
		{
...
#ifdef __AEGON_FIXES
				CItemElem tipItem = *pItemElem;
				if (IsUsingItem(pItemElem))
					tipItem.m_nItemNum = pItemElem->GetExtra();

				g_WndMng.PutToolTip_Item(&tipItem, point2, &rectHittest);
#else
			...
			if( pItemElem->GetProp()->dwPackMax > 1 )
			{
				...
				p2DRender->TextOut(x * 32 + 32 - size.cx, y * 32 + 32 - size.cy, szTemp, 0xFFffb912);
...
int CWndTradeCtrl::HitTest(CPoint point)
{
	int nDstId = -1;
	CPoint pt(3, 3);
	CRect rect;
	if (1) //m_dwListCtrlStyle == WLVS_ICON )
	{
		...
		for (int i = 0; i < MAX_TRADE; i++)
		{
			if (g_pPlayer->m_vtInfo.GetItem(i)) {
				int x = i % nWidth;
				int y = i / nWidth;
				rect.SetRect(x * 32, y * 32, x * 32 + 32, y * 32 + 32);
