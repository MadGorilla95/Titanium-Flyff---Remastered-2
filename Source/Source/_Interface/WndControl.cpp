	...
	else
	{
		...
		if( IsWndStyle( WSS_ALIGNHRIGHT ) )
		{
			...
			if (IsWndStyle(WSS_MONEY))
			{
				...
				sizeTitle = p2DRender->m_pFont->GetTextExtent(str);
			}
#else // __AEGON_FIXES
			CSize sizeTitle = p2DRender->m_pFont->GetTextExtent(m_strTitle);
#endif // __AEGON_FIXES
...
void CWndStatic::PaintFrame( C2DRender* p2DRender )
{
	...
	CRect rect = GetWindowRect();
#ifdef __AEGON_THEME_SWITCHER
	p2DRender->RenderRect(rect, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else
...
#ifdef __AEGON_THEME_SWITCHER
void CWndStatic::SetTextColor(BOOL bForce) 
{
	if (!(m_dwStyle & WSS_GROUPBOX) || bForce)
	{
		...
		if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
		{
			...
			{
				m_dwColor = it->second.dwStaticColor;
			}
		}
	}
}
void CWndStatic::OnThemeSelectionChanged() 
{
	SetTextColor();
...
CWndButton::CWndButton()
{
	...
	m_nPushColor      = D3DCOLOR_ARGB( 255, 255, 255, 255);
	m_nDisableColor   = D3DCOLOR_ARGB( 255, 255, 255, 255);
	m_nHighlightColor = D3DCOLOR_ARGB( 255, 255, 255, 255);
#else //__BUTTONS_V19
...
#ifdef __AEGON_THEME_SWITCHER
void CWndButton::FitTextureSizeY()
{
	if (m_pTexture)
	{
		if (IsWndStyle(WBS_RADIO) || IsWndStyle(WBS_CHECK))
		{
			CRect rect = GetWindowRect(TRUE);
			rect.bottom = rect.top + (m_pTexture->m_size.cy) + 2;
			SetWndRect(rect);
		}
	}
}
void CWndButton::FitTextureSizeAndTextX()
{
	if (m_pTexture)
	{
		if (IsWndStyle(WBS_RADIO) || IsWndStyle(WBS_CHECK)) 
		{
			int nMinSize = (m_pTexture->m_size.cx / 6) + GetFont()->GetTextExtent(GetTitle()).cx + 2;
			
			CRect rect = GetWindowRect(TRUE);

			if (nMinSize > rect.Width())
			{
				rect.right = rect.left + nMinSize;
				SetWndRect(rect);
			}
		}
	}
...
//		dwStyle |= WBS_SPRITE;
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)this, m_pApp->m_pd3dDevice, DEF_CTRL_RADIO, TRUE);
		FitTextureSizeY();
		FitTextureSizeAndTextX();
#else // __AEGON_THEME_SWITCHER
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)this, m_pApp->m_pd3dDevice, DEF_CTRL_CHECK, TRUE);
		FitTextureSizeY();
		FitTextureSizeAndTextX();
#else // __AEGON_THEME_SWITCHER
...
*/	
	...
	if(m_dwStyle & WBS_MENUITEM)
	{
...
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + 4 + 20, pt.y + 4, m_strTitle, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
		...
		else
		{
			...
			{
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->RenderLine(CPoint(2, 2), CPoint(5, 8), dwColorText);
				p2DRender->RenderLine(CPoint(2, 2), CPoint(6, 8), dwColorText);
				p2DRender->RenderLine(CPoint(5, 8), CPoint(8, 3), dwColorText);
#else
...
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + 4 + 20, pt.y + 4, m_strTitle, dwColorText);
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut( pt.x + 4 + 20, pt.y + 4, m_strTitle, 0xff000000 );
#endif // __AEGON_THEME_SWITCHER
			if( m_cHotkey )
...
#ifdef __AEGON_HOTKEY_F_KEYS
				unsigned char tmpChar = m_cHotkey;
				int nOffsetX = 0;
				if (tmpChar >= VK_F11 && tmpChar < VK_F24)
				{
					int nKeyNum = tmpChar - 121 + 10;
					strHotkey.Format("F%d", nKeyNum);
					nOffsetX = -5;
				}
#endif
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->TextOut(m_rectClient.right - 32 + nOffsetX, pt.y + 4, strHotkey, dwColorText);
#else
...
{
#ifdef __fmod
	SoundSystemClass::playSound(SND_INF_CLICK);
#else
...
void CWndButton::OnInitialUpdate()
{
	SetTextColors();
}


void CWndButton::OnThemeSelectionChanged() 
{
	OnThemeSwitch();
...
void CWndButton::SetTextColors() 
{
	...
	if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
	{
		if (it->second.nThemeType != 0)
		{
			...
			if (m_dwStyle & WBS_RADIO)
			{
				m_nFontColor = it->second.dwFontTextColor;
			}
			else if (m_dwStyle & WBS_CHECK)
			{
				m_nFontColor = it->second.dwFontTextColor;
			...
			{
				m_nFontColor = it->second.dwTextColor;
			}
...
void CWndButton::OnThemeSwitch() 
{
	if (IsWndStyle(WBS_RADIO) && m_strTextureName.IsEmpty())
	{
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)this, m_pApp->m_pd3dDevice, DEF_CTRL_RADIO, TRUE);
		FitTextureSizeY();
		FitTextureSizeAndTextX();
	}
	else if (IsWndStyle(WBS_CHECK) && m_strTextureName.IsEmpty())
	{
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)this, m_pApp->m_pd3dDevice, DEF_CTRL_CHECK, TRUE);
		FitTextureSizeY();
		FitTextureSizeAndTextX();
	}
	else if(!m_strTextureName.IsEmpty())
	{
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)this, m_pApp->m_pd3dDevice, m_strTextureName.GetString(), TRUE);
	}
}
#endif // __AEGON_THEME_SWITCHER
////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#ifdef __AEGON_THEME_SWITCHER
		if (pWndCheckBox->m_pTexture)
		{
			rectCheckBox.right = (pWndCheckBox->m_pTexture->m_size.cx / 6);
			rectCheckBox.bottom = pWndCheckBox->m_pTexture->m_size.cy;
		}
#endif
		pWndCheckBox->Create( "", WBS_CHECK, rectCheckBox, this, WIDC_CHECK );

#ifdef __AEGON_THEME_SWITCHER
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)pWndCheckBox, D3DDEVICE, _T(DEF_CTRL_CHECK), TRUE);
#else // __AEGON_THEME_SWITCHER
...
void CWndTreeCtrl::LoadTreeScript(LPCTSTR lpFileName) 
{
	...
	do {
#ifdef __WIKIPEDIA_ITEMS
		if (lpFileName == MakePath(DIR_CLIENT, "WikiItems.inc"))
			InterpriteScript(script, m_treeElem.m_ptrArray, TRUE);
		else
			InterpriteScript(script, m_treeElem.m_ptrArray, FALSE);
#else //__WIKIPEDIA_ITEMS
...
#ifdef __WIKIPEDIA_ITEMS
int CWndTreeCtrl::InterpriteScript(CScript& script, CPtrArray& ptrArray, BOOL bWiki, DWORD dataNum)
#else //__WIKIPEDIA_ITEMS
...
#endif //__WIKIPEDIA_ITEMS
{
	...
	while( *script.token != '}' && script.tok != FINISHED )
	{
		...
		if( *script.token == '{' )
		{
			if( lpTreeElem )
#ifdef __WIKIPEDIA_ITEMS
				dataNum = InterpriteScript(script, lpTreeElem->m_ptrArray, bWiki, dataNum);
#else //__WIKIPEDIA_ITEMS
				InterpriteScript(script, lpTreeElem->m_ptrArray, dataNum);
#endif //__WIKIPEDIA_ITEMS
		}
	}
...
void CWndTreeCtrl::OnInitialUpdate()
{
	...
	CRect rect = GetWindowRect();
#ifdef __AEGON_THEME_SWITCHER
	m_pTexButtOpen = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtTreeOpen.tga", COLOR_ITEM_BG);
	m_pTexButtClose = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtTreeClose.tga", COLOR_ITEM_BG);
#else // __AEGON_THEME_SWITCHER
	m_pTexButtOpen = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtTreeOpen.tga"), COLOR_ITEM_BG);
	m_pTexButtClose = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtTreeClose.tga"), COLOR_ITEM_BG);
#endif // __AEGON_THEME_SWITCHER
...
void CWndTreeCtrl::PaintTree(C2DRender* p2DRender,CPoint& pt,CPtrArray& ptrArray) 
{
	...
	for(int i = 0; i < ptrArray.GetSize(); i++)
	{
		...
		if (m_bMemberCheckingMode == TRUE && pTreeElem->m_pWndCheckBox)
		{
			...
			if (pWndCheckBox && pTreeElem->m_ptrArray.GetSize() == 0 && pWndCheckBox->m_pTexture)
			{
				nAddX = (pWndCheckBox->m_pTexture->m_size.cx / 6) - CHECK_BOX_SIZE_XY;
				if (nAddX < 0)
					nAddX = 0;

				nAddY = floor((pWndCheckBox->m_pTexture->m_size.cy - CHECK_BOX_SIZE_XY) / 2) - 1;
				if (nAddY < 0)
					nAddY = 0;
			}
		}
		...
		if( m_bMemberCheckingMode == TRUE && pTreeElem->m_pWndCheckBox )
		{
			...
			if( pTreeElem->m_ptrArray.GetSize() == 0 )
			{
				...
				if(pWndCheckBox->m_pTexture)
				{
					pWndCheckBox->SetWndRect(CRect(pt.x, pt.y - 1, pt.x + (pWndCheckBox->m_pTexture->m_size.cx / 6), pt.y + pWndCheckBox->m_pTexture->m_size.cy - 1));
				}
				else
...
#ifdef __AEGON_THEME_SWITCHER
		int nIncreaseY = GetFontHeight();
		if (bCheckBox && pTreeElem->m_pWndCheckBox && pTreeElem->m_pWndCheckBox->m_pTexture)
		{
			int nTmp = pTreeElem->m_pWndCheckBox->m_pTexture->m_size.cy + GetLineSpace();
			if (nTmp > nIncreaseY)
				nIncreaseY = nTmp;
		}
...
#ifdef __AEGON_THEME_SWITCHER
void CWndTreeCtrl::OnThemeSelectionChanged()
{
...
void CWndTreeCtrl::SetTextColors()
{
	m_nFontColor = D3DCOLOR_ARGB(255, 64, 64, 64);
	m_nSelectColor = D3DCOLOR_ARGB(255, 0, 0, 255);
	...
	if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
	{
		...
		{
			m_nFontColor = it->second.dwFontTextColor;
			m_nSelectColor = it->second.dwHighlightColor[HIGHLIGHT_COLOR_BLUE];
		}
	}
	...
	for (int i = 0; i < m_treeItemArray.GetSize(); i++)
	{
		...
		if (pTreeItem) 
		{
			pTreeItem->m_lpTreeElem->m_dwColor = m_nFontColor;
			pTreeItem->m_lpTreeElem->m_dwSelectColor = m_nSelectColor;
		}
	}
}
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_SCROLLBAR_STEP
	m_nScrollStep = 1;
#endif // __AEGON_SCROLLBAR_STEP
...
{
#ifdef __AEGON_HORI_SCROLLBAR
	int TEXTURE_H_SCROLL_ARROW_SIZE_X = 16;

	int TEXTURE_H_SCROLL_BAR_SIZE_X = 16;
	if(m_pTexButtHScrBar)
		TEXTURE_H_SCROLL_BAR_SIZE_X = m_pTexButtHScrBar->m_size.cx;
#endif // __AEGON_HORI_SCROLLBAR
	int TEXTURE_V_SCROLL_ARROW_SIZE_Y = 16;
	int TEXTURE_V_SCROLL_BAR_SIZE_Y = 16;
	if(m_pTexButtVScrBar)
		TEXTURE_V_SCROLL_BAR_SIZE_Y = m_pTexButtVScrBar->m_size.cy;
	...
	if (IsWndStyle(WBS_HORI))
	{
		rect.left -= nBarSizeOffset / 2;
		rect.right += nBarSizeOffset / 2;
	}
	else
	{
		rect.top -= nBarSizeOffset / 2;
		rect.bottom += nBarSizeOffset / 2;
	}
	...
	if (IsWndStyle(WBS_HSCROLL))
		p2DRender->RenderTexture(CPoint(point.x, point.y + TEXTURE_H_SCROLL_ARROW_SIZE_X), m_pTexButtHScrBar, m_nAlphaCount, fWidthSize, 1.0F);
	else
#endif // __AEGON_HORI_SCROLLBAR
...
void CWndScrollBar::OnDraw(C2DRender* p2DRender)
{
	if( GetScrollRange() && GetScrollRange() - m_nScrollPage > 0 )
	{
		...
		int TEXTURE_SCROLL_P_UP_SIZE_Y = m_pTexButtVScrPUp->m_size.cy;
		int TEXTURE_SCROLL_P_BAR_SIZE_Y = m_pTexButtVScrPBar->m_size.cy;
		int TEXTURE_SCROLL_P_DOWN_SIZE_Y = m_pTexButtVScrPDown->m_size.cy;

		CPoint point = rect.TopLeft();
		//point.y -= nBarSizeOffset / 2;
		FLOAT fHeightSize = static_cast<FLOAT>( (/*nBarSizeOffset +*/ rect.Height() / TEXTURE_SCROLL_P_BAR_SIZE_Y ) - 1 );
...
#else //__ALPHA_CHAT
				p2DRender->RenderTexture(CPoint(point.x, point.y), m_pTexButtHScrPLeft);
				p2DRender->RenderTexture(CPoint(point.x, point.y + TEXTURE_SCROLL_P_LEFT_SIZE_X), m_pTexButtHScrPBar, CWndBase::m_nAlpha, fWidthSize, 1.0F);
				p2DRender->RenderTexture(CPoint(point.x, point.y + (rect.Width() - TEXTURE_SCROLL_P_RIGHT_SIZE_X)), m_pTexButtHScrPRight);
#endif //__ALPHA_CHAT
			}
		}
		else 
...
#ifdef __ALPHA_CHAT
void CWndScrollBar::setAlpha(int alpha)
{
	m_nAlphaCount = alpha;
	m_wndArrow1.m_nAlphaCount = alpha;
	m_wndArrow2.m_nAlphaCount = alpha;
}
#endif //__ALPHA_CHAT

void CWndScrollBar::OnInitialUpdate()
{
	//m_pTexButtVScrUp    = m_texturePack.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "ButtVScrUp.bmp"    ), COLOR_ITEM_BG );
	//m_pTexButtVScrDown  = m_texturePack.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "ButtVScrDown.bmp"  ), COLOR_ITEM_BG );

#ifdef __AEGON_THEME_SWITCHER
	OffsetBarSize(CAegonThemeSwitcherMng::GetInstance()->GetScrollBarOffset());

	m_pTexButtVScrBar = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtVScrBar.bmp", COLOR_ITEM_BG);
	m_pTexButtVScrPUp = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtVScrPUp.bmp", COLOR_ITEM_BG);
	m_pTexButtVScrPDown = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtVScrPDown.bmp", COLOR_ITEM_BG);
	m_pTexButtVScrPBar = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtVScrPBar.bmp", COLOR_ITEM_BG);
#else // __AEGON_THEME_SWITCHER
	m_pTexButtVScrBar = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtVScrBar.bmp"), COLOR_ITEM_BG);
	m_pTexButtVScrPUp = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtVScrPUp.bmp"), COLOR_ITEM_BG);
	m_pTexButtVScrPDown = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtVScrPDown.bmp"), COLOR_ITEM_BG);
	m_pTexButtVScrPBar = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtVScrPBar.bmp"), COLOR_ITEM_BG);
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_HORI_SCROLLBAR
#ifdef __AEGON_THEME_SWITCHER
	m_pTexButtHScrBar = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtHScrBar.bmp", COLOR_ITEM_BG);
	m_pTexButtHScrPLeft = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtHScrPLeft.bmp", COLOR_ITEM_BG);
	m_pTexButtHScrPRight = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtHScrPRight.bmp", COLOR_ITEM_BG);
	m_pTexButtHScrPBar = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtHScrPBar.bmp", COLOR_ITEM_BG);
#else // __AEGON_THEME_SWITCHER
	m_pTexButtHScrBar = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtHScrBar.bmp"), COLOR_ITEM_BG);
	m_pTexButtHScrPLeft = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtHScrPLeft.bmp"), COLOR_ITEM_BG);
	m_pTexButtHScrPRight = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtHScrPRight.bmp"), COLOR_ITEM_BG);
	m_pTexButtHScrPBar = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtHScrPBar.bmp"), COLOR_ITEM_BG);
#endif // __AEGON_THEME_SWITCHER
#endif // __AEGON_HORI_SCROLLBAR
...
#ifdef __AEGON_THEME_SWITCHER
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndArrow1, m_pApp->m_pd3dDevice, "ButtHScrLeft.tga", TRUE);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndArrow2, m_pApp->m_pd3dDevice, "ButtHScrRight.tga", TRUE);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndArrow1, m_pApp->m_pd3dDevice, "ButtVScrUp.tga", TRUE);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
		CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndArrow2, m_pApp->m_pd3dDevice, "ButtVScrDown.tga", TRUE);
#else // __AEGON_THEME_SWITCHER
...
void CWndScrollBar::OffsetBarSize(int nOffset)
{
	nBarSizeOffset = nOffset;
}

void CWndScrollBar::OffsetButt1Pos(int nOffsetX, int nOffsetY)
{
	CRect rect = m_wndArrow1.GetWndRect();
	rect.left += nOffsetX;
	rect.right += nOffsetX;
	rect.top += nOffsetY;
	rect.bottom += nOffsetY;
	m_wndArrow1.SetWndRect(rect);
}
void CWndScrollBar::OffsetButt2Pos(int nOffsetX, int nOffsetY)
{
	CRect rect = m_wndArrow2.GetWndRect();
	rect.left += nOffsetX;
	rect.right += nOffsetX;
	rect.top += nOffsetY;
	rect.bottom += nOffsetY;
	m_wndArrow2.SetWndRect(rect);
}

#ifdef __AEGON_THEME_SWITCHER
void CWndScrollBar::OnThemeSelectionChanged() 
{
	OffsetBarSize(CAegonThemeSwitcherMng::GetInstance()->GetScrollBarOffset());
...
void CWndScrollBar::OnThemeSwitch() 
{
	if (!m_pApp)
		return;
...
CWndListBox::CWndListBox()
{
	...
	for(int i = 0; i < AEGACH_STATUS_MAX; i++)
		m_AchievementStatusIcon[i] = NULL;
...
void CWndListBox::OnDraw(C2DRender* p2DRender)
{
...
#ifdef __NEW_EXCHANGE_V19
	if (pWnd && pWnd->GetWndId() == APP_DIALOG_EVENT)
	{
		m_nFontHeight = 35;
		int nGapX = 40;
		CPoint pt(10, 5);
...
#else // __AEGON_THEME_SWITCHER
		CTexture* pArrow = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_THEME, "ExchangeArrow.tga"), COLOR_ERROR);
#endif// __AEGON_THEME_SWITCHER
		...
		for (unsigned i = nPos; i < nEnd; ++i)
		{
			...
			for (unsigned j = 0; j < pSet->vecSet[i].vecCondItem.size(); j++)
			{
				...
				if (pPropItem)
				{
					CTexture* pTex = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, pPropItem->szIcon), COLOR_ITEM_BG);
					DWORD dwBlend = 255;
					...
					m_p2DRender->TextOut(pt.x + 29 - size.cx, pt.y + 31 - size.cy + 4, szTemp, 0xFFffb912);
					...
					CSize size = m_p2DRender->m_pFont->GetTextExtent(szTemp);
#ifdef __AEGON_THEME_SWITCHER
					m_p2DRender->TextOut(pt.x + 30 - size.cx, pt.y + 32 - size.cy + 4, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
			...
			for (unsigned j = 0; j < pSet->vecSet[i].vecPayItem.size(); j++)
			{
				...
				if (pPropItem)
				{
					...
					CSize size = m_p2DRender->m_pFont->GetTextExtent(szTemp);
#ifdef __AEGON_THEME_SWITCHER
					m_p2DRender->TextOut(pt.x + 30 - size.cx, pt.y + 30 - size.cy + 4, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
...
#ifdef __LEEAEG_SHOP_FINDER
	if (pWnd && pWnd->GetWndId() == APP_SHOP_FINDER)
	{
		CWndShopFinder* pWndShopFinder = (CWndShopFinder*)pWnd;
		m_nFontHeight = 40;
		CPoint pt(10, 3);

		CRect rectClient = GetClientRect();

		int nRange = pWndShopFinder->m_vecItems.size();
		...
		int index = 0;
		for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < pWndShopFinder->m_vecItems.size(); i++)
		{
			if (i > (unsigned)(nPage + m_wndScrollBar.GetScrollPos()))
				break;

			ItemProp* pItemProp = pWndShopFinder->m_vecItems[i]->itemElem.GetPropA();

			DWORD dwCurrencyItem = 0;
			ItemProp* pCurrencyProp;
			CTexture* currencyTexture;

			switch (pWndShopFinder->m_vecItems[i]->itemElem.m_nBuyMode)
			{
				case 1: dwCurrencyItem = II_GOLD_SEED1; break;
				case 2: dwCurrencyItem = II_SYS_SYS_SCR_PERIN; break;
				case 3: dwCurrencyItem = II_CHP_RED; break;
				case 4: dwCurrencyItem = II_SYS_REDPERIN; break;
			}
			if (dwCurrencyItem != 0)
				pCurrencyProp = prj.GetItemProp(dwCurrencyItem);
			...
			if (pItemProp)
			{
				...
				if (i == m_nCurSelect)
				{
					CRect DrawRect = CRect(0, pt.y, rectClient.right - 6, pt.y + 34);
#ifdef __AEGON_THEME_SWITCHER
					p2DRender->RenderFillRect(DrawRect, RenderFillRect);
					p2DRender->RenderRect(DrawRect, RenderRect);
#else //__AEGON_THEME_SWITCHER
				...
				if (rectToolTip.PtInRect(m_ptMouse))
				{
					...
					g_WndMng.PutToolTip_Item(&Item, pt2, &rectToolTip, APP_SHOP_FINDER);
				}

				// Item Texture
				strPath.Format(DIR_ITEM  "%s", pItemProp->szIcon);
				itemTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG);
				...
				{
					strName = strName.Left(16);
					strName += "...";
				}
				p2DRender->TextOut(pt.x + 40, pt.y + 10, strName, dwColor);

				// Seller Name
				CString Seller = pWndShopFinder->m_vecItems[i]->nameSeller;
				p2DRender->TextOut(pt.x + 220 - (p2DRender->GetFont()->GetTextExtent(Seller).cx / 2), pt.y + 10, Seller, dwColor);

				// Item Price
				int Price = pWndShopFinder->m_vecItems[i]->itemElem.m_nCost;
#ifdef __CHIP_PLAYERSHOP
				CString totalprice, tmpPrice;
				tmpPrice.Format("%d", Price);
				
				totalprice.Format("%s", GetNumberFormatEx(tmpPrice.GetString()).GetString());

				//Currency Icon
				if (pCurrencyProp)
				{
					CString strCurrencyPath;
					strCurrencyPath.Format(DIR_ITEM  "%s", pCurrencyProp->szIcon);
					currencyTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strCurrencyPath, COLOR_ITEM_BG);

					p2DRender->TextOut(pt.x + 440 - p2DRender->GetFont()->GetTextExtent(totalprice).cx - currencyTexture->m_size.cx - 5, pt.y + 10, totalprice, dwColor);

					CPoint currencyPoint(pt.x + 440 - currencyTexture->m_size.cx, pt.y);
					currencyTexture->Render(p2DRender, currencyPoint);
				}
				else
				{
					p2DRender->TextOut(pt.x + 440 - p2DRender->GetFont()->GetTextExtent(totalprice).cx, pt.y + 10, totalprice, dwColor);
				}
#else //__CHIP_PLAYERSHOP
				p2DRender->TextOut(pt.x + 370, pt.y + 10, Price, dwColor);
#endif //__CHIP_PLAYERSHOP

				// Location
				CWndButton* buttLocation = (CWndButton*)pWndShopFinder->GetDlgItem(WIDC_BUTTON5);
				CString location;
				location.Format("%s", pWndShopFinder->m_vecItems[i]->location);
				p2DRender->TextOut(pt.x + 515 - (p2DRender->GetFont()->GetTextExtent(location).cx / 2), pt.y + 10, location, dwColor);
...
#ifdef __GIFTBOX_LIST_ITEMS
	if (pWnd && pWnd->GetWndId() == APP_GIFTBOX)
	{
		CWndGiftbox* wnd = (CWndGiftbox*)pWnd;
		m_nFontHeight = 36;
		auto list = wnd->Get();
		if (list.size() == 0)
			return;
		...
		int nPage = GetClientRect().Height() / m_nFontHeight;
		m_wndScrollBar.SetScrollRange(0, list.size() + 1);
		m_wndScrollBar.SetScrollPage(nPage);
		m_wndScrollBar.SetVisible(TRUE);

		for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < (unsigned)(list.size()); ++i)
		{
			auto it = list[i];
			CPoint ptIcon = pt;
			ptIcon.y += 3;
			...
			ItemProp* p = prj.GetItemProp(it.first);
			if (!p)
			{
				Error("Invalid ItemID %d in GiftBox Preview", it.first);
				continue;
			}
			...
			if (i == m_nCurSelect)
			{
				CRect DrawRect = CRect(0, pt.y, GetWndRect().right, pt.y + 34);
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->RenderFillRect(DrawRect, RenderFillRect);
				p2DRender->RenderRect(DrawRect, RenderRect);
#else //__AEGON_THEME_SWITCHER
			...
			pItemElem->m_dwItemId = p->dwID;
			pItemElem->m_nItemNum = 1;
			pItemElem->m_bCharged = p->bCharged;

			CTexture* pIcon = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, p->szIcon), COLOR_ITEM_BG);
			if (pIcon)
				pIcon->Render(p2DRender, ptIcon, 0xFFFFFFFF);

			CPoint ptLineStart, ptLineEnd;
			ptLineStart = CPoint(0, pt.y + 36);
			ptLineEnd = CPoint((pt.x + rect.right) - 10, pt.y + 36);
			p2DRender->RenderLine(ptLineStart, ptLineEnd, 0xFF776655);

			pt.x += 40;
			CString itemname;
			itemname.Format("%s", p->szName);
			if (itemname.GetLength() >= 23)
			{
				itemname = itemname.Left(20);
				itemname += "...";
			}
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + 1, pt.y + 8, itemname, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + 1, pt.y + 8, itemname, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER

			pt.x += 170;
			CString count;
			count.Format("%d", it.second.first);
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + 1, pt.y + 8, count, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + 1, pt.y + 8, count, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER

			CString chancestring;
			chancestring.Format("%0.2f%%", it.second.second / 10000);
			pt.x += 70;
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x, pt.y + 8, chancestring, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x, pt.y + 8, chancestring, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER

			pt.x = 0;
			pt.y += m_nFontHeight + 4;

			CRect pRect;
			pRect.SetRect(ptIcon, ptLineEnd);
			CPoint pMousePoint = GetMousePoint();
			if (pRect.PtInRect(pMousePoint) && GetWndRect().PtInRect(pMousePoint))
			{
				PUT_TOOL_TIP(*pRect, pMousePoint, pItemElem);
			}
		}
	}
#endif //__GIFTBOX_LIST_ITEMS
	...
	if (pWnd && pWnd->GetWndId() == APP_PARTY_FIND)
	{
		...
		if (pWndPartyFind)
		{
			...
			for (size_t i = 0; i < tmp.size(); i++)
			{
				...
				if (i == m_nCurSelect)
				{
					dwColor = D3DCOLOR_XRGB(97, 97, 255);
				}
				else {
#ifdef __AEGON_THEME_SWITCHER
					dwColor = CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
				...
				{
					const ItemProp* pItemProp = prj.GetItemProp(II_CHR_SYS_SCR_PSKILLFULL1);
					if (pItemProp)
					{
						if (std::strlen(pItemProp->szIcon))
						{
							pIcon = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, pItemProp->szIcon), COLOR_ITEM_BG);
							if (pIcon)
								p2DRender->RenderTexture(pt, pIcon, 255, 0.50f, 0.50f);
						}
					}
				}
				pt.x += 18;
				if (tmp[i].m_bExpUp)
				{
					const ItemProp* pItemProp = prj.GetItemProp(II_SYS_SYS_SCR_PARTYEXPUP01);
					if (pItemProp)
					{
						if (std::strlen(pItemProp->szIcon))
						{
				...
				if (tmp[i].m_bSkillUp)
				{
					const ItemProp* pItemProp = prj.GetItemProp(II_SYS_SYS_SCR_PARTYSKILLUP01);
					if (pItemProp)
					{
						if (std::strlen(pItemProp->szIcon))
						{
				...
				pt.x += 18;
				if (tmp[i].m_bLinkAttack)
				{
					const ItemProp* pItemProp = prj.GetPartySkill(ST_LINKATTACK);
					if (pItemProp)
					{
						if (std::strlen(pItemProp->szIcon))
						{
							pIcon = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ICON, pItemProp->szIcon), COLOR_ITEM_BG);
							if (pIcon)
								p2DRender->RenderTexture(pt, pIcon, 255, 0.50f, 0.50f);
						}
					}
				...
				pt.x += 15;
				if (tmp[i].m_bFortuneCircle)
				{
					const ItemProp* pItemProp = prj.GetPartySkill(ST_FORTUNECIRCLE);
					if (pItemProp)
					{
						if (std::strlen(pItemProp->szIcon))
						{
				...
				if (tmp[i].m_bGiftBox)
				{
					const ItemProp* pItemProp = prj.GetPartySkill(ST_GIFTBOX);
					if (pItemProp)
					{
						if (std::strlen(pItemProp->szIcon))
						{
...
#ifdef __WIKIPEDIA_MONSTER
	if (pWnd && pWnd->GetWndId() == APP_WIKIPEDIA_MONSTER)
	{
		m_nFontHeight = 35;
		CPoint pt(10, 3);
		vector<MoverProp*>* pVecMover = ((CWndWikipediaMonster*)pWnd)->GetMoverList();
		CRect rectClient = GetClientRect();
		int nPage = rectClient.Height() / m_nFontHeight;
		m_wndScrollBar.SetScrollRange(0, pVecMover->size());
		m_wndScrollBar.SetScrollPage(nPage);

		for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < (unsigned)(pVecMover->size()); ++i)
		{
			if (i > (unsigned)(nPage + m_wndScrollBar.GetScrollPos()))
				break;

#ifdef __AEGON_THEME_SWITCHER
			DWORD dwRenderLine = CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
			DWORD RenderFillRect = CAegonThemeSwitcherMng::GetInstance()->GetRenderFillRectColor();
			DWORD RenderRect = CAegonThemeSwitcherMng::GetInstance()->GetRenderRectColor();
#endif //__AEGON_THEME_SWITCHER

			p2DRender->RenderLine(CPoint(-2, pt.y + 35), CPoint(rectClient.right - 6, pt.y + 35), dwRenderLine);
			...
			// CTexture pTex;
			DWORD dwText;
			switch (pVecMover->at(i)->eElementType)
			{
			case SAI79::FIRE:
				dwText = TID_UPGRADE_FIRE;
				// pTex.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "eElementTypeRed01-2.tga" ), COLOR_ITEM_BG, TRUE);
				break;
			case SAI79::WATER:
				dwText = TID_UPGRADE_WATER;
				// pTex.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "eElementTypeBlue01-2.tga" ), COLOR_ITEM_BG, TRUE);
				break;
			case SAI79::ELECTRICITY:
				dwText = TID_UPGRADE_ELECTRICITY;
				// pTex.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "eElementTypeYellow01-2.tga" ), COLOR_ITEM_BG, TRUE);
				break;
			case SAI79::WIND:
				dwText = TID_UPGRADE_WIND;
				// pTex.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "eElementTypeGreen01-2.tga" ), COLOR_ITEM_BG, TRUE);
				break;
			case SAI79::EARTH:
				dwText = TID_UPGRADE_EARTH;
				// pTex.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "eElementTypePurple01-2.tga" ), COLOR_ITEM_BG, TRUE);
				break;
			default:
				dwText = TID_GAME_DROP_NONE;
				// pTex.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "eElementTypeWhite01-2.tga" ), COLOR_ITEM_BG, TRUE);
				break;
			}
			// pTex.Render( p2DRender, CPoint( pt.x, pt.y + 6 ) );
			CRect rectToolTip(pt.x, pt.y, pt.x + 35, pt.y + 35);
			if (rectToolTip.PtInRect(m_ptMouse))
			{
				CPoint pt2 = m_ptMouse;
				ClientToScreen(&pt2);
				ClientToScreen(&rectToolTip);
				g_toolTip.PutToolTip(100, prj.GetText(dwText), rectToolTip, pt2, 0);
			}

			if (pVecMover->at(i)->eElementType)
			{
				g_WndMng.m_pWndWorld->m_texAttrIcon.Render(p2DRender, CPoint(pt.x, pt.y + 5), pVecMover->at(i)->eElementType - 1, 255, 1.5f, 1.5f);
			}
			else
			{
				CTexture pTex;
				pTex.LoadTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, "eElementTypeNone.tga"), COLOR_ITEM_BG, TRUE);
				pTex.Render(p2DRender, CPoint(pt.x, pt.y + 6));
			}

			pt.y += 10;
			pt.x += 50;
			CString szTemp = pVecMover->at(i)->szName;
			if (szTemp.GetLength() > 16)
			{
				int	nReduceCount = 0;
				while (nReduceCount < 16)
				{
					if (IsDBCSLeadByte(szTemp[nReduceCount]))
						nReduceCount += 2;
					else
						nReduceCount++;
				}
				szTemp = szTemp.Left(nReduceCount);
				szTemp += "...";

				CRect rectToolTipName(pt.x - 10, pt.y, pt.x + 155, pt.y + 25);
				if (rectToolTipName.PtInRect(m_ptMouse))
				{
					CPoint pt2 = m_ptMouse;
					ClientToScreen(&pt2);
					ClientToScreen(&rectToolTipName);
					g_toolTip.PutToolTip(100, pVecMover->at(i)->szName, rectToolTipName, pt2, 0);
				}
			}
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x, pt.y, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x, pt.y, szTemp, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER

			// Level
			pt.x += 170;
			szTemp.Format("%d", pVecMover->at(i)->dwLevel);
			if (pVecMover->at(i)->dwLevel > 99) 
			{
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->TextOut(pt.x, pt.y, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			...
			else if (pVecMover->at(i)->dwLevel > 9) 
			{
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->TextOut(pt.x + 3, pt.y, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
				p2DRender->TextOut(pt.x + 3, pt.y, szTemp, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER
			}
			else 
			{
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->TextOut(pt.x + 6, pt.y, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
				p2DRender->TextOut(pt.x + 6, pt.y, szTemp, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER
			}
			...
			// Attack
			pt.x += 60;
			szTemp.Format("%d ~ %d", pVecMover->at(i)->dwAtkMin, pVecMover->at(i)->dwAtkMax);
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + (15 - szTemp.GetLength()) * 2, pt.y, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + (15 - szTemp.GetLength()) * 2, pt.y, szTemp, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER

			// Health
			pt.x += 120;
			szTemp.Format("%d", pVecMover->at(i)->dwAddHp);
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + (13 - szTemp.GetLength()) * 2, pt.y, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + (13 - szTemp.GetLength()) * 2, pt.y, szTemp, 0xFF000000);
#endif // __AEGON_THEME_SWITCHER

			// Exp
			pt.x += 95;
			szTemp.Format("%d", (int)pVecMover->at(i)->nExpValue);
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + (13 - szTemp.GetLength()) * 2, pt.y, szTemp, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
			p2DRender->TextOut(pt.x + (13 - szTemp.GetLength()) * 2, pt.y, szTemp, 0xFF000000);
	...
	if (pWnd && pWnd->GetWndId() == APP_WIKIPEDIA_TITLE)
	{
		int nTitleCount = ((CWndHonorTitle*)pWnd)->GetMapSize();//CTitleManager::Instance()->m_nCurrentTitleCount;
		int nRange = nTitleCount;
		int nPage = GetClientRect().Height() / m_nFontHeight;
		m_wndScrollBar.SetScrollRange(0, nRange);
		m_wndScrollBar.SetScrollPage(nPage);
	}
	...
	if (pWnd && pWnd->GetWndId() == APP_DP_CONFIRM)
	{
		CWndIngameDPPurchaseConfirm* pWndConfirmDPPurchase = (CWndIngameDPPurchaseConfirm*)pWnd;
		DWORD dwColorText = CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
		CPoint pt(0, 0);
		m_nFontHeight = 40;
		CRect rectClient = GetClientRect();
		int nPage = 120 / m_nFontHeight;
		m_wndScrollBar.SetScrollRange(0, pWndConfirmDPPurchase->m_vItemQuantityTuple.size());
		m_wndScrollBar.SetScrollPage(nPage);



		for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < (unsigned)pWndConfirmDPPurchase->m_vItemQuantityTuple.size(); i++)
		{
			if (i > (unsigned)(nPage + m_wndScrollBar.GetScrollPos()))
				break;

			p2DRender->RenderLine(CPoint(-2, pt.y + 37), CPoint(rectClient.right - 3, pt.y + 37), dwColorText);


			DWORD dwID = std::get<0>(pWndConfirmDPPurchase->m_vItemQuantityTuple[i]);
			int nNum = std::get<1>(pWndConfirmDPPurchase->m_vItemQuantityTuple[i]);

			ItemProp* pProp = prj.GetItemProp(dwID);
			if (pProp)
			{
				CTexture* pItemTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ITEM, pProp->szIcon), COLOR_ITEM_BG);
				if (pItemTexture) 
				{
					p2DRender->RenderTexture(pt, pItemTexture);
				}
			}
			CString tmpStr;
			tmpStr.Format("%dx", nNum);
			CSize textExtent = p2DRender->GetFont()->GetTextExtent(tmpStr.GetString());
			p2DRender->TextOut(pt.x + rectClient.Width() - textExtent.cx - 5, pt.y + (m_nFontHeight / 2) - (textExtent.cy / 2), tmpStr.GetString(), CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
			

			CRect rectToolTip(pt.x, pt.y, pt.x + rectClient.Width(), pt.y + 37);
			if (rectToolTip.PtInRect(m_ptMouse))
			{
				CPoint pt2 = m_ptMouse;
				ClientToScreen(&pt2);
				...
				g_WndMng.PutToolTip_Item(&Item, pt2, &rectToolTip, APP_DP_CONFIRM);
			}
			pt.y += m_nFontHeight;
		}
	...
	if (pWnd && pWnd->GetWndId() == APP_WIKI_ITEMS)
	{
		...
		for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < (unsigned)(pVecItems->size()); ++i)
		{
			...
			if (i == m_nCurSelect)
			{
				CRect DrawRect = CRect(0, pt.y, rectClient.right, pt.y + 34);
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->RenderFillRect(DrawRect, RenderFillRect);
				p2DRender->RenderRect(DrawRect, RenderRect);
#else //__AEGON_THEME_SWITCHER
			...
			if (strlen(pProp->szIcon))
			{
				...
				if (pProp->dwItemKind3 == IK3_EGG && pProp->dwID != II_PET_EGG)
					szIcon.Replace(".", "_00.");
				CTexture* pIcon = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, szIcon), COLOR_ITEM_BG);
			...
			if (strName.GetLength() >= 29 && pProp->dwItemJob != NULL_ID)
			{
				strName = strName.Left(27);
				strName += "...";
			}
#ifdef __AEGON_THEME_SWITCHER
			dwColor = CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	if (pWnd && pWnd->GetWndId() == APP_AEGON_ACHIEVEMENT_SYSTEM)
	{
		CString strStatusIcon;
		for (int i = 0; i < AEGACH_STATUS_MAX; i++)
		{
			if (m_AchievementStatusIcon[i] == NULL) 
			{
				switch (i) 
				{
				case AEGACH_STATUS_COMPLETED:
					strStatusIcon.Format("%s", MakePath(DIR_THEME_ACHIEVEMENT, "Achievement_Completed.dds").GetString());
					break;
				case AEGACH_STATUS_NOTSTARTED:
					strStatusIcon.Format("%s", MakePath(DIR_THEME_ACHIEVEMENT, "Achievement_NoStarted.dds").GetString());
					break;
				case AEGACH_STATUS_REQUIREMENTS:
					strStatusIcon.Format("%s", MakePath(DIR_THEME_ACHIEVEMENT, "Achievement_OtherMission.dds").GetString());
					break;
				case AEGACH_STATUS_STARTED:
					strStatusIcon.Format("%s", MakePath(DIR_THEME_ACHIEVEMENT, "Achievement_Started.dds").GetString());
					break;
				}
				
				m_AchievementStatusIcon[i] = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strStatusIcon, COLOR_ITEM_BG);
			}
		}

		CWndAchievementSystemMain* pWndAchievementSystem = (CWndAchievementSystemMain*)pWnd;
		m_nFontHeight = 40;
		CPoint pt(10, 0);

		CRect rectClient = GetClientRect();

		int nRange = pWndAchievementSystem->m_vecItems.size();
		int scrollPos = m_wndScrollBar.GetScrollPos();
		...
		int index = 0;
		for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < pWndAchievementSystem->m_vecItems.size(); i++)
		{
			if (i > (unsigned)(nPage + m_wndScrollBar.GetScrollPos()))
				break;

			pt.x = 10;
			ItemProp* pItemProp = prj.GetItemProp(pWndAchievementSystem->m_vecItems[i]->reward);
			...
			// Render Line
			p2DRender->RenderLine(CPoint(0, pt.y + m_nFontHeight), CPoint(rectClient.right - 3, pt.y + m_nFontHeight), dwColor);

			if (i == m_nCurSelect)
			{
				CRect DrawRect = CRect(0, pt.y + 3, rectClient.right - 6, pt.y + 37);
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->RenderFillRect(DrawRect, RenderFillRect);
				p2DRender->RenderRect(DrawRect, RenderRect);
#else //__AEGON_THEME_SWITCHER
			...
			CRect name = pWndAchievementSystem->GetDlgItem(WIDC_STATIC4)->GetWndRect();
			CRect mission = pWndAchievementSystem->GetDlgItem(WIDC_STATIC5)->GetWndRect();
			CRect reward = pWndAchievementSystem->GetDlgItem(WIDC_STATIC6)->GetWndRect();

			// status icon
			int statusIconNum = CAegonAchievementMng::GetInstance()->GetAchievementStatus(pWndAchievementSystem->m_vecItems[i]->achievementID, g_pPlayer);
			if (statusIconNum >= 0 && statusIconNum < AEGACH_STATUS_MAX && m_AchievementStatusIcon[statusIconNum])
			{
				m_AchievementStatusIcon[statusIconNum]->Render(p2DRender, CPoint(pt.x, pt.y + (m_nFontHeight / 2) - (m_AchievementStatusIcon[statusIconNum]->m_size.cy / 2)));
				pt.x += m_AchievementStatusIcon[statusIconNum]->m_size.cx + 5;
			}
			
			// Name
			CString strName = pWndAchievementSystem->m_vecItems[i]->achievementName;

			CSize strExtent = p2DRender->GetFont()->GetTextExtent(strName.GetString());

			BOOL bCutOff = FALSE;
			int nCutOffPoint = mission.left - GetWndRect().left - 5;
			if (pt.x + strExtent.cx > nCutOffPoint)
			{
				while (pt.x + (strExtent.cx + p2DRender->GetFont()->GetTextExtent("...").cx) > nCutOffPoint)
				{
					strName = strName.Left(strName.GetLength() - 1);
					strExtent = p2DRender->GetFont()->GetTextExtent(strName.GetString());
				}
				bCutOff = TRUE;
			}
			...
			p2DRender->TextOut(pt.x, pt.y + (m_nFontHeight / 2) - (p2DRender->GetFont()->GetTextExtent(strName.GetString()).cy / 2), strName, dwColor);

			// Mission
			pt.x = mission.left - GetWndRect().left;
			CString strMission;
			strMission.Format("%s", prj.GetText(pWndAchievementSystem->m_vecItems[i]->shortDescriptionTID));
			
			// Trim Mission Length					
			CString Trim;
			Trim.Format("%s", strMission.GetString());
			if (Trim.GetLength() > 45)
			{
				Trim = Trim.Left(42);
				Trim += "...";
			}

			p2DRender->TextOut(pt.x, pt.y + (m_nFontHeight / 2) - (p2DRender->GetFont()->GetTextExtent(Trim.GetString()).cy / 2), Trim, dwColor);
			...
			if (pItemProp)
			{
				...
				if (itemTexture) 
				{
					pt.x = (reward.left - GetWndRect().left) + (reward.right - reward.left) / 2 - (itemTexture->m_size.cx / 2) ;
					// Tooltip
					CRect rectToolTip(pt.x, pt.y, pt.x + 35, pt.y + 35);
					if (rectToolTip.PtInRect(m_ptMouse))
					{
						...
						CItemElem Item;
						Item.m_dwItemId = pItemProp->dwID;
						Item.m_nItemNum = pWndAchievementSystem->m_vecItems[i]->rewardNum;
						if (!pWndAchievementSystem->m_vecItems[i]->bRewardTradeable)
							Item.SetFlag(CItemElem::binds);
						g_WndMng.PutToolTip_Item(&Item, pt2, &rectToolTip, APP_AEGON_ACHIEVEMENT_SYSTEM);
					}

					// Render Item Texture
					itemTexture->Render(p2DRender, CPoint(pt.x, pt.y + (m_nFontHeight / 2) - (itemTexture->m_size.cy / 2)));
				}
...
#ifdef __BUYBACK
	if (pWnd && pWnd->GetWndId() == APP_BUYBACK)
	{
		m_nFontHeight = 35;
		CPoint pt(10, 3);
		...
		CRect rectClient = GetClientRect();
		int nPage = rectClient.Height() / m_nFontHeight;
		m_wndScrollBar.SetScrollRange(0, g_pPlayer->m_BuyBack.GetCount());
		m_wndScrollBar.SetScrollPage(nPage);
		for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < (unsigned)(g_pPlayer->m_BuyBack.GetCount()); i++)
		{
			if (i > (unsigned)(nPage + m_wndScrollBar.GetScrollPos()))
				break;

			CItemElem* pItemElem = g_pPlayer->m_BuyBack.GetAt(i);
			if (pItemElem)
			{
#ifdef __AEGON_THEME_SWITCHER
				DWORD dwRenderLine = CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
				...
				if (strlen(pItemElem->GetProp()->szIcon))
				{
					CTexture* pIcon = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, pItemElem->GetProp()->szIcon), COLOR_ITEM_BG);
					pIcon->Render(p2DRender, pt);
				}
				CRect rectToolTip(pt.x, pt.y, pt.x + 35, pt.y + 35);
				if (rectToolTip.PtInRect(m_ptMouse))
				...
				szPrice.Format("%I64d", (__int64)pItemElem->getBuyBackPrice() * pItemElem->m_nItemNum);
...
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->TextOut(pt.x, pt.y, szName, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
				p2DRender->TextOut(pt.x + 200 + ((3 - szItemQt.GetLength()) * 2), pt.y, szItemQt, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
				p2DRender->TextOut(pt.x + 280 + ((13 - szPrice.GetLength()) * 2), pt.y, szPrice, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
				p2DRender->TextOut(pt.x, pt.y, szName, COLOR_BLACK);
				p2DRender->TextOut(pt.x + 200 + ((3 - szItemQt.GetLength()) * 2), pt.y, szItemQt, COLOR_BLACK);
				p2DRender->TextOut(pt.x + 280 + ((13 - szPrice.GetLength()) * 2), pt.y, szPrice, COLOR_BLACK);
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __WIKIPEDIA_MONSTER_DROPLIST
	if (pWnd && pWnd->GetWndId() == APP_DROP_INFORMATION)
	{
#ifdef __AEGON_THEME_SWITCHER
		DWORD dwColorText = CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
#endif

		CWndDropInfo* pWndDropInfo = (CWndDropInfo*)pWnd;
		DWORD dwMonsterID = pWndDropInfo->GetMonsterID();
		MoverProp* pMoverProp = prj.GetMoverProp(dwMonsterID);
		vector<ItemProp*> pPropVector;
		if (pMoverProp)
		{
			for (unsigned i = 0; i < (unsigned)pMoverProp->m_DropList.size(); ++i)
			{
				DROPITEM & dropItem = pMoverProp->m_DropList[i];
				ItemProp *pProp = prj.GetItemProp(dropItem.dwIndex);
				pPropVector.push_back(pProp);
			}
			...
			int nPage = rectClient.Height() / m_nFontHeight;
			m_wndScrollBar.SetScrollRange(0, pPropVector.size());
			m_wndScrollBar.SetScrollPage(nPage);

			for (unsigned i = (unsigned)m_wndScrollBar.GetScrollPos(); i < (unsigned)(pPropVector.size()); ++i)
			{
				if (pPropVector[i])
				{
					if (i >(unsigned)(nPage + m_wndScrollBar.GetScrollPos()))
					...
					// Line
#ifdef __AEGON_THEME_SWITCHER
					p2DRender->RenderLine(CPoint(-2, pt.y + 37), CPoint(rectClient.right, pt.y + 37), dwColorText);
#else
					p2DRender->RenderLine(CPoint(-2, pt.y + 37), CPoint(rectClient.right, pt.y + 37), 0xFF776655);
#endif

					// Item Icon
					if (strlen(pPropVector[i]->szIcon))
					{
						CTexture* pIcon = CWndBase::m_textureMng.AddTexture(p2DRender->m_pd3dDevice, MakePath(DIR_ITEM, pPropVector[i]->szIcon), 0xffff00ff);
						pIcon->Render(p2DRender, pt);
					}
					...
					if (rectToolTip.PtInRect(m_ptMouse))
					{
						...
						ClientToScreen(&rectToolTip);
						CItemElem Item;
						Item.m_dwItemId = pPropVector[i]->dwID;
						g_WndMng.PutToolTip_Item(&Item, pt2, &rectToolTip, APP_DROP_INFORMATION);
					}
					...
					CString Trim;
					Trim.Format("%s", pPropVector[i]->szName);
					if (Trim.GetLength() > 22)
					{
						Trim = Trim.Left(20);
						Trim += "...";
					}
#ifdef __AEGON_THEME_SWITCHER
					p2DRender->TextOut(pt.x, pt.y, Trim, dwColorText);
#else // __AEGON_THEME_SWITCHER
					p2DRender->TextOut(pt.x, pt.y, Trim, COLOR_BLACK);
#endif // __AEGON_THEME_SWITCHER

					CString chancestring;

					double fChance = (((float)pMoverProp->m_DropList[i].dwProbability / (3000000000.0f / 100.0f)));
#ifdef __AEGON_CLIENTSIDE_PIECE_DROPRATE
					fChance *= prj.m_fItemPieceDropRate;
#endif
					if (g_pPlayer) 
					{
						if (g_pPlayer->HasBuff(BUFF_ITEM, II_SYS_SYS_LS_CHEERING))
							fChance *= 1.1F;

						if (g_pPlayer->HasBuff(BUFF_ITEM, II_SYS_SYS_VOTE_THANKS))
							fChance *= 1.05F;

						if (g_pPlayer->HasBuff(BUFF_ITEM, II_COUPLE_BUFF_MIRACLE_01))
							fChance *= 1.15F;

						fChance *= (g_pPlayer->GetAdjParam(DST_CHANCEDROP_PIECE) / 100) + 1.0f;

						fChance *= CPCBangInfo::GetInstance()->GetPieceItemDropFactor();
					}
					if (fChance < 0)
						fChance = 0.0f;
					else if (fChance > 100)
						fChance = 100.0f;
					chancestring.Format("%0.4f%%", fChance);
#ifdef __AEGON_THEME_SWITCHER
					p2DRender->TextOut(GetWndRect().Width() - 30 - p2DRender->GetFont()->GetTextExtent(chancestring.GetString()).cx, pt.y, chancestring, dwColorText);
#else // __AEGON_THEME_SWITCHER
					p2DRender->TextOut(pt.x + 170, pt.y, chancestring, COLOR_BLACK);
#endif // __AEGON_THEME_SWITCHER
...
#endif //__NEW_EXCHANGE_V19
#ifdef __LEEAEG_SHOP_FINDER
		if (pWnd->GetWndId() == APP_SHOP_FINDER)
		{
			CWndShopFinder* pWndShopFinder = (CWndShopFinder*)pWnd;
			CRect rect;

			for (int i = 0; i < pWndShopFinder->m_vecItems.size(); i++)
			{
				int nScrollBarWidth = IsWndStyle(WBS_VSCROLL) ? m_wndScrollBar.GetClientRect().Width() : 0;
				rect.SetRect(pt.x, pt.y, pt.x + m_rectWindow.Width() - nScrollBarWidth, pt.y + m_nFontHeight);
				if (rect.PtInRect(point))
...
#endif // __LEEAEG_SHOP_FINDER
#ifdef __WIKIPEDIA_TITLE
		if (pWnd->GetWndId() == APP_WIKIPEDIA_TITLE)
		{
			CPoint pt(0, 0);
			unsigned nPos = m_wndScrollBar.GetScrollPos();
			unsigned nEnd = nPos + 8;
...
#ifdef _WIN64
INT_PTR CWndListBox::GetItemData(int nIndex) const
#else
...
#ifdef _WIN64
int CWndListBox::SetItemData(int nIndex, INT_PTR dwItemData)
#else
...
int CWndListBox::SetItemDataPtr(int nIndex,void* pData)
{
	...
	LPLISTITEM lpListItem = (LPLISTITEM)m_listItemArray.GetAt(nIndex);
#ifdef _WIN64
	lpListItem->m_dwData = reinterpret_cast<INT_PTR>(pData);
#else
...
#if __VER >= 15 // __IMPROVE_QUEST_INTERFACE
#ifdef _WIN64
INT_PTR CWndListBox::GetItemData2(int nIndex) const
#else
...
#ifdef _WIN64
int CWndListBox::SetItemData2(int nIndex, INT_PTR dwItemData)
#else
...
int CWndListBox::SetItemData2Ptr( int nIndex,void* pData )
{
	...
	LPLISTITEM lpListItem = ( LPLISTITEM )m_listItemArray.GetAt( nIndex );
#ifdef _WIN64
	lpListItem->m_dwData2 = reinterpret_cast<INT_PTR>(pData);
#else
...
#ifdef __AEGON_THEME_SWITCHER
void CWndListBox::OnThemeSelectionChanged() 
{
	m_wndScrollBar.OnThemeSelectionChanged();
...
void CWndListBox::SetTextColors() 
{
	m_nFontColor = D3DCOLOR_ARGB(255, 64, 64, 64);
	m_nSelectColor = D3DCOLOR_ARGB(255, 64, 64, 255);
	m_dwOnMouseColor = D3DCOLOR_ARGB(255, 255, 128, 0);
...
void CWndText::OnInitialUpdate()
{
	...
	m_stringPlaceholder.Init(m_pFont, &GetClientRect());;
#endif // __PLACEHOLDER_TEXT 

#ifdef __AEGON_THEME_SWITCHER
	m_string.SetColor(CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
	m_stringPlaceholder.SetColor(CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __PLACEHOLDER_TEXT
	m_stringPlaceholder.Init(m_pFont, &rect);;
#endif // __PLACEHOLDER_TEXT
...
void CWndText::OnDraw( C2DRender* p2DRender )
{
	...
	if (g_WndMng.m_dwFontSwitched > 0)
	{
		m_pFont = CWndBase::m_Theme.m_pFontText;
		if (GetTickCount() > g_WndMng.m_dwFontSwitched + 1000)
			g_WndMng.m_dwFontSwitched = 0;
	}
	...
	{
		if (m_stringPlaceholder.GetLineCount() > 0)
		{
			BlockSetStyle(ESSTY_BLOCK);
			p2DRender->TextOut_EditString(2, 0, m_stringPlaceholder, 0, 0, m_nLineSpace);
		}
		return;
	}
#endif // __PLACEHOLDER_TEXT
...
#ifdef __AEGON_THEME_SWITCHER
						dwColor = ~dwColor | CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
#else // __AEGON_THEME_SWITCHER
...
#ifdef __PLACEHOLDER_TEXT
void CWndText::SetPlaceholderString(LPCTSTR pszString, DWORD dwColor)
{
	m_stringPlaceholder.Init(m_pFont, &GetClientRect());
	m_stringPlaceholder.SetParsingString(pszString, dwColor, 0x00000000, 0, 0x00000001, FALSE);
}
#endif // __PLACEHOLDER_TEXT
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
void CWndText::ForceSetEditString(CEditString strEdit) 
{
	m_string = strEdit;
...
#ifdef __AEGON_THEME_SWITCHER
void CWndText::OnThemeSelectionChanged()
{
	m_string.SetColor(CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
...
{
#ifdef __AEGON_THEME_SWITCHER
	DWORD dwColor = D3DCOLOR_ARGB(255, 226, 198, 181);
	map<int, __AEGON_THEME_UNIT>::iterator it = CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.find(g_Option.m_nSelectedThemeID);
	...
	if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
	{
		...
		{
			dwColor = it->second.dwColorFrameWnd;
		}
	}

	p2DRender->RenderFillRect(GetWindowRect(), dwColor);
#else // __AEGON_THEME_SWITCHER
...
BOOL CWndMenu::OnEraseBkgnd( C2DRender* p2DRender )
{
	...
	if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
	{
		...
		{
			dwColor = it->second.dwColorWnd;
		}
	}

	p2DRender->RenderFillRect(GetClientRect(), dwColor);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
void CWndListCtrl::OnThemeSelectionChanged() 
{
	SetTextColors();
	CWndBase::OnThemeSelectionChanged();
}
void CWndListCtrl::SetTextColors()
{
	m_nFontColor = D3DCOLOR_ARGB(255, 64, 64, 64);
	m_nSelectColor = D3DCOLOR_ARGB(255, 255, 0, 0);
...
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->TextOut(i * nLength + 2, y + 4, pItem->pszText, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
...
{
#ifdef __AEGON_THEME_SWITCHER
	if (IsWndStyle(WTCS_NO_DRAW_BOTTOM)) 
	{
		return;
	}
#endif // __AEGON_THEME_SWITCHER
	...
	if( m_nCurSelect >= 0 )
	{
		...
		if( nTabSize == 0 )
			nTabSize = 1;
#ifdef __AEGON_FIXES
		int nWidth = floor((double)rect.Width() / (double)8);
		int nSize = floor((double)nWidth / (double)nTabSize);
#else
...
#ifdef __AEGON_THEME_SWITCHER
		int TEXTURE_TAB_SIZE_X = m_aTexture[ 1 ].m_size.cx;
		int TEXTURE_TAB_SIZE_Y = m_aTexture[1].m_size.cy;
#else // __AEGON_THEME_SWITCHER
	...
	{
#ifdef __AEGON_FIXES
		int nLength = floor((double)rectClient.Width() / (double)nTabSize);
#else
		...
		for( int i = 0; i < nTabSize; ++i )
		{
			...
			if( pItem )
			{
				...
				case ALIGN_LEFT:
					{
#ifdef __AEGON_THEME_SWITCHER
						DWORD dwTextColor = (i != m_nCurSelect) ? 0xffffffff : CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
#else // __AEGON_THEME_SWITCHER
				...
				case ALIGN_CENTER:
					{
						...
						{
#ifdef __AEGON_FIXES
							int nRenderingPositionX = floor((double)((double)nLength - (double)size.cx) / (double)2);
#else
				...
				{
#ifdef __AEGON_THEME_SWITCHER
					p2DRender->TextOut(i* nLength + 2, y, pItem->pszText, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
...
void CWndTabCtrl::OnInitialUpdate()
{
	...
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[0], m_pApp->m_pd3dDevice, "WndTabTile10.tga", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[1], m_pApp->m_pd3dDevice, "WndTabTile11.tga", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[2], m_pApp->m_pd3dDevice, "WndTabTile12.tga", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[3], m_pApp->m_pd3dDevice, "WndTabTile13.tga", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[4], m_pApp->m_pd3dDevice, "WndTabTile14.tga", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[5], m_pApp->m_pd3dDevice, "WndTabTile15.tga", COLOR_ITEM_BG, TRUE);
	
#else // __AEGON_THEME_SWITCHER
	m_aTexture[0].LoadTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "WndTabTile10.tga"), COLOR_ITEM_BG, TRUE);
	m_aTexture[1].LoadTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "WndTabTile11.tga"), COLOR_ITEM_BG, TRUE);
	m_aTexture[2].LoadTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "WndTabTile12.tga"), COLOR_ITEM_BG, TRUE);
	m_aTexture[3].LoadTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "WndTabTile13.tga"), COLOR_ITEM_BG, TRUE);
	m_aTexture[4].LoadTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "WndTabTile14.tga"), COLOR_ITEM_BG, TRUE);
	m_aTexture[5].LoadTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "WndTabTile15.tga"), COLOR_ITEM_BG, TRUE);
#endif // __AEGON_THEME_SWITCHER
...
void CWndTabCtrl::AdditionalSkinTexture( LPWORD pDest, CSize sizeSurface, D3DFORMAT d3dFormat )
{
	...
	for( int i = 0; i < 9; i++ )
	{
		...
		if( m_strWndTileMap.Lookup( strFileName, (void*&)lpImage[i] ) == FALSE )
		{
			...
			if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
			{
				if (LoadImage(MakePath(it->second.szThemeFolder, strFileName), lpImage[i])) 
				{
					bSuccess = TRUE;
				}
			}
	...
	rectTab.right -= 15;
#ifdef __AEGON_FIXES
	nWidth = floor((double)rectTab.Width() / (double)8);
#else
...
#ifdef __AEGON_FIXES
	int nSize = floor((double)nWidth / (double)nTapSize);//m_aTab.size() ;
#else
...
BOOL CWndTabCtrl::InsertTexture( int nItem, LPCTSTR lpszFileName )
{
	...
	//m_apTexture[ nItem ]->LoadTexture( m_pApp->m_pd3dDevice, lpszFileName, COLOR_ITEM_BG, TRUE );
	return TRUE;
}
...
#ifdef __AEGON_THEME_SWITCHER
void CWndTabCtrl::OnThemeSelectionChanged() 
{
	OnThemeSwitch();
	int nTabSize = GetSize();
	for (int i = 0; i < nTabSize; i++)
	{
		LPWTCITEM tmpItem = GetTabItem(i);
		if (tmpItem->pWndBase)
		{
			tmpItem->pWndBase->OnThemeSelectionChanged();
			tmpItem->pWndBase->AdjustSubWindowsForTheme();
		}
	}
...
void CWndTabCtrl::OnThemeSwitch() 
{
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[0], m_pApp->m_pd3dDevice, "WndTabTile10.tga", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[1], m_pApp->m_pd3dDevice, "WndTabTile11.tga", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_aTexture[2], m_pApp->m_pd3dDevice, "WndTabTile12.tga", COLOR_ITEM_BG, TRUE);
...
#ifdef __AEGON_THEME_SWITCHER
	CTexture* pTextureButton = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtQuickListDn.tga", COLOR_ITEM_BG, TRUE);
	if (pTextureButton) 
	{
		CPoint ptCenter = rect.CenterPoint();
		rect.top = ptCenter.y - pTextureButton->m_size.cy / 2;
		rect.bottom = rect.top + pTextureButton->m_size.cy;
	}
...
#ifdef __AEGON_THEME_SWITCHER
	m_wndButton.m_pTexture = pTextureButton;
#else // __AEGON_THEME_SWITCHER
	m_wndButton.m_pTexture = m_textureMng.AddTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, "ButtQuickListDn.tga"), COLOR_ITEM_BG, TRUE);
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __COMBOBOX_FIX
	if (nNum > __COMBOBOX_FIX)
		nNum = __COMBOBOX_FIX;
...
int CWndComboBox::SetCurSel( int nSelect )
{
	...
	if (nSelect < 0 || nSelect >= m_wndListBox.GetCount())
		return 0;
...
#ifdef __AEGON_THEME_SWITCHER
void CWndComboBox::OnThemeSelectionChanged()
{
	m_wndButton.m_pTexture = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(m_pApp->m_pd3dDevice, "ButtQuickListDn.tga", COLOR_ITEM_BG, TRUE);

	m_wndScrollBar.OnThemeSelectionChanged();
	CWndEdit::OnThemeSelectionChanged();
}
#endif // __AEGON_THEME_SWITCHER