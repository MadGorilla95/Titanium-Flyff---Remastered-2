...
{
#ifdef __GAMEGUARD
	if (g_pPlayer == NULL || IsInvalidObj(g_pPlayer))
		return 0;
	if (g_pPlayer->IsDie())
		return 0;
#else // __GAMEGUARD
...
int		CWndWorld::ControlGround( DWORD dwMessage, CPoint point )
{
...
#ifdef __DPS_METER
	//if (g_WndMng.m_pWndWorld->m_bCtrlPushed && g_bKeyTable['D']) // Ctrl & D
	if (g_bKeyTable[g_Neuz.Key.chDpsMeter])
	{
		if (!g_WndMng.m_bToggledDpsMeter) 
		{
			CWndDpsMeter::GetInstance()->ToggleDisplay();
			g_WndMng.m_bToggledDpsMeter = TRUE;
			return nMsg;
		}
	}
	else if (g_WndMng.m_bToggledDpsMeter) 
	{
		g_WndMng.m_bToggledDpsMeter = FALSE;
	}
#endif //__DPS_METER
	...
	if( g_Neuz.m_bActiveNeuz == FALSE || ( pWndChat && pWndBaseFocus && pWndBaseFocus == pWndChat ) )
	{
		...
		g_bKeyTable[g_Neuz.Key.chQuest] = FALSE;
...
#ifdef __CHANGE_HOT_KEY
	bDown = g_bKeyTable[g_Neuz.Key.chBack];
#else // __CHANGE_HOT_KEY
	bDown = g_bKeyTable['S'];
#endif // __CHANGE_HOT_KEY
...
#ifdef __CHANGE_HOT_KEY
	bRight = g_bKeyTable[g_Neuz.Key.chRight];
#else // __CHANGE_HOT_KEY
...
#endif
#ifdef __SLOW_WALK_FIX
			if (pMover->GetWorld() && (pMover->GetWorld()->GetID() == WI_WORLD_ARENA || pMover->GetWorld()->GetID() == WI_WORLD_GUILDWAR || pMover->GetWorld()->GetID() == WI_WORLD_FWC))
			{
				bWalk = false;
			}
			else
...
//	fTurnAngle	= 0.6f;
	...
	if (g_pPlayer->m_Inventory.GetEquip(PARTS_RIDE) && (g_pPlayer->m_Inventory.GetEquip(PARTS_RIDE)->GetPropA()->dwItemKind3 == IK3_MOUNT))
		pItemProp = prj.GetItemProp(g_pPlayer->GetMountId());
	else
		pItemProp = prj.GetItemProp(g_pPlayer->GetRideItemIdx());
#else // __MOUNT
