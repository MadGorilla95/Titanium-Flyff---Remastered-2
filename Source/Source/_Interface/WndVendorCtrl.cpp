...
void CWndVendorCtrl::OnInitialUpdate( void )
{
	...
	m_pTex = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "WndPosMark.tga" ), COLOR_ITEM_BG );
}
...
void CWndVendorCtrl::OnDrawItemInfo( C2DRender* p2DRender, CItemElem* pItemElem, int nX, int nY )
{
	...
	if( pItemElem->GetAbilityOption() )
		strItem.Format( "%s +%d", pItemElem->GetName().GetString(), pItemElem->GetAbilityOption());
	else
	{
		strItem.Format( "%s", pItemElem->GetName().GetString() );
	}
...
#ifdef __AEGON_THEME_SWITCHER
	p2DRender->TextOut(nX + 50, nY + 3, strItem, dwColorText);
#else
	...
	switch (pItemElem->m_nBuyMode)
	{
		...
		case 2:
		{
			pItem = prj.GetItemProp(II_SYS_SYS_SCR_PERIN); 
			if (!pItem) return;
		}
		break;
		case 3:
		{
			pItem = prj.GetItemProp(II_CHP_RED); 
			if (!pItem) return;
		}
		break;
		case 4: 
		{
			pItem = prj.GetItemProp(II_SYS_REDPERIN); 
			if (!pItem) return;
		}
		break;
	...
	CTexture* pTexture = CWndBase::m_textureMng.AddTexture(D3DDEVICE, MakePath(DIR_ITEM, pItem->szIcon), COLOR_ITEM_BG);
	if (pTexture) {
		pTexture->Render(p2DRender, CPoint(nX + 50, nY + 15), CPoint(16, 16)); //nX + 165, nY + 6
	}
#endif // __CHIP_PLAYERSHOP
#ifdef __AEGON_THEME_SWITCHER
	DWORD dwCostColor = dwColorText;
#else
	DWORD dwCostColor = 0xff000000;
#endif

	if (dwCostTem >= 1000000000)		dwCostColor = 0xff990099;
	else if (dwCostTem >= 100000000)	dwCostColor = 0xffCC0303;
	else if (dwCostTem >= 10000000)		dwCostColor = 0xffFF6600;
	else if (dwCostTem >= 1000000)		dwCostColor = 0xff3333FF;
	else if (dwCostTem >= 100000)		dwCostColor = 0xff009900;

	p2DRender->TextOut( nX + 70, nY + 17, strItem, dwCostColor ); // nX + 50
...
#endif //__S8_VENDOR_REVISION
	{
		...
		if( pItemBase )
		{
			...
			if( pItemElem->GetProp()->dwPackMax > 1 )
			{
				...
				p2DRender->TextOut(nX - 7 + 32 - size.cx + 5, nY - 11 + 32 - size.cy + 12, szTemp, 0xFFffb912);
...
#if _WIN64
		m_GlobalShortcut.m_dwData = reinterpret_cast<UINT_PTR>(pItemBase);
#else
...
void CWndVendorCtrl::OnLButtonDblClk( UINT nFlags, CPoint point )
{
	...
	if( pItemBase )
	{
		...
		{
#ifdef __VENDING_NPC
			CWndInventory* pInventory = (CWndInventory*)g_WndMng.GetWndBase(APP_INVENTORY);
			if (pInventory && g_pPlayer->GetGold() - pItemBase->m_nCost >= 0)
			{
#endif // __VENDING_NPC
				...
				g_WndMng.PutString(prj.GetText(TID_DIAG_0048), NULL, COLOR_ERROR);
			}
#endif // __VENDING_NPC
		}
	}
...
#ifdef __AEGON_THEME_SWITCHER
	DWORD dwColor1 = CAegonThemeSwitcherMng::GetInstance()->GetTextColor();
