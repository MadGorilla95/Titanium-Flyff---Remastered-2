...
public:
	...
	void AddCaption( LPCTSTR lpszCaption, CD3DFontAPI* pFont, BOOL bChatLog = TRUE, DWORD dwColor = D3DCOLOR_ARGB(  255, 255, 255, 255 ), int nImage = 0, int nFont = 0);

#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
	CTexture* GetSystemImage(int nImage);
	CD3DFont* GetSystemFont(int nFont);
#endif // __AEGON_ADVANCED_ANNOUNCEMENT
...
public:
	...
	void RenderClashTime(CPoint ptBegin, C2DRender* p2DRender);
...
class CWndFarmCounter : public CWndNeuz
{
private:
#define WINDOWX 145
#define WINDOWY 205
#define MAX_CNTTEXTURE 12
#define MAX_CNTITEMS 10

	CTexture* m_pTexture[MAX_CNTTEXTURE];
	FLOAT oldpos;

public:
	CWndFarmCounter();
	virtual ~CWndFarmCounter();
	...
	void DoSelectItem(ItemProp* pItemProp);
	void LoadPresets();
	void SavePresets();
...
public:
	...
	char	m_szPlayerWarstr[64];
	DWORD	m_dwPlayerSiegeTime;
...
//#endif //__QUEST_HELPER
...
#ifdef __VENDING_NPC
	CRect m_rectNpcVendorIcon;
	DWORD m_dwNpcVendorItem;
#endif // __VENDING_NPC

#ifdef __PREMIUM
	DWORD m_dwPremiumItem;
#endif //__PREMIUM
	...
	BOOL UseSkillToFocusObj(CCtrl* pFocusObj, const int nIndex = -1);
...
public:
	...
	void RenderSmallOptBuffTime(C2DRender* p2DRender, CPoint& point, CTimeSpan& ct, DWORD dwColor);
	...
	BOOL m_bAchievementNoticeOpen;
	CRect m_rectAchievementNotice;
	...
	char m_szTournamentStr[256];
	DWORD m_tournamentCaptionTime;
	DWORD m_tournamentCaptionState;
