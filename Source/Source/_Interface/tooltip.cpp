...
void CToolTip::InitTexture()
{
	...
	for( int i = 0 ; i < MAX_TT ; ++i )
	{
		for( int j = 0 ; j < 9 ; ++ j )
		{
			...
			CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_apTextureToolTip[nloadTexture], g_Neuz.m_pd3dDevice, szTextName, COLOR_ITEM_BG, TRUE);
#else // __AEGON_THEME_SWITCHER
			m_apTextureToolTip[nloadTexture].LoadTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, szTextName ), COLOR_ITEM_BG, TRUE );
	...
	m_pUltimateTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG );
#ifdef __NEW_ITEM_VARUNA
	strPath = MakePath( DIR_ICON, "icon_Baruna01.dds" );
	m_pBarunaTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG );
#endif // __NEW_ITEM_VARUNA
#ifdef __AEGON_THEME_SWITCHER
	m_pJewelBgTexture = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(g_Neuz.m_pd3dDevice, "WndChgElemItem.bmp", COLOR_ITEM_BG);
#else // __AEGON_THEME_SWITCHER
	m_pJewelBgTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "WndChgElemItem.bmp"), COLOR_ITEM_BG );
#endif // __AEGON_THEME_SWITCHER
#endif //__EXT_PIERCING
}
/////////////////////////////////////////////////////////////////////////////
...
#ifdef _WIN64
void CToolTip::PutToolTip(ULONG_PTR dwToolTipId, CString& string, CRect rect, CPoint pt, int nToolTipPos)
#else
...
#ifdef _WIN64
void CToolTip::PutToolTip(ULONG_PTR dwToolTipId, LPCTSTR lpszString, CRect rect, CPoint pt, int nToolTipPos)
#else
...
#ifdef __AEGON_THEME_SWITCHER
	m_strToolTip.SetColor(CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#endif // __AEGON_THEME_SWITCHER
...
#ifdef _WIN64
void CToolTip::PutToolTip(ULONG_PTR dwToolTipId, CEditString& string, CRect rect, CPoint pt, int nToolTipPos)
#else
...
//				rect.SetRect( pt.x, pt.y, pt.x + m_rectRender.Width() + 10, pt.y + m_rectRender.Height() + (m_nSlot * 26) );
		...
		if(m_nAdded == 1)
		{
			...
			for(int i=0; i<m_nSlot; i++)
			{
				...
				if(m_nAddedJewel[i] != 0)
				{
					...
					if(pItemProp != NULL)
					{
						pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pItemProp->szIcon), COLOR_ITEM_BG );
						if(pTexture != NULL)
			...
			for(int i=0; i<m_nSlot; i++)
			{
				...
				//Slot Render
				pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "WndChgElemItem.bmp"), COLOR_ITEM_BG );
				if(pTexture != NULL)
					pTexture->RenderScal( p2DRender, point, 255, 0.7f, 0.7f );

				point.x += 36;
				pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "Window02.bmp"), COLOR_ITEM_BG );
				if(pTexture != NULL)
		...
		else if (m_nAdded == 3)
		{
			...
			CGuildMember* pMember = pGuild->GetMember(m_nPlayerId);
			int nClass = pMember->m_nClass;

			switch (pMember->m_nMemberLv)
			{
				case 0:
				{
					strPath = MakePath(DIR_ICON, "Icon_GuildLeader.dds");
					pTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG);
					if (pTexture != NULL)
						pTexture->Render(p2DRender, CPoint(PlusRect.left + 190, PlusRect.top + 10));
				}
				break;
				case 1:
				{
					strPath = MakePath(DIR_ICON, "Icon_GuildGeneral.dds");
					pTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG);
					if (pTexture != NULL)
						pTexture->Render(p2DRender, CPoint(PlusRect.left + 190, PlusRect.top + 10));
				}
				...
				case 2:
				{
					strPath = MakePath(DIR_ICON, "Icon_GuildOfficier.dds");
					pTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG);
					if (pTexture != NULL)
						pTexture->Render(p2DRender, CPoint(PlusRect.left + 190, PlusRect.top + 10));
				}
				...
				case 3:
				{
					strPath = MakePath(DIR_ICON, "Icon_GuildVeteran.dds");
					pTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG);
					if (pTexture != NULL)
						pTexture->Render(p2DRender, CPoint(PlusRect.left + 190, PlusRect.top + 10));
				}
				...
				case 4:
				{
					strPath = MakePath(DIR_ICON, "Icon_GuildRecrue.dds");
					pTexture = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG);
					if (pTexture != NULL)
						pTexture->Render(p2DRender, CPoint(PlusRect.left + 190, PlusRect.top + 10));
				}
...
#ifdef __MESSENGER_CTRL_EXTENTED
void CToolTip::SetGuildRankToolTip(DWORD dwToolTipId, int nPlayerId)
{
	m_nAdded = 3;
