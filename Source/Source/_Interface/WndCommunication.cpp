...
{
	//m_ptexMacroe.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_ICON, "icon_MacroChat.dds" ), COLOR_ITEM_BG );
	m_pTexMacro = m_textureMng.AddTexture( m_pApp->m_pd3dDevice, MakePath( DIR_ICON, "icon_MacroChat.dds" ), COLOR_ITEM_BG );//, bMyLoader );
#ifdef __AEGON_THEME_SWITCHER
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_texMacroChat, m_pApp->m_pd3dDevice, "wndButtMacroChat.tga", COLOR_ITEM_BG);
#else
	m_texMacroChat.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "wndButtMacroChat.tga" ), COLOR_ITEM_BG );
#endif
}
#ifdef __AEGON_THEME_SWITCHER
void CWndMacroChat::OnThemeSelectionChanged()
{
	CWndButton::OnThemeSelectionChanged();
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_texMacroChat, m_pApp->m_pd3dDevice, "wndButtMacroChat.tga", COLOR_ITEM_BG);
}
#endif
...
*/
//	m_texEdit.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "WndChat2.bmp" ), COLOR_ITEM_BG, TRUE );
/*
	rect = GetClientRect();
	rect.top = rect.bottom - 20;
...
#ifdef __AEGON_THEME_SWITCHER
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTGA(&m_wndChatEdit[0], "WndChatEdit00_1.tga");
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTGA(&m_wndChatEdit[1], "WndChatEdit01.tga");
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTGA(&m_wndChatEdit[2], "WndChatEdit02.tga");
...
{
#ifdef __ALPHA_CHAT
	const CRect rtChat = GetWindowRect();
	const CPoint ptMouse = GetMousePoint();
	m_bOnSurface = (rtChat.PtInRect(ptMouse) || m_nResizeDir > 0 || m_bPickup || m_wndEdit.m_bCaretVisible) ? true : false;

	if (g_Option.alphaChat)
	{
		const float fDelta = (static_cast<float>(g_tmCurrent - m_tmOld) / 1000.0f);
		m_tmOld = g_tmCurrent;
		...
		CWndButton* checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_CLOSE));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_RADIO1));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_RADIO2));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_RADIO3));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_RADIO4));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_RADIO5));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_CHECK1));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_CHECK3));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_BUTTON1));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_HELP));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_MOVELOCK));
		if (checkButton)
			checkButton->m_nAlphaCount = m_nAlphaCount;
		CWndStatic* checkStatic = dynamic_cast<CWndStatic*>(GetDlgItem(WIDC_STATIC1));
		if (checkStatic)
			checkStatic->m_nAlphaCount = m_nAlphaCount;
		m_wndText.m_wndScrollBar.setAlpha(m_nAlphaCount);
	}
	else
	{
		if (m_nAlphaCount > m_nAlpha)
			m_nAlphaCount = m_nAlpha;
		else if (m_nAlphaCount < 255)
		{
			m_nAlphaCount = m_nAlpha;
			CWndButton* checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_CLOSE));
			if (checkButton)
				checkButton->m_nAlphaCount = 255;
			checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_RADIO1));
			if (checkButton)
				checkButton->m_nAlphaCount = 255;
			checkButton = dynamic_cast<CWndButton*>(GetDlgItem(WIDC_RADIO2));
			...
			if (checkStatic)
				checkStatic->m_nAlphaCount = m_nAlphaCount;
			m_wndText.m_wndScrollBar.setAlpha(255);
		}
	}
#endif //__ALPHA_CHAT

#ifndef __FIX_NO_CHAT_CLEAR
	...
	if( 0 < strlen( g_Neuz.m_szWhisperName ) )
	{
		...
		{
			CString strWhisper;
			strWhisper.Format( "/say %s ", g_Neuz.m_szWhisperName );
			...
			if(lOffset != -1)
			{
				if( m_wndText.m_string.m_adwPlayerId.GetSize() > lOffset )
				{
					...
					if( dwPlayerId != 0 )
					{
						...
						bLink = true;
#ifdef __NEWCUR
						g_Neuz.SetMouseCursor(CUR_SELECT);
#else //__NEWCUR
...
void CWndChat::OnDestroyChildWnd( CWndBase* pWndChild )
{
	...
	else if( m_pWndChatFilter == pWndChild )
	{
		SAFE_DELETE( m_pWndChatFilter );
		pWndChild = NULL;
		...
		case APP_COMMAND:
			{
				if( message == WNM_DBLCLK )
				{
					...
					m_wndEdit.SetString( "/" );
#ifdef __AEGON_THEME_SWITCHER
					m_wndEdit.AddString(string, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
				m_wndEdit.m_string.SetColor(CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#endif // __AEGON_THEME_SWITCHER
...
BOOL CWndChat::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	if (m_pWndCommand)
		return m_pWndCommand->OnMouseWheel(nFlags, zDelta, pt);
	else
		return m_wndText.OnMouseWheel(nFlags, zDelta, pt);
...
{	
	...
	if (g_Option.alphaChat)
	{
		CRect rtChat = GetWindowRect();
		CPoint ptMouse = GetMousePoint();
		BOOL bOnSurface = rtChat.PtInRect(ptMouse);
