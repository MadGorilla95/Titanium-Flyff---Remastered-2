...
bool CIMEMgr::GetReadingWindowOrientation()
{
    ...
    if( !bHorizontalReading && ( m_dwId[0] & 0x0000FFFF ) == LANG_CHT )
    {
        ...
        HKEY hKey;
        DWORD dwVer = m_dwId[0] & COLOR_ERROR;
...
{ 
    ...
    for ( int y = 0 ; y < 9; y++ ) 
	{
		...
		p2DRender->TextOut(2, 2 + y * nMaxHeight, buf, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
...
#ifdef __AEGON_THEME_SWITCHER
		p2DRender->TextOut(2 + m_maxWidth * i, 2, text, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
...
void CReadingList::OnDraw( C2DRender* p2DRender )
{
...
#ifdef __AEGON_THEME_SWITCHER
			p2DRender->TextOut(x + (16 - size.cx) / 2, y + (16 - size.cy) / 2, text, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		p2DRender->TextOut( 2, 1, &*m_reading.begin(), CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
...
void CWndEdit::OnSetFocus(CWndBase* pOldWnd)
{
	...
	g_bKeyTable[g_Neuz.Key.chBack] = FALSE;
	g_bKeyTable[g_Neuz.Key.chRight] = FALSE;
...
#ifdef __NEWCUR
		g_Neuz.SetMouseCursor(CUR_EDIT);
#else //__NEWCUR
		...
		if( m_timerCaret.GetLeftTime() < 500 )
		{
			...
			else
			{
				...
				if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
				{
					...
					{
						dwColor = it->second.dwTextColor;
					}
				}
				p2DRender->RenderFillRect(CRect(pt1.x, pt1.y, pt1.x + 2, pt1.y + nFontBottom), dwColor);
#else // __AEGON_THEME_SWITCHER
				...
				case VK_RETURN:
					{
						...
						{
#ifdef __AEGON_THEME_SWITCHER
							m_string.Insert(m_dwOffset, '\n', CAegonThemeSwitcherMng::GetInstance()->GetTextColor(), 0, g_imeMgr.m_codePage);
#else // __AEGON_THEME_SWITCHER
...
*/
	...
	case WM_CHAR:
#ifdef __AEGON_FIXES
		if (wparam != VK_RETURN && static_cast<DWORD>(strlen(m_string)) < m_dwMaxStringNumber) 
		{
			OnChar_(wparam);
		}
...
void CWndEdit::OnChar_(UINT nChar)
{
	...
	if (static_cast<DWORD>(strlen(m_string)) > m_dwMaxStringNumber)
	{
		m_string = m_string.Left(m_dwMaxStringNumber);
...
#ifdef __AEGON_FIXES
void CWndEdit::OldOnChar_(UINT nChar)
#else
void CWndEdit::OnChar_(UINT nChar)
#endif
{
...
#ifdef __AEGON_THEME_SWITCHER
			m_string.Insert(m_dwOffset, buf, CAegonThemeSwitcherMng::GetInstance()->GetTextColor(), 0, g_imeMgr.m_codePage);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
		m_string.Insert(m_dwOffset, nChar, CAegonThemeSwitcherMng::GetInstance()->GetTextColor(), 0, g_imeMgr.m_codePage);
#else // __AEGON_THEME_SWITCHER
...
BOOL CWndEdit::GetResultString()
{
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		m_string.Insert(i + m_dwOffset, str[i], CAegonThemeSwitcherMng::GetInstance()->GetTextColor(), 0, g_imeMgr.m_codePage);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
void CWndEdit::OnThemeSelectionChanged()
{
	CWndText::OnThemeSelectionChanged();
}
#endif // __AEGON_THEME_SWITCHER