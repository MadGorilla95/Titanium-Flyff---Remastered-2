...
#ifdef __AEGON_MINIMIZE_WINDOWS
	CRect m_rectBeforeMinimized;
	CPoint m_ptLastMinimized;
	int m_nMinimizedMode;
	CString m_strTextureBeforeMinimized;
	BOOL m_bTileBeforeMinimized;
#endif
...
#ifdef __AEGON_APNG
static CAnimatedTextureMng    m_animatedTextureMng;
#endif // __AEGON_APNG

#ifdef __AEGON_THEME_SWITCHER
int m_nPickUpSpaceHeight;
int m_nTitleDrawOffsetY;
...
static void FreeTileTexture();
...
#ifdef __AEGON_THEME_SWITCHER
	BOOL SetTexture(LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR lpszFileName, BOOL bMyLoader = FALSE, BOOL bLogErrors = TRUE);
	...
	BOOL IsWndRoot() { return m_pWndRoot && this == m_pWndRoot; }
	...
	{ 
		...
		int nWndID = GetWndId();
		if (pWnd == NULL)
			Error("GetDlgItem : nID=%d not Found. Applet: %d", nID, nWndID);
#else //__GAMEGUARD
		if( pWnd == NULL )
...
public:
	...
	void SetThemeVariables();
	void AdjustSubWindowsForTheme();
#endif // __AEGON_THEME_SWITCHER
