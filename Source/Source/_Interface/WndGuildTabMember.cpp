...
#if __VER >= 11 // __CSC_VER11_4
#ifdef _WIN64
	pWndWorld->m_texPlayerDataIcon.Render(m_pApp->m_pd3dDevice, pVertex, ((INT_PTR)pVertices - (INT_PTR)pVertex) / sizeof(TEXTUREVERTEX2));
#else
	...
	for( int i=0; i<nMax; i++ )
	{
		...
		if( PtInRect( &rect, point ) )
		{
			...
			str.Format("%s\n%s %u\n%s %d\n%s %d\n%s", m_aList[nSelect].szName,
				...
				CAegonLastOnlineMng::GetInstance()->GetLastOnlineString(m_aList[nSelect].idPlayer).GetString()
			);
