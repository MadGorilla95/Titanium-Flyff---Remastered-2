...
void CWndMessageBox::OnInitialUpdate()
{
	...
	BOOL bTileBox = FALSE;
	int nSizeOffsetY = 0;// 24;

	if (GetWndId() == APP_MESSAGEBOX_TILES)
	{
		
		rect.bottom -= nSizeOffsetY;
		y -= nSizeOffsetY;
	...
	CSize size = CSize(72,20);//m_pSprPack->GetAt(9)->GetSize();
...
#ifdef __AEGON_LOGOUT_CHAR_SELECT
	CSize bigSize = CSize(146, 20);
	CRect rectTileBox_3(rect2_1.left + 3, rect2_1.bottom + 5, rect2_2.right - 3, rect2_1.bottom + 25);

	int nSizeAllButtonsX = size.cx * 2 + bigSize.cx;

	CRect bigRect3_1(x - (nSizeAllButtonsX / 2) - 10, y, x - (nSizeAllButtonsX / 2) - 10 + size.cx, y + size.cy);
	CRect bigRect3_3(bigRect3_1.right + 5, y, bigRect3_1.right + 5 + bigSize.cx, y + bigSize.cy);
	CRect bigRect3_2(bigRect3_3.right + 5, y, bigRect3_3.right + 5 + size.cx, y + size.cy);
	

	DWORD dwTextStyle = 0;
	if (!bTileBox)
	{
		dwTextStyle |= WBS_NODRAWFRAME;
	}
	m_wndText.Create(dwTextStyle, rect, this, 0);
#else
	...
	switch(m_nType)
	{
		...
		case MB_OK:
			...
			CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndButton1, m_pApp->m_pd3dDevice, "Button72x20.tga");
#else // __AEGON_THEME_SWITCHER
			m_wndButton1.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "Button72x20.tga" ) );
...
#endif // __AEGON_THEME_SWITCHER
#ifdef __AEGON_THEME_SWITCHER
			CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndButton2, m_pApp->m_pd3dDevice, "Button72x20.tga");
#else // __AEGON_THEME_SWITCHER
			m_wndButton2.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "Button72x20.tga" ) );
#endif // __AEGON_THEME_SWITCHER
			m_wndButton1.FitTextureSize();
		...
		case MB_YESNO:
			...
			break;
		case MB_YESNOCANCEL:
			m_wndButton1.Create("Yes"    ,0, rect3_1,this,IDYES);//   ,m_pSprPack,9);
			m_wndButton2.Create("No",0, rect3_2,this,IDNO);//    ,m_pSprPack,9);
			m_wndButton3.Create("Cancel",0, rect3_3,this,IDCANCEL);//,m_pSprPack,9);
			break;
#ifdef __AEGON_LOGOUT_CHAR_SELECT
		case 0x00000007L:
			m_wndButton1.Create("OK", 0, bigRect3_1, this, IDOK);
			m_wndButton2.Create("Cancel", 0, bigRect3_2, this, IDCANCEL);
			m_wndButton3.Create("Character select", 0, bigRect3_3, this, IDYES);
			CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndButton1, m_pApp->m_pd3dDevice, "Button72x20.tga");
			CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndButton2, m_pApp->m_pd3dDevice, "Button72x20.tga");
			CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)&m_wndButton3, m_pApp->m_pd3dDevice, "Button146x20.tga");
			m_wndButton1.FitTextureSize();
			m_wndButton2.FitTextureSize();
			m_wndButton3.FitTextureSize();
			break;
#endif
	}
...
#ifdef __AEGON_THEME_SWITCHER
	m_wndText.SetString( lpszMessage, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else //__AEGON_THEME_SWITCHER
