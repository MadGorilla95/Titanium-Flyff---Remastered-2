...
void CWndWebBox::OnDraw( C2DRender* p2DRender )
{
	...
	GetWindowPlacement(g_Neuz.GetSafeHwnd(), &wp);
	...
	if (g_Neuz.IsWindow())
	{
		...
		if (wp.showCmd == SW_MAXIMIZE)
		{
			...
			wp.rcNormalPosition = info.rcMonitor;
	...
	RECT windowBounds;
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
	x = wp.rcNormalPosition.left + wndRect.left + pCustom->rect.left - FrmW + xOffset;
	y = wp.rcNormalPosition.top + wndRect.top + pCustom->rect.top + FrmH + CapH + yOffset;
#else // __AEGON_FULLSCREEN_AND_RESIZE
	x = wp.rcNormalPosition.left + wndRect.left + pCustom->rect.left - FrmW;
	y = wp.rcNormalPosition.top + wndRect.top + pCustom->rect.top + FrmH + CapH;
#endif // __AEGON_FULLSCREEN_AND_RESIZE

#ifdef __AEGON_FULLSCREEN_AND_RESIZE
	int cx = WEB_DEFAULT_X;
	int cy = WEB_DEFAULT_Y;


	if (wndRect.Width() > g_Option.m_nResWidth - FrmW - 5)
	{
		cx = g_Option.m_nResWidth - pCustom->rect.left - FrmW;
		
	}
	if (wndRect.Height() > g_Option.m_nResHeight - CapH - FrmH - 5)
	{
		cy = g_Option.m_nResHeight - pCustom->rect.top - FrmH - CapH - 15;
	}
	

	CWebBox::GetInstance()->SetPositionAndSize(x, y, cx, cy);	
#else // __AEGON_FULLSCREEN_AND_RESIZE
	CWebBox::GetInstance()->SetPosition(x, y);
...
//		dwWinH	= clientH + ((FrmH-1) * 2) + MenuH + CapH; 
	...
	CRect tmpRect = pCustom->rect;
	tmpRect.left -= FrmW;
	tmpRect.top  += CapH+FrmH;
	
	//CWebBox::GetInstance()->Set_Rect(pCustom->rect);
	CWebBox::GetInstance()->Set_Rect(tmpRect);
