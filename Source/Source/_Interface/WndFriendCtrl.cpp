...
#ifdef _WIN64
	pWndWorld->m_texMsgIcon.Render(m_pApp->m_pd3dDevice, pVertex, (int)(((__int64)pVertices - (__int64)pVertex) / sizeof(TEXTUREVERTEX2)));
#else
...
void CWndFriendCtrl::OnLButtonDblClk( UINT nFlags, CPoint point )
{
	...
	if (nSelect != -1)
	{
		g_WndMng.OpenMessage(CPlayerDataCenter::GetInstance()->GetPlayerString(idFriend));
	}
#else
	if( nSelect != -1 )
	{
