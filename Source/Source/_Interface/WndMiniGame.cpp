...
{ 
	...
	pText->AddString(prj.GetText( TID_GAME_KAWIBAWIBO_GUID2 ), COLOR_ERROR);
} 
...
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
void CWndDiceGame::OnDestroyChildWnd(CWndBase* pWndChild)
{
	if (m_pWndDiceTender == pWndChild)
	{
		SAFE_DELETE(m_pWndDiceTender);
		pWndChild = NULL;
	}
	else if (m_pWndMessageBox == pWndChild)
	{
		SAFE_DELETE(m_pWndMessageBox);
		pWndChild = NULL;
	}
