...
#if defined(__AEGON_MASTER_KEEPS_SKILLS) && defined(__WORLDSERVER)
void CUserTaskBar::UpdateSkillShortcuts(CUser* pUser)
{
    for (int i = 0; i < MAX_SLOT_APPLET; i++)
    {
        if (m_aSlotApplet[i].m_dwShortcut == SHORTCUT_SKILL) 
        {
            LPSKILL lpSkill = pUser->GetSkill(m_aSlotApplet[i].m_dwType, m_aSlotApplet[i].m_dwId);

            if (!lpSkill || !pUser->CheckSkill(lpSkill->dwSkill))
    ...
    for (int i = 0; i < MAX_SLOT_ITEM_COUNT; i++)
    {
        ...
        {
            if (m_aSlotItem[i][j].m_dwShortcut == SHORTCUT_SKILL)
            {
                LPSKILL lpSkill = pUser->GetSkill(m_aSlotItem[i][j].m_dwType, m_aSlotItem[i][j].m_dwId);
                if (!lpSkill || !pUser->CheckSkill(lpSkill->dwSkill))
    ...
    for (int i = 0; i < MAX_ACTION_SLOT; i++) {
        ...
        {
            if (m_aSlotQueue[i][j].m_dwShortcut == SHORTCUT_SKILL)
            {
                LPSKILL lpSkill = pUser->GetSkill(m_aSlotQueue[i][j].m_dwType, m_aSlotQueue[i][j].m_dwId);
                if (!lpSkill || !pUser->CheckSkill(lpSkill->dwSkill))
                    memset(&m_aSlotQueue[i][j], 0, sizeof(SHORTCUT));
            }
        }
    }
    ...
    for (int i = 0; i < MAX_SLOT_QUEUE; i++)
    {
        ...
        {
            LPSKILL lpSkill = pUser->GetSkill(m_aSlotQueue[i].m_dwType, m_aSlotQueue[i].m_dwId);
            if (!lpSkill || !pUser->CheckSkill(lpSkill->dwSkill))
...
void CUserTaskBar::InitTaskBarShorcutKind( DWORD dwShortcutKind )
{
    ...
    for (int i = 0; i < MAX_ACTION_SLOT; i++) {
        for (int j = 0; j < MAX_SLOT_QUEUE; j++)
        {
            if (m_aSlotQueue[i][j].m_dwShortcut == dwShortcutKind)
            {
                memset(&m_aSlotQueue[i][j], 0, sizeof(SHORTCUT));
            }
        }
    ...
    if (ar.IsStoring())
    {
        ...
        *(UNALIGNED int*)(lpBuf + uOffset) = nCount;
        ...
        for (int i = 0; i < MAX_ACTION_SLOT; i++)
        {
            for (int j = 0; j < MAX_SLOT_QUEUE; j++)
            {
                if (m_aSlotQueue[i][j].m_dwShortcut != SHORTCUT_NONE)
                {
                    ar << i << j;
...
#ifdef __ACTION_SLOT_PAGES
	LPSHORTCUT pShortcut = &m_aSlotQueue[pUser->GetCurrentActionSlot()][m_nUsedSkillQueue];
#else // __ACTION_SLOT_PAGES
