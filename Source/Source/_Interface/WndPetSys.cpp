...
void CWndPetAwakCancel::SetDescription()
{
	...
	if(m_pText != NULL && checkflag)
	{
		...
		CString strText = szChar;
		CAegonThemeSwitcherMng::GetInstance()->ReplaceColorInString(&strText, 0xff0000ff, dwColorHighlightBlue);
		m_pText->m_string.AddParsingString(strText.GetString(), dwColorText);
#else
	...
	if( g_pPlayer->HasActivatedSystemPet() )
	{
...
#ifdef __AEGON_THEME_SWITCHER
				p2DRender->TextOut( 10, y + m_nTitleDrawOffsetY, m_strTitle, m_dwColor );
#else
		...
		else
		{
			...
			if( IsWndStyle( WBS_CAPTION ) )
			{
				...
				{
#ifdef __AEGON_THEME_SWITCHER
					m_pTheme->RenderWndBaseTitleBar(p2DRender, &rect, m_strTitle, m_dwColor, m_nTitleDrawOffsetY);
#else
...
void CWndPetStatus::DrawPetInformation(C2DRender* p2DRender)
{
	...
	if(m_pPetElem->m_pPet)
	{
		...
		p2DRender->TextOut( 112, 8, strTemp, dwColor);
...
#ifdef __AEGON_FIXES
		p2DRender->TextOut(112, 22, strTemp, dwColor);
#else
		...
		p2DRender->TextOut( 100, 36, strTemp, dwColor);
		if(m_bExtension)
		{
			...
			for(int i=PL_D; i<=nLevel; i++)
			{
				...
				if(bLevel != 0)
				{
					if(m_aPetLv[i] != bLevel)
					{
						...
						m_pTexPetLv[i] = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG);
...
void CWndPetStatus::OnDraw(C2DRender* p2DRender)
{
	...
	if( m_nDisplay != 0 )
	{
		...
		if(m_nDisplay == 1)
		{
			...
			p2DRender->TextOut( lpHP->rect.right - 75, lpHP->rect.top - 1, cbufHp, dwColor, 0xff000000 );

			nCharHP = wsprintf(cbufHp, "%dp", m_pPetElem->m_pPet->GetEnergy());

			int x = lpHP->rect.right-12;
			p2DRender->TextOut( x - (int)(nCharHP*5.8f) , lpHP->rect.top - 1, cbufHp, dwColor, 0xff000000 );
		}
		else
		{
			...
			p2DRender->TextOut( x - (int)(nCharHP*2.6f), lpHP->rect.top - 1, cbufHp, dwColor, 0xff000000 );
			
			nCharHP = wsprintf(cbufHp, "%d", m_pPetElem->m_pPet->GetMaxEnergy());
			
			x = lpHP->rect.right - 38 + 15;
			p2DRender->TextOut( x - nCharHP*2, lpHP->rect.top - 1, cbufHp, dwColor, 0xff000000 );
			
			p2DRender->TextOut( lpHP->rect.right - 42, lpHP->rect.top - 1, "/", dwColor, 0xff000000 );
		}
		...
		int x = lpHP->rect.right-15; // -40
		p2DRender->TextOut( x - (int)(nCharEXP*5.8f), lpExp->rect.top - 1, cbufExp, dwColor, 0xff000000 );
	}
...
void CWndPetStatus::OnInitialUpdate()
{
...
#ifdef __AEGON_THEME_SWITCHER
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_texGauEmptyNormal, m_pApp->m_pd3dDevice, "GauEmptyNormal.bmp", COLOR_ITEM_BG, TRUE);
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchLoadTexture(&m_texGauFillNormal, m_pApp->m_pd3dDevice, "GauEmptyNormal.bmp", COLOR_ITEM_BG, TRUE);

	m_pTexPetLvBg = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(g_Neuz.m_pd3dDevice, "PetLevelBg.tga", COLOR_ITEM_BG, TRUE);
	m_pTexPetStatusBg = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(g_Neuz.m_pd3dDevice, "PetStatusBg.tga", COLOR_ITEM_BG, TRUE);
	...
	m_pTexPetLvBg = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "PetLevelBg.tga"), COLOR_ITEM_BG );
	m_pTexPetStatusBg = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "PetStatusBg.tga"), COLOR_ITEM_BG, TRUE );
...
void CWndPetStatus::LockShowLevel(BOOL lFlag, int nLevel, int nPos)
{
	...
	else
	{
		...
		{
			strPath = m_strPathLvImage[bLevel-1];
			m_pTexPetLv[nLevel] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG );
		}
	}
}

#ifdef __AEGON_THEME_SWITCHER
void CWndPetStatus::OnThemeSelectionChanged() 
{
...
void CWndPetMiracle::PreLevelImgProcess()
{
	if(m_nStatus[0] == 0)
	{
		if(m_nCount[0]%4 == 0)
		{
			...
			{
#ifdef __fmod
				SoundSystemClass::playSound("InfOpen.wav");
#else
				PLAYSND( "InfOpen.wav" );
#endif
				int randnum = rand() % 9;
...
{
	if (m_pWndFoodConfrim == pWndChild)
	{
		SAFE_DELETE(m_pWndFoodConfrim);
		pWndChild = NULL;
	}
...
#ifdef __AEGON_THEME_SWITCHER
			m_p2DRender->TextOut(point.x + 36 - size.cx, point.y + 50 - size.cy, szTemp, dwColorHighlightBlue);
#else
...
#ifdef __AEGON_THEME_SWITCHER
	m_wndText.SetString(m_strText, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
#else // __AEGON_THEME_SWITCHER
...
{ 
	...
	if(pItemProp)
		m_pEggTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pItemProp->szIcon), COLOR_ITEM_BG );
...
#ifdef __AEGON_THEME_SWITCHER
	CString strText = scanner.m_pProg;
	CAegonThemeSwitcherMng::GetInstance()->ReplaceColorInString(&strText, 0xff0000ff, CAegonThemeSwitcherMng::GetInstance()->GetHighlightColor(HIGHLIGHT_COLOR_BLUE));
	CAegonThemeSwitcherMng::GetInstance()->ReplaceColorInString(&strText, 0xff000000, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());

	szChar = (CHAR*)strText.GetString();
#else
	szChar = scanner.m_pProg;
#endif
...
//////////////////////////////////////////////////////////////////////////
//	CWndPetMistakeCustom Class
//////////////////////////////////////////////////////////////////////////
CWndPetMistakeCustom::CWndPetMistakeCustom()
{
	m_nCount = 0;
	m_nDelay = 1;
	m_nStatus = -1;
...
void CWndPetMistakeCustom::OnDestroy()
{

	CWndPetStatus* pWndStatus = (CWndPetStatus*)g_WndMng.GetWndBase(APP_PET_STATUS);
	if (pWndStatus != NULL && m_bLocked)
	{
		pWndStatus->LockShowLevel(FALSE, m_nPetLevel, 1);
		m_bLocked = FALSE;
	}
...
void CWndPetMistakeCustom::OnDraw(C2DRender* p2DRender)
{
	...
	//레벨 텍스트 그리기.
	strEdit.SetString(m_strPetLevel, D3DCOLOR_XRGB(255, 255, 255), ESSTY_BOLD);
	p2DRender->TextOut_EditString(GetWndCtrl(WIDC_STATIC2)->rect.left + 8, GetWndCtrl(WIDC_STATIC2)->rect.top + 3, strEdit);
	...
	if (m_bReciveResult)
		...
		strPath = m_strPathLvImage[m_nMiracleLv - 1];
...
void CWndPetMistakeCustom::OnInitialUpdate()
{
	CWndNeuz::OnInitialUpdate();
	...
	if (g_pPlayer->HasActivatedSystemPet())
	{
		CItemElem* m_pPetElem = g_pPlayer->GetPetItem();
		m_nPetLevel = m_pPetElem->m_pPet->GetLevel();
		if (m_pPetElem->m_pPet != NULL && (m_nPetLevel == PL_C || m_nPetLevel == PL_B || m_nPetLevel == PL_A || m_nPetLevel == PL_S))
		{
			switch (m_nPetLevel)
			{
			case PL_C:
				m_strPetLevel = prj.GetText(TID_GAME_PET_GRADE_C); //"Lv. C"
				break;
			case PL_B:
				m_strPetLevel = prj.GetText(TID_GAME_PET_GRADE_B); //"Lv. B"
				break;
			case PL_A:
				m_strPetLevel = prj.GetText(TID_GAME_PET_GRADE_A); //"Lv. A"
				break;
			case PL_S:
				m_strPetLevel = prj.GetText(TID_GAME_PET_GRADE_S); //"Lv. S"
				break;
			}

			m_nMiracleLv = m_pPetElem->m_pPet->GetAvailLevel(m_nPetLevel);
			m_nCurLvCount = m_nMiracleLv - 1;
...
BOOL CWndPetMistakeCustom::Initialize(CWndBase* pWndParent, DWORD /*dwWndId*/)
{
	// Daisy에서 설정한 리소스로 윈도를 연다.
	return CWndNeuz::InitDialog(g_Neuz.GetSafeHwnd(), APP_PET_MISTAKE, 0, CPoint(0, 0), pWndParent);
}

BOOL CWndPetMistakeCustom::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	CWndButton* pButton1 = (CWndButton*)GetDlgItem(WIDC_BUTTON2);
	CWndButton* pButton2 = (CWndButton*)GetDlgItem(WIDC_OK);
	CWndButton* pCheckbox1 = (CWndButton*)GetDlgItem(WIDC_CHECK1);

	if (pCheckbox1->GetCheck()) {
	...
	if (message == WNM_CLICKED)
	{
		if (nID == WIDC_BUTTON2)
		{
			...
			//Send to Server
			if (m_dwObjId != -1 && !pCheckbox1->GetCheck()) {
				g_DPlay.SendPetTamerMistakeCustom(m_dwObjId);
				pCheckbox1->EnableWindow(FALSE);
...
void CWndPetMistakeCustom::ReceiveResult(int nCurLevel)
{
	m_nResCurLevel = nCurLevel;
	m_nMiracleLv = m_nResCurLevel;

	m_nStatus = 0;
	m_bReciveResult = TRUE;
}

void CWndPetMistakeCustom::SetItem(DWORD dwObjId)
{
	m_dwObjId = dwObjId;
}

BOOL CWndPetMistakeCustom::Process()
{
...
void CWndPetMistakeCustom::CurLevelImgProcess()
{
	if (m_nStatus == 0)
	{
		if (m_nCount % 4 == 0)
...
void CWndPetMistakeCustom::SetDescription(CHAR* szChar)
{
	CScript scanner;
	BOOL checkflag;
	checkflag = scanner.Load(MakePath(DIR_CLIENT, _T("PetMistake.inc")));
	szChar = scanner.m_pProg;
...
#ifdef __AEGON_THEME_SWITCHER
	m_pTexPetStatusBg = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(g_Neuz.m_pd3dDevice, "BuffpetStatusBg .tga", COLOR_ITEM_BG, TRUE);
#else // __AEGON_THEME_SWITCHER
	m_pTexPetStatusBg = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "BuffpetStatusBg .tga"), COLOR_ITEM_BG, TRUE );
#endif // __AEGON_THEME_SWITCHER
}
...
void CWndBuffPetStatus::DrawSlotItems( C2DRender* p2DRender )
{
	...
	for( int i=0; i<MAX_VIS; ++i )
	{
		if( i < nAvailableSlot )
		{
			...
			m_pTexture[ i ] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pProp->szIcon), COLOR_ITEM_BG );
		...
		else
		{
			//사용불가능 슬롯 
			wndCtrl = GetWndCtrl( m_nCtrlId[i] );
			CTexture* pTexClosed = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, "Icon_Lock.dds" ), COLOR_ITEM_BG );
			if( pTexClosed )
