...
#ifdef __AEGON_APNG
CAnimatedTextureMng CWndBase::m_animatedTextureMng;
#endif // __AEGON_APNG
...
{
	m_pTexForbid = CWndBase::m_textureMng.AddTexture( pd3dDevice, lpszFileName, COLOR_ITEM_BG );
...
BOOL CWndBase::SetTexture(LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR lpszFileName, BOOL bMyLoader, BOOL bLogErrors)
{
	m_pTexture = m_textureMng.AddTexture(pd3dDevice, lpszFileName, COLOR_ITEM_BG, bMyLoader, bLogErrors);
...
#ifdef __AEGON_THEME_SWITCHER
	m_nPickUpSpaceHeight = 20;
	m_nTitleDrawOffsetY = 0;
#endif // __AEGON_THEME_SWITCHER

#ifdef __AEGON_MINIMIZE_WINDOWS
	m_nMinimizedMode = __AEG_MINIMIZE_MODE_NONE;
	m_ptLastMinimized = CPoint(0, 0);
#endif
}
CWndBase::~CWndBase()
{
...
{
	{
		...
		if( m_pTexture )
		{
			...
			{
				if (GetWndId() == APP_AEGON_MESSENGER) 
				{
	...
	#ifdef __LEESE_CENTER_BASE_TEXT
					CPoint newpoint;
					CSize sizeTitle = p2DRender->m_pFont->GetTextExtent(m_strTitle);
					CRect rect = GetWindowRect();
					newpoint = (rect.Width() - sizeTitle.cx) / 2;
					p2DRender->TextOut(newpoint.x - 7, 3 + m_nTitleDrawOffsetY, m_strTitle, m_dwColor);
	#else //__LEESE_CENTER_BASE_TEXT
					p2DRender->TextOut( 10, 4, m_strTitle, m_dwColor );
	#endif //__LEESE_CENTER_BASE_TEXT
					p2DRender->SetFont( pOldFont );
				}				
...
//	if(!IsWndRoot())
	{
		for(int i = 0; i < m_wndArray.GetSize(); i++)
		{
			...
			if (m_nMinimizedMode != __AEG_MINIMIZE_MODE_NONE)
			{
				...
				CWndTitleBar* pWndTitleBar = dynamic_cast<CWndTitleBar*>(pWnd);
				if (!pWndTitleBar)
					continue;
			}
#endif
...
#ifdef __AEGON_APNG
		m_animatedTextureMng.DeleteDeviceObjects();
#endif // __AEGON_APNG
	}
	for(int i = 0; i < m_wndArray.GetSize(); i++) 
...
#ifdef _WIN64
			g_toolTip.PutToolTip(reinterpret_cast<ULONG_PTR>(this), m_strToolTip, m_rectCurrentClient, point, m_nToolTipPos);
#else
...
#ifdef __AEGON_MINIMIZE_WINDOWS
			BOOL bPickupMinimized = FALSE;
			CWndBase* pParent = NULL;
			
			if (CWndTitleBar* pWndTitlebar = dynamic_cast<CWndTitleBar*>(this))
			{
				pParent = GetParentWnd();

				if (pParent && pParent->m_nMinimizedMode == __AEG_MINIMIZE_MODE_TEX)
				{
					bPickupMinimized = TRUE;
				}
			}

			if (m_pWndFocus == this || bPickupMinimized)
#else
			if (m_pWndFocus == this)
#endif
			{
				...
				{
#ifdef __AEGON_MINIMIZE_WINDOWS
					if (((m_dwStyle & WBS_MOVE) || (pParent && bPickupMinimized && pParent->m_dwStyle & WBS_MOVE)) && IsPickupSpace(ptWindow))
					{
						if (bPickupMinimized && pParent)
						{
							pParent->DefWindowProc(message, wParam, lParam);
						}
						else 
						{
							m_bPickup = TRUE;
...
{ 
	...
	if( pWndFocus->m_pWndFocusChild && m_pParentWnd)
...
BOOL CWndBase::IsPickupSpace(CPoint ptWindow)
{
	...
	CRect rect = GetWindowRect();
#ifdef __AEGON_MINIMIZE_WINDOWS
	if (IsWndStyle(WBS_CAPTION) && m_nMinimizedMode != __AEG_MINIMIZE_MODE_TEX)
#else
	if( IsWndStyle( WBS_CAPTION ) )
#endif
		rect.bottom = m_nPickUpSpaceHeight;
...
{ 
	...
	if( m_bTile == FALSE )
	{
		...
		if( m_strWndTileMap.Lookup( m_strTexture, (void*&)lpImage ) == FALSE )
		{
			...
			{
				if (LoadImage(MakePath(_T(it->second.szThemeFolder), m_strTexture), lpImage))
				{
					bSuccess = TRUE;
				}	
			}
...
void CWndBase::AdjustWndBase( D3DFORMAT d3dFormat) //= D3DFMT_A4R4G4B4 )
{
...
#ifdef __AEGON_FIXES
	if (!pTexture->CreateTexture(m_pApp->m_pd3dDevice, size1.cx, size1.cy, 1, 0, d3dFormat, D3DPOOL_MANAGED)) 
	{
		Error("%s failed to create texture", __FUNCTION__);
		return;
	}
#else
	...
	if (pTexture->m_pTexture->LockRect(0, &lockedRect, 0, 0) == D3D_OK)// Lock
	{
		...
		if (m_nMinimizedMode != __AEG_MINIMIZE_MODE_NONE)
		{
			if (CWndNeuz* pWndNeuz = dynamic_cast<CWndNeuz*>(this))
			{
				if(m_nMinimizedMode != __AEG_MINIMIZE_MODE_TEX)
					pWndNeuz->m_wndTitleBar.AdditionalSkinTexture(pDest, size1, d3dFormat);
			}
		}
		else
		{
...
#ifdef __AEGON_THEME_SWITCHER
void CWndBase::OnThemeSelectionChanged()
{
	SetThemeVariables();
	AdjustWndBase();
}

void CWndBase::SetThemeVariables()
{
	DWORD dwWndID = GetWndId();

	m_nPickUpSpaceHeight = CAegonThemeSwitcherMng::GetInstance()->GetPickUpSpaceHeight(dwWndID);
	m_nTitleDrawOffsetY = CAegonThemeSwitcherMng::GetInstance()->GetTitleDrawOffset(dwWndID);
}

void CWndBase::AdjustSubWindowsForTheme() 
{
	CWndBase* pWndBase = this;

	for (int i = 0; i < pWndBase->m_wndArray.GetSize(); i++)
	{
		if (CWndBase* tmpCtrl = (CWndBase*)pWndBase->m_wndArray.GetAt(i))
		{
			tmpCtrl->OnThemeSelectionChanged();		
		}
	}
}
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_MINIMIZE_WINDOWS
void CWndBase::OnMinimizeToggle()
{
