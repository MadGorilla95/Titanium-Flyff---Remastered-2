				...
				if( lpShortcut->m_dwShortcut == SHORTCUT_ITEM )
				{
					...
					if( pItemElem  )
					{
						if( pItemElem->GetProp()->dwPackMax > 1 )
						{
							...
							p2DRender->TextOut(point.x + 31 - size.cx, point.y + 31 - size.cy, szTemp, 0xFFffb912);
...
void CWndTaskBar::Serialize( CAr & ar )
{
	...
	if( ar.IsStoring() )
	{
		...
		*(UNALIGNED int*)( lpBuf + uOffset )	= nCount;
		...
		for (int i = 0; i < MAX_ACTION_SLOT; i++)
		{
			...
			{
				if (m_aSlotQueue[i][j].m_dwShortcut == SHORTCUT_NONE)		// 값이 들어있는지 검사
				{
					ar << i << j;
					ar << m_aSlotQueue[i][j].m_dwShortcut << m_aSlotQueue[i][j].m_dwId << m_aSlotQueue[i][j].m_dwType;
					ar << m_aSlotQueue[i][j].m_dwIndex << m_aSlotQueue[i][j].m_dwUserId << m_aSlotQueue[i][j].m_dwData;
					nCount++;
				}
			}
		}
	...
	else
	{
		...
		for( int i = 0; i < nCount; i++ )
		{
			...
			else if (m_aSlotApplet[nIndex].m_dwShortcut == SHORTCUT_SWITCH_EQUIP)
			{
				if (g_pPlayer && m_aSlotApplet[nIndex].m_dwId >= g_pPlayer->m_vEquipSwitchInfo.size())
				{
					m_aSlotApplet[nIndex].Empty();
				}
			}
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (nIndex == m_nActionSlotIndex)
				m_nCurQueueNum++;

			ar >> m_aSlotQueue[nIndex][nIndex2].m_dwShortcut >> m_aSlotQueue[nIndex][nIndex2].m_dwId >> m_aSlotQueue[nIndex][nIndex2].m_dwType;
			ar >> m_aSlotQueue[nIndex][nIndex2].m_dwIndex >> m_aSlotQueue[nIndex][nIndex2].m_dwUserId >> m_aSlotQueue[nIndex][nIndex2].m_dwData;

			m_aSlotQueue[nIndex][nIndex2].m_dwIndex = nIndex;

			LPSKILL lpSkill = g_pPlayer->GetSkill(m_aSlotQueue[nIndex][nIndex2].m_dwType, m_aSlotQueue[nIndex][nIndex2].m_dwId);
			m_aSlotQueue[nIndex][nIndex2].m_pTexture = m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, lpSkill->GetProp()->szIcon/*pItemBase->GetProp()->szIcon*/), 0xffff00ff, FALSE);
		}
#else // __ACTION_SLOT_PAGES
		for( int i = 0; i < nCount; i++ )
		{
		...
		ar >> m_nActionPoint;
#ifndef __ACTION_SLOT_PAGES
		m_nCurQueueNum = nCount;
#endif // __ACTION_SLOT_PAGES
	}
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
	SetItemSlot(selectedSlot);
#endif // __AEGON_FULLSCREEN_AND_RESIZE
}
...
void CWndTaskBar::SetTaskBarTexture( LPSHORTCUT pShortcut )
{
	if( pShortcut->m_dwShortcut == SHORTCUT_APPLET )
	{
		...
		if( pAppletFunc )
		{
			pShortcut->m_pTexture = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice,  MakePath( DIR_ICON, pAppletFunc->m_pszIconName ), COLOR_ITEM_BG );
		} else
		{
		}
	}
	...
	else if ( pShortcut->m_dwShortcut == SHORTCUT_SKILL)
	{
		if( pShortcut->m_dwType == 2 )
		{
			...
			pShortcut->m_pTexture = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, pProp->szIcon/*pItemBase->GetProp()->szIcon*/), COLOR_ITEM_BG );
		}
		else
		{	
			...
			if( pSkillProp )
				pShortcut->m_pTexture = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, pSkillProp->szIcon ), COLOR_ITEM_BG );
		}
	}
#if __VER >= 12 // __LORD
	...
	{
		pShortcut->m_pTexture	= m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, "icon_MacroChat.dds" ), COLOR_ITEM_BG );
	}
#ifdef __AEGON_TELEPORT_SHORTCUT
	else if (pShortcut->m_dwShortcut == SHORTCUT_TELEPORT)
	{
		__AEGON_TELEPORT_UNIT* pUnit = CAegonTeleporterMng::GetInstance()->GetTeleUnitByID(pShortcut->m_dwId);
		if (pUnit) 
			pShortcut->m_pTexture = m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, pUnit->m_szNameShortcutTex), COLOR_ITEM_BG);
	}
#endif // __AEGON_TELEPORT_SHORTCUT
	else if( pShortcut->m_dwShortcut == SHORTCUT_EMOTICON )
	{
#ifdef __AEGON_EMOTE_SYSTEM
		//if (pShortcut->m_dwType == 1) 
		//{
		map<int, __AEGON_EMOTE_UNIT>* m_pEmoteMap = CAegonEmoticonMng::GetInstance()->GetEmoteMap(pShortcut->m_dwType);
		
		if (m_pEmoteMap && pShortcut->m_dwId >= 0 && pShortcut->m_dwId < m_pEmoteMap->size())
		{
			TCHAR buffer[MAX_SHORTCUT_STRING] = { 0 };
			CAegonEmoticonMng::GetInstance()->MakeEmoticonTooltipText(buffer, pShortcut->m_dwId, pShortcut->m_dwType);
			_tcscpy(pShortcut->m_szString, buffer);
			pShortcut->m_pTexture = CAegonEmoticonMng::GetInstance()->GetIconTexture(pShortcut->m_dwId, pShortcut->m_dwType);
		}
		//}
		//else
	...
	else if (pShortcut->m_dwShortcut == SHORTCUT_SWITCH_EQUIP)
	{
		if (pShortcut->m_dwId >= 0 && pShortcut->m_dwId < g_pPlayer->m_vEquipSwitchInfo.size())
		{
			...
			pShortcut->m_pTexture = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(g_Neuz.m_pd3dDevice, "icon_ability.dds", 0xffff00ff);
#else
			pShortcut->m_pTexture = m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, "icon_ability.dds"), 0xffff00ff);
#endif
		}
	}
#endif // __SWITCH_EQUIP
...
#ifdef __ACTION_SLOT_PAGES
	m_nActionSlotIndex = 0;
	m_paActionSlot = m_aSlotQueue[m_nActionSlotIndex];
#endif // __ACTION_SLOT_PAGES
	m_nActionPoint = 0;
	m_dwHighAlpha = 0;
//	m_nMaxSlotApplet = 5;
...
#ifdef __AEGON_THEME_SWITCHER
	m_pQueueIconOffsetPoint = CPoint(0, 0);

	m_pOffsetLoopIncrease = CPoint(0, 0);
	m_pOffsetLoopIncreaseQueue = CPoint(0, 0);
...
void CWndTaskBar::PutTooTip(LPSHORTCUT pShortcut, CPoint point, CRect* pRect, BOOL bSkipClientToScreen)
#else
...
#endif
{
	...
	if( pShortcut->m_dwShortcut == SHORTCUT_APPLET )
	{
		...
		//	{
...
#ifdef __AEGON_THEME_SWITCHER
				strEdit.AddString(pAppletFunc->m_pAppletDesc, dwTextColor);
			...
			unsigned char tmpChar = pAppletFunc->m_cHotkey;
			if (tmpChar >= VK_F11 && tmpChar <= VK_F24)
			{
				int nKeyNum = tmpChar - 121 + 10;
				string.Format("[%s F%d]", prj.GetText(TID_GAME_TOOLTIP_HOTKEY), nKeyNum);
			}
			else
#endif
				string.Format("[%s %c]", prj.GetText(TID_GAME_TOOLTIP_HOTKEY), pAppletFunc->m_cHotkey);
#ifdef __AEGON_THEME_SWITCHER
				strEdit.AddString(string, CAegonThemeSwitcherMng::GetInstance()->GetHighlightColor(HIGHLIGHT_COLOR_BLUE));
#else // __AEGON_THEME_SWITCHER
	...
	{
		CString tmpStr = prj.GetTeleportShortcutString(pShortcut->m_dwId);
		g_toolTip.PutToolTip(10000, tmpStr, *pRect, point, 0);
	}
#endif // __AEGON_TELEPORT_SHORTCUT
	else
...
//		if( pShortcut->m_dwType == 0 || pShortcut->m_dwType == 1 || pShortcut->m_dwType == 3 )
		{
			...
			g_WndMng.PutToolTip_Skill_Extended(lpSkill->dwSkill, lpSkill->dwLevel, point, pRect);
#else //__BETTER_SKILL_TOOLTIP
	...
	if( pShortcut->m_dwShortcut == SHORTCUT_MOTION )
	{
		...
		CEditString strEdit;
		strEdit.SetParsingString( string , CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
		g_toolTip.PutToolTip( pShortcut->m_dwId , strEdit, *pRect, point, 0 );
	}
	else
	...
	else if (pShortcut->m_dwShortcut == SHORTCUT_SWITCH_EQUIP)
	{
		...
		if (pShortcut->m_dwId >= 0 && pShortcut->m_dwId < g_pPlayer->m_vEquipSwitchInfo.size())
		{
			...
			strEdit.SetParsingString(pEquip->strTitle, CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
			g_toolTip.PutToolTip(pShortcut->m_dwId, strEdit, *pRect, point, 0);
		}
	}
#endif // __SWITCH_EQUIP
...
void CWndTaskBar::OnMouseWndSurface( CPoint point )
{
	...
	{
#ifdef __ACTION_SLOT_PAGES
		LPSHORTCUT lpShortcut = &m_paActionSlot[i];
#else // __ACTION_SLOT_PAGES
...
#ifdef __AEGON_THEME_SWITCHER
		rect += m_pOffsetLoopIncreaseQueue;
#endif // __AEGON_THEME_SWITCHER
	}
...
/*	
#ifdef __AEGON_THEME_SWITCHER
	CRect tmpRect = CRect(POINT_APPLET.x, POINT_APPLET.y, POINT_APPLET.x + ICON_SIZE, POINT_APPLET.y + ICON_SIZE);
	p2DRender->RenderRect(tmpRect, COLOR_ERROR);

	tmpRect = CRect(POINT_ITEM.x, POINT_ITEM.y, POINT_ITEM.x + ICON_SIZE, POINT_ITEM.y + ICON_SIZE);
	p2DRender->RenderRect(tmpRect, COLOR_SUCCESS);

	tmpRect = CRect(POINT_QUEUE.x, POINT_QUEUE.y, POINT_QUEUE.x + SKILL_SIZE, POINT_QUEUE.y + SKILL_SIZE);
	p2DRender->RenderRect(tmpRect, 0xFF0000FF);
#endif // __AEGON_THEME_SWITCHER
	*/
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		DWORD dwColor = CAegonThemeSwitcherMng::GetInstance()->GetHighlightColor(HIGHLIGHT_COLOR_BLUE);
		if (m_dwHighAlpha < 128)
			dwColor &= ~D3DCOLOR_ARGB(255 - m_dwHighAlpha, 0, 0, 0);
		else
			dwColor &= ~D3DCOLOR_ARGB(255 - (128 - (m_dwHighAlpha - 128)), 0, 0, 0);
		...
		if (it != CAegonThemeSwitcherMng::GetInstance()->m_mapThemes.end())
		{
			if (it->second.nThemeType != 0)
			{
				...
				int nWidth = rectApplet.Width();
				rectApplet.top = 42;
				rectApplet.bottom = 47;
				rectApplet.left = POINT_APPLET_X - 1;
				rectApplet.right = (POINT_APPLET_X + m_nMaxSlotApplet * ICON_SIZE) + (m_nMaxSlotApplet - 1) * m_pOffsetLoopIncrease.x;


				nWidth = rectItem.Width();
				rectItem.top = 42;
				rectItem.bottom = 47;
				rectItem.left = POINT_ITEM_X - 1;
				rectItem.right = (POINT_ITEM_X + MAX_SLOT_ITEM * ICON_SIZE) + (MAX_SLOT_ITEM - 1) * m_pOffsetLoopIncrease.x;



				rectSkill.top = 42;
				rectSkill.bottom = 47;
				rectSkill.left = POINT_QUEUE_X - 1;
				rectSkill.right = (POINT_QUEUE_X + MAX_SLOT_QUEUE * SKILL_SIZE) + (MAX_SLOT_QUEUE - 1) * m_pOffsetLoopIncreaseQueue.x;
			}
		}
		if (bDefaultTheme) 
		{
...
#ifdef __SWITCH_EQUIP
			case SHORTCUT_SWITCH_EQUIP:
#endif // __SWITCH_EQUIP
#ifdef __AEGON_TELEPORT_SHORTCUT
			case SHORTCUT_TELEPORT:
#endif // __AEGON_TELEPORT_SHORTCUT
	...
	for( int i = 0; i < m_nMaxSlotApplet; i++ )
	{
		/*
#ifdef __AEGON_THEME_SWITCHER
		CRect tmpRect = CRect(point.x, point.y, point.x + ICON_SIZE, point.y + ICON_SIZE);
		//p2DRender->RenderRect(tmpRect, COLOR_ERROR);
		p2DRender->TextOutA(tmpRect.left, tmpRect.top, "A", COLOR_ERROR);
#endif // __AEGON_THEME_SWITCHER
*/
		...
		if( !lpShortcut->IsEmpty() )
		{
			if (lpShortcut->m_pTexture) 
			{
				...
				if (lpShortcut->m_dwShortcut == SHORTCUT_SKILLFUN || lpShortcut->m_dwShortcut == SHORTCUT_SKILL)
				{
					tmpPoint.x = tmpPoint.x + (ICON_SIZE / 2) - (SKILL_SIZE / 2);
					tmpPoint.y = tmpPoint.y + (ICON_SIZE / 2) - (SKILL_SIZE / 2) - 2;
				}

				p2DRender->RenderTexture(tmpPoint, lpShortcut->m_pTexture);
#else
				p2DRender->RenderTexture(point, lpShortcut->m_pTexture);
#endif
			}
...
#ifdef __SWITCH_EQUIP
			if (lpShortcut->m_dwShortcut == SHORTCUT_SWITCH_EQUIP)
			{
				if (lpShortcut->m_dwId >= g_pPlayer->m_vEquipSwitchInfo.size())
				{
					lpShortcut->Empty();
...
#endif // __SWITCH_EQUIP
#ifdef __ACTION_SLOT_PAGES
			if (lpShortcut->m_dwShortcut == SHORTCUT_SKILLFUN)
			{
				CEditString strEdit;
				strEdit.Format("%d", lpShortcut->m_dwUserId + 1);
				CSize size = m_p2DRender->m_pFont->GetTextExtent(strEdit);
				p2DRender->TextOut(point.x + 32 - size.cx, point.y + 32 - size.cy, strEdit, 0xff0000ff);
				p2DRender->TextOut(point.x + 31 - size.cx, point.y + 31 - size.cy, strEdit, 0xffb0b0f0);
			}
			else
#endif // __ACTION_SLOT_PAGES
			if( lpShortcut->m_dwShortcut == SHORTCUT_ITEM )
			...
			if( lpShortcut->m_dwShortcut == SHORTCUT_APPLET )
			{
				...
				if( pAppletFunc && pAppletFunc->m_cHotkey )
				{
					...
					if (tmpChar >= VK_F11 && tmpChar <= VK_F24)
					{
						int nKeyNum = tmpChar - 121 + 10;
						CString string; 
						string.Format("F%d", nKeyNum);
						p2DRender->TextOut(ptHotkey.x - 4, ptHotkey.y - 0 - 4, string, 0xffffffff);
					}
					else
#endif
					{
...
/*
		...
		CRect tmpRect = CRect(point.x, point.y, point.x + ICON_SIZE, point.y + ICON_SIZE);
		//p2DRender->RenderRect(tmpRect, COLOR_ERROR);
		p2DRender->TextOutA(tmpRect.left, tmpRect.top, "B", COLOR_ERROR);
#endif // __AEGON_THEME_SWITCHER
*/
		...
		if( !lpShortcut->IsEmpty() )
		{
			...
			else if (lpShortcut->m_dwShortcut == SHORTCUT_SKILLFUN)
			{
				CEditString strEdit;
				CString str;
				str.Format("%d", lpShortcut->m_dwUserId + 1);
...
/*
	...
	for (int i = 0; i < 5; i++) 
	{
		...
		CRect tmpRect = CRect(tmpPoint.x, tmpPoint.y, tmpPoint.x + SKILL_SIZE, tmpPoint.y + SKILL_SIZE);
		//p2DRender->RenderRect(tmpRect, COLOR_ERROR);
		p2DRender->TextOutA(tmpRect.left, tmpRect.top, "C", COLOR_ERROR);
		tmpPoint += CPoint(SKILL_SIZE, 0);
		tmpPoint += m_pOffsetLoopIncreaseQueue;
	}
#endif // __AEGON_THEME_SWITCHER
*/
...
//					p2DRender->TextOut( point.x, point.y, g_pPlayer->m_nReUseDelay[ lpShortcut->m_dwId ] );
...
#ifdef __AEGON_THEME_SWITCHER
		point += m_pOffsetLoopIncreaseQueue;
#endif // __AEGON_THEME_SWITCHER
	}
	...
	#define LV5MAXAP	100
		...
		point.x += SKILL_SIZE;	// 스킬 2번째칸부터 쉐이드된다.
		{
...
#ifdef __AEGON_THEME_SWITCHER
			int tmpSize = SKILL_SIZE + m_pOffsetLoopIncreaseQueue.x;
			
			if (m_nActionPoint >= LV4MAXAP)	// LV4MAXAP를 넘어서면 5단계 시작.
				nAP2Size = (tmpSize * 3) + (((m_nActionPoint - LV4MAXAP) * tmpSize) / (LV5MAXAP - LV4MAXAP));
			else if (m_nActionPoint >= LV3MAXAP)
				nAP2Size = (tmpSize * 2) + (((m_nActionPoint - LV3MAXAP) * tmpSize) / (LV4MAXAP - LV3MAXAP));
			else if (m_nActionPoint >= LV2MAXAP)
				nAP2Size = (tmpSize * 1) + (((m_nActionPoint - LV2MAXAP) * tmpSize) / (LV3MAXAP - LV2MAXAP));
			else
				nAP2Size = (m_nActionPoint * tmpSize) / LV2MAXAP;
#else
			if( m_nActionPoint >= LV4MAXAP )	// LV4MAXAP를 넘어서면 5단계 시작.
			...
			point.x += nAP2Size;
#ifdef __AEGON_THEME_SWITCHER
			nAP2Size = (SKILL_SIZE * 4 + m_pOffsetLoopIncreaseQueue.x * 4) - nAP2Size;
#else
...
#ifdef __ACTION_SLOT_PAGES
	pWndButton = (CWndButton*)GetDlgItem(WIDC_UP2);
	rect = pWndButton->GetWindowRect(TRUE);
	rect.top = rect.bottom - 4;
	p2DRender->TextOut(rect.left, rect.top, m_nActionSlotIndex + 1, 0xffffffff);
#endif //__ACTION_SLOT_PAGES
...
void CWndTaskBar::OnInitialUpdate()
{
	...
	m_pTexture = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice,  MakePath( DIR_ICON, "icon_ActionSkill.dds" ), COLOR_ITEM_BG );

#ifdef __ACTION_SLOT_PAGES
	for (int i = 0; i < MAX_ACTION_SLOT; i++)
	{
		m_aSlotSkill[i].m_dwShortcut = SHORTCUT_SKILLFUN;
		m_aSlotSkill[i].m_pTexture = m_pTexture;
		m_aSlotSkill[i].m_dwUserId = i;
	}
#else // __ACTION_SLOT_PAGES
	...
	if (pThemeUnit)
	{
			m_nMaxSlotApplet += pThemeUnit->m_nMaxSlotAppletOffset;

			m_pOffsetLoopIncrease.x = pThemeUnit->m_nOffsetLoopIncrease;
			m_pOffsetLoopIncreaseQueue.x = pThemeUnit->m_nOffsetLoopIncreaseQueue;
			m_pQueueIconOffsetPoint.x = pThemeUnit->m_nQueueIconOffsetPoint;
	}

	/*CString tmpStr = "";
	tmpStr.Format("width: %d", m_rectClient.right - m_rectClient.left);
	...
	{
		int nDotPos = m_strTexture.ReverseFind('.');
		m_strTexture = m_strTexture.Left(nDotPos) + "_Extra" + m_strTexture.Right(m_strTexture.GetLength() - nDotPos);
	}
#endif
...
//	return CWndBase::Create( 0 | /*WBS_MODAL|*/ WBS_MANAGER | WBS_SOUND | WBS_NOFRAME, rect, &g_WndMng, APP_TASKBAR );
	BOOL bResult;
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
	int nWidth = g_WndMng.resizeWidth;

	if (!g_pD3dApp->IsWindow()) 
	{
		nWidth = g_Option.m_nResWidth;
	}
	else if (g_Option.m_bWindowMaximized)
	{
		CRect tmp;
...
#ifdef __SMALL_CENTERED_TASKBAR
	if (!g_Option.m_bCenterTaskbar)
	{
		...
		if (nWidth >= 2560)
		{
			...
			bResult = CWndNeuz::InitDialog(g_Neuz.GetSafeHwnd(), APP_TASKBAR2560W, WBS_MANAGER | WBS_SOUND, CPoint(0, 0), pWndParent);
		}
		else if (nWidth >= 1920)
		{
			m_nMaxSlotApplet = MAX_SLOT_APPLET;
	...
	if (!g_Option.m_bCenterTaskbar && nWidth >= 1600)
	{
			//already started
	}
	else
...
#ifdef __ACTION_SLOT_PAGES
int CWndTaskBar::GetCurrentActionSlot() const
{
	return m_nActionSlotIndex;
}

void CWndTaskBar::SetActionSlot(const int nSlot)
{
	if (nSlot >= 0 && nSlot < MAX_ACTION_SLOT) {
		m_paActionSlot = m_aSlotQueue[m_nActionSlotIndex = nSlot];
		m_nCurQueueNum = 0;
		for (int i = 0; i < MAX_SLOT_QUEUE; i++)
		{
			if (!m_paActionSlot[i].IsEmpty())
			{
				m_nCurQueueNum++;
			}
		}
...
BOOL CWndTaskBar::OnChildNotify(UINT message,UINT nID,LRESULT* pLResult)
{
	...
	switch( nID )
	{
		...
		case WIDC_UP2:
			m_nActionSlotIndex--;
			if (m_nActionSlotIndex < 0) m_nActionSlotIndex = MAX_ACTION_SLOT - 1;
			m_paActionSlot = m_aSlotQueue[m_nActionSlotIndex];
		...
		case WIDC_DOWN2:
			m_nActionSlotIndex++;
			if (m_nActionSlotIndex > MAX_ACTION_SLOT - 1) m_nActionSlotIndex = 0;
			m_paActionSlot = m_aSlotQueue[m_nActionSlotIndex];
		...
		case WIDC_BUTTON1: // Menu
		{
			...
			case TASKBAR_BOTTOM:
				if (g_WndMng.m_pWndMenu) 
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
					g_WndMng.m_pWndMenu->Move(CPoint(m_rectClient.left, rect.bottom - TASKBAR_HEIGHT - g_WndMng.m_pWndMenu->GetWndRect().Height()));
...
BACK:
#ifdef __ACTION_SLOT_PAGES
	if (!m_paActionSlot[nIndex].IsEmpty())
	{
		ItemProp* pItemProp = g_pPlayer->GetSkill(0, m_paActionSlot[nIndex].m_dwId)->GetProp();
		DWORD dwComboStyleSrc = pItemProp->dwComboStyle;
		int i = NULL;
		for (i = nIndex; i < MAX_SLOT_QUEUE - 1; i++)
		{
			memcpy(&m_paActionSlot[i], &m_paActionSlot[i + 1], sizeof(SHORTCUT));
			m_paActionSlot[i].m_dwIndex = i;
		}
		m_paActionSlot[i].m_dwShortcut = SHORTCUT_NONE;
		m_paActionSlot[i].m_dwId = 0;
		m_nCurQueueNum--;
		if (m_nCurQueueNum < 0)
			m_nCurQueueNum = 0;
...
{
#ifdef __AEGON_THEME_SWITCHER
	CRect rect = CRect(POINT_APPLET_X, POINT_APPLET_Y, (POINT_APPLET_X + m_nMaxSlotApplet * ICON_SIZE) + (m_nMaxSlotApplet - 1) * m_pOffsetLoopIncrease.x, POINT_APPLET_Y + ICON_SIZE);
#else // __AEGON_THEME_SWITCHER
	...
	if( rect.PtInRect( point ) )
	{
		point.x -= rect.left;
#ifdef __AEGON_THEME_SWITCHER
		point.x /= (ICON_SIZE + m_pOffsetLoopIncrease.x);
#else // __AEGON_THEME_SWITCHER
		point.x /= ICON_SIZE;
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_THEME_SWITCHER
	rect = CRect(POINT_ITEM_X, POINT_ITEM_Y, (POINT_ITEM_X + MAX_SLOT_ITEM * ICON_SIZE) + (MAX_SLOT_ITEM - 1) * m_pOffsetLoopIncrease.x, POINT_ITEM_Y + ICON_SIZE);
#else // __AEGON_THEME_SWITCHER
	rect = RECT_ITEM;
#endif // __AEGON_THEME_SWITCHER
	if( rect.PtInRect( point ) )
...
#ifdef __AEGON_THEME_SWITCHER
	rect = CRect(POINT_QUEUE_X, POINT_QUEUE_Y, (POINT_QUEUE_X + MAX_SLOT_QUEUE * SKILL_SIZE) + (MAX_SLOT_QUEUE - 1) * m_pOffsetLoopIncreaseQueue.x, POINT_QUEUE_Y + SKILL_SIZE);
#else // __AEGON_THEME_SWITCHER
	rect = RECT_QUEUE;
#endif // __AEGON_THEME_SWITCHER
	if( rect.PtInRect( point ) )
	{
		...
		point.y -= rect.top;
#ifdef __AEGON_THEME_SWITCHER
		point.x /= (SKILL_SIZE + m_pOffsetLoopIncreaseQueue.x);
#else // __AEGON_THEME_SWITCHER
		point.x /= SKILL_SIZE;
#endif // __AEGON_THEME_SWITCHER
...
#ifdef __ACTION_SLOT_PAGES
BOOL CWndTaskBar::SetShortcut(int nIndex, DWORD dwShortcut, DWORD dwType, DWORD dwId, CTexture* pTexture, int nWhere, DWORD dwUserId)
#else // __ACTION_SLOT_PAGES
...
//	pWndButton->Create( _T( "" ), 0, CRect( 65 + nIndex * 32, 3, 65 + nIndex * 32 + 32 , 3 + 32), this, dwId );
	...
	if( pTexture == NULL )
	{
...
#ifdef __SWITCH_EQUIP
		else if (dwShortcut == SHORTCUT_SWITCH_EQUIP)
		{
#ifdef __AEGON_THEME_SWITCHER
			pShortcut->m_pTexture = CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchAddTexture(g_Neuz.m_pd3dDevice, "icon_ability.dds", 0xffff00ff);
#else
		...
		if( dwShortcut == SHORTCUT_ITEM ) 
		{
			...
			CTexture* pTexture = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pItemBase->GetProp()->szIcon), COLOR_ITEM_BG );
		...
		*/
		...
		if( dwShortcut == SHORTCUT_SKILL ) 
		{
			...
			{	
				...
				//pWndButton->SetTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, lpSkill->GetProp()->szIcon ) );
				pShortcut->m_pTexture = m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, lpSkill->GetProp()->szIcon/*pItemBase->GetProp()->szIcon*/), COLOR_ITEM_BG, TRUE );
			}
		}	
		else
...
#ifdef __ACTION_SLOT_PAGES
			DWORD dwSkill = g_pPlayer->GetSkill(0, m_paActionSlot[nQueueDest].m_dwId)->dwSkill;
#else // __ACTION_SLOT_PAGES
...
#ifdef __ACTION_SLOT_PAGES
		DWORD dwSkill = g_pPlayer->GetSkill(0, m_paActionSlot[nQueueDest - 1].m_dwId)->dwSkill;
#else // __ACTION_SLOT_PAGES
...
#ifdef __ACTION_SLOT_PAGES
		dwSkill = g_pPlayer->GetSkill(0, m_paActionSlot[nQueueDest].m_dwId)->dwSkill;
#else // __ACTION_SLOT_PAGES
...
//	ItemProp* pItemProp1, * pItemProp2,* pItemProp3; 
	if( nIndex < m_nCurQueueNum ) // insert		// nIndex는 세팅할 칸의 인덱스
	{
		...
		{
			memcpy(&m_paActionSlot[i], &m_aSlotQueue[i - 1], sizeof(SHORTCUT));
		}
		pShortcut = &m_paActionSlot[nIndex];
#else // __ACTION_SLOT_PAGES
		for( int i = m_nCurQueueNum; i > nIndex; i-- )
		{
			memcpy( &m_aSlotQueue[ i ], &m_aSlotQueue[ i - 1 ], sizeof( SHORTCUT ) );
...
#ifdef __ACTION_SLOT_PAGES
		pShortcut = &m_paActionSlot[m_nCurQueueNum];
#else // __ACTION_SLOT_PAGES
...
#ifdef __ACTION_SLOT_PAGES
	if (pShortcut->m_dwType != SHORTCUT_SKILLFUN)
#endif // __ACTION_SLOT_PAGES
	pShortcut->m_dwUserId   = 0 ;
...
#ifdef __AEGON_TELEPORT_SHORTCUT
		&& pWndFrame->GetWndId() != APP_TELEPORTER
#endif // __AEGON_TELEPORT_SHORTCUT
...
#ifdef __AEGON_TELEPORT_SHORTCUT
		&& pShortcut->m_dwShortcut != SHORTCUT_TELEPORT
#endif // __AEGON_TELEPORT_SHORTCUT
		)
#else
...
#ifdef __ACTION_SLOT_PAGES
			SetShortcut(point.x, pShortcut->m_dwShortcut, pShortcut->m_dwType, pShortcut->m_dwId, pShortcut->m_pTexture, 0, pShortcut->m_dwUserId);
#else // __ACTION_SLOT_PAGES
...
//		rect.right += 32;
...
#ifdef __ACTION_SLOT_PAGES
			SetShortcut(point.x, pShortcut->m_dwShortcut, pShortcut->m_dwType, pShortcut->m_dwId, pShortcut->m_pTexture, 1, pShortcut->m_dwUserId);
#else // __ACTION_SLOT_PAGES
		...
		if( rect.PtInRect( point ) )
		{
			if( pShortcut->m_dwShortcut == SHORTCUT_SKILL && pShortcut->m_dwType == 0 )		// 스킬일경우만 등록
			{
				...
				{
					LPSHORTCUT lpShortcut = (LPSHORTCUT) pShortcut->m_dwData;
					if(lpShortcut && lpShortcut->m_dwData == 2 )
					{
	...
	if( pShortcut->m_pFromWnd == this && pShortcut->m_dwData != 0)
	{
		LPSHORTCUT lpShortcut = (LPSHORTCUT) pShortcut->m_dwData;
		// 아이템 삭제 서버로 전송
		if( lpShortcut->m_dwData == 0 )
...
LPSHORTCUT CWndTaskBar::Select( CPoint point )
{
...
#ifdef __ACTION_SLOT_PAGES
		pShortcut = &m_paActionSlot[point.x];
#else // __ACTION_SLOT_PAGES
	...
	{
#ifdef __ACTION_SLOT_PAGES
		m_aSlotSkill[m_nActionSlotIndex].m_dwShortcut = SHORTCUT_SKILLFUN;

		pShortcut = &m_aSlotSkill[m_nActionSlotIndex];
#else // __ACTION_SLOT_PAGES
		m_aSlotSkill.m_dwShortcut = SHORTCUT_SKILLFUN;
...
#if _WIN64
			m_GlobalShortcut.m_dwData = reinterpret_cast<ULONG_PTR>(m_pSelectShortcut);
#else
...
LPSKILL CWndTaskBar::GetCurrentSkillQueue()
{
	LPSKILL pSkill = NULL;
#ifdef __ACTION_SLOT_PAGES
	LPSHORTCUT pShortcut = &m_paActionSlot[m_nUsedSkillQueue];
#else // __ACTION_SLOT_PAGES
...
//
#ifdef __ACTION_SLOT_PAGES
BOOL CWndTaskBar::UseSkillQueue(CCtrl* pTargetObj, const int nIndex)
#else // __ACTION_SLOT_PAGES
...
#ifdef __MOUNT
	if (g_pPlayer->m_pActMover->IsUseMount())				return FALSE;
#endif // __MOUNT
...
#ifdef __ACTION_SLOT_PAGES
	int nSlotIndex = nIndex == -1 ? m_nUsedSkillQueue : nIndex;
	LPSHORTCUT pShortcut = &m_aSlotQueue[nSlotIndex][m_nUsedSkillQueue];
#else // __ACTION_SLOT_PAGES
	LPSHORTCUT pShortcut = &m_aSlotQueue[m_nUsedSkillQueue];
#endif // __ACTION_SLOT_PAGES
	...
	else
	{
		...
		else
		{
...
#else
#ifdef __AEGON_HOTKEY_F_KEYS
				unsigned char tmpChar = pApplet->m_cHotkey;
				if (g_bKeyTable[tmpChar])			//gmpbigsun: 하우징 Y키( 가구창 ) 검사 
#else
			...
			if( !pWndRegVend || !pWndVendorBuy )
			{
				...
				dwHotkey = dwHotkey1;
#ifdef __SINGLE_TASKBAR_SWAP
				DWORD dwHotkey2[10] = { 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x40 };
				if (g_Option.m_bTaskbarDisplay) 
				{
					dwHotkey = dwHotkey2;
				}
...
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
void CWndTaskBar::CenterTaskbar()
{
	int wndWidth;
	if (g_WndMng.resizeWidth > 0)
		wndWidth = g_WndMng.resizeWidth;
	else
		wndWidth = g_WndMng.GetWndRect().Width();

	if (!g_pD3dApp->IsWindow())
	{
		wndWidth = g_Option.m_nResWidth;
	}
	else if (g_Option.m_bWindowMaximized)
	{
		CRect tmp;
...
#ifdef __SMALL_CENTERED_TASKBAR
	if (g_Option.m_bCenterTaskbar && wndWidth >= 1440)
	{
		texWidth = 1440;
	}
	else
	...
	else if (wndWidth >= 1920)
	{
		texWidth = 1920;
	}
	else if (wndWidth >= 1680)
	{
		texWidth = 1680;
	}
	else if (wndWidth >= 1600)
	{
		texWidth = 1600;
	}
	else if (wndWidth >= 1440)
	{
		texWidth = 1440;
	}
	else if (wndWidth >= 1400)
	{
		texWidth = 1400;
	}
	else if (wndWidth >= 1360)
	{
		texWidth = 1360;
	}
	else if (wndWidth >= 1280)
	{
		texWidth = 1280;
	}
	else if (wndWidth >= 1024)
	{
		texWidth = 1024;
	}
	...
	if (g_Option.m_bCenterTaskbar) 
	{
		int freeSpace = wndWidth - texWidth;

		m_rectClient.right = m_rectClient.left + texWidth;
		m_rectClient.right += (freeSpace / 2);
		m_rectClient.left += (freeSpace / 2);

		m_rectLayout.right = m_rectLayout.left + texWidth;
		m_rectLayout.right += (freeSpace / 2);
		m_rectLayout.left += (freeSpace / 2);

		m_rectCurrentClient.right = m_rectCurrentClient.left + texWidth;
		m_rectCurrentClient.right += (freeSpace / 2);
		m_rectCurrentClient.left += (freeSpace / 2);

		m_rectCurrentWindow.right = m_rectCurrentWindow.left + texWidth;
		m_rectCurrentWindow.right += (freeSpace / 2);
		m_rectCurrentWindow.left += (freeSpace / 2);

		m_rectWindow.right = m_rectWindow.left + texWidth;
		m_rectWindow.right += (freeSpace / 2);
		m_rectWindow.left += (freeSpace / 2);
	}
	else 
	{
		m_rectClient.left = 0;
		...
		m_rectCurrentClient.left = 0;
		m_rectCurrentClient.right = m_rectCurrentClient.left + texWidth;

		m_rectCurrentWindow.left = 0;
		m_rectCurrentWindow.right = m_rectCurrentWindow.left + texWidth;
...
#ifdef __TITANIUM_TASKBAR

	//Website

	//Important Features
	pWndButton = AppendMenu(this, 0, APP_VENDOR_REVISION, GETTEXT(TID_APP_VENDOR)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuPrivateShop.dds")));

#ifdef __PREMIUM
	pWndButton = AppendMenu(this, 0, APP_PREMIUM_MENU, _T("Premium Menu")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_MenuPremium.png")));
#endif //__PREMIUM

#ifdef __AEGON_MESSENGER
	pWndButton = AppendMenu(this, 0, APP_AEGON_MESSENGER, _T("Messenger")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuChatting.dds")));
#endif

#ifdef __AEGON_INGAME_SHOP
	pWndButton = AppendMenu(this, 0, APP_AEGON_INGAME_SHOP, _T("Ingame Shop")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_Menu_ItemShop.dds")));
#endif //__AEGON_INGAME_SHOP

	//Titanium Systems Menu
	CWndButton* pWndButton4 = NULL;
	pWndButton4 = AppendMenu(this, 0, 0, _T("Titanium Systems")); pWndButton4->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_Titanium.png")));
	m_pMenu4 = new CWndMenu;
	m_pMenu4->CreateMenu(this);

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	pWndButton = AppendMenu(m_pMenu4, 0, APP_AEGON_ACHIEVEMENT_SYSTEM, _T("Achievement System")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuAchievement.png")));
#endif //__AEGON_ACHIEVEMENT_SYSTEM

#ifdef __FL_FARM_STAT
	pWndButton = AppendMenu(m_pMenu4, 0, APP_FARM_COUNTER, _T("Farm Counter")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuFarmCounter.dds")));
#endif //__FL_FARM_STAT

#ifdef __GUILDFINDER
	pWndButton = AppendMenu(m_pMenu4, 0, APP_GUILDFINDER, _T("Guild Finder")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuGuild.dds")));
#endif //__GUILDFINDER

/* -- Changed to NPC Loading
#ifdef __AEGON_LOOT_BOX
	pWndButton = AppendMenu(m_pMenu4, 0, APP_LOOTBOX, _T("Lootbox")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuLootbox.png")));
#endif //__AEGON_LOOT_BOX
*/
#ifdef __AEGON_MONSTER_HUNT
	pWndButton = AppendMenu(m_pMenu4, 0, APP_MONSTER_HUNT, _T("Monster Hunt")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuDamagedone.dds")));
#endif //__AEGON_MONSTER_HUNT

#ifdef __SYS_PARTYFINDER
	pWndButton = AppendMenu(m_pMenu4, 0, APP_PARTY_FIND, _T("Party Finder")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuParty.dds")));
#endif //__SYS_PARTYFINDER

#ifdef __SYS_PETFILTER
	pWndButton = AppendMenu(m_pMenu4, 0, APP_PETFILTER_MAIN, _T("Pet Filter")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuPetfilter.dds")));
#endif //__SYS_PETFILTER

#ifdef __AEGON_PVP_RANKING
	pWndButton = AppendMenu(m_pMenu4, 0, APP_PVP_RANKING, _T("PvP Ranking")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuPvPRanking.dds")));
#endif // __AEGON_PVP_RANKING

#ifdef __REPORT_SYSTEM
	pWndButton = AppendMenu(m_pMenu4, 0, APP_REPORT, _T("Report Problem")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuReport.png")));
#endif //__REPORT_SYSTEM

#ifdef __AEGON_SEASON_PASS
	pWndButton = AppendMenu(m_pMenu4, 0, APP_SEASON_PASS, _T("Season Pass")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuSP.png")));
#endif //__AEGON_SEASON_PASS

#ifdef __AEGON_TELEPORTER
	pWndButton = AppendMenu(m_pMenu4, 0, APP_TELEPORTER, _T("Teleporter")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuTeleport.dds")));
#endif

#ifdef __AZURIOM_IMAGES
	pWndButton = AppendMenu(m_pMenu4, 0, APP_AZURIOM_IMAGE, _T("Website Image")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuWebsiteImage.dds")));
#endif //__AZURIOM_IMAGES

#ifdef __AEGON_F_TOOL
	pWndButton = AppendMenu(m_pMenu4, 0, APP_AEGON_FTOOL, _T("F-Tool")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_Menu_FTool.dds")));
#endif

	pWndButton4->SetMenu(m_pMenu4);

	//Titanium Wiki Menu
	CWndButton* pWndButton5 = NULL;
	pWndButton5 = AppendMenu(this, 0, 0, _T("Titanium Wiki's")); pWndButton5->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuWiki.dds")));
	m_pMenu5 = new CWndMenu;
	m_pMenu5->CreateMenu(this);

#ifdef __WIKIPEDIA_ITEMS
	pWndButton = AppendMenu(m_pMenu5, 0, APP_WIKI_ITEMS, _T("Wikipedia - Items")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuWiki.dds")));
#endif //__WIKIPEDIA_ITEMS

#ifdef __WIKIPEDIA_JOB
	pWndButton = AppendMenu(m_pMenu5, 0, APP_WIKIPEDIA_JOB, _T("Wikipedia - Jobs")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuWikiJob.dds")));
#endif // __WIKIPEDIA_JOB

#ifdef __WIKIPEDIA_TITLE
	pWndButton = AppendMenu(m_pMenu5, 0, APP_WIKIPEDIA_TITLE, _T("Wikipedia - Titles")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuWikiTitiles.dds")));
#endif // __WIKIPEDIA_TITLE

#ifdef __WIKIPEDIA_MONSTER
	pWndButton = AppendMenu(m_pMenu5, 0, APP_WIKIPEDIA_MONSTER, _T("Wikipedia - Monsters")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuWikiMon.dds")));
	...
	{
		CWndButton* pWndButton6 = NULL;
		pWndButton6 = AppendMenu(this, 0, 0, _T("Titanium Staff")); pWndButton6->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuCharacter.dds")));
		m_pMenu6 = new CWndMenu;
		m_pMenu6->CreateMenu(this);
		pWndButton = AppendMenu(m_pMenu6, 0, APP_DEBUGINFO, GETTEXT(TID_APP_DEBUGINFO)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuDebug.dds")));
#ifdef __PLAYER_LIST
		pWndButton = AppendMenu(m_pMenu6, 0, APP_PLAYER_LIST, _T("Player List")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuCharacter.dds")));
#endif //__PLAYER_LIST

//#ifdef __BAN_SYSTEM
//		pWndButton = AppendMenu(m_pMenu6, 0, APP_PLAYER_PURGE, _T("Player Purge")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuCharacter.dds")));
//#endif //__BAN_SYSTEM

#ifdef __MODS_MANAGER
		pWndButton = AppendMenu(m_pMenu6, 0, APP_MODS_MANAGER, _T("Mods Manager")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_Infopang.dds")));
#endif //__MODS_MANAGER

#ifdef __AEGON_TOURNAMENT
		if (g_pPlayer->IsAuthHigher(AUTH_ADMINISTRATOR))
		{
			pWndButton = AppendMenu(m_pMenu6, 0, APP_AEGON_CREATE_TOURNAMENT, _T("Create Tournament")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_Infopang.dds")));
		}
#endif

#ifdef __ANARCHY_SYSTEM
		pWndButton = AppendMenu(m_pMenu6, 0, APP_ANARCHY_SKILL, _T("Anarchy Buffs")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuAnarchy.dds")));
#endif // __ANARCHY_SYSTEM

#ifdef __AEGON_PVP_MINIGAMES
		pWndButton = AppendMenu(m_pMenu6, 0, APP_PVP_MINIGAME_CREATE, _T("Create Minigame")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_Infopang.dds")));
		pWndButton = AppendMenu(m_pMenu6, 0, APP_PVP_MINIGAME_FINDER, _T("Find Minigame")); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_Infopang.dds")));
#endif

		pWndButton6->SetMenu(m_pMenu6);
	}

	//Default Flyff Menu's
	CWndButton* pWndButton7 = NULL;
	pWndButton7 = AppendMenu(this, 0, 0, _T("FlyFF Systems")); pWndButton7->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_Folder.dds")));

	m_pMenu7 = new CWndMenu;
	m_pMenu7->CreateMenu(this);
	pWndButton = AppendMenu(m_pMenu7, 0, APP_STATUS1, GETTEXT(TID_APP_STATUS)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuStatus.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_NAVIGATOR, GETTEXT(TID_APP_NAVIGATOR)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_MenuNavigator.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_CHARACTER3, GETTEXT(TID_APP_CHARACTER)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuCharacter.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_INVENTORY, GETTEXT(TID_APP_INVENTORY)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuInventory.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_BAG_EX, GETTEXT(TID_APP_BAG_EX)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuBackpack.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_HOUSING, GETTEXT(TID_GAME_HOUSING_BOX)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuHousing.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_SKILL4, GETTEXT(TID_APP_SKILL)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuSkill.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_QUEST_EX_LIST, GETTEXT(TID_APP_QUEST)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuQuest.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_MOTION, GETTEXT(TID_APP_MOTION)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuMotion.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_MAP_EX, GETTEXT(TID_APP_MAP)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuWorldmap.dds")));
	pWndButton = AppendMenu(m_pMenu7, 0, APP_COMM_ITEM, GETTEXT(TID_APP_ITEMTIME)); pWndButton->m_shortcut.m_dwShortcut = SHORTCUT_APPLET; SetTexture(pWndButton); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("Icon_MenuDuration.dds")));
	pWndButton7->SetMenu(m_pMenu7);

	//Other
	CWndButton* pWndButton8 = NULL;
	pWndButton8 = AppendMenu(this, 0, 0, _T("Other")); pWndButton8->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_Folder.dds")));
	m_pMenu8 = new CWndMenu;
	m_pMenu8->CreateMenu(this);
	pWndButton = AppendMenu(m_pMenu8, 0, APP_COMMUNICATION_CHAT, GETTEXT(TID_APP_COMMUNICATION_CHAT)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuChatting.dds")));
	pWndButton = AppendMenu(m_pMenu8, 0, APP_MESSENGER_, GETTEXT(TID_APP_MESSENGER)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuMessenger.dds")));
	pWndButton = AppendMenu(m_pMenu8, 0, APP_COUPLE_MAIN, GETTEXT(TID_GAME_COUPLE)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuCouple.dds")));
	pWndButton = AppendMenu(m_pMenu8, 0, APP_PARTY, GETTEXT(TID_APP_PARTY)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuParty.dds")));
	pWndButton = AppendMenu(m_pMenu8, 0, APP_GUILD, GETTEXT(TID_APP_COMPANY)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuGuild.dds")));
	pWndButton = AppendMenu(m_pMenu8, 0, APP_HELPER_HELP, GETTEXT(TID_APP_HELPER_HELP)); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuHelp.dds")));
	pWndButton8->SetMenu(m_pMenu8);

	//Websites
#ifdef __TASKBAR_OPEN_SITE
	pWndButton = AppendMenu(this, 0, APP_HOMEPAGE, "Website"); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuHomepage.png")));
	pWndButton = AppendMenu(this, 0, APP_DISCORD, "Discord"); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuDiscord.png")));
#endif //__TASKBAR_OPEN_SITE

	//Default
	pWndButton = AppendMenu( this, 0, APP_OPTIONEX, GETTEXT(TID_APP_OPTION) ); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuOption.dds")));
	pWndButton = AppendMenu( this, 0, APP_LOGOUT, GETTEXT( TID_APP_LOGOUT) ); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuLogout.dds")));
	pWndButton = AppendMenu( this, 0, APP_QUIT, GETTEXT( TID_APP_QUIT) ); pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, _T("icon_MenuQuit.dds")));

	//Styling -> Ignore
#ifdef __AEGON_THEME_SWITCHER
	CAegonThemeSwitcherMng::GetInstance()->ThemeSwitchSetTexture((CWndBase*)this, m_pApp->m_pd3dDevice, _T("WndTaskMenu.tga"), TRUE);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_HOTKEY_F_KEYS
		else if (tmpChar >= VK_F11 && tmpChar <= VK_F24)
		{
			int nKeyNum = tmpChar - 121 + 10;
			string.Format("%s\n[%s F%d]", pAppletFunc->m_pAppletDesc, prj.GetText(TID_GAME_TOOLTIP_HOTKEY), nKeyNum);
		}
#endif
		else
...
#ifdef __AEGON_THEME_SWITCHER
	CRect Rect = CRect(0, 0, (size * num + ((num-1) * m_pOffsetLoopIncreaseQueue.x)), size + m_pOffsetLoopIncreaseQueue.y);
#else // __AEGON_THEME_SWITCHER
...
#ifdef __AEGON_TELEPORT_SHORTCUT
	else if (lpShortcut->m_dwShortcut == SHORTCUT_TELEPORT)
	{
	}
#endif // __AEGON_TELEPORT_SHORTCUT
	else
