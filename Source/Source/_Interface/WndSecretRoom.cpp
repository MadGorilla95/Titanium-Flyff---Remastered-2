...
{ 
	...
	if(pWndText)
	{
		...
		pWndText->SetString(_T(prj.GetText(TID_GAME_SECRETROOM_CANCEL)), CAegonThemeSwitcherMng::GetInstance()->GetTextColor());
...
{ 
	...
	m_texGauHPEmptyNormal.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "GauEmptyNormal.bmp" ), COLOR_ITEM_BG, TRUE );
	m_texGauHPFillNormal.LoadTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "GauFillNormal.bmp" ), COLOR_ITEM_BG, TRUE );
			...
			if( IsValidObj(pObjMember) )
			{
				...
				if(i==0) //GuildMaster Color Set
				{
					...
					if(pObjMember->IsMaster())
						...
						strMember.Format( "%d-L. %s", pObjMember->GetLevel(), pObjMember->GetName() );
