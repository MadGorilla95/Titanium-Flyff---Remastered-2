...
#if _WIN64
INT_PTR CALLBACK	MaxConnDlgProc(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam);
INT_PTR CALLBACK	ToggleDlgProc(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam);
INT_PTR CALLBACK	SetTimeDlgProc(HWND hDlg, UINT iMessage, WPARAM wParam, LPARAM lParam);
#else
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	hMainWnd = hWnd = CreateWindow(szWindowClass, szTitle, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_VISIBLE,
	...
	int x = 20, y = 20;
	SetWindowPos(hWnd, NULL, x, y, 300, 200, SWP_SHOWWINDOW);
	...
	g_MyTrace.Initialize(hWnd, "Tahoma", RGB(0x41, 0x41, 0x41), RGB(0xff, 0xff, 0xff));
	CTime time = CTime::GetCurrentTime();
	g_MyTrace.Add(0, FALSE, "%s", time.Format("[%d/%m/%Y] [%H:%M:%S]"));
	g_MyTrace.AddLine('\0');
	g_MyTrace.AddLine('\0');
	g_MyTrace.Add(0, FALSE, "Accountserver started.");

	if (FALSE == g_dpSrvr.StartServer(PN_ACCOUNTSRVR_0) || FALSE == g_dpDbSrvr.StartServer(PN_ACCOUNTSRVR_1) || FALSE == CDPAdbill::GetInstance()->StartServer(PN_ADBILL))
	{
	...
	if (s.Load(lpszFileName))
	{
		...
		while (s.tok != FINISHED)
		{
			...
			else if (s.Token == "TEST")
			{
				...
				//sprintf( lpString, "LN Account", szTitle );
...
//				if( pServer->dwParent != NULL_ID )
			...
			else if (s.Token == "BillingIP")
			{
				s.GetToken();
#ifdef _WIN64
				GetBillingMgr()->SetConfig(BID_IP, (ULONG_PTR)(LPCTSTR)s.Token);
#else
