...
#ifdef __RT_1025
#ifdef _WIN64
	ar << static_cast<unsigned long>(g_Neuz.m_aRTMessenger[nSlot].size());
#else
...
void CDPLoginClient::OnError( CAr & ar )
{
	...
	switch( m_lError )
	{
		...
		case ERROR_BILLING_TIME_OVER:
			m_bShowDisconnectMsg = FALSE;
			g_WndMng.OpenMessageBoxUpper( prj.GetText(TID_DIAG_PLAYNOCHARGING), MB_OK, TRUE );
			break;
#if __VER >= 15 // __2ND_PASSWORD_SYSTEM
...
//		CMover* pMover;
		...
		for( int i = 0; i < CountEquip ; i++ )
		{
			...
			if (pItemProp->dwItemKind3 == IK3_ANIHAT)
				g_Neuz.m_apPlayer[slot]->InitializeAnimatedHat(pItemProp->dwID);
