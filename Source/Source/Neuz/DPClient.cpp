...
extern CWndTicketsDetails g_LayersDetail;
...
CDPClient::CDPClient()
{
	...
	ON_MSG(PACKETTYPE_SETGUILDCOLOR, &CDPClient::OnGuildSetColor);
	...
	ON_MSG(PACKETTYPE_GETDIFFPLAYER, &CDPClient::OnGetDiffPlayer);
...
void CDPClient::OnSnapshot(CAr& ar)
{
	...
	while (cb--)
	{
...
#ifdef __GUILDFINDER
		case SNAPSHOTTYPE_GUILD_FINDER_OPT:			OnGuildFinderOpt(ar); break;
		case SNAPSHOTTYPE_GUILDFINDER_LIST:			OnRefreshGuildList(ar); break;
		case SNAPSHOTTYPE_GUILDFINDER_APPLICATION:	OnGuildFinderAppication(ar); break;
#endif //__GUILDFINDER
...
#ifdef __AEGON_MONSTER_HUNT
		case SNAPSHOTTYPE_ADDMHPROGRESS: OnLoadMHProgress(ar); break;
#endif // __AEGON_MONSTER_HUNT

#ifdef __PARTY_ASSISTANT
		case SNAPSHOTTYPE_ASSIST_CLIENT: OnAssistClient(ar); break;
#endif //__PARTY_ASSISTANT

#ifdef __LEEAEG_SHOP_FINDER
		case SNAPSHOTTYPE_ADD_SHOP_FINDER_REFRESH: OnShopFinderRefresh(ar); break;
#endif // __LEEAEG_SHOP_FINDER

#ifdef __GIFTBOX_LIST_ITEMS
		case SNAPSHOTTYPE_GIFTBOX_PREVIEW: OnGBPreview(ar);		break;
#endif // __GIFTBOX_LIST_ITEMS

#ifdef __VENDING_NPC
		case SNAPSHOTTYPE_NPCVENDOR_LINK: OnVendorNPCLink(ar); break;
		case SNAPSHOTTYPE_NPCVENDOR_UNLINK:	OnVendorNPCUnlink(ar); break;
#endif // __VENDING_NPC
		...
		case SNAPSHOTTYPE_WEAPON_RARITY: OnWeaponRarity(ar); break;
		...
		case SNAPSHOTTYPE_UPDATE_TITLE:  OnUpdateTitle(objid, ar); break;
#endif // __SYS_CUSTOM_TITLE

#ifdef __PREMIUM
		case SNAPSHOTTYPE_UPDATE_PREMIUM: OnUpdatePremium(objid, ar); break;
#endif //__PREMIUM

#ifdef __SWITCH_EQUIP
		case SNAPSHOTTYPE_EQUIPSWITCH_UPDATE: OnEquipSwitch(ar); break;
...
#ifdef __HEAL_SHOW
		case SNAPSHOTTYPE_HEAL_DAMAGE: OnHealDamage(objid, ar);	break;
#endif // __HEAL_SHOW
...
#ifdef __AEGON_LOOT_BOX
		case SNAPSHOTTYPE_LOOTBOX_RESULT:		OnLootboxResult(ar); break;
#endif // __AEGON_LOOT_BOX

#ifdef __AEGON_VIEW_INV
		case SNAPSHOTTYPE_ADD_VIEW_INV:		OnViewInventory(objid, ar); break;
		case SNAPSHOTTYPE_ADD_VIEW_BAG:		OnViewBag(objid, ar); break;
		case SNAPSHOTTYPE_ADD_VIEW_BANK:	OnViewBank(objid, ar); break;
#endif // __AEGON_VIEW_INV

#ifdef __BADGE
		case SNAPSHOTTYPE_BADGE_UPDATE: OnBadgeListUpdate(ar); break;
		case SNAPSHOTTYPE_BADGE_CHANGE: OnBadgeChange(objid, ar); break;
#endif //__BADGE

#ifdef __AEGON_INGAME_DP
		case SNAPSHOTTYPE_DP_REFRESH: OnDPRefresh(objid, ar); break;
#endif // __AEGON_INGAME_DP

#ifdef __SYS_MONSTER_CLASH
		case SNAPSHOTTYPE_MONSTER_CLASH_REGISTER_WINDOW: OnMonsterClashRegisterWindow(ar); break;
		case SNAPSHOTTYPE_MONSTER_CLASH_REGISTER_MEMBER_WINDOW: OnMonsterClashRegisterMemberWindow(ar); break;
		case SNAPSHOTTYPE_MONSTER_CLASH_REGISTER: OnMonsterClashRegister(ar); break;
		case SNAPSHOTTYPE_MONSTER_CLASH_REGISTER_MEMBER: OnMonsterClashRegisterMember(ar); break;
		case SNAPSHOTTYPE_MONSTER_CLASH_UNREGISTER_MEMBER: OnMonsterClashUnRegisterMember(ar); break;
		case SNAPSHOTTYPE_MONSTER_CLASH_STATE: OnMonsterClashState(ar); break;
#endif //__SYS_MONSTER_CLASH

#ifdef __SHOPS_VISITED
		case SNAPSHOTTYPE_REMOVE_SHOP: OnRemoveShop(ar); break;
#endif //__SHOPS_VISITED

#ifdef __TICKETS_DETAILS
		case SNAPSHOTTYPE_TICKETDETAIL:	LayersDetailReceived(objid, ar); break;
#endif	//__TICKETS_DETAILS
...
#ifdef __CHECKLOAD
		case SNAPSHOTTYPE_CHECKLOAD: OnCheckLoad(ar); break;
#endif //__CHECKLOAD

#ifdef __TIMER
		case SNAPSHOTTYPE_TIMER: OnDungeonTimer(ar); break;
#endif //__TIMER

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
		case SNAPSHOTTYPE_ADDACHIEVEMENTPROGRESS: OnLoadAchievementProgress(ar); break;
		case SNAPSHOTTYPE_ADDFINISHEDACHIEVEMENT: OnFinishedAchievement(ar); break;
		case SNAPSHOTTYPE_ADDFAVACHIEVEMENTREFRESH: OnFavoriteAchievementRefresh(ar); break;
		case SNAPSHOTTYPE_ADDCOLLECTINGTIMEREFRESH: OnCollectingTimeRefresh(ar); break;
#endif // __AEGON_ACHIEVEMENT_SYSTEM
			
#ifdef __UPGRADE_SUCCESS
		case SNAPSHOTTYPE_UPGRADE_SUCCESS: OnUpgradeSuccess(ar); break;
#endif //__UPGRADE_SUCCESS

#ifdef __AFK_MONITORING
		case SNAPSHOTTYPE_SEND_AFKSTATE: OnSetAFKState(objid, ar); break;
#endif //__AFK_MONITORING

#ifdef __SYS_NAMECOLOR
		case SNAPSHOTTYPE_SETNAMECOLOR: OnSetNameColor(objid, ar); break;
#endif //__SYS_NAMECOLOR

#ifdef __AEGON_LAST_ONLINE
		case SNAPSHOTTYPE_ADDLASTONLINEREFRESH: OnLastOnlineRefresh(ar); break;
		case SNAPSHOTTYPE_ADDSINGLELASTONLINEREFRESH: OnSingleLastOnlineRefresh(ar); break;
		case SNAPSHOTTYPE_ADDOWNLASTONLINEREFRESH: OnOwnLastOnlineRefresh(ar); break;	
#endif // __AEGON_LAST_ONLINE

#ifdef __AEGON_INGAME_TIME
		case SNAPSHOTTYPE_ADDONLINETIMEREFRESH: OnOnlineTimeRefresh(ar); break;
#endif // __AEGON_INGAME_TIME

#ifdef __PLAYER_LIST
		case SNAPSHOTTYPE_PLAYER_LIST:	OnPlayerList(ar); break;
#endif // __PLAYER_LIST

#ifdef __AEGON_CRAFTING_SYSTEM
		case SNAPSHOTTYPE_CRAFTING_SUCCESS: OnCraftingSuccess(ar); break;
		case SNAPSHOTTYPE_CRAFTING_EXP_REFRESH: OnCraftingExpRefresh(ar); break;
#endif // __AEGON_CRAFTING_SYSTEM

#ifdef __WEAPON_MERGE
		case SNAPSHOTTYPE_MERGEWEAPON: OnMergedWeapon(ar); break;
#endif //__WEAPON_MERGE

#ifdef __BUYBACK
		case SNAPSHOTTYPE_BUYBACK:	OnBuyBack(ar); break;
		case SNAPSHOTTYPE_BUYBACK_REMOVE_ITEM:	OnBuyBackRemoveItem(ar); break;
#endif // __BUYBACK

#ifdef __CLIENT_ONLINE_COUNT
		case SNAPSHOTTYPE_PLAYER_COUNT: OnOnlineChange(ar); break;
#endif //__CLIENT_ONLINE_COUNT

#ifdef __AEGON_SEASON_PASS
		case SNAPSHOTTYPE_ADDSEASONPASSPROGRESS: OnLoadSeasonPassProgress(ar); break;
		case SNAPSHOTTYPE_ADDSEASONPASSUPDATE: OnUpdateSeasonPassProgress(ar); break;
		case SNAPSHOTTYPE_ADDSEASONPASSCOUNTDOWN: OnSeasonPassCountdown(ar); break;
#endif // __AEGON_SEASON_PASS

#ifdef __AEGON_PVP_RANKING
		case SNAPSHOTTYPE_ADD_PVP_RANKING: OnAddPvPRanking(ar); break;
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_PERMANENT_BUFF
		case SNAPSHOTTYPE_ADD_PERM_BUFF: OnAddPermBuffRefresh(ar); break;
#endif // __AEGON_PERMANENT_BUFF

#ifdef __AEGON_PREM_MOTIONS
		case SNAPSHOTTYPE_ADD_MOTION_REFRESH: OnAddMotionRefresh(ar); break;
#endif // __AEGON_PREM_MOTIONS

#ifdef __PET_GLOW
		case SNAPSHOTTYPE_PETGLOW_CHANGESFX: OnChangePetGlowSfx(objid, ar); break;
#endif // __PET_GLOW

#ifdef __COMBAT_LOG
		case SNAPSHOTTYPE_CLEARLOG: OnCombatLogReset(ar); break;
#endif //__COMBAT_LOG

#ifdef __AEGON_EMOTE_SYSTEM
		case SNAPSHOTTYPE_ADD_EMOTE_REFRESH: OnAddEmoteRefresh(ar); break;
#endif //__AEGON_EMOTE_SYSTEM

#ifdef __LEEAEG_SYSTEM_INDIVIDUAL
		case SNAPSHOTTYPE_SYSTEM_TEXT: OnSystem(ar); break;
#endif //__LEEAEG_SYSTEM_INDIVIDUAL

#ifdef __CRYSTAL
		case SNAPSHOTTYPE_CRYSTAL: OnCrystal(ar); break;
#endif // __CRYSTAL

#ifdef __AEGON_INGAME_SHOP
		case SNAPSHOTTYPE_INGAME_SHOP_INFO: OnAddIngameShopInfo(ar); break;
#endif //__AEGON_INGAME_SHOP

#ifdef __AEGON_TOURNAMENT
		case SNAPSHOTTYPE_TOURNAMENT_MAP: OnTournamentMap(ar); break;
		case SNAPSHOTTYPE_TOURNAMENTS_CHANGED: OnTournamentsChanged(ar); break;
#endif //__AEGON_TOURNAMENT

#ifdef __ROYAL_RUMBLE
		case SNAPSHOTTYPE_SIEGE_WAR: OnPlayerSiege(ar); break;
		case SNAPSHOTTYPE_PLAYERWAR_UPDATE: OnUpdatePlayerSiege(ar); break;
#endif // __ROYAL_RUMBLE

#ifdef __AEGON_AUTO_STATS
		case SNAPSHOTTYPE_AUTO_STATS: OnUpdateAutoStats(ar); break;
#endif //__AEGON_AUTO_STATS

#ifdef __AEGON_F_TOOL
		case SNAPSHOTTYPE_FTOOL_UPDATE: OnUpdateFTool(ar); break;
#endif //__AEGON_F_TOOL

#ifdef __SYS_REBIRTH
		case SNAPSHOTTYPE_REBIRTH_LEVEL: OnRebirthLevel(objid, ar); break;
#endif //__SYS_REBIRTH

#ifdef __AEGON_TIME_DIFF
		case SNAPSHOTTYPE_UPDATE_TIME_DIFF: OnUpdateTimeDiff(ar); break;
#endif

#ifdef __NEW_SFX_WINGS
		case SNAPSHOTTYPE_WING_SFX: OnChangeWingSfx(objid, ar); break;
#endif //__NEW_SFX_WINGS

#ifdef __FL_BAG_LOOT
		case SNAPSHOTTYPE_ADD_CLIENT_BAG: OnAddSelectedBag(ar); break;
#endif //__FL_BAG_LOOT

#ifdef __AEGON_PVP_MINIGAMES
		case SNAPSHOTTYPE_ADD_PVPMINIGAME_INVITE: OnAddPvPMinigameInvitation(ar); break;	
		case SNAPSHOTTYPE_ADD_PVPMINIGAME_REFRESH: OnAddPvPMinigameRefresh(ar); break;
		case SNAPSHOTTYPE_ADD_PVPMINIGAME_CHAT: OnAddPvPMinigameChat(ar); break;
		case SNAPSHOTTYPE_ADD_PVPMINIGAME_LIST: OnAddPvPMinigameList(ar); break;
#endif
...
#ifdef __AEGON_IMPROVED_SHOPDESIGN
		case SNAPSHOTTYPE_ADD_VENDOR_SHOPDESIGN: OnAddVendorShopDesign(ar); break;
#endif

#ifdef __AEGON_MESSENGER
		case SNAPSHOTTYPE_ADD_MESSENGER_UPDATE: OnAddMessengerStatus(ar); break;
		case SNAPSHOTTYPE_ADD_MESSENGER_CHAT_UNIT: OnAddMessengerChatUnitResult(ar); break;
#endif

#ifdef __SYS_LOCK_ITEM
		case SNAPSHOTTYPE_ADD_ITEM_LOCK: OnAddItemLock(ar); break;
#endif

#ifdef __MAX_COLLECT_BY_IP
		case SNAPSHOTTYPE_ADD_COLL_STATUS: OnAddCollectingStatus(ar); break;
#endif

#ifdef __STOP_EXP
		case SNAPSHOTTYPE_ADD_EXP_STATE: OnAddExpStopState(ar); break;
#endif

#ifdef __AEGON_BAG_FREE_MOVE
		case SNAPSHOTTYPE_MOVE_POCKET_ITEM: OnMovePocketItem(ar); break;
#endif

		default:
		{
			ASSERT(0);
#ifdef __LOG_UNKNOWN_SNAPSHOT
			Error("unknown snapshot hdr: %d prev: %d", hdr, prev);
#endif
...
void CDPClient::OnJoin(CAr& ar)
{
...
#ifdef __FLYFF_INITPAGE_EXT
	g_WndMng.m_pTheme->DestoryTitleWorld();
...
#ifdef __AEGON_FIXES
	if(pMover && !pMover->m_bPrintedItemDuration)
#else
	if (pMover)
#endif
	{
#ifdef __YS_CHATTING_BLOCKING_SYSTEM
		g_Option.loadBlockedUsers(pMover->GetName());
#endif
...
#ifdef __VENDING_NPC
				if (pItemElem->IsFlag(CItemElem::expired) && pItemElem->GetPropA()->dwItemKind3 != IK3_VENDING)
#else // __VENDING_NPC
...
#ifdef __SYS_MONSTER_CLASH
		CWndMonsterClashRank::GetInstance()->SetActive(FALSE);
#endif //__SYS_MONSTER_CLASH

#ifdef __STOP_EXP
		SendSetExpStop(g_Option.m_bExpStop);
#endif //__STOP_EXP
...
#ifdef __DISCORD_RPC
		if (pMover->GetWorld()) {
			g_Discord->Update(pMover->GetName(), pMover->GetWorld()->m_szWorldName, pMover->GetJob(), pMover->GetLevel()
#ifdef __SYS_REBIRTH
				, pMover->GetRebirthLevel()
#endif //__SYS_REBIRTH				
			);
		}
#endif //__DISCORD_RPC
		...
		pMover->m_bPrintedItemDuration = TRUE;
	}
	...
	g_pD3dApp->AegonSizeChange(TRUE);
#endif // __AEGON_FULLSCREEN_AND_RESIZE

#ifdef __SYS_CUSTOM_TITLE
	SendUpdateShowTitle(g_Option.m_bShowCustomTitle);
#endif // __SYS_CUSTOM_TITLE

#ifdef __PREMIUM
	SendUpdateShowPremium(g_Option.m_bShowPremiumTitle);
#endif // __PREMIUM

#ifdef __AEGON_SEASON_PASS
	SendLoadSeasonPassProgress();
#endif // __AEGON_SEASON_PASS
	...
	if (IsValidObj(g_pPlayer)) 
	{
		try
		{
			g_Option.LoadMessengerChats();
			CAegonMessengerMng::GetInstance()->SetLoadedId(g_pPlayer->m_idPlayer);
		}
		catch (...)
		{
			CAegonMessengerMng::GetInstance()->ClearAll();

			CString strFileOld, strFileName, strFileNew;
			strFileName.Format("chat_history_%s.tmmo", g_pPlayer->GetName());
			strFileOld = MakePath(DIR_MESSENGER_LOG, strFileName.GetString());
			strFileName.Format("chat_history_%s_broken.tmmo", g_pPlayer->GetName());
			strFileNew = MakePath(DIR_MESSENGER_LOG, strFileName.GetString());

			rename(strFileOld.GetString(), strFileNew.GetString());

			MessageBox(NULL, "Error Loading Messenger History.", "Error Report", MB_ICONWARNING | MB_OK);
		}
...
#ifdef __PARTY_ASSISTANT
					pd.data.nAssist = pPlayer->GetAssist();
#endif //__PARTY_ASSISTANT
#ifdef __LEESE_PARTY_LOCATION
					pd.data.nLocation = pPlayer->GetWorld()->GetID();
...
#ifdef __DISCORD_RPC
					if ((CMover*)pObj == g_pPlayer)
					{
						if (g_pPlayer)
							if (g_pPlayer->GetWorld())
							{
								g_Discord->Update(g_pPlayer->GetName(), g_pPlayer->GetWorld()->m_szWorldName);
							}
					}
#endif //__DISCORD_RPC
...
#ifdef __MOUNT
					&& (pPlayer->GetMountId() <= 0 || pPlayer->GetMountId() == NULL_ID)
#endif // __MOUNT
					)
				{
...
#ifdef __MOUNT
				if (pPlayer->m_pActMover->IsUseMount())
				{
					CModel* pModel = prj.m_modelMng.LoadModel(D3DDEVICE, OT_ITEM, pPlayer->GetMountId());
					CModelObject* pModelObject = (CModelObject*)pModel;
					if (pModelObject->m_pBone)
					{
						MOUNT_ANIMATION* pMountAnimation = prj.GetMountAnimation(pPlayer->GetMountId());
						if (pMountAnimation)
						{
							if (pModelObject)
							{
								CString strMotion;
								if (pPlayer->IsFly())
									strMotion = pModelObject->GetMotionFileName(pMountAnimation->m_aMonsterAnimation.find(MTI_FLY_STAND)->second);
								else
									strMotion = pModelObject->GetMotionFileName(pMountAnimation->m_aMonsterAnimation.find(MTI_STAND)->second);
								assert(strMotion != _T(""));
								pModelObject->LoadMotion(strMotion);
							}
					...
					pPlayer->SetMount(pModel, pPlayer->GetMountId());
					pPlayer->SetMotion(prj.GetMountAnimation(pPlayer->GetMountId())->dwPlayerAni, ANILOOP_LOOP);
				}
#endif // __MOUNT
	...
	else
	{
		...
		if (dwObjType == OT_MOVER)
		{
			...
			if (((CMover*)pObj)->IsNPC() && ((CMover*)pObj)->GetProp()->dwClass == RANK_BOSS)
			{
				...
				{
#ifdef __fmod
					SoundSystemClass::playMusic(BGM_IN_BOSS, true);
#else
...
#ifdef __fmod
				SoundSystemClass::playSound(pItemProp->dwSubDefine, { pObj->m_vPos.x, pObj->m_vPos.y, pObj->m_vPos.z });
#else
...
void CDPClient::OnChat(OBJID objid, CAr& ar)
{
	...
	if (IsValidObj(pMover))
	{
		...
		{
			if (prj.m_setBlockedUserID.find(pMover->GetName()) != prj.m_setBlockedUserID.end()) { return; }
		}
		...
		else
		{
			...
			if (pMover && pMover->IsPlayer()) 
			{
				if (pMover->IsAuthHigher(AUTH_HELPER))
				{
					sprintf(szChat, "[Staff] %s: %s", pMover->GetName(TRUE), szBuf);
				}
				else if (pMover->IsPremium() && pMover->m_bShowPremium)
				{
					sprintf(szChat, "[Premium] %s: %s", pMover->GetName(TRUE), szBuf);
				}
				else
				{
...
void CDPClient::OnDamage(OBJID objid, CAr& ar)
{
	...
	{
#ifdef __HEAL_SHOW
		pMover->IncHitPoint((int)((-1) * (int)(dwHit)), TRUE);
#else // __HEAL_SHOW
		...
		g_PASS:
...
#ifdef __COMBAT_LOG
						bool bAdd = false;
						CMover* atk = (CMover*)pAttacker;

						if (atk->IsActiveMover()) bAdd = true;

						if (atk->m_idparty != 0 && g_pPlayer->m_idparty != 0 && atk->m_idparty == g_pPlayer->m_idparty)
							bAdd = true;

						if (atk->IsNPC() &&
							(pMover->IsActiveMover() || (pMover->m_idparty != 0 && g_pPlayer->m_idparty != 0 && pMover->m_idparty == g_pPlayer->m_idparty))
							)
							bAdd = true;

						if (bAdd)
							CombatLog::GetInstance()->AddEvent(EventType::ET_DAMAGE, dwHit, atk->GetId(), (atk->IsPlayer() ? atk->GetName() : atk->GetProp()->szName), pMover->GetId(), (pMover->IsPlayer() ? pMover->GetName() : pMover->GetProp()->szName));
#endif //__COMBAT_LOG
					}
				}
				else
			...
			if (!pMover->IsPlayer() && ((CMover*)pAttacker) && ((CMover*)pAttacker) == g_pPlayer && dwHit != 0)
			{
				CWndFarmCounter* pWndFC = (CWndFarmCounter*)g_WndMng.GetWndBase(APP_FARM_COUNTER);
				if (pWndFC)
				{
					if (pWndFC->dwStartTime != NULL)
					{
						pWndFC->iTotalDamage += dwHit;
					}
				}
			}
#endif //__FL_FARM_STAT
#ifdef __MIKSIK_FLASH_WINDOW_EX
		...
		if (IsValidObj(pAttacker))
		{
			if (pAttacker->GetType() == OT_MOVER)
			{
				if (dwAtkFlags & AF_CRITICAL)
				{
					...
					{
#ifdef __fmod
						const D3DXVECTOR3& refVec = pAttacker->m_vPos;
						SoundSystemClass::playSound(pItemProp->dwSndAttack2, { refVec.x, refVec.y, refVec.z });
#else
...
void CDPClient::OnMoverDeath(OBJID objid, CAr& ar)
{
	...
	if (IsValidObj((CObj*)pMover))
	{
...
#ifdef __HEAL_SHOW
		pMover->IncHitPoint(-nHitPoint, FALSE);
#else // __HEAL_SHOW
...
#ifdef __AEGON_FIXES
		BOOL bAutoAttackNextTarget = TRUE;
#endif

#ifdef __RUMBLE_REVIVE
		CWorld* pMoverWorld = pMover->GetWorld();
		if (pMoverWorld && pMoverWorld->GetID() == WI_WORLD_FWC)
		{
			SendHdr(PACKETTYPE_REVIVAL_TO_LODESTAR);
			return;
		}
#endif //__RUMBLE_REVIVE

#ifdef __AEGON_MONSTER_HUNT
		CWndMonsterHunt* pWndMonsterHunt = (CWndMonsterHunt*)g_WndMng.GetApplet(APP_MONSTER_HUNT);
		
		if (g_Option.m_bPrintMHProgress || (pWndMonsterHunt && pWndMonsterHunt->m_wndQuest))
		{
			DWORD dwColorTextOut = CAegonThemeSwitcherMng::GetInstance()->GetHighlightColor(HIGHLIGHT_COLOR_GREEN);
			if (g_pPlayer && pAttacker && pAttacker == g_pPlayer && pMover && pMover->GetType() == OT_MOVER && !pMover->IsPlayer())
			{
				DWORD dwMobID = pMover->m_dwIndex;
				for (int i = 0; i < g_pPlayer->m_MHProgress.vecQuests.size(); i++)
				{
					__MONSTERHUNT_SINGLE* tmpUnit = &g_pPlayer->m_MHProgress.vecQuests[i];
					if (tmpUnit && tmpUnit->dwMobID == dwMobID && !tmpUnit->dwRewardSent)
					{
						map<DWORD, __MONSTERHUNT_QUEST>::iterator it = CMonsterHuntMng::GetInstance()->m_mapMonsterHunt.find(tmpUnit->dwQuestID);
						if (it != CMonsterHuntMng::GetInstance()->m_mapMonsterHunt.end()) 
						{
							if (tmpUnit->dwCount < it->second.mobNum) 
							{
								tmpUnit->dwCount++;
								if (g_Option.m_bPrintMHProgress)
								{
									CString strTmp;
									ItemProp* tmpProp = prj.GetItemProp(it->second.rewardItem);
									if(tmpProp)
										strTmp.Format("Monsterhunt %s %d/%d Reward: '%s'", pMover->GetName(), tmpUnit->dwCount, it->second.mobNum, tmpProp->szName);
									else
										strTmp.Format("Monsterhunt %s %d/%d", pMover->GetName(), tmpUnit->dwCount, it->second.mobNum);
									g_WndMng.PutString(strTmp.GetString(), 0, dwColorTextOut);
								}
								if (tmpUnit->dwCount == it->second.mobNum) 
								{
									tmpUnit->dwRewardSent = 1;
...
#endif //__CSC_VER11_2
#ifdef __AEGON_FIXES
			if (pAttacker->GetWorld()->GetObjFocus() == ((CObj*)pMover))
			{
				pAttacker->GetWorld()->SetObjFocus(NULL);
			}
			else
				bAutoAttackNextTarget = FALSE;
#else
			pAttacker->GetWorld()->SetObjFocus(NULL);
#endif
			((CWndWorld*)g_WndMng.m_pWndWorld)->SetLastTarget(NULL_ID);
		...
		if (IsValidObj(pAttacker)) 
		{
			int nJob = pAttacker->GetJob();
			BOOL bMagic = FALSE;
			if (nJob == JOB_MAGICIAN || nJob == JOB_PSYCHIKEEPER || nJob == JOB_ELEMENTOR || nJob == JOB_PSYCHIKEEPER_MASTER || nJob == JOB_ELEMENTOR_MASTER
				|| nJob == JOB_PSYCHIKEEPER_HERO || nJob == JOB_ELEMENTOR_HERO || nJob == JOB_MENTALIST_HERO || nJob == JOB_ELEMENTORLORD_HERO)
				bMagic = TRUE;
			{
#ifdef __AEGON_FIXES
				if (g_Option.m_bAutoAttackNT && bMagic == FALSE && bAutoAttackNextTarget)
#else
				if (g_Option.m_bAutoAttackNT && bMagic == FALSE)
#endif
				{
				...
				// pAttacker가 없을수도있다.
		{
...
#ifdef __FL_FARM_STAT
				if (pAttacker && pAttacker == g_pPlayer)
				{
					CWndFarmCounter* pWndFC = (CWndFarmCounter*)g_WndMng.GetWndBase(APP_FARM_COUNTER);
					if (pWndFC)
					{
...
#if _WIN64
			pMover->SendActMsg(OBJMSG_DIE, dwMsg, reinterpret_cast<UINT_PTR>(pAttacker));
#else
			...
			if (pMover->IsActiveMover())
			{
				...
				{
#ifdef __fmod
					SoundSystemClass::playMusic(BGM_IN_DEATH, false);
#else
...
void CDPClient::OnCreateItem(OBJID objid, CAr& ar)
{
	...
	if (IsValidObj((CObj*)pMover))
	{
		...
		if (bAdd)
		{
			...
			if (pWndFC)
			{
				if (pWndFC->dwStartTime != NULL)
				{
					...
					if (pProp)
					{
						if (pProp->dwID == II_SYS_SYS_SCR_PERIN)
						{
							pWndFC->nInt[1] += itemElem.m_nItemNum;
						}
						else
						{
							for (int i = 0; i < prj.m_vecCountItems.size(); i++)
							{
								if (prj.m_vecCountItems[i].pCountItems == pProp)
								{
									prj.m_vecCountItems[i].nCountItems += itemElem.m_nItemNum;
									break;
								}
							}
						}
		...
		for (int i = 0; i < nCount; i++)
		{
			...
			if (pMover->IsActiveMover())
#ifdef __fmod
				SoundSystemClass::playSound(SND_INF_GROUNDPICKUP, { vPos.x, vPos.y, vPos.z });
#else
...
void CDPClient::OnSetPos(OBJID objid, CAr& ar)
{
	...
	{
#ifdef __AEGON_FIXES
		if (IsValidObj(g_pPlayer) && g_pPlayer->GetId() == objid)
		{
			CWndShop* pWndShop = (CWndShop*)g_WndMng.GetWndBase(APP_SHOP_);
			if (pWndShop)
				pWndShop->Destroy();
			...
			if (bTeleport)
			{
				//get area info and select image
				CString strImageFile = __AEGON_DEFAULT_TELE_IMAGE;
				int nSizeWorldRegion = pWorld->m_aRegion.GetSize();
				for (int i = 0; i < nSizeWorldRegion; i++)
				{
					...
					if (lpRegionElem->m_rect.PtInRect(CPoint((int)(vPos.x), (int)(vPos.z))))
					{
						...
						strImageFile = prj.GetTeleportImageForRegion(CString(lpRegionElem->m_szTitle));
						if (strImageFile.Compare(__AEGON_DEFAULT_TELE_IMAGE) != 0)
							break;
					}
				}
				CTexture* pTextureImage = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME, strImageFile.GetString()), COLOR_ITEM_BG);

				if (pTextureImage) 
				{
					//g_Neuz.m_TexLoading.LoadTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME, strImageFile.GetString()), COLOR_ITEM_BG, FALSE);
					...
					if (g_WndMng.resizeWidth > 0)
					{
						if (g_Option.m_bWindowMaximized)
						{
							...
							pTextureImage->m_size = CSize(tmp.Width(), tmp.Height());
						...
						{
							pTextureImage->m_size = CSize(g_WndMng.resizeWidth, g_WndMng.resizeHeight);
					...
					{
						pTextureImage->m_size = CSize(FULLSCREEN_WIDTH, FULLSCREEN_HEIGHT);
						//g_Neuz.m_TexLoading.m_size = CSize(FULLSCREEN_WIDTH, FULLSCREEN_HEIGHT);
					}

					if (SUCCEEDED(g_Neuz.m_pd3dDevice->BeginScene()))
					{
						g_Neuz.m_2DRender.RenderTexture(CPoint(0, 0), pTextureImage /*&g_Neuz.m_TexLoading*/, g_Neuz.m_nTexAlpha);
					}
...
void CDPClient::OnReplace(CAr& ar)
{
...
#ifdef __AEGON_FIXES
	TCHAR lpString[128];
	ar.ReadString(lpString, 128);

	CWndShop* pWndShop = (CWndShop*)g_WndMng.GetWndBase(APP_SHOP_);
	if (pWndShop)
		pWndShop->Destroy();

	CWndAwakening* pWndAwake = (CWndAwakening*)g_WndMng.GetApplet(APP_AWAKENING);
	if (pWndAwake) 
	{
		pWndAwake->m_pItemElem = NULL;
		pWndAwake->m_pEItemProp = NULL;
		pWndAwake->m_pTexture = NULL;

		CWndButton* pButton = (CWndButton*)pWndAwake->GetDlgItem(WIDC_BUTTON1);
		if(pButton)
	...
	if (pWorld)
	{
		...
		TCHAR lpString[64];
		ar.ReadString(lpString, 64);
#endif
		if (strcmp(lpString, pWorld->m_szFileName) != 0)
		{
			SendHdr(PACKETTYPE_LEAVE);
			return;
		}
	}
...
#ifdef __VENDING_NPC
	if (VendorDeploy()->IsReady())
		VendorDeploy()->EndDeploy();
#endif // __VENDING_NPC
	...
	strPath = MakePath(DIR_THEME, __AEGON_DEFAULT_TELE_IMAGE);

	if(dwWorldID == WI_WORLD_MADRIGAL)
		strPath = MakePath(DIR_THEME, "MapLoading_Madrigal.png");
	else if (dwWorldID == WI_WORLD_GUILDWAR || (dwWorldID >= WI_WORLD_GUILDWAR1TO1_0 && dwWorldID <= WI_WORLD_GUILDWAR1TO1_L))
		strPath = MakePath(DIR_THEME, "MapLoading_GuildWar.png");
	else if (dwWorldID == WI_INSTANCE_OMINOUS)
		strPath = MakePath(DIR_THEME, "MapLoading_Aminus.png");
	else if (dwWorldID == WI_INSTANCE_OMINOUS_1)
		strPath = MakePath(DIR_THEME, "MapLoading_Aminus.png");
	else if (dwWorldID == WI_INSTANCE_DREADFULCAVE)
		strPath = MakePath(DIR_THEME, "MapLoading_Catacombs.png");
	else if (GuildHouse->IsMyGuildHouse(dwWorldID))
		strPath = MakePath(DIR_THEME, "MapLoading_Guildhouse.png");
#ifdef __LOADING_SCREEN_UPDATE
	else if (dwWorldID == WI_WORLD_ARENA || dwWorldID == WI_WORLD_ARENA_1)
		strPath = MakePath(DIR_THEME, "MapLoading_Arena.png");
	else if (dwWorldID == WI_WORLD_KEBARAS)
		strPath = MakePath(DIR_THEME, "MapLoading_Azria.png");
	else if (dwWorldID == WI_WORLD_CISLAND)
		strPath = MakePath(DIR_THEME, "MapLoading_Cisland.png");
	else if (dwWorldID == WI_WORLD_COLOSSEUM)
		strPath = MakePath(DIR_THEME, "MapLoading_Colosseum.png");
	else if (dwWorldID == WI_INSTANCE_BEHAMAH)
		strPath = MakePath(DIR_THEME, "MapLoading_Behemoth.png");
	else if (dwWorldID >= WI_WORLD_HEAVEN01 && dwWorldID <= WI_WORLD_HEAVEN05)
		strPath = MakePath(DIR_THEME, "MapLoading_Heaven.png");
	else if (dwWorldID == WI_DUNGEON_SA_TA)
		strPath = MakePath(DIR_THEME, "MapLoading_Ivillis.png");
	else if (dwWorldID == WI_INSTANCE_KALGAS)
		strPath = MakePath(DIR_THEME, "MapLoading_Kalgas.png");
	else if (dwWorldID == WI_DUNGEON_FL_MAS)
		strPath = MakePath(DIR_THEME, "MapLoading_Mas.png");
	else if (dwWorldID == WI_WORLD_RARTESIA)
		strPath = MakePath(DIR_THEME, "MapLoading_Rartesia.png");
	else if (dwWorldID >= WI_INSTANCE_RUSTIA && dwWorldID <= WI_INSTANCE_RUSTIA_1)
		strPath = MakePath(DIR_THEME, "MapLoading_Wilds.png");
	else if (dwWorldID == WI_DUNGEON_VOLCANE || dwWorldID == WI_DUNGEON_VOLCANERED)
		strPath = MakePath(DIR_THEME, "MapLoading_Volkane.png");
	else if (dwWorldID == WI_INSTANCE_HERNEOS || dwWorldID == WI_INSTANCE_HERNEOS_1)
		strPath = MakePath(DIR_THEME, "MapLoading_Herneros.png");
	else if (dwWorldID == WI_INSTANCE_SANPRES || dwWorldID == WI_INSTANCE_SANPRES_1)
		strPath = MakePath(DIR_THEME, "MapLoading_Sanpres.png");
	else if (dwWorldID == WI_INSTANCE_UPRESIA || dwWorldID == WI_INSTANCE_UPRESIA_1)
		strPath = MakePath(DIR_THEME, "MapLoading_Euphresia.png");
	else if (dwWorldID == WI_WORLD_DARKRARTESIA)
		strPath = MakePath(DIR_THEME, "MapLoading_DarkRartesia.png");
	else if (dwWorldID == WI_WORLD_MARKET)
		strPath = MakePath(DIR_THEME, "MapLoading_Market.png");
	else if (dwWorldID == WI_WORLD_FWC)
		strPath = MakePath(DIR_THEME, "MapLoading_Arena.png");
	else if (dwWorldID == WI_INSTANCE_CONTAMINTRAILS)
		strPath = MakePath(DIR_THEME, "MapLoading_RisisContamin.png");
	else if (dwWorldID == WI_INSTANCE_OPHIDIAN)
		strPath = MakePath(DIR_THEME, "MapLoading_Ophidian.png");
	else if (dwWorldID == WI_WORLD_MIKSIKPVP)
		strPath = MakePath(DIR_THEME, "MapLoading_Tournemant.png");
	else if (dwWorldID == WI_DUNGEON_CLOCKWORK)
		strPath = MakePath(DIR_THEME, "MapLoading_Clockwork.png");
	else if (dwWorldID == WI_DUNGEON_DA_DK)
		strPath = MakePath(DIR_THEME, "MapLoading_DekaneMine.png");
	else if (dwWorldID == WI_INSTANCE_FORTERESS)
		strPath = MakePath(DIR_THEME, "MapLoading_Forteress.png");
	...
	g_Neuz.m_TexLoading.LoadTexture(g_Neuz.m_pd3dDevice, strPath, COLOR_ITEM_BG, FALSE);
	...
	if (SUCCEEDED(g_Neuz.m_pd3dDevice->BeginScene()))
	{
		...
		else if (g_Option.m_nResWidth == 1360 && g_Option.m_nResHeight == 768)
		{
			...
			g_Neuz.m_TexLoading.m_size = CSize(1360, FULLSCREEN_HEIGHT);
		...
		{
			g_Neuz.m_TexLoading.m_size = CSize(1920, FULLSCREEN_HEIGHT);
		...
		{
			g_Neuz.m_TexLoading.m_size = CSize(2560, FULLSCREEN_HEIGHT);
...
void CDPClient::OnTrade(OBJID objid, CAr& ar)
{
	...
	if (IsValidObj((CObj*)pTrader))
	{
		...
		for (int i = 0; i < MAX_INVENTORY; i++)
		{
			...
			if (pItemBase)
			{
				//				CTexture* pTexture = CWndItemCtrl::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pItemBase->GetProp()->szIcon ), COLOR_ITEM_BG );
...
void CDPClient::OnUpdateItem(OBJID objid, CAr& ar)
{
	...
	if (cParam == UI_COOLTIME)
	{
		...
		if (pMover == pPlayer)
		{
			...
			if (nBag != -1)
				pItemBase = pPlayer->GetItemId2(nBag, nId);
...
#ifdef __FL_BAG_UPDATE
	if (nBag != -1)
		pMover->UpdateItemBag(nId, cParam, dwValue, nBag, dwTime);
	else
...
void CDPClient::OnSetExperience(OBJID objid, CAr& ar)
{
	...
	if (IsValidObj((CObj*)pMover))
	{
		...
		if (nSP < nSkillPoint)
		{
			...
			{
#ifdef __fmod
				SoundSystemClass::playMusic(BGM_IN_LEVELUP, false);
#else
...
#endif //__CSC_VER9_1
			if (g_pPlayer != NULL && pSkillTree && nSP != nSkillPoint)
		...
		if (g_pPlayer->m_aQuest[i].m_wId != 0xffff && g_pPlayer->m_aQuest[i].m_wId != 0xff)
		{
			if (g_pPlayer->m_aQuest[i].m_nState != 14)
			{
				...
				if (pQestProp)
				{
					...
					//g_WndMng.PutString( strMessage, NULL, COLOR_ERROR );
				}
			}
		}
	}
...
#ifdef __WELCOME_MESSAGE
	CString Name = g_pPlayer->GetName();
	g_WndMng.PutString("Welcome to Titanium MMO, " + Name + "!", NULL, prj.GetTextColor(TID_ADMIN_ANNOUNCE), CHATSTY_SYSTEM);
	g_Caption1.AddCaption("Welcome to Titanium MMO, " + Name + "!", g_WndMng.m_pWndWorld->m_pFontAPICaption);
#endif //__WELCOME_MESSAGE
...
void CDPClient::OnTaskBar(CAr& ar)
{
	...
	{
		CWndTaskBar* tmpTaskbar = new CWndTaskBar;
		tmpTaskbar->Initialize();
		tmpTaskbar->Serialize(ar);
		tmpTaskbar->Destroy();
		g_WndMng.Process();
		SAFE_DELETE(tmpTaskbar);
	}
#else
	else
	{ 
		...
		for (int i = 0; i < nCount; ++i)
		{
			ar.skip(sizeof(int) + sizeof(unsigned long) + sizeof(unsigned long));
			unsigned long type = ar.get<unsigned long>(); 
			ar.skip(sizeof(unsigned long) + sizeof(unsigned long) + sizeof(unsigned long long));

			if (type == SHORTCUT_CHAT)
			{
				unsigned long stringLen = ar.get<int>();
				if (stringLen > MAX_SHORTCUT_STRING || stringLen < 0)
					ar.skip(sizeof(char) * MAX_SHORTCUT_STRING);
				else
					ar.skip(sizeof(char) * stringLen);
		...
		for (int i = 0; i < nCount; i++)
		{
			ar.skip(sizeof(int) + sizeof(int) + sizeof(unsigned long) + sizeof(unsigned long));
			unsigned long type = ar.get<unsigned long>();
			ar.skip(sizeof(unsigned long) + sizeof(unsigned long) + sizeof(unsigned long long));
		...
		ar >> nCount;
		ar.skip((nCount * (sizeof(int) + sizeof(unsigned long long) + (sizeof(unsigned long) * 5))) + sizeof(int));
	}
#endif
}
...
void CDPClient::OnErrorParty(CAr& ar)
{
	...
	case ERROR_NOTTARGET:
	{
		...
		//g_WndMng.PutString( "타겟이 안잡혀 있거나 몬스터가 아니어서 사용할수 없습니다.", NULL, COLOR_ERROR );
	}
	break;
...
#ifdef __fmod
			SoundSystemClass::playMusic(BGM_IN_FITUP, false);
#else
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
		CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_JOINPARTY, 1, AEGACH_PROG_CMP);
#endif // __AEGON_ACHIEVEMENT_SYSTEM


#ifdef __AEGON_ACHIEVEMENT_SYSTEM
		if(g_Party.IsLeader(g_pPlayer->m_idPlayer))
			CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_PTLEADER, 1, AEGACH_PROG_CMP);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
		CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_PARTYLEVEL, nLevel+1, AEGACH_TVAR_CMP_BIGGER);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
	}
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	if(g_pPlayer->m_idPlayer == uidChangeLeader)
		CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_PTLEADER, 1, AEGACH_PROG_CMP);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
}
...
void CDPClient::OnSetPartyMode(CAr& ar)
{
	...
	if (pItemProp)
	{
		if (bOnOff)
		{
			...
			pWndWorld->m_buffs.AddBuff(BUFF_SKILL, pItemProp->dwID, 1, dwSkillTime);
#else
...
void CDPClient::OnPartyChat(CAr& ar)
{
	...
	if (pMover && pMover->IsPlayer()) 
	{
		if (pMover->IsAuthHigher(AUTH_HELPER))
		{
			str.Format("[Staff] [%s] %s", lpName, lpString);
		}
		else if (pMover->IsPremium() && pMover->m_bShowPremium)
		{
			str.Format("[Premium] [%s] %s", lpName, lpString);
		}
		else
		{
	...
	if (IsValidObj(pMover))
	{
		...
		if (pMover->IsPlayer())
		{
			if (prj.m_setBlockedUserID.find(lpName) != prj.m_setBlockedUserID.end())
				return;
		}
#endif
...
void CDPClient::OnPartyMapInfo(CAr& ar)
{
	...
	if (nPartyIndex < 0 || nPartyIndex >= MAX_PTMEMBER_SIZE)
		return;


	g_Party.SetPos(nPartyIndex, vPos);
	g_Party.SetWorld(nPartyIndex, dwWorldID);
...
void CDPClient::OnGameRate(CAr& ar)
{
	...
	case GAME_RATE_PIECE_ITEMDROP:
	{
		prj.m_fItemPieceDropRate = fRate;
	}
	break;
...
#ifdef __AEGON_FIXES
		default:
			Error("CDPClient::OnGuildCombat unknown nState: %d", nState);
			break;
#endif
	}
}
...
void CDPClient::OnGuildCombatState(CAr& ar)
{
	...
	if (pWndWorld && pWorld && pWorld->GetID() == WI_WORLD_GUILDWAR)
	{
		...
		{
#ifdef __fmod
			SoundSystemClass::playMusic(BGM_BA_CRISIS, false);
#else
...

	{
		...
		case CGuildCombat::ENTER_STATE:
		{
			...
			if (pGuild && g_GuildCombatMng.IsRequest())
			{
				...
				if (pGuildMemberl && (pGuildMemberl->m_nMemberLv == GUD_MASTER || pGuildMemberl->m_nMemberLv == GUD_KINGPIN || pGuild->IsCmdCap(pGuildMemberl->m_nMemberLv, PF_GUILDCOMBAT)))
...
#ifdef __PARTY_ASSISTANT
void CDPClient::SendSetPartyAssistant(u_long uLeaderId, u_long uAssist)
{
	BEFORESENDSOLE(ar, PACKETTYPE_ASSISTANT, DPID_UNKNOWN);
	ar << uLeaderId;
	ar << uAssist;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __LEESE_SECURITY_CHECKS
	if ((OBJMSG)dwAtkMsg == OBJMSG_ATK1 || (OBJMSG)dwAtkMsg == OBJMSG_ATK2 || (OBJMSG)dwAtkMsg == OBJMSG_ATK3 || (OBJMSG)dwAtkMsg == OBJMSG_ATK4)
	{
		SEND(ar, this, DPID_SERVERPLAYER);
	}
#else //__LEESE_SECURITY_CHECKS
	SEND(ar, this, DPID_SERVERPLAYER);
#endif //__LEESE_SECURITY_CHECKS
}
...
#ifdef __LEESE_SECURITY_CHECKS
	if ((OBJMSG)dwAtkMsg != OBJMSG_ATK1) return;
#endif //__LEESE_SECURITY_CHECKS
...
#ifdef __LEESE_SECURITY_CHECKS
	if ((OBJMSG)dwAtkMsg != OBJMSG_ATK_MAGIC1) return;
#endif //__LEESE_SECURITY_CHECKS
...
#ifdef __LEESE_SECURITY_CHECKS
	if ((OBJMSG)dwAtkMsg != OBJMSG_ATK_RANGE1) return;
#endif //__LEESE_SECURITY_CHECKS
...
#ifdef __RANGE_FIX
void CDPClient::SendUseSkill(WORD wType, WORD wId, OBJID objid, DWORD dwRange, float fRange, int nUseSkill, BOOL bControl, int nActionSlot)
#else //__RANGE_FIX
void CDPClient::SendUseSkill(WORD wType, WORD wId, OBJID objid, int nUseSkill, BOOL bControl, int nActionSlot)
#endif //__RANGE_FIX
...
#ifdef __STOP_EXP
void CDPClient::SendSetExpStop(BOOL State)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SETEXPSTOP, DPID_UNKNOWN);
	ar << State;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__STOP_EXP
...
#ifdef __FL_BAG_UPDATE
void CDPClient::SendDoUseItemBag(DWORD dwItemId, OBJID objid, int nBag)
{
	CMover* pPlayer = CMover::GetActiveMover();
	if (!pPlayer)
		return;
	...
	CItemBase* pItemBase = pPlayer->GetItemId2(nBag, dwId);
	if (!pItemBase)
		return;
	...
	BEFORESENDSOLE(ar, PACKETTYPE_DOUSEITEM_BAG, DPID_UNKNOWN);
	ar << dwItemId << objid << nBag;
	SEND(ar, this, DPID_SERVERPLAYER);
	g_nItemDelay = 60;
}
#endif //__FL_BAG_UPDATE
...
void CDPClient::SendDoUseItem(DWORD dwItemId, OBJID objid, int nPart, BOOL bResult)
{
...
#ifdef __RESTRICT_ITEM_USAGE
	CWorld* pWorld = g_pPlayer->GetWorld();
	if (pItemProp->dwID == II_CHR_FOO_COO_REMANTIS || pItemProp->dwID == II_CHR_FOO_COO_BLUEREMANTIS)
	{
		if (pWorld && pWorld->GetID() == WI_WORLD_GUILDWAR || pWorld->GetID() == WI_WORLD_ARENA || pWorld->GetID() == WI_WORLD_FWC
#ifdef __AEGON_TOURNAMENT
			|| pWorld->GetID() == AT_TOURNAMENT_MAP
#endif // __AEGON_TOURNAMENT
			)
		{
		}
		...
		{
			CString str;
			str.Format("You cannot use %s in this area.", pItemProp->szName);
			g_WndMng.PutString(str, nullptr, COLOR_ERROR);
			return;
		}
	}
#endif //__RESTRICT_ITEM_USAGE

#ifdef __AEGON_USEFULL_FUNCTIONS
	if (pItemProp->dwItemKind2 == IK2_AEGON) 
	{
		switch (pItemProp->dwItemKind3) 
		{
		case IK3_PERM_BUFF:
		{
			if (easy_find(g_pPlayer->m_vPermanenetBuffs, pItemProp->dwID)) 
			{
				g_WndMng.PutString("You already have that permanent buff.", 0, COLOR_ERROR);
				return;
			}
			break;
		}
		case IK3_UL_MOTION:
		{
			if (easy_find(g_pPlayer->m_vUnlockedMotions, pItemProp->dwID))
			{
				g_WndMng.PutString("You already unlocked this motion.", 0, COLOR_ERROR);
				return;
			}
			break;
		}
...
#endif // __AEGON_USEFULL_FUNCTIONS

#ifdef __SYS_LOCK_ITEM
	CItemElem* pItemElem = (CItemElem*)pItemBase;
	if (pItemElem && pItemElem->IsItemLocked() && pItemProp->dwParts == NULL_ID && pItemElem->IsEatPet() == FALSE && pItemElem->IsPet() == FALSE)
	{
		CString str;
		str.Format("Please unlock %s before using.", pItemProp->szName);
		g_WndMng.PutString(str, NULL, COLOR_WARNING);
		return;
	}
	...
	{
#ifdef __GAMEGUARD
		if ((pWndChangeWeapon->m_nWeaponType != WEAPON_UNIQUE && pWndChangeWeapon->m_nWeaponType != WEAPON_GENERAL) || pItemProp->dwID != II_SYS_SYS_SCR_SMELPROT3)
#else // __GAMEGUARD
	...
	if (pItemProp->dwID == II_SYS_SYS_SCR_GCHANAM)
	{
		...
		g_WndMng.m_pWndGuildName->SetData((BYTE)(pItemBase->m_dwObjId));
#else
		CWndGuildName* pWndGuildName = (CWndGuildName*)g_WndMng.GetWndBase(APP_GUILDNAME);
		if (!pWndGuildName)
		{
...
#ifdef __SYS_CUSTOM_TITLE
	if (pItemProp->dwID == II_SYS_SCR_CUSTOMTITLE)
	{
		if (g_WndMng.m_pCreateOwnTitle)
		{
			g_WndMng.m_pCreateOwnTitle->Destroy();
			SAFE_DELETE(g_WndMng.m_pCreateOwnTitle);
		}
		g_WndMng.m_pCreateOwnTitle = new CWndCreateOwnTitle();
		g_WndMng.m_pCreateOwnTitle->Initialize();
		return;
	}
	if (pItemProp->dwID == II_SYS_SCR_REMOVE_CUSTOM)
	{
		SendCreateOwnTitle(CString("$"), TRUE);
		return;
	}
#endif // __SYS_CUSTOM_TITLE
	...
	if (bResult)
	{
		...
		case II_SYS_SYS_SCR_PET_TAMER_MISTAKE:
			SAFE_DELETE(g_WndMng.m_pWndPetMistakeCustom);
			g_WndMng.m_pWndPetMistakeCustom = new CWndPetMistakeCustom;
			g_WndMng.m_pWndPetMistakeCustom->Initialize(&g_WndMng);
			g_WndMng.m_pWndPetMistakeCustom->SetItem(dwId);
			return;
#else //__AEGON_PETMISTAKE
		case II_SYS_SYS_SCR_PET_TAMER_MISTAKE:
			SAFE_DELETE(g_WndMng.m_pWndCommItemDlg);
			g_WndMng.m_pWndCommItemDlg = new CWndCommItemDlg;
			g_WndMng.m_pWndCommItemDlg->Initialize(&g_WndMng, APP_COMMITEM_DIALOG);
...
#ifdef __SYS_GUILDCOLOR
	if (pItemProp->dwID == II_SYS_SYS_SCR_GUILDCOLOR)
	{
		if (!g_pPlayer->GetGuild() || !g_GuildMng.GetGuild(g_pPlayer->GetGuild()->GetGuildId())->IsMaster(g_pPlayer->m_idPlayer))
		{
			g_WndMng.PutString(prj.GetText(TID_GAME_GUILDCOMBAT_NOT_GUILD_LEADER), nullptr, COLOR_ERROR);
			return;
		}

		CWndGuildColor* pWndGuildColor = (CWndGuildColor*)g_WndMng.GetWndBase(APP_GUILD_COLORCHANGE);
		if (!pWndGuildColor)
		{
			pWndGuildColor = new CWndGuildColor;
			pWndGuildColor->Initialize(&g_WndMng);
		}
	}
#endif //__SYS_GUILDCOLOR

#ifdef __SYS_NAMECOLOR
	if (pItemProp->dwID == II_SYS_SYS_SCR_NAMECOLOR)
	{
		CWndNameColor* pWndNameColor = (CWndNameColor*)g_WndMng.GetWndBase(APP_NAME_COLORCHANGE);
		if (!pWndNameColor)
		{
			pWndNameColor = new CWndNameColor;
			pWndNameColor->Initialize(&g_WndMng);
		}
	}
#endif //__SYS_NAMECOLOR
	...
	if (pItemProp->dwItemKind3 == IK3_VENDING)
	{
		if (bCreateNPC == FALSE && g_Neuz.m_pVendingNpcItem == NULL)
		{
#ifdef __SHOPS_ONLY_CONT
			BYTE UserContinent = CContinent::GetInstance()->GetTown(g_pPlayer->GetPos());
			if (UserContinent) // Town Check
			{
				if (UserContinent != TOWN_DARKEN)
				{
					g_WndMng.PutString("You're not allowed to create a offline vendor here.", NULL, COLOR_WARNING);
					g_WndMng.PutString("Please press (V) and move to Darken City.", NULL, COLOR_WARNING);
					return;
				}
			}
			else // No Town Found
			...
			CWndVendor* pWndVendor = (CWndVendor*)g_WndMng.GetWndVendorBase();
			if (!pWndVendor)
				pWndVendor = (CWndVendor*)g_WndMng.CreateApplet(APP_VENDOR_REVISION);
			if (pWndVendor)
				pWndVendor->m_pVendingNpcItem = (CItemElem*)pItemBase;
...
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
		if (!g_WndMng.m_pWndSummonParty) 
		{
			g_WndMng.m_pWndSummonParty = new CWndSummonParty;
			g_WndMng.m_pWndSummonParty->Initialize(&g_WndMng);
		}
		g_WndMng.m_pWndSummonParty->SetData((WORD)(pItemBase->m_dwObjId), 0);
#else
...
void CDPClient::SendSkillTaskBar(void)
{
	...
	{
		for (int j = 0; j < MAX_SLOT_QUEUE; j++)
		{
			ar << (BYTE)i << (BYTE)j << g_WndMng.m_pWndTaskBar->m_aSlotQueue[i][j].m_dwShortcut << g_WndMng.m_pWndTaskBar->m_aSlotQueue[i][j].m_dwId << g_WndMng.m_pWndTaskBar->m_aSlotQueue[i][j].m_dwType;
			ar << g_WndMng.m_pWndTaskBar->m_aSlotQueue[i][j].m_dwIndex << g_WndMng.m_pWndTaskBar->m_aSlotQueue[i][j].m_dwUserId << g_WndMng.m_pWndTaskBar->m_aSlotQueue[i][j].m_dwData;
		}
	}
	nCount = MAX_SLOT_QUEUE * MAX_ACTION_SLOT;
#else // __ACTION_SLOT_PAGES
...
#ifdef __FL_BAG_LOOT
void CDPClient::SendSelectedBag(int nSelect)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SELECTED_BAG, DPID_UNKNOWN);
	ar << nSelect;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__FL_BAG_LOOT
...
{
	m_ss.uFrameMove++;
#ifdef __OPTIMIZE_LATENCY
	if (fUnconditional || m_ss.uFrameMove % 15 == 0)
#else //__OPTIMIZE_LATENCY
	if (fUnconditional || m_ss.uFrameMove % 30 == 0)
#endif //__OPTIMIZE_LATENCY
	{
#if __VER >= 8	// __JEFF_VER_8
...
void CDPClient::SendCreateAngel(int nSuns, int nMoons)
{
...
void CDPClient::OnShout(CAr& ar)
{
...
#ifdef __YS_CHATTING_BLOCKING_SYSTEM
	if (prj.m_setBlockedUserID.find(lpszPlayer) != prj.m_setBlockedUserID.end()) { return; }
#endif
	...
	if (g_Option.m_bShout)
	{
...
#ifdef __AUTH_SHOUT
		if (dwAuth >= AUTH_HELPER)
		{
			str.Format("[Staff] [%s] %s", lpszPlayer, lpString);
		}
		else
#endif //__AUTH_SHOUT
#ifdef __PREMIUM
		if (bPremium)
		{
			str.Format("[Premium] [%s] %s", lpszPlayer, lpString);
		}
		else
		{
...
void CDPClient::OnPlayMusic(CAr& ar)
{
	...
	TRACE("OnPlayMusic, %d\n", idmusic);
#ifdef __fmod
	SoundSystemClass::playMusic(idmusic, true);
#else
	PlayMusic(idmusic, 1);
#endif
}
...
void CDPClient::OnPlaySound(CAr& ar)
{
	...
	if (nType == 0)
	{
		ar >> idsound;
#ifdef __fmod
		SoundSystemClass::playSound(idsound);
#else
		PLAYSND(idsound);
#endif
	}
...
#ifdef __YS_CHATTING_BLOCKING_SYSTEM
	if ((pFriend && pFriend->bBlock) || (prj.m_setBlockedUserID.find(sPlayerFrom) != prj.m_setBlockedUserID.end()))
#else
...
void CDPClient::OnSay(CAr& ar)
{
	...
	Friend* pFriend = g_WndMng.m_RTMessenger.GetFriend(idPlayer);
#ifdef __YS_CHATTING_BLOCKING_SYSTEM
	if (prj.m_setBlockedUserID.find(sPlayerFrom) != prj.m_setBlockedUserID.end()) { return; }
#endif
	...
	if (strcmp(sPlayerFrom, g_pPlayer->GetName()) != 0 && strlen(lpString) == 0)
		return;
	...
	CWndAegonMessenger* pWndMessenger = g_WndMng.OpenMessenger(bMinimized);
#endif
	...
	if (strcmp(sPlayerFrom, g_pPlayer->GetName()) == 0)
	{
		...
		if (pWndMessenger)
		{
			pWndMessenger->OnChatMessage(strFrom, strTo, lpString, nSearch == 1 ? __AEG_MESSAGE_TYPE_OFFLINE : __AEG_MESSAGE_TYPE_REG);
			pWndMessenger->SwitchToChat(strTo);
		}
		else 
		{
			char szText[512];
			sprintf_s(szText, 512, "OnSay: pWndMessage sPlayerFrom equals g_pPlayer %s %s %s", strFrom.GetString(), strTo.GetString(), lpString);
			Error(szText);
		}
	...
	else
	{
		...
		if (pWndMessenger) 
		{
			pWndMessenger->OnChatMessage(strFrom, strTo, lpString);
		}
#else
		CWndMessage* pWndMessage = g_WndMng.GetMessage(strFrom);
		if (pWndMessage)
...
void CDPClient::OnGetPlayerAddr(CAr& ar)
{
	...
	g_WndMng.PutString(str, NULL, COLOR_SUCCESS);
	// ip addr
}

#ifdef __AEGON_COUNT_DIFF_IP
void CDPClient::OnGetDiffPlayer(CAr& ar)
{
	int nCount;
	ar >> nCount;

	TRACE("OnGetDiffPlayer, %d\n", nCount);
	CString str;
	str.Format("[DIFFERENT IPs] %d", nCount);
	g_WndMng.PutString(str, NULL, COLOR_SUCCESS);
	// different ip count
}
#endif // __AEGON_COUNT_DIFF_IP
...
void CDPClient::OnSystem(CAr& ar)
{
	...
	{
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
		g_Caption1.AddCaption(str, pWndWorld->m_pFontAPITitle, FALSE, D3DCOLOR_ARGB(255, 255, 255, 153), nImage, nFont);
...
void CDPClient::OnModifyMode(OBJID objid, CAr& ar)
{
	...
	if (IsValidObj((CObj*)pPlayer)) {
		...
		if (pPlayer->IsActiveMover() && g_pPlayer->IsAuthHigher(AUTH_GAMEMASTER)) // 여기서 나면 출력
		{
			...
			if (pPlayer->IsMode(AEGON_MOB_IGNORE_MODE)) 
			{
				str += "MobIgnore||";
			}
#endif
#ifdef __AEGON_NINJA_MODE
			if (pPlayer->IsMode(AEGON_NINJA_MODE))
			{
				str += "Ninja||";
			}
#endif
...
void CDPClient::OnReturnSay(OBJID objid, CAr& ar)
{
	...
	{
		if (prj.m_setBlockedUserID.find(pPlayer->GetName()) != prj.m_setBlockedUserID.end()) { return; }
	}
#endif
...
#ifdef __SYS_GUILDCOLOR
	pGuild->m_adwGuildColor[0] = 255;
	pGuild->m_adwGuildColor[1] = 255;
	pGuild->m_adwGuildColor[2] = 255;
#endif //__SYS_GUILDCOLOR
...
void CDPClient::OnDestroyGuild(CAr& ar)
{
	...
	if (pGuild)
	{
		...
		if (pWndGuild)
		{
			...
			pWndGuild->m_WndGuildBuffs.UpdateData();
	...
	if (IsValidObj(pMover))
	{
		...
		if (g_pPlayer && g_pPlayer->GetId() == objid)
		{
			...
			else
			{
				...
				{
#ifdef __fmod
					SoundSystemClass::playMusic(BGM_IN_COMPANY, false);
#else
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
					CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_JOINGUILD, 1, AEGACH_PROG_CMP);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
				}
			}
			//			CWndX
...
void CDPClient::OnGuildChat(CAr& ar)
{
	...
	if (pMover && pMover->IsPlayer()) 
	{
		if (pMover->IsAuthHigher(AUTH_HELPER))
		{
			str.Format("[Staff] [%s] %s", sPlayer, sChat);
		}
		else if (pMover->IsPremium() && pMover->m_bShowPremium)
		{
			str.Format("[Premium] [%s] %s", sPlayer, sChat);
		}
		else
		{
...
void CDPClient::OnGuildInvite(CAr& ar)
{
	...
	if (g_Option.m_bBlockGuildInvites)
		return;
...
{
#ifndef __MESSENGER_CTRL_EXTENTED
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(vecPlayer.size());
#else
	ar << vecPlayer.size();
#endif
...
#ifdef __VENDING_NPC
void CDPClient::SendPVendorClose(OBJID objidVendor, BOOL bClearShop)
#else // __VENDING_NPC
...
#ifdef __CHIP_PLAYERSHOP
void CDPClient::SendRegisterPVendorItem(BYTE iIndex, BYTE nType, BYTE nId, short nNum, int nCost, int nMode)
...
#ifdef __VENDING_NPC
	pMover->m_vtInfo.m_tmOpened = tmpOpened;
#endif // __VENDING_NPC
...
#ifdef __CHIP_PLAYERSHOP
	g_Neuz.m_aSavedInven[iIndex].m_nBuyMode = 0;
#endif // __CHIP_PLAYERSHOP
}
...
void CDPClient::OnRegisterPVendorItem(OBJID objid, CAr& ar)
{
	...
	{
#ifdef __CHIP_PLAYERSHOP
		pMover->m_vtInfo.VendorSetItem(nId, iIndex, nNum, nCost, nBuyMode);
#else // __CHIP_PLAYERSHOP
...
#ifdef __CHIP_PLAYERSHOP
					pWndVendor->m_pwndVenderMessage->WriteBuyInfo(sBuyer, strItem, nNum, nTotal, pItemElem);
#else //__CHIP_PLAYERSHOP
...
#ifdef __CHIP_PLAYERSHOP
	g_Neuz.m_aSavedInven[nItem].m_nBuyMode = 0;
#endif //__CHIP_PLAYERSHOP
}
...
#ifdef __VENDING_NPC
		g_Neuz.m_mapVisitedShops[pPVendor->GetId()] = pPVendor->m_vtInfo.m_tmOpened;
#endif // __VENDING_NPC
...
void CDPClient::OnSetSkillState(CAr& ar)
{
	...
	if (wType == BUFF_ITEM2)
	{
		__int64 t;
		ar >> t;
		dwLevel = static_cast<__int64>(time_null()) + t;
	}
	else
		ar >> dwLevel;
...
#ifdef __INCREASE_SKILL_ID_VALUE
#ifdef __ANARCHY_SYSTEM
		bOverwrite = bOverwrite && pTarget->HasBuff(wType, dwID);
		pTarget->AddBuff(wType, dwID, dwLevel, dwTime, NULL_ID, bOverwrite);
#else // __ANARCHY_SYSTEM
		pTarget->AddBuff(wType, wID, dwLevel, dwTime);
#endif // __ANARCHY_SYSTEM
#else
#ifdef __ANARCHY_SYSTEM
		bOverwrite = bOverwrite && pTarget->HasBuff(wType, wID);
		pTarget->AddBuff(wType, wID, dwLevel, dwTime, NULL_ID, bOverwrite);
#else // __ANARCHY_SYSTEM
		pTarget->AddBuff(wType, wID, dwLevel, dwTime);
#endif // __ANARCHY_SYSTEM
#endif
...
void CDPClient::OnTag(OBJID objid, CAr& ar)
{
	...
	CWndAegonMessenger* pWndMessenger = NULL;
	if(m_nCount > 0)
		pWndMessenger = g_WndMng.OpenMessenger(TRUE);

	char strTagName[65];
	DWORD dwtegDate;
	char strTagMessage[256];
	for (int i = 0; i < m_nCount; i++)
	{
		memset(strTagName, 0, 65);
		memset(strTagMessage, 0, 256);

		ar.ReadString(strTagName, 65);
		ar >> dwtegDate;
		ar.ReadString(strTagMessage, 256);

		if (pWndMessenger && IsValidObj(g_pPlayer))
			pWndMessenger->OnChatMessage(strTagName, g_pPlayer->GetName(), strTagMessage, __AEG_MESSAGE_TYPE_OFFLINE, dwtegDate);
	}

	if (m_nCount)
	{
		CString str;
		str.Format(prj.GetText(TID_GAME_MESSNEWNOTE), m_nCount);
...
void CDPClient::OnDoApplySkill(OBJID objid, CAr& ar)
{
	...
	DWORD dwSkill, dwLevel;
	ar >> idTarget >> dwSkill >> dwLevel;
#ifdef __AEGON_OPTIONAL_SKILL_SFX
	int nSfxMode;
	ar >> nSfxMode;
#endif
	...
	if (IsValidObj((CObj*)pCtrl))
	{
		...
		if (IsValidObj((CObj*)pCtrlTarget))
		{
			...
			{
				pCtrl->DoApplySkill(pCtrlTarget, pSkillProp, pAddSkillProp, false, 0, 0, 0, nSfxMode);
			}
		}
	}
}
...
#ifdef __WEAPON_RARITY
		ar >> aEquipInfoAdd[nParts].nWeaponRarity;
#endif // __WEAPON_RARITY
#ifdef __WEAPON_MERGE
		ar >> aEquipInfoAdd[nParts].iMergedRandomOptItemId;
		ar >> aEquipInfoAdd[nParts].nMergedWeaponATK;
#endif //__WEAPON_MERGE
	}
...
void CDPClient::OnPostMail(CAr& ar)
{
	...
	//	OutputDebugString( lpOutputString );
	g_WndMng.PutString(lpOutputString, NULL, COLOR_ERROR);
#endif	// _DEBUG
}
...
void CDPClient::OnSummonFriend(CAr& ar)
{
	...
	{
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
		if (!g_WndMng.m_pWndSummonFriend)
		{
...
void CDPClient::OnSummonFriendConfirm(CAr& ar)
{
	...
	{
#ifdef __AEGON_FULLSCREEN_AND_RESIZE		
		if (!g_WndMng.m_pWndSummonFriendConfirm)
		{
			g_WndMng.m_pWndSummonFriendConfirm = new CWndSummonFriendMsg;
			g_WndMng.m_pWndSummonFriendConfirm->Initialize(&g_WndMng);
		}
		g_WndMng.m_pWndSummonFriendConfirm->SetData(objid, dwData, szName, szWorldName);
#else
...
void CDPClient::OnSummonPartyConfirm(CAr& ar)
{
	...
	{
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
		if (!g_WndMng.m_pWndSummonPartyConfirm)
		{
			g_WndMng.m_pWndSummonPartyConfirm = new CWndSummonPartyMsg;
			g_WndMng.m_pWndSummonPartyConfirm->Initialize(&g_WndMng);
		}
		g_WndMng.m_pWndSummonPartyConfirm->SetData(objid, dwData, szWorldName);
#else
...
#ifdef __PARTY_ASSISTANT 
void CDPClient::OnAssistClient(CAr& ar)
{
	BOOL send;
	ar >> send;

	g_pPlayer->m_bAssistMember = send == TRUE ? TRUE : FALSE;

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_PTASSIST, 1, AEGACH_PROG_CMP);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
}
#endif //__PARTY_ASSISTANT
...
void CDPClient::SendUltimateRemoveGem(OBJID objItemWeapon, OBJID objItemGem, int nSlot)
...
#ifdef __AEGON_PETMISTAKE
			CWndPetMistakeCustom* pWndPetMistakeCustom = (CWndPetMistakeCustom*)g_WndMng.GetWndBase(APP_PET_MISTAKE);
			if (pWndPetMistakeCustom != NULL)
				pWndPetMistakeCustom->Destroy();
#endif //__AEGON_PETMISTAKE
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(vecMemberId.size());
#else
	ar << vecMemberId.size();
#endif
...
void CDPClient::OnHonorListAck(CAr& ar)
{
	...
	for (int i = 0; i < MAX_HONOR_TITLE; i++)
	{
		...
		CTitleManager::Instance()->SetValue(i, -1, nHonorTitle[i]);
...
#ifdef __WIKIPEDIA_TITLE
	pWndBase = g_WndMng.GetWndBase(APP_WIKIPEDIA_TITLE);
	if (pWndBase)
		((CWndHonorTitle*)pWndBase)->RefreshList();
#endif // __WIKIPEDIA_TITLE
...
#if (defined (__FL_BAG_LOOT) && defined (__DROP_LOG)) || (defined (__FL_BAG_LOOT) && defined (__FL_FARM_STAT))
	if (pPlayer && bAddDropLog)
	{
#ifdef __DROP_LOG
		ItemProp* pProp = item.GetProp();
		if (pProp)
			CWndDropLog::GetInstance()->Add(pProp);
#endif //__DROP_LOG
				...
				if (pProp)
				{
					...
					else
					{
						for (int i = 0; i < prj.m_vecCountItems.size(); i++)
						{
							if (prj.m_vecCountItems[i].pCountItems == pProp)
							{
								prj.m_vecCountItems[i].nCountItems += item.m_nItemNum;
								break;
							}
						}
					}
...
void CDPClient::SendNPCBuffClass(const char* szKey)
{
	BEFORESENDSOLE(ar, PACKETTYPE_NPC_BUFF_CLASS, DPID_UNKNOWN);
	ar.WriteString(szKey);
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(vecLineUpMember.size());
#else
...
void CDPClient::SendTeleportToHeavenTower(int nUnitID)
{
...
void CDPClient::OnActivateVisPet(CAr& ar)
{
	...
	if (bRegularEatPet) 
	{
		...
		g_pPlayer->SetEatPetItem(objIdVisPetItem);
		return;
	}
...
#ifdef __SYS_LOCK_ITEM
void CDPClient::SendLockItem(const DWORD objId, bool bIsLocked)
{
...
void CDPClient::SendWikiCreateItem(CItemElem* pItemElem, LPCTSTR szName)
{
	BEFORESENDSOLE(ar, PACKETTYPE_WIKICI, DPID_UNKNOWN);
	pItemElem->Serialize(ar);
	ar.WriteString(szName);
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __AEGON_PETMISTAKE
void CDPClient::SendPetTamerMistakeCustom(DWORD dwId)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PET_TAMER_MISTAKE_CUSTOM, DPID_UNKNOWN);
	ar << dwId;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__AEGON_PETMISTAKE
...
#ifdef __AEGON_MONSTER_HUNT
void CDPClient::SendLoadMonsterHuntProgress()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDLOADMHPROGRESS, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnLoadMHProgress(CAr& ar)
{
	g_pPlayer->m_MHProgress.ReadMap(ar);
}
#endif // __AEGON_MONSTER_HUNT

#ifdef __LEEAEG_SHOP_FINDER
void CDPClient::SendShopItemTele(DWORD itemIndex)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SEND_SHOP_ITEM_TELE, DPID_UNKNOWN);
	ar << itemIndex;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::SendShopItemRefresh()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SEND_SHOP_ITEM_REFRESH, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnShopFinderRefresh(CAr& ar)
{
	CAegonShopFinderMng::GetInstance()->ReadMap(ar);
	CWndShopFinder* tmpWnd = (CWndShopFinder*)g_WndMng.GetWndBase(APP_SHOP_FINDER);

	if (tmpWnd)
		tmpWnd->RefreshItemMap();
}
#endif // __LEEAEG_SHOP_FINDER

#ifdef __GIFTBOX_LIST_ITEMS
void CDPClient::SendQueryGBPreview(DWORD dwID)
{
	BEFORESENDSOLE(ar, PACKETTYPE_GIFTBOX_PREVIEW, DPID_UNKNOWN);
	ar << dwID;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnGBPreview(CAr& ar)
{
	if (!g_WndMng.m_pWndGiftbox)
	{
		g_WndMng.m_pWndGiftbox = new CWndGiftbox;
		if (g_WndMng.m_pWndGiftbox->Serialize(ar))
		{
			g_WndMng.m_pWndGiftbox->Initialize(nullptr);
		}
	}
	else {
		g_WndMng.m_pWndGiftbox->Serialize(ar);
	}
}
#endif // __GIFTBOX_LIST_ITEMS

#ifdef __AEGON_RETURN_AWAKE
void CDPClient::SendReturnAwakening(DWORD dwItem, DWORD dwMaterial)
{
	BEFORESENDSOLE(ar, PACKETTYPE_RETURN_AWAKENING, DPID_UNKNOWN);
...
#ifdef	__VENDING_NPC
void CDPClient::SendCreateVendor(DWORD dwItemId, D3DXVECTOR3 vPos, FLOAT fAngle, char* szVendingTitle)
{

	BEFORESENDSOLE(ar, PACKETTYPE_CREATEVENDOR, DPID_UNKNOWN);
	ar << dwItemId;
	ar << vPos;
	ar << fAngle;
	ar.WriteString(szVendingTitle);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::SendUnlinkVendorNpc()
{
	BEFORESENDSOLE(ar, PACKETTYPE_UNLINKNPCVENDOR, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnVendorNPCLink(CAr& ar)
{
	DWORD dwVendorTime;
	DWORD dwVendorItem;
	ar >> dwVendorTime >> dwVendorItem;
#ifdef __INCREASE_SKILL_ID_VALUE
	g_pPlayer->m_buffs.AddBuff(BUFF_ITEM, dwVendorItem, 1, dwVendorTime * 1000);
#else
	g_pPlayer->m_buffs.AddBuff(BUFF_ITEM, (WORD)(dwVendorItem), 1, dwVendorTime * 1000);
#endif
}
void CDPClient::OnVendorNPCUnlink(CAr& ar)
{
	DWORD dwVendorItem;
	ar >> dwVendorItem;

#ifdef __INCREASE_SKILL_ID_VALUE
	g_pPlayer->m_buffs.RemoveBuff(BUFF_ITEM, dwVendorItem);
#else
	g_pPlayer->m_buffs.RemoveBuff(BUFF_ITEM, (WORD)(dwVendorItem));
#endif
}
#endif // __VENDING_NPC

#ifdef __AEGON_CONVERT_PERIN
void CDPClient::SendAegonConvertPerin()
{
	SendHdr(PACKETTYPE_AEGON_CONVERT_PERIN);
}
...
void CDPClient::OnReapGold(CAr& ar)
{
	...
	CWndFarmCounter* pWndFC = (CWndFarmCounter*)g_WndMng.GetWndBase(APP_FARM_COUNTER);
	if (bAddCount) // If its from monsters
	{
		if (g_pPlayer && nGold >= 1 && nGold <= INT_MAX)
			CWndDropLog::GetInstance()->AddGold(g_pPlayer->GetGold() >= INT_MAX ? 0 : nGold);

		if (pWndFC && pWndFC->dwStartTime != NULL)
		{
			//if (g_pPlayer && nGold >= 1 && nGold <= INT_MAX && g_pPlayer->GetGold() < INT_MAX && ((pWndFC->nInt[0] + nGold) <= INT_MAX)) // Some meme checks
			{
				pWndFC->iTotalMoney += __int64(nGold);
			}
		}
	}
#else //__FL_FARM_STAT
...
#ifdef __WEAPON_RARITY
void CDPClient::SendUpdateWeaponRarity(DWORD dwItem1, DWORD dwItem2)
{
	BEFORESENDSOLE(ar, PACKETTYPE_WEAPON_RARITY, DPID_UNKNOWN);
	ar << dwItem1 << dwItem2;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnWeaponRarity(CAr& ar)
{
	DWORD dwID;
	int nRarity;
	ar >> dwID >> nRarity;
	CItemElem* pItem = g_pPlayer->m_Inventory.GetAtId(dwID);
	if (pItem)
		pItem->SetWeaponRarity(pItem->GetWeaponRarity() + 1);
}
#endif // __WEAPON_RARITY
...
#ifdef __SYS_CUSTOM_TITLE
void CDPClient::OnUpdateTitle(OBJID objid, CAr& ar)
{
	u_long idPlayer;
	ar >> idPlayer;
...
#ifdef __AEGON_FIXES
	TCHAR	m_szTitle[MAX_CUSTOM_TITLE];
	BOOL bShowTitle;
	ar.ReadString(m_szTitle, MAX_CUSTOM_TITLE);
	ar >> bShowTitle;
	if (IsValidObj(pMover))
	{
		strcpy(pMover->m_szOwnTitle, m_szTitle);
		pMover->m_bShowTitle = bShowTitle;
	}
#else
	if (IsValidObj(pMover))
	{
		ar.ReadString(pMover->m_szOwnTitle, MAX_CUSTOM_TITLE);
		ar >> pMover->m_bShowTitle;
	}
#endif
}
void CDPClient::SendUpdateShowTitle(BOOL bState)
{
	BEFORESENDSOLE(ar, PACKETTYPE_UPDATE_SHOW_TITLE, DPID_UNKNOWN);
	ar << bState;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::SendCreateOwnTitle(CString str, BOOL b)
{
	BEFORESENDSOLE(ar, PACKETTYPE_CREATE_OWN_TITLE, DPID_UNKNOWN);
	ar.WriteString(str);
	ar << b;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __PREMIUM
void CDPClient::OnUpdatePremium(OBJID objid, CAr& ar)
{
	u_long idPlayer;
	ar >> idPlayer;

	CMover* pMover = prj.GetMover(objid);
	if (IsValidObj(g_pPlayer) && idPlayer == g_pPlayer->m_idPlayer)
		pMover = g_pPlayer;

	BOOL bShowPremium = FALSE;
	ar >> bShowPremium;

	if (IsValidObj(pMover))
	{
		pMover->m_bShowPremium = bShowPremium;
	}
}
void CDPClient::SendUpdateShowPremium(BOOL bState)
{
	BEFORESENDSOLE(ar, PACKETTYPE_UPDATE_SHOW_PREMIUM, DPID_UNKNOWN);
	ar << bState;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__PREMIUM
...
#ifdef __MOUNT
void CDPClient::OnUpdateMount(OBJID objid, CAr& ar)
{
	...
	if (IsValidObj((CObj*)pCtrl))
	{
		CMover* pMover = (CMover*)pCtrl;
		if (dwID == 0)
			pMover->SendActMsg(OBJMSG_MODE_FLY, dwMountId, 0, 1);
		else if (dwID == 1)
			pMover->m_pActMover->m_bLand = TRUE;
		else if (dwID == 2)
		{
			pMover->SendActMsg(OBJMSG_MODE_GROUND, 0, 0, 1);
			pMover->SendActMsg(OBJMSG_MOUNT_GROUND_STAND, dwMountId, 0, 1);
		}
	}
}

void CDPClient::SendUpdateMount(DWORD dwID)
{
	BEFORESENDSOLE(ar, PACKETTYPE_UPDATE_MOUNT, DPID_UNKNOWN);
	ar << dwID;
	ar << g_pPlayer->GetMountId();
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif // __MOUNT

#ifdef __HEAL_SHOW
void CDPClient::OnHealDamage(OBJID objid, CAr& ar)
{
	OBJID objidAttacker;
	DWORD dwHit;
	ar >> objidAttacker >> dwHit;
	...
	if (dwHit > (unsigned)pMover->GetMaxHitPoint())
		dwHit = pMover->GetMaxHitPoint();
	...
	if (IsValidObj(pMover))
	{
		...
		g_PASS:
			...
			g_HealNumMng.AddNumber(pMover->GetPos(), dwHit, (pMover->IsActiveMover() ? 0 : 1));
		}
	}
}
#endif // __HEAL_SHOW
...
{
	CWndPartyFind* pWndPartyFinder = (CWndPartyFind*)g_WndMng.GetApplet(APP_PARTY_FIND);
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		ar >> tmplist.m_uPartyId;
		ar.ReadString(tmplist.m_sParty);
		ar >> tmplist.m_nSizeofMember;
		ar >> tmplist.m_nLevel >> tmplist.m_nPoint;
		ar >> tmplist.m_nLeaderId;
		ar.ReadString(tmplist.m_sLeader);
		ar.ReadString(tmplist.m_sPartyList);
		ar.Read(&tmplist.m_bLinkAttack, sizeof(tmplist.m_bLinkAttack));
		ar.Read(&tmplist.m_bFortuneCircle, sizeof(tmplist.m_bFortuneCircle));
		ar.Read(&tmplist.m_bGiftBox, sizeof(tmplist.m_bGiftBox));
		ar.Read(&tmplist.m_bSkillFull, sizeof(tmplist.m_bSkillFull));
		ar.Read(&tmplist.m_bExpUp, sizeof(tmplist.m_bExpUp));
		ar.Read(&tmplist.m_bSkillUp, sizeof(tmplist.m_bSkillUp));
...
#ifdef __ANARCHY_SYSTEM
void CDPClient::SendAnarchyBuffs(int buffs, int nMode)
{
	BEFORESENDSOLE(ar, PACKETTYPE_ANARCHY_BUFFS, DPID_UNKNOWN);
	ar << buffs << nMode;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif // __ANARCHY_SYSTEM
...
#ifdef __AEGON_TELEPORTER
void CDPClient::SendTeleportByApp(int nUnitID)
{
...
#ifdef __AEGON_LOOT_BOX
void CDPClient::SendLootBoxStart(int nSelectedBox, DWORD ticketObjID, int nBuyMode)
{
	BEFORESENDSOLE(ar, PACKETTYPE_LOOTBOX_START, DPID_UNKNOWN);
	ar << nSelectedBox;
	ar << ticketObjID;
	ar << nBuyMode;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnLootboxResult(CAr& ar)
{
	__LOOTBOX_RESULT result;
	ar >> result;

	CWndLootBoxMain* tmpWnd = (CWndLootBoxMain*)g_WndMng.GetApplet(APP_LOOTBOX);

	if (tmpWnd)
		tmpWnd->ReceiveResult(result);
}
void CDPClient::SendLootBoxCollect()
{
	BEFORESENDSOLE(ar, PACKETTYPE_LOOTBOX_COLLECT, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif // __AEGON_LOOT_BOX

#ifdef __ADVANCED_MAIL
void CDPClient::SendQueryGetAndDeleteMail(u_long nMail, BOOL bItem, DWORD dwGold)
{
	...
	if (!pMover)
		return;
	if ((pMover->GetGold() + dwGold) > 2147483647)
	{
		strErr.Format("Failed to get Mail. Penya Full!");
		g_WndMng.PutString(strErr, NULL, COLOR_ERROR);
		return;
	}
	if (!pMover->m_Inventory.GetEmptyCount() && bItem)
	{
		strErr.Format("Failed to get Mail. Inventory Full!");
		g_WndMng.PutString(strErr, NULL, COLOR_ERROR);
		return;
	}

	if (bItem)
		SendQueryGetMailItem(nMail);

	if (dwGold)
		SendQueryGetMailGold(nMail);

	Sleep(10);
	SendQueryRemoveMail(nMail);
}
void CDPClient::SendQueryGetAndDeleteMailAll()
{
	CMover* pMover = g_pPlayer;
	CString strErr;
	BOOL bItem = FALSE;
	...
	auto box = CMailBox::GetInstance();
	CMailBox& mailbox = *box;

	int freeSlots = pMover->m_Inventory.GetEmptyCount();
	unsigned __int64 moverGold = pMover->GetGold();
	...
	for (int i = 0; i < box->size(); i++) {
		...
		SendQueryGetAndDeleteMail(mail->m_nMail, bItem, dwGold);
	}
}
#endif // __ADVANCED_MAIL

#ifdef __AEGON_VIEW_INV
void CDPClient::SendViewInventory(u_long uidPlayer, int nContainer)
{
	if (g_pPlayer == NULL)
		return;

	BEFORESENDSOLE(ar, PACKETTYPE_VIEW_INVENTORY, DPID_UNKNOWN);
	ar << g_pPlayer->m_idPlayer << uidPlayer << nContainer;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnViewInventory(OBJID objid, CAr& ar)
{
	CWndViewInventoryMain* invMain = (CWndViewInventoryMain*)g_WndMng.GetWndBase(APP_VIEW_INVENTORY);

	if (invMain)
	{
		invMain->m_viewMover.SetMethod(METHOD_NONE);
		invMain->m_viewMover.Serialize(ar);
		invMain->m_viewMover.SetEatPetId(NULL_ID);
		invMain->m_viewMover.SetPetId(NULL_ID);
		invMain->m_viewMover.m_objid = NULL_ID;

		invMain->OnInventoryView(&invMain->m_viewMover);
		
		if (g_WndMng.m_pWndViewBag)
		{
			CRect rectOld = g_WndMng.m_pWndViewBag->GetWndRect();
			SAFE_DELETE(g_WndMng.m_pWndViewBag);

			invMain->OnBagView(&invMain->m_viewMover);

			if (g_WndMng.m_pWndViewBag)
			{
				g_WndMng.m_pWndViewBag->SetWndRect(rectOld);
			}
		}
	}
	else 
	{
		CMover* tmpMover = new CMover;
		tmpMover->SetMethod(METHOD_NONE);
		tmpMover->Serialize(ar);
		tmpMover->SetEatPetId(NULL_ID);
		tmpMover->SetPetId(NULL_ID);
		tmpMover->m_objid = NULL_ID;
		SAFE_DELETE(tmpMover);
	}
}
void CDPClient::OnViewBag(OBJID objid, CAr& ar)
{
	CWndViewInventoryMain* invMain = (CWndViewInventoryMain*)g_WndMng.GetWndBase(APP_VIEW_INVENTORY);
...
void CDPClient::OnViewBank(OBJID objid, CAr& ar)
{
}

void CDPClient::SendViewInventoryCommand(CItemElem* commandItem, int nCommand, int nContainer, int nContainerField)
{
	if (g_pPlayer == NULL || !g_pPlayer->IsAuthHigher(AUTH_GAMEMASTER2))
		return;
	/*
	* nCommand:
	* 0:=Copy
	...
	*/
	...
	u_long viewMoverID = ((CWndViewInventoryMain*)g_WndMng.GetWndBase(APP_VIEW_INVENTORY))->m_viewMover.m_idPlayer;

	BEFORESENDSOLE(ar, PACKETTYPE_VIEW_INVENTORY_COMMAND, DPID_UNKNOWN);
	ar << g_pPlayer->m_idPlayer << viewMoverID << nCommand << nContainer << nContainerField;
...
void CDPClient::SendPlayerPetfilter()
{
	if (IsInvalidObj(g_pPlayer))
		return;

	BEFORESENDSOLE(ar, PACKETTYPE_PETFILTER, DPID_UNKNOWN);
	ar << g_pPlayer->m_dwPetfilter;
	g_pPlayer->m_sPetfilterLists.Write(ar);
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__SYS_PETFILTER

#ifdef __BADGE
void CDPClient::SendChangeBadgeIcon(int nChange)
{
	BEFORESENDSOLE(ar, PACKETTYPE_BADGE_CHANGE, DPID_UNKNOWN);
	ar << nChange;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnBadgeListUpdate(CAr& ar)
{
	ar.ReadContainerPOD(g_pPlayer->GetOwnedBadges(), 200);
}

void CDPClient::OnBadgeChange(OBJID objid, CAr& ar)
{
	int nChange;
	ar >> nChange;
	...
	if (IsValidObj(pMover))
	{
		pMover->SetBadge(nChange);
	}
}
#endif //__BADGE

#ifdef __AEGON_INGAME_DP
void CDPClient::OnDPRefresh(OBJID objid, CAr& ar)
{
	int nNewDPBalance;
	ar >> nNewDPBalance;

	g_pPlayer->SetIngameDP(nNewDPBalance);

}
#endif // __AEGON_INGAME_DP

#ifdef __SYS_GUILDCOLOR
void CDPClient::OnGuildSetColor(CAr& ar)
{
	u_long idGuild;
	DWORD R, G, B;
	ar >> idGuild >> R >> G >> B;
	...
	if (pGuild)
	{
		pGuild->m_adwGuildColor[0] = R;
		pGuild->m_adwGuildColor[1] = G;
		pGuild->m_adwGuildColor[2] = B;
		g_GuildMng.SetColor(pGuild, R, G, B);
	}
}
void CDPClient::SendGuildSetColor(int R, int G, int B)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SETGUILDCOLOR_CTS, DPID_UNKNOWN);
	ar << R << G << B;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__SYS_GUILDCOLOR
...
#ifdef __SYS_NAMECOLOR
void CDPClient::SendSetNameColor(int R, int G, int B)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SETNAMECOLOR, DPID_UNKNOWN);
	ar << R << G << B;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnSetNameColor(OBJID objid, CAr& ar)
{
	int r, g, b;
	ar >> r >> g >> b;
	CCtrl* pCtrl = prj.GetCtrl(objid);
...
#ifdef __GUILDFINDER
void CDPClient::OnGuildFinderAppication(CAr& ar)
{
	u_long idPlayer;
	int nJob, nLevel;
	BYTE byCheckHero;
	...
	SAFE_DELETE(g_WndMng.m_pWndGuildFinderReq);
	g_WndMng.m_pWndGuildFinderReq = new CWndGuildFinderReq(idPlayer, nLevel, nJob, byCheckHero);
	g_WndMng.m_pWndGuildFinderReq->Initialize();
	return;
}

void CDPClient::OnGuildRequest(u_long uGuildid, u_long uMasterid)
{
	BEFORESENDSOLE(ar, PACKETTYPE_GUILD_REQUEST, DPID_UNKNOWN);
	ar << uGuildid << uMasterid;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnRefreshGuildList(CAr& ar)
{
	g_WndMng.PutString("", NULL, prj.GetTextColor(TID_GAME_REMOVEQUEST));
	int nCount;

	CWndGuildFinder* pWndGuildFinder = (CWndGuildFinder*)g_WndMng.GetApplet(APP_GUILDFINDER);
	if (!pWndGuildFinder)
	{
		g_WndMng.PutString("", NULL, prj.GetTextColor(TID_GAME_REMOVEQUEST));
		return;
	}
	pWndGuildFinder->m_mapItem.clear();

	ar >> nCount;
	for (int i = 0; i < nCount; i++)
	{
		GUILDFINDER_LIST tmplist;

		ar >> tmplist.m_uGuildId;
		ar.ReadString(tmplist.m_sGuild);
		ar >> tmplist.m_nSizeofMember;
		ar >> tmplist.m_nMaxSize;
		ar >> tmplist.m_nLevel;
		ar >> tmplist.m_nLeaderId;
		pWndGuildFinder->m_mapItem.push_back(tmplist);
	}

	pWndGuildFinder->RefreshItemList();

}

void CDPClient::OnGuildFinderOpt(CAr& ar)
{
	u_long idGuild;
	BOOL bGuildFinder;
	ar >> idGuild;
	...
	pGuild->m_bGuildFinder = bGuildFinder;

	g_WndMng.PutString("Guild Finder status toggled.", NULL, COLOR_SUCCESS);
...
void CDPClient::SendGuildFinder(BOOL bGuildFinder)
{
	BEFORESENDSOLE(ar, PACKETTYPE_NW_GUILDFINDER, DPID_UNKNOWN);
	ar << bGuildFinder;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__GUILDFINDER
...
void CDPClient::SendRemoveCmm(DWORD nItem)
{
	BEFORESENDSOLE(ar, PACKETTYPE_REMOVE_CMM, DPID_UNKNOWN);
	ar << nItem;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __SYS_MONSTER_CLASH
void CDPClient::SendMonsterClashRegisterMember(u_long idPlayer)
{
	BEFORESENDSOLE(ar, PACKETTYPE_MONSTER_CLASH_REGISTER_MEMBER, DPID_UNKNOWN);
	ar << idPlayer;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::SendMonsterClashUnRegisterMember(u_long idPlayer)
{
	BEFORESENDSOLE(ar, PACKETTYPE_MONSTER_CLASH_UNREGISTER_MEMBER, DPID_UNKNOWN);
	ar << idPlayer;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnMonsterClashRegisterWindow(CAr& ar)
{
	if (g_WndMng.pWndMonsterClashRegister)
		SAFE_DELETE(g_WndMng.pWndMonsterClashRegister);

	g_WndMng.pWndMonsterClashRegister = new CWndMonsterClashRegister();
	g_WndMng.pWndMonsterClashRegister->Initialize(&g_WndMng);
	...
	for (int i = 0; i < nSize; i++)
	{
		...
		TCHAR tGuild[64];
		ar.ReadString(tGuild, 64);

		g_WndMng.pWndMonsterClashRegister->AddGuild(dwGuildId, tGuild);
	}
}

void CDPClient::OnMonsterClashRegisterMemberWindow(CAr& ar)
{
	if (g_WndMng.pWndMonsterClashRegisterMember)
		SAFE_DELETE(g_WndMng.pWndMonsterClashRegisterMember);

	g_WndMng.pWndMonsterClashRegisterMember = new CWndMonsterClashRegisterMember();
	g_WndMng.pWndMonsterClashRegisterMember->Initialize(&g_WndMng);
	...
	for (int i = 0; i < nSize; i++)
	{
		...
		TCHAR tName[64];
		ar.ReadString(tName, 64);

		g_WndMng.pWndMonsterClashRegisterMember->AddMemberL(idPlayer, tName);
	}
	ar >> nSize;
	for (int i = 0; i < nSize; i++)
	{
		...
		g_WndMng.pWndMonsterClashRegisterMember->AddMemberR(idPlayer, tName);
	}
}

void CDPClient::OnMonsterClashRegister(CAr& ar)
{
	DWORD dwGuildId;
	ar >> dwGuildId;
...
void CDPClient::OnMonsterClashRegisterMember(CAr& ar)
{
	u_long idPlayer;
	ar >> idPlayer;
	...
	if (g_WndMng.pWndMonsterClashRegisterMember)
	{
		g_WndMng.pWndMonsterClashRegisterMember->RemoveMemberR(idPlayer, tName);
		g_WndMng.pWndMonsterClashRegisterMember->AddMemberL(idPlayer, tName);
	}
}

void CDPClient::OnMonsterClashUnRegisterMember(CAr& ar)
{
	u_long idPlayer;
	ar >> idPlayer;
	...
	if (g_WndMng.pWndMonsterClashRegisterMember)
	{
		g_WndMng.pWndMonsterClashRegisterMember->RemoveMemberL(idPlayer, tName);
		g_WndMng.pWndMonsterClashRegisterMember->AddMemberR(idPlayer, tName);
	}
}
void CDPClient::OnMonsterClashState(CAr& ar)
{
	CWndMonsterClashRank::GetInstance()->Serialize(ar);
}
#endif //__SYS_MONSTER_CLASH

#ifdef __REPORT_SYSTEM
void CDPClient::SendReport(BYTE nReportType, char* lpszText)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDREPORT, DPID_UNKNOWN);
	ar << nReportType;
	ar.WriteString(lpszText);
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __SYS_SANDBOX
void CDPClient::SendSandboxClear()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SANDBOX, DPID_UNKNOWN);
	ar << (int)TRUE;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::SendSandboxSpawn(DWORD dwIndex, BOOL bDamage)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SANDBOX, DPID_UNKNOWN);
	ar << (int)FALSE;
	ar << dwIndex;
...
void CDPClient::SendSandboxTeleport()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SANDBOX_TELEPORT, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__SYS_SANDBOX

#ifdef __SHOPS_VISITED
void CDPClient::OnRemoveShop(CAr& ar) {
	OBJID obj;
	ar >> obj;
	g_Neuz.RemoveShopFromList(obj);
}
#endif //__SHOPS_VISITED

#ifdef __TICKETS_DETAILS
void CDPClient::LayersDetailReceived(OBJID objid, CAr& ar)
{
	g_LayersDetail.OnLayersDetail(ar);
}
#endif //__TICKETS_DETAILS
...
#ifdef __CHECKLOAD
void CDPClient::GetCheckLoad(const unsigned long worldId, const int layer)
{
	BEFORESENDSOLE(ar, PACKETTYPE_REQUEST_CHECKLOAD, DPID_UNKNOWN);
	ar << worldId << layer;
	SEND(ar, this, DPID_SERVERPLAYER);
}

#include "WndMapEx.h"
void CDPClient::OnCheckLoad(CAr& ar)
{
	...
	playerPositions.reserve(vecSize);

	for (size_t i = 0; i < vecSize; ++i)
		playerPositions.emplace_back(ar.get<D3DXVECTOR3>());

	CWndMapEx* pWndMapEx = static_cast<CWndMapEx*>(g_WndMng.CreateApplet(APP_MAP_EX));
	if (!pWndMapEx)
		return;

	pWndMapEx->SetConstructionMode(CWndMapEx::CHECKLOAD);
	pWndMapEx->addCheckLoad(std::move(playerPositions), worldId);
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
void CDPClient::SendLoadAchievementProgress()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDLOADACHIEVEMENTPROG, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnLoadAchievementProgress(CAr& ar)
{
	g_pPlayer->GetAchievementProgress()->ReadMap(ar);

	CWndAchievementSystemMain* pAchWnd = (CWndAchievementSystemMain*)g_WndMng.GetWndBase(APP_AEGON_ACHIEVEMENT_SYSTEM);
	if (pAchWnd)
		pAchWnd->OnProgressRefresh();
}
void CDPClient::SendCheckAchievementProgress(int mode, int msg)
{
	BEFORESENDSOLE(ar, PACKETTYPE_ONCHECKACHIEVEMENTPROG, DPID_UNKNOWN);
	ar << mode << msg;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnFinishedAchievement(CAr& ar)
{
	int achievementID;
	ar >> achievementID;

	g_pPlayer->m_AchievementNoticeID.push_back(achievementID);
}
void CDPClient::SendCollectAchievement(int achievementID)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDCOLLECTACHIEVEMENT, DPID_UNKNOWN);
	ar << achievementID;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::SendAchievementFavorite(int achievementID)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDACHIEVEMENTFAV, DPID_UNKNOWN);
	ar << achievementID;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnFavoriteAchievementRefresh(CAr& ar)
{
	int vecSize;
	ar >> vecSize;

	g_pPlayer->m_AchievementFavorites.clear();
	for (int i = 0; i < vecSize; i++) 
	{
		int achievementID;
		ar >> achievementID;
		g_pPlayer->m_AchievementFavorites.push_back(achievementID);
	}

	CWndAchievementSystemMain* pAchWnd = (CWndAchievementSystemMain*)g_WndMng.GetWndBase(APP_AEGON_ACHIEVEMENT_SYSTEM);
	if (pAchWnd)
		pAchWnd->OnFavoritesRefresh();

}
void CDPClient::OnCollectingTimeRefresh(CAr& ar)
{
	__int64 newCollectingTime;
	ar >> newCollectingTime;

	if (newCollectingTime > 0)
		g_pPlayer->m_nCollectingTime = newCollectingTime;

	CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_COLLECTXTIME, (g_pPlayer->m_nCollectingTime / 60 / 60), AEGACH_PROG_CMP);
}

#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __UPGRADE_SUCCESS
void CDPClient::OnUpgradeSuccess(CAr& ar)
{
	//	ar >> prj.m_mapUpgradeSuccess;
}
#endif //__UPGRADE_SUCCESS

#ifdef __AFK_MONITORING
void CDPClient::OnSetAFKState(OBJID objid, CAr& ar)
{
	DWORD dwState;
	CMover* pMover = prj.GetMover(objid);
	ar >> dwState;
	if (pMover)
	{
		pMover->m_dwAFKState = dwState;
	}
}
#endif //__AFK_MONITORING

#ifdef __AEGON_FULLSCREEN_AND_RESIZE
void CDPClient::SendReloadTaskbar()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDRELOADTASKBAR, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __AEGON_LAST_ONLINE
void CDPClient::OnLastOnlineRefresh(CAr& ar)
{
	CAegonLastOnlineMng::GetInstance()->ReadMap(ar);
}
void CDPClient::OnSingleLastOnlineRefresh(CAr& ar)
{
	__LAST_ONLINE_UNIT tmpUnit;
	tmpUnit.Init();
	ar.ReadString(tmpUnit.m_szName, 64);
	ar >> tmpUnit.playerID;
	ar >> tmpUnit.m_nLastOnline;
	ar >> tmpUnit.m_bOnline;

	map<u_long, __LAST_ONLINE_UNIT>::iterator it = CAegonLastOnlineMng::GetInstance()->m_mapLastOnline.find(tmpUnit.playerID);
	if (it != CAegonLastOnlineMng::GetInstance()->m_mapLastOnline.end()) 
	{
		if(strcmp(it->second.m_szName, tmpUnit.m_szName) != 0)
			sprintf(it->second.m_szName, tmpUnit.m_szName);
		it->second.m_nLastOnline = tmpUnit.m_nLastOnline;
		it->second.m_bOnline = tmpUnit.m_bOnline;
	}
	else 
	{
		CAegonLastOnlineMng::GetInstance()->m_mapLastOnline.insert(make_pair(tmpUnit.playerID, tmpUnit));
	}
	
}
void CDPClient::OnOwnLastOnlineRefresh(CAr& ar)
{
	if (g_pPlayer == NULL)
		return;

	__int64 newLastOnlineTime;
	ar >> newLastOnlineTime;
	g_pPlayer->m_nLastOnline = newLastOnlineTime;
}
#endif // __AEGON_LAST_ONLINE


#ifdef __AEGON_INGAME_TIME
void CDPClient::OnOnlineTimeRefresh(CAr& ar)
{
	if (g_pPlayer == NULL)
		return;

	__int64 newOnlineTime;
	ar >> newOnlineTime;
	g_pPlayer->m_nOnlineTime = newOnlineTime;
}
#endif // __AEGON_INGAME_TIME

#ifdef __PLAYER_LIST
void CDPClient::SendPlayerListRequest(BOOL bCheck, BOOL bCheck1, BOOL bCheck2, BOOL bCheck3, BOOL bCheck4, BOOL bCheck5, BOOL bCheck6, BOOL bCheck7, BOOL bCheck8, BOOL bCheck9, BOOL bCheck10, BOOL bCheck11, BOOL bCheck12, BOOL bCheck13, BOOL bCheck14, BOOL bCheck15, BOOL bCheck16, int nMin, int nMax, CString search)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PLAYER_LIST_SEND_REQUEST, DPID_UNKNOWN);
	ar << bCheck << bCheck1 << bCheck2 << bCheck3 << bCheck4 << bCheck5 << bCheck6 << bCheck7 << bCheck8 << bCheck9 << bCheck10 << bCheck11 << bCheck12 << bCheck13 << bCheck14 << bCheck15 << bCheck16 << nMin << nMax;
	ar.WriteString(search);
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnPlayerList(CAr& ar)
{
	CWndPlayerList* pWndPlayerList = (CWndPlayerList*)g_WndMng.GetWndBase(APP_PLAYER_LIST);
	if (pWndPlayerList)
		pWndPlayerList->OnGetPlayerList(ar);
}
#endif // __PLAYER_LIST

#ifdef __AEGON_CRAFTING_SYSTEM
void CDPClient::SendCraftRecipe(__CRAFTING_UNIT* m_selectedUnit, CItemElem* componentItems[AEGCRAFT_MAXCOMPONENTS])
{
	if (!IsValidObj(g_pPlayer) || !m_selectedUnit)
		return;


	BEFORESENDSOLE(ar, PACKETTYPE_SENDCRAFTRECIPE, DPID_UNKNOWN);
	ar << m_selectedUnit->unitID;
	ar << (int)m_selectedUnit->components.size();
	for (int i = 0; i < m_selectedUnit->components.size(); i++) 
	{
		if (componentItems[i])
			ar << componentItems[i]->m_dwObjId;
		else
			ar << ((DWORD)0);
	}
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnCraftingSuccess(CAr& ar)
{
	if (g_pPlayer == NULL)
		return;
	...
	{
		g_WndMng.m_pWndCraftingMain->OnSuccessFullCraft();
	}
}
void CDPClient::OnCraftingExpRefresh(CAr& ar)
{
	if (g_pPlayer == NULL)
		return;
	...
	int newCraftExp;
	ar >> newCraftExp;
	g_pPlayer->m_nCraftingExp = newCraftExp;

	if (g_WndMng.m_pWndCraftingMain)
	{
		g_WndMng.m_pWndCraftingMain->OnCraftingExpRefresh();
	}
}
#endif // __AEGON_CRAFTING_SYSTEM

#ifdef __WEAPON_MERGE
void CDPClient::SendWeaponMerge(DWORD dwIdSrc, DWORD dwIdDest, DWORD dwIdProt)
{
	BEFORESENDSOLE(ar, PACKETTYPE_WEAPONMERGE, DPID_UNKNOWN);
	ar << dwIdSrc << dwIdDest << dwIdProt;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
void CDPClient::OnMergedWeapon(CAr& ar)
{
	DWORD dwObj;
	BOOL bCancel;
	...
	CItemElem* pItem = g_pPlayer->m_Inventory.GetAtId(dwObj);
	...
	if (bCancel)
	{
		...
		pItem->SetMergedRandomOptItemId(0);
		pItem->SetMergedWeaponATK(0);
	}
	else 
	{
		int nSize, nATK;//, nAdded;
		__int64 nRandomOptMerge;
		ar >> nSize;
		pItem->SetMergedPiercingSize(nSize);
		for (int i = 0; i < nSize; ++i)
		{
			DWORD dwItem;
			ar >> dwItem;
			pItem->SetMergedPiercingItem(i, dwItem);
		}
		ar >> nRandomOptMerge;
		pItem->SetMergedRandomOptItemId(nRandomOptMerge);
		ar >> nATK;
		pItem->SetMergedWeaponATK(nATK);

		pItem->SetFlag(CItemElem::merged);
	}
}
#endif //__WEAPON_MERGE
...
void CDPClient::OnBuyBack(CAr& ar)
{
	CItemElem itemElem;
	itemElem.Serialize(ar);
	g_pPlayer->m_BuyBack.Add(&itemElem);

	CWndBuyBack* pWndBuyBack = (CWndBuyBack*)g_WndMng.GetWndBase(APP_BUYBACK);
	if (pWndBuyBack)
...
void CDPClient::OnBuyBackRemoveItem(CAr& ar)
{
	...
	if (nItem != -1)
	{
		g_pPlayer->m_BuyBack.RemoveAtId(nItem);
		Reorganize(&g_pPlayer->m_BuyBack);

		CWndBuyBack* pWndBuyBack = (CWndBuyBack*)g_WndMng.GetWndBase(APP_BUYBACK);
		if (pWndBuyBack)
			pWndBuyBack->ProcessListBox();
	}
	g_pPlayer->m_bCanBuy = TRUE;
}
void CDPClient::SendBuyBack(int nItem)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SEND_BUYBACK, DPID_UNKNOWN);
	ar << nItem;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif // __BUYBACK
...
#ifdef __CLIENT_ONLINE_COUNT
void CDPClient::OnOnlineChange(CAr& ar)
{
	long PlayerCount;
	ar >> PlayerCount;
...
#ifdef __AEGON_SEASON_PASS
void CDPClient::SendLoadSeasonPassProgress()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDLOADSEASONPASSPROG, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnLoadSeasonPassProgress(CAr& ar)
{
	g_pPlayer->m_SeasonpassProgress.ReadMap(ar);

	CWndSeasonPassMain* pWnd = (CWndSeasonPassMain*)g_WndMng.GetWndBase(APP_SEASON_PASS);
	if (pWnd)
		pWnd->OnProgressRefresh();
}
void CDPClient::OnUpdateSeasonPassProgress(CAr& ar)
{
	g_pPlayer->m_SeasonpassProgress.ReadSingle(ar);

	CWndSeasonPassMain* pWnd = (CWndSeasonPassMain*)g_WndMng.GetWndBase(APP_SEASON_PASS);
	if (pWnd)
		pWnd->OnProgressRefresh();
}
void CDPClient::SendClaimSeasonPassReward(int passID, int rewardID)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDCLAIMSEASONPASSREWARD, DPID_UNKNOWN);
	ar << passID;
	ar << rewardID;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::SendGetSeasonPassCountdown()
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDGETSEASONPASSCOUNTDOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnSeasonPassCountdown(CAr& ar)
{
	time_t pveCountdown, pvpCountdown, currentServerTime;
	ar >> pveCountdown;
	ar >> pvpCountdown;
	ar >> currentServerTime;
	...
	time_t timeDiff = currentServerTime - currentTime;
	pveCountdown -= timeDiff;
	pvpCountdown -= timeDiff;

	__SEASONPASS_UNIT* pveUnit = CAegonSeasonPassMng::GetInstance()->GetCurrentUnit(TRUE);
	if (pveUnit)
		pveUnit->timeEnd = pveCountdown;

	__SEASONPASS_UNIT* pvpUnit = CAegonSeasonPassMng::GetInstance()->GetCurrentUnit(FALSE);
	if (pvpUnit)
		pvpUnit->timeEnd = pvpCountdown;
}
void CDPClient::SendPurchaseSeasonPass(BOOL bPvE, int nPassID)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDPURCHASESEASONPASS, DPID_UNKNOWN);
	ar << bPvE;
	ar << nPassID;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
void CDPClient::SendAegonSystemShout(LPCSTR szString, BOOL bShowName, BOOL bShowAuth, int nImage, int nFont)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SENDAEGONSYSTEMSHOUT, DPID_UNKNOWN);
...
#ifdef __SWITCH_EQUIP
void CDPClient::OnEquipSwitch(CAr& ar)
{
	if (IsValidObj(g_pPlayer)) 
	{
		g_pPlayer->ReadEQSwitch(ar, &g_pPlayer->m_vEquipSwitchInfo);
	}
#ifdef __MIKSIK_EQUALIZED_MODE
	CWndSwitchEquip* pWndSwitch = (CWndSwitchEquip*)g_WndMng.GetWndBase(APP_SWITCHEQUIP);
	if (pWndSwitch)
	{
		CWndComboBox* pComboBox = (CWndComboBox*)pWndSwitch->GetDlgItem(WIDC_COMBOBOX);
		if (!pComboBox)
			pComboBox = (CWndComboBox*)pWndSwitch->GetDlgItem(WIDC_COMBOBOX1);

		if (pComboBox)
		{
			pComboBox->ResetContent();
			pComboBox->AddString("Create Macro");
			for (auto& pEquip : g_pPlayer->m_vEquipSwitchInfo)
				pComboBox->AddString(pEquip.strTitle);

			pWndSwitch->LoadSwitch();
...
void CDPClient::SendEquipSwitch()
{
	if (IsValidObj(g_pPlayer)) 
	{
		BEFORESENDSOLE(ar, PACKETTYPE_EQUIPSWITCH_UPDATE, DPID_UNKNOWN);
		g_pPlayer->WriteEQSwitch(ar, &g_pPlayer->m_vEquipSwitchInfo);
		SEND(ar, this, DPID_SERVERPLAYER);
	}
	
}
void CDPClient::SendUseEquipSwitch(int nIndex)
{
	BEFORESENDSOLE(ar, PACKETTYPE_EQUIPSWITCH_USE, DPID_UNKNOWN);
	ar << nIndex;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif // __SWITCH_EQUIP
...
#ifdef __AEGON_PVP_RANKING
void CDPClient::SendGetPvPRanking(int nMode)
{
	BEFORESENDSOLE(ar, PACKETTYPE_GET_PVP_RANKING, DPID_UNKNOWN);
	ar << nMode;
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnAddPvPRanking(CAr& ar) 
{
	CAegonPvPRankingMng::GetInstance()->ReadMap(ar);

	CWndPvPRankingMain* pWnd = (CWndPvPRankingMain*)g_WndMng.GetApplet(APP_PVP_RANKING);
	if (pWnd) 
	{
		pWnd->OnPvPRankingRefresh();
	}
}
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_PERMANENT_BUFF
void CDPClient::OnAddPermBuffRefresh(CAr& ar)
{
	if (IsValidObj(g_pPlayer)) 
	{
		ar >> g_pPlayer->m_vPermanenetBuffs;
	}
}
#endif // __AEGON_PERMANENT_BUFF

#ifdef __AEGON_PREM_MOTIONS
void CDPClient::OnAddMotionRefresh(CAr& ar)
{
	if (IsValidObj(g_pPlayer)) 
	{
		ar >> g_pPlayer->m_vUnlockedMotions;
	}
}
void CDPClient::SendPurchaseMotion(DWORD dwMotionID)
{
	if (IsValidObj(g_pPlayer))
	{
		BEFORESENDSOLE(ar, PACKETTYPE_SENDPURCHASEMOTION, DPID_UNKNOWN);
		ar << dwMotionID;
		SEND(ar, this, DPID_SERVERPLAYER);
	}
}
...
#ifdef __AEGON_EMOTE_SYSTEM
void CDPClient::OnAddEmoteRefresh(CAr& ar)
{
	if (IsValidObj(g_pPlayer))
	{
		ar >> g_pPlayer->m_vUnlockedEmotes;
	}
}
void CDPClient::SendPurchaseEmote(DWORD dwEmoteID)
{
	if (IsValidObj(g_pPlayer))
	{
		BEFORESENDSOLE(ar, PACKETTYPE_SENDPURCHASEEMOTE, DPID_UNKNOWN);
		ar << dwEmoteID;
		SEND(ar, this, DPID_SERVERPLAYER);
	}
}
...
#ifdef __PET_GLOW
void CDPClient::OnChangePetGlowSfx(OBJID objId, CAr& ar)
{
	DWORD dwSfxId;
	ar >> dwSfxId;
	CMover* pMover = prj.GetMover(objId);
	if (IsValidObj(pMover))
	{
		pMover->m_dwPetGlow = dwSfxId;
		pMover->SetSfxPetGlow(dwSfxId);
	}
}

void CDPClient::OnChangePetGlow(DWORD m_dwObjId, int m_nSelected)
{
	BEFORESENDSOLE(ar, PACKETTYPE_CHANGEPETGLOW, DPID_UNKNOWN);
	ar << m_dwObjId;
	ar << m_nSelected;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
void CDPClient::SendModsManager(BYTE nMode)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SEND_MODS_MANAGER, DPID_UNKNOWN);
	ar << nMode;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __SYS_WORLDBOSS
void CDPClient::SendHWID() 
{
	BEFORESENDSOLE(ar, PACKETTYPE_REGISTER_HWID, DPID_UNKNOWN);
	ar.WriteString(g_Neuz.m_WinGUID);
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //__SYS_WORLDBOSS

#ifdef __COMBAT_LOG
void CDPClient::OnCombatLogReset(CAr& ar)
{
	CombatLog::GetInstance()->ClearLog();
}
#endif //__COMBAT_LOG
...
void CDPClient::SendQuickSell(vector<DWORD> vecItems, BOOL bUppedItems, BOOL bStattedItems, BOOL bAwakedItems, BOOL bRaretyItems, BOOL bElementItems)
#else
void CDPClient::SendQuickSell(vector<DWORD> vecItems)
#endif
{
	if (vecItems.size() == 0) return;
	BEFORESENDSOLE(ar, PACKETTYPE_QUICKSELL, DPID_UNKNOWN);
...
#if _WIN64
	ar << static_cast<unsigned long>(vecItems.size());
#else
	ar << vecItems.size();
#endif
...
#ifdef __CRYSTAL
void CDPClient::SendUpdateCrystal(DWORD dwIdSrc, DWORD dwIdDest)
{
	BEFORESENDSOLE(ar, PACKETTYPE_CRYSTAL, DPID_UNKNOWN);
	ar << dwIdSrc << dwIdDest;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnCrystal(CAr& ar) // snapshot
{
	DWORD dwID, dwCrystal;
	ar >> dwID >> dwCrystal;
	CItemElem* pItem = g_pPlayer->m_Inventory.GetAtId(dwID);

	if (!pItem)
		return;
	pItem->SetCrystal(dwCrystal);
}
#endif // __CRYSTAL

#ifdef __AEGON_INGAME_SHOP
void CDPClient::OnAddIngameShopInfo(CAr& ar)
{
	CAegonIngameShopMng::GetInstance()->ReadCategoryMap(ar);
	CAegonIngameShopMng::GetInstance()->ReadItemMap(ar);

	CWndIngameShopMain* m_pWndIngameShop = (CWndIngameShopMain*)g_WndMng.GetApplet(APP_AEGON_INGAME_SHOP);
	if (m_pWndIngameShop) 
	{
		m_pWndIngameShop->Destroy();
		g_WndMng.Process();
		g_WndMng.CreateApplet(APP_AEGON_INGAME_SHOP);
	}
}
void CDPClient::SendWebshopPurchase(DWORD dwUnitID, int nAmount)
{
	if (IsValidObj(g_pPlayer))
	{
		BEFORESENDSOLE(ar, PACKETTYPE_SENDWEBSHOPPURCHASE, DPID_UNKNOWN);
		ar << dwUnitID;
		ar << nAmount;
		SEND(ar, this, DPID_SERVERPLAYER);
	}
...
#ifdef __AEGON_TOURNAMENT
void CDPClient::SendCreateTournament(__TOURNAMENT_UNIT* pTournamentUnit)
{
	if (!pTournamentUnit)
		return;
	if (IsValidObj(g_pPlayer) && g_pPlayer->IsAuthHigher(AUTH_ADMINISTRATOR))
	{
		BEFORESENDSOLE(ar, PACKETTYPE_SENDCREATETOURNAMENT, DPID_UNKNOWN);
		pTournamentUnit->Write(ar);
		SEND(ar, this, DPID_SERVERPLAYER);
	}
}
void CDPClient::SendGetTournamentMap() 
{

	BEFORESENDSOLE(ar, PACKETTYPE_TOURNAMENT_GET_MAP, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnTournamentMap(CAr& ar)
{
	CAegonTournamentMng::GetInstance()->ReadTournamentMap(ar);
	if (g_WndMng.m_pWndAegonTournament)
	{
		g_WndMng.m_pWndAegonTournament->OnGetTournamentMap();
	}
}
void CDPClient::OnTournamentsChanged(CAr& ar)
{
	if (g_WndMng.m_pWndAegonTournament) 
	{
		SendGetTournamentMap();
	}
}
void CDPClient::SendDeleteTournament(DWORD dwTournamentID)
{
	if (IsValidObj(g_pPlayer) && g_pPlayer->IsAuthHigher(AUTH_ADMINISTRATOR))
	{
		BEFORESENDSOLE(ar, PACKETTYPE_SENDDELETETOURNAMENT, DPID_UNKNOWN);
		ar << dwTournamentID;
		SEND(ar, this, DPID_SERVERPLAYER);
	}
}
...
#ifdef __ROYAL_RUMBLE
void CDPClient::OnPlayerSiege(CAr& ar)
{
	BYTE nState;
	ar >> nState;
	switch (nState)
	{
	case WAR_ADDJOINTIME:
		OnAddSiegeTime(ar);
		break;
	case WAR_ADDCLIENTUPDATE:
		OnUpdatePlayerSiege(ar);
		break;
	case WAR_ADDFIGHTSTATE:
		OnAddFightState(ar);
		break;
	case WAR_REJOIN_WINDOW:
		OnSiegeRejoinWindow(ar);
		break;
	case WAR_ADDRANKING:
		OnSiegeRankingList(ar);
		break;
	}
}

void CDPClient::OnSiegeRankingList(CAr& ar)
{
	u_int nCount;
	ar >> nCount;
	...
	{
		SIEGE_RANK player;
		ar.ReadString(player.szName);
		ar >> player.uidPlayer;
		ar >> player.nJob;
		ar >> player.nKills;
		ar >> player.nDeaths;
		ar >> player.nPoints;
		g_War.m_vecRankSiege.push_back(player);
	}
}

void CDPClient::OnAddSiegeTime(CAr& ar)
{
	int nState;
	DWORD dwTime;
	ar >> dwTime;
	ar >> nState;
	g_War.SetState(nState);
	CWndWorld* pWndWorld = (CWndWorld*)g_WndMng.GetWndBase(APP_WORLD);
	if (pWndWorld)
		pWndWorld->m_dwPlayerSiegeTime = dwTime + GetTickCount();
}

void CDPClient::OnUpdatePlayerSiege(CAr& ar)
{
	u_int nCount;
	ar >> nCount;
	g_War.m_vecWarPlayers.clear();
	...
	{
		WAR_PLAYER pPlayer;
		ar.ReadString(pPlayer.m_szName);
		ar >> pPlayer.m_nKills;
		ar >> pPlayer.m_nDeaths;
		ar >> pPlayer.m_nPoints;
		g_War.m_vecWarPlayers.push_back(pPlayer);
	}
}

void CDPClient::SendJoinWarPlayer(BOOL bJoin, BOOL bMenu)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PLAYERWAR_JOIN, DPID_UNKNOWN);
	ar << bJoin << bMenu;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::SendRejoinSiege(u_long idPlayer)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PLAYERWAR_JOIN, DPID_UNKNOWN);
	ar << idPlayer << FALSE;
	SEND(ar, this, DPID_SERVERPLAYER);
...
void CDPClient::OnAddFightState(CAr& ar)
{
	BOOL bFightState;
	ar >> bFightState;
	g_pPlayer->SetSiegeFightState(bFightState);
}

void CDPClient::OnSiegeRejoinWindow(CAr& ar)
{
	BOOL bFightState;
	DWORD dwTime;
	ar >> bFightState >> dwTime;

	if (g_pPlayer->GetSiegeFightState() != bFightState)
		g_pPlayer->SetSiegeFightState(bFightState);
}

void CDPClient::SendGetSiegeRanking(u_long idPlayer, BOOL bFightRanking)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PLAYERWAR_RANKING, DPID_UNKNOWN);
	ar << bFightRanking;
	SEND(ar, this, DPID_SERVERPLAYER);
}
#endif //  __ROYAL_RUMBLE
...
#ifdef __AEGON_AUTO_STATS
void CDPClient::SendUpdateAutoStats(int nSelectedAutoStat)
{
	if (nSelectedAutoStat < 0 || nSelectedAutoStat > 4)
	{
		g_WndMng.PutString("invalid auto stat value", 0, COLOR_ERROR);
		return;
	}
	if (IsValidObj(g_pPlayer))
	{
		BEFORESENDSOLE(ar, PACKETTYPE_UPDATE_AUTO_STATS, DPID_UNKNOWN);
		ar << nSelectedAutoStat;;
		SEND(ar, this, DPID_SERVERPLAYER);
	}
}
void CDPClient::OnUpdateAutoStats(CAr& ar)
{
	int nSelectedAutoStat;
	ar >> nSelectedAutoStat;
	if (IsValidObj(g_pPlayer))
	{
		g_pPlayer->m_nAutoStat = nSelectedAutoStat;
	}
}
#endif

#ifdef __AEGON_F_TOOL
void CDPClient::SendUpdateFTools()
{
	if (IsValidObj(g_pPlayer))
	{
		BEFORESENDSOLE(ar, PACKETTYPE_UPDATE_FTOOLS, DPID_UNKNOWN);
		ar << static_cast<unsigned long>(g_pPlayer->m_vFToolUnits.size());
		for (int i = 0; i < g_pPlayer->m_vFToolUnits.size(); i++) 
		{
			g_pPlayer->m_vFToolUnits[i].Write(ar);
		}
		SEND(ar, this, DPID_SERVERPLAYER);
	}
}
void CDPClient::OnUpdateFTool(CAr& ar)
{
	if(IsValidObj(g_pPlayer))
		g_pPlayer->m_vFToolUnits.clear();
	unsigned long nSize;
	ar >> nSize;
	for (int i = 0; i < nSize; i++) 
	{
		...
		tmpUnit.Read(ar);
		tmpUnit.TimeCheck();
		if (tmpUnit.IsValid() && IsValidObj(g_pPlayer))
		{
...
void CDPClient::SendRemoveSkillBuff(DWORD dwSkillBuff)
{
	BEFORESENDSOLE(ar, PACKETTYPE_REMOVE_SKILL_BUFF, DPID_UNKNOWN);
	ar << dwSkillBuff;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __SYS_REBIRTH
void CDPClient::OnRebirthLevel(OBJID objid, CAr& ar) const
{
	CMover* pMover = prj.GetMover(objid);
	USHORT nLevel;
	...
	{
		// Set Rebirth Level
		pMover->SetRebirthLevel(nLevel);

		// Update Discord RPC
#if defined (__DISCORD_RPC) && defined (__SYS_REBIRTH)
		if (pMover->GetWorld())
		{
			g_Discord->Update(pMover->GetName(), pMover->GetWorld()->m_szWorldName, pMover->GetJob(), pMover->GetLevel(), pMover->GetRebirthLevel());
		}
#endif //__DISCORD_RPC
	}
}
...
#ifdef __AEGON_TIME_DIFF
void CDPClient::SendUpdateTimeDiff()
{
	SendHdr(PACKETTYPE_UPDATE_TIME_DIFF);
}
void CDPClient::OnUpdateTimeDiff(CAr& ar)
{
	time_t st;
	ar >> st;
	st += 60;
	time_t diff = st - time_null();

	if (diff < 120 && diff > -120)
		g_WndMng.m_nTimeDiff = 0;
	else
		g_WndMng.m_nTimeDiff = diff / 3600;
}
#endif

#ifdef __NEW_SFX_WINGS
void CDPClient::OnChangeWingSfx(OBJID objId, CAr& ar)
{
	DWORD dwSfxId;
	ar >> dwSfxId;
	CMover* pMover = prj.GetMover(objId);
	if (IsValidObj(pMover))
	{
		pMover->SetWingSfx(dwSfxId);
	}
}
#endif //__NEW_SFX_WINGS

#ifdef __FL_BAG_LOOT
void CDPClient::OnAddSelectedBag(CAr& ar)
{
	int nSelectedBag;
	ar >> nSelectedBag;

	if (g_pPlayer && nSelectedBag >= 0 && nSelectedBag <= 3)
		g_pPlayer->m_nSelectedBag = nSelectedBag;
}
#endif //__FL_BAG_LOOT

#ifdef __AEGON_PVP_MINIGAMES
void CDPClient::SendPvPMinigameCreate(int nGameID)
{
	if (IsInvalidObj(g_pPlayer))
		return;

	BEFORESENDSOLE(ar, PACKETTYPE_PVPMINIGAME_CREATE, DPID_UNKNOWN);
	ar << nGameID;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnAddPvPMinigameInvitation(CAr& ar)
{
	u_long uChallengerId;
	int nGameID;
	LPCTSTR challengerName;
	char szNameBuf[MAX_NAME];

	ar >> uChallengerId >> nGameID;
	ar.ReadString(szNameBuf, MAX_NAME);


	const char* szChallengerName = szNameBuf;

	CMover* pMover = prj.GetUserByID(g_pPlayer->GetId());


	CMover* pPlayer = prj.GetUserByID(uChallengerId);

	if (IsValidObj(pPlayer))
	{
		szChallengerName = pPlayer->GetName();
	}

	//replace with extra option
	if (g_Option.m_bTrade == FALSE) 
	{
		SendBlock(3, g_pPlayer->GetName(), szChallengerName);
		return;
	}
	...
	if (pWndBaseBuf)
	{
		SendBlock(4, g_pPlayer->GetName(), szChallengerName);
		return;
	}
	...
	if (pWndBaseBuf)
	{
		SendBlock(5, g_pPlayer->GetName(), szChallengerName);
		return;
	}
	...
	if (pWndBaseBuf)
	{
		SendBlock(8, g_pPlayer->GetName(), szChallengerName);
		return;
	}
	...
	//new invite window
	if (g_WndMng.m_pWndAegonPvPGameInvitation) 
	{
		SendBlock(5, g_pPlayer->GetName(), szChallengerName);
		return;
	}
	...
	g_WndMng.m_pWndAegonPvPGameInvitation = new CWndAegonPvPGameInvitation;
	g_WndMng.m_pWndAegonPvPGameInvitation->Initialize();
	g_WndMng.m_pWndAegonPvPGameInvitation->OnSetInfo(szChallengerName, uChallengerId, nGameID);
}
void CDPClient::OnAddPvPMinigameRefresh(CAr& ar)
{
	int nGameID = 0;
	ar >> nGameID;

	CAegonPvPMinigameBase* pGame = CAegonPvPMinigamesMng::GetInstance()->GetActiveGame();
	if (pGame) //refresh game state
	{
		pGame->ReadUpdateFromBuffer(ar);
	}
	else //start game
	{
		pGame = CAegonPvPMinigamesMng::GetInstance()->CreateNewGame(nGameID);
		pGame->ReadFromBuffer(ar);
		CAegonPvPMinigamesMng::GetInstance()->SetActiveGame(pGame);
	}

	if (pGame) 
	{
		SAFE_DELETE(g_WndMng.m_pWndAegonPvPGameCreate);
		SAFE_DELETE(g_WndMng.m_pWndAegonPvPGameFinder);

		if (!g_WndMng.m_pWndAegonPvPGameMain) 
		{
			g_WndMng.m_pWndAegonPvPGameMain = new CWndAegonPvPGameMain;
			g_WndMng.m_pWndAegonPvPGameMain->Initialize();
		}

		g_WndMng.m_pWndAegonPvPGameMain->OnGameRefresh();
	}

}
void CDPClient::SendPvPMinigameCommand(__MINIGAME_COMMAND sCommand, char* szName)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PVPMINIGAME_COMMAND, DPID_UNKNOWN);
	ar << sCommand;
...
void CDPClient::SendPvPMinigameChat(CString* strMessage)
{
	BEFORESENDSOLE(ar, PACKETTYPE_PVPMINIGAME_CHAT, DPID_UNKNOWN);
	ar.WriteString(strMessage->GetString());
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnAddPvPMinigameChat(CAr& ar)
{
	BOOL bSystem;
	char szMessage[1024];
	...
	if (g_WndMng.m_pWndAegonPvPGameMain) 
	{
		g_WndMng.m_pWndAegonPvPGameMain->AddChatMessage(szMessage, bSystem);
	}
}
void CDPClient::SendPvPMinigameGetList()
{
	BEFORESENDSOLE(ar, PACKETTYPE_PVPMINIGAME_GET_LIST, DPID_UNKNOWN);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnAddPvPMinigameList(CAr& ar)
{
	CAegonPvPMinigamesMng::GetInstance()->ReadMinigameList(ar);
}
#endif
...
void CDPClient::OnOpenSelectTmpWnd(CAr& ar)
{
	int nType;
	ar >> nType;
	if (!g_WndMng.m_WndSelectTmpClass)
	{
...
void CDPClient::OnSerializeInvAndBuffs(CAr& ar)
{
	...
	if (pMover)
	{
		pMover->m_Inventory.Serialize(ar);
		pMover->m_buffs.Serialize(ar);
...
#ifdef __LOOKCHANGE /// CHANGE IT TO YOUR DEFINE !!!!!! (if you have this system and this system have other #define in your source)
...
#ifdef __AEGON_IMPROVED_SHOPDESIGN
void CDPClient::OnAddVendorShopDesign(CAr& ar)
{
	OBJID objid;
	DWORD dwShopDesign;
	ar >> objid;
	...
	pVendor->m_dwShopDesign = dwShopDesign;
}
#endif

#ifdef __AEGON_MESSENGER
void CDPClient::SendAddMessengerStatus()
{
	...
	BEFORESENDSOLE(ar, PACKETTYPE_MESSENGER_STATUS_UPDATE, DPID_UNKNOWN);
	ar << static_cast<unsigned long>(pOpenChats->size());
	for (int i = 0; i < pOpenChats->size(); i++)
	{
		ar.WriteString((*pOpenChats)[i]->m_strName);
	}
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnAddMessengerStatus(CAr& ar)
{
	vector<__AEGON_CHAT_UNIT*>* pOpenChats = CAegonMessengerMng::GetInstance()->GetOpenChats();
	...
	for (int i = 0; i < nSize; i++) 
	{
		...
		if (pOpenChats && i >= 0 && i < pOpenChats->size())
		{
			(*pOpenChats)[i]->m_nLastStatus = nStatus;
		}
	}
}
void CDPClient::SendAddMessengerChatUnit(char* szName)
{
	BEFORESENDSOLE(ar, PACKETTYPE_MESSENGER_ADD_CHAT_UNIT, DPID_UNKNOWN);
	ar.WriteString(szName);
	SEND(ar, this, DPID_SERVERPLAYER);
}
void CDPClient::OnAddMessengerChatUnitResult(CAr& ar)
{
	BOOL bExists;
	ar >> bExists;
	...
	CWndAegonMessenger* pWndMessenger = g_WndMng.OpenMessenger();
	if (bExists && strlen(szName) > 0)
	{
		if (pWndMessenger && IsValidObj(g_pPlayer))
		{
			pWndMessenger->AddChatUnit(szName, __AEG_CHAT_STATUS_OFFLINE);
			pWndMessenger->SwitchToChat(szName);
		}
		else
		{
			CString strSay;
			strSay.Format("/say %s", szName);
			SendChat(strSay.GetString());
		}
	}
	else 
	{
		CString str;
		str.Format(prj.GetText(TID_GAME_MSGNOTEXIST), szName);
		g_WndMng.PutString(str, NULL, prj.GetTextColor(TID_GAME_MSGNOTEXIST));
	}
}
#endif
...
#ifdef __SYS_LOCK_ITEM
void CDPClient::OnAddItemLock(CAr& ar)
{
	OBJID objid;
	bool bIsLocked;
	ar >> objid;
	...
	CItemElem* pItem = g_pPlayer->m_Inventory.GetAtId(objid);
	if (pItem) 
	{
		pItem->SetItemLock(bIsLocked);
	}
}
#endif

#ifdef __SYS_LOCK_ITEM
void CDPClient::OnAddCollectingStatus(CAr& ar)
{
	int nStatus;
	ar >> nStatus;
	...
	if (g_WndMng.m_pWndCollecting) 
	{
		...
		{
		case 0:
			g_WndMng.m_pWndCollecting->m_bIsCollecting = FALSE;
			break;
		case 1:
			g_WndMng.m_pWndCollecting->m_bIsCollecting = TRUE;
			break;
		}
...
#ifdef __STOP_EXP
void CDPClient::OnAddExpStopState(CAr& ar)
{
	BOOL bState;
	ar >> bState;

	g_Option.m_bExpStop = bState;

	/*
	if (g_Option.m_bExpStop) {
		g_WndMng.PutString("You've blocked your experience gain.", NULL, COLOR_WARNING);
	}
	else {
		g_WndMng.PutString("You are now gaining experience again.", NULL, COLOR_SUCCESS);
	}
	*/

	if (IsValidObj(g_pPlayer)) 
	{
		g_pPlayer->SetExpStop(bState);
	}
}
#endif

#ifdef __AEGON_BAG_FREE_MOVE
void CDPClient::SendMovePocketItem(int nPocket, int nSrcIndex, int nDestIndex)
{
	BEFORESENDSOLE(ar, PACKETTYPE_MOVE_POCKET_ITEM, DPID_UNKNOWN);
	ar << nPocket << nSrcIndex << nDestIndex;
	SEND(ar, this, DPID_SERVERPLAYER);
}

void CDPClient::OnMovePocketItem(CAr& ar)
{
	int nPocket, nSrcIndex, nDestIndex;
	ar >> nPocket >> nSrcIndex >> nDestIndex;
	...
	if (IsValidObj(g_pPlayer))
	{
		...
		if (nPocket == 0 && (nDestIndex >= POCKET_SLOT_SIZE_0 || nSrcIndex >= POCKET_SLOT_SIZE_0))
			return;
		else if (nPocket > 0 && (nDestIndex >= POCKET_SLOT_SIZE_1_2 || nSrcIndex >= POCKET_SLOT_SIZE_1_2))
			return;

		if (nDestIndex < 0 || nSrcIndex < 0)
			return;

		if (!g_pPlayer->m_Pocket.IsAvailable(nPocket, TRUE))
			return;

		CPocket* pPocket = g_pPlayer->m_Pocket.GetPocket(nPocket);
		if (!pPocket || pPocket->IsExpired())
			return;

		pPocket->Swap(nSrcIndex, nDestIndex);
	}
}
#endif
