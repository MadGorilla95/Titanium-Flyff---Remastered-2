...
//	void	SendTradeClearGold( void );//	{	SendHdr( PACKETTYPE_TRADECLEARGOLD );	}
...
#ifdef __VENDING_NPC
	void	SendPVendorClose(OBJID objidVendor, BOOL bClearShop);
#else // __VENDING_NPC
	void	SendPVendorClose(OBJID objidVendor);
#endif // __VENDING_NPC

#ifdef __CHIP_PLAYERSHOP
	void	SendRegisterPVendorItem(BYTE iIndex, BYTE nType, BYTE nId, short nNum, int nCost, int nMode);
	...
	void	SendSetPartyAssistant(u_long uLeaderId, u_long uAssist);
	...
	void	SendQuickSell(vector<DWORD> vecItems, BOOL bUppedItems, BOOL bStattedItems, BOOL bAwakedItems, BOOL bRaretyItems, BOOL bElementItems);
#else
	void	SendQuickSell(vector<DWORD> vecItems);
#endif
...
#ifdef __RANGE_FIX
	void	SendUseSkill(WORD wType, WORD wId, OBJID objid, DWORD dwRange, float fRange, int nUseType = 0, int bControl = FALSE, int nActionSlot = -1);
#else //__RANGE_FIX
	void	SendUseSkill(WORD wType, WORD wId, OBJID objid, int nUseType = 0, int bControl = FALSE, int nActionSlot = -1);
#endif //__RANGE_FIX
...
#ifdef __STOP_EXP
	void	SendSetExpStop(BOOL nState);
#endif //__STOP_EXP
...
//________________________________________________________________________________
...
#ifdef __FL_BAG_LOOT
	void	SendSelectedBag(int nSelect);
#endif //__FL_BAG_LOOT
#endif	// __SYS_POCKET
#ifdef __JEFF_11
	...
	void	SendGuildSetColor(int R, int G, int B);
	void	OnGuildSetColor(CAr& ar);
#endif //__SYS_GUILDCOLOR

#ifdef __SYS_NAMECOLOR
	void	SendSetNameColor(int R, int G, int B);
	void	OnSetNameColor(OBJID objid, CAr& ar);
...
//________________________________________________________________________________
	...
	void	SendDoUseItemBag(DWORD dwItemId, OBJID objid, int nBag);
	...
	void SendUltimateRemoveGem(OBJID objItemWeapon, OBJID objItemGem, int nSlot);
	...
	void SendCreateAngel(int nSuns, int nMoons);
...
public:
	...
	void	SendNPCBuffClass(const char* szKey);
...
public:
	void	SendTeleportToHeavenTower( int nUnitID );
private:
...
public:
	...
	void SendLockItem(const DWORD objId, bool bIsLocked);
#endif // __SYS_LOCK_ITEM

#ifdef __WIKIPEDIA_ITEMS
	void SendWikiCreateItem(CItemElem* pItemElem, LPCTSTR szName);
#endif //__WIKIPEDIA_ITEMS

#ifdef __AEGON_PETMISTAKE
	void	SendPetTamerMistakeCustom(DWORD dwId);
#endif // __AEGON_PETMISTAKE

#ifdef __AEGON_MONSTER_HUNT
	void	SendLoadMonsterHuntProgress();
	void    OnLoadMHProgress(CAr& ar);
#endif // __AEGON_MONSTER_HUNT

#ifdef __LEEAEG_SHOP_FINDER
	void	SendShopItemTele(DWORD itemIndex);
	void	SendShopItemRefresh();
	void	OnShopFinderRefresh(CAr& ar);
#endif // __LEEAEG_SHOP_FINDER

#ifdef __GIFTBOX_LIST_ITEMS
	void SendQueryGBPreview(DWORD dwID);
	void OnGBPreview(CAr& ar);
#endif //__GIFTBOX_LIST_ITEMS

#ifdef __AEGON_RETURN_AWAKE
	void	SendReturnAwakening(DWORD dwItem, DWORD dwMaterial);
#endif // __AEGON_RETURN_AWAKE

#ifdef __VENDING_NPC
	void    SendCreateVendor(DWORD dwItemId, D3DXVECTOR3 vPos, FLOAT fAngle, char* szVendingTitle);
	void    SendUnlinkVendorNpc();
	void    OnVendorNPCLink(CAr& ar);
	void    OnVendorNPCUnlink(CAr& ar);
#endif // __VENDING_NPC

#ifdef __AEGON_CONVERT_PERIN
	void SendAegonConvertPerin();
	...
	void	SendUpdateWeaponRarity(DWORD dwItem1, DWORD dwItem2);
	void	OnWeaponRarity(CAr& ar);
	...
	void	OnUpdateTitle(OBJID objid, CAr& ar);
	void	SendUpdateShowTitle(BOOL bState);
	void	SendCreateOwnTitle(CString str, BOOL b = FALSE);
#endif // __SYS_CUSTOM_TITLE

#ifdef __PREMIUM
	void	OnUpdatePremium(OBJID objid, CAr& ar);
	void	SendUpdateShowPremium(BOOL bState);
#endif //__PREMIUM

#ifdef __MOUNT
	void	OnUpdateMount(OBJID objid, CAr& ar);
	void 	SendUpdateMount(DWORD dwID);
#endif // __MOUNT

#ifdef __HEAL_SHOW
	void	OnHealDamage(OBJID objid, CAr& ar);
#endif // __HEAL_SHOW
	...
	void	SendAnarchyBuffs(int buffs, int nMode);
#endif // __ANARCHY_SYSTEM

#ifdef __AEGON_TELEPORTER
	void    SendTeleportByApp(int nUnitID);
#endif

#ifdef __AEGON_LOOT_BOX
	void	SendLootBoxStart(int nSelectedBox, DWORD ticketObjID, int nBuyMode);
	void	OnLootboxResult(CAr& ar);
	void	SendLootBoxCollect();
#endif // __AEGON_LOOT_BOX

#ifdef __ADVANCED_MAIL
	void	SendQueryGetAndDeleteMail(u_long nMail, BOOL bItem, DWORD dwGold);
	void	SendQueryGetAndDeleteMailAll();
#endif // __ADVANCED_MAIL

#ifdef __AEGON_VIEW_INV
	void	SendViewInventory(u_long uidPlayer, int nContainer);
	void	OnViewInventory(OBJID objid, CAr& ar);
	void	OnViewBag(OBJID objid, CAr& ar);
	void	OnViewBank(OBJID objid, CAr& ar);

	void	SendViewInventoryCommand(CItemElem* commandItem, int nCommand, int nContainer, int nContainerField = -1);
#endif // __AEGON_VIEW_INV

#ifdef __SYS_PETFILTER
	void	SendPlayerPetfilter();
#endif //__SYS_PETFILTER

#ifdef __BADGE
	void	SendChangeBadgeIcon(int nChange);
	void	OnBadgeListUpdate(CAr& ar);
	void	OnBadgeChange(OBJID objid, CAr& ar);
#endif //__BADGE

#ifdef __AEGON_INGAME_DP
	void	OnDPRefresh(OBJID objid, CAr& ar);
#endif // __AEGON_INGAME_DP

#ifdef __GUILDFINDER
	void	OnRefreshGuildList(CAr& ar);
	void	OnGuildRequest(u_long uGuildid, u_long uMasterid);
	void	OnGuildFinderAppication(CAr& ar);
	void	OnGuildFinderOpt(CAr& ar);
	void	SendGuildFinder(BOOL bGuildFinder);
#endif //__GUILDFINDER

#ifdef	__REMOVE_DURATION_ITEMS
	void	SendRemoveCmm(DWORD nItem);
#endif	// __REMOVE_DURATION_ITEMS

#ifdef __SYS_MONSTER_CLASH
	void	SendMonsterClashRegisterMember(u_long idPlayer);
	void	SendMonsterClashUnRegisterMember(u_long idPlayer);
	void	OnMonsterClashRegisterWindow(CAr& ar);
	void	OnMonsterClashRegisterMemberWindow(CAr& ar);
	void	OnMonsterClashRegister(CAr& ar);
	void	OnMonsterClashRegisterMember(CAr& ar);
	void	OnMonsterClashUnRegisterMember(CAr& ar);
	void	OnMonsterClashState(CAr& ar);
#endif //__SYS_MONSTER_CLASH

#ifdef __REPORT_SYSTEM
	void	SendReport(BYTE nReportType, char* lpszText);
#endif // __REPORT_SYSTEM

#ifdef __SYS_SANDBOX
	void	SendSandboxSpawn(DWORD dwIndex, BOOL bDamage);
	void	SendSandboxClear();
	void	SendSandboxTeleport();
#endif //__SYS_SANDBOX

#ifdef __SHOPS_VISITED
	void	OnRemoveShop(CAr& ar);
#endif //__SHOPS_VISITED

#ifdef __TICKETS_DETAILS
	void   LayersDetailReceived(OBJID objid, CAr& ar);
...
#ifdef __CHECKLOAD
	void	GetCheckLoad(const unsigned long worldId, const int layer);
	void	OnCheckLoad(CAr& ar);
#endif //__CHECKLOAD
	...
	void	SendLoadAchievementProgress();
	void	OnLoadAchievementProgress(CAr& ar);
	void	SendCheckAchievementProgress(int mode, int msg);
	void	OnFinishedAchievement(CAr& ar);
	void	SendCollectAchievement(int achievementID);
	void	SendAchievementFavorite(int achievementID);
	void	OnFavoriteAchievementRefresh(CAr& ar);
	void	OnCollectingTimeRefresh(CAr& ar);
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __UPGRADE_SUCCESS
	void	OnUpgradeSuccess(CAr& ar);
#endif //__UPGRADE_SUCCESS

#ifdef __AFK_MONITORING
	void	OnSetAFKState(OBJID objid, CAr& ar);
#endif //__AFK_MONITORING

#ifdef __AEGON_FULLSCREEN_AND_RESIZE
	void	SendReloadTaskbar();
#endif // __AEGON_FULLSCREEN_AND_RESIZE

#ifdef __AEGON_LAST_ONLINE
	void	OnLastOnlineRefresh(CAr& ar);
	void	OnSingleLastOnlineRefresh(CAr& ar);
	void	OnOwnLastOnlineRefresh(CAr& ar);
#endif // __AEGON_LAST_ONLINE

#ifdef __AEGON_INGAME_TIME
	void	OnOnlineTimeRefresh(CAr& ar);
#endif // __AEGON_INGAME_TIME

#ifdef __PLAYER_LIST
	void	SendPlayerListRequest(BOOL bCheck, BOOL bCheck1, BOOL bCheck2, BOOL bCheck3, BOOL bCheck4, BOOL bCheck5, BOOL bCheck6, BOOL bCheck7, BOOL bCheck8, BOOL bCheck9, BOOL bCheck10, BOOL bCheck11, BOOL bCheck12, BOOL bCheck13, BOOL bCheck14, BOOL bCheck15, BOOL bCheck16, int nMin, int nMax, CString search);
	void	OnPlayerList(CAr& ar);
#endif // __PLAYER_LIST

#ifdef __AEGON_CRAFTING_SYSTEM
	void	SendCraftRecipe(__CRAFTING_UNIT* m_selectedUnit, CItemElem* componentItems[AEGCRAFT_MAXCOMPONENTS]);
	void	OnCraftingSuccess(CAr& ar);
	void	OnCraftingExpRefresh(CAr& ar);
#endif // __AEGON_CRAFTING_SYSTEM

#ifdef __WEAPON_MERGE
	void	SendWeaponMerge(DWORD dwIdSrc, DWORD dwIdDest, DWORD dwIdProt);
	void	SendWeaponMergeCancel(DWORD dwIdItem);
	void	OnMergedWeapon(CAr& ar);
	...
	void	OnOnlineChange(CAr& ar);
#endif //__CLIENT_ONLINE_COUNT

#ifdef __AEGON_SEASON_PASS
	void	SendLoadSeasonPassProgress();
	void	OnLoadSeasonPassProgress(CAr& ar);
	void	OnUpdateSeasonPassProgress(CAr& ar);
	void	SendClaimSeasonPassReward(int passID, int rewardID);
	void	SendGetSeasonPassCountdown();
	void	OnSeasonPassCountdown(CAr& ar);
	void	SendPurchaseSeasonPass(BOOL bPvE, int nPassID);
#endif // __AEGON_SEASON_PASS

#ifdef __AEGON_ADVANCED_ANNOUNCEMENT
	void	SendAegonSystemShout(LPCSTR szString, BOOL bShowName, BOOL bShowAuth, int nImage, int nFont);
#endif // __AEGON_ADVANCED_ANNOUNCEMENT

#ifdef __SWITCH_EQUIP
	void	OnEquipSwitch(CAr& ar);
	void	SendEquipSwitch();
	void	SendUseEquipSwitch(int nIndex);
#endif // __SWITCH_EQUIP

#ifdef __AEGON_PVP_RANKING
	void	SendGetPvPRanking(int nMode);
	void	OnAddPvPRanking(CAr& ar);
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_PERMANENT_BUFF
	void	OnAddPermBuffRefresh(CAr& ar);
#endif // __AEGON_PERMANENT_BUFF

#ifdef __AEGON_PREM_MOTIONS
	void	OnAddMotionRefresh(CAr& ar);
	void	SendPurchaseMotion(DWORD dwMotionID);
#endif // __AEGON_PREM_MOTIONS

#ifdef __PET_GLOW
	void	OnChangePetGlowSfx(OBJID objId, CAr& ar);
	void	OnChangePetGlow(DWORD m_dwObjId, int m_nSelected);
#endif // __PET_GLOW

#ifdef __MODS_MANAGER
	void	SendModsManager(BYTE nMode);
...
#ifdef __COMBAT_LOG
	void	OnCombatLogReset(CAr& ar);
#endif //__COMBAT_LOG

#ifdef __AEGON_EMOTE_SYSTEM
	void	OnAddEmoteRefresh(CAr& ar);
	void	SendPurchaseEmote(DWORD dwEmoteID);
#endif //__AEGON_EMOTE_SYSTEM

#ifdef __CRYSTAL
	void	SendUpdateCrystal(DWORD dwIdSrc, DWORD dwIdDest);
	void	OnCrystal(CAr& ar);
#endif // __CRYSTAL

#ifdef __AEGON_INGAME_SHOP
	void	OnAddIngameShopInfo(CAr& ar);
	void	SendWebshopPurchase(DWORD dwUnitID, int nAmount);
#endif //__AEGON_INGAME_SHOP

#ifdef __AEGON_TOURNAMENT
	void	SendCreateTournament(__TOURNAMENT_UNIT* pTournamentUnit);
	void	SendGetTournamentMap();
	void	OnTournamentMap(CAr& ar);
	void	OnTournamentsChanged(CAr& ar);
	void	SendDeleteTournament(DWORD dwTournamentID);
#endif //__AEGON_TOURNAMENT

#ifdef __ROYAL_RUMBLE
	void	OnPlayerSiege(CAr& ar);
	void	OnAddSiegeTime(CAr& ar);
	void	OnUpdatePlayerSiege(CAr& ar);
	void	SendJoinWarPlayer(BOOL bJoin, BOOL bMenu);
	void	OnEndState(CAr& ar);
	void	OnAddFightState(CAr& ar);
	void	OnSiegeRejoinWindow(CAr& ar);
	void	SendRejoinSiege(u_long idPlayer);
	void	OnSiegeRankingList(CAr& ar);
	void	SendGetSiegeRanking(u_long idPlayer, BOOL bFightRanking);
	void	OnUpdateSingleUser(CAr& ar);
#endif // __ROYAL_RUMBLE

#ifdef __AEGON_AUTO_STATS
	void SendUpdateAutoStats(int nSelectedAutoStat);
	void OnUpdateAutoStats(CAr& ar);
#endif

#ifdef __AEGON_F_TOOL
	void SendUpdateFTools();
	void OnUpdateFTool(CAr& ar);
#endif

#ifdef __REMOVE_SKILL_BUFF
	void	SendRemoveSkillBuff(DWORD dwSkillBuff);
#endif // __REMOVE_SKILL_BUFF

#ifdef __SYS_REBIRTH
	void OnRebirthLevel(OBJID objid, CAr& ar) const;
#endif //__SYS_REBIRTH

#ifdef __AEGON_TIME_DIFF
	void SendUpdateTimeDiff();
	void OnUpdateTimeDiff(CAr& ar);
#endif

#ifdef __NEW_SFX_WINGS
	void OnChangeWingSfx(OBJID objId, CAr& ar);
#endif //__NEW_SFX_WINGS

#ifdef __FL_BAG_LOOT
	void OnAddSelectedBag(CAr& ar);
#endif //__FL_BAG_LOOT

#ifdef __AEGON_PVP_MINIGAMES
	void SendPvPMinigameCreate(int nGameID);
	void OnAddPvPMinigameInvitation(CAr& ar);
	void OnAddPvPMinigameRefresh(CAr& ar);
	void SendPvPMinigameCommand(__MINIGAME_COMMAND pCommand, char* szName = NULL);
	void SendPvPMinigameChat(CString* strMessage);
	void OnAddPvPMinigameChat(CAr& ar);
	void SendPvPMinigameGetList();
	void OnAddPvPMinigameList(CAr& ar);
#endif

#ifdef __AEGON_IMPROVED_SHOPDESIGN
	void OnAddVendorShopDesign(CAr& ar);
#endif

#ifdef __AEGON_MESSENGER
	void SendAddMessengerStatus();
	void OnAddMessengerStatus(CAr& ar);

	void SendAddMessengerChatUnit(char* szName);
	void OnAddMessengerChatUnitResult(CAr& ar);
#endif

#ifdef __SYS_LOCK_ITEM
	void OnAddItemLock(CAr& ar);
#endif

#ifdef __SYS_LOCK_ITEM
	void OnAddCollectingStatus(CAr& ar);
#endif

#ifdef __STOP_EXP
	void OnAddExpStopState(CAr& ar);
#endif

#ifdef __AEGON_BAG_FREE_MOVE
	void SendMovePocketItem(int nPocket, int nSrcIndex, int nDestIndex);
	void OnMovePocketItem(CAr& ar);
#endif
