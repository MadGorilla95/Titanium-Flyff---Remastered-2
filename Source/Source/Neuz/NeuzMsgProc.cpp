...
FARPROC WINAPI delayHook(const unsigned dliNotify, PDelayLoadInfo pdli)
{
	if (pdli)
	{
		...
		{
#ifdef __fmod
			if (_strcmpi(pdli->szDll, "fmod.dll") == 0)
#if _WIN64
				return reinterpret_cast<FARPROC>(LoadLibrary("fmod_x64.dll"));
#else
				return reinterpret_cast<FARPROC>(LoadLibrary("fmod_x86.dll"));
#endif
#endif
...
//	char szCachePort[16];
	...
	g_Neuz.Key.chBack = 'S';
	g_Neuz.Key.chRight = 'D';
#endif // __CHANGE_HOT_KEY

#ifdef __DPS_METER
	g_Neuz.Key.chDpsMeter = VK_F11;//'C';
#endif
}
...
int APIENTRY WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	if (lstrcmp(g_Neuz.m_lpCertifierAddr, "127.0.0.1") == 0)
		WEB_ADDRESS_DEFAULT = "http://localhost/shop?is_game=1&m_idPlayer=%d&m_nServer=%d";
	else
		WEB_ADDRESS_DEFAULT = "https://titanium-mmo.com/shop?is_game=1&m_idPlayer=%d&m_nServer=%d";
	...
	CAegonEmoticonMng::GetInstance()->LoadAllEmotes();
