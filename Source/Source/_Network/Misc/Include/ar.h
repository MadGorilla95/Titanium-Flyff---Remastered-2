...
public:
	...
	void skip(const size_t amount = sizeof(bool))
	{
		if (m_lpBufCur + amount > m_lpBufMax)
...
{
#if _WIN64
	CheckBuf(sizeof(unsigned long) + static_cast<unsigned long>(v.size()) * sizeof(T));
	*this << static_cast<unsigned long>(v.size());
#else
...
{
#if _WIN64
	CheckBuf(sizeof(unsigned long) + static_cast<unsigned long>(m.size()) * sizeof(T) * sizeof(U));
	*this << static_cast<unsigned long>(m.size());
#else
...
{
#if _WIN64
	operator<<(static_cast<unsigned long>(_Container.size()));
#else
