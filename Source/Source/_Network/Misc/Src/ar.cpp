// String format:
//      UNICODE strings are always prefixed by 0xff, 0xfffe
//      if < 0xff chars: len:BYTE, TCHAR chars
//      if >= 0xff characters: 0xff, len:WORD, TCHAR chars
//      if >= 0xfffe characters: 0xff, 0xffff, len:DWORD, TCHARs

#include "StdAfx.h"
#include "Ar.h"

#ifdef __VM_0819
CHeapMng*	CAr::m_pHeapMng		= new CHeapMng( "CAr" );
#else	// __VM_0819
CHeapMng*	CAr::m_pHeapMng		= new CHeapMng;
#endif	// __VM_0819

CAr::CAr( void* lpBuf, u_int nBufSize )
{
	if( lpBuf )
	{
		m_nMode		= load;
		m_lpBufStart	= (LPBYTE)lpBuf;
		m_nBufSize	= nBufSize;
	}
	else
	{
		m_nMode		= store;
		m_lpBufStart	= m_lpBuf;
		m_nBufSize	= nGrowSize;
	}
	m_lpBufMax	= m_lpBufStart + m_nBufSize;
	m_lpBufCur	= m_lpBufStart;
}

CAr::~CAr()
{
	if( IsStoring() && ( m_nBufSize > nGrowSize ) )
	{
#ifdef __VM_0819
		CAr::m_pHeapMng->Free( m_lpBufStart, m_nBufSize );
#else	// __VM_0819
		CAr::m_pHeapMng->Free( m_lpBufStart );
#endif	// __VM_0819
	}
}

void CAr::Read( void* lpBuf, u_int nSize )
{
	if( nSize == 0 )
		return;

	ASSERT( IsLoading() );
	ASSERT( lpBuf );

#ifdef __AEGON_FIXES
	if (!IsLoading())
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
	if (!lpBuf)
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
#endif

	if( m_lpBufCur + nSize <= m_lpBufMax )
	{
		memcpy( lpBuf, m_lpBufCur, nSize );
		m_lpBufCur	+= nSize;
	}
	else	// overflow
	{
		memset( lpBuf, 0, nSize );
		m_lpBufCur	= m_lpBufMax;
	}
}

void CAr::Write( const void* lpBuf, u_int nSize )
{
	if( nSize == 0 )
		return;
	
	ASSERT( IsStoring() );
	ASSERT( lpBuf );

#ifdef __AEGON_FIXES
	if (!IsStoring())
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
	if (!lpBuf)
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
#endif
	
	CheckBuf( nSize );

	memcpy( m_lpBufCur, lpBuf, nSize );
	m_lpBufCur	+= nSize;
}

// special functions for text file input and output
void CAr::WriteString( LPCTSTR lpsz )
{
	int nLen	= _tcslen( lpsz );
	*this << nLen;
	Write( lpsz, sizeof(TCHAR) * nLen );
}

LPTSTR CAr::ReadString( LPTSTR lpsz )
{
	int nLen;
	*this >> nLen;
	Read( lpsz, sizeof(TCHAR) * nLen );
#if defined(__CLIENT) || defined(__DBSERVER)
	for( int i=0;i<nLen;i++ )
#ifdef __CLIENT
		if( lpsz[i] == '\x60' )
			lpsz[i] = '\x27';
#else // __CLIENT
#ifdef __DBSERVER
		if( lpsz[i] == '\x27' )
			lpsz[i] = '\x60';
#endif // __DBSERVER
#endif // __CLIENT
#endif // defined(__CLIENT) || defined(__DBSERVER)
	lpsz[nLen] = '\0';
	return lpsz;
}

LPTSTR CAr::ReadString( LPTSTR lpsz, int nBufSize )
{
	int nLen;
	*this >> nLen;

	int nReadable	= nBufSize - 1;
	if( nLen > nReadable || nLen < 0 )
	{
		memset( (void*)lpsz, 0, nBufSize );
		m_lpBufCur	= m_lpBufMax;
		return lpsz;
	}
	else
	{
		Read( lpsz, sizeof(TCHAR) * nLen );
#if defined(__CLIENT) || defined(__DBSERVER)
		for( int i=0;i<nLen;i++ )
#ifdef __CLIENT
			if( lpsz[i] == '\x60' )
				lpsz[i] = '\x27';
#else // __CLIENT
#ifdef __DBSERVER
			if( lpsz[i] == '\x27' )
				lpsz[i] = '\x60';
#endif // __DBSERVER
#endif // __CLIENT
#endif // defined(__CLIENT) || defined(__DBSERVER)

		lpsz[nLen] = '\0';
		return lpsz;
	}
}

void CAr::Reserve( u_int nSize )
{
	ASSERT( IsStoring() );
	ASSERT( m_lpBufCur == m_lpBufStart );

#ifdef __AEGON_FIXES
	if (!IsStoring())
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
	if (m_lpBufCur != m_lpBufStart)
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
#endif

	if( nSize <= nGrowSize )
		return;
//	LPBYTE lpBuf	= (LPBYTE) heapAlloc( nSize );
	LPBYTE lpBuf	= (LPBYTE)CAr::m_pHeapMng->Malloc( nSize );
	m_lpBufStart	= lpBuf;

	ASSERT( m_lpBufStart );
	m_nBufSize	= nSize;
	m_lpBufCur	= m_lpBufStart;
	m_lpBufMax	= m_lpBufStart + m_nBufSize;
}

void CAr::CheckBuf( u_int nSize )
{
#ifdef __AEGON_FIXES
	if (!m_lpBufMax) 
	{
		m_lpBufMax = m_lpBufStart + m_nBufSize;
		Error("%s %s %d m_lpBufMax was NULL now %p (m_lpBufStart: %p + m_nBufSize: %u)", __FILE__, __FUNCTION__, __LINE__, (void*)m_lpBufMax, (void*)m_lpBufStart, m_nBufSize);
	}
	if (m_nMode > CAr::load) 
	{
		Error("%s %d invalid ar m_nMode: %d", __FILE__, __LINE__, (int)m_nMode);
	}
#endif

	if( m_lpBufCur + nSize > m_lpBufMax )
	{
		u_int uOffset	= m_lpBufCur - m_lpBufStart;

//		u_int nExtension	= nGrowSize * ( nSize / nGrowSize + 1 );
		u_int nExtension	= m_nBufSize * 2;
		
		if( m_nBufSize > nGrowSize )
		{
#ifdef __VM_0819
			m_lpBufStart	= (LPBYTE)CAr::m_pHeapMng->Realloc( m_lpBufStart, m_nBufSize + nExtension, m_nBufSize );
#else	// __VM_0819
#if defined(__AEGON_FIXES) && defined(__WORLDSERVER)
			if (m_pHeapMng == (void*)-1 || m_lpBufStart == (void*)-1 || m_lpBufCur == (void*)-1 || m_lpBufMax == (void*)-1) 
			{
				Error("%s %s %d heap realloc failed - m_pHeapMng: %p - m_lpBufStart: %p - m_lpBufCur: %p - m_lpBufMax: %p - m_nBufSize: %u - nExtension: %u", __FILE__, __FUNCTION__, __LINE__, (void*)CAr::m_pHeapMng, (void*)m_lpBufStart, (void*)m_lpBufCur, (void*)m_lpBufMax, m_nBufSize, nExtension);
			}
#endif
			m_lpBufStart	= (LPBYTE)CAr::m_pHeapMng->Realloc( m_lpBufStart, m_nBufSize + nExtension );
#endif	// __VM_0819

// 			//	BEGINTEST
// 			Error( "m_nBufSize : %d, Realloc Size : %d", m_nBufSize, m_nBufSize + nExtension );
		}
		else
		{
			LPBYTE lpBuf	= (LPBYTE)CAr::m_pHeapMng->Malloc( m_nBufSize + nExtension );
			memcpy( lpBuf, m_lpBufStart, m_nBufSize );
			m_lpBufStart	= lpBuf;
		}

		ASSERT( m_lpBufStart );

#ifdef __AEGON_FIXES
		if (!m_lpBufStart)
		{
			Error("%s %d", __FILE__, __LINE__);
			return;
		}
#endif

		m_nBufSize	+= nExtension;
		m_lpBufCur	= m_lpBufStart + uOffset;
		m_lpBufMax	= m_lpBufStart + m_nBufSize;
	}
}

LPBYTE CAr::GetBuffer( int* pnBufSize )
{
	ASSERT( IsStoring() );
	ASSERT( pnBufSize );

#ifdef __AEGON_FIXES
	if (!IsStoring())
	{
		Error("%s %d", __FILE__, __LINE__);
		return NULL;
	}
	if (!pnBufSize) 
	{
		Error("%s %d", __FILE__, __LINE__);
		return NULL;
	}
#endif

	*pnBufSize	= m_lpBufCur - m_lpBufStart;
	return m_lpBufStart;
}

void CAr::Flush( void )
{
	ASSERT( IsStoring() );

#ifdef __AEGON_FIXES
	if (!IsStoring())
	{
		Error("%s %d", __FILE__, __LINE__);
	}
#endif
	m_lpBufCur	= m_lpBufStart;
}

void CAr::ReelIn( u_int uOffset )
{
	ASSERT( IsStoring() );
	ASSERT( m_lpBufStart + uOffset <= m_lpBufCur );

#ifdef __AEGON_FIXES
	if (!IsStoring()) 
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
	if (m_lpBufStart + uOffset > m_lpBufCur) 
	{
		Error("%s %d", __FILE__, __LINE__);
		return;
	}
#endif

#if 1
	if( m_nBufSize > nGrowSize )
	{
#ifdef __VM_0819
		CAr::m_pHeapMng->Free( m_lpBufStart, m_nBufSize );
#else	// __VM_0819
		CAr::m_pHeapMng->Free( m_lpBufStart );
#endif	// __VM_0819
		m_lpBufStart	= m_lpBuf;
		m_nBufSize	= nGrowSize;
		m_lpBufMax	= m_lpBufStart + m_nBufSize;
	}
#endif	// 1
	m_lpBufCur	= m_lpBufStart + uOffset;
}