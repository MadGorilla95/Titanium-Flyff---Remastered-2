...
#ifdef _WIN64
	if (!CreateIoCompletionPort((HANDLE)pChild->GetHandle(), GetCompletionPort(iIoWorker), (ULONG_PTR)hSocket, GetIoWorkerCount()))
#else
			...
			case ( WAIT_OBJECT_0+1 ) :
				{
					...
					{
#ifdef _WIN64
						PostQueuedCompletionStatus(pSock->GetCompletionPort(iIoWorker), NEWSOCKETMSG2, (ULONG_PTR)hSocket, (LPOVERLAPPED)dwAddr);
#else
					...
					{
#ifdef _WIN64
						PostQueuedCompletionStatus(pSock->GetCompletionPort(iIoWorker), NEWSOCKETMSG, (ULONG_PTR)hSocket, (LPOVERLAPPED)dwAddr);
#else
						PostQueuedCompletionStatus( pSock->GetCompletionPort( iIoWorker ), NEWSOCKETMSG, (DWORD)hSocket, (LPOVERLAPPED)dwAddr );
#endif
					}
