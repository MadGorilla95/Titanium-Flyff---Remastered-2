...
#endif	// __CRC
{
...
#ifdef _WIN64
	if (!CreateIoCompletionPort((HANDLE)pSock->GetHandle(), m_phCompletionPort[iIoWorker], (ULONG_PTR)(pSock->GetHandle()), 0))
#else
...
#ifdef _WIN64
	PostQueuedCompletionStatus(m_phCompletionPort[iIoWorker], NEWSOCKETMSG, (ULONG_PTR)pSock->GetHandle(), NULL);
#else
	...
	if( CSock::crcWrite & dwcrc )
	{
		if( WaitForSingleObject( m_hProtocolId, uWaitingTime ) == WAIT_TIMEOUT )
		{
			TRACE( "WAIT_TIMEOUT\n" );
#ifdef _WIN64
			PostQueuedCompletionStatus(m_phCompletionPort[iIoWorker], (DWORD)-4, (ULONG_PTR)pSock->GetHandle(), NULL);
#else
...
u_int	__stdcall IoWorkerThread( LPVOID lpvThreadParameter )
{
	...
	{
#ifdef _WIN64
		fOk		= GetQueuedCompletionStatus(pDPSock->GetCompletionPort(lIoWorker - 1), &dwBytes, reinterpret_cast<PULONG_PTR>(&hSocket), &lpov, INFINITE);
#else
		...
		{
#ifdef _WIN64
			UINT_PTR dwAddr = reinterpret_cast<UINT_PTR>(lpov);
#else
