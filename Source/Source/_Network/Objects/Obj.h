...
public:
	...
	DWORD		m_shopItemIndex;
	BOOL		b_shopFinderItem;
...
public:
	...
	void	SetMergedPiercingSize(int nSize) { m_piercing.SetMergedPiercingSize(nSize); }
	int		GetMergedPiercingSize() { return m_piercing.GetMergedPiercingSize(); }
	void	SetMergedPiercingItem(int nth, DWORD dwItem) { m_piercing.SetMergedPiercingItem(nth, dwItem); }
	DWORD	GetMergedPiercingItem(int nth) { return m_piercing.GetMergedPiercingItem(nth); };
...
private:
	...
	__int64		m_iMergedRandomOptItemId;
#endif //__WEAPON_MERGE
...
private:
	bool		isLocked;
public:
	const bool& IsItemLocked() const { return isLocked; }
	void SetItemLock(const bool lock) { isLocked = lock; }
	void LockItem() { isLocked = true; }
	void UnlockItem() { isLocked = false; }
#endif // __SYS_LOCK_ITEM

#ifdef __CHIP_PLAYERSHOP
	BYTE		GetBuyMode(void) { return m_nBuyMode; }
	void		SetBuyMode(BYTE nBuyMode) { m_nBuyMode = nBuyMode; }
#endif // __CHIP_PLAYERSHOP
...
#ifdef __AEGON_RETURN_AWAKE
	__int64		GetOldRandomOptItemId(void) { return m_iOldRandomOptItemId; }
	void		SetOldRandomOptItemId(__int64 iOldRandomOptItemId) { m_iOldRandomOptItemId = iOldRandomOptItemId; }
	int			GetOldRandomOpt(void) { return static_cast<int>(m_iOldRandomOptItemId & 0x00000000000000FF); }
#endif // __AEGON_RETURN_AWAKE
...
public:
	int		GetWeaponRarity() { return m_nWeaponRarity; }
	void	SetWeaponRarity(int i) { m_nWeaponRarity = i; }
	void	SetRandomWeaponRarity();
#endif // __WEAPON_RARITY

#ifdef __WEAPON_MERGE
	void	SetMergedWeaponATK(int nATK) { m_nMergedWeaponATK = nATK; }
	int		GetMergedWeaponATK() { return m_nMergedWeaponATK; }
	void	SetMergedRandomOptItemId(__int64 iID) { m_iMergedRandomOptItemId = iID; }
	__int64 GetMergedRandomOptItemId() { return m_iMergedRandomOptItemId; }
	__int64* GetMergedRandomOptItemIdPtr() { return &m_iMergedRandomOptItemId; }
#endif //__WEAPON_MERGE

#ifdef __CRYSTAL
	void SetCrystal(DWORD dwId) { m_dwCrystal = dwId; }
	DWORD GetCrystal() { return m_dwCrystal; }
#endif // __CRYSTAL

#else	// __SYS_IDENTIFY
private:
	int			m_nRandomOptItemId;
	// __AEGON_RETURN_AWAKE
	int			m_nOldRandomOptItemId;
	// __AEGON_RETURN_AWAKE
public:
	...
	// __AEGON_RETURN_AWAKE
	int		GetOldRandomOptItemId(void) { return m_nOldRandomOptItemId; }
	void	SetOldRandomOptItemId(int nOldRandomOptItemId) { m_nOldRandomOptItemId = nOldRandomOptItemId; }
	int		GetOldRandomOpt(void) { return m_nOldRandomOptItemId; }
	// __AEGON_RETURN_AWAKE
...
#ifdef _WIN64
	__int64	GetVisKeepTime(int nth) { return m_piercing.GetVisKeepTime(nth); }
#else
...
#ifdef __BUYBACK
public:
	int		getBuyBackPrice() { return m_nBuyBackPrice; }
	void	setBuyBackPrice(int nBuyBackPrice) { m_nBuyBackPrice = nBuyBackPrice; }

	int		getBuyBackTaxes() { return m_nBuyBackTaxes; }
	void	setBuyBackTaxes(int nBuyBackTaxes) { m_nBuyBackTaxes = nBuyBackTaxes; }

	BYTE	getBuyBackCont() { return m_nBuyBackCont; }
	void	setBuyBackCont(BYTE nBuyBackCont) { m_nBuyBackCont = nBuyBackCont; }
private:
	int		m_nBuyBackPrice;
	int		m_nBuyBackTaxes;
	BYTE	m_nBuyBackCont;
...
public:
	...
	BOOL	GetAssist(void) { return m_bAssistMember; }
...
public:
	...
	__ACHIEVEMENT_PROGRESS* GetAchievementProgress() { return &m_AchievementProgress; };
...
public:
	...
	inline vector<int>& GetOwnedBadges() {
		return m_OwnedBadges;
	}
#endif //__BADGE

#ifdef __SYS_REBIRTH
	void SetRebirthLevel(USHORT nRebirthLevel)
	{
		m_nRebirthLevel = nRebirthLevel;
	}
...
#ifdef __GAMEGUARD
	return ((this->m_dwAuthorization >= AUTH_ADMINISTRATOR) && ((m_dwMode & FREE_MONEY_MODE) == FREE_MONEY_MODE)) ? INT_MAX : m_dwGold;
#else // __GAMEGUARD
	return m_dwGold;
#endif // __GAMEGUARD
}
