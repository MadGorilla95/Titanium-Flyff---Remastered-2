...
BOOL CMover::IsAbnormalPlayerData()
{
	...
	for( int i = 0; i < nMax; i++ )
	{
		...
		{
			ItemProp* pItemProp = prj.GetItemProp(m_Inventory.m_apItem[i].m_dwItemId);
#ifdef __GAMEGUARD
			if (!pItemProp)
				return TRUE;

			if (pItemProp->dwItemKind3 == IK3_VIRTUAL)
				return TRUE;
#else // __GAMEGUARD
...
#ifdef __LEEAEG_SHOP_FINDER
	m_shopItemIndex = ie.m_shopItemIndex;
	b_shopFinderItem = ie.b_shopFinderItem;
#endif // __LEEAEG_SHOP_FINDER

#ifdef __CHIP_PLAYERSHOP
	m_nBuyMode = ie.GetBuyMode();
#endif // __CHIP_PLAYERSHOP
...
#ifdef __SYS_CUSTOM_TITLE
	strcpy(m_szOwnTitle, pMover->m_szOwnTitle);
	m_bShowTitle = pMover->m_bShowTitle;
#endif // __SYS_CUSTOM_TITLE

#ifdef __AEGON_INGAME_DP
	m_bDPCommandInProgress = pMover->m_bDPCommandInProgress;
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LOOT_BOX
	m_bLootBoxInProgress = pMover->m_bLootBoxInProgress;
	m_dwLootboxItem = pMover->m_dwLootboxItem;
	m_dwLootboxItemNum = pMover->m_dwLootboxItemNum;
#endif // __AEGON_LOOT_BOX

#ifdef __BADGE
	m_nBadge = pMover->GetBadge();
	GetOwnedBadges() = pMover->GetOwnedBadges();
#endif //__BADGE

#ifdef __AEGON_INGAME_DP
	m_nIngameDP = pMover->m_nIngameDP;
#endif // __AEGON_INGAME_DP

#ifdef __PREMIUM
	m_bShowPremium = pMover->m_bShowPremium;
#endif //__PREMIUM

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	m_nCollectingTime = pMover->m_nCollectingTime;
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_LAST_ONLINE
	m_nLastOnline = pMover->m_nLastOnline;
#endif // __AEGON_LAST_ONLINE

#ifdef __AEGON_INGAME_TIME
	m_nOnlineTime = pMover->m_nOnlineTime;
#endif // __AEGON_INGAME_TIME

#ifdef __AEGON_CRAFTING_SYSTEM
	m_nCraftingExp = pMover->m_nCraftingExp;
#endif // __AEGON_CRAFTING_SYSTEM
...
#ifdef __AEGON_MONSTER_HUNT
	m_MHProgress = pMover->m_MHProgress;
#endif // __AEGON_MONSTER_HUNT

#ifdef __PARTY_ASSISTANT
	m_bAssistMember = pMover->m_bAssistMember;
#endif //__PARTY_ASSISTANT

#ifdef __VENDING_NPC
	m_idVendorNPC = pMover->m_idVendorNPC;

#ifdef __AEGON_IMPROVED_SHOPDESIGN
	m_dwShopDesign = pMover->m_dwShopDesign;
#endif
#endif // __VENDING_NPC

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	m_AchievementProgress = pMover->m_AchievementProgress;

	m_AchievementFavorites = pMover->m_AchievementFavorites;
#endif // __AEGON_ACHIEVEMENT_SYSTEM
...
#ifdef __AEGON_SEASON_PASS
	m_SeasonpassProgress = pMover->m_SeasonpassProgress;
#endif // __AEGON_SEASON_PASS

#ifdef __SWITCH_EQUIP
	m_vEquipSwitchInfo = pMover->m_vEquipSwitchInfo;
#endif // __SWITCH_EQUIP

#ifdef __AEGON_PERMANENT_BUFF
	m_vPermanenetBuffs = pMover->m_vPermanenetBuffs;;
#endif // __AEGON_PERMANENT_BUFF

#ifdef __AEGON_PREM_MOTIONS
	m_vUnlockedMotions = pMover->m_vUnlockedMotions;
#endif // __AEGON_PREM_MOTIONS

#ifdef __AEGON_EMOTE_SYSTEM
	m_vUnlockedEmotes = pMover->m_vUnlockedEmotes;
#endif //__AEGON_EMOTE_SYSTEM

#ifdef __AEGON_AUTO_STATS
	m_nAutoStat = pMover->m_nAutoStat;
#endif //__AEGON_AUTO_STATS

#ifdef __AEGON_F_TOOL
	m_vFToolUnits = pMover->m_vFToolUnits;
#endif //__AEGON_F_TOOL

#ifdef __SYS_REBIRTH
	m_nRebirthLevel = pMover->GetRebirthLevel();
#endif //__SYS_REBIRTH

#ifdef __FL_BAG_LOOT
	m_nSelectedBag = pMover->m_nSelectedBag;
#endif //__FL_BAG_LOOT

#ifdef __AEGON_FIXES
	m_dwOwnerID = pMover->m_dwOwnerID;
#endif

#ifdef __AEGON_DB_PETFILTER
	m_dwPetfilter = pMover->m_dwPetfilter;
	m_sPetfilterLists = pMover->m_sPetfilterLists;
#endif
