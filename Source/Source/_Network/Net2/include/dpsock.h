...
BOOL CDPSock<T>::JoinToServer( DWORD dwcrc, u_long uWaitingTime )
	{
		...
		CreateIoWorker( (u_long)0x01 );
#ifdef _WIN64
		if (!CreateIoCompletionPort((HANDLE)pSock->GetHandle(), m_hCompletionPort, (ULONG_PTR)(pSock->GetHandle()), 0))
#else
...
#ifdef _WIN64
		PostQueuedCompletionStatus(m_hCompletionPort, NEWSOCKETMSG, (ULONG_PTR)pSock->GetHandle(), NULL);
#else
...
int CDPSock<T>::IoWorkerThread( void )
	{
		...
		{
#if _WIN64
			fOk = GetQueuedCompletionStatus(GetCompletionPort(), &dwBytes, &hSocket, &lpov, INFINITE);
