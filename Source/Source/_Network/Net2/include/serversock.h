...
#ifdef _WIN64
					if (!CreateIoCompletionPort((HANDLE)pChild->GetHandle(), GetCompletionPort(), (ULONG_PTR)hSocket, GetIoWorkerCount()))
#else 
...
#ifdef _WIN64
					PostQueuedCompletionStatus(GetCompletionPort(), NEWSOCKETMSG, (ULONG_PTR)hSocket, NULL);
#else 
