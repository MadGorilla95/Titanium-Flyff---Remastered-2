...
inline int GetInt64FromStr(char* pBuf, int* pLocation)
{
	char strTemp[50];
	int count = 0;
	while (pBuf[*pLocation] != ',' && pBuf[*pLocation] != '/' && pBuf[*pLocation] != 0) {
...
inline BOOL IsPwdValid( const TCHAR* lpszPassword )
{
	...
	if( strPassword.IsEmpty() == FALSE )
	{
		...
		if (strPassword.GetLength() < 3 || (strPassword.GetLength() > 16 && strPassword.GetLength() != 32))
#else
...
#ifdef __PREMIUM
		// Time elapses while logged out as well.
		bSMModetime[SM_PREMIUM] = TRUE;
		// Item for rendering the icon.
		dwSMItemID[SM_PREMIUM] = II_SCR_PREM_NOTUSE;
#endif //__PREMIUM
	}
}
