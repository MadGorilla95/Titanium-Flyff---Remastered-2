...
#endif	// __GPAUTH_01
{
	...
	char szQuery[200] = { 0, };
	sprintf(szQuery, "SELECT TOP 1 [account], [Azuriom_user_access_token] FROM [ACCOUNT_DBF].[dbo].[ACCOUNT_TBL] WHERE [account] = '%s'", CUserMng::GetInstance()->GetUser(dpId)->GetAccount());
	query.Exec(szQuery);
	char token[192] = { 0, };
	while (query.Fetch()) {
		query.GetStr("Azuriom_user_access_token", token);
	}
