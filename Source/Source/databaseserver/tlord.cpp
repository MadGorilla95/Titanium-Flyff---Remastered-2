...
{	// 데이터베이스로부터 선거 상태 복원
	...
	while( pQuery->Fetch() )
	{
		...
		time_t tCreate	= pQuery->GetInt64( "tCreate" );
#else
...
{	// 군주 입찰 저장
	...
	IElection* pElection	= m_pLord->GetElection();
#if !defined(_USE_32BIT_TIME_T)
	sprintf(szQuery, "uspElectionAddDeposit %d, %d, %d, %I64d, %I64d", g_appInfo.dwSys, pElection->GetId(), idPlayer, iDeposit, static_cast<__int64>(tCreate));
#else
