...
#ifdef __VENDING_NPC
	ON_MSG(PACKETTYPE_ADDVENDORNPC, &CDPTrans::OnAddVendorNPC);
	ON_MSG(PACKETTYPE_REMOVEVENDORNPC, &CDPTrans::OnRemoveVendorNPC);
	ON_MSG(PACKETTYPE_UPDATEVENDORNPC, &CDPTrans::OnUpdateVendorNPC);
	ON_MSG(PACKETTYPE_GETVENDORNPC, &CDPTrans::OnGetVendorNPC);
	ON_MSG(PACKETTYPE_ADDVENDORNPCLOG, &CDPTrans::OnAddVendorNPCLog);
	ON_MSG(PACKETTYPE_REMOVEVENDORNPCLOG, &CDPTrans::OnRemoveVendorNPCLog);
	ON_MSG(PACKETTYPE_SHOPCLOSELOG, &CDPTrans::OnShopCloseLog);
#endif // __VENDING_NPC

#ifdef __AEGON_INGAME_DP
	ON_MSG(PACKETTYPE_DP_COMMAND, &CDPTrans::OnIngameDPCommand);
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LAST_ONLINE
	ON_MSG(PACKETTYPE_LASTONLINE_INIT_LOAD, &CDPTrans::OnInitLoadLastLogin);
#endif // __AEGON_LAST_ONLINE

#ifdef __BAN_SYSTEM
	ON_MSG(PACKETTYPE_BAN_ACCOUNT, &CDPTrans::OnBanUser);
#endif // __BAN_SYSTEM

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	ON_MSG(PACKETTYPE_UPDATEACHIEVEMENTOFFLINE, &CDPTrans::OnSaveAchievementOffline);
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_PVP_RANKING
	ON_MSG(PACKETTYPE_PVPRANKING_INIT_LOAD, &CDPTrans::OnInitLoadPvPRanking);
	ON_MSG(PACKETTYPE_PVPRANKING_SAVE, &CDPTrans::OnSavePvPRanking);
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_INGAME_SHOP
	ON_MSG(PACKETTYPE_INGAMESHOP_INIT_LOAD, &CDPTrans::OnInitLoadIngameShop);
#endif

#ifdef __AEGON_TOURNAMENT
	ON_MSG(PACKETTYPE_TOURNAMENT_DB_LOAD, &CDPTrans::OnLoadTournaments);
	ON_MSG(PACKETTYPE_TOURNAMENT_DB_SAVE, &CDPTrans::OnSaveTournaments);
#endif //__AEGON_TOURNAMENT

#ifdef __ROYAL_RUMBLE
	ON_MSG(PACKETTYPE_PLAYERWAR_SAVE, &CDPTrans::OnSaveWarAllWarPlayers);
	ON_MSG(PACKETTYPE_PLAYERWAR_LOAD, &CDPTrans::OnLoadSiegeScore);
...
#ifdef _WIN64
	g_DbManager.spQueue.push_back(pMover2);
#else
...
void CDPTrans::SendEventLuaState( map<BYTE, BOOL> mapState, BOOL bTextOut, DPID dpId )
{
	...
	ar << bTextOut;
#ifdef _WIN64
	ar << static_cast<unsigned long>(mapState.size());
#else
	ar << mapState.size();
#endif 
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(vecT.size());
#else
	ar << vecT.size();
#endif
...
#ifdef _WIN64
		ar << static_cast<unsigned long>(vecT[i].vecMemberId.size());
#else
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(vecF.size());
#else
	ar << vecF.size();
#endif
...
void CDPTrans::OnUpdatePlayerData( CAr & ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	pPlayerData->data.nAssist = data.nAssist;
#endif //__PARTY_ASSISTANT
	pPlayerData->data.nVer++;
#ifdef __LEESE_PARTY_LOCATION
	pPlayerData->data.nLocation = data.nLocation;
...
#ifdef _WIN64
		ar << static_cast<unsigned long>(CTax::GetInstance()->m_vecdwOccupationShopItem.size());
#else
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(vec_dwNowPlayerId.size());
#else
...
#ifdef _WIN64
	ar << static_cast<unsigned long>(vec_prevRanking.size());
#else
...
#ifdef __VENDING_NPC
void CDPTrans::OnAddVendorNPC(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->dpid = dpid;
	lpDbOverlappedPlus->dpidCache = dpidCache;
	lpDbOverlappedPlus->dpidUser = dpidUser;
	lpDbOverlappedPlus->nQueryMode = VENDNPC_ADD;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}

void CDPTrans::OnRemoveVendorNPC(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->dpid = dpid;
	lpDbOverlappedPlus->dpidCache = dpidCache;
	lpDbOverlappedPlus->dpidUser = dpidUser;
	lpDbOverlappedPlus->nQueryMode = VENDNPC_REM;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}

void CDPTrans::OnUpdateVendorNPC(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->dpid = dpid;
	lpDbOverlappedPlus->dpidCache = dpidCache;
	lpDbOverlappedPlus->dpidUser = dpidUser;
	lpDbOverlappedPlus->nQueryMode = VENDNPC_UPDATE;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}

void CDPTrans::OnGetVendorNPC(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);

	lpDbOverlappedPlus->nQueryMode = VENDNPC_GET;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}

void CDPTrans::SendAddVendorNPC(u_long idPlayer, u_long objidVendor, u_long idVendor)
{
...
void CDPTrans::SendGetVendorNPC(u_long idVendor, u_long idPlayer, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, char* szShopName, char* szPlayerName, DWORD dwItemID, CItemContainer<CItemElem>* pInven, u_long idChannel, u_long idWorld, DWORD dwShopDesign)
#else
void CDPTrans::SendGetVendorNPC(u_long idVendor, u_long idPlayer, D3DXVECTOR3 vPos, FLOAT fAngle, DWORD dwSkillTime, FLOAT fMonsterSize, char* szShopName, char* szPlayerName, DWORD dwItemID, CItemContainer<CItemElem>* pInven, u_long idChannel, u_long idWorld)
#endif
{
	...
	ar << dwShopDesign;
#endif
	ar.WriteString(szShopName);
	ar.WriteString(szPlayerName);
	pInven->Serialize(ar);
	SEND(ar, this, DPID_ALLPLAYERS);
}

void CDPTrans::OnAddVendorNPCLog(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->dpid = dpid;
	lpDbOverlappedPlus->dpidCache = dpidCache;
	lpDbOverlappedPlus->dpidUser = dpidUser;
	lpDbOverlappedPlus->nQueryMode = VENDNPC_ADD_LOG;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}

void CDPTrans::OnRemoveVendorNPCLog(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->dpid = dpid;
	lpDbOverlappedPlus->dpidCache = dpidCache;
	lpDbOverlappedPlus->dpidUser = dpidUser;
	lpDbOverlappedPlus->nQueryMode = VENDNPC_REM_LOG;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}
void CDPTrans::OnShopCloseLog(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->dpid = dpid;
	lpDbOverlappedPlus->dpidCache = dpidCache;
	lpDbOverlappedPlus->dpidUser = dpidUser;
	lpDbOverlappedPlus->nQueryMode = SHOPCLOSE_LOG;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}
#endif // __VENDING_NPC

#ifdef __AEGON_INGAME_DP
void CDPTrans::OnIngameDPCommand(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->dpid = dpid;
	lpDbOverlappedPlus->dpidCache = dpidCache;
	lpDbOverlappedPlus->dpidUser = dpidUser;
	lpDbOverlappedPlus->nQueryMode = INGAMEDP_COMMAND;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LAST_ONLINE
void CDPTrans::OnInitLoadLastLogin(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	g_DbManager.OnInitialLoadLastLogin();
}
#endif // __AEGON_LAST_ONLINE

#ifdef __BAN_SYSTEM
void CDPTrans::OnBanUser(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->nQueryMode = BAN_USER;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPGuild, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}
#endif // __BAN_SYSTEM

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
void CDPTrans::OnSaveAchievementOffline(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	u_long playerID;
	int achievementID, additionalProg;
	ar >> playerID;
	ar >> achievementID;
	ar >> additionalProg;
	g_DbManager.UpdateAchievementForOfflineUser(playerID, achievementID, additionalProg);

}
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_PVP_RANKING
void CDPTrans::OnInitLoadPvPRanking(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	g_DbManager.OnInitialLoadPvPRanking();
}
void CDPTrans::OnSavePvPRanking(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize) 
{
	__PVPRANKING_PROGRESS_UNIT tmpUnit;
	tmpUnit.Read(ar);
	__PVPRANKING_PROGRESS_UNIT* mapUnit = CAegonPvPRankingMng::GetInstance()->GetStatUnit(tmpUnit.m_nPlayerID);
	if (mapUnit) 
	{
		mapUnit->Copy(&tmpUnit);
		g_DbManager.OnSavePvPRanking(mapUnit);
	}
	else 
	{
		CAegonPvPRankingMng::GetInstance()->playerRankingMap.insert(make_pair(tmpUnit.m_nPlayerID, tmpUnit));

		mapUnit = CAegonPvPRankingMng::GetInstance()->GetStatUnit(tmpUnit.m_nPlayerID);
		if (mapUnit) 
		{
			g_DbManager.OnSavePvPRanking(mapUnit);
		}
...
#ifdef __AEGON_INGAME_SHOP
void CDPTrans::OnInitLoadIngameShop(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	g_DbManager.OnInitialLoadIngameShop();
}
#endif

#ifdef __AEGON_TOURNAMENT
void CDPTrans::OnLoadTournaments(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	g_DbManager.LoadTournaments();
}
void CDPTrans::OnSaveTournaments(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	CAegonTournamentMng::GetInstance()->ReadTournamentMap(ar);
	g_DbManager.SaveTournaments();
}
#endif

#ifdef __ROYAL_RUMBLE
void CDPTrans::OnSaveWarAllWarPlayers(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->nQueryMode = SAVE_WAR_PLAYERS;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}

void CDPTrans::OnLoadSiegeScore(CAr& ar, DPID dpid, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->nQueryMode = LOAD_WAR_PLAYER;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPPut, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}

void CDPTrans::SendSiegeScore()
{
	BEFORESENDDUAL(ar, PACKETTYPE_PLAYERWAR_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << g_War.m_vecRankSiege.size();
	for (u_int i = 0; i < g_War.m_vecRankSiege.size(); i++)
	{
		ar.WriteString(g_War.m_vecRankSiege[i].szName);
		ar << g_War.m_vecRankSiege[i].uidPlayer;
		ar << g_War.m_vecRankSiege[i].nJob;
		ar << g_War.m_vecRankSiege[i].nKills;
		ar << g_War.m_vecRankSiege[i].nDeaths;
		ar << g_War.m_vecRankSiege[i].nPoints;
	}
	SEND(ar, this, DPID_ALLPLAYERS);
}
#endif // __ROYAL_RUMBLE
