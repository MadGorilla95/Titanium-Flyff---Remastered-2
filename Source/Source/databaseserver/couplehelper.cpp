...
BOOL CCoupleController::Restore()
{
	...
	while( pQuery->Fetch() )
	{
		...
		time_t tPropose = pQuery->GetInt64("tPropose");
#else
...
{
#if !defined(_USE_32BIT_TIME_T)
	if (!GetQueryObject()->Execute("uspAddPropose %d, %d, %I64d", g_appInfo.dwSys, idProposer, static_cast<__int64>(t)))
#else
...
{
#if !defined(_USE_32BIT_TIME_T)
	if (!GetQueryObject()->Execute("uspDeletePropose %d, %I64d", g_appInfo.dwSys, static_cast<__int64>(t)))
#else
