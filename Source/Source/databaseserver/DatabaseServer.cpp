...
//		ASSERT( 0 );
	...
	g_MyTrace.AddLine( '\0' );
	g_MyTrace.AddLine('\0');
	g_MyTrace.Add(0, FALSE, "DatabaseServer started.");
...
BOOL Script( LPCSTR lpszFileName )
{
	...
	if( s.Load( lpszFileName ) )
	{
		...
		while( s.tok != FINISHED )
		{
			...
			else if (s.Token == "DB_PWD_AZURIOM")
			{
				s.GetToken();
				strcpy(db.DB_ADMIN_PASS_AZURIOM, s.Token);
			}
			else if (s.Token == "DB_ADMIN_ID_AZURIOM")
			{
				s.GetToken();
				strcpy(DB_ADMIN_ID_AZURIOM, s.Token);
			}
