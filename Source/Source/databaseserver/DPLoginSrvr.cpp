...
{
#ifdef __GAMEGUARD
	if (!prj.IsAllowedLetter(lpszAccount))
	{
		Error("CDPLoginSrvr::SendPlayerList() : Invalid Letter in lpszAccount [%s]", lpszAccount);
		return;
	}
	if (!prj.IsAllowedLetter(lpszPassword))
	{
		Error("CDPLoginSrvr::SendPlayerList() : Invalid Letter in lpszPassword [%s]", lpszPassword);
		return;
	}
#endif // __GAMEGUARD
...
void CDPLoginSrvr::OnCreatePlayer( CAr & ar, DPID dpid, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if (!prj.IsAllowedLetter(szAccount))
	{
		Error("CDPLoginSrvr::OnCreatePlayer() : Invalid Letter in szAccount [%s]", szAccount);
		return;
	}
	if (!prj.IsAllowedLetter(szPassword))
	{
		Error("CDPLoginSrvr::OnCreatePlayer() : Invalid Letter in szPassword [%s]", szPassword);
		return;
	}
#endif // __GAMEGUARD
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
...
void CDPLoginSrvr::OnRemovePlayer( CAr & ar, DPID dpid, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if (!prj.IsAllowedLetter(szAccount))
	{
		Error("CDPLoginSrvr::OnRemovePlayer() : Invalid Letter in szAccount [%s]", szAccount);
		return;
	}
	if (!prj.IsAllowedLetter(szPassword))
	{
		Error("CDPLoginSrvr::OnRemovePlayer() : Invalid Letter in szPassword [%s]", szPassword);
		return;
	}
	if (!prj.IsAllowedLetter(szNo))
	{
		Error("CDPLoginSrvr::OnRemovePlayer() : Invalid Letter in szNo [%s]", szNo);
		return;
	}
#endif // __GAMEGUARD
	CAr ar1;
...
void CDPLoginSrvr::OnGetPlayerList( CAr & ar, DPID dpid, LPBYTE lpBuf, u_long uBufSize )
{
	...
	{
		Error("CDPLoginSrvr::OnGetPlayerList() : Invalid Letter in szAccount [%s]", szAccount);
		return;
	}
	if (!prj.IsAllowedLetter(szPassword, FALSE))
	{
		Error("CDPLoginSrvr::OnGetPlayerList() : Invalid Letter in szPassword [%s]", szPassword);
		return;
	}
#endif // __GAMEGUARD
	CAr ar1;
...
void CDPLoginSrvr::OnLeave( CAr & ar, DPID dpid, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if (!prj.IsAllowedLetter(szAccount, FALSE))
	{
		Error("CDPLoginSrvr::OnLeave() : Invalid Letter in szAccount [%s]", szAccount);
		return;
	}
#endif // __GAMEGUARD
	...
	{
		Error("CDPLoginSrvr::OnLeave() : Invalid Letter in lpszAccount [%s]", lpszAccount);
		return;
	}
#endif // __GAMEGUARD
