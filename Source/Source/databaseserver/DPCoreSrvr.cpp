...
CDPCoreSrvr::CDPCoreSrvr()
{
	...
	ON_MSG(PACKETTYPE_SETGUILDCOLOR, &CDPCoreSrvr::OnGuildSetColor);
...
#ifdef __GUILDFINDER
	ON_MSG(PACKETTYPE_CD_GUILD_FINDER, &CDPCoreSrvr::OnGuildFinder);
#endif //__GUILDFINDER
}
...
#if _WIN64
	ar << static_cast<unsigned long>(vecstrMessage.size());
#else
...
#ifdef __SYS_GUILDCOLOR
void CDPCoreSrvr::OnGuildSetColor(CAr& ar, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->nQueryMode = GUILD_SETCOLOR;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPGuild, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}
#endif //__SYS_GUILDCOLOR

#ifdef __GUILDFINDER
void CDPCoreSrvr::OnGuildFinder(CAr& ar, LPBYTE lpBuf, u_long uBufSize)
{
	LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus = g_DbManager.AllocRequest();
	g_DbManager.MakeRequest(lpDbOverlappedPlus, lpBuf, uBufSize);
	lpDbOverlappedPlus->nQueryMode = UPDATE_GUILD_FINDER;
	PostQueuedCompletionStatus(g_DbManager.m_hIOCPGuild, 1, NULL, &lpDbOverlappedPlus->Overlapped);
}
#endif //__GUILDFINDER
