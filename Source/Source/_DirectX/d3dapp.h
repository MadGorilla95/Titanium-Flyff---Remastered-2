...
protected:
    ...
    virtual HRESULT AdjustWindowForChange(BOOL bInvisible = FALSE);
...
public:
    DWORD windowedWidth;
    DWORD windowedHeight;
    void AegonToggleFullscreen();
    void RebuildPresentParamsFromSettings();
    void AegonSizeChange(BOOL force = FALSE);
#endif // __AEGON_FULLSCREEN_AND_RESIZE
    ...
    int InstallCrashRpt();
    int UninstallCrashRpt();
    void LogLastCrashRptError(char* szFile, int nLine);
#endif

#ifdef __AEGON_SPLASH_SCREEN
    HBITMAP m_bitmapSplashScreen;
    HWND m_hWndSplashScreenOwner;
    HWND m_hWndSplashScreen;
    HRGN m_hrgnSplashScreen;
    void CreateSplashScreen(HINSTANCE hInst);
