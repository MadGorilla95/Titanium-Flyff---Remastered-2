...
HRESULT CD3DApplication::Create( HINSTANCE hInstance )
{
    ...
    if (m_bFakeFullMode) 
    {
        tmpFullscreen = g_Option.m_bStartFullScreen;
...
#ifdef __AEGON_SPLASH_SCREEN
        g_Neuz.CreateSplashScreen(hInstance);
#endif
...
#ifdef __CLIENT_LOAD_MIDDLE  
#ifdef __CLIENT
        if (!g_Option.m_bStartFullScreen)
        {
            int xPos = (GetSystemMetrics(SM_CXSCREEN) - rc.right) / 2;
            int yPos = (GetSystemMetrics(SM_CYSCREEN) - rc.bottom) / 2;
            SetWindowPos(m_hWnd, 0, xPos, yPos, g_Option.m_nResWidth, g_Option.m_nResHeight, NULL);
        }
#endif //__CLIENT
#endif //__CLIENT_LOAD_MIDDLE
    ...
    windowedWidth = m_d3dpp.BackBufferWidth;
    windowedHeight = m_d3dpp.BackBufferHeight;

    if (m_bFakeFullMode && tmpFullscreen)
    {
        g_Option.m_bStartFullScreen = TRUE;
        AegonToggleFullscreen();
    ...
    if (g_Option.m_bWindowMaximized) 
    {
        m_bMinimized = false;
        m_bMaximized = true;
#ifndef __AEGON_SPLASH_SCREEN
        ShowWindow(m_hWnd, SW_MAXIMIZE);
#endif
       // AegonSizeChange(TRUE);
    ...
    CWndBase::m_Theme.SwitchButtonFont();
#endif
...
EndWindowedDeviceComboSearch:
    ...
    {
#ifdef __IMPROVEGFX0112
        if (g_Option.m_nViewArea != 0)
            m_d3dSettings.Windowed_DepthStencilBufferFormat = *(D3DFORMAT*)pBestDeviceCombo->pDepthStencilFormatList->GetPtr(0);
        else
            m_d3dSettings.Windowed_DepthStencilBufferFormat = *(D3DFORMAT*)pBestDeviceCombo->pDepthStencilFormatList->GetPtr(pBestDeviceCombo->pDepthStencilFormatList->Count() - 1);
#else // __IMPROVEGFX0112
        m_d3dSettings.Windowed_DepthStencilBufferFormat = *(D3DFORMAT*)pBestDeviceCombo->pDepthStencilFormatList->GetPtr(0);
#endif // __IMPROVEGFX0112
    }
...
EndFullscreenDeviceComboSearch:
    ...
    if (m_d3dEnumeration.AppUsesDepthBuffer)
    {
        ...
        if (g_Option.m_nViewArea != 0)
            ...
            m_d3dSettings.Fullscreen_DepthStencilBufferFormat = *(D3DFORMAT*)pBestDeviceCombo->pDepthStencilFormatList->GetPtr(pBestDeviceCombo->pDepthStencilFormatList->Count() - 1);
#else // __IMPROVEGFX0112
        m_d3dSettings.Fullscreen_DepthStencilBufferFormat = *(D3DFORMAT*)pBestDeviceCombo->pDepthStencilFormatList->GetPtr(0);
#endif // __IMPROVEGFX0112
    }
                                  ...
                                  LPARAM lParam )
{
    switch( uMsg )
    {
#if defined(__MUTEBGSOUNDS) || defined(__BACKGROUNDRENDER)
    case WM_ACTIVATE:
        if (LOWORD(wParam) == WA_INACTIVE)
        {
#ifdef __MUTEBGSOUNDS
#ifdef __fmod
            if (g_Option.muteSoundsInBG)
                SoundSystemClass::muteSounds(true);
            if (g_Option.muteMusicInBG)
                SoundSystemClass::muteMusic(true);
#else
            if (g_Option.muteSoundsInBG)
                g_SoundMng.ToggleMute(true);
            if (g_Option.muteMusicInBG)
                ToggleMusicMute(true);
#endif
#endif // __MUTEBGSOUNDS
#ifdef __BACKGROUNDRENDER
            if (g_Option.inactiveRender == 1)
            {
                SetTimer(hWnd, 2, 1000, nullptr);
                SetTimer(hWnd, 3, USER_TIMER_MINIMUM, nullptr);
                perSecond = true;
            }
            else if (g_Option.inactiveRender == 2)
            {
                SetTimer(hWnd, 3, USER_TIMER_MINIMUM, nullptr);
                perSecond = true;
            }
...
#ifdef __MUTEBGSOUNDS
#ifdef __fmod
            if (g_Option.muteSoundsInBG == 1)
                SoundSystemClass::muteSounds(false);
            if (g_Option.muteMusicInBG == 1)
                SoundSystemClass::muteMusic(false);
#else
            if (g_Option.muteSoundsInBG)
                g_SoundMng.ToggleMute(false);
            if (g_Option.muteMusicInBG)
        ...
        break;
#endif // __MUTEBGSOUNDS || __BACKGROUNDRENDER
        case WM_PAINT:
        ...
        case WM_GETMINMAXINFO:
            ...
            break;
#if defined(__WND_BUTTONS_FIX) || defined(__BACKGROUNDRENDER) || defined(__MINIMIZEFIX)
        case WM_TIMER:
            switch (wParam)
            {
            case 1: // Render with frame move
                if (m_bActive)
            ...
            case 3: //framemove
            {
                ...
                const float fAppTime = DXUtil_Timer(TIMER_GETAPPTIME);
                const float fElapsedAppTime = DXUtil_Timer(TIMER_GETELAPSEDTIME);
...
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
                    AegonSizeChange(TRUE);
                    AegonSizeChange(TRUE); //fixes black screen when minimizing during login
#else // __AEGON_FULLSCREEN_AND_RESIZE
                    HandlePossibleSizeChange();
#endif // __AEGON_FULLSCREEN_AND_RESIZE
                }
...
//                m_pd3dDevice->ShowCursor( TRUE );
...
#ifndef __AEGON_FULLSCREEN_AND_RESIZE
                if( !m_bWindowed )
       ...
       case WM_ENTERMENULOOP:
        ...
        case WM_SYSCOMMAND:
            ...
            switch( wParam )
            {
				...
				case SC_CLOSE:
                    ...
                    if (!g_WndMng.IsOpenWnd(APP_AEGON_LOGIN) && !g_WndMng.IsOpenWnd(APP_SELECT_CHANNEL) && !g_WndMng.IsOpenWnd(APP_SELECT_SERVER)
        ...
        case WM_CLOSE:
#ifdef __CRASH_REPORT
            g_Neuz.m_bSendCrashReport = FALSE;
#endif
...
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::AdjustWindowForChange(BOOL bInvisible)
{
	if( m_bFakeFullMode )
	{
		DWORD dwStyle = 0;
...
int CALLBACK CrashCallback(CR_CRASH_CALLBACK_INFO* pInfo)
{
    ...
    if (!g_Neuz.m_bSendCrashReport)
        return CR_CB_CANCEL;

    //add ingame info
    crAddProperty("Account", g_Neuz.m_szAccount);

    if (g_pPlayer)
        crAddProperty("Player", g_pPlayer->GetName());
    else
        crAddProperty("Player", "NULL");


    //add all logfiles to the crashreport
    int rv = crAddFile2(".\\Log\\*.txt", NULL, "logfile", CR_AF_MAKE_FILE_COPY | CR_AF_MISSING_FILE_OK);
    if (rv != 0) 
    {
        g_Neuz.LogLastCrashRptError(__FILE__, __LINE__);
    }
...
INT CD3DApplication::Run()
{
...
#ifdef __CRASH_REPORT
    int nCrashReport = InstallCrashRpt();
    if (nCrashReport != 0)
    {
        LogLastCrashRptError(__FILE__, __LINE__);
    }
    int rv = crSetCrashCallback(CrashCallback, NULL);
    if (rv != 0)
    {
        LogLastCrashRptError(__FILE__, __LINE__);
    }
    ...
    if(g_Option.m_bWindowMaximized)
        ...
        ShowWindow(g_Neuz.m_hWnd, SW_SHOW);
#endif
    ...
    while( WM_QUIT != msg.message  )
    {
		...
		{
#if defined (__MINIMIZEFIX) || defined(__BACKGROUNDRENDER)
			...
			Sleep(1);
#endif //__WND_BUTTONS
#endif // __MINIMIZEFIX || __BACKGROUNDRENDER
...
//#endif // not debug
    ...
    if (nCrashReport == 0) 
    {
        nCrashReport = UninstallCrashRpt();
        if (nCrashReport != 0)
        {
            LogLastCrashRptError(__FILE__, __LINE__);
        }
...
//	}
...
#endif
	{
        ...
        if (!m_pd3dDevice) 
        {
            ADDERRORMSG("CD3DApplication::Render3DEnvironment() m_pd3dDevice null");
            m_bDeviceLost = true;
        }
        else
#endif
...
#ifdef __AEGON_FULLSCREEN_AND_RESIZE
void CD3DApplication::AegonToggleFullscreen()
{
    HRESULT hr;

    Pause(true); 

    ChangeWindows(!g_Option.m_bStartFullScreen);   
    m_d3dSettings.IsWindowed = m_bWindowed;
    //m_bIgnoreSizeChange = true;
    ...
    {  
        ...
        if (ChangeDisplaySettingsExA(info.szDevice, &m_PrevDevMode, NULL, CDS_TEST, NULL) == DISP_CHANGE_SUCCESSFUL)
        {
            ChangeDisplaySettingsExA(info.szDevice, &m_PrevDevMode, NULL, CDS_UPDATEREGISTRY, NULL);
            ...
            if (!g_Option.m_bStartFullScreen)
            {
                ...
                int xPos = info.rcMonitor.left + ((monitor_width - g_WndMng.resizeWidth) / 2);
                int yPos = info.rcMonitor.top + ((monitor_height - g_WndMng.resizeHeight) / 2);

                SetWindowPos(m_hWnd, 0, xPos, yPos, g_WndMng.resizeWidth, g_WndMng.resizeHeight, NULL);
            }
        }       
    }
    else 
    {        
        m_d3dSettings.Fullscreen_DisplayMode.Width = monitor_width;
        m_d3dSettings.Fullscreen_DisplayMode.Height = monitor_height;

        ZeroMemory(&m_PrevDevMode, sizeof(DEVMODE));
        m_PrevDevMode.dmSize = sizeof(DEVMODE);
        m_PrevDevMode.dmFields = DM_BITSPERPEL | DM_PELSWIDTH | DM_PELSHEIGHT | DM_DISPLAYFREQUENCY;

        EnumDisplaySettings(info.szDevice, ENUM_CURRENT_SETTINGS, &m_PrevDevMode);
        ...
        dm.dmPelsWidth = monitor_width;
        dm.dmPelsHeight = monitor_height;
        
        dm.dmDisplayFrequency = m_PrevDevMode.dmDisplayFrequency;

        // 바꿀 수 있는지 시험해보고 바꿈
        if (ChangeDisplaySettingsExA(info.szDevice, &dm, NULL, CDS_TEST, NULL) == DISP_CHANGE_SUCCESSFUL)
        {
            ChangeDisplaySettingsExA(info.szDevice, &dm, NULL, CDS_FULLSCREEN, NULL);

            SetWindowPos(m_hWnd, HWND_NOTOPMOST, info.rcMonitor.left, info.rcMonitor.top, monitor_width, monitor_height, SWP_NOREPOSITION);
        }
    }
    int AdapterOrdinalOld = m_d3dSettings.AdapterOrdinal();
    D3DDEVTYPE DevTypeOld = m_d3dSettings.DevType();
    ...
    {
        // Reset the 3D device
        RebuildPresentParamsFromSettings();
        hr = Reset3DEnvironment();
    }
    if (FAILED(hr))
    {
   ...
   // m_bIgnoreSizeChange = false;
    ...
    AegonSizeChange(TRUE); // to fix missing taskbar and wrong size in fullscreen
    
}
void CD3DApplication::RebuildPresentParamsFromSettings()
{
    if (m_bWindowed)
    {
        ...
        m_d3dpp.BackBufferWidth = m_rcWindowClient.right - m_rcWindowClient.left;
        m_d3dpp.BackBufferHeight = m_rcWindowClient.bottom - m_rcWindowClient.top;
        //m_d3dpp.BackBufferWidth = windowedWidth;
        //m_d3dpp.BackBufferHeight = windowedHeight;
        m_d3dpp.BackBufferFormat = m_d3dSettings.Windowed_DisplayMode.Format;            
    }
    else 
    {
        m_d3dpp.BackBufferWidth = m_d3dSettings.Fullscreen_DisplayMode.Width;
        m_d3dpp.BackBufferHeight = m_d3dSettings.Fullscreen_DisplayMode.Height;
        m_d3dpp.BackBufferFormat = m_d3dSettings.Fullscreen_DisplayMode.Format;
    }
    return;
}
void CD3DApplication::AegonSizeChange(BOOL force)
{
    HRESULT hr = S_OK;
    RECT rcClientOld;
    rcClientOld = m_rcWindowClient;
    ...
    if (force || (rcClientOld.right - rcClientOld.left !=
        m_rcWindowClient.right - m_rcWindowClient.left ||
        rcClientOld.bottom - rcClientOld.top !=
        m_rcWindowClient.bottom - m_rcWindowClient.top))
    {
        ...
        FULLSCREEN_WIDTH = m_rcWindowBounds.right - m_rcWindowBounds.left;
        FULLSCREEN_HEIGHT = m_rcWindowBounds.bottom - m_rcWindowBounds.top;
             
        CRect rect = g_WndMng.GetWndRect();
       
        if (m_bWindowed)
        {
            rect.left = m_rcWindowClient.left;
            rect.right = m_rcWindowClient.right;
            rect.top = m_rcWindowClient.top;
            rect.bottom = m_rcWindowClient.bottom;
        }
        else
        {
            rect.left = 0;
            rect.right = m_d3dSettings.Fullscreen_DisplayMode.Width;
            rect.top = 0;
            rect.bottom = m_d3dSettings.Fullscreen_DisplayMode.Height ;
        }

       

        g_WndMng.SetWndRect(rect);

        if (g_WndMng.m_pWndWorld) 
        {            
            g_WndMng.m_pWndWorld->SetWndRect(rect);
        }
       
        //move windows //invalidate gauge etc
        g_WndMng.OnClientResize(m_rcWindowClient.right - m_rcWindowClient.left, m_rcWindowClient.bottom - m_rcWindowClient.top, rcClientOld.right - rcClientOld.left, rcClientOld.bottom - rcClientOld.top);

        if (m_bWindowed) 
        {
            g_WndMng.resizeWidth = rect.Width();
            g_WndMng.resizeHeight = rect.Height();
        }
       
        if (m_bMaximized == false) 
        {
            g_Option.m_nResWidth = m_rcWindowBounds.right - m_rcWindowBounds.left;
            g_Option.m_nResHeight = m_rcWindowBounds.bottom - m_rcWindowBounds.top;
        }
        g_Option.m_bWindowMaximized = m_bMaximized == false ? FALSE : TRUE;       

        //windows that need special treatment
        g_WndMng.RestartTaskBar();
     
        if (g_WndMng.GetApplet(APP_WEBBOX)) 
        {
            
            g_WndMng.GetApplet(APP_WEBBOX)->SetFocus();
...
void CD3DApplication::OnAegonFontSwitch() 
{
    if (g_Option.m_strFont.GetLength() <= 1)
        return;
    ...
    CWndBase::m_Theme.AegonOnSwitchFont();
    g_WndMng.OnAegonFontSwitch();
}
#endif // __AEGON_FONT_SWITCH


#ifdef __CRASH_REPORT
int CD3DApplication::InstallCrashRpt()
{

    CR_INSTALL_INFOA cr_info;
    memset(&cr_info, 0, sizeof(CR_INSTALL_INFO));

    cr_info.cb = sizeof(CR_INSTALL_INFO);
    cr_info.pszAppName = "Titanium MMO";

#if defined(__LOCALSERVER)
    cr_info.pszAppVersion = "Localserver";
    //cr_info.pszUrl = "http://127.0.0.1/storage/crashrpt.php";
    cr_info.dwFlags |= CR_INST_DONT_SEND_REPORT;
#elif defined(__TESTSERVER)
    cr_info.pszAppVersion = "Testserver";
    cr_info.pszUrl = "https://titanium-mmo.com/storage/crashrpt.php";
#elif defined(__LIVESERVER)
    cr_info.pszAppVersion = "Liveserver";
    cr_info.pszUrl = "https://titanium-mmo.com/storage/crashrpt.php";
#else
    cr_info.pszAppVersion = "Unknown server";
    cr_info.pszUrl = "https://titanium-mmo.com/storage/crashrpt.php";
#endif
    cr_info.pszEmailTo = NULL;
    cr_info.pszEmailSubject = NULL;

    cr_info.uPriorities[CR_HTTP] = 1;
    cr_info.uPriorities[CR_SMTP] = CR_NEGATIVE_PRIORITY;
    cr_info.uPriorities[CR_SMAPI] = CR_NEGATIVE_PRIORITY;

    cr_info.dwFlags |= CR_INST_ALL_POSSIBLE_HANDLERS;
    cr_info.pszPrivacyPolicyURL = "https://titanium-mmo.com/";
    cr_info.pszDebugHelpDLL = NULL;
    cr_info.uMiniDumpType = MiniDumpNormal;
#ifdef _WIN64
    cr_info.pszCrashSenderPath = ".\\CrashReport\\x64";
    cr_info.pszErrorReportSaveDir = ".\\CrashReport\\x64\\ErrorReports";
    cr_info.pszLangFilePath = ".\\CrashReport\\x64\\crashrpt_lang.ini";
#else
    cr_info.pszCrashSenderPath = ".\\CrashReport";
    cr_info.pszErrorReportSaveDir = ".\\CrashReport\\ErrorReports";
    cr_info.pszLangFilePath = ".\\CrashReport\\crashrpt_lang.ini";
#endif
    cr_info.pszRestartCmdLine = NULL;
    cr_info.pszEmailText = NULL;
    cr_info.pszSmtpProxy = NULL;
    cr_info.pszCustomSenderIcon = NULL;
    cr_info.pszSmtpLogin = NULL;
    cr_info.pszSmtpPassword = NULL;
    cr_info.nRestartTimeout = 0;

    return crInstall(&cr_info);
}
int CD3DApplication::UninstallCrashRpt()
{
    return crUninstall();
}
void CD3DApplication::LogLastCrashRptError(char* szFile, int nLine)
{
    char szError[4096] = { 0, };
    crGetLastErrorMsg(szError, 4096);
    Error("File: '%s' Line: '%d' -> CrashRpt error: %s", szFile, nLine, szError);
}
#endif
...
#ifdef __AEGON_SPLASH_SCREEN
void CD3DApplication::CreateSplashScreen(HINSTANCE hInst)
{
    if (!m_hWndSplashScreen) 
    {
        CString strClassName = "SplashScreen";
        ...
        wc.lpszClassName = strClassName.GetString();
        RegisterClass(&wc);

        m_hWndSplashScreenOwner = CreateWindow(strClassName.GetString(), NULL, WS_POPUP,
            0, 0, 0, 0, NULL, NULL, hInst, NULL);
        m_hWndSplashScreen = CreateWindowEx(WS_EX_LAYERED, strClassName.GetString(), NULL, WS_POPUP | WS_VISIBLE,
            0, 0, 0, 0, m_hWndSplashScreenOwner, NULL, hInst, NULL);

        SetWindowPos(m_hWndSplashScreenOwner, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOREDRAW);
        SetWindowPos(m_hWndSplashScreen, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOREDRAW);
        
        m_bitmapSplashScreen = LoadSplashImage();
        
        if (!m_bitmapSplashScreen) 
        {
#ifdef __LOCALSERVER
            CString strError;
            DWORD dwErrorCode = GetLastError();
            strError.Format("Error Code: %d", dwErrorCode);
            MessageBox(NULL, strError.GetString(), "err", MB_ICONERROR | MB_OK);
#endif
        }
        SetSplashImage(m_hWndSplashScreen, m_bitmapSplashScreen);
    }
}
void CD3DApplication::SetSplashImage(HWND hwndSplash, HBITMAP hbmpSplash)
{
...
void CD3DApplication::DestroySplashScreen() 
{
    if (m_hWndSplashScreen) 
    {
        DestroyWindow(m_hWndSplashScreen);
        m_hWndSplashScreen = NULL;
    }
    if (m_hWndSplashScreenOwner)
    {
        DestroyWindow(m_hWndSplashScreenOwner);
        m_hWndSplashScreenOwner = NULL;
    }
}
// Creates a stream object initialized with the data from an executable resource.

IStream* CD3DApplication::CreateStreamOnResource(LPCTSTR lpName, LPCTSTR lpType)
{
...
IWICBitmapSource* CD3DApplication::LoadBitmapFromStream(IStream* ipImageStream)
{
    // initialize return value
...
HBITMAP CD3DApplication::CreateHBITMAP(IWICBitmapSource* ipBitmap)
{
    // initialize return value
...
HBITMAP CD3DApplication::LoadSplashImage()
{
