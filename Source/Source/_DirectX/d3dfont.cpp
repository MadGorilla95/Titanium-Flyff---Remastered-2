...
HRESULT CD3DFont::InitDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	GetTextExtentPoint32(  m_hDC, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", 27, &m_sizeBitmap );
...
#ifdef _WIN64
			if (EnumFontFamiliesEx(GetDC(nullptr), &logFont, static_cast<FONTENUMPROC>(EnumFontFamExProc), reinterpret_cast<LPARAM>(pLangData->font.fdLang.faceNT), 0) == 0)
			{
				fontFace = pLangData->font.fdLang.faceNT;
			}
			else if (EnumFontFamiliesEx(GetDC(nullptr), &logFont, static_cast<FONTENUMPROC>(EnumFontFamExProc), reinterpret_cast<LPARAM>(pLangData->font.fdLang.face9x), 0) == 0)
			{
...
#ifdef __AEGON_CUSTOM_FONTS
	sx += (double)m_dwAegonSpacing / (double)2.0;
#endif // __AEGON_CUSTOM_FONTS
	...
	for( int t = 0; t < m_apTexture.GetSize(); t++ )
	{
		...
		while( *iter )
		{
			...
			if( *(BYTE*)iter > ' ' ) 
			{
				...
				{
					*pVertices++ = InitFont2DVertex( D3DXVECTOR4(sx+0.0f-0.5f,sy+h-0.5f,0.9f,1.0f), dwColor, tx1, ty2 );
					*pVertices++ = InitFont2DVertex( D3DXVECTOR4(sx+0.0f-0.5f,sy+0.0f-0.5f,0.9f,1.0f), dwColor, tx1, ty1 );
...
#ifdef __AEGON_CUSTOM_FONTS
	fRowWidth -= (float)m_dwAegonSpacing;
#endif // __AEGON_CUSTOM_FONTS
