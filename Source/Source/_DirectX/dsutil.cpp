...
CSound* CSoundManager::GetSound( LPTSTR pszFileName )
{
    ...
    const auto iter = m_map.find(pszFileName);
    if (iter != m_map.cend())
        return iter->second;
#else // __MUTEBGSOUNDS
...
//	m_bAllowMovementTimer = FALSE;
	...
	{
#ifdef __MUTEBGSOUNDS
        m_map.insert(std::make_pair(pszFileName, nullptr));
        Error("Missing file %s", pszFileName);
#endif // __MUTEBGSOUNDS
...
#ifdef __MUTEBGSOUNDS
        m_map.insert(std::make_pair(pszFileName, nullptr));
        Error("too many channels %s", pszFileName);
#endif // __MUTEBGSOUNDS
...
#ifdef __MUTEBGSOUNDS
        m_map.insert(std::make_pair(pszFileName, nullptr));
        Error("non PCM type %s", pszFileName);
#endif // __MUTEBGSOUNDS
...
#ifdef __MUTEBGSOUNDS
        m_map.insert(std::make_pair(pszFileName, nullptr));
        Error("Cannot create sound %s", pszFileName);
#endif // __MUTEBGSOUNDS
...
#ifdef __MUTEBGSOUNDS
        m_map.insert(std::make_pair(pszFileName, nullptr));
        Error("no 3d buffer interface s", pszFileName);
#endif // __MUTEBGSOUNDS
...
#ifdef __MUTEBGSOUNDS
        m_map.insert(std::make_pair(pszFileName, pSound));
#else // __MUTEBGSOUNDS
...
#ifdef __MUTEBGSOUNDS
    for (std::map<std::string, CSound*>::iterator iter = m_map.begin(); iter != m_map.end(); ++iter)
    {
        delete iter->second;
    }
    m_map.clear();
...
#ifdef __MUTEBGSOUNDS
void CSoundManager::ChangeVolume(long volume)
{
    m_nSoundVolume = volume;
    for (std::map<std::string, CSound*>::value_type& sound : m_map) //sound array
    {
        sound.second->ChangeVolume(m_nSoundVolume);
    }
}
void CSoundManager::ToggleMute(const bool mute)
{
    if (mute)
        ChangeVolume(static_cast<int>((-1) * (1.0f) * 5000));
    else
        ChangeVolume(static_cast<int>((-1) * (1.0f - g_Option.m_fEffectVolume) * 5000));
}
#endif // __MUTEBGSOUNDS
...
#ifdef __MUTEBGSOUNDS
void CSound::ChangeVolume(const long volume)
{
    if (m_apDSBuffer == nullptr)
        return;

    HRESULT hr = 0;
    for (unsigned long i = 0; i < m_dwNumBuffers; i++)
        hr |= m_apDSBuffer[i]->SetVolume(static_cast<float>(volume));
}
#endif // __MUTEBGSOUNDS
