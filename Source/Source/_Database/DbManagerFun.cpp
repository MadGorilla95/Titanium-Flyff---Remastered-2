...
BOOL CDbManager::GetBank( CMover* pMover, CQuery *qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, int nSlot )
{
	...
	while( '$' != ExtBank[CountStr] )
	{
		...
		_int64 iOldRandomOptItemId = GetInt64PaFromStr(ExtBank, &CountStr);
		if (iOldRandomOptItemId == -102)
			iOldRandomOptItemId = 0;
		pMover->m_Bank[nSlot].m_apItem[nExtBankCount].SetOldRandomOptItemId(iOldRandomOptItemId);
...
#endif // __PETVIS
#ifdef __AEGON_RETURN_AWAKE
		pMover->m_Bank[nSlot].m_apItem[nExtBankCount].SetOldRandomOptItemId(GetInt64PaFromStr(ExtBank, &CountStr));
#endif // __AEGON_RETURN_AWAKE
		++CountStr;
		++nExtBankCount;
	}
...
#endif // __PETVIS
#ifdef __AEGON_RETURN_AWAKE
		__int64 iOldRandomOptItemId = GetInt64PaFromStr(ExtBank, &CountStr);
		if (iOldRandomOptItemId == -102)
			iOldRandomOptItemId = 0;
		GuildBank->m_apItem[nExtBank].SetOldRandomOptItemId(iOldRandomOptItemId);
#endif // __AEGON_RETURN_AWAKE
		++CountStr; 		++nExtBank;
	}
...
#ifdef __AEGON_RETURN_AWAKE
			__int64 iOldRandomOptItemId = GetInt64PaFromStr(szExt, &nOffset);
			if (iOldRandomOptItemId == -102)
				iOldRandomOptItemId = 0;
			pPocket->m_apItem[i].SetOldRandomOptItemId(iOldRandomOptItemId);
#endif // __AEGON_RETURN_AWAKE
...
#endif // __PETVIS
#ifdef __AEGON_RETURN_AWAKE
		__int64 iOldRandomOptItemId = GetInt64PaFromStr(ExtInven, &CountStr);
		if (iOldRandomOptItemId == -102)
			iOldRandomOptItemId = 0;
		pMover->m_Inventory.m_apItem[nExtInven].SetOldRandomOptItemId(iOldRandomOptItemId);
#endif // __AEGON_RETURN_AWAKE
#else	// __SYS_IDENTIFY
...
void CDbManager::LoadPiercingInfo( CItemElem & itemElem, char* szPirecingInven, int* pLocation )
{
	...
	if( itemElem.IsVisPet() )
	{
		...
		{
#ifdef _WIN64
			itemElem.SetVisKeepTime(i, (__int64)GetInt64FromStr(szPirecingInven, pLocation));
#else
	...
	{
		itemElem.SetMergedPiercingSize(GetIntFromStr(szPirecingInven, pLocation));
		for (int i = 0; i < itemElem.GetMergedPiercingSize(); ++i)
			itemElem.SetMergedPiercingItem(i, (DWORD)GetIntFromStr(szPirecingInven, pLocation));
	}
...
BOOL CDbManager::GetTaskBar( CMover* pMover, CQuery *qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
...
#if _WIN64
		pMover->m_UserTaskBar.m_aSlotApplet[nIndex].m_dwData = GetInt64FromStr(AppletTaskBar, &CountStr);
#else
		pMover->m_UserTaskBar.m_aSlotApplet[nIndex].m_dwData = GetInt64FromStr(AppletTaskBar, &CountStr);
...
#if _WIN64
		pMover->m_UserTaskBar.m_aSlotItem[nSlotIndex][nIndex].m_dwData = GetInt64FromStr(ItemTaskBar, &CountStr);
	...
	{
		i = GetIntFromStr(SkillTaskBar, &CountStr);
		nIndex = GetIntFromStr(SkillTaskBar, &CountStr);
		pMover->m_UserTaskBar.m_aSlotQueue[i][nIndex].m_dwShortcut = (DWORD)GetIntFromStr(SkillTaskBar, &CountStr);
		pMover->m_UserTaskBar.m_aSlotQueue[i][nIndex].m_dwId = (DWORD)GetIntFromStr(SkillTaskBar, &CountStr);
		pMover->m_UserTaskBar.m_aSlotQueue[i][nIndex].m_dwType = (DWORD)GetIntFromStr(SkillTaskBar, &CountStr);
		pMover->m_UserTaskBar.m_aSlotQueue[i][nIndex].m_dwIndex = (DWORD)GetIntFromStr(SkillTaskBar, &CountStr);
		pMover->m_UserTaskBar.m_aSlotQueue[i][nIndex].m_dwUserId = (DWORD)GetIntFromStr(SkillTaskBar, &CountStr);
#if _WIN64
		pMover->m_UserTaskBar.m_aSlotQueue[i][nIndex].m_dwData = GetInt64FromStr(SkillTaskBar, &CountStr);
#else
		pMover->m_UserTaskBar.m_aSlotQueue[i][nIndex].m_dwData = GetInt64FromStr(SkillTaskBar, &CountStr);
#endif
	}
...
#if _WIN64
		pMover->m_UserTaskBar.m_aSlotQueue[nIndex].m_dwData = GetInt64FromStr(SkillTaskBar, &CountStr);
#else
		pMover->m_UserTaskBar.m_aSlotQueue[nIndex].m_dwData = GetInt64FromStr(SkillTaskBar, &CountStr);
...
BOOL CDbManager::GetSKillInfluence( CMover* pMover, CQuery *qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	while( '$' != szSKillInfluence[CountStr] )
	{
		...
		DWORD dwId = GetIntFromStr(szSKillInfluence, &CountStr);
#else
...
#ifdef __INCREASE_SKILL_ID_VALUE
		if (wType == 0 && dwId == 0 && dwLevel == 0 && tmTotal == 0)
#else
...
int CDbManager::GetOneItem( CItemElem* pItemElem, char* pstrItem, int *pLocation )
{
	...
	int isLocked = GetIntPaFromStr(pstrItem, pLocation);
	if (isLocked > 1 || isLocked < 0)
		isLocked = 0;

	pItemElem->SetItemLock((bool)isLocked);
#endif // __SYS_LOCK_ITEM

#ifdef __CHIP_PLAYERSHOP
	pItemElem->m_nBuyMode = (BOOL)GetIntPaFromStr(pstrItem, pLocation);
#endif // __CHIP_PLAYERSHOP

#ifdef __WEAPON_RARITY
	pItemElem->SetWeaponRarity(GetIntPaFromStr(pstrItem, pLocation));
	if (pItemElem->GetWeaponRarity() < 0)
		pItemElem->SetWeaponRarity(0);
#endif // __WEAPON_RARITY

#ifdef __UPGRADE_SUCCESS
	pItemElem->m_nUpgradeTries = GetIntPaFromStr(pstrItem, pLocation);
#endif // __UPGRADE_SUCCESS

#ifdef __WEAPON_MERGE
	__int64 mergeOptItemId = GetInt64PaFromStr(pstrItem, pLocation);
	int mergeWeaponAtk = GetIntPaFromStr(pstrItem, pLocation);
	if (pItemElem->IsMerged())
	{
		pItemElem->SetMergedRandomOptItemId(mergeOptItemId);
		pItemElem->SetMergedWeaponATK(mergeWeaponAtk);
	}
#endif //__WEAPON_MERGE

#ifdef __PET_GLOW
	pItemElem->m_nPetGlow = GetIntPaFromStr(pstrItem, pLocation);
#endif // __PET_GLOW

#ifdef __CRYSTAL
	pItemElem->SetCrystal((DWORD)GetIntPaFromStr(pstrItem, pLocation));
#endif // __CRYSTAL
...
BOOL CDbManager::GetMonsterHunt(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CString strQuery;
	...
	strQuery.Format("SELECT * FROM [MONSTERHUNT_TBL] WHERE [m_idPlayer] = '%07d' AND [serverindex] = '%02d' ORDER BY [m_nQuestID] ASC", pMover->m_idPlayer, g_appInfo.dwSys);
	if (!pMover || !pQuery->Exec(strQuery))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	}
	...
	while (pQuery->Fetch())
	{
		...
		questID = pQuery->GetInt("m_nQuestID");
		mobID = pQuery->GetInt("m_nMobID");
		count = pQuery->GetInt("m_nCount");
		rewardSent = pQuery->GetInt("m_nRewardSent");

		tmpUnit.Set(questID, mobID, count, rewardSent);
		pMover->m_MHProgress.vecQuests.push_back(tmpUnit);
...
#ifdef __BADGE
BOOL CDbManager::GetBadge(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	int pos = 0;
	char szBadges[3096] = { 0, };

	pMover->SetBadge(qry->GetInt("m_nBadge"));
	qry->GetStr("m_Badges", szBadges);
	VERIFYSTRING_RETURN(szBadges, pMover->m_szName);
	while ('$' != szBadges[pos])
	{
		int nBadge = GetIntFromStr(szBadges, &pos);
		pMover->GetOwnedBadges().push_back(nBadge);
	}
...
BOOL CDbManager::GetAchievement(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	if (!pMover)
	{
		return FALSE;
	...
	pMover->m_AchievementProgress.Init();
	...
	for(; it != CAegonAchievementMng::GetInstance()->m_mapAchievements.end(); it++)
	{
		...
		tmpStr.Format("SELECT m_nAchievementProg, m_nRewardSent FROM ACHIEVEMENT_TBL WHERE m_idPlayer = '%07d' AND m_nAchievementID = %d", pMover->m_idPlayer, it->second.achievementID);
		...
		achievementID = it->second.achievementID;
		achievementProg = pQuery->GetInt("m_nAchievementProg");
		rewardSent = pQuery->GetInt("m_nRewardSent");
		...
		tmpUnit.SetUnitProgress(achievementProg, rewardSent);
		pMover->m_AchievementProgress.AddUnit(tmpUnit);
...
BOOL CDbManager::GetSeasonPassProgress(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	if (!pMover)
	{
		return FALSE;
	...
	pMover->m_SeasonpassProgress.Init();

	tmpStr.Format("SELECT * FROM SEASONPASS_TBL WHERE m_idPlayer = '%07d'", pMover->m_idPlayer);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	while (pQuery->Fetch()) 
	{
		...
		passID = pQuery->GetInt("m_nPassID");
		progressPoints = pQuery->GetInt("m_nPassPoints");
		boughtPremium = pQuery->GetInt("m_nBoughtPass");
		pQuery->GetStr("m_szClaimed", rewardsClaimed);

		if (passID < 0 || progressPoints < 0 || boughtPremium < 0)
		{
			SAFE_DELETE(pQuery);
			return FALSE;
		}
		...
		tmpUnit.progressPoints = progressPoints;
		tmpUnit.boughtPremium = boughtPremium;
		
		int pos = 0;
		VERIFYSTRING_RETURN(rewardsClaimed, pMover->m_szName);
		while ('$' != rewardsClaimed[pos])
		{
			int nClaimed = GetIntFromStr(rewardsClaimed, &pos);
			tmpUnit.rewardsClaimed.push_back(nClaimed);
		}

		pMover->m_SeasonpassProgress.AddUnit(tmpUnit);
...
void CDbManager::GetEquipSwitch(CMover* pMover, CQuery* pQuery)
{
	CString strQuery;
	strQuery.Format("SELECT * FROM [EQUIPSWITCH_TBL] WHERE [m_idPlayer] = '%07d' AND [serverindex] = '%02d'", pMover->m_idPlayer, g_appInfo.dwSys);
	if (pQuery->Exec(strQuery))
	{
		while (pQuery->Fetch())
		{
			__EQUIP_SWITCH_INFO esi;
			char sObjIndex[128] = { 0, };
			char szDBTitle[128] = {0, };
			int pos = 0;
			pQuery->GetStr("m_szTitle", szDBTitle);
			GetStrFromDBFormat(esi.strTitle, szDBTitle, pos);

			esi.nActivate = pQuery->GetInt("m_nActivate");

			int CountStr = 0;
			pQuery->GetStr("m_szObjIndex", sObjIndex);
			for (int i = 0; i < MAX_HUMAN_PARTS; i++)
			{
				esi.dwId[i] = (int)GetIntFromStr(sObjIndex, &CountStr);
				if (esi.dwId[i] == -1)
					esi.dwId[i] = NULL_ID;
			}

			pMover->m_vEquipSwitchInfo.push_back(esi);
		}
	}
}
...
BOOL CDbManager::GetPermanentBuffs(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	if (!pMover)
	{
		return FALSE;
	...
	CString tmpStr;
	pMover->m_vPermanenetBuffs.clear();

	tmpStr.Format("SELECT * FROM PERM_BUFFS_TBL WHERE [m_idPlayer] = '%07d' AND [serverindex] = '%02d'", pMover->m_idPlayer, g_appInfo.dwSys);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	if (pQuery->Fetch())
	{
		...
		pQuery->GetStr("m_szBuffs", szPermBuffs);

		int nPos = 0;
		DWORD tmpBuffID;
		tmpBuffID = GetIntPaFromStr(szPermBuffs, &nPos);

		while (tmpBuffID > 0)
		{
			pMover->m_vPermanenetBuffs.push_back(tmpBuffID);
			nPos++;
			tmpBuffID = GetIntPaFromStr(szPermBuffs, &nPos);
		}
	}
...
BOOL CDbManager::GetUnlockedMotions(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	if (!pMover)
	{
		return FALSE;
	...
	CString tmpStr;
	pMover->m_vUnlockedMotions.clear();

	tmpStr.Format("SELECT * FROM UNLOCKED_MOTIONS_TBL WHERE [m_idPlayer] = '%07d' AND [serverindex] = '%02d'", pMover->m_idPlayer, g_appInfo.dwSys);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	if (pQuery->Fetch())
	{
		...
		pQuery->GetStr("m_szUnlockedMotions", szUnlockedMotions);

		int nPos = 0;
		DWORD tmpUnlockedID;
		tmpUnlockedID = GetIntPaFromStr(szUnlockedMotions, &nPos);

		while (tmpUnlockedID > 0)
		{
			pMover->m_vUnlockedMotions.push_back(tmpUnlockedID);
			nPos++;
			tmpUnlockedID = GetIntPaFromStr(szUnlockedMotions, &nPos);
		}
	}
...
BOOL CDbManager::GetUnlockedEmotes(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	if (!pMover)
	{
		return FALSE;
	...
	CString tmpStr;
	pMover->m_vUnlockedEmotes.clear();

	tmpStr.Format("SELECT * FROM UNLOCKED_EMOTES_TBL WHERE [m_idPlayer] = '%07d' AND [serverindex] = '%02d'", pMover->m_idPlayer, g_appInfo.dwSys);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	if (pQuery->Fetch())
	{
		...
		pQuery->GetStr("m_szUnlockedEmotes", szUnlockedEmotes);

		int nPos = 0;
		DWORD tmpUnlockedID;
		tmpUnlockedID = GetIntPaFromStr(szUnlockedEmotes, &nPos);

		while (tmpUnlockedID > 0)
		{
			pMover->m_vUnlockedEmotes.push_back(tmpUnlockedID);
			nPos++;
			tmpUnlockedID = GetIntPaFromStr(szUnlockedEmotes, &nPos);
		}
	}
...
BOOL CDbManager::AegonDeletedCharCheck(CMover* pMover)
{
	if (!pMover)
	{
		return FALSE;
	...
	tmpStr.Format("SELECT m_nDeleted, isblock FROM CHARACTER_TBL WHERE [m_idPlayer] = '%07d' AND [serverindex] = '%02d'", pMover->m_idPlayer, g_appInfo.dwSys);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	int nDeleted = pQuery->GetInt("m_nDeleted");
	char cIsBlock = pQuery->GetChar("isblock");

	if (cIsBlock != 'F' || nDeleted != 0)
	{
		Error("CDbManager::AegonDeletedCharCheck Blocked join of deleted character. m_idPlayer: %07d - nDeleted: %d - isblock: %c", pMover->m_idPlayer, nDeleted, cIsBlock);
		SAFE_DELETE(pQuery);
		return FALSE;
	}
...
void CDbManager::LoadWarPlayer(CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	OutputDebugString("CDbManager::LoadWarPlayer - IN");
	g_War.m_vecRankSiege.clear();
	CAr arRead(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	char szQuery[QUERY_SIZE] = { 0, };
	sprintf(szQuery, "{ call CHARACTER_01_DBF.dbo.LoadSiegeRanking('%02d') }", g_appInfo.dwSys);
	if (qry->Exec(szQuery) == FALSE)
	{
		WriteLog("[Player Siege] LoadWarPlayer  %s, %d\t%s", __FILE__, __LINE__, szQuery);
	...
	{
		SIEGE_RANK player;
		qry->GetStr("szName", player.szName);
		player.uidPlayer = qry->GetInt("idPlayer");
		player.nJob = qry->GetInt("nJob");
		player.nPoints = qry->GetInt("nPoints");
		player.nKills = qry->GetInt("nKills");
		player.nDeaths = qry->GetInt("nDeaths");
		g_War.m_vecRankSiege.push_back(player);
		//	Error("LoadWarPlayer");
	}

	CDPTrans::GetInstance()->SendSiegeScore();
	FreeRequest(lpDbOverlappedPlus);
	OutputDebugString("CDbManager::LoadWarPlayer - OUT");
}
#endif // __ROYAL_RUMBLE


#ifdef __AEGON_F_TOOL
BOOL CDbManager::AegonGetFToolUnits(CMover* pMover)
{
	if (!pMover)
	{
		return FALSE;
	...
	CString tmpStr;
	pMover->m_vFToolUnits.clear();

	tmpStr.Format("SELECT * FROM FTOOL_TBL WHERE [m_idPlayer] = '%07d' AND [serverindex] = '%02d'", pMover->m_idPlayer, g_appInfo.dwSys);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	{
		char szFTool[4096] = { 0, };
		pQuery->GetStr("m_szFTool", szFTool);
		int nPos = 0;


		char szIntervall[32] = { 0, };
		...
		while (szFTool[nPos] != '$')
		{
			...
			memset(szIntervall, 0, 32);

			tmpUnit.m_nPage = GetIntPaFromStr(szFTool, &nPos);
			tmpUnit.m_nFKey = GetIntPaFromStr(szFTool, &nPos);
			GetStrPaFromStr(szFTool, szIntervall, &nPos);
			nPos++;

			string strIntervall = szIntervall;
			float fTmp = stof(strIntervall);
			tmpUnit.m_fIntervall = fTmp;

			if (tmpUnit.IsValid())
			{
				pMover->m_vFToolUnits.push_back(tmpUnit);
			}
...
BOOL CDbManager::GetPetfilterLists(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	int pos = 0;
	char szLists[3096] = { 0, };


	qry->GetStr("m_szPetfilterLists", szLists);
	VERIFYSTRING_RETURN(szLists, pMover->m_szName);
	while ('$' != szLists[pos])
	{
		DWORD dwListItem = GetIntFromStr(szLists, &pos);
