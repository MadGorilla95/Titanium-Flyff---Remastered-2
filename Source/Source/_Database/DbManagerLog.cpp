...
#ifdef __VENDING_NPC
void CDbManager::VendNpcInsertLog(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	...
	{
		CItemElem* pItemElem = &tmpInventory.m_apItem[i];
		SaveOneItem(pItemElem, &is);
		...
		sprintf(is.szExt, "%d,%d,%I64d,%d,%d/",
			tmpInventory.m_apItem[i].m_bCharged,
			tmpInventory.m_apItem[i].m_dwKeepTime,
			tmpInventory.m_apItem[i].GetRandomOptItemId(),
			static_cast<int>(tmpInventory.m_apItem[i].m_bTranformVisPet),
			tmpInventory.m_apItem[i].m_nCost
		);
		strncat(icsInventory.szExt, is.szExt, sizeof(is.szExt));
		sprintf(Onem_apIndex, "%d/", tmpInventory.m_apIndex[i]);
		strncat(icsInventory.szIndex, Onem_apIndex, sizeof(Onem_apIndex));
		sprintf(sPerObjIndex, "%d/", tmpInventory.m_apItem[i].m_dwObjIndex);
	...
	char szQuery[QUERY_SIZE] = { 0, };
	sprintf(szQuery, "uspVendorNpcLogInsert '%07d', %d, %d, %1.2f, %1.2f, %1.2f, %1.2f, %d, %1.2f, '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d",
	...
	{
		Error("CDbManager::VendNpcInsertLog[Line:%u] Failed, Query: %s", __LINE__, szQuery);
		return;
	}
...
void CDbManager::VendNpcRemLog(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	...
	char szQuery[QUERY_SIZE] = { 0, };
	sprintf(szQuery, "uspVendorNpcLogDelete %d", idVendor);
	if (!pQuery->Exec(szQuery))
		Error("CDbManager::VendNpcRemLog[Line:%u] Failed, Query: %s", __LINE__, szQuery);
}

void CDbManager::ShopCloseLog(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	...
	int nCount;
	ar >> nCount;
	CItemElem apItem[MAX_VENDITEM];
	int nExtra[MAX_VENDITEM];
	for (int i = 0; i < nCount; i++)
	{
		apItem[i].Serialize(ar);
		ar >> nExtra[i];
	}
	...
	for (int i = 0; i < nCount; i++)
	{
		CItemElem* pItemElem = (CItemElem*)&apItem[i];
		SaveOneItem(pItemElem, &is);
		strncat(icsInventory.szItem, is.szItem, sizeof(is.szItem));
		strncat(icsInventory.szPiercing, is.szPiercing, sizeof(is.szPiercing));
		strncat(icsInventory.szExt, is.szExt, sizeof(is.szExt));
	...
	char szQuery[QUERY_SIZE] = { 0, };
	sprintf(szQuery, "uspShopCloseLog '%d', '%07d', '%s', '%s', '%s', '%s'",
		time, idPlayer, icsInventory.szItem, icsInventory.szPet, icsInventory.szExt, icsInventory.szPiercing);

	if (!pQuery->Exec(szQuery))
	{
		Error("CDbManager::ShopCloseLog[Line:%u] Failed, Query: %s", __LINE__, szQuery);
		return;
	}
