...
void CDbManager::Join(CQuery* qry, CQuery* qry1, CQuery* qrylog, LPDB_OVERLAPPED_PLUS lpDBOP)
{
...
#ifdef	__FIX_DUPE_ITEM_CHAR
	if (qry->GetChar("isblock") != 'F')
	{
		FreeRequest(lpDBOP);
		return;
	}
#endif	//__FIX_DUPE_ITEM_CHAR
...
#ifdef __VENDING_NPC
	mover.m_idVendorNPC = qry->GetInt("m_idVendorNPC");
#endif // __VENDING_NPC
	...
	int item = 0;
	int itemNum = 0;
	item = qry->GetInt("m_nLootboxItem");
	itemNum = qry->GetInt("m_nLootboxItemNum");
	if (item < 0)
		item = 0;
	if (itemNum < 0)
		itemNum = 0;
	mover.m_dwLootboxItem = item;
	mover.m_dwLootboxItemNum = itemNum;
	if (item > 0)
		mover.m_bLootBoxInProgress = TRUE;
	...
	sprintf(szAchFav, "");
	int AchFavPos = 0;
	qry->GetStr("m_szAchFav", szAchFav);
	mover.m_AchievementFavorites.clear();
	while ('$' != szAchFav[AchFavPos])
	{
		int nAchFav = GetIntFromStr(szAchFav, &AchFavPos);
		mover.m_AchievementFavorites.push_back(nAchFav);
	}
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_INGAME_DP
	g_DbManager.SetDonatePointsOnJoin(&mover, lpDBOP);
#endif // __AEGON_INGAME_DP

#ifdef __SYS_NAMECOLOR
	mover.m_ncR = qry->GetInt("m_ncR");
	mover.m_ncG = qry->GetInt("m_ncG");
	mover.m_ncB = qry->GetInt("m_ncB");
#endif //__SYS_NAMECOLOR

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	mover.m_nCollectingTime = qry->GetInt64("m_nCollectingTime");
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_LAST_ONLINE
	_time64(&mover.m_nLastOnline);
#endif // __AEGON_LAST_ONLINE

#ifdef __AEGON_INGAME_TIME
	mover.m_nOnlineTime = qry->GetInt64("m_nOnlineTime");
#endif // __AEGON_INGAME_TIME

#ifdef __AEGON_CRAFTING_SYSTEM
	mover.m_nCraftingExp = qry->GetInt("m_nCraftingExp");
#endif // __AEGON_CRAFTING_SYSTEM

#ifdef __AEGON_AUTO_STATS
	mover.m_nAutoStat = qry->GetInt("m_nAutoStat");
#endif

#ifdef __SYS_REBIRTH
	mover.SetRebirthLevel((USHORT)qry->GetInt("m_nRebirthLevel"));
#endif //__SYS_REBIRTH

#ifdef __FL_BAG_LOOT
	mover.m_nSelectedBag = qry->GetInt("m_nSelectedBag");
#endif //__FL_BAG_LOOT

#ifdef __AEGON_DB_PETFILTER
	mover.m_dwPetfilter = qry->GetInt64("m_dwPetfilter");

	char szPetfilterLists[4096] = { 0, };
	memset(szPetfilterLists, 0, 4096);
	qry->GetStr("m_szPetfilterLists", szPetfilterLists);

	mover.m_sPetfilterLists.ReadFromString(szPetfilterLists);
#endif
...
#ifdef __AEGON_MESSENGER
	SelectTag(qry, lpDBOP, mover.m_idPlayer, &tags);
#else
...
#ifdef __AEGON_MESSENGER
	SendJoin(&mover, lpDBOP, dwAuthKey, dwBillingClass, szPartyName, szBankPass, dwPlayTime, dwFlyTime, nMaximumLevel, &tags, szTimeGuild);
#else
