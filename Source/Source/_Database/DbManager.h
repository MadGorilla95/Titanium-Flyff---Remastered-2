...
#ifdef __THREAD_UPDATE
	const int	MAX_UPDATETHREAD_SIZE = 16;
#else //__THREAD_UPDATE
	const int	MAX_UPDATETHREAD_SIZE = 8;
#endif //__THREAD_UPDATE
...
public:
	...
	template <class Q = T> typename std::enable_if<std::is_same<Q, CMover*>::value, void>::type push_back(const T& item)
	{
#if _HAS_CXX17
		std::scoped_lock lock(muxQueue);
#else
		std::lock_guard<std::mutex> lock(muxQueue);
#endif
		const unsigned long id = item->m_idPlayer;
		auto iter = std::find_if(deqQueue.begin(), deqQueue.end(), [id, item](const T& pMover) 
			{ 
				return (pMover && id == pMover->m_idPlayer); 
			});
		if (iter != deqQueue.end())
		{
			auto var = *(iter);
			delete var;
			*(iter) = std::move(item);
...
//	CMclCritSec		m_csGuildCombat;
	...
	char DB_ADMIN_PASS_AZURIOM[256];
#endif
...
public:
	...
	void	SendJoin(CMover* pMover, LPDB_OVERLAPPED_PLUS lpDBOP, DWORD dwAuthKey,
		...
		DWORD dwPlayTime, DWORD dwFlyTime, int nMaximumLevel, vector<TAG_ENTRY>* tags,
		LPCTSTR szTimeGuild);
#else
	void	SendJoin( CMover* pMover, LPDB_OVERLAPPED_PLUS lpDBOP, DWORD dwAuthKey, 
						  DWORD dwBillingClass, LPCTSTR szPartyName, LPCTSTR szBankPass, 
...
//	u_long	GetPlayerID( LPCSTR lpszPlayer );
	...
	void	GetItemFromMail(CQuery* pQuery, CItemElem* pItemElem, BOOL bExtendedItemLoading = TRUE);
#else
...
#ifdef __GUILDFINDER
	void	UpdateGuildFinder(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif //__GUILDFINDER
...
#ifdef __SYS_GUILDCOLOR
	void	SetGuildColor(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif //__SYS_GUILDCOLOR
...
#ifdef __AEGON_MESSENGER
	void	SelectTag(CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, u_long uidPlayer, vector<TAG_ENTRY>* tags);
	void	WriteTag(CAr& ar, vector<TAG_ENTRY>* tags);
#else
...
private:
	...
	BOOL	GetMonsterHunt(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void	SaveMonsterHunt(CMover* pMover, CQuery* qry);
#endif // __AEGON_MONSTER_HUNT
	
#ifdef __VENDING_NPC
	void VendNpcAdd(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void VendNpcRem(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void VendNpcUpdate(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void VendNpcGet(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);

	void VendNpcInsertLog(CQuery* pQueryLog, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void VendNpcRemLog(CQuery* pQueryLog, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void ShopCloseLog(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif // __VENDING_NPC

#ifdef __BADGE
	BOOL	GetBadge(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void	SaveBadge(CMover* pMover, CQuery* qry);
#endif //__BADGE

#ifdef __AEGON_INGAME_DP
	BOOL	GetDonatePointsFromDB(int* userDP, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	BOOL	SetDonatePointsOnJoin(CMover* pMover, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	BOOL	OnDonatePointsFunc(LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, __DP_COMMAND_INFO info, u_long playerID);
	BOOL	AdjustDonatePoints(LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, int value);
	void	OnDonatePointsFuncPacket(LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	BOOL	GetAchievement(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void	SaveAchievement(CMover* pMover, CQuery* qry);
#endif // __AEGON_ACHIEVEMENT_SYSTEM

	
#ifdef __AEGON_LAST_ONLINE
public:
	void OnInitialLoadLastLogin();
#endif // __AEGON_LAST_ONLINE

#ifdef __BAN_SYSTEM
	void	BanUser(CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif // __BAN_SYSTEM

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	void	UpdateAchievementForOfflineUser(u_long playerID, int achievementID, int additionalProg);
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_SEASON_PASS
	BOOL	GetSeasonPassProgress(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
	void	SaveSeasonPassProgress(CMover* pMover, CQuery* qry);
#endif // __AEGON_SEASON_PASS

#ifdef __SWITCH_EQUIP
	void	SaveEquipSwitch(CMover* pMover, CQuery* pQuery);
	void	GetEquipSwitch(CMover* pMover, CQuery* pQuery);
#endif // __SWITCH_EQUIP

#ifdef __AEGON_PVP_RANKING
public:
	void OnInitialLoadPvPRanking();
	void OnSavePvPRanking(__PVPRANKING_PROGRESS_UNIT* saveUnit);
#endif // __AEGON_PVP_RANKING

#ifdef __AEGON_PERMANENT_BUFF
public:
	void	SavePermanentBuffs(CMover* pMover, CQuery* qry);
	BOOL	GetPermanentBuffs(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif // __AEGON_PERMANENT_BUFF

#ifdef __AEGON_PREM_MOTIONS
public:
		void	SaveUnlockedMotions(CMover* pMover, CQuery* qry);
		BOOL	GetUnlockedMotions(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif // __AEGON_PREM_MOTIONS

#ifdef __AEGON_EMOTE_SYSTEM
public:
		void	SaveUnlockedEmotes(CMover* pMover, CQuery* qry);
		BOOL	GetUnlockedEmotes(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif //__AEGON_EMOTE_SYSTEM

#ifdef __AEGON_DELETED_CHAR_CHECK
		BOOL	AegonDeletedCharCheck(CMover* pMover);
#endif

#ifdef __AEGON_INGAME_SHOP
		void OnInitialLoadIngameShop();
...
#ifdef __ROYAL_RUMBLE
private:
		void	SaveWarPlayers(CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
		void	LoadWarPlayer(CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif // __ROYAL_RUMBLE

#ifdef __AEGON_F_TOOL
		void	SaveFToolUnits(CMover* pMover, CQuery* qry);
		BOOL	AegonGetFToolUnits(CMover* pMover);
#endif
	...
	BOOL GetPetfilterLists(CMover* pMover, CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus);
#endif
...
#ifdef _WIN64
	std::array<std::thread, MAX_UPDATETHREAD_SIZE> saveplayerThread;
	void saveTh();
	public:
		tsQueue<CMover*> spQueue;
	private:
#else
