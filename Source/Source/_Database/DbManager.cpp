...
#ifdef __AEGON_INGAME_DP_AZURIOM
	DB_ADMIN_PASS_AZURIOM[0] = '\0';
#endif
}
...
void CDbManager::SaveTemporalEquipment(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	...
	for (DWORD ch = 0; ch < tmpInventory.m_dwItemMax; ch++)
	{
		SaveOneItem(&tmpInventory.m_apItem[ch], &is);
		strncat(icsInventory.szItem, is.szItem, sizeof(is.szItem));
		strncat(icsInventory.szExt, is.szExt, sizeof(is.szExt));
		strncat(icsInventory.szPiercing, is.szPiercing, sizeof(is.szPiercing));
		sprintf(Onem_apIndex, "%d/", tmpInventory.m_apIndex[ch]);
		strncat(icsInventory.szIndex, Onem_apIndex, sizeof(Onem_apIndex));
		sprintf(sPerObjIndex, "%d/", tmpInventory.m_apItem[ch].m_dwObjIndex);
		strcat(icsInventory.szObjIndex, sPerObjIndex);
		strncat(icsInventory.szPet, is.szPet, sizeof(is.szPet));

		if (tmpInventory.m_apItem[ch].m_bCharged || 0 < tmpInventory.m_apItem[ch].m_dwKeepTime || 0 != tmpInventory.m_apItem[ch].GetRandomOptItemId())
			bExtInven = TRUE;
		if (tmpInventory.m_apItem[ch].IsPiercedItem())
			bPirecingInven = TRUE;
		if (tmpInventory.m_apItem[ch].m_pPet)
			bPet = TRUE;
	}

	strncat(icsInventory.szItem, NullStr, sizeof(NullStr));
	strncat(icsInventory.szIndex, NullStr, sizeof(NullStr));
	strcat(icsInventory.szObjIndex, NullStr);
	if (bExtInven == FALSE)
		*icsInventory.szExt = '\0';
	if (bPirecingInven == FALSE)
		*icsInventory.szPiercing = '\0';
	if (bPet == FALSE)
		*icsInventory.szPet = '\0';
	strcat(icsInventory.szExt, NullStr);
	strcat(icsInventory.szPiercing, NullStr);
	strcat(icsInventory.szPet, NullStr);
...
void CDbManager::LoadTemporalEquipment(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	...
	while (pQueryChar->Fetch())
	{
		...
		int IndexItem = 0;
		int CountStr = 0;
		while ('$' != icsInventory.szItem[CountStr])
		{
			CItemElem BufItemElem;
			IndexItem = GetOneItem(&BufItemElem, icsInventory.szItem, &CountStr);
			BufItemElem.m_byFlag |= CItemElem::binds;
			tmpInventory.m_apItem[IndexItem] = BufItemElem;
		}

		CountStr = 0;
		int itemCount = 0;
		while ('$' != icsInventory.szIndex[CountStr])
		{
			tmpInventory.m_apIndex[itemCount] = (DWORD)GetIntFromStr(icsInventory.szIndex, &CountStr);
			itemCount++;
		}

		CountStr = 0;
		int IndexObjIndex = 0;
		while ('$' != icsInventory.szObjIndex[CountStr])
		{
			tmpInventory.m_apItem[IndexObjIndex].m_dwObjIndex = (DWORD)GetIntFromStr(icsInventory.szObjIndex, &CountStr);
			IndexObjIndex++;
		}

		CountStr = 0;
		int nExtInven = 0;
		while ('$' != icsInventory.szExt[CountStr])
		{
			tmpInventory.m_apItem[nExtInven].m_bCharged = (BOOL)GetIntPaFromStr(icsInventory.szExt, &CountStr);
			if (tmpInventory.m_apItem[nExtInven].m_bCharged != 1)
				tmpInventory.m_apItem[nExtInven].m_bCharged = 0;

			tmpInventory.m_apItem[nExtInven].m_dwKeepTime = (DWORD)GetIntPaFromStr(icsInventory.szExt, &CountStr);

			__int64 iRandomOptItemId = GetInt64PaFromStr(icsInventory.szExt, &CountStr);
			if (iRandomOptItemId == -102)
				iRandomOptItemId = 0;

			tmpInventory.m_apItem[nExtInven].SetRandomOptItemId(iRandomOptItemId);
			tmpInventory.m_apItem[nExtInven].m_bTranformVisPet = static_cast<BOOL>(GetIntPaFromStr(icsInventory.szExt, &CountStr));
			++CountStr;
			++nExtInven;
		}
		...
		while ('$' != icsInventory.szPiercing[CountStr])
		{
			LoadPiercingInfo(tmpInventory.m_apItem[nPirecingInven], icsInventory.szPiercing, &CountStr);
			++nPirecingInven;
		}

		CountStr = 0;
		int nId = 0;
		while ('$' != icsInventory.szPet[CountStr])
		{
			BOOL bPet = (BOOL)GetIntFromStr(icsInventory.szPet, &CountStr);
			if (bPet)
			{
				SAFE_DELETE(tmpInventory.m_apItem[nId].m_pPet);
				CPet* pPet = tmpInventory.m_apItem[nId].m_pPet = new CPet;
				BYTE nKind = (BYTE)GetIntFromStr(icsInventory.szPet, &CountStr);
				pPet->SetKind(nKind);
				BYTE nLevel = (BYTE)GetIntFromStr(icsInventory.szPet, &CountStr);
				pPet->SetLevel(nLevel);
				DWORD dwExp = (DWORD)GetIntFromStr(icsInventory.szPet, &CountStr);
				pPet->SetExp(dwExp);
				WORD wEnergy = (WORD)GetIntFromStr(icsInventory.szPet, &CountStr);
				pPet->SetEnergy(wEnergy);
				WORD wLife = (WORD)GetIntPaFromStr(icsInventory.szPet, &CountStr);
				pPet->SetLife(wLife);
				for (int i = PL_D; i <= pPet->GetLevel(); i++)
				{
					BYTE nAvailLevel = (BYTE)GetIntPaFromStr(icsInventory.szPet, &CountStr);
					pPet->SetAvailLevel(i, nAvailLevel);
				}
				char szFmt[MAX_PET_NAME_FMT] = { 0, };
				GetStrFromStr(icsInventory.szPet, szFmt, &CountStr);
...
void CDbManager::CreatePlayer( CQuery *qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	if (prj.IsAllowedLetter(lpDbOverlappedPlus->AccountInfo.szPassword) == FALSE)
	{
		Error("CDbManager::CreatePlayer() : Invalid Letter in szPassword [%s]", lpDbOverlappedPlus->AccountInfo.szPassword);
		FreeRequest(lpDbOverlappedPlus);
		return;
	}
#endif // __GAMEGUARD
	...
	if (nSlot < 0 || nSlot > 2)
	{
		Error("CDbManager::CreatePlayer() : User[%s] - Hack Attempt", lpDbOverlappedPlus->AccountInfo.szAccount);
		FreeRequest(lpDbOverlappedPlus);
		return;
	}
#endif // __GAMEGUARD
...
#ifdef __GAMEGUARD
	char szStr[MAX_G_NAME];
	strcpy(szStr, lpDbOverlappedPlus->AccountInfo.szPlayer);
	...
	{
		if (szStr[i] == szH[0] || szStr[i] == szA[0] || szStr[i] == '_' || szStr[i] == '-' || szStr[i] == ',' || szStr[i] == '.'
			|| szStr[i] == '+' || szStr[i] == '*' || szStr[i] == '!' || szStr[i] == '"' || szStr[i] == '?' || szStr[i] == '%'
			|| szStr[i] == '&' || szStr[i] == '/' || szStr[i] == '(' || szStr[i] == ')' || szStr[i] == '=' || szStr[i] == '?'
			|| szStr[i] == '`' || szStr[i] == '?' || szStr[i] == '<' || szStr[i] == '>' || szStr[i] == '|' || szStr[i] == '}'
			|| szStr[i] == ']' || szStr[i] == '[' || szStr[i] == '{' || szStr[i] == '~' || szStr[i] == '^' || szStr[i] == '$'
			|| szStr[i] == '?' || szStr[i] == ';' || szStr[i] == ':' || szStr[i] == '#')
		{
			FreeRequest(lpDbOverlappedPlus);
			return;
		}

		if (szStr[i] != 'a' && szStr[i] != 'b' && szStr[i] != 'c' && szStr[i] != 'd' && szStr[i] != 'e' && szStr[i] != 'f'
			&& szStr[i] != 'g' && szStr[i] != 'h' && szStr[i] != 'i' && szStr[i] != 'j' && szStr[i] != 'k' && szStr[i] != 'l'
			&& szStr[i] != 'm' && szStr[i] != 'n' && szStr[i] != 'o' && szStr[i] != 'p' && szStr[i] != 'q' && szStr[i] != 'r'
			&& szStr[i] != 's' && szStr[i] != 't' && szStr[i] != 'u' && szStr[i] != 'v' && szStr[i] != 'w' && szStr[i] != 'x'
			&& szStr[i] != 'y' && szStr[i] != 'z' && szStr[i] != 'A' && szStr[i] != 'B' && szStr[i] != 'C' && szStr[i] != 'D'
			&& szStr[i] != 'E' && szStr[i] != 'F' && szStr[i] != 'G' && szStr[i] != 'H' && szStr[i] != 'I' && szStr[i] != 'J'
			&& szStr[i] != 'K' && szStr[i] != 'L' && szStr[i] != 'M' && szStr[i] != 'N' && szStr[i] != 'O' && szStr[i] != 'P'
			&& szStr[i] != 'Q' && szStr[i] != 'R' && szStr[i] != 'S' && szStr[i] != 'T' && szStr[i] != 'U' && szStr[i] != 'V'
			&& szStr[i] != 'W' && szStr[i] != 'X' && szStr[i] != 'Y' && szStr[i] != 'Z' && szStr[i] != '0' && szStr[i] != '1'
			&& szStr[i] != '2' && szStr[i] != '3' && szStr[i] != '4' && szStr[i] != '5' && szStr[i] != '6' && szStr[i] != '7'
			&& szStr[i] != '8' && szStr[i] != '9')
		{
			FreeRequest(lpDbOverlappedPlus);
			return;
...
#ifdef __GAMEGUARD
	char szQuerySafe[QUERY_SIZE] = { 0, };
	sprintf(szQuerySafe, "SELECT m_szName FROM CHARACTER_TBL WHERE account='%s' AND playerslot='%i' AND isblock = 'F' ", lpDbOverlappedPlus->AccountInfo.szAccount, nSlot);

	if (FALSE == qry->Exec(szQuerySafe))
	{
		FreeRequest(lpDbOverlappedPlus);
		return;
	}
	...
	sprintf( szQuery, "{ call CHARACTER_01_DBF.dbo.CHECK_CHARS('%s', '%d') }", lpDbOverlappedPlus->AccountInfo.szAccount, nSlot );
	if( !qry->Exec( szQuery ) )
	{
	...
	//sprintf(szQuery1, "SELECT * FROM CHARACTER_TBL WHERE m_szName='%s' AND m_nDeleted ='0'", lpDbOverlappedPlus->AccountInfo.szPlayer);
	sprintf(szQuery1, "SELECT * FROM CHARACTER_TBL WHERE m_szName='%s'", lpDbOverlappedPlus->AccountInfo.szPlayer);
	if (!qry->Exec(szQuery1))
	{
		WriteLog("%s, %d\r\n\t%s", __FILE__, __LINE__, szQuery1);
		FreeRequest(lpDbOverlappedPlus);
...
#ifdef __GAMEGUARD
	if (!prj.IsAllowedLetter(szNo))
	{
		Error("CDbManager::RemovePlayer() : Invalid Letter in szNo [%s]", szNo);
		return;
	}
	if (!prj.IsAllowedLetter(lpDbOverlappedPlus->AccountInfo.szAccount))
	{
		Error("CDbManager::RemovePlayer() : Invalid Letter in szAccount [%s]", lpDbOverlappedPlus->AccountInfo.szAccount);
		return;
	}
	if (!prj.IsAllowedLetter(lpDbOverlappedPlus->AccountInfo.szPassword))
	{
		Error("CDbManager::RemovePlayer() : Invalid Letter in szPassword [%s]", lpDbOverlappedPlus->AccountInfo.szPassword);
		return;
	}
#endif // __GAMEGUARD
...
//			qry->Clear();
...

		{
			...
			while( '$' != Inven[CountStr] )
			{
				...
				if( IndexItem == -1 )
				{
					Error( "Inventory : %s, %d", mover.m_szName, BufItemElem.m_dwItemId );
				}
				else
				{
...
//		pd.data.dwState	= (DWORD)query.GetInt( "m_nMessengerState" );
		...
		pd.data.nLocation = (DWORD)query.GetInt("dwWorldID");
...
BOOL CDbManager::SendItemtoCharacter( int nSlot, CMover* pMover, CQuery *qry, CQuery *qry1, CQuery *qrylog, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
		...
		while( qry->Fetch() )
		{
			...
			pItemElem->SetFlag(CItemElem::webshop);
#endif
#else // __EXT_PIERCING
...
//					return;
			...
			else	// 아이템 지급
			{
				...
				if( iSerialNumber != 0 
#ifdef __AEGON_FIXES
					|| SendItemContents.itemElem.m_dwItemId == II_SYS_VOTECOIN
#endif
					)
				{
					bResult		= TRUE;
...
BOOL CDbManager::GetSendItem( CQuery *pQry, __SendItemContents * pSendItemContents )
{
...
#if __VER >= 12 // __EXT_PIERCING
#ifdef __AEGON_FIXES
	int nIndex = pQry->GetInt("adwItemId0");
...
#ifdef __AEGON_FIXES
	pItemElem->m_bCharged = pItemProp->bCharged;
#endif
	...
	pItemElem->m_nUpgradeTries = pQry->GetInt("nUpgradeTries");
...
#ifdef __AEGON_FIXES
	if (pItemElem->m_dwItemId == II_SYS_VOTECOIN) 
	{
		int nNum = pItemElem->m_nItemNum;
		CItemElem tmpItemElem;
		tmpItemElem.m_dwItemId = II_SYS_VOTECOIN;
		tmpItemElem.m_nItemNum = pItemElem->m_nItemNum;
		tmpItemElem.m_bCharged = pItemElem->m_bCharged;

		if (!pMover->m_Inventory.Add(&tmpItemElem, NULL, NULL, NULL))
		{
			FILEOUT("..\\SendItem.log", "Error votecoin Full IdPlayer = %d, PlayerName = %s, ItemName : %s", pMover->m_idPlayer, pMover->m_szName, SendItemContents.OneItem);
			return 0;
		}
	}
	else
...
#ifdef __VENDING_NPC
			case VENDNPC_ADD:
				VendNpcAdd(pQueryChar, lpDbOverlappedPlus);
				break;

			case VENDNPC_REM:
				VendNpcRem(pQueryChar, lpDbOverlappedPlus);
				break;

			case VENDNPC_UPDATE:
				VendNpcUpdate(pQueryChar, lpDbOverlappedPlus);
				break;

			case VENDNPC_GET:
				VendNpcGet(pQueryChar, lpDbOverlappedPlus);
				break;

			case VENDNPC_ADD_LOG:
				VendNpcInsertLog(pQueryLog, lpDbOverlappedPlus);
				break;

			case VENDNPC_REM_LOG:
				VendNpcRemLog(pQueryLog, lpDbOverlappedPlus);
				break;

			case SHOPCLOSE_LOG:
				ShopCloseLog(pQueryLog, lpDbOverlappedPlus);
				break;
#endif // __VENDING_NPC
#ifdef __AEGON_INGAME_DP
			case INGAMEDP_COMMAND:
				OnDonatePointsFuncPacket(lpDbOverlappedPlus);
				break;
#endif // __AEGON_INGAME_DP	
#ifdef __ROYAL_RUMBLE
			case SAVE_WAR_PLAYERS:
				SaveWarPlayers(pQueryLog, lpDbOverlappedPlus);
				break;
			case LOAD_WAR_PLAYER:
				LoadWarPlayer(pQueryLog, lpDbOverlappedPlus);
				break;
...
{
#ifdef _WIN64
	for (auto& arry : saveplayerThread)
		arry = std::thread(&CDbManager::saveTh, this);
#else
	DWORD dwThreadId;
...
//	1
	...
	while( pQuery->Fetch() )
	{
		...
		pGuild->m_adwGuildColor[0] = (DWORD)pQuery->GetInt("m_dwGuildColor_1");
		pGuild->m_adwGuildColor[1] = (DWORD)pQuery->GetInt("m_dwGuildColor_2");
		pGuild->m_adwGuildColor[2] = (DWORD)pQuery->GetInt("m_dwGuildColor_3");
...
#ifdef __GUILDFINDER
		pGuild->m_bGuildFinder = pQuery->GetInt("m_bGuildFinder");
#endif //__GUILDFINDER
...
#ifdef __SYS_GUILDCOLOR
void CDbManager::SetGuildColor(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	DWORD idGuild;
	int R, G, B;
...
#ifdef __GUILDFINDER
void CDbManager::UpdateGuildFinder(CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	u_long idGuild;
	DWORD bGuildFinder2;
	ar >> idGuild >> bGuildFinder2;

	GUILD_QUERYINFO info("U0");
	info.idGuild = idGuild;
	info.bGuildFinder = bGuildFinder2;
...
//		GuildBank.Clear();
		...
		while( '$' != ExtBank[CountStr] )
		{
			...
			GuildBank.m_apItem[nExtBank].SetOldRandomOptItemId(GetInt64PaFromStr(ExtBank, &CountStr));
...
void CDbManager::WriteTag(CAr& ar, vector<TAG_ENTRY>* tags)
{
	unsigned long nTagCount = tags->size();
	ar << nTagCount;

	const TAG_ENTRY* pEntry;
	for (unsigned long i = 0; i < nTagCount; ++i)
	{
		pEntry = &(*tags)[i];
...
#endif
#ifdef __AEGON_MESSENGER
void CDbManager::SelectTag(CQuery* qry, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, u_long uidPlayer, vector<TAG_ENTRY>* tags)
#else
...
#endif
{
	...
	while( qry->Fetch() )
	{
		...
		tmpEntry.idFrom = qry->GetInt("f_idPlayer");	// 보낸사람 
		//tmpEntry.dwDate = qry->GetInt("CreateTime");	// 보낸시각 
		char szCreateTime[32];
		qry->GetStr("CreateTime", szCreateTime);
		...
		std::memcpy(year, &szCreateTime[0], 4);
		std::memcpy(month, &szCreateTime[5], 2);
		std::memcpy(day, &szCreateTime[8], 2);
		std::memcpy(hour, &szCreateTime[11], 2);
		std::memcpy(min, &szCreateTime[14], 2);
		...
		else 
		{
			...
			timestamp = _mktime64(&atm);

			tmpEntry.dwDate = timestamp;
		}

		qry->GetStr("m_Message", tmpEntry.szString);	// 메세지 
		tags->push_back(tmpEntry);
#else
...
#ifdef __AEGON_MESSENGER
	CString tmpStr;
	tmpStr.Format("DELETE FROM TAG_TBL WHERE serverindex = '%02d' AND m_idPlayer = '%07d' AND State = 'D'", g_appInfo.dwSys, uidPlayer);
	qry->Execute(tmpStr.GetString());
#else
	return count;
#endif
}
...
void CDbManager::FormatStrToHex(char* szDst, const char* szSrc)
{
	char* pCur = szDst;
...
#ifdef __GAMEGUARD
	char szTempGuild[64];
	char* pStr = szTempGuild;

	for (size_t i = 0; i < strlen(szGuild) && i < 48; ++i)
	{
		if (*(szGuild + i) == '\'')
			*pStr++ = '\'';
		*pStr++ = *(szGuild + i);
...
void CDbManager::DBQryGuild( char* szSql, const GUILD_QUERYINFO& info )
{
	...
	for (size_t i = 0; i < strlen(info.pszGuild) && i < 48; ++i)
	{
		if (*(info.pszGuild + i) == '\'')
			*pStr++ = '\'';
		*pStr++ = *(info.pszGuild + i);
	}
	*pStr = 0;
#endif // __GAMEGUARD
...
#ifdef __SYS_GUILDCOLOR
			", @im_dwGC_1=%d, @im_dwGC_2=%d, @im_dwGC_3=%d"
#endif //__SYS_GUILDCOLOR	
#ifdef __GUILDFINDER
			", @im_dwGF=%d"
#endif //__GUILDFINDER
...
void CDbManager::MakeQueryAddMail( char* szSql, CMail* pMail, u_long idReceiver )
{
	...
	ItemStruct tmpItemStruct;
	memset(&tmpItemStruct, 0, sizeof(ItemStruct));

#ifdef __AEGON_FIXES
	char szPetName[32];
	memset(szPetName, 0, 32);
	if (bPet) 
	{
		strncat(szPetName, pet.GetName(), MAX_PET_NAME);
	}
	strncat(szPetName, NullStr, sizeof(NullStr));
#endif

#ifdef __AEGON_MAIL_ITEM_SAVE
	char szItem[1024] = {0, };
	char szItemExt[1024] = { 0, };
	char szItemPet[1024] = { 0, };
	char szItemPiercing[1024] = { 0, };

	memset(szItem, 0, 1024);
	memset(szItemExt, 0, 1024);
	memset(szItemPet, 0, 1024);
	memset(szItemPiercing, 0, 1024);

	if (pMail->m_pItemElem) 
	{
		g_DbManager.SaveOneItem(&item, &tmpItemStruct);

		strncat(szItem, tmpItemStruct.szItem, sizeof(tmpItemStruct.szItem));
		strncat(szItemExt, tmpItemStruct.szExt, sizeof(tmpItemStruct.szExt));
		strncat(szItemPet, tmpItemStruct.szPet, sizeof(tmpItemStruct.szPet));
		strncat(szItemPiercing, tmpItemStruct.szPiercing, sizeof(tmpItemStruct.szPiercing));

		strncat(szItem, NullStr, sizeof(NullStr));
		strncat(szItemExt, NullStr, sizeof(NullStr));
		strncat(szItemPet, NullStr, sizeof(NullStr));
		strncat(szItemPiercing, NullStr, sizeof(NullStr));
	}
	else 
	{
		strncat(szItem, NullStr, sizeof(NullStr));
	...
	while( true )
	{
		...
		switch( lpDbOverlappedPlus->nQueryMode )
		{
			...
			case UPDATE_GUILD_NOTICE:
				...
				break;
#ifdef __GUILDFINDER
			case UPDATE_GUILD_FINDER:
				UpdateGuildFinder(pQuery, lpDbOverlappedPlus);
				break;
#endif //__GUILDFINDER
...
#ifdef __SYS_GUILDCOLOR
			case GUILD_SETCOLOR:
				SetGuildColor(pQuery, lpDbOverlappedPlus);
				break;
#endif //__SYS_GUILDCOLOR
			case ADD_GUILD_VOTE:
...
#ifdef __BAN_SYSTEM
			case BAN_USER:
				BanUser(pQueryLog, lpDbOverlappedPlus);
				break;
#endif // __BAN_SYSTEM
...
#ifdef __AEGON_MAIL_ITEM_SAVE
			if (pMail->m_pItemElem->m_dwItemId != dwItemId)
			{
				Error("CDbManager::LoadPost changed item id");
...
#ifdef __AEGON_MAIL_ITEM_SAVE
void CDbManager::GetItemFromMail(CQuery* pQuery, CItemElem* pItemElem, BOOL bExtendedItemLoading)
#else
void CDbManager::GetItemFromMail( CQuery* pQuery, CItemElem* pItemElem )
#endif
{
#ifdef __AEGON_MAIL_ITEM_SAVE
	if (bExtendedItemLoading) 
	{
		char szItem[1024];
		char szItemExt[1024];
		char szItemPet[1024];
		char szItemPiercing[1024];
		...
		pQuery->GetStr("szItemExt", szItemExt);
		pQuery->GetStr("szItemPet", szItemPet);
		pQuery->GetStr("szItemPiercing", szItemPiercing);

		int CountStr = 0;
		int nIndexItem = 0;
		nIndexItem = GetOneItem(pItemElem, szItem, &CountStr);

		CountStr = 0;
		int nExtInven = 0;
		while ('$' != szItemExt[CountStr])
		{
			pItemElem->m_bCharged = (BOOL)GetIntPaFromStr(szItemExt, &CountStr);
			if (pItemElem->m_bCharged != 1)
				pItemElem->m_bCharged = 0;

			pItemElem->m_dwKeepTime = (DWORD)GetIntPaFromStr(szItemExt, &CountStr);

			__int64 iRandomOptItemId = GetInt64PaFromStr(szItemExt, &CountStr);

			if (iRandomOptItemId == -102)
				iRandomOptItemId = 0;
			pItemElem->SetRandomOptItemId(iRandomOptItemId);

			pItemElem->m_bTranformVisPet = static_cast<BOOL>(GetIntPaFromStr(szItemExt, &CountStr));
			pItemElem->m_nCost = GetIntPaFromStr(szItemExt, &CountStr);
			GetIntPaFromStr(szItemExt, &CountStr);
			GetIntPaFromStr(szItemExt, &CountStr);
			++CountStr;
			++nExtInven;
		}
		...
		while ('$' != szItemPiercing[CountStr])
		{
			LoadPiercingInfo(*pItemElem, szItemPiercing, &CountStr);
			++nPirecingInven;
		}
		...
		while ('$' != szItemPet[CountStr])
		{
			BOOL bPet = (BOOL)GetIntFromStr(szItemPet, &CountStr);
			if (bPet)
			{
				SAFE_DELETE(pItemElem->m_pPet);
				CPet* pPet = pItemElem->m_pPet = new CPet;
				BYTE nKind = (BYTE)GetIntFromStr(szItemPet, &CountStr);
				pPet->SetKind(nKind);
				BYTE nLevel = (BYTE)GetIntFromStr(szItemPet, &CountStr);
				pPet->SetLevel(nLevel);
				DWORD dwExp = (DWORD)GetIntFromStr(szItemPet, &CountStr);
				pPet->SetExp(dwExp);
				WORD wEnergy = (WORD)GetIntFromStr(szItemPet, &CountStr);
				pPet->SetEnergy(wEnergy);
				WORD wLife = (WORD)GetIntPaFromStr(szItemPet, &CountStr);
				pPet->SetLife(wLife);
				for (int i = PL_D; i <= pPet->GetLevel(); i++)
				{
					BYTE nAvailLevel = (BYTE)GetIntPaFromStr(szItemPet, &CountStr);
					pPet->SetAvailLevel(i, nAvailLevel);
				}
				char szFmt[MAX_PET_NAME_FMT] = { 0, };
				GetStrFromStr(szItemPet, szFmt, &CountStr);
				char szName[MAX_PET_NAME] = { 0, };
				GetDBFormatStr(szName, MAX_PET_NAME, szFmt);
				pPet->SetName(szName);
			}
...
#ifdef __UPGRADE_SUCCESS
	pItemElem->m_nUpgradeTries = pQuery->GetInt("nUpgradeTries");
#endif // __UPGRADE_SUCCESS
...
BOOL CDbManager::QueryGetMailRealTime( CQuery* pQuery )
{
	...
	while( pQuery->Fetch() )
	{
		...
		if( dwItemId && nItemFlag == 0 )
		{
			...
			if (pMail->m_pItemElem->m_dwItemId != dwItemId) 
			{
				Error("CDbManager::QueryGetMailRealTime changed item id");
				SAFE_DELETE(pMail->m_pItemElem);
				pMail->m_pItemElem = new CItemElem;
				pMail->m_pItemElem->m_dwItemId = dwItemId;
				GetItemFromMail(pQuery, pMail->m_pItemElem, FALSE);
			}
#endif
			
			if( CheckValidItem( dwItemId, pMail->m_pItemElem->m_nItemNum ) == FALSE )
			{
				Error("CDbManager::QueryGetMailRealTime invalid item check");
				SAFE_DELETE( pMail );
				continue;
			}
...
#ifdef __VENDING_NPC
void CDbManager::VendNpcAdd(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	...
	ar >> idPlayer >> objidNpc >> vPos >> fAngle >> dwSkillTime >> fMonsterSize >> dwItemID >> idChannel >> idWorld;
	...
	for (DWORD ch = 0; ch < MAX_VENDITEM; ch++)	// 0-504
	{
		SaveOneItem(&tmpInventory.m_apItem[ch], &is);
		strncat(icsInventory.szItem, is.szItem, sizeof(is.szItem));
		sprintf(is.szExt, "%d,%d,%I64d,%d,%d/",
			tmpInventory.m_apItem[ch].m_bCharged,
			tmpInventory.m_apItem[ch].m_dwKeepTime,
			tmpInventory.m_apItem[ch].GetRandomOptItemId(),
			static_cast<int>(tmpInventory.m_apItem[ch].m_bTranformVisPet),
			tmpInventory.m_apItem[ch].m_nCost
		);
		strncat(icsInventory.szExt, is.szExt, sizeof(is.szExt));
		strncat(icsInventory.szPiercing, is.szPiercing, sizeof(is.szPiercing));
		sprintf(Onem_apIndex, "%d/", tmpInventory.m_apIndex[ch]);
...
#ifdef __AEGON_IMPROVED_SHOPDESIGN
	sprintf(szQuery, "uspVendorNpcInsert '%07d', %d, %d, %1.2f, %1.2f, %1.2f, %1.2f, %d, %1.2f, '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d, %d",
		idPlayer, idWorld, idChannel, vPos.x, vPos.y, vPos.z, fAngle, dwSkillTime, fMonsterSize, strShopName.GetString(), icsInventory.szItem, icsInventory.szPet, icsInventory.szExt, icsInventory.szPiercing, icsInventory.szIndex, icsInventory.szObjIndex, dwItemID, dwShopDesign);
#else
	sprintf(szQuery, "uspVendorNpcInsert '%07d', %d, %d, %1.2f, %1.2f, %1.2f, %1.2f, %d, %1.2f, '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d",
		idPlayer, idWorld, idChannel, vPos.x, vPos.y, vPos.z, fAngle, dwSkillTime, fMonsterSize, strShopName.GetString(), icsInventory.szItem, icsInventory.szPet, icsInventory.szExt, icsInventory.szPiercing, icsInventory.szIndex, icsInventory.szObjIndex, dwItemID);
#endif

	if (!pQueryChar->Exec(szQuery) || !pQueryChar->Fetch())
	{
		Error("CDbManager::VendNpcAdd[Line:%u] Failed, Query: %s", __LINE__, szQuery);
		return;
	}
	u_long idVendor = (DWORD)pQueryChar->GetInt("idVendor");
	CDPTrans::GetInstance()->SendAddVendorNPC(idPlayer, objidNpc, idVendor);
}

void CDbManager::VendNpcRem(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	...
	char szQuery[QUERY_SIZE] = { 0, };
	sprintf(szQuery, "uspVendorNpcDelete %d", idVendor);
	if (!pQueryChar->Exec(szQuery))
		Error("CDbManager::VendNpcRem[Line:%u] Failed, Query: %s", __LINE__, szQuery);
}

void CDbManager::VendNpcUpdate(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	...
	char szQuery[QUERY_SIZE] = { 0, };
#ifdef __AEGON_IMPROVED_SHOPDESIGN
	sprintf(szQuery, "uspVendorNpcUpdate %d, %d, '%s', '%s', '%s', '%s', '%s', '%s', %d",
		idVendor, dwSkillTime, icsInventory.szItem, icsInventory.szPet, icsInventory.szExt, icsInventory.szPiercing, icsInventory.szIndex, icsInventory.szObjIndex, dwShopDesign);
#else
	sprintf(szQuery, "uspVendorNpcUpdate %d, %d, '%s', '%s', '%s', '%s', '%s', '%s'",
		idVendor, dwSkillTime, icsInventory.szItem, icsInventory.szPet, icsInventory.szExt, icsInventory.szPiercing, icsInventory.szIndex, icsInventory.szObjIndex);
#endif
	if (!pQueryChar->Exec(szQuery))
		Error("CDbManager::VendNpcUpdate[Line:%u] Failed, Query: %s", __LINE__, szQuery);
}

void CDbManager::VendNpcGet(CQuery* pQueryChar, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	CAr ar(lpDbOverlappedPlus->lpBuf, lpDbOverlappedPlus->uBufSize);
	int nCount = 0;
	DPID dpidMulti = lpDbOverlappedPlus->dpid;
	...
	BEFORESENDDUAL(out, PACKETTYPE_GETVENDORNPC, DPID_UNKNOWN, DPID_UNKNOWN);
	u_long uOffset = out.GetOffset();
	out << nCount;

	sprintf(szQuery, "uspVendorNpcGet %d", idChannel);
	if (!pQueryChar->Exec(szQuery))
	{
		Error("CDbManager::VendNpcGet[Line:%u] Failed, Query: %s", __LINE__, szQuery);
		return;
	}
	while (pQueryChar->Fetch())
	{
		...
		char        szPlayerID[16];
		pQueryChar->GetStr("idPlayer", szPlayerID);
		pQueryChar->GetStr("m_szName", szPlayerName);

		u_long      idPlayer = (DWORD)atoi(szPlayerID);
		u_long      idVendor = (DWORD)pQueryChar->GetInt("idVendor");
		DWORD       dwSkillTime = (DWORD)pQueryChar->GetInt("nRemaining");
		DWORD       dwItemID = (DWORD)pQueryChar->GetInt("dwItemID");

#ifdef __AEGON_IMPROVED_SHOPDESIGN
		DWORD       dwShopDesign= (DWORD)pQueryChar->GetInt("dwShopDesign");
#endif

		u_long      idWorld = (DWORD)pQueryChar->GetInt("idWorld");
		FLOAT       fAngle = pQueryChar->GetFloat("fAngle");
		FLOAT       fMonsterSize = pQueryChar->GetFloat("fScale");

		D3DXVECTOR3 vPos;
		vPos.x = pQueryChar->GetFloat("fPosX");
		vPos.y = pQueryChar->GetFloat("fPosY");
		vPos.z = pQueryChar->GetFloat("fPosZ");

		char        szShopName[64];
		pQueryChar->GetStr("szShopName", szShopName);
		...
		while ('$' != icsInventory.szExt[CountStr])
		{
			...
			tmpInventory.m_apItem[nExtInven].m_bTranformVisPet = static_cast<BOOL>(GetIntPaFromStr(icsInventory.szExt, &CountStr));
			tmpInventory.m_apItem[nExtInven].m_nCost = GetIntPaFromStr(icsInventory.szExt, &CountStr);
			GetIntPaFromStr(icsInventory.szExt, &CountStr);
			GetIntPaFromStr(icsInventory.szExt, &CountStr);
			++CountStr;
			++nExtInven;
		}
		...
		out << idVendor << idPlayer << vPos << fAngle << dwSkillTime << fMonsterSize << dwItemID << idChannel << idWorld;
#ifdef __AEGON_IMPROVED_SHOPDESIGN
		out << dwShopDesign;
#endif
		out.WriteString(szShopName);
		out.WriteString(szPlayerName);
		tmpInventory.Serialize(out);
		nCount++;
	}
...
#ifdef __AEGON_INGAME_DP
BOOL CDbManager::GetDonatePointsFromDB(int* userDP, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	int nAzuriomID = -1;
	...
	if (FALSE == pQuery->Connect(3, "login", DB_ADMIN_ID_CHARACTER01, DB_ADMIN_PASS_CHARACTER01))
	{
		WriteLog("%s, %d AccountDB Connect", __FILE__, __LINE__);
		SAFE_DELETE(pQuery);
		return FALSE;
	}
#ifdef __AEGON_INGAME_DP_AZURIOM
	tmpStr.Format("SELECT Azuriom_user_id FROM ACCOUNT_TBL WHERE account = '%s'", lpDbOverlappedPlus->AccountInfo.szAccount);
#else // __AEGON_INGAME_DP_AZURIOM
	tmpStr.Format("SELECT cash FROM ACCOUNT_TBL WHERE account = '%s'", lpDbOverlappedPlus->AccountInfo.szAccount);
#endif // __AEGON_INGAME_DP_AZURIOM
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	}
...
#ifdef __AEGON_INGAME_DP_AZURIOM
	nAzuriomID = pQuery->GetInt("Azuriom_user_id");
	if (nAzuriomID < 0)
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	pQuery->Clear();
	pQuery->DisConnect();
	if (FALSE == pQuery->Connect(3, AZURIOM_DB, DB_ADMIN_ID_AZURIOM, DB_ADMIN_PASS_AZURIOM))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	}
	tmpStr.Format("SELECT money FROM %s.users WHERE id = '%d'", AZURIOM_DB, nAzuriomID);
	...
	*userDP = (int)floor(pQuery->GetFloat("money"));
#else // __AEGON_INGAME_DP_AZURIOM
	*userDP = pQuery->GetInt("cash");
#endif // __AEGON_INGAME_DP_AZURIOM
...
BOOL CDbManager::SetDonatePointsOnJoin(CMover* pMover, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	int userDP = 0;
	if (GetDonatePointsFromDB(&userDP, lpDbOverlappedPlus))
	{
		pMover->SetIngameDP(userDP);
		return TRUE;
	}
	else 
	{
		pMover->SetIngameDP(0);
		return FALSE;
	}
...
BOOL CDbManager::OnDonatePointsFunc(LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, __DP_COMMAND_INFO info, u_long playerID)
{
	DPID dpidMulti = lpDbOverlappedPlus->dpid;
	int userDP = 0;
	...
	info.newDPBalance = userDP;

	if (info.dwAmount <= 0) 
	{
		Error("%s dwAmount: %lu", __FUNCTION__, info.dwAmount);
		return FALSE;
	}

	if (info.dwAmount != 1 && info.command != INGAMEDP_WEBSHOP) 
	{
		Error("%s dwAmount: %lu - command: %d", __FUNCTION__, info.dwAmount, info.command);
		return FALSE;
	}

	if (userDP < (info.dpCost * info.dwAmount))
	{
		info.command = INGAMEDP_CASH_ERROR;
		BEFORESENDDUAL(out, PACKETTYPE_DP_COMMAND, DPID_UNKNOWN, DPID_UNKNOWN);
		out << playerID;
		out << info;
		SEND(out, CDPTrans::GetInstance(), dpidMulti);
		return FALSE;
	...
	case INGAMEDP_BUYLOOTBOX:
	case INGAMEDP_BUYSEASONPASS:
	case INGAMEDP_BUYMOTION:
	case INGAMEDP_BUYEMOTE:
#ifdef __AEGON_INGAME_SHOP
	case INGAMEDP_WEBSHOP:
#endif
		if (AdjustDonatePoints(lpDbOverlappedPlus, (-1) * (info.dpCost * info.dwAmount)))
		{
			info.newDPBalance = userDP - (info.dpCost * info.dwAmount);
			BEFORESENDDUAL(out, PACKETTYPE_DP_COMMAND, DPID_UNKNOWN, DPID_UNKNOWN);
			out << playerID;
			out << info;
			SEND(out, CDPTrans::GetInstance(), dpidMulti);
...
BOOL CDbManager::AdjustDonatePoints(LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus, int value)
{
#ifdef __AEGON_INGAME_DP_AZURIOM
	int nAzuriomID = 0;
#endif // __AEGON_INGAME_DP_AZURIOM
	...
	tmpStr.Format("SELECT Azuriom_user_id FROM ACCOUNT_TBL WHERE account = '%s'", lpDbOverlappedPlus->AccountInfo.szAccount);
#else // __AEGON_INGAME_DP_AZURIOM
	tmpStr.Format("UPDATE ACCOUNT_TBL SET cash = cash + %d WHERE account = '%s'", value, lpDbOverlappedPlus->AccountInfo.szAccount);
#endif // __AEGON_INGAME_DP_AZURIOM
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
	...
	tmpStr.Format("UPDATE %s.users SET money = money + %d WHERE id = '%d'", AZURIOM_DB, value, nAzuriomID);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return FALSE;
	...
	int nUserDP = 0;
	if (GetDonatePointsFromDB(&nUserDP, lpDbOverlappedPlus)) 
	{
		if (nUserDP < 0 && value < 0) 
		{
			tmpStr.Format("UPDATE %s.users SET money = money + %d WHERE id = '%d'", AZURIOM_DB, (-1) * value, nAzuriomID);
			if (!pQuery->Execute(tmpStr.GetString()))
			{
				Error("Failed to reset ingame DP. value: %d azuriomID: %d", value, nAzuriomID);
			}
	...
	{
		Error("Failed to check new DP value after adjust. adjust value: %d azuriomID: %d", value, nAzuriomID);
	}
#endif // __AEGON_INGAME_DP_AZURIOM
...
void CDbManager::OnDonatePointsFuncPacket(LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus)
{
	u_long playerID;
	...
	OnDonatePointsFunc(lpDbOverlappedPlus, info, playerID);

}
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LAST_ONLINE
void CDbManager::OnInitialLoadLastLogin()
{
	...
	CString tmpStr;
	tmpStr.Format("SELECT m_idPlayer, serverindex, m_szName, m_nLastOnline FROM CHARACTER_TBL");
	...
	CAegonLastOnlineMng::GetInstance()->m_mapLastOnline.clear();

	while (pQuery->Fetch())
	{
		tmpUnit.Init();
		pQuery->GetStr("m_szName", tmpUnit.m_szName);
		tmpUnit.playerID = pQuery->GetInt("m_idPlayer");
		tmpUnit.m_nLastOnline = pQuery->GetInt64("m_nLastOnline");
		tmpUnit.m_bOnline = FALSE;

		if (tmpUnit.playerID > 0) 
		{
			CAegonLastOnlineMng::GetInstance()->m_mapLastOnline.insert(make_pair(tmpUnit.playerID, tmpUnit));
		}
	}
	
	BEFORESENDDUAL(out, PACKETTYPE_LASTONLINE_INIT_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	CAegonLastOnlineMng::GetInstance()->WriteMap(out);
	SEND(out, CDPTrans::GetInstance(), DPID_SYSMSG);
...
#ifdef __AEGON_PVP_RANKING
void CDbManager::OnInitialLoadPvPRanking()
{
	...
	CString tmpStr;
	tmpStr.Format("SELECT * FROM PVP_RANKING_TBL");
	...
	CAegonPvPRankingMng::GetInstance()->playerRankingMap.clear();

	char szProgress[4096] = { 0, };

	while (pQuery->Fetch())
	{
		memset(szProgress, 0, 4096);

		tmpUnit.Init();
		tmpUnit.m_nPlayerID = pQuery->GetInt("m_idPlayer");
		pQuery->GetStr("m_szName", tmpUnit.m_szName);
		pQuery->GetStr("m_szProgress", szProgress);

		tmpUnit.ReadValuesFromString(szProgress);

		if (tmpUnit.m_nPlayerID > 0)
		{
			CAegonPvPRankingMng::GetInstance()->playerRankingMap.insert(make_pair(tmpUnit.m_nPlayerID, tmpUnit));
		}
	}

	BEFORESENDDUAL(out, PACKETTYPE_PVPRANKING_INIT_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	CAegonPvPRankingMng::GetInstance()->WriteMap(out);
	SEND(out, CDPTrans::GetInstance(), DPID_SYSMSG);

	SAFE_DELETE(pQuery);
}

void CDbManager::OnSavePvPRanking(__PVPRANKING_PROGRESS_UNIT* saveUnit)
{
	__PVPRANKING_PROGRESS_UNIT tmpUnit;
	char szProgress[4096] = { 0, };
	...
	memset(szProgress, 0, 4096);
	saveUnit->WriteValuesToString(szProgress);
	strncat(szProgress, NullStr, sizeof(NullStr));

	strQuery.Format("{ CALL SAVE_PVP_RANKING('%07d', '%02d', '%s', '%s')}", saveUnit->m_nPlayerID, g_appInfo.dwSys, saveUnit->m_szName, szProgress);
	if (!pQuery->Exec(strQuery))
	{
		WriteLog("%s - %07d, %02d, %s, %s", __FUNCTION__, saveUnit->m_nPlayerID, g_appInfo.dwSys, saveUnit->m_szName, szProgress);
	}
...
void CDbManager::saveTh()
{
	...
	{
		char s[128];
		sprintf_s(s, "%s, %d", __FILE__, __LINE__);
		AfxMessageBox(s);
		SAFE_DELETE(pQuery);
		SAFE_DELETE(pQueryLog);
		return;
	...
	while (true)
	{
		...
		if (!spQueue.empty())
		{
			CMover* player = spQueue.pop_front();

			if (player)
			{
				if (player->m_idPlayer > 0) 
				{
					SavePlayer(pQuery, pQueryLog, player, szQuery);
#ifdef __INVALID_LOGIN_0612
					if (player->m_bLogout)
						g_DbManager.m_AccountCacheMgr.ChangeMultiServer(player->m_szAccount, 0);
#endif
				}
				SAFE_DELETE(player);
			}
...
#ifdef __AEGON_INGAME_SHOP
void CDbManager::OnInitialLoadIngameShop()
{
	CQuery* pQuery = new CQuery;
	CString tmpStr;
	__AEGIGNSHP_CATEGORY tmpCategoryUnit;
	__AEGIGNSHP_ITEM_UNIT tmpItemUnit;
	...
	tmpStr.Format("SELECT * FROM %s.shop_categories", AZURIOM_DB);
	...
	CAegonIngameShopMng::GetInstance()->m_mapCategories.clear();

	while (pQuery->Fetch())
	{
		tmpCategoryUnit.Init();

		tmpCategoryUnit.m_dwCategoryID = pQuery->GetInt("id");
		pQuery->GetStr("name", tmpCategoryUnit.m_szName);
		tmpCategoryUnit.m_nPosition = (DWORD)pQuery->GetInt("position");
		tmpCategoryUnit.m_nParentID = pQuery->GetInt("parent_id");
		tmpCategoryUnit.m_bCumulatePurchases = pQuery->GetInt("cumulate_purchases") == 0 ? FALSE : TRUE;
		tmpCategoryUnit.m_bIsEnabled = pQuery->GetInt("is_enabled") == 0 ? FALSE : TRUE;

		if(CAegonIngameShopMng::GetInstance()->m_mapCategories.find(tmpCategoryUnit.m_nPosition) == CAegonIngameShopMng::GetInstance()->m_mapCategories.end())
		{
			CAegonIngameShopMng::GetInstance()->m_mapCategories.insert(make_pair(tmpCategoryUnit.m_nPosition, tmpCategoryUnit));
		}
	}

	tmpStr.Format("SELECT * FROM %s.shop_packages", AZURIOM_DB);
	...
	CAegonIngameShopMng::GetInstance()->m_mapShopItems.clear();
	...
	while (pQuery->Fetch())
	{
		...
		tmpItemUnit.m_dwItemUnitID = pQuery->GetInt("id");
		tmpItemUnit.m_dwCategoryID = pQuery->GetInt("category_id");
		pQuery->GetStr("name", tmpItemUnit.m_szName);
		//pQuery->GetStr("short_description", tmpItemUnit.m_szShortDescr);
		//pQuery->GetStr("description", tmpItemUnit.m_szDescr);
		tmpItemUnit.m_nPosition = pQuery->GetInt("position");
		tmpItemUnit.m_nPrice = (int)pQuery->GetFloat("price");
		tmpItemUnit.m_bIsEnabled = pQuery->GetInt("is_enabled") == 0 ? FALSE : TRUE;

		pQuery->GetStr("commands", szCommand);
		...
		while (token != NULL) 
		{
			...
			{
				strItemID.erase(std::remove(strItemID.begin(), strItemID.end(), removeChars[i]), strItemID.end());
				strItemName.erase(std::remove(strItemName.begin(), strItemName.end(), removeChars[i]), strItemName.end());
				strItemNum.erase(std::remove(strItemNum.begin(), strItemNum.end(), removeChars[i]), strItemNum.end());
			}

			

			DWORD dwItemID = (DWORD)stoi(strItemID);
			int nItemNum = stoi(strItemNum);

			strItemName.copy(tmpCommandUnit.m_szItemName, 64);
			tmpCommandUnit.m_dwItemID = dwItemID;
			tmpCommandUnit.m_dwItemNum = nItemNum;
			
			if (tmpCommandUnit.IsValid()) 
			{
				tmpItemUnit.m_vCommandUnits.push_back(tmpCommandUnit);
			}
		}
		
		if (CAegonIngameShopMng::GetInstance()->m_mapShopItems.find(tmpItemUnit.m_dwItemUnitID) == CAegonIngameShopMng::GetInstance()->m_mapShopItems.end())
		{
			CAegonIngameShopMng::GetInstance()->m_mapShopItems.insert(make_pair(tmpItemUnit.m_dwItemUnitID, tmpItemUnit));
		}
	}



	BEFORESENDDUAL(out, PACKETTYPE_INGAMESHOP_INIT_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	CAegonIngameShopMng::GetInstance()->WriteCategoryMap(out);
	CAegonIngameShopMng::GetInstance()->WriteItemMap(out);
	SEND(out, CDPTrans::GetInstance(), DPID_SYSMSG);
...
#ifdef __AEGON_TOURNAMENT
void	CDbManager::LoadTournaments()
{
	CQuery* pQuery = new CQuery;
	CString tmpStr;
	...
	tmpStr.Format("SELECT * FROM TOURNAMENTS_TBL WHERE serverindex = '%02d'", g_appInfo.dwSys);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return;
	...
	CAegonTournamentMng::GetInstance()->m_mQueuedTournaments.clear();
	while (pQuery->Fetch())
	{
		__TOURNAMENT_UNIT tmpUnit;
		char szRewards[4096] = { 0, };
		char szTeams[4096] = { 0, };

		tmpUnit.m_dwTournamentID = pQuery->GetInt("m_nTournamentID");
		tmpUnit.m_dwTeamSize = pQuery->GetInt("m_nTeamSize");
		tmpUnit.m_dwRounds = pQuery->GetInt("m_nRounds");
		tmpUnit.m_dwRoundTime = pQuery->GetInt("m_nRoundTime");
		tmpUnit.m_sStartTime.tm_hour = pQuery->GetInt("m_nStartTimeHour");
		tmpUnit.m_sStartTime.tm_min = pQuery->GetInt("m_nStartTimeMin");
		tmpUnit.m_sStartTime.tm_mday = pQuery->GetInt("m_nStartTimeDay");
		tmpUnit.m_sStartTime.tm_mon = pQuery->GetInt("m_nStartTimeMonth");
		tmpUnit.m_sStartTime.tm_year = pQuery->GetInt("m_nStartTimeYear");
		tmpUnit.m_sStartTime.tm_isdst = pQuery->GetInt("m_nStartTimeDst");
		tmpUnit.m_bFriendlyFire = pQuery->GetInt("m_nFriendlyFire") == 0 ? FALSE : TRUE;
		tmpUnit.m_dwLevelReqMin = pQuery->GetInt("m_nLevelReqMin");
		tmpUnit.m_dwLevelReqMax = pQuery->GetInt("m_nLevelReqMax");
		tmpUnit.m_dwAllowedClasses = pQuery->GetInt("m_nAllowedClasses");
		tmpUnit.m_dwTeamReqNum = pQuery->GetInt("m_nTeamReqNum");

		mktime(&tmpUnit.m_sStartTime);

		pQuery->GetStr("m_szRewards", szRewards);
		pQuery->GetStr("m_szTeams", szTeams);

		int nPos = 0;
		tmpUnit.ReadRewardsFromString(szRewards, &nPos);
	...
	BEFORESENDDUAL(out, PACKETTYPE_TOURNAMENT_DB_LOAD, DPID_UNKNOWN, DPID_UNKNOWN);
	CAegonTournamentMng::GetInstance()->WriteTournamentMap(out);
	SEND(out, CDPTrans::GetInstance(), DPID_SYSMSG);

}
void	CDbManager::SaveTournaments()
{
	CQuery* pQuery = new CQuery;
	CString tmpStr;
	...
	tmpStr.Format("DELETE FROM TOURNAMENTS_TBL WHERE serverindex = '%02d'", g_appInfo.dwSys);
	if (!pQuery->Execute(tmpStr.GetString()))
	{
		SAFE_DELETE(pQuery);
		return;
	...
	for (; it != CAegonTournamentMng::GetInstance()->m_mQueuedTournaments.end(); it++) 
	{
		__TOURNAMENT_UNIT* tmpUnit = &it->second;
		if (!tmpUnit)
			continue;
		...
		memset(szRewards, 0, 4096);
		tmpUnit->WriteRewardsToString(szRewards);
		strncat(szRewards, NullStr, sizeof(NullStr));



		memset(szTeams, 0, 4096);
		//TODO teams
		strncat(szTeams, NullStr, sizeof(NullStr));

		int nFriendlyFire = tmpUnit->m_bFriendlyFire ? 1 : 0;

		tmpStr.Format("INSERT INTO TOURNAMENTS_TBL (serverindex, m_nTournamentID, m_nTeamSize, m_nRounds, m_nRoundTime, m_nStartTimeHour, m_nStartTimeMin, m_nStartTimeDay, m_nStartTimeMonth, m_nStartTimeYear, m_nStartTimeDst, m_nFriendlyFire, m_nLevelReqMin, m_nLevelReqMax, m_nAllowedClasses, m_nTeamReqNum, m_szRewards, m_szTeams) \
			VALUES ('%02d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s')",
			g_appInfo.dwSys, tmpUnit->m_dwTournamentID, tmpUnit->m_dwTeamSize, tmpUnit->m_dwRounds, tmpUnit->m_dwRoundTime, tmpUnit->m_sStartTime.tm_hour, tmpUnit->m_sStartTime.tm_min, tmpUnit->m_sStartTime.tm_mday, tmpUnit->m_sStartTime.tm_mon, tmpUnit->m_sStartTime.tm_year, tmpUnit->m_sStartTime.tm_isdst, nFriendlyFire, tmpUnit->m_dwLevelReqMin,
			tmpUnit->m_dwLevelReqMax, tmpUnit->m_dwAllowedClasses, tmpUnit->m_dwTeamReqNum, szRewards, szTeams);

		pQuery->Execute(tmpStr.GetString());
	}
