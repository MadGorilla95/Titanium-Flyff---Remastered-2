...
#ifdef __CREATE_FOLDER
	DOWNLOAD_ERROR_CREATEFOLDER,
#endif // __CREATE_FOLDER
};
...
#ifdef __CREATE_FOLDER
	else if (nResult == DOWNLOAD_ERROR_CREATEFOLDER)
	{
		sprintf(szBuffer, "ERROR: Create Folder");
	}
#endif // __CREATE_FOLDER
	else
	{
...
#ifdef __CREATE_FOLDER		
	if (!CreateFolder(info.szServerPath))
		return DOWNLOAD_ERROR_CREATEFOLDER;
#endif // __CREATE_FOLDER		
...
#ifdef __CREATE_FOLDER
UINT CHttpDownload::CreateFolder(char path[MAX_PATH])
{
	CString tmpStr, eraseStr;
	size_t erasePos, lastSlashPos, slashPos;
	int nSubstrings = 0;
	CString substrings[50];

	tmpStr.Format("%s", path);

	eraseStr.Format(SERVER_ROOT "/");
	erasePos = tmpStr.Find(eraseStr);
	if (erasePos != std::string::npos)
	{
		//remove SERVER_ROOT
		tmpStr.Delete(erasePos, eraseStr.GetLength());
	}

	lastSlashPos = ((string)tmpStr).std::string::find_last_of("/");
	if (lastSlashPos != std::string::npos)
	{
		//remove filename
		tmpStr.Delete(lastSlashPos + 1, tmpStr.GetLength() - lastSlashPos);
	}

	//get substrings
	slashPos = tmpStr.Find("/");
	while (slashPos != std::string::npos)
	{
		if (tmpStr.GetLength() <= 1 || slashPos > tmpStr.GetLength())
		{
			slashPos = std::string::npos;
			continue;
		}

		substrings[nSubstrings] = tmpStr.Mid(0, slashPos);
		tmpStr.Delete(0, slashPos + 1);

		nSubstrings++;
		slashPos = tmpStr.Find("/");
	}

	//create folders from substrings
	for (int i = 0; i < nSubstrings; i++)
	{
		...
		for (int j = 0; j <= i; j++)
		{
			tmp.Append(substrings[j]);
			tmp.Append("/");
		}

		if (CreateDirectory(tmp, NULL) || GetLastError() == ERROR_ALREADY_EXISTS)
		{
			//folder exists / succesfully created
			if (i + 1 == nSubstrings)
			{
				//all folders exist / created
				return 1;
			}
