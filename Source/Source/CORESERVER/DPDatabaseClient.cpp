...
void CDPDatabaseClient::SendSetGuildColor(DWORD guildid, int r, int g, int b)
{
	BEFORESEND(ar, PACKETTYPE_SETGUILDCOLOR);
	ar << guildid << r << g << b;
	SEND(ar, this, DPID_SERVERPLAYER);
}
...
#ifdef __GUILDFINDER
void CDPDatabaseClient::SendGuildFinder(u_long idGuild, BOOL bGuildFinder)
{
	BEFORESEND(ar, PACKETTYPE_CD_GUILD_FINDER);
	ar << idGuild;
	ar << bGuildFinder;
	SEND(ar, this, DPID_SERVERPLAYER);
}
