...
CDPCacheSrvr::CDPCacheSrvr() 
{
	...
	ON_MSG(PACKETTYPE_SETGUILDCOLOR, &CDPCacheSrvr::OnGuildSetColor);
	...
	ON_MSG(PACKETTYPE_ASSISTANT, &CDPCacheSrvr::OnPartyAssistant);
...
void CDPCacheSrvr::SendGetDiffPlayer(int nCount, CPlayer* pOperator)
{
	BEFORESENDSOLE(ar, PACKETTYPE_GETDIFFPLAYER, pOperator->dpidUser);
	ar << nCount;
	SEND(ar, this, pOperator->dpidCache);
}
...
void CDPCacheSrvr::SendSystem(const CHAR* lpString, int nImage, int nFont)
...
#ifdef __PARTY_ASSISTANT
void CDPCacheSrvr::OnPartyAssistant(CAr& ar, DPID dpidCache, DPID dpidUser, u_long uBufSize)
{
	u_long _idLeader, idAssistant;
	ar >> _idLeader >> idAssistant;
	...
	if (pLeader->uKey != idAssistant)
	{
		if (FALSE == pParty->IsLeader(pLeader->uKey))
		{
			return;
		}
		if (!pParty->IsMember(idAssistant))
		{
			SendErrorParty(ERROR_NOTMAMBER, pLeader);//
			return;
		}
	...
	int nFind = pParty->FindMember(idAssistant);
	if (nFind == -1 || pParty->m_aMember[nFind].m_bRemove)
		return;

	g_dpCoreSrvr.SendPartyAssistant(pParty->m_uPartyId, idAssistant);
}
#endif //__PARTY_ASSISTANT
...
void CDPCacheSrvr::OnAddPartyMember( CAr & ar, DPID dpidCache, DPID dpidUser, u_long uBufSize )
{
...
#ifdef __GAMEGUARD
			g_dpCoreSrvr.SendAddPartyMember(pParty->m_uPartyId, pLeader->uKey, nLeaderLevel, nLeaderJob, (BYTE)(dwLSex), pMember->uKey, nMemberLevel, nMemberJob, (BYTE)(dwMSex), pParty->m_nModeTime);
#else // __GAMEGUARD
	...
	else	// new
	{
		...
		if( uPartyId != 0 )
		{
			...
			g_dpCoreSrvr.SendAddPartyMember(uPartyId, pLeader->uKey, nLeaderLevel, nLeaderJob, (BYTE)(dwLSex), pMember->uKey, nMemberLevel, nMemberJob, (BYTE)(dwMSex), pParty->m_nModeTime);
#else // __GAMEGUARD
...
void CDPCacheSrvr::OnPartyChangeTroup( CAr & ar, DPID dpidCache, DPID dpidUser, u_long uBufSize )
{
	...
	{	
		...
		{
#ifdef __RENAMEPARTY
			if (pPlayer->m_dwLastName + SEC(5) < GetTickCount())
			{
				if (pParty->m_nKindTroup == 1 && pParty->GetLevel() >= 10)
				{
					if (g_PartyMng.IsPartyName(szPartyName) == FALSE)
				...
				pPlayer->m_dwLastName = GetTickCount();
			}
			else
			{
				SendDefinedText(TID_PARTY_WAIT, pPlayer->dpidCache, pPlayer->dpidUser, "");
			}
#endif //__RENAMEPARTY
		}
	}
...
void CDPCacheSrvr::OnAddGuildMember(CAr& ar, DPID dpidCache, DPID dpidUser, u_long uBufSize)
{
	...
	{
#ifdef __AEGON_SHOW_GUILD_JOIN_COUNTDOWN
		CTimeSpan tJoinCountdown = pPlayer->m_tGuildMember - tCurrent;
#endif

#ifdef __AEGON_SHOW_GUILD_JOIN_COUNTDOWN
		CString strTmp;
		LONG nTime;
		if (tJoinCountdown.GetTotalMinutes() > 0) 
		{
			strTmp.Format("%lld Minutes", tJoinCountdown.GetTotalMinutes());
		}
		else
		{
			strTmp.Format("%lld Seconds", tJoinCountdown.GetTotalSeconds());
		}
		SendDefinedText( TID_GAME_GUILDNOTINCLUDE, pMaster->dpidCache, pMaster->dpidUser, "%s", strTmp.GetString());
		SendDefinedText( TID_GAME_GUILDNOTINCLUDE, pPlayer->dpidCache, pPlayer->dpidUser, "%s", strTmp.GetString());
#else
...
#ifdef __SYS_GUILDCOLOR
void CDPCacheSrvr::OnGuildSetColor(CAr& ar, DPID dpidCache, DPID dpidUser, u_long uBufSize)
{
	DWORD R, G, B;
	u_long idGuild;
	...
	if (pGuild)
		if (pGuild->IsMaster(pPlayer->uKey))
			if (g_GuildMng.SetColor(pGuild, R, G, B))
			{
				g_dpCoreSrvr.SendGuildSetColor(idGuild, R, G, B);
				g_dpDatabaseClient.SendSetGuildColor(idGuild, R, G, B);
				SendGuildSetColor(idGuild, R, G, B);
			...
			SendDefinedText(TID_GAME_GUILDONLYMASTER, pPlayer->dpidCache, dpidUser, "");
}
void CDPCacheSrvr::SendGuildSetColor(DWORD guildid, int R, int G, int B)
{
	BEFORESENDSOLE(ar, PACKETTYPE_SETGUILDCOLOR, DPID_ALLPLAYERS);
	ar << guildid << R << G << B;
	SEND(ar, this, DPID_ALLPLAYERS);
}
#endif //__SYS_GUILDCOLOR
