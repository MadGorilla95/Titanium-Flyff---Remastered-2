...
CDPCoreSrvr::CDPCoreSrvr()
{
	...
	ON_MSG(PACKETTYPE_SETGUILDCOLOR, &CDPCoreSrvr::OnSetGuildColor);
#endif //__SYS_GUILDCOLOR
#ifdef __SYS_MAP_BUFFS
	ON_MSG(PACKETTYPE_CHECK_MAPBUFFS, &CDPCoreSrvr::OnCommandCheckMapBuffs);
	...
	ON_MSG(PACKETTYPE_GETDIFFPLAYER, &CDPCoreSrvr::OnGetDiffPlayer);
...
#ifdef __VENDING_NPC
	ON_MSG(PACKETTYPE_REMOVEVENDORNPC, &CDPCoreSrvr::OnRemoveNPCVendor);
#endif // __VENDING_NPC

#ifdef __ANARCHY_SYSTEM
	ON_MSG(PACKETTYPE_ANARCHY_BUFFS, &CDPCoreSrvr::OnAnarchyBuff);
#endif // __ANARCHY_SYSTEM

#ifdef __GUILDFINDER
	ON_MSG(PACKETTYPE_WC_GUILDFINDER, &CDPCoreSrvr::OnGuildFinder);
#endif //__GUILDFINDER
...
void CDPCoreSrvr::OnSay( CAr & ar, DPID dpidFrom, DPID dpidCache, DPID dpidUser, u_long uBufSize )
{
	...
	if (bToName)
	{
		ar.ReadString(szToName, MAX_PLAYER);
	}
#endif
...
#ifdef __AEGON_MESSENGER
	else if(pFrom && bToName)
	{
		g_DPCacheSrvr.SendSay(pFrom->lpszPlayer, szToName, idFrom, idTo, lpString, pFrom, 1);
	}
#endif
	else
	{
...
#ifdef __AEGON_COUNT_DIFF_IP
void CDPCoreSrvr::OnGetDiffPlayer(CAr& ar, DPID dpidFrom, DPID dpidCache, DPID dpidUser, u_long)
{
	u_long idOperator;
	ar >> idOperator;
	...
	for (it = g_PlayerMng.m_players.begin(); it != g_PlayerMng.m_players.end(); ++it)
	{
		if (find(ipVec.begin(), ipVec.end(), it->second->lpAddr) != ipVec.end())
		{
			//ip already in vector
		}
		else
		{
			CString strTmp;
			strTmp.Format("%s", it->second->lpAddr);
			ipVec.push_back(strTmp);
		}
	}

	int nCount = ipVec.size();

	if (pOperator)
	{
		//g_DPCacheSrvr.SendGetPlayerAddr(pPlayer->lpszPlayer, pPlayer->lpAddr, pOperator);
		g_DPCacheSrvr.SendGetDiffPlayer(nCount, pOperator);
	}
}
#endif // __AEGON_COUNT_DIFF_IP
...
void CDPCoreSrvr::OnSystem( CAr & ar, DPID, DPID, DPID, u_long uBufSize )
{
	...
	ar >> nImage;
	ar >> nFont;
	g_DPCacheSrvr.SendSystem(lpString, nImage, nFont);
...
#ifdef __GAMEGUARD
void CDPCoreSrvr::SendAddPartyMember(u_long uPartyId, u_long idLeader, long nLeaderLevel, long nLeaderJob, BYTE nLeaderSex, u_long idMember, long nMemberLevel, long nMemberJob, BYTE nMemberSex, int* nModeTime)
#else // __GAMEGUARD
...
void CDPCoreSrvr::SendPartyAssistant(u_long uPartyId, u_long uidAssistant)
{
	BEFORESENDDUAL(ar, PACKETTYPE_ASSISTANT, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << uPartyId << uidAssistant;
	SEND(ar, this, DPID_ALLPLAYERS);
}
...
#ifdef __GAMEGUARD
			g_dpCoreSrvr.SendAddPartyMember(pParty->m_uPartyId, idLeader, nLeaderLevel, nLeaderJob, (BYTE)(dwLSex), idMember, nMemberLevel, nMemberJob, (BYTE)(dwMSex), pParty->m_nModeTime);
#else // __GAMEGUARD
...
#ifdef __GAMEGUARD
			CParty* pParty = g_PartyMng.GetParty(uPartyId);
			g_dpCoreSrvr.SendAddPartyMember(uPartyId, idLeader, nLeaderLevel, nLeaderJob, (BYTE)(dwLSex), idMember, nMemberLevel, nMemberJob, (BYTE)(dwMSex), pParty->m_nModeTime);
#else // __GAMEGUARD
...
#ifdef __VENDING_NPC
void CDPCoreSrvr::OnRemoveNPCVendor(CAr& ar, DPID, DPID, DPID, u_long)
{
	u_long idPlayer;
	ar >> idPlayer;
	SendRemoveNPCVendor(idPlayer);
}

void CDPCoreSrvr::SendRemoveNPCVendor(u_long idPlayer)
{
...
#ifdef __ANARCHY_SYSTEM
void CDPCoreSrvr::OnAnarchyBuff(CAr& ar, DPID, DPID, DPID, u_long)
{
	DWORD dwId;
	ar >> dwId;
	{
...
#ifdef __SYS_GUILDCOLOR
void CDPCoreSrvr::SendGuildSetColor(u_long idGuild, int R, int G, int B)
{
	BEFORESENDDUAL(ar, PACKETTYPE_SETGUILDCOLOR, DPID_UNKNOWN, DPID_UNKNOWN);
	ar << idGuild << R << G << B;
	SEND(ar, this, DPID_ALLPLAYERS);
}
void CDPCoreSrvr::OnSetGuildColor(CAr& ar, DPID, DPID, DPID, u_long)
{
	DWORD guildid;
	int r, g, b;
	...
	g_dpDatabaseClient.SendSetGuildColor(guildid, r, g, b);
	SendGuildSetColor(guildid, r, g, b);
	g_DPCacheSrvr.SendGuildSetColor(guildid, r, g, b);
}
#endif //__SYS_GUILDCOLOR

#ifdef __GUILDFINDER
void CDPCoreSrvr::OnGuildFinder(CAr& ar, DPID, DPID, DPID, u_long)
{
	CMclAutoLock	Lock(g_PlayerMng.m_AddRemoveLock);
	CMclAutoLock	Lock2(g_GuildMng.m_AddRemoveLock);
	...
	g_dpDatabaseClient.SendGuildFinder(idGuild, bGuildFinder);
	{
		BEFORESENDDUAL(ar, PACKETTYPE_CW_GUILDFINDER, DPID_UNKNOWN, DPID_UNKNOWN);
		ar << idGuild;
		ar << bGuildFinder;
		SEND(ar, this, DPID_ALLPLAYERS);
	}
...
#ifdef __SYS_MAP_BUFFS
void CDPCoreSrvr::SendCheckMapBuffs() 
{
	BEFORESENDDUAL(ar, PACKETTYPE_CHECK_MAPBUFFS, DPID_UNKNOWN, DPID_UNKNOWN);
	SEND(ar, this, DPID_ALLPLAYERS);
}
void CDPCoreSrvr::OnCommandCheckMapBuffs(CAr& ar, DPID, DPID, DPID, u_long) 
{
	SendCheckMapBuffs();
}
#endif //__SYS_MAP_BUFFS
