...
void IElection::sProperty::Serialize( CAr & ar )
{
	if( ar.IsStoring() )
	{
...
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_vReturnDepositRates.size());
#else
		...
		{
#ifdef _WIN64
			ar << static_cast<unsigned long>(m_vItems[iIndex].size());
#else
...
#if _WIN64
		ar << static_cast<unsigned long>(m_vCandidates.size());
#else
...
{
	m_pTexture	= CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, "Skill_TroGiftbox02.dds" ), COLOR_ITEM_BG );
}
#endif	// __CLIENT
...
void ILordEvent::Serialize(CAr& ar)
{
	...
	{
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_vComponents.size());
#else
