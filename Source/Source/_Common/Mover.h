...
#define	COLOR_ENEMY				0xFFFF3030	//Red -> Siege Enemy
...
// Authorized Characters -> Please sync with Discord Hex Color

#define COLOR_ADMINISTRATOR     0xFFCC0000 // Administrator (Dark Red)
#define COLOR_OPERATOR			0XFF00DBFF // Developer (Light Blue)
#define COLOR_GAMEMASTER3		0XFFAD1457 // Game Designer (Purple)
#define COLOR_GAMEMASTER2		0XFF1F8B4C // Head GM (Dark Green)
#define COLOR_GAMEMASTER        0XFF9B59B6 // GM (Pink)
#define COLOR_HELPER	        0XFF666699 // Test GM (Light Red)
#define COLOR_BALANCE			0XFF2ECC71 // Balance (Light Green)

// Titanium MMO Specific
#define COLOR_PREMIUM			0XFFFFA500 // Orange

// Titanium MMO Easy Defines
#define COLOR_ERROR				0xFFFF0000 // Red
#define COLOR_SUCCESS			0XFF00F000 // Green
#define COLOR_ITEM_BG			0XFFFF00FF // Pink
#define COLOR_BLACK				0XFF000000 // Black
#define COLOR_WHITE				0XFFFFFFFF // White
#define COLOR_PLACEHOLDER		0XFF8C8C8C // Gray
#define COLOR_ITEMINFO			0XFF666666 // Dark Gray
#define COLOR_WARNING			0XFFFF8C00 // Orange
...
#define UI_OLDRANDOMOPTITEMID	19
#endif // __AEGON_RETURN_AWAKE

#ifdef __WEAPON_RARITY
#define UI_WEAPON_RARITY		20
#endif // __WEAPON_RARITY

#ifdef __UPGRADE_SUCCESS
#define UI_UPGRADE_SUCCESS		21
#endif //__UPGRADE_SUCCESS

#ifdef __PET_GLOW
#define UI_CHANGE_PET_GLOW      22
#endif // __PET_GLOW

#ifdef __STACKABLE_POWERUPS
#define UI_UPDATE_BUFFTIME		23
#endif // __STACKABLE_POWERUPS
...
//	PIERCINGINFO	piercingInfo;
	...
	__int64 iMergedRandomOptItemId;
	int nMergedWeaponATK;
...
#ifdef __WEAPON_RARITY
		nWeaponRarity = e.nWeaponRarity;
#endif // __WEAPON_RARITY
#ifdef __CLIENT
		pTexture	= e.pTexture;
#endif	// __CLIENT
#ifdef __WEAPON_MERGE
		iMergedRandomOptItemId = e.iMergedRandomOptItemId;
		nMergedWeaponATK = e.nMergedWeaponATK;
#endif //__WEAPON_MERGE
		return *this;
	}
...
public:
	...
	BOOL					IsVendorOpen();
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	int						GetAchievementSellCount() { return m_dwAchievementSellCount; }
	void					IncreaseAchievementSellCount() { m_dwAchievementSellCount++; }
	void					ResetAchievementSellCount() { m_dwAchievementSellCount = 0; }
#endif // __AEGON_ACHIEVEMENT_SYSTEM
#ifdef	__VENDING_NPC
	CItemBase** CVTInfo::GetAllItems()
	{
		return m_apItem_VT;
	}
	...
	void                    SetOwner(CMover* pMover);
	void                    SetOwnerId(u_long idOwner)
	{
		m_idOwner = idOwner;
	}
#endif //__VENDING_NPC
...
#ifdef __VENDING_NPC
	void					VendorClose(BOOL bClearTitle = TRUE, BOOL bClearShop = TRUE);	// 개인 상점 닫기
#else	// __VENDING_NPC
	...
	void					VendorSetItem(BYTE nId, BYTE i, short nNum, int nCost, BYTE nBuyMode);
...
#ifdef __VENDING_NPC
	void                    VendorExchange();
	void                    VendorGetItems(CItemBase** ppItems);
#endif // __VENDING_NPC
...
#define REPLACE( uMulti, dwWorld, vPos, type, nLayer )				CheckSandBox( (uMulti), (dwWorld), (vPos), (type), (nLayer) )
#define REPLACE_IMAGE_ARG( uMulti, dwWorld, vPos, type, nLayer)		CheckSandBox( (uMulti), (dwWorld), (vPos), (type), (nLayer), (TRUE) )
...
public:	
	...
	void	InitializeAnimatedHat(DWORD dwItemId);
private:
	CModelObject* m_pAnimatedHat;
public:
	void SetAnimatedHat(CModelObject* pModel) { m_pAnimatedHat = pModel; }
	CModelObject* GainAniHatInfo() { return m_pAnimatedHat; }
#endif //__LNB_ANIMATED_HATS

#ifdef __SYS_SANDBOX
	BOOL	IsWorldSandBox();
	BOOL	CheckSandBox(u_long u_multi, DWORD dwWorldId, D3DXVECTOR3 vPos, REPLACE_TYPE, int nLayer, BOOL bTeleportImage = FALSE);
#endif //__SYS_SANDBOX
...
public:
...
#ifdef __WORLDSERVER
	void	EnterTmpPve(int nType, int nClass);
	void	OutTmpPve(BOOL bSave = TRUE);
	void	DisableAllPets();
	...
	BOOL	HasBuff(WORD wType, DWORD dwId);
	void	RemoveBuff(WORD wType, DWORD dwId);
#else
	...
	int		CountBuffByIk3(DWORD dwIk3);
	...
	BOOL	AddBuff(WORD wType, DWORD dwId, DWORD dwLevel, DWORD tmTime, OBJID oiAttacker = NULL_ID, bool bOverwrite = false);
...
#else
#ifdef __ANARCHY_SYSTEM
	BOOL	AddBuff(WORD wType, WORD wId, DWORD dwLevel, DWORD tmTime, OBJID oiAttacker = NULL_ID, bool bOverwrite = false);
#else // __ANARCHY_SYSTEM
	BOOL	AddBuff(WORD wType, WORD wId, DWORD dwLevel, DWORD tmTime, OBJID oiAttacker = NULL_ID);
#endif // __ANARCHY_SYSTEM
#endif
...
public:
...
#ifdef __AEGON_FLY_LEVEL
	LONG	GetFlightLv(void) { return (GetLevel() >= __AEGON_FLY_LEVEL ? 1 : 0); }
#else
...
#ifdef __VENDING_NPC
	BOOL            m_bNPCVendor;
	u_long          m_idVendorNPC;
	DWORD           m_dwVendorItemID;
#ifdef __AEGON_IMPROVED_SHOPDESIGN
	DWORD			m_dwShopDesign;
#endif
#ifdef __WORLDSERVER
	...
	__AEGON_PET_FILTER_LISTS m_sPetfilterLists;
#endif
...
#ifdef __GAMEGUARD
	int				m_wPoisonDamage;			/// 독으로 깎일 데미지
#else // __GAMEGUARD
...
#ifdef __GAMEGUARD
	int				m_wBleedingDamage;			/// 출혈로 깎일 데미지
#else // __GAMEGUARD
...
#ifdef __PET_GLOW
	CSfx*			m_pSfxPetGlow;
	void            SetSfxPetGlow(DWORD GlowEffect);
#endif // __PET_GLOW
#endif //__CLIENT
...
#ifdef __AEGON_FIXES
	OBJID		m_objIdEatPet;	// 소환중인 비스펫의 인벤토리 위치
public:
	CItemElem* GetEatPetItem(void) { return m_Inventory.GetAtId(m_objIdEatPet); }
	void		SetEatPetItem(OBJID objId) { m_objIdEatPet = objId; }
#endif
...
public:
	TCHAR	m_szOwnTitle[MAX_CUSTOM_TITLE];
	BOOL	m_bShowTitle;
...
public:
	...
	BOOL	CreateItem2(CItemElem* pItem, int nPocket, BOOL bAddDropLog = TRUE);
#else
...
public:
	std::unordered_map<DWORD, LONG> m_adjParamAry;
	std::unordered_map<DWORD, LONG> m_chgParamAry;
#else
private:
...
public:
	...
	BOOL m_bIsWorldBoss;
	vector<u_long> m_wbAttackers;
	vector<const char*> _worldbossHWID;
#endif // __SYS_WORLDBOSS

#ifdef __ANI_WING
	CModelObject* m_pMask;
#endif
...
#ifdef __MOUNT
	DWORD			m_dwMountItemID;
	CModelObject* m_pMountModel;
#endif // __MOUNT
...
private:
	int		m_nIngameDP;
public:
	int		GetIngameDP() { return m_nIngameDP; }
	void	SetIngameDP(int nValue) { m_nIngameDP = nValue; }
...
//	virtual int		OnActCollecting();				// User만 사용되는 것이므로 CUser가서 찾을것.
	...
	void			InitLevel(int nJob, LONG nLevel, BOOL bChangedJob = FALSE);			// 객체를 nLevel로 바꿔줌
#else
	void			InitLevel( int nJob, LONG nLevel, BOOL bGamma = TRUE );			// 객체를 nLevel로 바꿔줌
#endif
#ifdef __NEW_CHANGE_JOB
	void			ChangeJob(int nJob, BOOL bGamma = TRUE);
...
#ifdef __GAMEGUARD
	BOOL			SetPoison(BOOL bApply, OBJID idAttacker = NULL_ID, DWORD tmMaxTime = 0, DWORD tmUnit = 0, int wDamage = 0);	// this를 독에 걸리게 한다.
#else // __GAMEGUARD
...
#ifdef __GAMEGUARD
	BOOL			SetBleeding(BOOL bApply, OBJID idAttacker = NULL_ID, DWORD tmMaxTime = 0, DWORD tmUnit = 0, int wDamage = 0);	// this를 출혈시킨다.
#else // __GAMEGUARD
...
#ifdef __ANI_WING
	void			SetMaskWing(CModel* pModel)
	{
		m_pMask = (CModelObject*)pModel;
	};
#endif

#ifdef __MOUNT
	DWORD			GetMountId() { return m_dwMountItemID; }
	CModelObject* GetMountModel() { return m_pMountModel; }
	void			SetMount(CModel* pModel, int nMountItemID = 0) { m_dwMountItemID = nMountItemID; m_pMountModel = (CModelObject*)pModel; };
#endif // __MOUNT

	void			ClearDuel();
	void			ClearDuelParty();	
#ifdef _WIN64
	[[nodiscard]] int SendActMsg(OBJMSG dwMsg, INT_PTR nParam1 = 0, INT_PTR nParam2 = 0, INT_PTR nParam3 = 0, INT_PTR nParam4 = 0) const;
#else
...
#ifdef __STOP_EXP
	void			SetExpStop(BOOL bState) { m_bExpStop = bState; }
	BOOL			IsExpStop() { return m_bExpStop; }
	BOOL			m_bExpStop;
#endif //__STOP_EXP
...
#ifdef __HEAL_SHOW
	void			IncHitPoint(int nVal, bool bNum);
#else // __HEAL_SHOW
...
private:
	BOOL			m_bFightState;
public:
	BOOL			IsSiegeTarget(CMover* pMover);
	BOOL			GetSiegeFightState() { return m_bFightState; };
	void			SetSiegeFightState(BOOL bFightState) { m_bFightState = bFightState; };
...
#ifdef __SYS_PLAYER_SYNC
	void			ResetDestParamRandomOpt(CItemElem* pItemElem, BOOL bSend = FALSE);
...
#ifdef __SYS_PLAYER_SYNC
	void	ResetDestParamRandomOptOrigin(CItemElem* pItemElem, BOOL bSend = FALSE);
#else //__SYS_PLAYER_SYNC
	...
	void			UpdateItemBag(BYTE nId, CHAR cParam, DWORD dwValue, int nBag, DWORD dwTime = 0);
...
#ifdef __FL_BAG_UPDATE
	BOOL			DoUseItemBag(DWORD dwItemId, DWORD dwFocusId, int nBag);
#endif //__FL_BAG_UPDATE
...
//	BOOL			IsResourceMonster() { return m_nResource != -1; }		// 자원몬스터냐? -1이면 자원몬스터가 아니다. 0 ~ 자원몬스터라는 뜻.
	...
	int				GetQueueCastingTime(const int nActionSlot);
	...
	BOOL			CreateItem(CItemBase* pItemBase, BYTE* pnId = NULL, short* pnNum = NULL, BYTE nCount = 0, BOOL bAdd = FALSE);
#else
	BOOL			CreateItem(CItemBase* pItemBase, BYTE* pnId = NULL, short* pnNum = NULL, BYTE nCount = 0);
#endif
#ifdef __MIKSIK_EQUALIZED_MODE
	void			CreateItemTmp(CItemBase* pItemBase);
#endif //__MIKSIK_EQUALIZED_MODE
	...
	BOOL			Replace( u_long uIdofMulti, DWORD dwWorldID, D3DXVECTOR3 & vPos, REPLACE_TYPE type, int nLayer, BOOL bTeleportImage = FALSE );
#if __VER < 8 // __S8_PK
...
#ifdef __MOUNT
	void			MountMotion(CModelObject* pModel);
#endif // __MOUNT
...
#ifdef __AEGON_MONSTER_HUNT
public:
	__MONSTERHUNT_PROGRESS m_MHProgress;
	__MONSTERHUNT_PROGRESS* GetMHProgress();
	void ProcessMonsterHunt(CMover* pAttacker);
#endif // __AEGON_MONSTER_HUNT

#ifdef __PARTY_ASSISTANT
	BOOL	m_bAssistMember;
	BOOL	GetAssist() { return m_bAssistMember; }
#endif //__PARTY_ASSISTANT

#ifdef __WORLDSERVER
	void AddSfxSnd(DWORD dwSfx, DWORD dwSound);
#endif // __WORLDSERVER

#ifdef __AEGON_INGAME_DP
	BOOL	m_bDPCommandInProgress;
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LOOT_BOX
	BOOL	m_bLootBoxInProgress;
	DWORD	m_dwLootboxItem;
	DWORD	m_dwLootboxItemNum;
#endif // __AEGON_LOOT_BOX

#ifdef __AEGON_TOURNAMENT
public:
	int				m_nTournamentState;
	vector<u_long>	m_vFriendlyIDs;
	BOOL			IsTournamentTarget(CMover* pMover);
	void			ProcessTournament();
#endif // __AEGON_TOURNAMENT

#ifdef __BADGE
	void SetBadge(const int nbadge);
	int GetBadge() const;

#ifdef __WORLDSERVER
	void SetBadgeAddType(int Req, int Cond, int Default = 0);
#endif //__WORLDSERVER

	inline vector<int>& GetOwnedBadges() { return m_OwnedBadges; }

protected:
	vector<int> m_OwnedBadges;
	int m_nBadge;
#endif //__BADGE

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
public:
	__ACHIEVEMENT_PROGRESS	m_AchievementProgress;
	__ACHIEVEMENT_PROGRESS*  GetAchievementProgress();

	DWORD m_AchievementNoticeTime;
	vector<int> m_AchievementNoticeID;

	vector<int> m_AchievementFavorites;

	__int64 m_nCollectingTime;
	__time64_t m_nCollectingTimePeriod;
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_USEFULL_FUNCTIONS
public:
	CString GetAuthString();
...
#ifdef __BUYBACK
	CItemContainer<CItemElem>	m_BuyBack;
#ifdef __CLIENT
	BOOL						m_bCanBuy;
#endif // __CLIENT
#endif // __BUYBACK
...
#ifdef __AEGON_SEASON_PASS
	BOOL m_bUpdateSeasonPass;
	__SEASONPASS_PROGRESS m_SeasonpassProgress;
#endif // __AEGON_SEASON_PASS

#ifdef __SWITCH_EQUIP
	vector<__EQUIP_SWITCH_INFO> m_vEquipSwitchInfo;
	DWORD m_dwEquipSwitchTime;
	void WriteEQSwitch(CAr& ar, vector<__EQUIP_SWITCH_INFO>* sourceVec);
	void ReadEQSwitch(CAr& ar, vector<__EQUIP_SWITCH_INFO>* vecTemp);
#endif // __SWITCH_EQUIP

#ifdef __AEGON_PVP_RANKING
	DWORD m_dwLastPvPRankRefresh;
#endif // __AEGON_PVP_RANKING
	
#ifdef __AEGON_PERMANENT_BUFF
	vector<DWORD> m_vPermanenetBuffs;
#endif // __AEGON_PERMANENT_BUFF

#ifdef __AEGON_PREM_MOTIONS
	vector<DWORD> m_vUnlockedMotions;
#endif // __AEGON_PREM_MOTIONS

#ifdef __AEGON_EMOTE_SYSTEM
	vector<DWORD> m_vUnlockedEmotes;
...
#ifdef __AEGON_F_TOOL
	vector<__AEGON_F_TOOL_UNIT> m_vFToolUnits;
	void ProcessFTool();
#endif

#ifdef __AEGON_FIXES
	DWORD m_dwLastQuestProcess;
#endif

#ifdef __AEGON_BOSS_MECHANICS
	DWORD m_dwLastSpecialAttack;
#endif

#ifdef __SYS_REBIRTH
protected:
	USHORT m_nRebirthLevel;
public:
	USHORT GetRebirthLevel() const;
	void SetRebirthLevel(USHORT nLevel);
#endif //__SYS_REBIRTH
...
public:
#if defined(__CLIENT)
	const void SetWingSfx(const DWORD sfx);
#elif defined(__WORLDSERVER)
	const void SetWingSfx(const DWORD sfx) { m_dwWingSfx = sfx; }
#endif
	const DWORD GetWingSfx() const { return m_dwWingSfx; }
	const void CreateWingSfx();
#endif //__NEW_SFX_WINGS

#ifdef __FL_BAG_UPDATE
	BOOL	CanBeUsedInBag(ItemProp* pItemProp);
#endif //__FL_BAG_UPDATE

#ifdef __AEGON_FIXES
	BOOL m_bPrintedItemDuration;
#endif

#ifdef __AEGON_FIXES
	DWORD m_dwOwnerID;
	DWORD m_dwLastInventoryFullMsg;
#endif

#ifdef __AEGON_FIXES
	void SetPartyMemberPos(OBJID objid, D3DXVECTOR3 vPos, int nPlayerID, DWORD dwWorldID);
#endif
};

#ifdef __SYS_REBIRTH
inline USHORT CMover::GetRebirthLevel() const
{
	return m_nRebirthLevel;
}

inline void CMover::SetRebirthLevel(USHORT nLevel)
{
	m_nRebirthLevel = nLevel;
}
#endif //__SYS_REBIRTH
...
#ifdef _WIN64
inline int CMover::SendActMsg(const OBJMSG dwMsg, const INT_PTR nParam1, const INT_PTR nParam2, const INT_PTR nParam3, const INT_PTR nParam4) const
#else
...
#ifdef __GAMEGUARD
	int nGold;
	return ((this->m_dwAuthorization >= AUTH_ADMINISTRATOR) && ((m_dwMode & FREE_MONEY_MODE) == FREE_MONEY_MODE)) ? nGold = INT_MAX : nGold = m_dwGold;
#else //__GAMEGUARD
	int nGold = m_dwGold;
#endif //__GAMEGUARD
...
inline void CMover::SetGold( int nGold )
{
	ASSERT( nGold >= 0 );
#ifdef __GAMEGUARD
	if (!((this->m_dwAuthorization >= AUTH_ADMINISTRATOR) && ((m_dwMode & FREE_MONEY_MODE) == FREE_MONEY_MODE)))
#endif // __GAMEGUARD
	{
