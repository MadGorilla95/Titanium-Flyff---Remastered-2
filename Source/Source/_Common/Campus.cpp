...
void CCampus::Serialize( CAr & ar )
{
	if( ar.IsStoring() )
	{
		ar << m_idCampus << m_idMaster;
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_mapCM.size());
#else
		ar << m_mapCM.size();
#endif
		for( MAP_CM::iterator it = m_mapCM.begin(); it != m_mapCM.end(); ++it )
...
void CCampusMng::Serialize( CAr & ar )
{
	...
	if( ar.IsStoring() )
	{
		ar << m_idCampus;
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_mapCampus.size());
#else
		ar << m_mapCampus.size();
#endif
		for( MAP_CAMPUS::iterator it = m_mapCampus.begin(); it != m_mapCampus.end(); ++it )
...
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_mapPid2Cid.size());
#else
