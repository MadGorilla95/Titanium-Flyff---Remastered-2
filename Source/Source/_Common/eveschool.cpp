...
void CGuildCombat::GuildCombatRequest( CUser* pUser, DWORD dwPenya )
{
	...
	if (pGuild && (pGuild->IsMaster(pUser->m_idPlayer) || pGuild->IsCmdCap(pMember->m_nMemberLv, PF_GUILDCOMBAT))) // If Master or Assist
...
#endif //__NEW_GUILD_PERM
	{
		...
		{
			pUser->AddText("You can't join guild siege without a guild name!", COLOR_ERROR);
			return;
		}
		char szStr[MAX_G_NAME];
		strcpy(szStr, pGuild->m_szGuild);
		...
		for (int i = 0; i < nLen; i++)
		{
			...
			if (szStr[i] != 'a' && szStr[i] != 'b' && szStr[i] != 'c' && szStr[i] != 'd' && szStr[i] != 'e' && szStr[i] != 'f'
				...
				&& szStr[i] != 'W' && szStr[i] != 'X' && szStr[i] != 'Y' && szStr[i] != 'Z')
			{
				pUser->AddDiagText(prj.GetText(TID_DIAG_0020));
				return;
			}
...
void CGuildCombat::GuildCombatResult( BOOL nResult, u_long idGuildWin )
{
	...
	for (int i = 0; i < m_vecGuildCombatMem.size(); i++) 
	{
		for (int j = 0; j < m_vecGuildCombatMem[i]->vecGCSelectMember.size(); j++) 
		{
			CUser* tmpUser = (CUser*)prj.GetUserByID(m_vecGuildCombatMem[i]->vecGCSelectMember[j]->uidPlayer);
			if (IsValidObj(tmpUser))
			{
				__PVPRANKING_PROGRESS_UNIT* tmpUnit = CAegonPvPRankingMng::GetInstance()->GetStatUnit(tmpUser->m_idPlayer);
				if (tmpUnit) 
				{
					tmpUnit->m_Guildwar.m_nParticipations++;
					tmpUnit->CalculatePoints();
					CAegonPvPRankingMng::GetInstance()->SaveStatUnit(tmpUser->m_idPlayer);
				}
			}
		}
	}
	...
	if( 0 < vecSameidGuildWin.size() )
	{
		...
		if( pGuild )
		{
			...
			__GuildCombatMember* combatMembers = FindGuildCombatMember(pGuild->m_idGuild);
			if (combatMembers) 
			{
				for (int i = 0; i < combatMembers->vecGCSelectMember.size(); i++) 
				{
					CUser* tmpUser = (CUser*)prj.GetUserByID(combatMembers->vecGCSelectMember[i]->uidPlayer);
					if (IsValidObj(tmpUser) && tmpUser->m_AchievementProgress.isUnfinishedType(AEGACH_GWWIN))
					{
						CAegonAchievementMng::GetInstance()->AddAchievementProgressForType(tmpUser, AEGACH_GWWIN, 1);

#ifdef __AEGON_PVP_RANKING
						__PVPRANKING_PROGRESS_UNIT* tmpUnit = CAegonPvPRankingMng::GetInstance()->GetStatUnit(tmpUser->m_idPlayer);
						if (tmpUnit)
						{
							tmpUnit->m_Guildwar.m_nWins++;
							tmpUnit->CalculatePoints();
							CAegonPvPRankingMng::GetInstance()->SaveStatUnit(tmpUser->m_idPlayer);
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
		CUser* mvpUser = (CUser*)prj.GetUserByID(m_uBestPlayer);
		if (IsValidObj(mvpUser) && mvpUser->m_AchievementProgress.isUnfinishedType(AEGACH_GWMVP))
		{
			CAegonAchievementMng::GetInstance()->AddAchievementProgressForType(mvpUser, AEGACH_GWMVP, 1);
		}

#ifdef __AEGON_PVP_RANKING
		__PVPRANKING_PROGRESS_UNIT* tmpUnit = CAegonPvPRankingMng::GetInstance()->GetStatUnit(mvpUser->m_idPlayer);
		if (tmpUnit)
		{
			tmpUnit->m_Guildwar.m_nMvp++;
			tmpUnit->CalculatePoints();
			CAegonPvPRankingMng::GetInstance()->SaveStatUnit(mvpUser->m_idPlayer);
...
void CGuildCombat::GuildCombatResultRanking()
{
...
#ifdef __MIKSIK_GS_REWARD
		float fChipNum = 100;		//Change Guildsiege RC Reward
#else // __MIKSIK_GS_REWARD
		float fChipNum = m_nJoinPenya * vecGCRanking.size() * 0.9f * 0.00001f * 0.1f;
#endif // __MIKSIK_GS_REWARD
		switch( i )
		{
			case 0 :	// 1위 
				fChipNum *= 0.7f;
	...
	for (map<u_long, CGuildMember*>::iterator i = pGuild->m_mapPMember.begin(); i != pGuild->m_mapPMember.end(); ++i)
	{
		...
		CUser* pUsertmp = (CUser*)prj.GetUserByID(pMember->m_idPlayer);
		if (IsValidObj(pUsertmp))
		{
			if (m_nWinGuildCount == 1)
				pUsertmp->AddBuff(BUFF_ITEM, II_SYS_SYS_LS_WINNERDIGNITY, 0, d * 1000, 0);
			else if (m_nWinGuildCount == 2)
				pUsertmp->AddBuff(BUFF_ITEM, II_SYS_SYS_LS_SPLENDIDGLORY, 0, d * 1000, 0);
		}
	}
...
#ifdef __LAYER_1015
	g_UserMng.ReplaceWorld( WI_WORLD_GUILDWAR, WI_WORLD_MADRIGAL, 6975.65f, 3331.5f, nDefaultLayer );
#else	// __LAYER_1015
...
#ifdef __NEW_GUILD_PERM
				if (pGuildMember->m_nMemberLv == GUD_MASTER || pGuildMember->m_nMemberLv == GUD_KINGPIN || pGuild->IsCmdCap(pGuildMember->m_nMemberLv, PF_GUILDCOMBAT))
#else //__NEW_GUILD_PERM
