...
#ifdef __fmod
				SoundSystemClass::playSound(m_dwSndHit, { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
void CSfxSkillMagStrongWind::Process()
{
	...
	if( m_bHit == FALSE ) 
	{
		...
		{
#ifdef __fmod
			SoundSystemClass::playSound(SND_PC_SKILLM_STRONGWIND2, { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
void CSfxSkillMagSwordWind::Process()
{
	...
	if(m_bHit==FALSE) 
	{
		...
		{
#ifdef __fmod
			SoundSystemClass::playSound(SND_PC_SKILLM_SWORDWIND2, { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
void CSfxSkillMagFireBoomerang::Process()
{
	...
	if(m_bHit==FALSE) 
	{
		...
		{
#ifdef __fmod
			SoundSystemClass::playSound(SND_PC_SKILLM_FIREBOOMERANG2, { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
//		DamageToTarget( m_nDmgCnt++, 0, 0, nMaxDmgCnt );
		...
		if( m_nDmgCnt >= nMaxDmgCnt )
			m_idSfxHit = 0;
#ifdef __fmod
		SoundSystemClass::playSound(SND_PC_SKILLM_HOTAIR2, { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
#ifdef __VENDING_NPC
	if (g_Neuz.m_pCreateMonItem == NULL && g_Neuz.m_pVendingNpcItem == NULL)
		Delete();
#else // __VENDING_NPC
...
void CSfxWings::Render(LPDIRECT3DDEVICE9 pd3dDevice)
{
	if (!IsVisible() || (IsCull() && GetType() != 1))
		return;
	...
	m_pSfxObj->m_vScale = GetScale();
	m_pSfxObj->m_matScale = m_matScale;
	m_pSfxObj->Render(pd3dDevice, nullptr);
}
