...
// 히트포인트 증감
#ifdef __HEAL_SHOW
void CMover::IncHitPoint(int nVal, bool bNum)
#else // __HEAL_SHOW
...
#endif // __HEAL_SHOW
{
	...
	if (bNum)
		g_UserMng.AddHealDamage(this, GETID(this), nVal);
...
#ifdef __WORLDSERVER
		if (GetWorld())
			CMapBuffManager::GetInstance()->Handle(this, this->GetWorld()->GetID());
#endif //__WORLDSERVER
#endif //__SYS_MAP_BUFFS
...
BOOL CMover::SetExperience( EXPINTEGER nExp1, int nLevel )
{
	...
	if( nLevel > m_nLevel )
	{
		...
		if( g_WndMng.m_pWndWorld )
		{
				...
				g_Caption1.AddCaption(prj.GetText(TID_GAME_LEVELUP_CAPTION), NULL, NULL, D3DCOLOR_ARGB(255, 255, 255, 255), 6, 0);
#else //__AEGON_ADVANCED_ANNOUNCEMENT
			if (::GetLanguage() != LANG_JAP)
				g_Caption1.AddCaption(prj.GetText(TID_GAME_LEVELUP_CAPTION), g_WndMng.m_pWndWorld->m_pFontAPICaption);
			else
		...
		if( IsPlayer() )
		{
			...
			if( pWndWorld ) 
			{
				...
				BOOL bAddCharButton = TRUE;
				if (g_pPlayer) 
				{
					if (g_pPlayer->m_nAutoStat > 0 && g_pPlayer->m_nAutoStat < 5)
						bAddCharButton = FALSE;
				}
...
#ifdef __CLIENT
	case DST_COLLECTOR_TIME:
	{
		if (m_nMaxCltTime)
		{
			m_nMaxCltTime = (int)(m_nMaxCltTime * (100.f - nAdjParameterValue) / 100.0f);
		}
		break;
	}
#endif // __CLIENT
...
{
	auto findDST = m_adjParamAry.find(nDestParameter);
	if (findDST != m_adjParamAry.end())
		return findDST->second;

	return 0;
}
int CMover::GetChgParam(int nDestParameter)
{
	auto findDST = m_chgParamAry.find(nDestParameter);
	if (findDST != m_chgParamAry.end())
		return findDST->second;
...
{
#ifdef	__GAMEGUARD
	int nHitPoint = GetParam(DST_HP, m_nHitPoint);
...
#endif	// __EVENT1206
#ifdef	__GAMEGUARD
	int nManaPoint = GetParam(DST_MP, m_nManaPoint);
	int nMaxManaPoint = GetMaxManaPoint();

	if (nManaPoint > nMaxManaPoint)
		return nMaxManaPoint;
	else
		return nManaPoint;
#endif // __GAMEGUARD
...
#endif	// __EVENT1206
#ifdef	__GAMEGUARD
	int nFatiguePoint = GetParam(DST_FP, m_nFatiguePoint);
	int nMaxFatiguePoint = GetMaxFatiguePoint();

	if (nFatiguePoint > nMaxFatiguePoint)
		return nMaxFatiguePoint;
	else
		return nFatiguePoint;
#endif // __GAMEGUARD
...
//		return fSrcSpeed * (nChgValue / 100.0f);		// ex) fSrcSpeed * 0.5f
	...
	{
#ifdef __MAX_SPEED
		if (nAdjValue > __MAX_SPEED && this->m_dwAuthorization < AUTH_GAMEMASTER3)
		{
			nAdjValue = __MAX_SPEED;
		}
#endif // __MAX_SPEED
#ifdef __JEFF_11
		fSrcSpeed = fSrcSpeed + ( fSrcSpeed * (nAdjValue / 1000.0f) );
		if( fSrcSpeed < 0.0F )
			fSrcSpeed	= 0.0F;
#endif	// __JEFF_11
		return fSrcSpeed;
...
#ifdef __CLIENT
#ifdef __fmod
	SoundSystemClass::playMusic(BGM_EV_END, false);
#else
...
#ifdef __CLIENT
#ifdef __fmod
		SoundSystemClass::playMusic(BGM_EV_START, false);
#else
...
void CMover::UpdateItemEx( unsigned char id, char cParam, __int64 iValue )
{
	...
	if( pItemElem )
	{
		switch( cParam )
		{
			...
			case UI_OLDRANDOMOPTITEMID:
			{
				pItemElem->SetOldRandomOptItemId(iValue);
				break;
			}
...
void CMover::UpdateItemBag(BYTE nId, CHAR cParam, DWORD dwValue, int nBag, DWORD dwTime)
{
	CItemBase* pItemBase = GetItemId2(nBag, nId);
	...
	if (pItemBase)
	{
		...
		{
			if ((int)dwValue <= 0)
				m_Pocket.RemoveAtId(nBag, nId, 1);
			else
...
#ifdef __WORLDSERVER
	if (IsPlayer())
		static_cast<CUser*>(this)->AddUpdateItem(0, nId, cParam, dwValue, dwTime, nBag);
#endif // __WORLDSERVER
}
...
#endif // __PETVIS
{
	...
	if( pItemBase )
	{
		switch( cParam )
		{
			...
			case UI_UPDATE_BUFFTIME:
			{
				ItemProp* pItemProp = prj.GetItemProp(dwValue);
				if (pItemProp)
				{
					IBuff* pBuff = m_buffs.GetBuff(BUFF_ITEM, pItemProp->dwID);
					if (pBuff)
					{
						pBuff->AddTotal(pItemProp->dwSkillTime + pBuff->GetRemain());
					}
				}
				break;
			}
			...
			case UI_ULTIMATE_PIERCING:
				{
					...
					int nCnt = 0;
					int tmp_dia[5] = { 0,0,0,0,0 };
					memset(tmp_dia, 0, sizeof(tmp_dia));
					for (int i = 0; i < ((CItemElem*)pItemBase)->GetUltimatePiercingSize(); i++)
					{
						if (((CItemElem*)pItemBase)->GetUltimatePiercingItem(i) != 0)
						{
							tmp_dia[nCnt] = ((CItemElem*)pItemBase)->GetUltimatePiercingItem(i);
							nCnt++;
						}
					}
					for (int i = 0; i < ((CItemElem*)pItemBase)->GetUltimatePiercingSize(); i++)
					{
						((CItemElem*)pItemBase)->SetUltimatePiercingItem(i, tmp_dia[i]);
					}
...
#ifdef __fmod
					SoundSystemClass::playSound(SND_INF_EQUIPACCESSORY);
#else
			...
			case UI_OLDRANDOMOPTITEMID:
			{
				((CItemElem*)pItemBase)->SetOldRandomOpt(dwValue);
				break;
			}
#endif // __AEGON_RETURN_AWAKE

#ifdef __WEAPON_RARITY
			case UI_WEAPON_RARITY:
			{
				((CItemElem*)pItemBase)->SetWeaponRarity(dwValue);
				break;
			}
#endif // __WEAPON_RARITY

#ifdef __UPGRADE_SUCCESS
			case UI_UPGRADE_SUCCESS:
			{
				static_cast<CItemElem*>(pItemBase)->m_nUpgradeTries = dwValue;
				break;
			}
#endif // __UPGRADE_SUCCESS

#ifdef __PET_GLOW
			case UI_CHANGE_PET_GLOW:
			{
				((CItemElem*)pItemBase)->m_nPetGlow = dwValue;
				break;
			}
#endif // __PET_GLOW
...
float CMover::GetItemDropRateFactor( CMover* pAttacker )
{
	...
	fFactor *= (pAttacker->GetAdjParam(DST_CHANCEDROP) / 100) + 1.0f;
...
#ifdef __SYS_REBIRTH
	fFactor *= prj.GetRebirthProperty(pAttacker->GetRebirthLevel()).fDropRate;
#endif //__SYS_REBIRTH
#endif	// __WORLDSERVER
	return fFactor;
}
...
float CMover::GetPieceItemDropRateFactor( CMover* pAttacker )
{
	...
	fFactor *= (pAttacker->GetAdjParam(DST_CHANCEDROP_PIECE) / 100) + 1.0f;
...
#endif // __EVENTLUA
#ifdef __SYS_REBIRTH
	fFactor *= prj.GetRebirthProperty(m_nRebirthLevel).fExpRate;
#endif *//__SYS_REBIRTH
...
#ifdef __SYS_PLAYER_SYNC
void CMover::SetParam(int dst, int adj) {
	auto findparam = m_adjParamAry.find(dst);
	if (findparam == m_adjParamAry.end()) {
		// Insert & Set Param
		m_adjParamAry.insert(make_pair(
			(DWORD)dst,
			(LONG)adj
		));
	}
