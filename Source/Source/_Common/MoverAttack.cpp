...
#ifdef __MAX_SPEED
#ifdef __CLIENT
	if (nSpeedRun > __MAX_SPEED && !g_pPlayer->IsAuthHigher(AUTH_GAMEMASTER3))
...
#ifdef __MAP_RESTRICTION
	if (GetWorld() && IsAuthHigher(AUTH_GAMEMASTER3) == FALSE)
	{
		DWORD dwWorld = GetWorld()->GetID();
		if (prj.m_mapRestrict[dwWorld][RST_CAST].size() != 0)
			if (nSpellCastSpeed > prj.m_mapRestrict[dwWorld][RST_CAST][0])
				nSpellCastSpeed = prj.m_mapRestrict[dwWorld][RST_CAST][0];
	}
#endif // __MAP_RESTRICTION
...
{
	int nReflectDamage;
	nReflectDamage = GetParam(DST_REFLECT_DAMAGE, 0);
...
int CMover::GetWeaponATK( DWORD dwWeaponType )
{
	...
	case WT_RANGE_BOW:
#ifdef __AEGON_BOW_FACTOR
		nATK = (int)((((GetDex() - 14) * GetJobPropFactor(JOB_PROP_BOW) + (GetLevel() * 1.3f) + (GetStr() * 0.2f)) * 0.7f));
#else //__AEGON_BOW_FACTOR
	...
	if( IsPlayer() )
	{
		...
		*pnMax += nPlus;
		...
		if( pWeapon )
		{
			...
			WeaponRarity* nWeaponRarity = prj.m_nWeaponRarity.GetAt(pWeapon->GetWeaponRarity());
			if (nWeaponRarity && pWeapon->GetWeaponRarity() > 0)
			{
				*pnMin = (int)(*pnMin * nWeaponRarity->m_fAttackRate);
				*pnMax = (int)(*pnMax * nWeaponRarity->m_fAttackRate);
			}
...
#ifdef __GAMEGUARD
		&& ((CUser*)this)->GetWorld()->GetID() != WI_WORLD_ARENA && ((CUser*)this)->GetWorld()->GetID() != WI_WORLD_GUILDWAR
#endif //__GAMEGUARD
#ifdef __ROYAL_RUMBLE
		&& ((CUser*)this)->GetWorld()->GetID() != WI_WORLD_FWC
...
#ifdef __JEFF_11
#ifdef __MIKSIK_BOSSCRIT
        if( fCriticalBonus < 1.0F )
            fCriticalBonus    = 1.0F;
#else
        if( fCriticalBonus < 0.1F )
            fCriticalBonus    = 0.1F;
#endif //__MIKSIK_BOSSCRIT
#endif // __JEFF_11
...
{	
	...
	if (m_nTournamentState == 1 && pMover->m_nTournamentState == 1)
		return TRUE;
...
{
	if (pMover && pMover->IsPlayer() && pMover->m_nTournamentState == 1 && m_nTournamentState == 1)
	{
		if (easy_find(m_vFriendlyIDs, pMover->m_idPlayer))
			return FALSE;
...
void CMover::SetDamagedMotion( CMover* pAttacker, DWORD dwAtkFlags )
{
	...
	if( m_pActMover->GetCastingEndTick() > dwTmpTick && ( nTmpSkillID == SI_KNT_HERO_DRAWING 
...
#ifdef __ROYAL_RUMBLE
BOOL CMover::IsSiegeTarget(CMover* pMover)
{
	if (GetWorld()->m_dwWorldID != WI_WORLD_FWC)
		return FALSE;
