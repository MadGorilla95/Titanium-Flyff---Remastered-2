...
void CMover::Init()
{
	...
	m_idPlayer			= NULL_ID;
#if defined(__ANI_WING) && defined(__CLIENT)
	m_pMask = NULL;
#endif
...
//	m_nCollectOwnCnt = 0;
...
#ifdef __4TH_HIT_HACK
	last_atkmsg = 0;
#endif //__4TH_HIT_HACK
	...
	m_bDPCommandInProgress = FALSE;
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LOOT_BOX
	m_bLootBoxInProgress = FALSE;
	m_dwLootboxItem = 0;
	m_dwLootboxItemNum = 0;
#endif // __AEGON_LOOT_BOX

#ifdef __AEGON_TOURNAMENT
	m_nTournamentState = 0;
	m_vFriendlyIDs.clear();
#endif // __AEGON_TOURNAMENT
	...
	m_AchievementProgress.Init();

	m_AchievementNoticeTime = 0;
	m_AchievementNoticeID.clear();

	m_AchievementFavorites.clear();

	m_nCollectingTime = 0;
	m_nCollectingTimePeriod = 0;
...
#ifdef __BUYBACK
	m_BuyBack.SetItemContainer(ITYPE_ITEM, MAX_BUYBACK);
#ifdef __CLIENT
	m_bCanBuy = TRUE;
#endif // __CLIENT
#endif // __BUYBACK
...
#if defined(__SYS_WORLDBOSS) && defined(__WORLDSERVER)
	m_bIsWorldBoss = FALSE;
...
#ifdef __AEGON_FIXES
	m_dwLastQuestProcess = 0;
#endif

#ifdef __AEGON_BOSS_MECHANICS
	m_dwLastSpecialAttack = 0;
#endif
...
#ifdef __AEGON_FIXES
	m_bPrintedItemDuration = FALSE;
#endif

#ifdef __AEGON_FIXES
	m_dwOwnerID = NULL_ID;
	m_dwLastInventoryFullMsg = 0;
#endif

#ifdef __AEGON_DB_PETFILTER
	m_dwPetfilter = 0;
	m_sPetfilterLists.Init();
#endif
}
...
#ifdef __V19_FORMATS
						if( aItemprop->dwDestParam3 != -1 )
							ResetDestParam( aItemprop->dwDestParam3, aItemprop->nAdjParamVal3 );
						if( aItemprop->dwDestParam4 != -1 )
							ResetDestParam( aItemprop->dwDestParam4, aItemprop->nAdjParamVal4 );
						if( aItemprop->dwDestParam5 != -1 )
							ResetDestParam( aItemprop->dwDestParam5, aItemprop->nAdjParamVal5 );
						if( aItemprop->dwDestParam6 != -1 )
							ResetDestParam( aItemprop->dwDestParam6, aItemprop->nAdjParamVal6 );
#endif //__V19_FORMATS
					}
#ifdef __PREMIUM
					else if (i == SM_PREMIUM)
					{
						if (aItemprop->dwDestParam1 != -1)
							ResetDestParam(aItemprop->dwDestParam1, aItemprop->nAdjParamVal1);
						if (aItemprop->dwDestParam2 != -1)
...
void CMover::RemoveSMMode(int nType)
{
	time_t tmCur = (time_t)(CTime::GetCurrentTime().GetTime());
	if (nType >= 0 && nType < SM_MAX)
	{
		int i = nType;
		if (m_dwSMTime[i] > 0)
		{
...
#ifdef __AEGON_FIXES
		if (m_dwLastQuestProcess + 1000 > GetTickCount())
			return;
#endif
...
#ifdef __AEGON_FIXES
		m_dwLastQuestProcess = GetTickCount();
#endif
...
#endif // __VENDOR_UPDATE
				{
					if(pCharacter->m_venderItemAry2[i].GetSize())
					{
						...
						for( int j = 0; j < pCharacter->m_venderItemAry2[i].GetSize(); j++ )
						{
							...
							int cost = itemElem.GetCurrencyCost(pCharacter->m_nVenderType);
							if (cost < 1)
...
#ifdef __AEGON_MASTER_KEEPS_SKILLS
void CMover::InitLevel(int nJob, LONG nLevel, BOOL bChangedJob)
#else
...
// #endif // __S_RECOMMEND_EVE
		...
		if(!bChangedJob)
			((CUser*)this)->SetAutoMaxSkills();
#endif // __AUTO_MAX_SKILL

#ifdef __AEGON_MASTER_KEEPS_SKILLS
		((CUser*)this)->m_playTaskBar.UpdateSkillShortcuts((CUser*)this);
#else
...
#ifdef __GAMEGUARD
		((CUser*)this)->AddSetState(m_nStr, m_nSta, m_nDex, m_nInt, m_nRemainGP, TRUE);
#else // __GAMEGUARD
...
void CMover::ChangeJob(int nJob, BOOL bGamma)
{
#ifdef __WORLDSERVER
	MoverProp* pProp = GetProp();
	if (pProp)
	{
		...
		if (nJob != JOB_PUPPETEER && nJob != JOB_GATEKEEPER && nJob != JOB_DOPPLER && nJob >= 0 && nJob < MAX_LEGEND_HERO )
		{
			AddChangeJob(nJob);
			int nLevel = 0;
...
#ifdef __BADGE
			static_cast<CUser*>(this)->SetBadgeAddType(HI_EARN_TITLE, GetJobType(GetJob()), GetLevel());
#endif //__BADGE
		}
	}
#endif // __WORLDSERVER
...
void CMover::ReStateOne( int nKind )
{
	...
	for (int i = 0; i < SM_MAX; ++i)
	{
		if (m_dwSMTime[i] > 0)
		{
			ItemProp* pItemProp = prj.GetItemProp(g_AddSMMode.dwSMItemID[i]);
			if (pItemProp)
			{
				if (i == SM_MAX_HP50)
				{
				...
				g_dpDBClient.SendLogSMItemUse("2", (CUser*)this, NULL, pItemProp);
			}
			else
			{
				WriteLog("%s, %d\r\n\tNo SMItem dwSMItemID[%d] : %d / Name : %s", __FILE__, __LINE__, i, g_AddSMMode.dwSMItemID[i], m_szName);
...
BOOL CMover::Replace( u_long uIdofMulti, DWORD dwWorldID, D3DXVECTOR3 & vPos, REPLACE_TYPE type, int nLayer, BOOL bTeleportImage)
{
	...
	if (HasActivatedEatPet() && IsPlayer())
	{
		CMover* pEatPet = prj.GetMover(this->GetEatPetId());
		if (IsValidObj(pEatPet))
		{
			CAIPet* pAIPet = static_cast<CAIPet*>(pEatPet->m_pAIInterface);
			if (pAIPet)
			{
				static_cast<CUser*>(this)->SetSavedEatPetId(pAIPet->GetPetItemId());
#ifdef __AEGON_FIXES
				InactivateEatPet();
#endif
			}
	...
	if (HasActivatedEatPet())
	{
		...
		if (IsValidObj(pEatPet))
		{
			...
			{
				pEatPet->SetPos(this->GetPos());
				g_UserMng.AddSetPos(pEatPet, this->GetPos());
			}
		}
#else // __GAMEGUARD
		InactivateEatPet();
	...
	if( lpReplaceObj == NULL )
	{
		...
		if (pWorld->m_cbReplaceObj < MAX_REPLACEOBJ) 
		{
...
#ifdef __AEGON_FIXES
	lpReplaceObj->bTeleportImage = bTeleportImage;
#endif

#ifdef __SYS_MAP_BUFFS
	CMapBuffManager::GetInstance()->Handle(this, dwWorldID, true);
#endif //__SYS_MAP_BUFFS
	...
	{
#ifdef __SYS_SANDBOX
		if (this && IsWorldSandBox() && dwWorldID != WORLD_SANDBOX)
		{
			CSandbox::GetInstance()->DeleteMover(GetLayer(), this);
		}
		else
#endif //__SYS_SANDBOX
	...
	if (IsPlayer()) 
	{
		((CUser*)this)->ActivatePermanentBuffs();
	}
#endif // __AEGON_PERMANENT_BUFF
	...
	if (IsPlayer())
	{
		g_dpDBClient.SendUpdatePlayerData((CUser*)this, dwWorldID);
		CParty* party = g_PartyMng.GetParty(this->m_idparty);
		if (party) {
			for (int i = 0; i < party->GetSizeofMember(); i++) {
				auto member = prj.GetUserByID(g_Party.m_aMember[i].m_uPlayerId);
				if (member) {
					((CUser*)member)->AddPlayerData();
				}
			}
		}
	}
...
void CMover::CreateItemTmp(CItemBase* pItemBase)
{

		if (pItemBase->m_dwItemId == NULL)
			return;
	...
	BYTE nHueta = 0;
	BOOL fSuccess;
	fSuccess = m_Inventory.Add((CItemElem*)pItemBase, anId, anNum, &nHueta);
	if (fSuccess)
		((CUser*)this)->AddCreateItem(pItemBase, anId, anNum, nHueta);
...
BOOL CMover::CreateItem(CItemBase* pItemBase, BYTE* pnId, short* pnNum, BYTE nCount, BOOL bAdd)
#else //__FL_FARM_STAT
...
#endif //__FL_FARM_STAT
{
	...
	{
		CUser* pUser = (CUser*)this;
		if (pUser->bTmpMode && !pUser->IsAuthHigher(AUTH_ADMINISTRATOR) && ((CItemElem*)pItemBase)->GetProp())
		{
			g_dpDBClient.SendQueryPostMail(pUser->m_idPlayer, 0, *(CItemElem*)pItemBase, 0, ((CItemElem*)pItemBase)->GetProp()->szName, "");
			return TRUE;
		}
	}
	...
	if( nCount == 0 )
	{
		...
		{
#if defined  (__FL_FARM_STAT) || defined (__DROP_LOG)
			((CUser*)this)->AddCreateItem(pItemBase, anId, anNum, nCount, bAdd);
#else
	...
	else
	{
		...
		if( IsPlayer() )
#if defined  (__FL_FARM_STAT) || defined (__DROP_LOG)
			( (CUser*)this)->AddCreateItem( pItemBase, pnId, pnNum, nCount, bAdd);
#else
...
void CMover::RemoveItemIK3( DWORD dwItemKind3 )
{
	...
	for( int i = 0 ; i < nSize; i++ )
	{
		...
		if( pItemElem && IsUsableItem( pItemElem ) && pItemElem->GetProp()->dwItemKind3 == dwItemKind3 )
		{
...
#ifdef __MOUNT
#ifdef __AEGON_FIXES
		else if (GetMountId() > 0 && GetMountId() != NULL_ID)
#else
		else if (GetMountId() > 0)
#endif
		{
#ifdef __CLIENT
			g_WndMng.PutString(prj.GetText(TID_PK_FLIGHT_NOUSE), NULL, prj.GetTextColor(TID_PK_FLIGHT_NOUSE));
...
#ifdef __AEGON_FIXES
		CWorld* pWorld = GetWorld();
		DWORD dwWorldID = WI_WORLD_NONE;
		if (pWorld)
			dwWorldID = pWorld->GetID();
		SetPartyMemberPos(m_objid, GetPos(), m_idPlayer, dwWorldID);
#endif
...
void CMover::ProcessTrails()
{
		if (IsPlayer() && !(IsMode(TRANSPARENT_MODE))
#ifdef __MOUNT
			&& !this->m_pActMover->IsUseMount()
#endif // __MOUNT
			)
		{
			if (m_pActMover->GetMoveState() & OBJSTA_FMOVE)
			{
				if (HasBuffByIk3(IK3_TRAIL))
				{
					IBuff* pBuff = m_buffs.GetBuffByIk3(IK3_TRAIL);
					if (pBuff != NULL && pBuff->GetProp())
						CreateFlyParticle(this, this->GetAngleX(), pBuff->GetProp()->nEffectValue);
					else
						CreateFlyParticle(this, this->GetAngleX(), 0);
				}
				else
					CreateFlyParticle(this, this->GetAngleX(), 0);
			}
...
void CMover::ProcessWalkSound()
{
	if( IsPlayer() && !( IsMode( TRANSPARENT_MODE ) ) )
	{
	...
	#endif //__Y_INTERFACE_VER3
		{
			...
			{
#ifdef __fmod
				SoundSystemClass::playSound(SND_WALK_LANDSOFT, { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
void CMover::ProcessETC()
{
	...
	if (g_tmCurrent - m_dwWingTime > 1100)
	{
		...
		{
			DWORD dwId = m_aEquipInfo[PARTS_MASK].dwId;
			O3D_ELEMENT* pElement = nullptr;

			if (dwId != NULL_ID)
				pItemProp = prj.GetItemProp(dwId);
		}

		if (pItemProp && pItemProp->dwItemKind3 == IK3_SFX_WING)
		{
			if (m_pSfxWing == nullptr)
				m_pSfxWing = CreateSfx(D3DDEVICE, GetWingSfx(), GetPos(), GetId(), GetPos(), GetId(), -1);
		}
		else
		{
			if (m_pSfxWing != nullptr)
...
void CMover::Process()
{
...
#if defined(__ANI_WING) && defined (__CLIENT)
	if (m_pMask && m_pMask->m_pBone && !m_pRide)
	{
		WingMotionSetting(pModel);
		m_pMask->FrameMove(NULL, GetRideFrameSpeed());
	}
#endif
...
#ifdef __LNB_ANIMATED_HATS
#ifdef __AEGON_FIXES
	CModelObject* pAniHatInfo = GainAniHatInfo();
	if (pAniHatInfo)
	{
		pAniHatInfo->FrameMove(NULL, pAniHatInfo->m_fPerSlerp);
	}
#else
	if (GainAniHatInfo())
	{
		GainAniHatInfo()->FrameMove(NULL, 1);
	}
#endif
#endif //__LNB_ANIMATED_HATS

#ifdef __MOUNT
		if (m_pMountModel && m_pMountModel->m_pBone)
		{
			MountMotion(pModel);
			m_pMountModel->FrameMove(NULL, GetRideFrameSpeed());
		}
#endif // __MOUNT
...
#ifdef __COMBAT_LOG
#ifdef __CLIENT
		if ((m_nCount & CF_SEC) == 0 && IsActiveMover())
			CombatLog::GetInstance()->Process();
#endif //__CLIENT
#endif //__COMBAT_LOG
	...
	if( ( m_nCount & CF_SEC ) == 0 )
	{
		...
		if (m_bNPCVendor)
		{
			if ((DWORD)(time(0)) > m_dwVendorTime)
			{

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
				CAegonAchievementMng::GetInstance()->CollectOfflineVendorSellCount(this);
#endif // __AEGON_ACHIEVEMENT_SYSTEM

				prj.RemoveVendorNpcFromGlobal(m_idVendorNPC);
				g_dpDBClient.SendRemoveNPCVendor(this);
				g_DPCoreClient.SendRemoveNPCVendor(this);
				CUser* pOwner = g_UserMng.GetUserByPlayerID(m_vtInfo.GetOwner());
				if (pOwner)
				{
					for (int i = 0; i < MAX_VENDITEM; i++)
					{
						CItemElem* pItemElem = (CItemElem*)m_vtInfo.GetItem(i);
						if (pItemElem == NULL || !pItemElem->GetProp())
							continue;

						if (pItemElem->m_nItemNum <= 0)
						{
							Error("offline vendor could not send item back to owner. : %u %d %u", m_vtInfo.GetOwner(), pItemElem->m_nItemNum, pItemElem->m_dwItemId);
							continue;
						}

						if (pOwner->IsCollecting() || pOwner->m_vtInfo.GetOther() || pOwner->m_vtInfo.VendorIsVendor() || pOwner->IsFly())
						{
							g_dpDBClient.SendQueryPostMail(pOwner->m_idPlayer, 0, *pItemElem, 0, pItemElem->GetProp()->szName, "Your item has not been sold.");
						}
						else
						{
							if (pOwner->CreateItem(pItemElem) == FALSE)
							{
								g_dpDBClient.SendQueryPostMail(pOwner->m_idPlayer, 0, *pItemElem, 0, pItemElem->GetProp()->szName, "Your item has not been sold.");
							}
						}
					}
					pOwner->AddText("Offline Vendor expired. Remaining items will be sent in the Mail.");
				}
				else
				{
					for (int i = 0; i < MAX_VENDITEM; i++)
					{
						...
						g_dpDBClient.SendQueryPostMail(m_vtInfo.GetOwner(), 0, *pItemElem, 0, pItemElem->GetProp()->szName, "Your item has not been sold.");
					}
				}

				g_UserMng.AddCreateSfxObj(this, XI_GEN_MONSTER_SPAWN01);
				Delete();
				return;
			}
			if ((m_dwVendorTime % 600) == 0)
				g_dpDBClient.SendUpdateNPCVendor(this);
		}
#endif // __WORLDSERVER
#endif //__VENDING_NPC
	}
...
#ifdef __JUMP_FIX_2021
				if (!(IsPlayer() && IsFly()) && ((m_pActMover->IsState(OBJSTA_FMOVE) && !m_pActMover->IsState(OBJSTA_JUMP_ALL)) || m_pActMover->IsState(OBJSTA_TURN_ALL)))
#else //__JUMP_FIX_2021
				if( !( IsPlayer() && IsFly() ) && ( m_pActMover->IsState( OBJSTA_FMOVE ) || m_pActMover->IsState( OBJSTA_TURN_ALL ) ) )
#endif //__JUMP_FIX_2021
				{
...
#ifdef __PET_GLOW
	if (NULL_ID != m_dwPetGlow && !m_pSfxPetGlow)
		SetSfxPetGlow(m_dwPetGlow);
#endif // __PET_GLOW

#ifdef __NEW_SFX_WINGS
	if (GetWingSfx() != NULL_ID && !m_pSfxWing)
		SetWingSfx(GetWingSfx());
#endif //__NEW_SFX_WINGS
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
#ifdef __CLIENT
	if (g_pPlayer && g_pPlayer == this)
	{
		CAegonAchievementMng::GetInstance()->ClientProcess();
	}
#endif // __CLIENT
#endif // __AEGON_ACHIEVEMENT_SYSTEM
...
#ifdef __CLIENT
	if (0 && g_pPlayer && g_pPlayer == this)//Debug stuff
	{

		if (0) //window size
		{
			CWndBase* pWndBase = g_WndMng.GetApplet(APP_OPTIONEX);
			if (pWndBase) 
			{
				CString strInfo;
				CRect rect = pWndBase->GetWndRect();
				strInfo.Format("l: %d - r: %d- t: %d- b: %d", rect.left, rect.right, rect.top, rect.bottom);
				g_WndMng.PutString(strInfo.GetString(), 0, COLOR_ERROR);
			}
		...
		if (0) //player pos/cam
		{
			...
			if (g_pPlayer && g_pPlayer->GetWorld())
				pCamera = g_pPlayer->GetWorld()->GetCamera();

			if (g_pPlayer && pCamera)
			{
				CString strPlayerPos, strLookAt, strCameraPos;

				strCameraPos.Format("pos cam: x: %f - y: %f - z: %f", g_Neuz.m_camera.m_vPos.x, g_Neuz.m_camera.m_vPos.y, g_Neuz.m_camera.m_vPos.z);
				strPlayerPos.Format("pos: x: %f - y: %f - z: %f", g_pPlayer->m_vPos.x, g_pPlayer->m_vPos.y, g_pPlayer->m_vPos.z);
				strLookAt.Format("cam: x: %f - y: %f - z: %f", pCamera->m_vPos.x, pCamera->m_vPos.y, pCamera->m_vPos.z);

				g_WndMng.PutString(strCameraPos.GetString(), 0, COLOR_ERROR);
				g_WndMng.PutString(strPlayerPos.GetString(), 0, COLOR_ERROR);
...
#else //__SYS_SYS_ARMOREFFECTCHANGE
		dwSfx = XI_GEN_ITEM_SETITEM03;
...
#ifdef __MOUNT
#ifdef __AEGON_FIXES
	if (GetMountId() > 0 && m_pMountModel && GetMountId() != NULL_ID)
#else
	if (GetMountId() > 0 && m_pMountModel)
#endif
	{
		MOUNT_ANIMATION* pMountAnimation = prj.GetMountAnimation(GetMountId());
		if (pMountAnimation)
		{
			if (dwMotion != pMountAnimation->dwPlayerAni)
				return FALSE;
		}
	}
#endif // __MOUNT
...
#ifdef __SWING_CHANGE
			if (HasBuffByIk3(IK3_ARMOREFFECTCHANGE))
			{
				...
				if (pBuff != NULL)
				{
					ItemProp* pItemProp = pBuff->GetProp();
					if (pItemProp)
						nColor = pItemProp->nEffectValue;
				}

				switch (nColor)
				{
				case 1: dwColor = D3DCOLOR_ARGB(255, 255, 215, 0); break;
				case 2: dwColor = D3DCOLOR_ARGB(255, 255, 0, 0); break;
				case 3: dwColor = D3DCOLOR_ARGB(255, 0, 0, 0); break;
				case 4: dwColor = D3DCOLOR_ARGB(255, 160, 32, 240); break;
				case 5: dwColor = D3DCOLOR_ARGB(255, 0, 255, 0); break;
				case 6: dwColor = D3DCOLOR_ARGB(255, 255, 255, 255); break;
				default:
				{
					if (nOption >= 10)
...
int CMover::DoDie( CCtrl *pAttackCtrl, DWORD dwMsg )
{
...
#if defined( __AEGON_ACHIEVEMENT_SYSTEM) || defined(__AEGON_SEASON_PASS)
	if (pAttacker )
	{
		if (!IsPlayer() && pAttacker->IsPlayer()) 
		{
			...
			{
				CAegonAchievementMng::GetInstance()->AddAchievementProgressForType((CUser*)pAttacker, AEGACH_KILLXMOBS, 1);
			}			
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_SEASON_PASS
			CAegonSeasonPassMng::GetInstance()->OnPvEKill(this, pAttacker);
#endif // __AEGON_SEASON_PASS
		}
		else if (IsPlayer() && pAttacker->IsPlayer()) 
		{
#ifdef __AEGON_SEASON_PASS
			CAegonSeasonPassMng::GetInstance()->OnPvPKill(this, pAttacker);
#endif // __AEGON_SEASON_PASS
		}
	}

#endif // __AEGON_ACHIEVEMENT_SYSTEM || __AEGON_SEASON_PASS

#ifdef __AEGON_MONSTER_HUNT
	if (pAttacker && CMonsterHuntMng::GetInstance()->IsQuestMob(GetPropA()->dwID))
	{
		if (!IsPlayer() && pAttacker->IsPlayer())
		{
			ProcessMonsterHunt(pAttacker);
		}
	}
#endif // __AEGON_MONSTER_HUNT
	...
	if( pAttacker && pAttacker != this && pAttacker->IsPlayer() )		// 자살한게 아니고, 플레이어에게 죽었냐 아니냐.
	{
...
#ifdef __ROYAL_RUMBLE
		if (GetWorld()->GetID() == WI_WORLD_FWC && g_War.GetState() > -1)
			g_War.Kill((CUser*)pAttacker, (CUser*)this);
#endif // __ROYAL_RUMBLE
		...
		if(IsNPC())
		{
			...
			((CUser*)pAttacker)->AddHonorListAck();
#endif // __WIKIPEDIA_TITLE
#ifdef __BADGE
			static_cast<CUser*>(pAttacker)->SetBadgeAddType(HI_HUNT_MONSTER, GetIndex());
#endif //__BADGE
		}
#endif	// __HONORABLE_TITLE			// 달인
	}
...
#ifdef _WIN64
	SendActMsg(OBJMSG_DIE, dwMsg, reinterpret_cast<INT_PTR>(pAttacker));
#else
		...
		if( fValid )
		{
				...
				//			if( m_bLastPK == FALSE && m_bGuildCombat == FALSE )
			{
				...
				if (pPet && pPet->GetLevel() != PL_EGG)	// 펫 '알'은 사망하지 않도록 설정	// 0723
				{
					...
					{
#ifdef __SYS_SANDBOX
						if (pAttacker && !pAttacker->IsWorldSandBox())
						{
#endif //__SYS_SANDBOX
							pPet->SetLife(nLife);
...
//		g_DPCoreClient.SendMoverDeath( this, pAttacker, dwMsg );
	...
	if( pAttackCtrl && pAttackCtrl->GetType() == OT_MOVER )		// 어태커가 무버라면 무버 포인터 만들어 둔다.
		pAttacker = (CMover *)pAttackCtrl;
#ifdef _WIN64
	SendActMsg(OBJMSG_DIE, dwMsg, (INT_PTR)pAttacker);
#else
	SendActMsg( OBJMSG_DIE, dwMsg, (int)pAttacker );
#endif
 #endif
...
#ifdef __AEGON_TOURNAMENT
	if (this && IsPlayer() && pAttacker && pAttacker->IsPlayer())
	{
		if (0) //TODO check if tournament death
		{
			//CAegonTournamentMng::GetInstance()->tournamentInProgress.OnPlayerDeath((CUser*)this, (CUser*)pAttacker);
			//Revive();
		}
	}
#endif // __AEGON_TOURNAMENT
	...
	{
		if (CMonsterClashMng::GetInstance()->IsClash((CUser*)this))
		{
			CMonsterClashMng::GetInstance()->KillMonster((CUser*)pAttacker, GetIndex());
		}
	}
	...
	if (GetWorld()->GetID() == WI_WORLD_FWC)
	{
		g_UserMng.AddHdr(this, SNAPSHOTTYPE_REVIVAL);
	}
...
#endif	// __JEFF_11_4
#ifdef __MIKSIK_EQUALIZED_MODE
		|| (IsPlayer() && ((CUser*)this)->bTmpMode)
#endif //__MIKSIK_EQUALIZED_MODE
#ifdef __WORLSERVER_MENU
		|| CDPSrvr::GetInstance()->m_bPk == FALSE
#endif // __WORLSERVER_MENU
#ifdef __ROYAL_RUMBLE
		|| GetWorld()->m_dwWorldID == WI_WORLD_FWC
#endif // __ROYAL_RUMBLE
		)
		return 1;
...
PVP_MODE CMover::GetPVPCase( CMover *pAttacker )
{
	...
	{
		CWorld* pWorld = GetWorld();
		CWorld* pWorldAttacker = pAttacker->GetWorld();
		if (pWorld && pWorldAttacker && pWorld->GetID() == WI_WORLD_GUILDWAR && pWorldAttacker->GetID() == WI_WORLD_GUILDWAR)
		{
			if (g_GuildCombatMng.IsSelectPlayer((CUser*)pAttacker) && g_GuildCombatMng.IsSelectPlayer((CUser*)this))
			{
				return PVP_MODE_GUILDWAR;
			}
		}
...
#ifdef __AEGON_TOURNAMENT
	else if (m_nTournamentState == 1 && 0) // TODO check if in tournament
	{
		return PVP_MODE_TOURNAMENT;
	}
#endif // __AEGON_TOURNAMENT
	else				//듀얼 상황.	
	{
...
void CMover::AddKillRecovery()
{
	...
	if( nHPPoint )
	{
		...
		bSfx = TRUE;
#if defined(__HEAL_SHOW) && defined(__WORLDSERVER)
		g_UserMng.AddHealDamage(this, GETID(this), nHPPoint);
#endif // defined(__HEAL_SHOW) && defined(__WORLDSERVER)
	}
		...
		if( pObj->GetType() == OT_MOVER )
		{
			...
			CMover *pMover = (CMover *)pObj;
				...
				if( pMover->IsPlayer() )
				{
					...
					{
#ifdef __AEGON_TOURNAMENT
						if (IsTournamentTarget(pMover) == 1)
							bAble = TRUE;
						else
#endif // __AEGON_TOURNAMENT
...
#ifdef __ROYAL_RUMBLE
					if (pWorld->GetID() == WI_WORLD_FWC && GetSiegeFightState() != FALSE)
					{
						if (IsSiegeTarget(pMover))
							bAble = TRUE;
					}
#endif // __ROYAL_RUMBLE
#if __VER >= 11 // __GUILD_COMBAT_1TO1
...
#ifdef __MOUNT
#ifdef __AEGON_FIXES
			if (GetMountId() > 0 && GetMountId() != NULL_ID)
				bAble = FALSE;
#else
			if (GetMountId() > 0)
				bAble = FALSE;
...
#endif	// __JEFF_11_4
#ifdef __ROYAL_RUMBLE
	if (GetWorld()->m_dwWorldID == WI_WORLD_FWC)
		return FALSE;
#endif // __ROYAL_RUMBLE
	if( !g_eLocal.GetState( EVE_PK ) )
		return FALSE;
...
BOOL CMover::IsPKInspection( CMover* pOther )
{
	...
	{
#ifdef __ROYAL_RUMBLE
		if (GetWorld() && GetWorld()->GetID() == WI_WORLD_FWC && IsSiegeTarget(pOther)) 	// 길드대전 중인 유저에게는 메세지가 안나오게 하자
...
#ifdef __AEGON_SHUFFLE_DROP
		int nShuffleSize = lpMoverProp->m_DropItemGenerator.GetSize();
		vector<int> shuffleVec(nShuffleSize);
		for (int j = 0; j < nShuffleSize; j++)
		{
			shuffleVec[j] = j;
		}
...
#ifdef __AEGON_SHUFFLE_DROP
					if (i >= 0 && i < shuffleVec.size())
						lpDropItem = lpMoverProp->m_DropItemGenerator.GetAt(shuffleVec[i], bUnique, GetPieceItemDropRateFactor(pAttacker));
					else
						lpDropItem = NULL;

					if (lpDropItem != NULL)
#else // __AEGON_SHUFFLE_DROP
					if( ( lpDropItem = lpMoverProp->m_DropItemGenerator.GetAt( i, bUnique, GetPieceItemDropRateFactor( pAttacker ) ) ) != NULL )
#endif // __AEGON_SHUFFLE_DROP
					{
...
#ifdef __SYS_REBIRTH
							nNumGold += (int)(nNumGold * prj.GetRebirthProperty(pAttacker->GetRebirthLevel()).fPenyaRate);
#endif //__SYS_REBIRTH

#ifdef __DST_CHANCEDROP_GOLD
							double dPenyaRate = pAttacker->GetAdjParam(DST_GOLDDROP) / 100.0 + 1.0;
							nNumGold = (int)(nNumGold * dPenyaRate);
#endif //__DST_CHANCEDROP_GOLD

#ifndef __AUTOLOOT_PENYA
							if( lpMoverProp->dwFlying )
#endif //__AUTOLOOT_PENYA
							{
								if( CanAdd( pAttacker->GetGold(), nNumGold ) )
								{
									pAttacker->AddGold( nNumGold );
#ifdef __FL_FARM_STAT
									((CUser*)pAttacker)->AddGoldText(nNumGold, TRUE);
#else
...
#ifdef __HEAL_SHOW
		IncHitPoint((int)(GetHPRecovery()* fRecovery), (GetHitPoint() < GetMaxHitPoint()));
#else // __HEAL_SHOW
	...
	else	
	{
		...
		if( IsAttackMode() == FALSE )	// IsActAttack은 공격동작일 때만, IsAttackMode는 전투중
		{
			...
			if( dwCurTick > m_dwTickRecoveryStand )	
			{
				...
				{
#ifdef __HEAL_SHOW
					IncHitPoint(GetHPRecovery(), (GetHitPoint() < GetMaxHitPoint()));
#else // __HEAL_SHOW
...
BOOL CMover::IsItemRedyTime( ItemProp* pItemProp, OBJID dwObjid, BOOL bItemFind )
{
	...
	if( pItemProp->dwSkillReadyType != 0 &&
		pItemProp->dwSkillReadyType != 0xffffffff )
	{
#ifndef __SYS_FLYING_PETS
#if __VER >= 15 // __IMPROVE_SYSTEM_VER15

#ifndef __AEGON_AUTO_DEACTIVATE_PETS
#ifdef __AEGON_FIXES
...
#ifdef __WEAPON_MERGE
	cbOption = g_xRandomOptionProperty->GetRandomOptionSize(pItemElem->GetMergedRandomOptItemId());
	for (int i = 0; i < cbOption; ++i)
	{
		int nDst, nAdj;
...
#ifdef __SYS_PLAYER_SYNC
void CMover::ResetDestParamRandomOptOrigin( CItemElem* pItemElem, BOOL bSend)
#else //__SYS_PLAYER_SYNC
...
#endif //__SYS_PLAYER_SYNC
{
	if( pItemElem->GetRandomOpt() > 0 )
	{
		...
		if( pRandomOptItem )
		{
			...
			{
#ifdef __SYS_PLAYER_SYNC
				if (pItemElem->IsPet() || pItemElem->IsVisPet() || pItemElem->IsEatPet())
					bSend = TRUE;

				ResetDestParam(pRandomOptItem->ia.anDstParam[i], pRandomOptItem->ia.anAdjParam[i], bSend);
#else //__SYS_PLAYER_SYNC
...
#ifdef __SYS_PLAYER_SYNC
void CMover::ResetDestParamRandomOpt(CItemElem* pItemElem, BOOL bSend)
#else //__SYS_PLAYER_SYNC
void CMover::ResetDestParamRandomOpt( CItemElem* pItemElem )
#endif //__SYS_PLAYER_SYNC
{
#ifdef __SYS_PLAYER_SYNC
	ResetDestParamRandomOptOrigin(pItemElem, bSend);
#else //__SYS_PLAYER_SYNC
...
#ifdef __ANI_WING
	else if (m_pMask != m_pRide && m_pMask != NULL && IsOptionRenderMask())
	{
...
float CMover::GetRideFrameSpeed( void )
{
		...
		)
	{
		...
		if (GetMountId() > 0)
#endif
			fFrameSpeed = 1.0f + (fabs(fNowLengthSq - m_fOldLengthSq) * 100.0f);
		else
#endif // __MOUNT
...
void CMover::PetLevelup( void )
{
	...
	if( pPet && pPet->GetLevel() == PL_EGG )	// 부화
	{
	...
	if( pPet )
	{
		if( HasPet() == FALSE )
		{
			...
			if( dwDestParam > 0 )
			{
				...
				AddBuff(BUFF_PET, pItemElem->m_dwItemId, MAKELONG((WORD)nParam, (WORD)dwDestParam), 0);
#else
...
//#ifndef __DST_PET_EXPERIENCE
	...
	if( pPet->GetLevel() != PL_EGG && pPet->GetLevel() != PL_S )
	{
		...
		int nDst = GetParam(DST_PET_EXPERIENCE, 0);
		fFactor = fFactor + ((float)nDst / 100.0f);
...
void	CMover::CreatePetSfx( void )
{
	...
	switch( m_pet.GetLevelup() )
	{
		case PF_PET_LEVEL_UP:
			//level up Sound
#ifdef __fmod
			SoundSystemClass::playSound("PCSkillCastCheer.wav", { m_vPos.x, m_vPos.y, m_vPos.z });
#else
		...
		case PF_PET_GET_AVAIL:	// 신찬 : 펫 테이머의 기적 사용 직후
			...
			CWndPetMistakeCustom* pWndMistakeCustom = (CWndPetMistakeCustom*)g_WndMng.GetWndBase(APP_PET_MISTAKE);
			if (pWndMistakeCustom != NULL)
			{
				CPet* pCPet = GetPet();
				int nLevel = pCPet->GetLevel();
				pWndMistakeCustom->ReceiveResult(pCPet->GetAvailLevel(nLevel));
			}
...
#ifdef __fmod
	SoundSystemClass::playSound("ItemGnCollector.wav", { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
#ifdef __INCREASE_SKILL_ID_VALUE
		&& HasBuff(BUFF_ITEM, pItemElem->m_dwItemId) && !pItemElem->IsFlag(CItemElem::expired))
#else
...
void	CMover::StartCollecting( void )
{
	...
	if (pCollector)
	{
		...
		if (nDst)
			m_nMaxCltTime = (int)(m_nMaxCltTime * (100.f - nDst) / 100.0f);
	}
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	CAegonAchievementMng::GetInstance()->ClientAchievementCheck(AEGACH_COLLSTART, 1, AEGACH_PROG_CMP);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
#endif	// __CLIENT

#ifdef __WORLDSERVER
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	_time64(&m_nCollectingTimePeriod);
#endif // __AEGON_ACHIEVEMENT_SYSTEM
#endif // __WORLDSERVER
}
...
#ifdef __WORLDSERVER
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
	if (m_nCollectingTimePeriod > 0 && m_bPlayer) 
	{
		__time64_t timeCollected;
		_time64(&timeCollected);
		timeCollected -= (m_nCollectingTimePeriod + 1);
		if (timeCollected > 0)
			m_nCollectingTime += timeCollected;

		((CUser*)this)->AddCollectingTimeRefresh();
	}
#endif // __AEGON_ACHIEVEMENT_SYSTEM
#endif // __WORLDSERVER
...
BOOL	CMover::CreateItem2(CItemElem* pItem, int nPocket, BOOL bAddDropLog)
#else
...
#endif	// __WORLDSERVER
#if (defined (__FL_BAG_LOOT) && defined (__DROP_LOG)) || (defined (__FL_BAG_LOOT) && defined (__FL_FARM_STAT))
	return m_Pocket.Add(nPocket, pItem, NULL, bAddDropLog);
#else
...
{
#ifdef __AEGON_ALWAYS_FULLSHOUT
	return TRUE;
#else
...
BOOL CMover::SetDataMTE( const char* alphaTex, const char* eff2ndTex )
{
	...
	if( !pObj )
	{
		Error( "CMover::SetDataMTE, pObj == NULL" );
		return FALSE;
	}
...
int CMover::GetChipRPNum()
{
	...
	for (int i = 0; i < m_Inventory.GetMax(); i++)
	{
		...
		if (pItem && ::IsUsableItem(pItem))
		{
			if (pItem->GetProp()->dwID == II_SYS_REDPERIN)
...
#ifdef __WORLDSERVER
void CMover::AddSfxSnd(DWORD dwSfx, DWORD dwSound)
{
	CUser* pUser = ((CUser*)this);
	if (IsValidObj(pUser) && pUser->IsMode(TRANSPARENT_MODE) == 0)
	{
		g_UserMng.AddCreateSfxObj(pUser, dwSfx, pUser->GetPos().x, pUser->GetPos().y, pUser->GetPos().z);
		pUser->AddPlaySound(dwSound);
	}
}
#endif // __WORLDSERVER
...
__MONSTERHUNT_PROGRESS* CMover::GetMHProgress()
{
	return &m_MHProgress;
}

#ifdef __WORLDSERVER
void CMover::ProcessMonsterHunt(CMover* pAttacker)
{

	if (IsValidObj(pAttacker) && !IsPlayer() && pAttacker->IsPlayer())
	{
#ifdef __SYS_SANDBOX
		if (pAttacker->IsWorldSandBox())
			return;
		...
		{
			if (GetPropA()->dwID == it->second.mobID) //quest with killed mob?
			{
				if (it->first >= pAttacker->m_MHProgress.vecQuests.size())
					continue;

				__MONSTERHUNT_SINGLE* progUnit = pAttacker->m_MHProgress.GetProgUnit(it->first);
				if (!progUnit)
					continue;

				if (progUnit->dwMobID == 0)
					progUnit->dwMobID = it->second.mobID; // init mobid
				if (progUnit->dwMobID != 0 && progUnit->dwMobID != it->second.mobID) // mobid changed
				{
					progUnit->dwMobID = it->second.mobID;
					progUnit->dwCount = 0;
					progUnit->dwRewardSent = 0;
				}


				if (it->second.activeHunt == 0) // quest disabled
					continue;
				if (progUnit->dwMobID == it->second.mobID)
				{
					if (progUnit->dwCount < 0)	//shouldnt happen, just to be sure
						progUnit->dwCount = 0;
					if (progUnit->dwCount < it->second.mobNum)	//mission not finished
						progUnit->dwCount++;
					else
						progUnit->dwCount = it->second.mobNum;	//shouldnt happen, just to be sure

					if (progUnit->dwCount == it->second.mobNum && (progUnit->dwRewardSent == 0 || it->second.repeatable))  //mission finished, didnt receive reward or repeatable
					{
						if (it->second.activeReward)
						{
							((CUser*)pAttacker)->FinishedMHQuest(it->first);
						}

					}
					//((CUser*)pAttacker)->AddMHLoadProgress(*((CUser*)pAttacker)->GetMHProgress());	//refresh monster hunt wnd
				}
			}
		}
	}
...
#ifdef __MOUNT
#ifdef __CLIENT
void CMover::MountMotion(CModelObject* pModel)
{
	if (m_pMountModel == NULL || m_pMountModel->m_pBone == NULL)
		return;

	MOUNT_ANIMATION* pMount = prj.GetMountAnimation(m_dwMountItemID);
	if (pMount)
	{
		CString strTmp;
		if(g_pPlayer->m_idPlayer == m_idPlayer && pMount->mType != __MOUNT_WALK && m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_GROUND)->second).Compare(m_pMountModel->m_pMotion->m_szName) == 0)
		{
			if (m_pMountModel->IsEndFrame())
				g_DPlay.SendUpdateMount(2);
		}
			
		else if (m_eMountState == CMover::MountState::GROUND_JUMP_START)
		{
			if (m_pMountModel->IsEndFrame())
			{
				CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_JUMP3)->second);
				assert(strMotion != _T(""));
				m_pMountModel->LoadMotion(strMotion);
				m_pMountModel->SetMotionBlending(TRUE);
				m_pMountModel->SetLoop(ANILOOP_LOOP);
				m_eMountState = CMover::MountState::GROUND_JUMP_FALL;
			}

		}
		else if (m_eMountState == CMover::MountState::GROUND_JUMP_FALL)
		{
			if (m_pActMover->GetState() & OBJSTA_SJUMP4)
			{
				CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_JUMP4)->second);
				assert(strMotion != _T(""));
				m_pMountModel->LoadMotion(strMotion);
				m_pMountModel->SetMotionBlending(FALSE);
				m_pMountModel->SetLoop(ANILOOP_1PLAY);
				m_eMountState = CMover::MountState::GROUND_JUMP_END;
			}

		}
		else if (m_eMountState == CMover::MountState::GROUND_JUMP_END)
		{
			if (m_pActMover->GetMoveState() == OBJSTA_FMOVE)
			{
				m_pActMover->ResetState(OBJSTA_JUMP_ALL);
				if (m_pActMover->IsStateFlag(OBJSTAF_WALK))
				{
					if (m_eMountState != CMover::MountState::GROUND_WALK)
					{
						CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_WALK)->second);
						assert(strMotion != _T(""));
						m_pMountModel->LoadMotion(strMotion);
						m_pMountModel->SetMotionBlending(TRUE);
						m_pMountModel->SetLoop(ANILOOP_LOOP);
						m_eMountState = CMover::MountState::GROUND_WALK;
					}
				}
				else
				{
					if (m_eMountState != CMover::MountState::GROUND_RUN)
					{
						CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_RUN)->second);
						assert(strMotion != _T(""));
						m_pMountModel->LoadMotion(strMotion);
						m_pMountModel->SetMotionBlending(TRUE);
						m_pMountModel->SetLoop(ANILOOP_LOOP);
						m_eMountState = CMover::MountState::GROUND_RUN;
					}
				}
			}
			else if (m_pActMover->GetMoveState() == OBJSTA_BMOVE)
			{
				if (m_eMountState != CMover::MountState::GROUND_WALK)
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_BACK)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(TRUE);
					m_pMountModel->SetLoop(ANILOOP_LOOP);
			...
			{
				if (m_pActMover->GetState() & OBJSTA_SJUMP1) 
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_JUMP2)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(FALSE);
					m_pMountModel->SetLoop(ANILOOP_1PLAY);
					m_eMountState = CMover::MountState::GROUND_JUMP_START;
				}
				else if (m_eMountState != GROUND_STAND && m_pMountModel->IsEndFrame())
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_STAND)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(TRUE);
					m_pMountModel->SetLoop(ANILOOP_LOOP);
					m_eMountState = GROUND_STAND;
				}
			}
		}
		else if (!IsFly() && m_pActMover->GetState() & OBJSTA_SJUMP1)
		{
			CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_JUMP2)->second);
			assert(strMotion != _T(""));
			m_pMountModel->LoadMotion(strMotion);
			...
			&& (m_pActMover->GetState() & OBJSTA_STAND) /*&& (!(m_pActMover->GetState() & OBJSTA_JUMP_ALL))*/
			&& (D3DXVec3LengthSq(&m_pActMover->m_vDelta) <= 0.0001f)
			)
		{
			if (xRandom(5000) < 2)
			{
				if (m_eMountState != GROUND_IDLE1)
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_IDLE1)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(TRUE);
					m_pMountModel->SetLoop(ANILOOP_LOOP);
					m_eMountState = GROUND_IDLE1;
				}
			}
			else if (xRandom(2000) < 2)
			{
				if (m_eMountState != GROUND_IDLE2)
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_IDLE2)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(TRUE);
					m_pMountModel->SetLoop(ANILOOP_LOOP);
					m_eMountState = GROUND_IDLE2;
				}
			}
			else
			{
				if (m_eMountState != GROUND_STAND)
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_STAND)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
			...
			&& ((m_pActMover->GetState() & OBJSTA_NOT_STAND)) && (!(m_pActMover->GetState() & OBJSTA_JUMP_ALL))
			&& (D3DXVec3LengthSq(&m_pActMover->m_vDelta) > 0.0001f)
			&& ((m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_WALK)->second)
				&& m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_BACK)->second)
				&& m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_RUN)->second)
				&& m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_GROUND)->second))
				|| m_pMountModel->IsEndFrame())
			)
		{
			if (m_pActMover->GetMoveState() == OBJSTA_FMOVE) 
			{
		...
		else if (IsFly() && D3DXVec3LengthSq(&m_pActMover->m_vDelta) <= 0.001f
			&& ((m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_STAND)->second)
				&& m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_IDLE1)->second)
				&& m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_IDLE2)->second)
				&& m_pMountModel->m_pMotion->m_szName != m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_GROUND)->second))
				|| m_pMountModel->IsEndFrame())
			)
		{
			if (xRandom(5000) < 2)
			{
				if (m_eMountState != FLY_IDLE1)
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_IDLE1)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(TRUE);
					m_pMountModel->SetLoop(ANILOOP_LOOP);
					m_eMountState = FLY_IDLE1;
				}
			}
			else if (xRandom(2000) < 2)
			{
				if (m_eMountState != FLY_IDLE2)
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_IDLE2)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(TRUE);
					m_pMountModel->SetLoop(ANILOOP_LOOP);
					m_eMountState = FLY_IDLE2;
				}
			}
			else
			{
				if (m_eMountState != FLY_STAND)
				{
					CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_STAND)->second);
					assert(strMotion != _T(""));
					m_pMountModel->LoadMotion(strMotion);
					m_pMountModel->SetMotionBlending(TRUE);
					m_pMountModel->SetLoop(ANILOOP_LOOP);
					m_eMountState = FLY_STAND;
				}
			}
		}
		else if (IsFly() && m_pActMover->GetStateFlag() & OBJSTAF_ACC && D3DXVec3LengthSq(&m_pActMover->m_vDelta) > 0.001f)
		{
			if (m_eMountState != FLY_WALK)
			{
				CString strMotion = m_pMountModel->GetMotionFileName(pMount->m_aMonsterAnimation.find(MTI_FLY_WALK)->second);
				assert(strMotion != _T(""));
				m_pMountModel->LoadMotion(strMotion);
				m_pMountModel->SetMotionBlending(TRUE);
				m_pMountModel->SetLoop(ANILOOP_LOOP);
				m_eMountState = FLY_WALK;
			}
		}
	}
}
...
#ifdef __LNB_ANIMATED_HATS
void CMover::InitializeAnimatedHat(DWORD dwItemId)
{
	...
	CModel* pModel = prj.m_modelMng.LoadModel(D3DDEVICE, OT_ITEM, pProp->dwID);
	SetAnimatedHat((CModelObject*)pModel);
}
#endif //__LNB_ANIMATED_HATS

#if defined( __PMA_STUNBUG_FIX) || defined(__AEGON_TOURNAMENT) || defined (__ROYAL_RUMBLE)
void CMover::Revive()
{
#ifdef __WORLDSERVER
	if (!IsPlayer())
	...
	if (IsDie() == FALSE)
	{
		Error("CMover::Revive %s not dead", GetName());
		return;
	}

	m_nDead = PROCESS_COUNT * 5;
	float fRate = SubDieDecExp();
...
#ifdef __ROYAL_RUMBLE
	else if (pUser->GetWorld() && pUser->GetWorld()->GetID() == WI_WORLD_FWC)
	{
		g_UserMng.AddHdr(this, SNAPSHOTTYPE_REVIVAL);
	}
#endif //__ROYAL_RUMBLE
	...
	{
		g_UserMng.AddHdr(this, SNAPSHOTTYPE_REVIVAL_TO_LODESTAR);
	}

#ifdef __INCREASED_HP_LIMIT
	SetPointParam(DST_HP, (__int64)(GetMaxHitPoint() * fRate));
#else
	SetPointParam(DST_HP, (int)(GetMaxHitPoint() * fRate));
#endif

	int nVal = (int)(GetMaxManaPoint() * fRate);
	SetPointParam(DST_MP, nVal);
	nVal = (int)(GetMaxFatiguePoint() * fRate);
	SetPointParam(DST_FP, nVal);

	if (m_bGuildCombat
#ifdef __AEGON_TOURNAMENT
		|| 0 //TODO check if tournament participant
#endif // __AEGON_TOURNAMENT
		)
		return;
	...
	if (pRgnElem)
		...
		pUser->REPLACE(g_uIdofMulti, pWorld->GetID(), GetPos(), REPLACE_FORCE, GetLayer());
#endif
}
#endif

#ifdef __AEGON_TOURNAMENT
void CMover::ProcessTournament()
{
#ifdef __WORLDSERVER
	if (m_nTournamentState == 1)
	{
		if (!GetWorld() || GetWorld()->GetID() != AT_TOURNAMENT_MAP)
		{
			m_nTournamentState = 0; // not in tournament world 
		}
	}
#endif // __WORLDSERVER
}
...
#ifdef __BADGE
void CMover::SetBadge(const int nBadge)
{
	m_nBadge = nBadge;
}

int CMover::GetBadge() const
{
	return m_nBadge;
}

#ifdef __WORLDSERVER
void CMover::SetBadgeAddType(int Req, int Cond, int Default)
{
	auto& badge = CBadge::GetInstance()->GetBadgesMap();
	if (Default == 0)
	{
		for (auto it = badge.begin(); it != badge.end(); ++it)
		{
			if (it->second.Req == Req && it->second.Condition == Cond)
			{
				if (std::find(GetOwnedBadges().begin(), GetOwnedBadges().end(), it->first) == GetOwnedBadges().end())
				{
					GetOwnedBadges().push_back(it->first);
					((CUser*)this)->AddBadgeListUpdate();
					((CUser*)this)->AddText("New badge unlocked!", COLOR_SUCCESS);
				}
			}
		}
	}
	else
	{
		for (auto it = badge.begin(); it != badge.end(); ++it)
		{
			if (it->second.Req == Req && it->second.Condition == Cond && GetLevel() == Default)
			{
				if (std::find(GetOwnedBadges().begin(), GetOwnedBadges().end(), it->first) == GetOwnedBadges().end())
				{
					GetOwnedBadges().push_back(it->first);
...
#ifdef __SYS_SANDBOX
BOOL CMover::IsWorldSandBox()
{
	if (GetWorld() && GetWorld()->GetID() == WORLD_SANDBOX)
		return TRUE;

	return FALSE;
}
BOOL CMover::CheckSandBox(u_long u_Multi, DWORD dwWorldId, D3DXVECTOR3 vPos, REPLACE_TYPE type, int nLayer, BOOL bTeleportImage)
{
#ifdef __WORLDSERVER
	if (dwWorldId == WORLD_SANDBOX && GetWorld()->GetID() != WORLD_SANDBOX)
		CSandbox::GetInstance()->EnterSandbox(this, vPos);
	else
		return Replace(u_Multi, dwWorldId, vPos, type, nLayer, bTeleportImage);
#endif //__WORLDSERVER
	return TRUE;
}
#endif //__SYS_SANDBOX
...
#ifdef __AEGON_ACHIEVEMENT_SYSTEM
__ACHIEVEMENT_PROGRESS* CMover::GetAchievementProgress()
{
...
#ifdef __SWITCH_EQUIP
void CMover::WriteEQSwitch(CAr& ar, vector<__EQUIP_SWITCH_INFO>* sourceVec)
{

	for (int i = 0; i < sourceVec->size(); i++)
	{
		(*sourceVec)[i].strTitle[19] = '\0';
	}

#if _WIN64
	ar << static_cast<unsigned long>(sourceVec->size());
#else
	ar << sourceVec->size();
#endif
	for (int i = 0; i < sourceVec->size(); i++)
	{
		...
		if (strlen((*sourceVec)[i].strTitle) < 4) 
		{
			CString tmpStr;
			tmpStr.Format("%4d", i);
			ar.WriteString(tmpStr.GetString());
		}
		else 
		{
			ar.WriteString((*sourceVec)[i].strTitle);
		}

		ar << (*sourceVec)[i].nActivate;
		for (int j = 0; j < MAX_HUMAN_PARTS; j++)
		{
			ar << (*sourceVec)[i].dwId[j];
		}
	}
}
void CMover::ReadEQSwitch(CAr& ar, vector<__EQUIP_SWITCH_INFO>* destVec)
{
	destVec->clear();
	int nSize;
	ar >> nSize;
	for (int i = 0; i < nSize; i++)
	{
		__EQUIP_SWITCH_INFO tmpInfo;
		ar.ReadString(tmpInfo.strTitle, 20);
		ar >> tmpInfo.nActivate;
		for (int j = 0; j < MAX_HUMAN_PARTS; j++)
		{
			ar >> tmpInfo.dwId[j];
		}
		tmpInfo.strTitle[19] = '\0';
		if (strlen(tmpInfo.strTitle) < 4) 
		{
			CString tmpStr;
			tmpStr.Format("%4d", i);
			strcpy(tmpInfo.strTitle, tmpStr.GetString());
		}

		destVec->push_back(tmpInfo);
...
void CMover::ProcessFTool()
{
	...
	{
		if (!g_pPlayer->m_vFToolUnits[i].m_bActive)
			continue;
		if (g_pPlayer->m_vFToolUnits[i].m_dwNextUse > GetTickCount())
			continue;
		if (!g_pPlayer->m_vFToolUnits[i].IsValid())
			continue;
		if (!g_WndMng.m_pWndTaskBar)
			return;

		LPSHORTCUT pShortcut = &g_WndMng.m_pWndTaskBar->m_aSlotItem[g_pPlayer->m_vFToolUnits[i].m_nPage][g_pPlayer->m_vFToolUnits[i].m_nFKey];
		if (!pShortcut || pShortcut->m_dwShortcut == SHORTCUT_NONE)
		{
			g_pPlayer->m_vFToolUnits[i].m_dwNextUse = 0;
			g_pPlayer->m_vFToolUnits[i].m_bActive = FALSE;
			continue;
		}

		if (pShortcut->m_dwShortcut == SHORTCUT_CHAT) 
		{
			CString strChat = pShortcut->m_szString;
			if (strChat.Left(3).CompareNoCase("/s ") == 0)
			{
				g_WndMng.PutString("You can not use the F-Tool for shouting. Use the Auto-Shout system instead.", 0, COLOR_ERROR);
				g_pPlayer->m_vFToolUnits[i].m_bActive = FALSE;
				continue;
			}
		}
		...
		g_pPlayer->m_vFToolUnits[i].m_dwNextUse = GetTickCount() + ceil(g_pPlayer->m_vFToolUnits[i].m_fIntervall * 1000.0f);
	}
}
#endif
#endif
...
#ifdef __PREMIUM
BOOL CMover::IsPremium()
{
	if (IsSMMode(SM_PREMIUM)) return TRUE;
	return FALSE;
}
#endif //__PREMIUM

#if defined(__NEW_SFX_WINGS) && defined(__CLIENT)
const void CMover::SetWingSfx(const DWORD dwSfx)
{
	if (m_pSfxWing/* && dwSfx == NULL_ID*/)
	{
		g_WorldMng.Get()->RemoveObj(m_pSfxWing);
		m_pSfxWing = nullptr;
	}

	if (dwSfx != NULL_ID)
		m_pSfxWing = CreateSfx(g_Neuz.m_pd3dDevice, dwSfx, GetPos(), GetId(), GetPos(), GetId(), -1);
}
#endif //__NEW_SFX_WINGS

#ifdef __FL_BAG_UPDATE
BOOL CMover::CanBeUsedInBag(ItemProp* pItemProp)
{
	CMover* pMover;
	if (IsValidObj(pMover) && pItemProp)
	{
		if (pItemProp->dwParts != NULL_ID)
			return FALSE;

		if (pItemProp->dwItemKind3 == IK3_EGG || pItemProp->dwItemKind3 == IK3_PET || pItemProp->dwItemKind3 == IK3_SUMMON_NPC)
			return FALSE;

		if (pItemProp->dwItemKind2 == IK2_MATERIAL || pItemProp->dwItemKind3 == IK3_RANDOM_SCROLL)
			return FALSE;
		...
		if (pItemProp->dwItemKind2 == IK2_BLINKWING)
			return FALSE;

		if (pItemProp->dwItemKind2 == IK2_TREASURE)
			return FALSE;
		...
		if (pMover->m_vtInfo.GetOther())
			return FALSE;

		if (pMover->m_vtInfo.VendorIsVendor())
			return FALSE;
...
#ifdef __MIKSIK_EQUALIZED_MODE
#ifdef __WORLDSERVER
void CMover::EnterTmpPve(int nType, int nClass)
{
	CUser* pUser = (CUser*)this;
	if (!pUser->bTmpMode)
	{
		...
		// Clear Pets //
		pUser->SetSavedEatPetId(NULL_ID);  // If you have error on this string -> just delete it. If you not understand -> just ask me.
		DisableAllPets();
		...
		pUser->m_UserTaskBarTemp.m_nActionPoint = pUser->m_playTaskBar.m_nActionPoint;
		...
		for (int i = 0; i < m_Inventory.GetMax(); i++)
		{
			...
			if (pItem && pItem->IsVisPet())
			{
				for (int j = 0; j < pItem->GetPiercingSize(); j++)
					pItem->SetVisKeepTime(j, 10800 + time_null());
			}
		}
		...
		RedoEquip(FALSE);
		pUser->AddInvAndBuffsSerialize();
		g_UserMng.AddUpdateEquipInfo(this);
	}
}
void CMover::OutTmpPve(BOOL bSave)
{
	CUser* pUser = (CUser*)this;
	if (pUser->bTmpMode)
	{
		...
		pUser->m_playTaskBar.m_nActionPoint = pUser->m_UserTaskBarTemp.m_nActionPoint;
...
#ifdef __AEGON_FIXES
void CMover::SetPartyMemberPos(OBJID objid, D3DXVECTOR3 vPos, int nPlayerID, DWORD dwWorldID)
{
	for (int i = 0; i < MAX_PTMEMBER_SIZE; i++)
	{
		if (nPlayerID == g_Party.m_aMember[i].m_uPlayerId)
		{
			g_Party.m_aMember[i].m_vPos = vPos;
			if(dwWorldID != 0)
				g_Party.m_aMember[i].m_dwWorldID = dwWorldID;
			return;
		}
	}
}
