...
#ifdef __AEGON_FIXES
#define __AEGON_DEFAULT_TELE_IMAGE	"MapLoading_Default.png"
#endif
////////////////////////////////////////////////////////////////////////////////////////////////////
// extern 
////////////////////////////////////////////////////////////////////////////////////////////////////
...
//#endif	// __GIFTBOX0213
	...
	vector<pair<DWORD, pair<DWORD, DWORD>>> GetBoxContent(DWORD dwGiftBox);
...
public:
	...
	std::set<std::string> m_setBlockedUserID;
#endif
#endif
...
#ifdef __VENDING_NPC
	map<u_long, CMover*>        m_mapVendorNPC;
#endif // __VENDING_NPC
...
public:
#ifdef __VENDING_NPC
	CMover* GetVendorNpcByID(u_long idVendor);
	BOOL            AddVendorNpcToGlobal(u_long idVendor, CMover* pMover);
	BOOL            RemoveVendorNpcFromGlobal(u_long idVendor);
#endif // __VENDING_NPC
	...
	ItemProp* GetItemPropDebug( int nIndex, LPCTSTR szInfo);
	ItemProp* GetItemProp_AdvancedDebug(int arg1, const char* file, int line, const char* caller_name);
	ItemProp* GetItemProp_AdvancedDebug(LPCTSTR arg1, const char* file, int line, const char* caller_name);
#else
...
#if defined( __AEGON_GETITEMPROP_DEBUG) && defined(__CLIENT)
	ItemProp* GetItemPropDebug(LPCTSTR lpszItem, LPCTSTR szInfo);
#else
...
#ifdef __AEGON_PREM_MOTIONS
	MotionProp*		GetMotionPropByAniID(DWORD dwAniID);
	MotionProp*		GetMotionPropByUnlockItem(DWORD dwUnlockItemID);
#endif // __AEGON_PREM_MOTIONS
...
#ifdef __FL_FARM_STAT
#ifdef __CLIENT
	vector<COUNT_ITEMS> m_vecCountItems;
	BOOL	LoadCountItems(LPCTSTR lpszFilename);
	void	ReSaveCountItems(LPCTSTR lpszFilename);
#endif // __CLIENT
#endif //__FL_FARM_STAT
...
public:
	...
	CString			GetTeleportString(int nIndex);
	CString			GetTeleportLandName(int nIndex);
	CString			GetTeleportShortcutString(int nIndex);
...
#ifndef __DBSERVER
	CFixedArray< WeaponRarity >    m_nWeaponRarity;
	void	LoadWeaponRarity();
	BOOL	CheckWeaponRarity(int nLevel);
	DWORD	m_dwUpgradePrice;
	int		m_nHighWeaponRarityLevel;
#endif // __DBSERVER
#endif // __WEAPON_RARITY

#ifdef __MOUNT
	map<DWORD, MOUNT_ANIMATION> m_aMountAnimation;
	BOOL LoadMountAnimationList(LPCTSTR lpszFileName);
	MOUNT_ANIMATION* GetMountAnimation(DWORD dwID);
#ifdef __MOUNT_TOWN_RESTRICTION
	map<DWORD, DWORD> m_aMountForbiddenTown;
	BOOL LoadMountForbidTown(LPCTSTR lpszFileName);
	BOOL IsForbiddenTownMount(DWORD dwID);
#endif // __MOUNT_TOWN_RESTRICTION
#endif // __MOUNT

#ifdef __ANARCHY_SYSTEM
	vector<AnarchyBuffProp>m_vAnarchyBuffProp;
	bool LoadAnarchyBuffs(LPCTSTR lpszFileName);
#endif // __ANARCHY_SYSTEM

#if defined(__WIKIPEDIA_MONSTER) && defined(__CLIENT)
	void LoadMonsterBlacklist();
	vector<DWORD>				vMonsterBlacklist;
#endif //__WIKIPEDIA_MONSTER

#if defined(__WIKIPEDIA_ITEMS) && defined(__CLIENT)
	void LoadWikiBlacklist();
	vector<DWORD>				vWikiBlacklist;
#endif //__WIKIPEDIA_MONSTER

#ifdef __UPGRADE_SUCCESS
#if	(defined __WORLDSERVER || defined __CLIENT)
	BOOL LoadUpgradeSuccess(LPCTSTR str);
#endif // __WORLDSERVER
	map<DWORD, map<int, int>>	m_mapUpgradeSuccess;
#endif // __UPGRADE_SUCCESS

#ifdef __SYS_REBIRTH
private:
	RebirthPropEntry* m_rgRebirthPropChart;
	USHORT m_nMaxRebirthLevel;
#ifdef __WORLDSERVER
	map<USHORT, __int64 > m_mapRebirthItems;
#endif
public:
	const USHORT GetMaximumRebirthLevel() const;
	const RebirthPropEntry& GetRebirthProperty(USHORT nCurrentLevel) const;
	bool LoadRebirthProp();
#ifdef __WORLDSERVER
	bool GetRebirthLevelUpGift(const USHORT nCurrentLevel, DWORD& dwItemId, USHORT& nCount) const;
#endif //__WORLDSERVER
#endif //__SYS_REBIRTH

#if defined(__CLIENT) && defined(__AEGON_FIXES)
	void InitializeRegionTeleportMap();
	void AddRegionToTeleportMap(LPCTSTR strRegionTitle, LPCTSTR strImage);
	CString GetTeleportImageForRegion(CString strRegionName);
	void LoadTeleportImageTextures();
#endif
...
#if defined( __AEGON_GETITEMPROP_DEBUG) && defined(__CLIENT)
inline ItemProp* CProject::GetItemPropDebug(LPCTSTR lpszItem, LPCTSTR szInfo)
{
	map<string, DWORD>::iterator i = m_mapII.find(lpszItem);
	if (i != m_mapII.end())
		return GetItemPropDebug(i->second, szInfo);
	return NULL;
}
#else
...
#if defined( __AEGON_GETITEMPROP_DEBUG) && defined(__CLIENT)
inline ItemProp* CProject::GetItemPropDebug(int nIndex, LPCTSTR szInfo)
{
	CString error_message;
	error_message.Format("GetItemProp range_error - Info -> %s <-", szInfo);
	VERIFY_RANGE_ASSERT(nIndex, 0, m_aPropItem.GetSize(), error_message.GetString(), NULL);
...
#ifdef __VENDING_NPC
inline CMover* CProject::GetVendorNpcByID(u_long idVendor)
{
	map<u_long, CMover*>::iterator i = m_mapVendorNPC.find(idVendor);
	if (i != m_mapVendorNPC.end())
		return i->second;
	return NULL;
}

inline BOOL CProject::AddVendorNpcToGlobal(u_long idVendor, CMover* pMover)
{
	map<u_long, CMover*>::iterator i = m_mapVendorNPC.find(idVendor);
	if (i != m_mapVendorNPC.end())
		return FALSE;

	m_mapVendorNPC.insert(make_pair(idVendor, pMover));
	return TRUE;
}

inline BOOL CProject::RemoveVendorNpcFromGlobal(u_long idVendor)
{
	map<u_long, CMover*>::iterator i = m_mapVendorNPC.find(idVendor);
	if (i == m_mapVendorNPC.end())
		return FALSE;

	m_mapVendorNPC.erase(idVendor);
	return TRUE;
}
#endif // __VENDING_NPC

#ifdef __SYS_REBIRTH
inline const USHORT CProject::GetMaximumRebirthLevel() const
{
	return m_nMaxRebirthLevel;
}

#ifdef __WORLDSERVER
inline bool CProject::GetRebirthLevelUpGift(const USHORT nCurrentLevel, DWORD& dwItemId, USHORT& nCount) const
{
	auto iter = m_mapRebirthItems.find(nCurrentLevel);

	if (iter == m_mapRebirthItems.end())
		return false;

	dwItemId = iter->second & 0x00000000FFFFFFFF;
	nCount = (iter->second & 0x0000FFFF00000000) >> 32;
	return true;
}
#endif

inline const RebirthPropEntry& CProject::GetRebirthProperty(USHORT nCurrentLevel) const
{
	ASSERT(nCurrentLevel <= m_nMaxRebirthLevel);
	ASSERT(m_rgRebirthPropChart);
	return m_rgRebirthPropChart[nCurrentLevel];
}
#endif //__SYS_REBIRTH
