...
#ifdef __GH_DUPE_FIX			
				if (gfi.dwItemId >= MAX_UPKEEP_DAY)
				{
					pUser->AddDefinedText(TID_GAME_GUILDHOUSE_TAX_MAX, "%d", MAX_UPKEEP_DAY);
					return FALSE;
				}
...
#endif // __GUILD_HOUSE_MIDDLE
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_vecFntInfo.size());
#else
...
#ifdef __GAMEGUARD
	if (pUser->GetWorld()->GetID() != WI_GUILDHOUSE_SMALL)
		return FALSE;
#endif // __GAMEGUARD
...
void CGuildHouseMng::Serialize( CAr & ar )
{
	...
	{
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_mapGuildHouse.size());
#else
		...
		default:
			{
				if( pUser->GetWorld() && pUser->GetWorld()->GetID() == WI_WORLD_MADRIGAL )
				{
					...
					pUser->m_vMarkingPos = D3DXVECTOR3( 6975.65f, 0.0f, 3331.5f );
				}
				else
				{
