...
#ifdef __DEV_FREECAM
	m_vFree = D3DXVECTOR3(50.0f, 110.0f, 50.0f);
	m_fAngleX = 0.0f;
	m_fAngleY = 0.0f;
	m_idMoverTarget = NULL_ID;
	m_idMoverTargetRot = NULL_ID;
#endif //__DEV_FREECAM
...
//		float z = (float)(8.0f - xRandom(16)) / 256.0f;
	...
	{
		if ((g_pPlayer || m_idMoverTarget != NULL_ID) && g_Option.m_bRollEffect)
		{
			FLOAT fAngleMover = 0.0f;
			if (m_idMoverTarget != NULL_ID)
			{
				CMover* pMoverTarget = prj.GetMover(m_idMoverTarget);
				if (pMoverTarget != nullptr)
					fAngleMover = pMoverTarget->GetAngle();
				else
					m_idMoverTarget = NULL_ID;
			}
			else if (g_pPlayer != NULL)
				fAngleMover = g_pPlayer->GetAngle();
			D3DXMATRIX mRot;
			FLOAT   fRollAng = ((CWndWorld*)g_WndMng.m_pWndWorld)->m_fRollAng;
			D3DXMatrixRotationYawPitchRoll(&mRot, D3DXToRadian(-fAngleMover), 0, D3DXToRadian(fRollAng));
	...
	*/
	...
	m_matInvView._41 = 0.0f;
	m_matInvView._42 = 0.0f;
	m_matInvView._43 = 0.0f;
...
void CBackCamera::Lock()
{
	...
	if( g_WorldMng.Get() )
	{
		...
		if (!(m_idMoverTarget != NULL_ID && CMover::GetActiveMover() && CMover::GetActiveMover()->m_pActMover->IsFly()))
		{
			FLOAT fAngle = 0.0f;
			if (m_idMoverTarget != NULL_ID)
			{
				CMover* pMoverTarget = prj.GetMover(m_idMoverTarget);
				if (pMoverTarget != nullptr)
					fAngle = pMoverTarget->GetAngle();
				else
					m_idMoverTarget = NULL_ID;
			}
			else if (CMover::GetActiveMover() != NULL)
				fAngle = CMover::GetActiveMover()->GetAngle();
			m_fRotx = -fAngle;
		}
#else //__DEV_FREECAM
		CMover* pMoverTarget = (CMover*)g_WorldMng.Get()->GetObjFocus() ;
...
#ifdef __DEV_FREECAM
	if (m_nCamMode == CM_ROTATE)
	{
		CMover* pMoverTargetRot = prj.GetMover(m_idMoverTargetRot);
		if (pMoverTargetRot != nullptr)
		{
			D3DXVECTOR3 vDist = pMoverTargetRot->GetPos() - m_vFree;
			vDist *= 0.06667F;
			m_vFree += vDist;

			vDist = CMover::GetActiveMover()->GetPos() - m_vPos;
			vDist *= 0.06667F;
			if (!CMover::GetActiveMover()->IsRangeObj(m_vPos, 4.0f))
				m_vPos += vDist;
		}
	}

	if (m_nCamMode == CM_FREE || m_nCamMode == CM_ROTATE)
		vPos = m_vFree;
	else
		m_vFree = vPos;
#endif //__DEV_FREECAM
...
#ifdef __DEV_FREECAM
	float fAngleX = pMover->GetAngleX();
