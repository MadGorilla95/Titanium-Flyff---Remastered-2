...
#if defined (__GAMEGUARD) || defined (__FL_FARM_STAT)
	pDrawVertices->vec.z = 0.0f;
#else // __GAMEGUARD
	pDrawVertices->vec.z = -0.0f;
#endif // __GAMEGUARD
...
{
#ifdef __FIX_TEXTUREBUG
	try {
#endif //__FIX_TEXTUREBUG
...
BOOL C2DRender::RenderFillZone( CPoint pt1, CPoint pt2, CPoint pt3, CPoint pt4, DWORD dwColor )
{
	pt1 += m_ptOrigin;
	pt2 += m_ptOrigin;
	pt3 += m_ptOrigin;
	...
	DRAWVERTEX* pDrawVertices;
	HRESULT hr = m_pVBRect->Lock( 0, sizeof(DRAWVERTEX) * 5 , (void**)&pDrawVertices, D3DLOCK_DISCARD );
	pDrawVertices->vec.x     = (FLOAT)pt1.x;
	pDrawVertices->vec.y     = (FLOAT)pt1.y;
	pDrawVertices->vec.z     = 0.0f;
	pDrawVertices->rhw   = 1.0f;
	pDrawVertices->color = dwColor;
	pDrawVertices++;
	pDrawVertices->vec.x     = (FLOAT)pt2.x;
	pDrawVertices->vec.y     = (FLOAT)pt2.y;
	pDrawVertices->vec.z     = 0.0f;
	...
	pDrawVertices->color = dwColor;
	pDrawVertices++;
	pDrawVertices->vec.x     = (FLOAT)pt4.x;
	pDrawVertices->vec.y     = (FLOAT)pt4.y;
	pDrawVertices->vec.z     = 0.0f;
	pDrawVertices->rhw   = 1.0f;
	pDrawVertices->color = dwColor;
	pDrawVertices++;
    ...
    if( ( dwColor & 0xff000000 ) != 0xff000000 ) ////bSupportsAlphaBlend )
    {
		m_pd3dDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, TRUE);
    }
    else
...
BOOL CTexture::LoadTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader, BOOL bLogErrors )
{
	...
	if( bMyLoader == FALSE ) // dds only
	{
		...
		HRESULT hr = ::LoadTextureFromRes( pd3dDevice, pFileName, 
			...
			D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, d3dKeyColor, &imageInfo, NULL, &m_pTexture, bLogErrors );
#ifdef __AEGON_THEME_SWITCHER
		if( hr != S_OK && bLogErrors)
	...
	if( strstr( szFileName, ".bmp" ) )
	{
		IMAGE image;
		LPBYTE lpData = NULL;
		if( LoadBMP( pFileName, &image, bLogErrors) == TRUE )
		{
			lpData = image.lpData;
			m_size = image.size;
			size = m_size;
		...
		//#define BMP32BIT
			...
			Error("CTexture::LoadTexture : %s read error [BMP]", pFileName);
#else // __AEGON_THEME_SWITCHER
		else
			Error( "CTexture::LoadTexture : %s read error [BMP]", pFileName );
#endif // __AEGON_THEME_SWITCHER
	...
	if( strstr( szFileName, ".tga" ) )
	{
		LPBYTE lpData = NULL;
		CTarga targa;
		if( targa.Load( pFileName, &lpData, &m_size, bLogErrors ) == TRUE )
		{
			size = m_size;
			AdjustSize( &m_size );
			int nDestLine = m_size.cx - size.cx;
		#define BMP32BIT
			...
			Error("CTexture::LoadTexture : %s read error [TGA]", pFileName);
#else // __AEGON_THEME_SWITCHER
		else
			Error("CTexture::LoadTexture : %s read error [TGA]", pFileName);
#endif // __AEGON_THEME_SWITCHER
		...
		{
			D3DXIMAGE_INFO imageInfo;
			auto hr = LoadTextureFromRes(pd3dDevice, pFileName, D3DX_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, D3DX_FILTER_TRIANGLE | D3DX_FILTER_MIRROR, D3DX_FILTER_TRIANGLE | D3DX_FILTER_MIRROR, 0, &imageInfo, nullptr, &m_pTexture, bLogErrors);
...
#endif // __AEGON_THEME_SWITCHER
			{
				Error("CTexture::LoadTexture : %s read error. [PNG] hresult %d", pFileName, hr);
				return FALSE;
			}
...
#ifdef __NAVIGATOR_RENDER_BUG_FIX
	if (!pTexture)
		return;
#endif // __NAVIGATOR_RENDER_BUG_FIX
...
BOOL CTextureMng::RemoveTexture( LPCTSTR pKey )
{
	...
	{
#ifdef __AEGON_FIXES
		if ((*mapTexItor).second == nullptr) 
		{
			if(m_mapLoadRetry.find(pKey) != m_mapLoadRetry.end())
				m_mapLoadRetry.erase(pKey);
		}
#endif
...
CTexture* CTextureMng::AddTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader, BOOL bLogErrors)
{
	...
	MapTexItor mapTexItor;
	 ...
	 if (mapTexItor != m_mapTexture.end()) 
	 {
		 ...
		 if (pTexture == nullptr) 
		 {
			 map<std::string, DWORD>::iterator it = m_mapLoadRetry.find(pFileName);
			 if (it != m_mapLoadRetry.end()) 
			 {
				 if (it->second < GetTickCount() + 5000) 
				 {
					 bTryAgain = TRUE;
				 }
			 }
	...
	if( pTexture->LoadTexture( pd3dDevice, pFileName, d3dKeyColor, bMyLoader, bLogErrors ) )
	{
...
#ifdef __GAMEGUARD
	// fix frame drops when texture not found...
	if (bLogErrors) 
	{
		m_mapTexture.insert(MapTexType(pFileName, nullptr));
		m_mapLoadRetry.insert(make_pair(pFileName, GetTickCount()));
	}
#endif //__GAMEGUARD
...
void CDamageNum::Render(CTexturePack *textPackNum)
{
	...
	{ 
		...
		sprintf_s(strTemp, 64, "%d", m_nNumber);
		...
		int spacePosition = nLength % 3;
		for (int i = 0; i < nLength; i++)
		{
			unsigned long nIndex = strTemp[i] - '0' + m_nAttribute * 16;
...
#ifdef __CLIENT
#ifndef __VM_0820
MemPooler<CHealNum>*	CHealNum::m_pPool	= new MemPooler<CHealNum>( 32 );
#endif	// __VM_0820

void CHealNum::Process()
{
	switch( m_nState )
	{
	case 0:	// init
		{
		D3DXVECTOR3 vTemp;
		g_HealNumMng.m_viewport.X = 0;
		g_HealNumMng.m_viewport.Y = 0;
		D3DXVec3Project( &vTemp, &(m_vPos + D3DXVECTOR3(0, 1.0f, 0)), &g_HealNumMng.m_viewport, &g_HealNumMng.m_matProj, &g_HealNumMng.m_matView, &g_HealNumMng.m_matWorld );
		m_fY = vTemp.y;
		m_fDy = 10.0f;
		m_nState ++;
		}
...
void CHealNum::Render(CTexturePack *textPackNum)
{
	
	FLOAT fX,fY;
	D3DXVECTOR3 vTemp;
	// 3d 좌표를 화면상의 좌표로 변환한다
	D3DXVec3TransformCoord( &vTemp, &m_vPos, &g_HealNumMng.m_matView );
	if( vTemp.z < 0 )	return;
	g_HealNumMng.m_viewport.X = 0;
	g_HealNumMng.m_viewport.Y = 0;
	D3DXVec3Project( &vTemp, &(m_vPos + D3DXVECTOR3(0, 1.0f, 0)), &g_HealNumMng.m_viewport, &g_HealNumMng.m_matProj, &g_HealNumMng.m_matView, &g_HealNumMng.m_matWorld );
	...
	if( m_nAttribute < 4 ) { // 숫자인 경우
		...
		for( int i = 0; i < nLength; i++ )
		{
			...
			{
				LPCTSTR szErr = Error( "HealNum::Render : %s %d", strTemp, m_nAttribute );
				ADDERRORMSG( szErr );
			}
...
void CHealNumMng::LoadTexture( LPDIRECT3DDEVICE9 pd3dDevice )
{
	m_textPackNum.LoadScript( pd3dDevice, MakePath( DIR_SFX, _T( "DmgHealEffect.inc" ) ) );
}

CHealNumMng::~CHealNumMng()
{
	DeleteDeviceObjects();
}

BOOL CHealNumMng::DeleteDeviceObjects()
{
	CHealNum* pHealNum;
	for( int i = 0; i < m_Array.GetSize(); i++ )
	{
		pHealNum = (CHealNum*)m_Array.GetAt(i);
		safe_delete( pHealNum );
	}
	m_Array.RemoveAll();
	m_textPackNum.DeleteDeviceObjects();
	return TRUE;
...
void CHealNumMng::AddNumber( D3DXVECTOR3 vPos, DWORD nNumber, DWORD nAttribute )
{
	CHealNum* pHealNum = new CHealNum( vPos, nNumber, nAttribute );
	m_Array.Add( pHealNum );
}

HRESULT	CHealNumMng::RestoreDeviceObjects()
{
	m_textPackNum.RestoreDeviceObjects(g_Neuz.m_pd3dDevice);
	return S_OK;
}

HRESULT	CHealNumMng::InvalidateDeviceObjects()
{
	m_textPackNum.InvalidateDeviceObjects();
	return S_OK;
}

void CHealNumMng::Process()
{
	CHealNum* pHealNum;
...
void CHealNumMng::Render()
{
	CHealNum* pHealNum;
	for( int i = 0; i < m_Array.GetSize(); i++ )
	{
		pHealNum = (CHealNum*)m_Array.GetAt(i);
		pHealNum->Render(&m_textPackNum);
	}
}
#endif // __CLIENT
#endif // __HEAL_SHOW
