...
#ifdef _WIN64
				szPartName[reinterpret_cast<INT_PTR>(pszPart2) - reinterpret_cast<INT_PTR>(lpModelElem->m_szPart)] = 0;
#else
...
#ifdef __AEGON_FIXES
	if (((CModelObject*)pModel)->LoadMotion(szMotionName) == FAIL)// bone animation 읽음
		return FALSE;		
#else // __AEGON_FIXES
