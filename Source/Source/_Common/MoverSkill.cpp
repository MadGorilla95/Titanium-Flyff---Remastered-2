...
int	CMover::GetQueueCastingTime(const int nActionSlot)
...
#endif // __ACTION_SLOT_PAGES
{
	...
	int		i;
#ifdef __ACTION_SLOT_PAGES
	SHORTCUT* pSlotQueue = pUser->m_playTaskBar.m_aSlotQueue[nActionSlot];
#else // __ACTION_SLOT_PAGES
...
*/
	...
	if( pSkillProp->dwSkillType == KT_MAGIC )
	{
		...
		#ifdef __WORLDSERVER
#ifdef __ACTION_SLOT_PAGES
			nCastingTime = GetQueueCastingTime(dynamic_cast<CUser*>(this)->GetCurrentActionSlot());
#else // __ACTION_SLOT_PAGES
...
#endif // __VER >= 8 // __S8_PK
{
...
#ifdef __MOUNT
	if (m_pActMover->IsUseMount()) return FALSE;
#endif // __MOUNT
...
#ifdef __SPAM_PROTECT_SKILLS
	if (IsRegionAttr(RA_SAFETY))
	{
		if (!IsValidObj(this))
			return FALSE;

		if (dwSkill == SI_RIN_HEAL_HEALRAIN)
		{
			((CUser*)this)->AddText("You can't use that skill here.", COLOR_ERROR);
			return FALSE;
		}
	}
#endif //__SPAM_PROTECT_SKILLS
...
#ifdef __GAMEGUARD
		CGuildCombat::__GuildCombatMember* pGCMember = g_GuildCombatMng.FindGuildCombatMember(m_idGuild);
		if (pGCMember != NULL)
		{
			...
			for (int veci = pGCMember->vecGCSelectMember.size() - 1; veci >= 0; --veci)
			{
				...
				if (pJoinPlayer->uidPlayer == m_idPlayer && pJoinPlayer->nlife <= 0 && IsPlayer())
				{
	...
	if( GetWorld()->GetID() == WI_WORLD_GUILDWAR )
	{
		...
		if( pSkillProp->dwID == SI_MAG_MAG_BLINKPOOL 
#ifndef __AEGON_RM_HERO_SKILL
			|| pSkillProp->dwID == SI_RIG_HERO_RETURN
#endif
			)
...
*/
	...
	if(pSkillProp->dwSkillType == BUFF_SKILL)
	{
		...
		IBuff* pBuff = searchMover->m_buffs.GetBuff(BUFF_SKILL, pSkillProp->dwID);
#else
...
#ifdef __fmod
						SoundSystemClass::playSound(nindex);
#else
	...
	if (g_eLocal.GetState(EVE_PK) && CDPSrvr::GetInstance()->m_bPk == TRUE)
#else // __WORLSERVER_MENU
...
#ifdef __BADGE
		static_cast<CUser*>(this)->SetBadgeAddType(HI_USE_ITEM, pItemProp->dwID);
#endif //__BADGE
	}
#ifndef __MAINSERVER
	else
...
BOOL CMover::DoUseItemBag(DWORD dwData, DWORD dwFocusId, int nBag)
{
#ifdef __WORLDSERVER
	if (IsDie())
		return FALSE;
	...
	CItemBase* pItemBase = GetItemId2(nBag, dwId);
#ifdef __SECURITY_FIXES
	if (!pItemBase)
		return FALSE;
#endif //  __SECURITY_FIXES
	...
	{
		CItemElem* pHat = ((CUser*)this)->m_Inventory.GetEquip(PARTS_HAT);
		if (pHat && (pHat->GetProp()->dwID == II_ARM_M_CHR_DARKVADER01HAT || pHat->GetProp()->dwID == II_ARM_F_CHR_DARKVADER01HAT))
			return FALSE;
	}
#endif // __GAMEGUARD
	CWorld* pWorld = GetWorld();
	if (pWorld && pWorld->GetID() == WI_WORLD_GUILDWAR)
	{
		...
		{
			if (IsPlayer())
				((CUser*)this)->AddText(prj.GetText(TID_GAME_GUILDCOMBAT_STANDS_NOTUSEITEM));
			return FALSE;
		}
	}
...
#endif
	{
		...
		case IK2_AEGON:
		{
			...
			{
				if (!((CUser*)this)->DoUsePermBuff(pItemProp))
				{
					return FALSE;
				}
			}
			...
			else if (pItemProp->dwItemKind3 == IK3_UL_MOTION)
			{
				if (!((CUser*)this)->DoUnlockMotion(pItemProp))
				{
					return FALSE;
				}
			}
			...
			else if (pItemProp->dwItemKind3 == IK3_UL_EMOTE)
			{
				if (!((CUser*)this)->DoUnlockEmote(pItemProp))
				{
					return FALSE;
				}
			}
		...
		case IK2_BUFF2:
		{
			...
			if (pItemProp->dwItemKind3 == IK3_SHOPDESIGN && HasBuffByIk3(IK3_SHOPDESIGN)) {
				((CUser*)this)->AddText("You already have an active shop design!", COLOR_ERROR);
				return FALSE;
			}
#endif
			...
			// dwLevel
#ifdef __INCREASE_SKILL_ID_VALUE
			AddBuff(BUFF_ITEM2, pItemElem->m_dwItemId, t, 0);
#else
		...
		case IK2_BUFF:
		{
			...
			int nVal = IsDoUseBuff(pItemProp);
			if (nVal != 0 && nVal != -1)
				return FALSE;
			...
			{
#ifdef __INCREASE_SKILL_ID_VALUE
				IBuff* pBuff = m_buffs.GetBuff(BUFF_ITEM, pItemProp->dwID);
#else
				IBuff* pBuff = m_buffs.GetBuff(BUFF_ITEM, (WORD)(pItemProp->dwID));
#endif
				if (pBuff)
				{
					if (pItemProp->dwSkillTime + pBuff->GetRemain() > 3600000) // 1 Hr Limit
					{
						((CUser*)this)->AddText("Stack limit reached!", COLOR_ERROR);
						return FALSE;
					}
					else
					{
						UpdateItem((BYTE)(pItemElem->m_dwObjId), UI_UPDATE_BUFFTIME, pItemProp->dwID);
					}
				}
			}
			else
#else // __STACKABLE_POWERUPS
			if (IsDoUseBuff(pItemProp) != 0)
				return FALSE;
		...
		case IK2_SYSTEM:
		{
			...
			nResult = DoUseItemSystem(pItemProp, pItemElem);
			{
				...
				{
#if !defined (__AUTO_MAX_SKILL) && !defined (__TITANIUM_DISABLED)
					...
					if (pItemElem->m_dwItemId == II_CHR_SYS_SCR_RESTATE && pItemElem->m_bCharged != TRUE)
					{
		...
		case IK2_AIRFUEL:
		{
			...
			if (pRideProp && TRUE == IsPlayer() && m_pActMover->IsFly())
			{
				...
				else
				{
					...
					{
						((CUser*)this)->AddDefinedText(TID_GAME_COMBFUEL, "");
						return FALSE;
					}
					else
						if (pRideProp->dwItemKind3 == IK3_STICK && pItemProp->dwItemKind3 == IK3_BFLIGHT)
						{
							((CUser*)this)->AddDefinedText(TID_GAME_BOARDFUEL, "");
							return FALSE;
						}
				...
				((CUser*)this)->AddSetFuel(m_nFuel, m_tmAccFuel);
			}
			else
				return FALSE;
		}
		...
		UpdateItemBag((BYTE)(dwId), cUIParam, pItemElem->m_nItemNum, nBag);
	}
#endif // WORLDSERVER
		...
		case IK2_AEGON:
		{
			...
			case IK2_BUFF2:
				...
				if (pItemProp->dwItemKind3 == IK3_SHOPDESIGN && HasBuffByIk3(IK3_SHOPDESIGN)) 
				{
					...
					{
						if(((CUser*)this)->IsPlayer())
							CAegonShopDesignMng::GetInstance()->OnPlayerShopDesignBuffChange((CUser*)this);
					}
#endif
...
#ifdef __MOUNT
							else if (m_pActMover->IsUseMount())
							{
								((CUser*)this)->AddDefinedText(TID_PK_FLIGHT_NOUSE);
								bGMTime = FALSE;
							}
#endif // __MOUNT
...
#ifdef __SECURITY_FIXES
	if (GetWorld()->GetID() == WI_WORLD_ARENA)
		return FALSE;
#endif //  __SECURITY_FIXES
...
int	CMover::DoUseItemSystem( ItemProp *pItemProp, CItemElem* pItemElem, int nPart )
{
	...
	break;
#endif //__SYS_SCR_PARTYSPEEDLEVELUP
...
#ifdef __SYS_REBIRTH
	case II_SYS_SCR_REBIRTH_LEVEL:
	{
		if (false == ((CUser*)this)->ProcessRebirthLevelUp())
			nResult = 99;
	}
	break;
#endif //__SYS_REBIRTH
...
//					else
	...
	case II_SYS_SYS_SCR_PERIN:
		{
#ifdef __SYS_LOCK_ITEM
			if (IsUsableItem(pItemElem) && pItemElem->IsItemLocked())
			{
				((CUser*)this)->AddText("Please unlock Perin before using it.", COLOR_WARNING);
				return 99;
			}
#endif // __SYS_LOCK_ITEM
			int nState = GetSummonState();
...
#ifdef __DROP_LOG
					((CUser*)this)->AddGoldText(100000000);
#endif // __DROP_LOG
	...
	{
#ifdef __GAMEGUARD
		if (((CUser*)this)->GetWorld() && ((CUser*)this)->GetWorld()->GetID() == WI_WORLD_GUILDWAR)
		{
			((CUser*)this)->AddText("You can't use tickets here.");
			nResult = 1;
		}
		else
#endif //__GAMEGUARD
...
#ifdef __INCREASE_SKILL_ID_VALUE
	if (HasBuff(BUFF_ITEM, pItemProp->dwID))
#else
	if (HasBuff(BUFF_ITEM, (WORD)(pItemProp->dwID)))
#endif
	{
...
#ifdef __INCREASE_SKILL_ID_VALUE
			if (HasBuff(BUFF_ITEM, info->dwIDSecond))
#else
			if (HasBuff(BUFF_ITEM, (WORD)(info->dwIDSecond)))
#endif
			{
#ifdef __INCREASE_SKILL_ID_VALUE
				if (HasBuff(BUFF_ITEM, info->dwIDThird))
#else
				if (HasBuff(BUFF_ITEM, (WORD)(info->dwIDThird)))
#endif
				{
#ifdef __INCREASE_SKILL_ID_VALUE
					if (HasBuff(BUFF_ITEM, info->dwIDFourth))
#else
					if (HasBuff(BUFF_ITEM, (WORD)(info->dwIDFourth)))
#endif
					{
#ifdef __INCREASE_SKILL_ID_VALUE
						if (HasBuff(BUFF_ITEM, info->dwIDFifth))
#else
...
#ifdef __INCREASE_SKILL_ID_VALUE
		AddBuff(BUFF_ITEM, pItemProp->dwID, 0, pItemProp->dwSkillTime, GetId());
#else
...
#endif // __BUFF_TOGIFT
#ifdef __TRAILS
		|| pItemProp->dwItemKind3 == IK3_TRAIL
#endif // __TRAILS
		)
	{
		if( HasBuffByIk3( pItemProp->dwItemKind3 ) )
...
#endif	// __JEFF_11_1
#ifdef __AEGON_PERMANENT_BUFF
	else if (pItemProp->dwItemKind3 == IK3_PERM_BUFF) 
	{
...
#endif // __AEGON_PERMANENT_BUFF
#ifdef __INCREASE_SKILL_ID_VALUE
	else if (HasBuff(BUFF_ITEM, pItemProp->dwID))
#else
...
#define II_SYS_SYS_SCR_PARTYSKILLUP01_01 20297
...
#ifdef __GAMEGUARD
	if (HasBuff(BUFF_ITEM2, II_CHR_FOO_COO_GRILLEDEEL) || HasBuff(BUFF_ITEM2, II_CHR_FOO_COO_GRILLEDEEL_1))
		nResult = 1;
#endif // __GAMEGUARD
	...
	case 4:
		...
		break;	
	case 5:
		((CUser*)this)->AddText("You already have this permanent buff.", COLOR_ERROR);
		break;
#endif // __CSC_VER8_5
	}
	return nResult;
...
#ifdef __HEAL_SHOW
					if (pItemProp->dwDestParam[i] == DST_HP)
						g_UserMng.AddHealDamage(this, GETID(this), nPoint);
#endif // __HEAL_SHOW
...
BOOL CMover::DoUseItemSexChange( int nFace )
{
	...
	if (pUser->bTmpMode)
	{
		pUser->AddText("You cant use it while you are in temporal mode.");
		return FALSE;
	}
...
#ifdef __SECURITY_FIXES
								if (pUser->GetWorld())
									this->REPLACE(g_uIdofMulti, pUser->GetWorld()->GetID(), pUser->GetPos(), REPLACE_NORMAL, pUser->GetLayer());
...
int CMover::DoUseItemFeedPocket( CItemElem* pPocket )
{
	...
	// 활성화 검사
#ifdef __INCREASE_SKILL_ID_VALUE
	if (HasBuff(BUFF_ITEM, pPocket->m_dwItemId))
#else
...
#endif
	{
		...
		{
#ifdef __INCREASE_SKILL_ID_VALUE
			RemoveBuff(BUFF_ITEM, pPocket->m_dwItemId);
#else
...
#ifdef __GAMEGUARD
	if (GetTickCount() - static_cast<CUser*>(this)->m_tLastOpenPet > 1000)
		static_cast<CUser*>(this)->m_nNbrOpenPet = 0;

	if (static_cast<CUser*>(this)->m_nNbrOpenPet > 0) return;

	static_cast<CUser*>(this)->m_nNbrOpenPet++;
	static_cast<CUser*>(this)->m_tLastOpenPet = GetTickCount();
#endif // __GAMEGUARD
...
#ifdef __AEGON_QOL_STUFF
		if (!bSamePet && !HasActivatedEatPet())
		{
...
//		ASSERT( pItemProp->dwLinkKind != NULL_ID )
	...
	if( pEatPet )
	{
		...
		if( pAIPet )
		{
...
#ifdef __PET_GLOW
				pEatPet->m_dwPetGlow = pItemElem->GetPetGlowChange();
#endif // __PET_GLOW
			...
			if (!pItemElem->IsVisPet())
			{
				SetEatPetItem(pItemElem->m_dwObjId);
				static_cast<CUser*>(this)->AddActivateVisPet(pItemElem->m_dwObjId, GetEatPetId(), TRUE);
			}
#endif
		}
	}
...
void CMover::InactivateEatPet( void )
{
	...
	if( IsValidObj( pEatPet ) )
	{
		...
		if( pAIPet )
		{
			...
			if (!pItemElem)
			{
				SetEatPetItem(NULL_ID);
				static_cast<CUser*>(this)->AddActivateVisPet(NULL_ID, NULL_ID, TRUE);
			}
#endif
		}
#endif	// __PET_0519
...
void CMover::SetPetVisDST( CItemElem* pItemElem )
{
	...
	{
#ifdef __INCREASE_SKILL_ID_VALUE
#ifndef __EATPET_STATS
		AddBuff(BUFF_EQUIP, pItemElem->m_dwItemId, 1, 999999999);
#endif
#else
...
#ifdef __INCREASE_SKILL_ID_VALUE
#ifndef __EATPET_STATS
		m_buffs.RemoveBuff(BUFF_EQUIP, pItemElem->m_dwItemId, FALSE);
#endif
#else
...
{
#ifdef __SECURITY_FIXES
	if (!pItemElemVisPet || !pItemPropVis)
...
#ifdef __BUFF_1107
#ifdef __INCREASE_SKILL_ID_VALUE
	IBuff* pBuff = m_buffs.GetBuff(BUFF_ITEM, pItemElem->m_dwItemId);
#else
