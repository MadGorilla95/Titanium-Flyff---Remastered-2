...
public:
	...
	int				DoApplySkill(CCtrl* pTarget, ItemProp* pSkillProp, AddSkillProp* pAddSkillProp, bool bIgnoreProp = false, int nParam = 0, BOOL bOnlyDmg = FALSE, BOOL bControl = FALSE, int nSfxMode = 0);		// 스킬을 타겟중심으로 적용
#else
...
#if __VER >= 8 // __S8_PK
#ifdef __AEGON_OPTIONAL_SKILL_SFX
	int				DoApplySkillEx(CCtrl* pTarget, ItemProp* pSkillProp, AddSkillProp* pAddSkillProp, bool bIgnoreProp, int nParam, BOOL bOnlyDmg, BOOL bControl, int nSfxMode = 0);
#else
