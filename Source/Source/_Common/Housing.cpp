...
void CHousing::Serialize( CAr & ar )
{
	...
	{
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_vecHousingInfo.size());
#else
...
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_vecIdVisitAllow.size()); // 방문 허용 목록...
#else
...
void CHousing::SetBuff( DWORD dwItemId, BOOL bSet )
{
	...
	if( IsValidObj( pUser ) && pItemProp )
	{
		if( bSet )
			...
			pUser->RemoveBuff(BUFF_ITEM, dwItemId);
#else
...
BOOL CHousingMng::DestroyHousing( DWORD dwPlayerId )
{
	...
	Error( "CHousingMng::DestroyHousing() - Housing not found, playerID: %07d", dwPlayerId );
	return FALSE;
}
...
CDeployManager::CDeployManager(void)
{
	...
	{
#ifdef __AEGON_FIXES
		CTexture* tmpTex = NULL;
		tmpTex = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_MODELTEX, _T("red.tga")), COLOR_ITEM_BG);
		if (tmpTex) 
		{
			m_pRed = tmpTex->m_pTexture;
		}

		tmpTex = NULL;
		tmpTex = (CTexture*)CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_MODELTEX, _T("Obj_MiniWall01.dds")), COLOR_ITEM_BG);
		if (tmpTex) 
		{
			m_pOriginWall = tmpTex->m_pTexture;
		}

		tmpTex = NULL;
		tmpTex = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_WORLDTEX, _T("Miniroom_floor01.dds")), COLOR_ITEM_BG);
		if (tmpTex) 
		{
			m_pOriginTile = tmpTex->m_pTexture;
		}
#else
		m_pRed		  = ((CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_MODELTEX, _T("red.tga")), COLOR_ITEM_BG ))->m_pTexture;
		m_pOriginWall = ((CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_MODELTEX, _T("Obj_MiniWall01.dds")), COLOR_ITEM_BG ))->m_pTexture;
		m_pOriginTile = ((CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_WORLDTEX, _T("Miniroom_floor01.dds")), COLOR_ITEM_BG ))->m_pTexture;
#endif
	}
...
BOOL	CDeployManager::ChangeWallTex(TCHAR* pTexName)
{
	...
	if(pWorld)
	{
		...
		{
			if(pTexName)
				((CModelObject*)pObj->m_pModel)->GetObject3D()->SetTexture(((CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_MODELTEX, _T(pTexName)), COLOR_ITEM_BG ))->m_pTexture);
			else
...
BOOL	CDeployManager::ChangeTileTex(TCHAR* pTexName)
{
	...
	{
		if(pTexName)
			pWorld->ForceTexture(((CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_MODELTEX, _T(pTexName)), COLOR_ITEM_BG ))->m_pTexture);
		else
...
BOOL	CGuildDeployManager::ChangeWallTex(TCHAR* pTexName)
{
	...
	if( !m_pOriginWall )
	{
		...
		CTexture* pCTex = (CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_MODELTEX, (LPCTSTR)filename.c_str() ), COLOR_ITEM_BG );
...
BOOL	CGuildDeployManager::ChangeTileTex(TCHAR* pTexName)
{
	if( !m_pOriginTile )
	{
		...
		CTexture* pCTex = (CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_WORLDTEX, (LPCTSTR)filename.c_str() ), COLOR_ITEM_BG );
	...
	if(pWorld)
	{
		if(pTexName)
		{
			IDirect3DTexture9* pApplyTex = ((CTexture*)CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_MODELTEX, _T(pTexName)), COLOR_ITEM_BG ))->m_pTexture;
...
#ifdef __VENDING_NPC
CVendorDeploy::CVendorDeploy(void)
{
	m_pRed = NULL;
...
CVendorDeploy::~CVendorDeploy(void)
{
	...
	{
		if (g_WorldMng.Get())
			g_WorldMng.Get()->RemoveObj(m_pTargetObj);
	}
...
CVendorDeploy* CVendorDeploy::GetInstance()
{
	static CVendorDeploy sVendorDeploy;
	return &sVendorDeploy;
}

BOOL    CVendorDeploy::LoadToDeploy(int nItemId, const HOUSING_ITEM& housingInfo)
{
	CWorld* pWorld = g_WorldMng.Get();
	if (!pWorld)
		return FALSE;
	...
	*m_pItem = housingInfo;
	...
	ItemProp* pPropItem = prj.GetItemProp(nItemId);
	if (!pPropItem)
	{
		Error("CVendorDeploy::LoadToDeploy() GetItemProp failed, %d", nItemId);
		return FALSE;
	}

	MoverProp* pMob = prj.GetMoverProp(pPropItem->dwAbilityMin);
	if (!pMob || !pMob->dwID)
	{
		Error("CVendorDeploy::LoadToDeploy() GetMoverProp failed, %d", pPropItem->dwAbilityMin);
		return FALSE;
	}

	m_pTargetObj = CreateObj(g_Neuz.m_pd3dDevice, OT_MOVER, pMob->dwID, TRUE);
	if (!m_pTargetObj)
	{
		assert(0 && "CreateObj failed");
		return FALSE;
	...
	FLOAT fScale = (FLOAT)pPropItem->fFlightSpeed;
	m_nBlendFactor = 80;
	pWorld->AddObj(m_pTargetObj, FALSE);

	m_pTargetObj->SetAngle(m_pItem->m_fAngle);
	m_pTargetObj->SetPos(m_pItem->m_vPos);
	m_pTargetObj->SetScale(D3DXVECTOR3(-fScale, -fScale, -fScale));
	m_pTargetObj->UpdateBoundBox();
	...
	{
		int nMrt = ((CModelObject*)m_pTargetObj->m_pModel)->GetObject3D()->GetGMOBJECT(i)->m_nMaxMtrlBlk;
		for (int j = 0; j < nMrt; j++)
		{
			LPDIRECT3DTEXTURE9 pTexture = ((CModelObject*)m_pTargetObj->m_pModel)->GetObject3D()->GetGMOBJECT(i)->m_pMtrlBlkTexture[j];
			if (pTexture)
				m_vecOriginal.push_back(pTexture);
		}
	}
...
HOUSING_ITEM* CVendorDeploy::EndDeploy(BOOL bClose)
{
	if (m_pTargetObj)
	{
		ChangeObjMode(NORMAL_MODE);
...
BOOL    CVendorDeploy::IsReady()
{
	if (IsValidObj(m_pTargetObj))
		return TRUE;
	else
		return FALSE;
}

BOOL    CVendorDeploy::ChangeObjMode(int nType)
{
	...
	if (IsValidObj(m_pTargetObj))
	{
		...
		case RED_MODE:
		{
			...
			for (int i = 0; i < m_nNumTex; ++i)
			{
				int nMrt = ((CModelObject*)m_pTargetObj->m_pModel)->GetObject3D()->GetGMOBJECT(i)->m_nMaxMtrlBlk;
				for (int j = 0; j < nMrt; j++)
					((CModelObject*)m_pTargetObj->m_pModel)->GetObject3D()->GetGMOBJECT(i)->m_pMtrlBlkTexture[j] = m_pRed;
			}
		}
		break;
		case NORMAL_MODE:
		{
			...
			for (int i = 0; i < m_nNumTex; ++i)
			{
				...
				for (int j = 0; j < nMrt; j++)
				{
					if (iter != m_vecOriginal.end())
					{
						((CModelObject*)m_pTargetObj->m_pModel)->GetObject3D()->GetGMOBJECT(i)->m_pMtrlBlkTexture[j] = *iter;
						iter++;
					}
				}
			}
...
BOOL CVendorDeploy::CheckCollision()
{
	if (IsValidObj(m_pTargetObj))
	{
		CWorld* pWorld = g_WorldMng.Get();
		...
		return pWorld->ProcessObjCollision(m_pTargetObj->GetPos(), m_pTargetObj, NULL);
	}
	return FALSE;
}

BOOL CVendorDeploy::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	BOOL bProcessed = FALSE;
