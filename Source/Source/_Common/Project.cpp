...
#if defined (__AEGON_TELEPORT_SHORTCUT) && defined (__CLIENT)
#include "defineText.h"
#endif // __AEGON_TELEPORT_SHORTCUT) && __CLIENT
...
#if defined(__CLIENT) && defined(__AEGON_FIXES)
map<CString, CString> CProject::m_mapTeleportImages;
BOOL CProject::m_bInitializedMap = FALSE;
#endif
#if defined(__CLIENT) && defined(__AEGON_CLIENTSIDE_PIECE_DROPRATE)
FLOAT	CProject::m_fItemPieceDropRate = 1.0f;
#endif
...
#ifdef __SYS_REBIRTH
	m_rgRebirthPropChart = nullptr;
	m_nMaxRebirthLevel = 0;
#endif //__SYS_REBIRTH

#if defined(__CLIENT) && defined(__AEGON_FIXES)
	m_bInitializedMap = FALSE;
#endif
...
#ifdef __SYS_REBIRTH
	SAFE_DELETE_ARRAY(m_rgRebirthPropChart);
#endif //__SYS_REBIRTH
}
...
BOOL CProject::OpenProject( LPCTSTR lpszFileName )
{
	...
	do 
	{
		...
		{	
			...
			if(!CWndBase::m_Theme.m_bLoadTerrainScript)
				CWndBase::m_Theme.m_bLoadTerrainScript = m_terrainMng.LoadScript( scanner.token );
	...
	LoadMountForbidTown("MountTownForbidden.inc");
...
//	CTestCaseMgr::Instance()->Test();
...
#ifdef __ANARCHY_SYSTEM
	LoadAnarchyBuffs("AnarchyBuffs.inc");
#endif // __ANARCHY_SYSTEM

#if defined(__BADGE) && (defined(__CLIENT) || defined(__WORLDSERVER))
	CBadge::GetInstance()->LoadBadge();
#endif //__BADGE

#if defined (__WIKIPEDIA_MONSTER) && defined (__CLIENT)
	LoadMonsterBlacklist();
#endif //__WIKIPEDIA_MONSTER

#if defined (__WIKIPEDIA_ITEMS) && defined (__CLIENT)
	LoadWikiBlacklist();
#endif //__WIKIPEDIA_ITEMS

#if defined __UPGRADE_SUCCESS && (defined __WORLDSERVER || defined __CLIENT)
	LoadUpgradeSuccess("UpgradeSuccess.inc");
#endif // __UPGRADE_SUCCESS

#ifdef __SYS_MAP_BUFFS
#ifdef __WORLDSERVER
	CMapBuffManager::GetInstance()->Load();
#endif // __WORLDSERVER
#endif //__SYS_MAP_BUFFS
...
#ifdef __SYS_WORLDBOSS
#ifdef __WORLDSERVER
	CWorldBossController::Instance()->LoadScript("WorldBoss.xml");
#endif // __WORLDSERVER
#endif //__SYS_WORLDBOSS
	...
	if (!g_GuildMng.LoadBuffs()) {
		Error("Not loaded: guildbuff.inc");
	}
#endif //__WORLDSERVER
#endif //__WBQT_GUILD_BUFFS

#ifdef __SYS_REBIRTH
	if (false == LoadRebirthProp())
	{
		Error("Not loaded: Rebirth.inc");
		return FALSE;
	}
#endif //__SYS_REBIRTH
...
#ifdef __CLIENT
BOOL CProject::LoadCountItems(LPCTSTR lpszFilename)
{
	m_vecCountItems.clear();
	...
	if (file.Open(lpszFilename, "rb"))
	{
		...
		{
			int nItemID = file.GetDW();
			ItemProp* pItemProp = prj.GetItemProp(nItemID);
			if (pItemProp)
			{
				COUNT_ITEMS mCountItems;
				mCountItems.nCountItems = 0;
				mCountItems.pCountItems = pItemProp;
				m_vecCountItems.push_back(mCountItems);
			}
		}
		file.Close();
		return TRUE;
...
void CProject::ReSaveCountItems(LPCTSTR lpszFilename)
{
	CFileIO file;
	if (file.Open(lpszFilename, "wb") == FALSE)
		return;

	file.PutDW(m_vecCountItems.size());
	for (auto it : m_vecCountItems)
	{
		file.PutDW(it.pCountItems->dwID);
	}
...
BOOL CProject::LoadMotionProp( LPCTSTR lpszFileName )
{
	...
	while( script.tok != FINISHED )
	{
		...
		MotionProp.dwUnlockItemID = script.GetNumber();
		MotionProp.nPriceDP = script.GetNumber();
...
#ifdef __V19_FORMATS
				if( pProp->m_nAttackFirstRange > 80 || pProp->m_nAttackFirstRange <= 0 )
#else //__V19_FORMATS
...
#ifdef __AEGON_FIXES
				BOOL bAdd = TRUE;
				if (di.dwProbability > 3000000000)
				{
					Error("DropItem misconfiguration 0 for %s. probability: %lld", pProp ? pProp->szName : "error", di.dwProbability);
					bAdd = FALSE;
				}
				if (((long long)di.dwProbability) < 0)
				{
					Error("DropItem misconfiguration 0 for %s. probability: %lld", pProp ? pProp->szName : "error", di.dwProbability);
					bAdd = FALSE;
				}
...
#ifdef __AEGON_FIXES
				BOOL bAdd = TRUE;
				if (di.dwNumber > di.dwNumber2) 
				{
					Error("DropGold misconfiguration 0 for %s. %d/%d", pProp ? pProp->szName : "error", di.dwNumber, di.dwNumber2);
					bAdd = FALSE;
				}
				if(di.dwNumber == 0 && di.dwNumber == di.dwNumber2)
				{
					Error("DropGold misconfiguration 1 for %s. %d/%d", pProp ? pProp->szName : "error", di.dwNumber, di.dwNumber2);
					bAdd = FALSE;
				}
				if (di.dwNumber > INT_MAX || di.dwNumber2 > INT_MAX)
				{
					Error("DropGold misconfiguration 2 for %s. %d/%d", pProp ? pProp->szName : "error", di.dwNumber, di.dwNumber2);
					bAdd = FALSE;
				}
				if (((int)di.dwNumber) < 0 || ((int)di.dwNumber2) < 0)
				{
					Error("DropGold misconfiguration 3 for %s. %d/%d", pProp ? pProp->szName : "error", (int)di.dwNumber, (int)di.dwNumber2);
					bAdd = FALSE;
				}
				if(bAdd)
					pProp->m_DropItemGenerator.AddTail(di, pProp->szName);	// copy
...
#ifdef __BUYBACK
					if( nMMI == MMI_TRADE )
						lpCharacter->m_abMoverMenu[ MMI_BUYBACK ] = TRUE;
#endif // __BUYBACK
*/
			}
			else
				...
				if (script.Token == ",")
				{
					...
					if (pItemProp)
					{
						if (lpCharacter->m_nVenderType != 0)
						{
							...
							pItemProp->alternatePrices.AddPrice(lpCharacter->m_nVenderType, int(dwItemCost));
...
#if defined __WORLDSERVER || (defined __CLIENT && defined __WIKIPEDIA_MONSTER_DROPLIST)
...
vector<pair<DWORD, pair<DWORD, DWORD>>> CGiftboxMan::GetBoxContent(DWORD dwGiftbox)
{
	vector<pair<DWORD, pair<DWORD, DWORD>>> retmap;
	retmap.clear();

	auto find = m_mapIdx.find(dwGiftbox);
	if (find != m_mapIdx.end())
	{
		PGIFTBOX pBox = &m_vGiftBox[find->second];
		int old_prob = 0;
		for (int i = 0; i < pBox->nSize; i++)
		{
			retmap.push_back(make_pair(
				pBox->adwItem[i],
				make_pair(
					pBox->anNum[i],
					(pBox->adwProbability[i] - old_prob)
				)
			));
			old_prob = pBox->adwProbability[i];
		}
	}
...
void CSetItem::GetItemEquiped(int& nEqui)
{
	for (int i = 0; i < m_avail.nSize; i++)
	{
...
#ifdef __RIDE_ACTIVATION_TIME
							if (pItemProp->dwItemKind1 == IK1_RIDE)
								pItemProp->dwSkillReadyType = __RIDE_ACTIVATION_TIME;
#endif
...
#ifdef __AEGON_BOW_FACTOR
		pProperty->fBowFactor        = scanner.GetFloat();
#endif // __AEGON_BOW_FACTOR
#ifdef __V19_FORMATS
		scanner.GetToken();
...
BOOL CProject::IsInvalidName( LPCSTR szName )
{
	...
	if (strlen(szName) < 3)
		return TRUE;
	if ((szName[0] >= '0' && szName[0] <= '9') && !IsMultiByte(szName))
		return TRUE;
#endif
...
#if defined(__CLIENT) && defined(__AEGON_FONT_SWITCH)
BOOL CALLBACK EnumFamCallBack(LPLOGFONT lplf, LPNEWTEXTMETRIC lpntm, DWORD FontType, LPVOID aParam)
{
	CString strTmp;
	strTmp.Format("%s", lplf->lfFaceName);
	if (strTmp.GetLength() > 0)
	{
		if (strTmp.FindOneOf("@") != -1)
		{
			strTmp.Remove('@');
		}
		
		if (!easy_find(prj.vecAvailableFonts, strTmp)) 
		{
			prj.vecAvailableFonts.push_back(strTmp);
		}
...
BOOL CProject::LoadAvailFont()
{
	HDC hDC = ::GetDC(g_Neuz.GetSafeHwnd());

	prj.vecAvailableFonts.clear();

	CString strTmp;
	strTmp = "Verdana";
	prj.vecAvailableFonts.push_back(strTmp);

	strTmp = AEG_DEFAULT_THEME;
	prj.vecAvailableFonts.push_back(strTmp);

	EnumFontFamilies(hDC, (LPCTSTR)NULL,
		(FONTENUMPROC)EnumFamCallBack, 0);

	ReleaseDC(g_Neuz.GetSafeHwnd(), hDC);

	std::sort(prj.vecAvailableFonts.begin(), prj.vecAvailableFonts.end());

	return TRUE;
}
#endif //__AEGON_FONT_SWITCH

#if defined(__CLIENT) && defined(__SWITCH_DMG_RENDER)
BOOL CProject::LoadAvailDmg()
{
	prj.DmgData.insert(make_pair(0, "None"));
	prj.DmgData.insert(make_pair(1, "Spaces"));
	prj.DmgData.insert(make_pair(2, "Commas"));
	prj.DmgData.insert(make_pair(3, "Points"));
	return TRUE;
}
#endif //__SWITCH_DMG_RENDER

#if defined(__CLIENT) && defined(__SYS_PET_NO_CLICK)
BOOL CProject::LoadAvailPet()
{
	prj.PetData.insert(make_pair(0, "None"));
	prj.PetData.insert(make_pair(1, "No Click"));
	prj.PetData.insert(make_pair(2, "Hide Pets"));
	return TRUE;
}
#endif //__SWITCH_DMG_RENDER

#ifdef __MOUNT
MOUNT_ANIMATION* CProject::GetMountAnimation(DWORD dwID)
{
	map<DWORD, MOUNT_ANIMATION>::iterator it = m_aMountAnimation.find(dwID);
	if (it != m_aMountAnimation.end())
		return &it->second;
	return NULL;
}
#ifdef __MOUNT_TOWN_RESTRICTION
BOOL CProject::IsForbiddenTownMount(DWORD dwID)
{
	map<DWORD, DWORD>::iterator it = m_aMountForbiddenTown.find(dwID);
	if (it != m_aMountForbiddenTown.end())
		return TRUE;
	return FALSE;
}
#endif // __MOUNT_TOWN_RESTRICTION
...
#if defined (__WIKIPEDIA_MONSTER) && defined (__CLIENT)
void CProject::LoadMonsterBlacklist() {
	vMonsterBlacklist.empty();

	auto doc = new XMLWrapper("MonsterBlacklist.xml", "Blacklist");
	bool bFound = false;
	DWORD id;

	for (auto mob : doc->Get()) {
		id = mob.GetID("ID", bFound);
		if (!bFound) {
			Error("CProject::LoadMonsterBlacklist - Invalid Monster ID: %s", mob.GetString("ID"));
		}
		else {
			vMonsterBlacklist.push_back(id);
		}
	}
	delete doc;
}
...
#if defined (__WIKIPEDIA_ITEMS) && defined (__CLIENT)
void CProject::LoadWikiBlacklist() {
	vWikiBlacklist.empty();

	auto doc = new XMLWrapper("WikiBlacklist.xml", "Blacklist");
	bool bFound = false;

	DWORD id;
	for (auto item : doc->Get()) {
		id = item.GetID("ID", bFound);
		if (!bFound) {
			Error("CProject::LoadWikiBlacklist - Invalid Item ID: %s", item.GetString("ID"));
		}
		else {
			vWikiBlacklist.push_back(id);
		}
	}
	delete doc;
}
...
#if defined (__AEGON_TELEPORT_SHORTCUT) && defined (__CLIENT)
CString	CProject::GetTeleportString(int nIndex)
{
	return CAegonTeleporterMng::GetInstance()->GetTeleportString(nIndex);
}
CString	CProject::GetTeleportLandName(int nIndex) 
{
	return CAegonTeleporterMng::GetInstance()->GetTeleportLandName(nIndex);
}

CString CProject::GetTeleportShortcutString(int nIndex) 
{
	return CAegonTeleporterMng::GetInstance()->GetTeleportShortCutString(nIndex);
}
#endif // __AEGON_TELEPORT_SHORTCUT && __CLIENT

#ifdef __AEGON_PREM_MOTIONS
MotionProp* CProject::GetMotionPropByAniID(DWORD dwAniID) 
{
	for (int i = 0; i < m_aPropMotion.GetSize(); i++) 
	{
		MotionProp* tmpProp = m_aPropMotion.GetAt(i);
		if (tmpProp && tmpProp->dwMotion == dwAniID)
			return tmpProp;
	}
	return NULL;
}
MotionProp* CProject::GetMotionPropByUnlockItem(DWORD dwUnlockItemID) 
{
	for (int i = 0; i < m_aPropMotion.GetSize(); i++)
	{
		MotionProp* tmpProp = m_aPropMotion.GetAt(i);
		if (tmpProp && tmpProp->dwUnlockItemID == dwUnlockItemID)
			return tmpProp;
	}
	return NULL;
}
...
#if defined( __AEGON_GETITEMPROP_DEBUG) && defined(__CLIENT)
ItemProp* CProject::GetItemProp_AdvancedDebug(int arg1, const char* file, int line, const char* caller_name)
{
	CString strFile = file;
	int nPos = strFile.ReverseFind('\\') + 1;
	strFile.Format("%s", strFile.Right(strFile.GetLength() - nPos).GetString());
	CString strInfo;
	strInfo.Format("File: '%s' - Line: '%d', Caller: '%s'", strFile.GetString(), line, caller_name);
	return CProject::GetItemPropDebug(arg1, strInfo.GetString());
}
ItemProp* CProject::GetItemProp_AdvancedDebug(LPCTSTR arg1, const char* file, int line, const char* caller_name)
{
	CString strInfo;
	strInfo.Format("File: '%s' - Line: '%d', Caller: '%s'", file, line, caller_name);
	return GetItemPropDebug(arg1, strInfo.GetString());
}
#endif

#ifdef __SYS_REBIRTH
bool CProject::LoadRebirthProp()
{
	bool Success = false;
	CScript script;
	if (script.Load("Rebirth.inc"))
	{
		Success = true;
		...
		while (script.tok != FINISHED)
		{
			if (script.Token == "Max")
			{
				m_nMaxRebirthLevel = (USHORT)script.GetNumber();
				if (m_nMaxRebirthLevel)
					m_rgRebirthPropChart = new RebirthPropEntry[m_nMaxRebirthLevel + 1];
			}
			else if (script.Token == "Rates")
			{
				script.GetToken(); // {
				script.GetToken();
				while (*script.token != '}')
				{
					if (nChartCounter <= m_nMaxRebirthLevel)
					{
#ifdef __WORLDSERVER
						m_rgRebirthPropChart[nChartCounter].fExpRate = atof(script.token);
						m_rgRebirthPropChart[nChartCounter].fDropRate = script.GetFloat();
						m_rgRebirthPropChart[nChartCounter].fPenyaRate = script.GetFloat();
						script.GetNumber(); // name colour, client only
#else //__WORLDSERVER
						// first exp, second drop, third penya (world only)
						script.GetToken();
						script.GetToken();
						m_rgRebirthPropChart[nChartCounter].dwNameColour = script.GetNumber();
#endif //__WORLDSERVER
						++nChartCounter;
					}
					else
						Error("Error in Rebirth Config > RateChart");
					script.GetToken();
				}
			}
#ifdef __WORLDSERVER
			else if (script.Token == "Gifts")
			{
				script.GetToken(); // {
				script.GetToken();
				while (*script.token != '}')
				{
					USHORT nLevel = (USHORT)atoi(script.token);
					DWORD dwItem = script.GetNumber();
					USHORT nCount = (USHORT)script.GetNumber();

					__int64 nVal = ((__int64)nCount << 32) | dwItem;

					if (nLevel <= m_nMaxRebirthLevel && GetItemProp(dwItem))
						m_mapRebirthItems.insert(make_pair(nLevel, nVal));
					else
						Error("Error in Rebirth Config > ItemChart");
					script.GetToken();
				}
			}
#endif //__WORLDSERVER
...
#if defined(__CLIENT) && defined(__AEGON_FIXES)
void CProject::InitializeRegionTeleportMap() 
{
	m_mapTeleportImages.clear();

	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000007", "MapLoading_Flaris.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000006", "MapLoading_FlarisEast.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000031", "MapLoading_SaintMorning.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000048", "MapLoading_GardenOfRhisis.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000062", "MapLoading_Darkon.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000096", "MapLoading_Darkon1.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000054", "MapLoading_Darkon3.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000082", "MapLoading_Shaduwar.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000086", "MapLoading_ValleyOfRisen.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000075", "MapLoading_Kaillun.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000076", "MapLoading_Bahara.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000013", "MapLoading_FlarisColli.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000036", "MapLoading_SMColli.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000069", "MapLoading_DarkonColli.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000104", "MapLoading_Ophidian.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000105", "MapLoading_Ivillis.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000106", "MapLoading_DekaneMine.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000053", "MapLoading_Wilds.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000081", "MapLoading_Wilds.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000070", "MapLoading_Euphresia.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000087", "MapLoading_Herneros.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000089", "MapLoading_Herneros.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000082", "MapLoading_Sanpres.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000084", "MapLoading_Aminus.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000085", "MapLoading_Aminus.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000072", "MapLoading_Catacombs.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000063", "MapLoading_Clockwork.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000077", "MapLoading_Behemoth.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000071", "MapLoading_Kalgas.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000102", "MapLoading_Mas.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000043", "MapLoading_SaintMorning.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000043", "MapLoading_RisisContami.png");
	AddRegionToTeleportMap("IDS_WDMADRIGAL_WLD_000103", "MapLoading_Forteress.png");

	if (m_mapTeleportImages.size() > 0) 
	{
		m_bInitializedMap = TRUE;
		LoadTeleportImageTextures();
	}
}
void CProject::AddRegionToTeleportMap(LPCTSTR szRegionTitle, LPCTSTR szImage)
{
	auto it = CScript::m_mapString.find(szRegionTitle);
	if (it != CScript::m_mapString.end()) 
	{
		CString strRegion = it->second.c_str();
		strRegion.Replace("\\n", "\r\n");
		m_mapTeleportImages.insert(make_pair(strRegion, CString(szImage)));
	}
}
CString CProject::GetTeleportImageForRegion(CString strRegionName)
{
	if (!m_bInitializedMap)
		InitializeRegionTeleportMap();

	CString strOut = __AEGON_DEFAULT_TELE_IMAGE;

	strRegionName.Replace("\\n", "\r\n");
	auto it = m_mapTeleportImages.find(strRegionName);
	if (it != m_mapTeleportImages.end())
		strOut = it->second;

	return strOut;
}
void CProject::LoadTeleportImageTextures()
{
	auto it = m_mapTeleportImages.begin();
	for (; it != m_mapTeleportImages.end(); it++)
	{
		if(g_Neuz.m_pd3dDevice)
			CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_THEME, it->second.GetString()), COLOR_ITEM_BG);
	}
}
#endif