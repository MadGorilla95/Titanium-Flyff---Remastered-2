...
struct PARTYFINDER_LIST
{
	...
	void Init()
	{
		...
		m_nPoint = 0;
		m_nLeaderId = 0;
		memset(m_sLeader, 0, sizeof(m_sLeader));
		memset(m_sPartyList, 0, sizeof(m_sPartyList));
		m_bLinkAttack = false;
		m_bFortuneCircle = false;
		m_bGiftBox = false;
		m_bSkillFull = false;
		m_bExpUp = false;
		m_bSkillUp = false;
...
#ifdef __AEGON_FIXES
	DWORD	GetWorld(int i) { return m_aMember[i].m_dwWorldID; }
	void	SetWorld(int i, DWORD dwWorldID) { m_aMember[i].m_dwWorldID = dwWorldID; }
#endif
