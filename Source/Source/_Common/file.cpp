...
#include "AES.h"
#define ENC_KEY 30, 9, 19, 96, 227, 5, 58, 64, 3, 39, 12, 110, 72, 66, 84, 221, 61, 185, 49, 83, 218, 20, 134, 205
#define ENC_EXT ".tmmo"
#endif // __AES
...
void CResFile::AddResource( TCHAR* lpszResName )
{
	...
	CFile file;
#ifdef __AEGON_FIXES
	BOOL bFileOpen = file.Open(lpszResName, CFile::modeRead);
	int nTries = 0;
	int nSleepTime = 100;
	int nMaxTries = MulDiv(60, 1000, nSleepTime);

	while (nTries < nMaxTries && !bFileOpen)
	{
		nTries++;
		Sleep(nSleepTime);
		bFileOpen = file.Open(lpszResName, CFile::modeRead);
	}

	if (!bFileOpen) 
	{
		WriteError("failed to open res file: %s", lpszResName);
		return;
	}
#else
	...
	if (bEncryption)
	{
		int nBlockHeaderSize = ((nFileHeaderSize + 15) & (~15));
		pHeader = new char[nBlockHeaderSize];
		char* pCryptHeader = new char[nBlockHeaderSize];
		file.Read(pCryptHeader, nBlockHeaderSize);

		for (int i = 0; i < nBlockHeaderSize / 16; ++i)
		{
			m_crypt.DecryptBlock(reinterpret_cast<const unsigned char*>(pCryptHeader + i * 16), reinterpret_cast<unsigned char*>(pHeader + i * 16));
		}
		safe_delete_array(pCryptHeader);
	}
	else
	{
		pHeader = new char[nFileHeaderSize];
...
#if defined(_WIN64)
	unsigned long time_;
#elif defined(__AES) && defined(__CLIENT)
	long time_;
#else
	time_t time_;
#endif
...
#if defined(__AES) && defined(__CLIENT)
		memcpy(szFullFileName, &pHeader[nHeaderPosition], nFileNameLength); nHeaderPosition += nFileNameLength;
#else // __AES
...
#if defined(__AES) && defined(__CLIENT)
		memcpy(&time_, &pHeader[nHeaderPosition], sizeof(long)); nHeaderPosition += sizeof(long);
#else // __AES
...
#if defined(__AES) && defined(__CLIENT)
	unsigned char szKey[24] = { ENC_KEY };
...
#if defined(__AES) && defined(__CLIENT)
				if (CompareExt(c_file.name, ENC_EXT))
#else // __AES
...
#ifdef __AEGON_FIXES
		BOOL bOpenFile = m_File.Open(lpRes->szResourceFile, CFile::modeRead | CFile::shareDenyNone, &fileExc);
		int nTries = 0;
		int nSleepTime = 100;
		int nMaxTries = MulDiv(60, 1000, nSleepTime);

		while (nTries < nMaxTries && !bOpenFile)
		{
			nTries++;
			Sleep(nSleepTime);
			bOpenFile = m_File.Open(lpRes->szResourceFile, CFile::modeRead | CFile::shareDenyNone, &fileExc);
		}
...
LPVOID CResFile::Read( void )
{
	...
	else
	{
		...
		if (IsEncryption())
		{
			int nReadCount = (m_nFileCurrentPosition - sizeof(int) - sizeof(bool)) % 16;
			int nEncSize = ((size + nReadCount) + 15) & (~15);
			char* szBlockBuffer = new char[nEncSize];
			char* szEncBuffer = new char[nEncSize];
			m_File.Seek(m_nFileCurrentPosition - nReadCount, CFile::begin);
			m_File.Read(szEncBuffer, nEncSize);
			for (int i = 0; i < nEncSize / 16; ++i)
			{
				m_crypt.DecryptBlock(reinterpret_cast<const unsigned char*>(szEncBuffer + i * 16), reinterpret_cast<unsigned char*>(szBlockBuffer + i * 16));
			}
			memcpy(ptr, szBlockBuffer + nReadCount, size);
			safe_delete_array(szBlockBuffer);
			safe_delete_array(szEncBuffer);
...
size_t CResFile::Read( void *ptr, size_t size, size_t n /* = 1  */ )
{
	...
	else
	{
		...
		{
			int nReadCount = (m_nFileCurrentPosition - sizeof(int) - sizeof(bool)) % 16;
			int nEncSize = (((size * n + nReadCount) + 15) & (~15));
			char* szBlockBuffer = new char[nEncSize];
			char* szEncBuffer = new char[nEncSize];
			m_File.Seek(m_nFileCurrentPosition - nReadCount, CFile::begin);
			size_ = m_File.Read(szEncBuffer, nEncSize);
			for (int i = 0; i < nEncSize / 16; ++i)
			{
				m_crypt.DecryptBlock(reinterpret_cast<const unsigned char*>(szEncBuffer + i * 16), reinterpret_cast<unsigned char*>(szBlockBuffer + i * 16));
			}
			memcpy(ptr, szBlockBuffer + nReadCount, size * n);
			safe_delete_array(szBlockBuffer);
			safe_delete_array(szEncBuffer);
			m_nFileCurrentPosition += (size_ - nReadCount < size* n ? size_ - nReadCount : size * n);
			m_File.Seek(m_nFileCurrentPosition, CFile::begin);
		}
		else
		{
			size_ = m_File.Read(ptr, size * n);
