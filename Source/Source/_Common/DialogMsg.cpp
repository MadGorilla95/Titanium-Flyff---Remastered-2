...
#ifndef __AEGON_IMPROVED_SHOPDESIGN
	for (int i = 0; i < 3; i++)
		m_pTex[i] = NULL;
#endif
}
...
HRESULT CDialogMsg::InitDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	CAegonShopDesignMng::GetInstance(); //Trigger shop design load
#else
	m_pTex[0] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "texDialogBox_left.tga" ), COLOR_ITEM_BG );
	m_pTex[1] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "texDialogBox_center.TGA" ), COLOR_ITEM_BG );
	m_pTex[2] = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "texDialogBox_right.tga" ), COLOR_ITEM_BG );
#endif
...
void CDialogMsg::Render( C2DRender* p2DRender )
{
	...
	for( int i = 0; i < m_textArray.GetSize(); i++ )
	{
		...
		BOOL bEnd = TRUE;
		__AEGON_EMOTE_UNIT* pEmoteUnit = lpCustomText->m_pEmoteUnit;
		if (pEmoteUnit && pEmoteUnit->bAnimated && pEmoteUnit->m_pAniTexture)
		{
			
			bEnd = pEmoteUnit->m_pAniTexture->IsLastFrame(&lpCustomText->m_nCurrentAniImage);
		}
		if (!lpCustomText->m_bInfinite && lpCustomText->m_timer.TimeOut() && bEnd)
#else
...
#endif
		{
			...
			if (pEmoteUnit)
			{
				if(pEmoteUnit->bAnimated && pEmoteUnit->m_pAniTexture)
				{
					pEmoteUnit->m_pAniTexture->SetCurrentImage(0);
				}
			}
#endif
			safe_delete( lpCustomText );
		...
		else
		{
			...
			if (pEmoteUnit)
			{
				if (pEmoteUnit->bAnimated) 
				{
					if (pEmoteUnit->m_pAniTexture)
					{
						if (pObj->IsCull() == FALSE)
						{
							// 월드 좌표를 스크린 좌표로 프로젝션 한다.
...
#ifdef __MOUNT
#ifdef __AEGON_FIXES
							if (((CMover*)pObj)->GetMountModel() && ((CMover*)pObj)->GetMountId() > 0 && ((CMover*)pObj)->GetMountId() != NULL_ID)
#else
							if (((CMover*)pObj)->GetMountModel() && ((CMover*)pObj)->GetMountId() > 0)
#endif
							{
							...
							SIZE textureSize = pEmoteUnit->m_pAniTexture->GetSize();
							CPoint point;
							point.x = (LONG)(vOut.x - textureSize.cx / 2);
							point.y = (LONG)(vOut.y - textureSize.cy) - 39;
							...
							if (!pEmoteUnit->m_bRepeat && pEmoteUnit->m_pAniTexture->IsLastFrame(&lpCustomText->m_nCurrentAniImage))
							{
								lpCustomText->m_timer.Set(0);
							}

							pEmoteUnit->m_pAniTexture->SetCurrentImage(lpCustomText->m_nCurrentAniImage);
							pEmoteUnit->m_pAniTexture->Render(p2DRender, point, FALSE);
							pEmoteUnit->m_pAniTexture->UpdateCurrentImage(&lpCustomText->m_nCurrentAniImage, &lpCustomText->m_dwLastRender);
						}
					}
				}
				else 
				{
					if (pEmoteUnit->m_pTexture) 
					{
						if (pObj->IsCull() == FALSE)
						{
							// 월드 좌표를 스크린 좌표로 프로젝션 한다.
							...
							SIZE textureSize = pEmoteUnit->m_pTexture->m_size;
							CPoint point;
							point.x = (LONG)(vOut.x - textureSize.cx / 2);
							point.y = (LONG)(vOut.y - textureSize.cy) - 39;
							...
							p2DRender->RenderTexture(point, pEmoteUnit->m_pTexture);
						}
					}
			...
			if( pObj->IsCull() == FALSE )
			{
...
#ifdef __AEGON_FIXES
				y += __AEGON_OFFSET_PLAYER_NAME_Y;
#endif
				...
				if( lpCustomText->m_nKind == CHAT_SHOUT )
				{
					...
					pShoutTex = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, buffer ), COLOR_ITEM_BG );
					if( pShoutTex )
					{
...
g_ShoutChat:
	...
	for( int i = 0; i < m_VendortextArray.GetSize(); i++ )
	{
		...
		if( pObj->IsCull() == FALSE )
		{
...
#ifdef __LEESE_VENDING_RENDER
			ptTex1.x = (LONG)(vOut.x - 32);
#else //__LEESE_VENDING_RENDER
			ptTex1.x = (LONG)( vOut.x-24 );
#endif //__LEESE_VENDING_RENDER
			ptTex1.y = (LONG)( vOut.y-fGap );
			
#ifdef __AEGON_IMPROVED_SHOPDESIGN
			CMover* pShopMover = (CMover*)pObj;
			__AEGON_SHOP_DESIGN* pDesign = NULL;

			if (IsValidObj(pShopMover))
			{
				if (pShopMover->IsPlayer()) //Player
				{
					...
					{
						if (pShopMover->HasBuff(BUFF_ITEM2, it->first)) {
							pDesign = &it->second;
							break;
						}
					}
				}
				else if (pShopMover->m_bNPCVendor) //Offline Vendor
				{
					map<int, __AEGON_SHOP_DESIGN>::iterator it = CAegonShopDesignMng::GetInstance()->m_mapShopDesigns.find(pShopMover->m_dwShopDesign);
					if (it != CAegonShopDesignMng::GetInstance()->m_mapShopDesigns.end())
					{
						pDesign = &it->second;
					}
				}
			...
			if (!pDesign)
			{
				pDesign = &CAegonShopDesignMng::GetInstance()->m_mapShopDesigns[0];
			}

			CTexture* texShopDesignLeft = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_SHOPS, pDesign->szNameTexLeft), COLOR_ITEM_BG);
			CTexture* texShopDesignCenter = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_SHOPS, pDesign->szNameTexCenter), COLOR_ITEM_BG);
			CTexture* texShopDesignRight = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_SHOPS, pDesign->szNameTexRight), COLOR_ITEM_BG);


			if (texShopDesignLeft)
				p2DRender->RenderTexture(ptTex1, texShopDesignLeft, nAlpha, 1.0f, 1.0f);
#else
			p2DRender->RenderTexture(ptTex1, m_pTex[0], nAlpha, 1.0f, 1.0f);
#endif

#ifdef __LEESE_VENDING_RENDER
			ptTex1.x = (LONG)( vOut.x );
#else //__LEESE_VENDING_RENDER
...
#ifdef __AEGON_IMPROVED_SHOPDESIGN
			if (texShopDesignCenter)
				p2DRender->RenderTextureEx(ptTex1, ptTex2, texShopDesignCenter, nAlpha, 1.0f, 1.0f);
#else
			p2DRender->RenderTextureEx(ptTex1, ptTex2, m_pTex[1], nAlpha, 1.0f, 1.0f);
#endif

#ifdef __LEESE_VENDING_RENDER
			ptTex1.x = (LONG)((int)(vOut.x) + ptTex2.x);
#else //__LEESE_VENDING_RENDER
...
#ifdef __AEGON_IMPROVED_SHOPDESIGN
			if (texShopDesignRight)
				p2DRender->RenderTexture(ptTex1, texShopDesignRight, nAlpha, 1.0f, 1.0f);
#else
			...
			if (g_Neuz.IsVisitedShop(((CMover*)pObj)->m_objid) && ((CMover*)pObj)->m_objid != g_pPlayer->GetId())
				lpCustomText->m_string.SetColor(0xaa000000);
...
#ifdef __AEGON_EMOTE_SYSTEM
void CDialogMsg::AddAegonEmoticonUser(CObj* pObj, __AEGON_EMOTE_UNIT* pEmoteUnit)
{

	if (pEmoteUnit && pEmoteUnit->nEmoteID >= 0 && pEmoteUnit->nEmoteID < CAegonEmoticonMng::GetInstance()->m_mapEmotes.size())
	{
		AddTexture(pObj, NULL, pEmoteUnit);
	}
...
#ifdef __AEGON_EMOTE_SYSTEM
void CDialogMsg::AddTexture(CObj* pObj, CTexture* pTexture, __AEGON_EMOTE_UNIT* emoteUnit)
...
#ifdef __AEGON_EMOTE_SYSTEM
	lpCustomText->m_pEmoteUnit = emoteUnit;
#endif //__AEGON_EMOTE_SYSTEM
...
void CDialogMsg::AddMessage( CObj* pObj, LPCTSTR lpszMessage, DWORD RGB, int nKind, DWORD dwPStyle )
{
	if( pObj->GetType() == OT_MOVER )
	{
		...
		{			
			...
			scanner.GetToken();
#ifdef __AEGON_EMOTE_SYSTEM
			__AEGON_EMOTE_UNIT* emoteUnit = CAegonEmoticonMng::GetInstance()->GetUnitByCommand(lpszMessage, 1);
			if(!emoteUnit)
				emoteUnit = CAegonEmoticonMng::GetInstance()->GetUnitByCommand(lpszMessage, 2);
			if (emoteUnit)
			{
				AddAegonEmoticonUser(pObj, emoteUnit);
				return;
			}
#else //__AEGON_EMOTE_SYSTEM
...
void CDialogMsg::AddVendorMessage(CObj *pObj, LPCTSTR lpszMessage, DWORD RGB)
{
	...
	*/
	...
	if( lpCustomText->m_bInfinite )
	{
		...
		dwColor = ( rand()%2 ) ? COLOR_ERROR: 0xff0000ff;
...
void CDialogMsg::RemoveVendorMessage(CObj *pObj)
{
	for( int i = 0; i < m_VendortextArray.GetSize(); i++ )
	{
		...
		if( lpCustomText->m_pObj == pObj )
		{
			safe_delete( lpCustomText );
#ifdef __SHOPS_VISITED
			g_Neuz.RemoveShopFromList(((CMover*)pObj)->m_objid);
#endif //__SHOPS_VISITED
			m_VendortextArray.RemoveAt( i );
			return;
		}
