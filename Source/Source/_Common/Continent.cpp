...
BYTE CContinent::GetAchievementArea(CMover* pMover)
{
	if (IsValidObj(pMover))
	{
		switch (pMover->GetWorld()->GetID())
		{
...
unsigned char CContinent::worldIdToMCD(const unsigned long worldId, const bool getWorld)
{
	...
	case WI_INSTANCE_OPHIDIAN: return DUNGEON_OPHIDIAN;
	case WI_INSTANCE_KALGAS: return DUNGEON_KALGAS;
	default: return CONT_FLARIS;
	}
}

unsigned char CContinent::getWorld(const CMover* const player)
{
