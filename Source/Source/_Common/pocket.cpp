...
BOOL	CPocketController::Add(int nPocket, CItemElem* pItemElem, vector<PocketController_Result>* apResult, BOOL bAddDropLog)
#else
...
#ifdef __WORLDSERVER
#if (defined (__FL_BAG_LOOT) && defined (__DROP_LOG)) || (defined (__FL_BAG_LOOT) && defined (__FL_FARM_STAT))
		((CUser*)m_pMover)->AddPocketAddItem(nPocket, pItemElem, bAddDropLog == TRUE ? TRUE : FALSE);
#else
