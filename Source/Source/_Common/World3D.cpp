...
#ifdef __DEV_FREECAM
CMover*		CWorld::m_amvrSelectObserve[MAX_MOVEROBSERVE];
#endif //__DEV_FREECAM
#endif
...
FLOAT CWorld::m_fFarPlane = g_Option.m_nViewArea != 0 ? 512.0f : 1024.0f;
...
{ 
	...
	{
#ifdef _WIN64
		//LPCTSTR szErr = Error("Projection : ActiveMover==NULL 0x%16x", (__int64)g_pPlayer);
#else
...
int ObjSortFarToNear2( const VOID* arg1, const VOID* arg2 )
{
	...
	float	fDist1 = a1->m_fDistCamera;
	float	fDist2 = a2->m_fDistCamera;
#else //__GAMEGUARD
	float	fDist1 = (*(CObj**)arg1)->m_fDistCamera;
	float	fDist2 = (*(CObj**)arg2)->m_fDistCamera;
#endif //__GAMEGUARD
...
#ifdef __DEV_FREECAM
	memset(CWorld::m_amvrSelectObserve, 0, sizeof(CMover*) * MAX_MOVEROBSERVE);
#endif //__DEV_FREECAM
#ifdef __ROYAL_RUMBLE
	if (GetID() == WI_WORLD_GUILDWAR || GetID() == WI_WORLD_FWC)
#else // __ROYAL_RUMBLE
...
#ifdef __SFX_OPT_FIX
				if (lpModelElem && g_Option.m_bSFXRenderOff && pObj->GetType() == OT_SFX)
					continue;
#else //__SFX_OPT_FIX
					...
					if( bScan )
					{
						...
						{	
							if( pObj->GetType() == OT_MOVER )
							{
								...
								{
#ifdef __ROYAL_RUMBLE
									if (pMover->GetWorld()->GetID() == WI_WORLD_FWC)
									{
...
//						m_pd3dDevice->SetRenderState( D3DRS_RANGEFOGENABLE, TRUE );
			...
			if( g_Option.m_nShadow < 2 )	
			{
				...
				if (g_pPlayer == NULL)
				{
					if (g_WndMng.m_pTheme->m_bRenderTitleWorld)
					{
						D3DXVECTOR3 tmp = GetCamera()->GetPos();
						kMyPos = D3DXVECTOR3(tmp.x, GetLandHeight(tmp.x, tmp.z), tmp.z);
					}
					else
						kMyPos = D3DXVECTOR3(0, 0, 0);
				}
				else
					kMyPos = g_pPlayer->GetPos();
...
//							m_pd3dDevice->SetRenderState( D3DRS_RANGEFOGENABLE, TRUE );
			...
			for( i = 0; i < m_nObjCullSize; i++)
			{
				...
				CMover* pMover = (CMover*) pObj;
#ifdef __SYS_NO_SHOP_SFX
				if (!pMover->m_vtInfo.IsVendorOpen())
#endif //__SYS_NO_SHOP_SFX
...
//#endif
	...
	for( i = 0; i < m_nSfxCullSize; i++)
	{
		...
		{
			CMover* pMover = prj.GetMover(dynamic_cast<CSfx*>(pObj)->m_idSrc);
			if (IsValidObj(pMover) && pMover->IsPlayer())
			{
				if (pMover->m_vtInfo.IsVendorOpen())
					continue;

				if (g_Option.m_nSfxLevel <= 0 && !dynamic_cast<CSfx*>(pObj)->GetSkill())
					continue;

				if (g_Option.m_bSFXRenderOff && dynamic_cast<CSfx*>(pObj)->GetSkill() != FALSE)
					continue;
			}
		}
#else //__SYS_NO_SHOP_SFX
	#ifdef __SFX_OPT
...
void RenderShadowMap( LPDIRECT3DDEVICE9 pd3dDevice, CObj **pList, int nMax )
{
	...
	else
	{
		if (g_WndMng.m_pTheme->m_bRenderTitleWorld)
		{
			...
			v1 = { v2.x + 5, g_WndMng.m_pTheme->m_pTitleWorld->GetLandHeight(v2.x, v2.z), v2.z };
		}
	}
#else //__FLYFF_INITPAGE_EXT
	fDistLight = 20.0f + (g_pPlayer->m_fDistCamera - 4.0f) * 5.375f;
	...
	D3DXVECTOR3 vLightPos = vLightDir * -fDistLight;
...
void CWorld::SetLight( BOOL bLight )
{
	...
	{
		if (g_WndMng.m_pTheme->m_bRenderTitleWorld)
			pInfo = GetInContinent(g_WndMng.m_pTheme->m_pTitleWorld->GetCamera()->GetPos());
		else
			pInfo = GetInContinent(D3DXVECTOR3(0, 0, 0));
	}
	else 
	{ 
		pInfo = GetInContinent(g_pPlayer->GetPos()); 
	}
#else //__FLYFF_INITPAGE_EXT
				...
				if( pObj != pExceptionObj && ( ObjTypeToObjFilter( pObj->GetType() ) & dwObjectFilter ) )
				{
					if( pObj->GetType() == OT_MOVER )
					{
						...
						else
						{
							...
							if (g_Option.petClickOption)
							{
								CMover* pPet = dynamic_cast<CMover*>(pObj);
								MoverProp* pMoverProp = pPet->GetProp();
								if (pMoverProp)
								{
									if (pMoverProp->dwAI == AII_EGG || pMoverProp->dwAI == AII_PET)
									{
										if (!(GetAsyncKeyState(VK_MENU) & 0x8000) && g_Option.petClickOption == 1)
											continue;
										else
											continue;
									}
