...
#endif	// __WORLDSERVER
{
	...
	m_fFogEndValue = g_Option.m_nViewArea != 0 ? 400.0f : (m_fFarPlane / 2);
...
#ifdef __AEGON_FIXES
	if (nIndex < 0 || nIndex >= MAX_DYNAMICOBJ)
		return FALSE;
...
#ifdef __AEGON_FIXES
	if (pObj->m_dwObjAryIdx >= MAX_DYNAMICOBJ) 
	{
		WriteError("RemoveObjArray  pObj->m_dwObjAryIdx >= MAX_DYNAMICOBJ ");
		return;
	}
#endif
...
void CWorld::_replace( void )
{
	...
	for( int i = 0; i < m_cbReplaceObj; ++i )
	{
...
#ifdef __AEGON_FIXES
		BOOL bTeleportImage = m_aReplaceObj[i].bTeleportImage;
#endif
...
#endif	// __LAYER_1015
		{
...
#ifdef __AEGON_FIXES
			g_UserMng.AddSetPos((CCtrl*)pMover, vPos, bTeleportImage);
#else
			...
			if (pMover->IsPlayer())
			{
				CItemElem* pItemElem = pMover->m_Inventory.GetEquip(PARTS_MASK);
				if (pItemElem)
				{
					...
					if (pItemProp && pItemProp->dwItemKind3 == IK3_SFX_WING)
					{
						...
						if (pItemElem && pItemElem->IsChangedLook())
						{
							...
							if (pItemProp2)
								pMover->SetWingSfx(pItemProp2->dwSfxObj2);
						}
						else
#endif //__LOOKCHANGE
						{
							pMover->SetWingSfx(pItemProp->dwSfxObj2);
						}
					}
					g_UserMng.AddChangeWingSfx(pMover);
				}
			}
		...
		if (pMover->IsPlayer())
		{
			CUser* pUser = static_cast<CUser*>(pMover);
			if (pUser->GetSavedEatPetId() != NULL_ID)
			{
				CItemElem* pItemElem = static_cast<CItemElem*>(pUser->GetItemId(pUser->GetSavedEatPetId()));
#ifdef __AEGON_FIXES
				...
				pUser->InactivateEatPet();
				if (IsUsableItem(pItemElem))
					pUser->DoUseItem(MAKELONG(ITYPE_ITEM, pItemElem->m_dwObjId), pItemElem->m_dwObjId);
#endif
...
#ifdef __AZRIA_1023
#ifdef __V18TICKET
	int nExpand = CTicketProperty::GetInstance()->getExpandedLayer(m_dwWorldID);
#else //__V18TICKET
