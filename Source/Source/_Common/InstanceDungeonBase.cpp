...
void CInstanceDungeonBase::SerializeAllInfo( CAr & ar )
{
	if( ar.IsStoring() )
	{
		// 던전 정보
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_mapID.size());
#else
		ar << m_mapID.size();
#endif
...
#ifdef _WIN64
			ar << static_cast<unsigned long>(pvecTemp->size());
#else
			ar << pvecTemp->size();
#endif
		...
		// 플레이어 쿨타임 정보
#ifdef _WIN64
		ar << static_cast<unsigned long>(m_mapCTInfo.size());
#else
		ar << m_mapCTInfo.size();
#endif
		for( MAP_CTINFO::iterator it=m_mapCTInfo.begin(); it!=m_mapCTInfo.end(); it++)
		{
			...
			pvecIDCT = &it->second;
#ifdef _WIN64
			ar << static_cast<unsigned long>(pvecIDCT->size());
#else
			ar << pvecIDCT->size();
#endif
...
void CInstanceDungeonBase::SetNextState( ID_INFO* pInfo, DWORD dwDungeonId )
{
	if( pInfo && pInfo->nState < ID_BOSS )
	{
		...
		{
			CUser* pUser2 = static_cast<CUser*>(prj.GetUserByID(pInfo->vPlayerID[i]));
...
#ifdef __LEESE_STAFF_DUNGEON
	if (pUser->IsAuthorization(AUTH_ADMINISTRATOR)) // Admin
	{
		return TRUE;
	}
	else
#endif //__LEESE_STAFF_DUNGEON
	{
...
#ifdef __INFO_DUNGEONS
BOOL CInstanceDungeonBase::IncreasePlayerCount(CUser* pUser, DWORD dwDungeonId, DWORD dwWorldId)
			...
			if( pMoverProp )
			{
				...
				if (nState == ID_BOSS)
				{
					...
					for (DWORD i = 0; i < pWorld->m_dwObjNum; i++)
					{
						pObj = pWorld->m_apObject[i];
						if (IsValidObj(pObj) && pObj->GetLayer() == static_cast<int>(dwDungeonId)
							&& pObj->GetType() == OT_MOVER && static_cast<CUser*>(pObj)->IsPlayer())
						{
							static_cast<CUser*>(pObj)->AddDefinedCaption(TRUE, TID_DUNGEON_BOSS_SPAWN, "\"%s\"", pMoverProp->szName);
						}
					}
				}
#endif //__SYS_DUNGEONANNOUNCE
			}
		}
	}
}
...
BOOL CInstanceDungeonBase::TeleportToDungeon( CUser* pUser, DWORD dwWorldId, DWORD dwDungeonId )
{
	...
	{
		pUser->AddDefinedText(TID_ERROR_NOFLY);
		return FALSE;
	}
#endif //__MOUNT
	...
	if( pInfo )	// 해당 던전이 존재하면 채널 검사...
	{
		...
		if( pWorld->m_linkMap.GetLinkMap( static_cast<int>( dwDungeonId ) ) )
		{
...
#ifdef __INFO_DUNGEONS
			IncreasePlayerCount(pUser, static_cast<DWORD>(pUser->m_idparty), dwWorldId);
#else //__INFO_DUNGEONS
			...
			if (!pCT_Info || pCT_Info->dwDungeonId != dwDungeonId)
			{
				...
				DWORD time = GetCoolTime(dwWorldId);
				DWORD timereduce = 0;
				timereduce = (DWORD)(time * ((float)(pUser->GetAdjParam(DST_COOLTIME) / 100.00f)));
				CInstanceDungeonHelper::GetInstance()->SendInstanceDungeonSetCoolTimeInfo(g_uKey, GetType(), pUser->m_idPlayer, COOLTIME_INFO(dwWorldId, dwDungeonId, ((time - timereduce) + GetTickCount())));
#else //__DST_COOLTIME_DUNGEON
...
BOOL CInstanceDungeonBase::ForceTeleportToDungeon(CUser* pUser, DWORD dwWorldId, DWORD dwDungeonId, D3DXVECTOR3 vPos)
{
	CWorld* pWorld = g_WorldMng.GetWorld(dwWorldId);
	if (!pWorld)
		return FALSE;
	...
	if (pInfo)	// 해당 던전이 존재하면 채널 검사...
	{
		if (pInfo->uMultiKey != g_uKey)	// 해당 채널에 생성된 던전이 아니면 입장 불가
		{
			pUser->AddText("Wrong channel.", COLOR_ERROR);
			return FALSE;
		}
		...
		if (pWorld->m_linkMap.GetLinkMap(static_cast<int>(dwDungeonId)))
		{
			...
			pUser->REPLACE(g_uIdofMulti, dwWorldId, vPos, REPLACE_NORMAL, static_cast<int>(dwDungeonId));
#ifdef __INFO_DUNGEONS
			IncreasePlayerCount(pUser, static_cast<DWORD>(dwDungeonId), dwWorldId);
#else //__INFO_DUNGEONS
			IncreasePlayerCount(static_cast<DWORD>(dwDungeonId), dwWorldId);
#endif //__INFO_DUNGEONS			
			return TRUE;
		}
		else
...
#endif //__INFO_DUNGEONS
{
	if( IsNowStateMonster( dwWorldId, dwDungeonId, dwMonsterId ) )
	{
		...
		if( pInfo )
		{
			...
			if (pInfo->nState == ID_BOSS && pInfo->nKillCount >= GetObjCount(dwWorldId, dwDungeonId)) 
			{
				CParty* pParty = g_PartyMng.GetParty(dwDungeonId);
				if (pParty != NULL)
				{
					for (int i = 0; i < pParty->GetSizeofMember(); ++i)
					{
						...
						if (IsValidObj(pPartyMember))
						{
							...
							if (pWorld) 
							{
								...
								{
									CAegonAchievementMng::GetInstance()->AddAchievementProgressForType(pPartyMember, AEGACH_DUNGEONRUN, 1, dwWorldId);
								}
							}
						}
					}
...
#ifdef __TIMER
void CInstanceDungeonBase::SerializeDungeonCoolTime(u_long idplayer, vector<CoolDungeons>& v)
{
...
int CInstanceDungeonBase::GetMonsterState(DWORD dwWorldId, DWORD dwDungeonId, DWORD dwMonsterId)
{
	MAP_IDDATA::iterator it = m_mapDungeonData.find(dwWorldId);
	if (it == m_mapDungeonData.end())
		return -1;
