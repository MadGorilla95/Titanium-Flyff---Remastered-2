...
LPCTSTR Error( LPCTSTR strFormat, ... )
{
	...
	if( n > 0 )
	{
			...
			GetLocalTime( &time );
#ifdef _WIN64
			sprintf(szStr, "64bit %d/%2d/%2d   %02d:%02d:%02d   %s\n",
				time.wYear, time.wMonth, time.wDay,
				time.wHour, time.wMinute, time.wSecond,
				szBuff);
#else
			sprintf( szStr, "32bit %d/%2d/%2d   %02d:%02d:%02d   %s\n", 
				time.wYear, time.wMonth, time.wDay,
				time.wHour, time.wMinute, time.wSecond, 
				szBuff );	
#endif
...
LPCTSTR LogSuccessfullDPPurchase(BOOL bStaffMember, LPCTSTR strFormat, ...)
{
	char szBuff[8192];
	va_list args;
	va_start(args, strFormat);
	...
	if (n > 0)
	{
		...
		if(bStaffMember)
			DEBUGOUTDPPURCHASE(szStr, _T("..\\1. DPPurchaseLog_STAFF.txt"));
		else
			DEBUGOUTDPPURCHASE(szStr);
	}
	return "";
}
#endif
...
CString Separator64(__int64 nNum, char cSeparator)
{
	CString szNumber;
