...
void CMover::UpdateParts( int nSex, int nSkinSet, int nFace, int nHairMesh, int nHeadMesh, PEQUIP_INFO pEquipInfo, CModelObject* pModel, CItemContainer< CItemElem  >* pInventory, BOOL bIfParts, CMover* pMover )
{
	...
	for( int i = 0; i < MAX_HUMAN_PARTS; i++ )
	{
		...
		if( pItemProp )
		{
			...
			if (pItemProp->dwItemKind3 != IK3_SFX_WING)
#endif //__NEW_SFX_WINGS
			{
				if (bIfParts || (i == PARTS_RWEAPON || i == PARTS_LWEAPON || i == PARTS_SHIELD))
				{
#ifdef __LNB_ANIMATED_HATS
					if (pItemProp->dwItemKind3 == IK3_ANIHAT)
					{
						if (pMover)
							pMover->InitializeAnimatedHat(pItemProp->dwID);

						int nIdx = pModel->GetPartsIdx(PARTS_HAT);
						pModel->Destroy(nIdx);
					}
					else
#endif //__LNB_ANIMATED_HATS
#ifdef __ANI_WING
						if (pItemProp->dwItemKind3 == IK3_ANI_WING)
						{
							memset(szPartsName, 0, sizeof(szPartsName));

							int nIdx = pModel->GetPartsIdx(PARTS_MASK);
							pModel->Destroy(nIdx);
						}
						else
#endif //__ANI_WING
					  ...
					  PEQUIP_INFO pEquipInfo, CModelObject* pModel, BOOL bEquip, CMover *pMover ) 
{
...
#ifdef __ANI_WING
	if (pItemProp && dwParts == PARTS_MASK)
	{
		if (bEquip && pItemProp->dwItemKind3 == IK3_ANI_WING)
		{
...
#ifdef __LNB_ANIMATED_HATS
		if (pItemProp->dwParts == PARTS_HAT)
		{
			if (pMover)
			{
				pMover->SetAnimatedHat(nullptr);
	...
	if( bEquip )
	{
...
#ifdef __JEWEL_EQUIPMENTFIX
				if (bEquip && dwElem1 != NULL_ID && dwElem2 != NULL_ID)
				{
					if (dwElem1 == pItemProp->dwID)
						dwParts = pItemProp->dwPartsub;
				}
#endif //__JEWEL_EQUIPMENTFIX

#ifdef __ACCELOGIC
				DWORD dwElem3 = NULL_ID;
				if (pItemElem)
					dwElem3 = pItemElem->m_dwItemId;
		...
		default:
#ifdef __NEW_SFX_WINGS
			if (bIfParts && pItemProp->dwItemKind3 != IK3_SFX_WING)
#else //__NEW_SFX_WINGS
			if (bIfParts)
#endif //__NEW_SFX_WINGS
				((CModelObject*)pModel)->LoadElement( szPartsName, dwParts );
			break;
	...
	if( bEquip )
	{
		if( pItemProp )
		{
			...
			{
				if (g_Neuz.m_nSoundTick < GetTickCount() - 500)
				{
#ifdef __fmod
					SoundSystemClass::playSound(SND_INF_EQUIPWEAPON, { vPos.x, vPos.y, vPos.z });
#else
					PLAYSND(SND_INF_EQUIPWEAPON, &vPos);
#endif
					g_Neuz.m_nSoundTick = GetTickCount();
				}
			}
			else if (pItemProp->dwItemKind1 == IK1_GENERAL && pItemProp->dwItemKind2 == IK2_JEWELRY)
			{
				...
				{
#ifdef __fmod
					SoundSystemClass::playSound(SND_INF_EQUIPACCESSORY, { vPos.x, vPos.y, vPos.z });
#else
...
#ifdef __SYS_GLYPHS
			else if (pItemProp->dwItemKind1 == IK1_GENERAL && pItemProp->dwItemKind2 == IK2_BULLET)
			{
				if (g_Neuz.m_nSoundTick < GetTickCount() - 500)
				{
#ifdef __fmod
					SoundSystemClass::playSound(SND_ITEM_DROPDING, { vPos.x, vPos.y, vPos.z });
#else
					PLAYSND(SND_ITEM_DROPDING, &vPos);
#endif
					g_Neuz.m_nSoundTick = GetTickCount();
				}
			}
			...
			else if (pItemProp->dwItemKind1 == IK1_ARMOR && pItemProp->dwItemKind2 == IK2_CLOTH)
			{
				...
				{
#ifdef __fmod
					SoundSystemClass::playSound(SND_INF_EQUIPCLOTH, { vPos.x, vPos.y, vPos.z });
#else
			...
			else if (pItemProp->dwItemKind1 == IK1_ARMOR && pItemProp->dwItemKind2 == IK2_ARMOR || pItemProp->dwItemKind2 == IK2_ARMORETC)
			{
				if (g_Neuz.m_nSoundTick < GetTickCount() - 500)
				{
#ifdef __fmod
					SoundSystemClass::playSound(SND_INF_EQUIPARMOR, { vPos.x, vPos.y, vPos.z });
#else
	...
	{
#ifdef __fmod
		SoundSystemClass::playSound(SND_INF_INVENTORYDROP, { vPos.x, vPos.y, vPos.z });
#else
...
#ifdef __WORLDSERVER
#ifdef __AEGON_AUTO_DEACTIVATE_PETS
...
#ifdef __SYS_GLYPHS
			|| pItemProp->dwItemKind2 == IK2_BULLET
#endif //__SYS_GLYPHS
			)
		{
...
BOOL CMover::DoEquip( CItemElem* pItemElem, BOOL bEquip, int nPart )
{
	...
	if( bEquip )
	{
		...
		if (pItemProp->dwItemKind3 == IK3_SFX_WING)
		{
			...
			{
				ItemProp* pItemProp2 = prj.GetItemProp(pItemElem->GetLook());
				if (pItemProp2)
					SetWingSfx(pItemProp2->dwSfxObj2);
			}
			else
#endif //__LOOKCHANGE		
			{
				SetWingSfx(pItemProp->dwSfxObj2);
			}
			g_UserMng.AddChangeWingSfx(this);
		}
#endif //__NEW_SFX_WINGS
	}
	else
	...
	if( pItemProp->dwParts == PARTS_RIDE )
	{
		if( bEquip )
		{
			...
			if( IsPlayer() )
			{
#ifdef __GAMEGUARD
#ifdef __fmod
				SoundSystemClass::playMusic(BGM_TH_FLYING, true);
#else
				...
				{
#ifdef __fmod
					SoundSystemClass::playMusic(BGM_EV_LIGHTWING, true);
#else
			...
			{
				MOUNT_ANIMATION* pMount = prj.GetMountAnimation(pItemProp->dwID);
				if (pMount && (pMount->mType == __MOUNT_WALK || pMount->mType == __MOUNT_WALK_FLY || pMount->mType == __MOUNT_FLY))
					SendActMsg(OBJMSG_MOUNT_GROUND_STAND, pItemProp->dwID, 0, 1);
				else
			SendActMsg( OBJMSG_MODE_FLY, pItemProp->dwID, 0, 1 );
		}
#endif // __MOUNT
	...
	if( pProp->dwParts == PARTS_RIDE )
	{
		if( bEquip )
		{
			...
			else
			{
				...
				if (pMount && (pMount->mType == __MOUNT_WALK || pMount->mType == __MOUNT_WALK_FLY || pMount->mType == __MOUNT_FLY))
					SendActMsg(OBJMSG_MOUNT_GROUND_STAND, pProp->dwID, 0, 1);
				else
					SendActMsg(OBJMSG_MODE_FLY, pProp->dwID, 0, 1);
			}
#endif // __MOUNT
...
#ifdef __LNB_ANIMATED_HATS
	else if (pProp->dwItemKind3 == IK3_ANIHAT)
	{
		if (bEquip)
			InitializeAnimatedHat(pProp->dwID);
		else
...
#if defined(__NEW_SFX_WINGS) && defined(__WORLDSERVER)
				if (pItemProp)
				{
					if (pItemProp->dwItemKind3 == IK3_SFX_WING)
					{
...
#ifdef __MOUNT
						&& pItemProp->dwItemKind3 != IK3_MOUNT
#endif // __MOUNT
						)	// 빗자루 ID가 없으면
					{
...
#ifdef __MOUNT
					if (pItemProp->dwItemKind3 == IK3_MOUNT && m_dwMountItemID == 0)
					{
						m_dwMountItemID = pItemProp->dwID;
					}
#endif // __MOUNT
...
void CMover::SetDestParamEquip( ItemProp* pItemProp, CItemElem* pItemElem, BOOL bIgnoreSetItem )
{
	...
	if (pItemElem)
	{
		...
		{
			bAdd = FALSE;
			float fFactor = nWeaponRarity->m_fStatsRate;
#if __VER >= 19
			for (int i = 0; i < 6; i++)
...
#endif // __VER >= 19
			{
				if (pItemProp->dwDestParam[i] != -1)
					SetDestParam(pItemElem->GetProp()->dwDestParam[i], (int)((pItemElem->GetProp()->nAdjParamVal[i]) * fFactor), NULL_CHGPARAM);
			}
		}
	}
	...
	if (pCrystal != NULL)
	{
...
#endif // __VER >= 19
		{
			if (pCrystal->dwDestParam[i] != -1)
				ResetDestParam(pCrystal->dwDestParam[i], (int)((pCrystal->nAdjParamVal[i])));
		}
	}
#endif // __CRYSTAL
...
/*
// buggy shit, dst_giftbox already added above
#ifdef __WORLDSERVER
#ifdef __DST_GIFTBOX
#ifdef __V19_FORMATS
	...
	{
		static_cast<CUser*>(this)->AddBuff(BUFF_EQUIP, pItemProp->dwID, 1, 999999999); 
	}
#else
...
*/
	...
	if( pItemElem && !pItemElem->IsFlag( CItemElem::expired ) )
	{
		...
		if (pItemElem->IsMerged())
			SetDestParam(DST_CHR_DMG, pItemElem->GetMergedWeaponATK(), NULL_CHGPARAM);
...
#endif //__MIKSIK_EQUALIZED_MODE
{
	...
	if (pItemElem)
	{
		...
		if (nWeaponRarity && pItemElem->GetWeaponRarity() > 0)
		{
			...
			{
				if (pItemProp->dwDestParam[i] != -1)
					ResetDestParam(pItemElem->GetProp()->dwDestParam[i], (int)((pItemElem->GetProp()->nAdjParamVal[i]) * fFactor));
			}
		}
	}
#endif // __WEAPON_RARITY
	...
	if (pItemElem)
	{
		...
		if (pCrystal != NULL)
		{
			...
			{
				if (pCrystal->dwDestParam[i] != -1)
					SetDestParam(pCrystal->dwDestParam[i], (int)((pCrystal->nAdjParamVal[i])), NULL_CHGPARAM);
			}
		}
	}
#endif // __CRYSTAL
...
/*
//buggy shit, dst_giftbox already removed above
#ifdef __WORLDSERVER
#ifdef __DST_GIFTBOX
#ifdef __V19_FORMATS
		if (pItemProp->dwDestParam[0] == DST_GIFTBOX || pItemProp->dwDestParam[1] == DST_GIFTBOX || pItemProp->dwDestParam[2] == DST_GIFTBOX || pItemProp->dwDestParam[3] == DST_GIFTBOX || pItemProp->dwDestParam[4] == DST_GIFTBOX || pItemProp->dwDestParam[5] == DST_GIFTBOX)
		...
		{
#ifdef __INCREASE_SKILL_ID_VALUE
			((CUser*)this)->RemoveBuff(BUFF_EQUIP, pItemProp->dwID); 
#else
...
#ifdef __WEAPON_MERGE
		if (pItemElem->IsMerged())
			ResetDestParam(DST_CHR_DMG, pItemElem->GetMergedWeaponATK());
#endif //__WEAPON_MERGE
	}
}
...
void CMover::PickupGoldCore( int nGold )
{
	...
	if( IsPlayer() )
	{
		...
		((CUser*)this)->AddGoldText(nGold, TRUE);
#else
...
#ifdef __GAMEGUARD
	if (pItemProp->dwParts != nPart && nPart >= 0)
#else // __GAMEGUARD
...
#ifdef __GAMEGUARD
	int nHitPoint = GetHitPoint();
	int nMaxHitPoint = GetMaxHitPoint();
	int nManaPoint = GetManaPoint();
	int nMaxManaPoint = GetMaxManaPoint();
	int nFatiguePoint = GetFatiguePoint();
	int nMaxFatiguePoint = GetMaxFatiguePoint();

	if (nHitPoint > nMaxHitPoint)
		SetPointParam(DST_HP, nMaxHitPoint);
	if (nManaPoint > nMaxManaPoint)
		SetPointParam(DST_MP, nMaxManaPoint);
	if (nFatiguePoint > nMaxFatiguePoint)
		SetPointParam(DST_FP, nMaxFatiguePoint);
#endif // __GAMEGUARD
#endif // __WORLDSERVER
