...
//		_tcscpy( prop.szName, scanner.token );
		...
		if (prop.dwItemKind1 == IK1_RIDE)
			prop.dwSkillReadyType = __RIDE_ACTIVATION_TIME;
#endif
...
#ifdef __V19_FORMATS
		prop.dwItemGrade			= scanner.GetNumber();  //Unique (rare) (drops?)
		prop.bCanTrade				= scanner.GetNumber();  //Trade locked?
		prop.dwMainCategory			= scanner.GetNumber();	//FlyCoupon
		prop.dwSubCategory			= scanner.GetNumber();	//Flycoupon
		prop.bCanHaveServerTransform = scanner.GetNumber();	//Can transfer servers
		prop.bCanSavePotion	 		= scanner.GetNumber();	//Elidins poition
		prop.bCanLooksChange		= scanner.GetNumber();	//Can be changed looks
		prop.blsLooksChangeMaterial = scanner.GetNumber();  //Can be used as looks
#endif //__V19_FORMATS
	#ifdef __WORLDSERVER
		#ifdef __INTERNALSERVER
...
#endif //__V19_FORMATS
#ifdef __TITANIUM_MAPS
		, "World\\DuForteress\\DuForteress.txt.txt"
		, "World\\DuAdena\\DuAdena.txt.txt"
		, "World\\DuFlorensia\\DuFlorensia.txt.txt"
		, "World\\DuFFPY\\DuFFPY.txt.txt"
		, "World\\DuPandora\\DuPandora.txt.txt"
		, "World\\DuOphidian\\DuOphidian.txt.txt"
		, "World\\WdFarmingMap\\WdFarmingMap.txt.txt"
#endif //__TITANIUM_MAPS
#ifdef __SYS_SANDBOX
		, "World\\WdSandBox\\WdSandBox.txt.txt"
#endif //__SYS_SANDBOX
	};
...
#ifndef __DBSERVER
void CProject::LoadWeaponRarity()
{
	CScript s;
	if (s.Load("WeaponRarity.inc") == FALSE)
	{
		Error("WeaponRarity.inc not found!");
		return;
	}

	WeaponRarity pWeaponRarity;
	ZeroMemory(&pWeaponRarity, sizeof(pWeaponRarity));
	m_nHighWeaponRarityLevel = 0;

	int nCount = 0;
	s.GetToken();
	while (s.tok != FINISHED)
	{
		if (s.Token == "Configuration")
		{
			s.GetToken(); // {
			while (s.Token != "}")
			{
				if (s.Token == "dwUpgradePrice")
				{
					s.GetToken(); // =
					prj.m_dwUpgradePrice = s.GetNumber();
					s.GetToken(); // ;
				}
				s.GetToken();
			}
			...
			if (s.Token == "Add_Weapon_Rarity")
			{
			s.GetToken(); // {
			while (s.Token != "}")
			{
				if (s.Token == "nRarityLevel")
				{
					s.GetToken(); // =
					pWeaponRarity.m_nRarityLevel = s.GetNumber();
					if (pWeaponRarity.m_nRarityLevel > m_nHighWeaponRarityLevel)
						m_nHighWeaponRarityLevel = pWeaponRarity.m_nRarityLevel;
					s.GetToken(); // ;
				}
				else
				if (s.Token == "szName")
				{
					s.GetToken(); // =
					s.GetToken();
					pWeaponRarity.m_szName = new char[strlen(s.token) + 1];
					strcpy(pWeaponRarity.m_szName, s.token);
					s.GetToken(); // ;
				}
				else
				if (s.Token == "dwColor")
				{
					s.GetToken(); // =
					pWeaponRarity.m_dwColor = s.GetHex();
					s.GetToken(); // ;
				}
				else
				if (s.Token == "fUpgradeRate")
				{
					s.GetToken(); // =
					pWeaponRarity.m_fUpgradeRate = s.GetNumber();
					s.GetToken(); // ;
				}
				else
				if (s.Token == "fAttackRate")
				{
					s.GetToken(); // =
					pWeaponRarity.m_fAttackRate = s.GetFloat(FALSE);
					s.GetToken(); // ;
				}
				else
				if (s.Token == "fStatsRate")
				{
					s.GetToken(); // =
					pWeaponRarity.m_fStatsRate = s.GetFloat(FALSE);
					s.GetToken(); // ;
				}
#ifdef __BLING_BLING
				else
				if (s.Token == "SetEffect")
				{
					s.GetToken(); // (
					s.GetToken(); // Effect
					if (s.Token == "reflect")
					{
						pWeaponRarity.m_bReflect = TRUE;
					}
					s.GetToken(); // )
					s.GetToken(); // ;
				}
					...
					s.GetToken();
						...
						nCount++;
						prj.m_nWeaponRarity.SetAtGrow(pWeaponRarity.m_nRarityLevel, &pWeaponRarity);
				}
			else
			if (s.Token == "Drop")
			{
				int nTotalDropLuck = 0;

				s.GetToken(); // {
				for (int i = 0; i < nCount; i++)
				{
					int nRarityLevel = s.GetNumber();
					int nDropLuck = s.GetNumber();

					WeaponRarity* nWeaponRarity = prj.m_nWeaponRarity.GetAt(nRarityLevel);
					if (nWeaponRarity)
					{
						nWeaponRarity->m_nDropLuck = nDropLuck;
						nTotalDropLuck += nDropLuck;
					}
					else
						Error("CProject::LoadWeaponRarity : Rarity %d doesn't exist!", nRarityLevel);

				}
				s.GetToken(); // }

#ifdef __LEESE_RARITY_1000
				if (nTotalDropLuck != 1000)
				{
					Error("CProject::LoadWeaponRarity : nTotalDropLuck != 1000");
				}
#else //__LEESE_RARITY_1000
				if (nTotalDropLuck != 100)
					Error("CProject::LoadWeaponRarity : nTotalDropLuck != 100");
#endif //__LEESE_RARITY_1000
			}
		s.GetToken();
	}
	prj.m_nWeaponRarity.Optimize();
}

BOOL CProject::CheckWeaponRarity(int nLevel)
{
	return nLevel >= 0 && nLevel <= prj.m_nHighWeaponRarityLevel;
}
#endif // __DBSERVER
...
#ifdef __MOUNT
#ifndef __DBSERVER
BOOL CProject::LoadMountAnimationList(LPCTSTR lpszFileName)
{
	CScript script;
	if (script.Load(lpszFileName) == FALSE)
	{
		Error("Mount.inc not found!");
		return FALSE;
	}
	MOUNT_ANIMATION m_MountAnimation;
	...
	while (script.tok != FINISHED)
	{
		...
		while (script.Token != _T("}"))
		{
			if (script.Token == _T("SetBindBone"))
			{
				script.GetToken(); // (
				m_MountAnimation.dwBindBone = script.GetNumber();
				script.GetToken(); // )
				script.GetToken(); // ;	
			}
			else if (script.Token == _T("PlayerAnimation"))
			{
				script.GetToken();
				m_MountAnimation.dwPlayerAni = script.GetNumber();
				script.GetToken();
				script.GetToken();
			}
			else if (script.Token == _T("SetGroundSpeed"))
			{
				script.GetToken();
				m_MountAnimation.fGroundSpeed = script.GetFloat(TRUE);
			}
			else if (script.Token == _T("SetFlightSpeed"))
			{
				script.GetToken();
				m_MountAnimation.fFlightSpeed = script.GetFloat(TRUE);
			}
			else if (script.Token == _T("SetMount"))
			{
				script.GetToken(); // (
				m_MountAnimation.mType = script.GetNumber();
				script.GetToken(); // )
				script.GetToken(); // ;
			}
			else if (script.Token == _T("Rotation"))
			{
				script.GetToken(); // (
				m_MountAnimation.vRotation.x = script.GetFloat(FALSE);
				m_MountAnimation.vRotation.y = script.GetFloat(FALSE);
				m_MountAnimation.vRotation.z = script.GetFloat(FALSE);
			}
			else if (script.Token == _T("SetOffSet"))
			{
				script.GetToken(); // (
				m_MountAnimation.vOffSet.x = script.GetFloat(FALSE);
				m_MountAnimation.vOffSet.y = script.GetFloat(FALSE);
				m_MountAnimation.vOffSet.z = script.GetFloat(FALSE);
			}
			else if (script.Token == _T("Animation"))
			{
				script.GetToken();//{
				script.GetToken();//Animation Name
				while (script.Token != _T("}"))
				{
					CString strMotion = "";
					int		m_nMotion = 0;
					strMotion = script.Token;
					m_nMotion = script.GetNumber();
					m_MountAnimation.m_aMonsterAnimation.insert(make_pair(m_nMotion, strMotion));
					script.GetToken();//Animation Name
				}
			}
			script.GetToken();
		}
		m_aMountAnimation.insert(make_pair(dwID, m_MountAnimation));
		m_MountAnimation.m_aMonsterAnimation.clear();
		dwID = script.GetNumber();
	}
	return TRUE;
}
#ifdef __MOUNT_TOWN_RESTRICTION
BOOL CProject::LoadMountForbidTown(LPCTSTR lpszFileName)
{
	CScript script;
	if (script.Load(lpszFileName) == FALSE)
	{
		Error("MountTownForbidden.inc not found!");
		return FALSE;
	}

	DWORD dwID = NULL_ID;
	DWORD forbidMode = NULL_ID;

	script.GetToken();
	while (script.tok != FINISHED)
	{

		if (script.Token == _T("SetForbiddenMount"))
		{
			script.GetToken(); // {

			dwID = script.GetNumber();
			forbidMode = script.GetNumber();

			if (m_aMountForbiddenTown.find(dwID) == m_aMountForbiddenTown.end())
				m_aMountForbiddenTown.insert(make_pair(dwID, forbidMode));
...
bool CProject::LoadAnarchyBuffs(LPCSTR lpszFileName)
{
	CScript scanner;
	if (scanner.Load(lpszFileName, FALSE) == FALSE)
	{
		Error("%s File %s not found", __FUNCTION__, lpszFileName);
		return FALSE;
	}
	...
	while (scanner.tok != FINISHED)
	{
		if (scanner.Token == "Buff")
		{
			scanner.GetToken(); // {
			scanner.GetToken(); // } or id
			while (scanner.Token != "}")
			{
				AnarchyBuffProp prop;
				ZeroMemory(&prop, sizeof prop);
				prop.nId = atoi(scanner.Token);
				prop.nPerin = scanner.GetNumber();
				prop.currency = scanner.GetNumber();
				m_vAnarchyBuffProp.push_back(prop);
				scanner.GetToken();
			}
		}
		scanner.GetToken();
...
BOOL CProject::LoadUpgradeSuccess(LPCTSTR lpszFileName)
{
	CScript script;
	if (script.Load(lpszFileName) == FALSE)
	{
		Error("Could not load %s!", lpszFileName);
		return FALSE;
	}
	...
	while (script.tok != FINISHED)
	{
		DWORD dwId = atoi(script.Token);
		script.GetToken(); // {

		map<int, int> upgradeMap;
		script.GetToken();
		while (*script.token != '}')
		{
			int nUpgrade = atoi(script.Token);
			int nAmount = script.GetNumber();
			upgradeMap.insert(make_pair(nUpgrade, nAmount));

			script.GetToken();
		}

		prj.m_mapUpgradeSuccess.insert(make_pair(dwId, upgradeMap));
		script.GetToken();
	}
