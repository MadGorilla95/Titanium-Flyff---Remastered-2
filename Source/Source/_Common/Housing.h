...
public:
	CVendorDeploy();
	virtual ~CVendorDeploy();

	static CVendorDeploy* GetInstance();

	BOOL    LoadToDeploy(int nItemId, const HOUSING_ITEM& housingInfo);
	BOOL    ChangeObjMode(int nType);
	HOUSING_ITEM* EndDeploy(BOOL bClose = TRUE);
...
inline CVendorDeploy* VendorDeploy()
{
	return CVendorDeploy::GetInstance();
}
#endif // __VENDING_NPC
