...
{
	CTexture* pPCArrowTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, _T( "ImgMapArrow.bmp" ) ), COLOR_ITEM_BG );
	if( pPCArrowTexture )
	{
		m_pPCArrowTexture = pPCArrowTexture;
	}

	CTexture* pPartyPCArrowTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, _T( "ImgMapArrowParty.bmp" ) ), COLOR_ITEM_BG );
	if( pPartyPCArrowTexture )
	{
		m_pPartyPCArrowTexture = pPartyPCArrowTexture;
	}

	CTexture* pTeleportationPositionTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "ButtTeleport.bmp"), COLOR_ITEM_BG );
	if( pTeleportationPositionTexture )
	{
		m_pTeleportationPositionTexture = pTeleportationPositionTexture;
	}

	CTexture* pDestinationPositionTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "ButtDestination.bmp"), COLOR_ITEM_BG );
	if( pDestinationPositionTexture )
	{
		m_pDestinationPositionTexture = pDestinationPositionTexture;
	}

	CTexture* pNPCPositionTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "MapNPCPositionMark.bmp"), COLOR_ITEM_BG );
	if( pNPCPositionTexture )
	{
		m_pNPCPositionTexture = pNPCPositionTexture;
	}

	CTexture* pUserMarkPositionTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "WndRainbowRaceLadderFail.tga"), COLOR_ITEM_BG );
