...
#ifdef __AEGON_FIXES
	if (IsInvalidObj(pOwner) || IsInvalidObj(pMover))
		return FALSE;
#endif
...
//		vPos	+= pOwner->GetPos();
		...
		if (fDistSq > 1.5f) // wait till distance before the pet moves
...
#ifdef __BLOU_EXPERIMENTAL_FIX2
		if (!IsUsableItem(pItem) || pComponent->nNum < 1 || pComponent->nNum > pItem->m_nItemNum)
			return FALSE;
#endif //__BLOU_EXPERIMENTAL_FIX2
