	...
	if( m_dwSMTime[SM_VELOCIJUMP] > 0 )
	{
		...
		if( aItemprop )
		{
			...
			if( aItemprop->dwDestParam3 != -1 )
				SetDestParam( aItemprop->dwDestParam3, aItemprop->nAdjParamVal3, NULL_CHGPARAM );
			if( aItemprop->dwDestParam4 != -1 )
				SetDestParam( aItemprop->dwDestParam4, aItemprop->nAdjParamVal4, NULL_CHGPARAM );
			if( aItemprop->dwDestParam5 != -1 )
				SetDestParam( aItemprop->dwDestParam5, aItemprop->nAdjParamVal5, NULL_CHGPARAM );
			if( aItemprop->dwDestParam6 != -1 )
				SetDestParam( aItemprop->dwDestParam6, aItemprop->nAdjParamVal6, NULL_CHGPARAM );
#endif //__V19_FORMATS
		}
	}

#ifdef __PREMIUM
	if (m_dwSMTime[SM_PREMIUM] > 0)
	{
		aItemprop = prj.GetItemProp(g_AddSMMode.dwSMItemID[SM_PREMIUM]);
		if (aItemprop)
		{
			if (aItemprop->dwDestParam1 != -1)
				SetDestParam(aItemprop->dwDestParam1, aItemprop->nAdjParamVal1, NULL_CHGPARAM);
...
void CMover::Serialize( CAr & ar )
{
	...
	if( ar.IsStoring() )	// STORING
	{
		...
		if( m_bPlayer )	// PLAYER
		{
			...
			ar << m_bDPCommandInProgress;
#endif // __AEGON_INGAME_DP

#ifdef __AEGON_LOOT_BOX
			ar << m_bLootBoxInProgress;
			ar << m_dwLootboxItem;
			ar << m_dwLootboxItemNum;
		...
		ar << static_cast<unsigned long>(m_vFToolUnits.size());
		for (int i = 0; i < m_vFToolUnits.size(); i++) 
		{
			m_vFToolUnits[i].Write(ar);
		}
#endif
		...
		m_sPetfilterLists.Write(ar);
			...
			if( CObj::GetMethod() == METHOD_NONE )
			{
				...
				m_MHProgress.WriteMap(ar);
#endif //__AEGON_MONSTER_HUNT

#ifdef __BADGE
				ar.WriteContainerPOD(GetOwnedBadges());
#endif //__BADGEE

#ifdef __AEGON_ACHIEVEMENT_SYSTEM
				m_AchievementProgress.WriteMap(ar);

#if _WIN64
				ar << static_cast<unsigned long>(m_AchievementFavorites.size());
#else
				ar << m_AchievementFavorites.size();
#endif
				for (int i = 0; i < m_AchievementFavorites.size(); i++) 
				{
					ar << m_AchievementFavorites[i];
				}
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_SEASON_PASS
				m_SeasonpassProgress.WriteMap(ar);
...
#	endif
...
#ifdef __FIX_GENDER_NPC
			ar << m_bySex;
#endif //__FIX_GENDER_NPC
		...
		{
			// Main Stats
			ar << GetParam(DST_STR, 0);
			ar << GetParam(DST_DEX, 0);
			ar << GetParam(DST_INT, 0);
			ar << GetParam(DST_STA, 0);

			// Speed
			ar << GetParam(DST_SPEED, 0);
			ar << GetParam(DST_CHRSTATE, 0);
			ar << GetParam(DST_JUMPING, 0);

			// Max.HP
			ar << GetParam(DST_HP_MAX_RATE, 0);
			ar << GetParam(DST_HP_MAX, 0);

			// For Spell animations
			ar << GetParam(DST_SPELL_RATE, 0);

			// For Attack Animations
			ar << GetParam(DST_ATTACKSPEED, 0);
		}
#endif // __DBSERVER
#endif // __SYS_PLAYER_SYNC
	...
	else	// LOADING
	{
		...
		ar >> vDestPos;
#ifndef __DBSERVER
		if (vDestPos.x != 0.0f && vDestPos.z != 0.0f)
			SetDestPos(vDestPos, m_bForward, FALSE);
#endif // __DBSERVER
#endif // __SYS_PLAYER_SYNC
...
#	endif	// __WORLDSERVER	//__CLIENT
...
#ifdef __SYS_CUSTOM_TITLE
		ar.ReadString(m_szOwnTitle, MAX_CUSTOM_TITLE);
		ar >> m_bShowTitle;
#endif //__SYS_CUSTOM_TITLE
...
#ifdef __SWITCH_EQUIP
		ReadEQSwitch(ar, &m_vEquipSwitchInfo);
#endif // __SWITCH_EQUIP
		...
		{
			m_vFToolUnits.clear();
			unsigned int nSizeFTool;
			ar >> nSizeFTool;
			for (int i = 0; i < nSizeFTool; i++)
			{
				__AEGON_F_TOOL_UNIT tmpUnit;
				tmpUnit.Read(ar);
				if (m_vFToolUnits.size() < MAX_F_TOOL_UNITS)
					m_vFToolUnits.push_back(tmpUnit);
			}
		}
#endif
		...
		m_sPetfilterLists.Read(ar);
			...
			if( CObj::GetMethod() == METHOD_NONE )
			{
...
#if defined(__ANI_WING) && !defined(__DBSERVER)
				if (!IsMode(MODE_OPTION_DONT_RENDER_MASK))
				{
					pItemElem = m_Inventory.GetEquip(PARTS_MASK);
					if (pItemElem)
					{
						if (pItemElem->GetProp()->dwItemKind3 == IK3_ANI_WING)
						{
				...
				for( int l = 0 ; l < MAX_HONOR_TITLE ; ++l )
				{
					...
					CTitleManager::Instance()->SetValue(l, -1, m_aHonorTitle[l]);
...
#ifdef __AEGON_MONSTER_HUNT
				m_MHProgress.ReadMap(ar);
#endif //__AEGON_MONSTER_HUNT

#ifdef __BADGE
				ar.ReadContainerPOD(GetOwnedBadges(), 200);
#endif //__BADGE


#ifdef __AEGON_ACHIEVEMENT_SYSTEM
				m_AchievementProgress.ReadMap(ar);
				...
				for (int i = 0; i < vecSize; i++)
				{
					int favID;
					ar >> favID;
					m_AchievementFavorites.push_back(favID);
				}
#endif // __AEGON_ACHIEVEMENT_SYSTEM

#ifdef __AEGON_SEASON_PASS
				m_SeasonpassProgress.ReadMap(ar);
#endif // __AEGON_SEASON_PASS
...
#ifdef __MOUNT
					&& prj.GetItemProp(m_aEquipInfo[PARTS_RIDE].dwId)->dwItemKind3 != IK3_MOUNT
#endif // __MOUNT	
					)
					m_dwRideItemIdx		= m_aEquipInfo[PARTS_RIDE].dwId;
#ifdef __MOUNT
				else if (m_aEquipInfo[PARTS_RIDE].dwId != NULL_ID && prj.GetItemProp(m_aEquipInfo[PARTS_RIDE].dwId)->dwItemKind3 == IK3_MOUNT)
					m_dwMountItemID = m_aEquipInfo[PARTS_RIDE].dwId;
#endif // __MOUNT
		...
		if (CObj::GetMethod() == METHOD_EXCLUDE_ITEM && m_bPlayer)
		{
			...
			// this is for posi stuff
			ar >> c;
			SetParam(DST_SPEED, c);
			ar >> c;
			SetParam(DST_CHRSTATE, c);
			ar >> c;
			SetParam(DST_JUMPING, c);

			// Max. HP
			// Max.HP
			ar >> c;
			SetParam(DST_HP_MAX_RATE, c);
			ar >> c;
			SetParam(DST_HP_MAX, c);

			// For Spell animations
			ar >> c;
			SetParam(DST_SPELL_RATE, c);

			// For Attack animations
			ar >> c;
			SetParam(DST_ATTACKSPEED, c);
		}
#endif // __DBSERVER
#endif // __SYS_PLAYER_SYNC
