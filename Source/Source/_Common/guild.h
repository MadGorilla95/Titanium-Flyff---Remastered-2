...
const DWORD PF_GUILDCOMBAT				= 0x00000080;
const DWORD PF_SECRETROOM				= 0x00000100;
#endif //__NEW_GUILD_PERM

#ifdef __GUILDFINDER
struct GUILDFINDER_LIST
{
	u_long	m_uGuildId;
	TCHAR	m_sGuild[33];
	int		m_nSizeofMember;
	...
	LONG	m_nLevel;
	u_long	m_nLeaderId;
	GUILDFINDER_LIST() { Init(); };
	~GUILDFINDER_LIST() {};

	void Init() { memset(m_sGuild, 0, sizeof(m_sGuild)); m_uGuildId = m_nSizeofMember = m_nLevel = m_nLeaderId = m_nMaxSize = 0; };
};
#endif //__GUILDFINDER
...
{
	DWORD dwBuffID;
	DWORD dwLvConstraint;
	USHORT nMemCountConstraint;
	USHORT nMemOnlineConstraint;
} GUILD_BUFF_INFO, * PGUILD_BUFF_INFO;
#endif //__WORLDSERVER
...
#ifdef __SYS_GUILDCOLOR
	BOOL	SetColor(CGuild* pGuild, DWORD R, DWORD G, DWORD B);
#endif //__SYS_GUILDCOLOR
...
#ifdef __WORLDSERVER
	bool	LoadBuffs();
	void	AddBuffs(CGuild* pGuild, CUser* pUser = nullptr);
	void	RemoveBuffs(CGuild* pGuild, CUser* pUser = nullptr);

private:
	vector<GUILD_BUFF_INFO> m_vecBuffInfo;
#endif //__WORLDSERVER
#endif //__WBQT_GUILD_BUFFS
};
