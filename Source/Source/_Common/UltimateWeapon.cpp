...
int CUltimateWeapon::RemoveGem( CUser* pUser, OBJID objItemId, OBJID objItemGem, int nSlot)
...
#endif //__SYS_DIA_REMOVE_CHOICE
{
	...
	if( nRandom < m_nRemoveGemProb )
	{
		// 성공시 - 보석 제거
#ifdef __SYS_DIA_REMOVE_CHOICE
		if (pItemElem->GetUltimatePiercingItem(nSlot) != 0)
		{
			pUser->UpdateItem((BYTE)(pItemElem->m_dwObjId), UI_ULTIMATE_PIERCING, nSlot);
...
int CUltimateWeapon::EnchantWeapon( CUser* pUser, OBJID objItem, OBJID objItemGem )
{
	...
	bool bSuccess = false;
	bool bFind = false;
	auto iter = prj.m_mapUpgradeSuccess.find(pItemElemWeapon->GetProp()->dwReferStat1);
	if (iter != prj.m_mapUpgradeSuccess.end())
	{
		const map<int, int>& upgrade = iter->second;
		auto iterUpgrade = upgrade.find(pItemElemWeapon->GetAbilityOption());
		if (iterUpgrade != upgrade.end())
		{
			bFind = true;
			bSuccess = iterUpgrade->second > 0 && pItemElemWeapon->m_nUpgradeTries + 1 >= iterUpgrade->second;
		}
	}

	if (nRandom < it->second && !bSuccess)
...
#ifdef __UPGRADE_SUCCESS
		pUser->UpdateItem((BYTE)pItemElemWeapon->m_dwObjId, UI_UPGRADE_SUCCESS, 0);
#endif // __UPGRADE_SUCCESS
...
#ifdef __UPGRADE_SUCCESS
			if (bFind)
				pUser->UpdateItem((BYTE)pItemElemWeapon->m_dwObjId, UI_UPGRADE_SUCCESS, pItemElemWeapon->m_nUpgradeTries + 1);
#endif // __UPGRADE_SUCCESS
		}
		else	// 얼터멋 웨폰 파괴
		{
...
BYTE CUltimateWeapon::SmeltSafetyUltimate( CUser* pUser, CItemElem* pItemMain, CItemElem* pItemMaterial, CItemElem* pItemProtScr )
{
	...
	if ((int)(xRandom(1000000)) > it->second && !bSuccess)
#else // __UPGRADE_SUCCESS
