...
void CCtrl::CreateSkillSfx( CCtrl *pTarget, ItemProp *pSkillProp, AddSkillProp *pAddSkillProp )
{
		...
		
	{
			...
			} else
			{
				if( GetType() == OT_MOVER )
				{
					if( pSkillProp->dwSkillType == NULL_ID )	// NULL_ID면 아이템들
					{
						...
						if (((CMover*)this)->HasBuff(BUFF_ITEM, pSkillProp->dwID) == FALSE)	// 이미 실행되고 있지 않을때만 이펙 출력
#else
...
#ifdef __INCREASE_SKILL_ID_VALUE
						if (((CMover*)this)->HasBuff(BUFF_SKILL, pSkillProp->dwID) == FALSE
		...
		if( pSfx )
		{
			D3DXVECTOR3 vPos = GetPos();
#ifdef __fmod
			SoundSystemClass::playSound(pSkillProp->dwSndAttack1, { vPos.x, vPos.y, vPos.z });
#else
			...
			if( nAttackerPK == 1 || nDefenDerPK == 1 )
			{
				if( pSkillProp->nEvildoing < 0 )
				{
					...
					if( !(pAttacker->IsPVPTarget( pDefender ) || 
						...
						|| pAttacker->IsSiegeTarget(pDefender)
...
#ifdef __INCREASE_SKILL_ID_VALUE
					bNew = static_cast<CMover*>(this)->AddBuff(wType, pSkillProp->dwID, nLevel, nSkillTime + (int)dwTime1 + (int)dwTime2, pSrc->GetId());
#else
...
#ifdef __INCREASE_SKILL_ID_VALUE
				bNew = ((CMover*)this)->HasBuff(wType, pSkillProp->dwID) ? FALSE : TRUE;
#else
...
#if __VER >= 8 // __S8_PK
int		CCtrl::DoApplySkill( CCtrl *pTarget, ItemProp *pSkillProp, AddSkillProp *pAddSkillProp, bool bIgnoreProb, int nParam, BOOL bOnlyDmg, BOOL bControl, int nSfxMode)
#else // __VER >= 8 // __S8_PK
...
{
#if __VER >= 8 // __S8_PK
	int nResult = DoApplySkillEx( pTarget, pSkillProp, pAddSkillProp, bIgnoreProb, nParam, bOnlyDmg, bControl, nSfxMode);
#else // __VER >= 8 // __S8_PK
...
int		CCtrl::DoApplySkillEx(CCtrl* pTarget, ItemProp* pSkillProp, AddSkillProp* pAddSkillProp, bool bIgnoreProb, int nParam, BOOL bOnlyDmg, BOOL bControl, int nSfxMode)
#else
...
#endif // __VER >= 8 // __S8_PK
{
	...
	if( pSrc->GetType() == OT_MOVER )
	{
		...
		if (this != pSrc && pSrc->GetType() == OT_MOVER && ((CMover*)pSrc)->IsPlayer())
		{
			...
			if (fX > 1.0f && fZ > 1.0f)
			{
				D3DXVECTOR3 v3Lenn = D3DXVECTOR3(fX, 0.0f, fZ);
				float fTotalLenn = D3DXVec3LengthSq(&v3Lenn);

				fTotalLenn -= GetRadiusXZ();
				fTotalLenn -= pSrc->GetRadiusXZ();

				float fAttRange = ((CMover*)pSrc)->GetAttackRange(pSkillProp->dwAttackRange);
				if (fAttRange && fTotalLenn < fAttRange)
					return 0;
			}
		}
#endif // __GAMEGUARD
