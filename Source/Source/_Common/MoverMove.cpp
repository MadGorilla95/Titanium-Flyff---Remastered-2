...
#ifdef __fmod
		if (rectSaintmorning.PtInRect(CPoint((int)(x), (int)(z)))) { SoundSystemClass::playMusic(BGM_BA_SAINTMORNING, true); }
		else if (x < 4609) { SoundSystemClass::playMusic(BGM_BA_DARKON, true); }
		else if (x < 6309) { SoundSystemClass::playMusic(BGM_BA_CRISIS, true); }
		else if (x < 8138) { SoundSystemClass::playMusic(BGM_BA_FLARIS, true); }
		else { SoundSystemClass::playMusic(BGM_BA_SAINTMORNING, true); }
...
#ifdef	__RANGE_FIX
					DWORD dwRange = pSkill->GetProp()->dwAttackRange;
					float fRange = ((CMover*)pObj)->GetAttackRange(dwRange);

#ifdef __ACTION_SLOT_PAGES
					g_DPlay.SendUseSkill(0, nSkillIdx, idTarget, dwRange, fRange, sutType, bControl, g_WndMng.m_pWndWorld->GetCurrentActionSlot());
#else // __ACTION_SLOT_PAGES
					g_DPlay.SendUseSkill(0, nSkillIdx, idTarget, sutType, bControl);
#endif // __ACTION_SLOT_PAGES
#else //__RANGE_FIX
#ifdef __ACTION_SLOT_PAGES
					g_DPlay.SendUseSkill(0, nSkillIdx, idTarget, sutType, bControl, g_WndMng.m_pWndWorld->GetCurrentActionSlot());
#else // __ACTION_SLOT_PAGES
...
#ifdef __GAMEGUARD
				if (pItemProp && pItemProp->dwItemKind3 == IK3_YOYO && IsInteriorityJob(JOB_ACROBAT))
#else // __GAMEGUARD
	...
	case OBJACT_USESKILL:
		if( pObj->GetType() == OT_MOVER )	// 타겟이 무버일때면 처리함.
		{
			...
			if( pSkill->dwSkill == SI_MAG_MAG_BLINKPOOL 
				...
				|| pSkill->dwSkill == SI_RIG_HERO_RETURN 
#endif
				)
...
#ifdef __BS_FIX_ARRIVEPOS_ALGO		// ¸¶¿i½º ¸nAuAAC￥ AIμ¿½A Ay´eAa°u °°Aº°æ¿i Aß°￡¿¡ ¸ØAß´A Co≫oAI AO¾u´U.
