...
#ifdef __CLIENT
#ifdef __fmod
		D3DXVECTOR3 vec = pMover->GetPos();
		SoundSystemClass::playSound(pMover->GetProp()->dwSndDmg2, { vec.x, vec.y, vec.z });
#else
...
#else //__FIX_REMOVESFX
		if (IsValidObj(pObjDest))
			m_vPosDest = pObjDest->GetPos();
#endif //__FIX_REMOVESFX
