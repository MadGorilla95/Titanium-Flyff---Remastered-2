...
int CRandomOptionProperty::GetRandomOptionKind( CItemElem* pItemElem )
{
	...
	switch( pProp->dwParts )
	{
		...
		{
#ifdef __GAMEGUARD
			if (pProp->dwID != II_GEN_TOO_COL_NOMAL
				&& pProp->dwID != II_GEN_TOO_COL_VTHREE
				&& pProp->dwID != II_GEN_TOO_COL_POWER
				&& pProp->dwID != II_GEN_TOO_COL_ULTRA
				&& pProp->dwID != II_GEN_TOO_COL_NORMAL
				&& pProp->dwID != II_GEN_TOO_COL_EVENT)
#endif // __GAMEGUARD
			{
...
int CRandomOptionProperty::MergeWeaponRandomOption(CItemElem* pSource, CItemElem* pDest)
{
	int nSizeSource = g_xRandomOptionProperty->GetRandomOptionSize(pSource->GetRandomOptItemId());
	int nSizeDest = g_xRandomOptionProperty->GetRandomOptionSize(pDest->GetRandomOptItemId());
	if (nSizeSource < 0)
		return 1;
	if (nSizeSource + nSizeDest > 6)
		return 2;
	pDest->SetMergedRandomOptItemId(pSource->GetRandomOptItemId());
	return 0;
}
...
#ifdef __AEGON_AWAKE_COLOR
BOOL CRandomOptionProperty::IsMaxAdjParamary(int nRandomOptionKind, int nDstParam, int nAdjParam)
{
	if (nRandomOptionKind < 0 || nRandomOptionKind >= eMaxRandomOptionKind)
		return FALSE;

	if (nDstParam < 0 || nDstParam >= MAX_ADJPARAMARY)
		return FALSE;

	if (m_anMaxAdjParamary[nRandomOptionKind][nDstParam] != 0 && m_anMaxAdjParamary[nRandomOptionKind][nDstParam] <= nAdjParam)
	{
		return TRUE;
	}
	return FALSE;
