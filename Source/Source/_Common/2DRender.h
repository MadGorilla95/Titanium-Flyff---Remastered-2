...
public:
	...
	BOOL RenderFillZone( CPoint pt1, CPoint pt2, CPoint pt3, CPoint pt4, DWORD dwColor );
...
public:
	...
	BOOL LoadTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader = FALSE, BOOL bLogErrors = TRUE);
...
public:
...
#ifdef __AEGON_FIXES
#ifdef __CLIENT
	map<std::string, DWORD> m_mapLoadRetry;
#endif
#endif
	...
	BOOL DeleteDeviceObjects();
	CTexture* AddTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader = FALSE, BOOL bLogErrors = TRUE );
...
public:
	...
	CHealNum(D3DXVECTOR3 vPos, DWORD nNumber, DWORD nAttribute)
	{
		m_nFrame = 0; m_nNumber = nNumber; m_vPos = vPos; m_nAttribute = nAttribute; m_nState = 0; m_nCnt = 0;
	}
	~CHealNum() {};
...
#ifndef __VM_0820
#ifndef __MEM_TRACE
	static MemPooler<CHealNum>* m_pPool;
	void* operator new(size_t nSize) { return CHealNum::m_pPool->Alloc(); }
	void* operator new(size_t nSize, LPCSTR lpszFileName, int nLine) { return CHealNum::m_pPool->Alloc(); }
	void	operator delete(void* lpMem) { CHealNum::m_pPool->Free((CHealNum*)lpMem); }
	void	operator delete(void* lpMem, LPCSTR lpszFileName, int nLine) { CHealNum::m_pPool->Free((CHealNum*)lpMem); }
#endif	// __MEM_TRACE
#endif	// __VM_0820
#endif	// __CLIENT
};
...
public:
	...
	CHealNumMng() { D3DXMatrixIdentity(&m_matWorld); };
	~CHealNumMng();
