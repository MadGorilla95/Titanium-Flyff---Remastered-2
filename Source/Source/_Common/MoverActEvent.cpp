			...
			if( pAddSkillProp )
			{
				if( pSkillProp->dwReferTarget1 == RT_HEAL || pSkillProp->dwReferTarget2 == RT_HEAL )
				{
					...
					if( (pAddSkillProp->dwDestParam1 == DST_HP || pAddSkillProp->dwDestParam2 == DST_HP) && nIncHP > 0 )
					{
							...
							nIncHP = (int)(((float)nIncHP) * (GetParam(DST_HEALING_AMP, 100) / 100.0F));
...
#ifdef __HEAL_SHOW
					g_UserMng.AddHealDamage(pTarget, GETID(pTarget), nIncHP);
#endif // __HEAL_SHOW	
				}
			}
			break;
...
//		for( int nDmgCnt = 0; nDmgCnt < nMaxDmgCnt; nDmgCnt++ )
	...
	if( bSuccess )	// 스킬사용 실패하면 마나안깎임. / 경험치 안오름.
	{
		...
		#endif	//__LEGEND	//	10차 전승시스템	Neuz, World, Trans
		{
			...
			{
#ifdef __AEGON_RM_HERO_SKILL
				if (IsValidObj(pFocusObj) && pFocusObj->IsPlayer())
				{
					vector<DWORD> vBuffs;
					...
					for (int i = 0; i < vBuffs.size(); i++) 
					{
						...
						if (pSkillProp)
						{
							...
							if (pAddSkillProp)
							{
								DoApplySkill(pFocusObj, pSkillProp, pAddSkillProp, false, 0, 0, 0, 1);
								g_UserMng.AddDoApplySkill(this, pFocusObj->GetId(), vBuffs[i], pSkillProp->dwExpertMax, 1);
							}
						}
					}
				}
			...
			if( pSkillProp->dwID == SI_KNT_HERO_DRAWING || pSkillProp->dwID == SI_LOD_SUP_PULLING )
			{
				...
				if (pFocusObj->GetWorld()->GetID() != GetWorld()->GetID())
					return FALSE;
				if (this->IsSchoolTarget(pFocusObj) != pFocusObj->IsSchoolTarget(this))
					return FALSE;
#endif // __GAMEGUARD
...
void	CMover::OnAttackMelee_BigMuscle( DWORD dwState, CMover *pHitObj )
{
...
#ifdef __fmod
			SoundSystemClass::playSound(SND_PC_SKILLD_HITOFPENYA);
#else
	...
	#ifdef __CLIENT
#ifdef __fmod
				SoundSystemClass::playSound(SND_PC_BOSS_STUN);
#else
...
void	CMover::OnAttackMelee_Krrr( DWORD dwState, CMover *pHitObj )
{
...
#ifdef __fmod
			const D3DXVECTOR3 vec = pHitObj->GetPos();
			SoundSystemClass::playSound(SND_PC_BOSS_CRASH, { vec.x, vec.y, vec.z });
#else
	...
	case OBJSTA_ATK4:		// 주문을 외우고 플레이어들 한곳에 모인후 광역데미지 줌
		...
		#ifdef __CLIENT
#ifdef __fmod
				SoundSystemClass::playSound(SND_PC_BOSS_SUMMONS);
#else
...
#ifdef __RANGE_FIX
	if (pHitObj->GetType() == OT_MOVER)
	{
		ItemProp* pItemProp = GetActiveHandItemProp();
		if (pItemProp && IsRangeObj(pHitObj, 5.2f))
			bSuccess = TRUE;
		else
			bSuccess = FALSE;
	}
...
*/
#ifdef __fmod
			SoundSystemClass::playSound(pItemProp->dwSndAttack1, { m_vPos.x, m_vPos.y, m_vPos.z });
#else
...
void	CMover::OnActDrop()
{
	...
	if( IsActiveMover() )
	{
		if( m_nCorr != 0 )
		{
#ifndef __MOVE_LIKE_OFFI
			if (m_nCorr < 0) {
				ClearDest();
				SendActMsg(OBJMSG_STAND);
			}
#endif // !__MOVE_LIKE_OFFI
			if( m_pActMover->IsFly() )
...
BOOL CMover::IsLoot( CItem *pItem, BOOL bPet )
{
	...
	if( bPet && bTake )
	{
		...
		if (pItemProp->dwItemKind3 != IK3_GOLD) 
		{
			...
			{
				CPocket* pPocket = m_Pocket.GetPocket(m_nSelectedBag - 1);
				if (!pPocket || pPocket->IsFull(pItemElem, pItemProp, pItemElem->m_nItemNum)) 
				{
					if (m_Inventory.IsFull(pItemElem, pItemProp, pItemElem->m_nItemNum)) 
					{
						bErrMsgFull = TRUE;
			...
			if (bErrMsgFull && m_bPlayer && GetTickCount() > m_dwLastInventoryFullMsg + 5000)
			{
				CString strError;
				strError.Format("%s full.", m_nSelectedBag == 0 ? "Inventory" : "Bag");
				((CUser*)this)->AddText(strError.GetString(), COLOR_ERROR);
				m_dwLastInventoryFullMsg = GetTickCount();
			}
		}
#else
...
BOOL CMover::SubLootDropMobSingle( CItem *pItem )
{
	...
	if (m_nSelectedBag == 0)
	{
		...
		bSuccess = CreateItem(pItemBase, NULL, NULL, 0, TRUE);
#else
	...
	{
		bSuccess = CreateItem2((CItemElem*)pItemBase, m_nSelectedBag - 1);
		if (!bSuccess)
		{
#if defined  (__FL_FARM_STAT) || defined (__DROP_LOG)
			bSuccess = CreateItem(pItemBase, NULL, NULL, 0, TRUE);
...
BOOL CMover::SubLootDropMobParty( CItem *pItem, CParty *pParty )
{
...
#ifdef __FL_BAG_LOOT
	if (pGetUser->m_nSelectedBag == 0)
	{
#if defined  (__FL_FARM_STAT) || defined (__DROP_LOG)
		bSuccess = pGetUser->CreateItem(pItemBase, NULL, NULL, 0, TRUE);
#else
	...
	{
		bSuccess = pGetUser->CreateItem2((CItemElem*)pItemBase, pGetUser->m_nSelectedBag - 1);
		if (!bSuccess)
		{
#if defined  (__FL_FARM_STAT) || defined (__DROP_LOG)
			bSuccess = pGetUser->CreateItem(pItemBase, NULL, NULL, 0, TRUE);
	...
	for( int i = 0; i < nMaxListMember; i++ )
	{
		...
		if( TRUE == pMember->IsPlayer() && pItemBase->GetProp() != NULL )
		{
			if( bSuccess )
			{
				...
				{
					CString partyDrop;
					partyDrop.Format("%s has received: %s", pGetUser->GetName(), pItemBase->GetProp()->szName);
					((CUser*)pMember)->AddText(partyDrop);
				}
#ifndef __DROP_LOG
				else
				{
...
BOOL CMover::SubLootDropNotMob( CItem *pItem )
{
	...
	{
		bSuccess = CreateItem2((CItemElem*)pItemBase, this->m_nSelectedBag - 1);
		if (!bSuccess)
		{
#if defined  (__FL_FARM_STAT) || defined (__DROP_LOG)
			bSuccess = CreateItem(pItemBase, NULL, NULL, 0, TRUE);
...
#ifdef __CLIENT
#ifdef __fmod
	SoundSystemClass::playSound(SND_INF_GROUNDPICKUP);
#else
...
#ifdef __RANGE_COLLECT_ITEM
		CObj* pObj = NULL;
		int nRange = 0;
		D3DXVECTOR3 vPos = GetPos();
		D3DXVECTOR3 vPosPet;
		D3DXVECTOR3 vDist, vDistPet;
		FLOAT fDistSq = 9999999.0f;
		FLOAT fDistSqPet = 9999999.0f;

		if (HasActivatedEatPet())
		{
			CMover* eatPet = prj.GetMover(GetEatPetId());
			if (eatPet)
				vPosPet = eatPet->GetPos();
		}

		FOR_LINKMAP(GetWorld(), vPos, pObj, nRange, CObj::linkDynamic, GetLayer())
		{
			if (pObj->GetType() == OT_ITEM && pObj->IsDelete() == FALSE)
			{
				...
				if (HasActivatedEatPet()) 
				{
					vDistPet = pObj->GetPos() - vPosPet;
					fDistSqPet = D3DXVec3LengthSq(&vDistPet);
				}
				else 
				{
					fDistSqPet = 9999999.0f;
				}
					
				if (fDistSqPet < fDistSq)
					fDistSq = fDistSqPet;
				
				if (fDistSq < 12.0f * 12.0f && pItemBase->GetProp()->dwID == pObj->GetIndex()) //originally 8*8 range
				{
