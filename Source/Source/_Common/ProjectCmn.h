...
#define MIN_CUSTOM_TITLE	4
#define MAX_CUSTOM_TITLE	20
#endif // __SYS_CUSTOM_TITLE

#ifdef __ANARCHY_SYSTEM
#define MAX_ANARCHY_BUFFS	12
#endif // __ANARCHY_SYSTEM

#ifdef __SYS_AUTO_SHOUT
#define MAX_AUTO_SHOUT		8
#endif // __SYS_AUTO_SHOUT

#ifdef __SYS_SANDBOX
#define WORLD_SANDBOX		WI_WORLD_SANDBOX
#endif //__SYS_SANDBOX

#ifdef __BUYBACK
#define MAX_BUYBACK				25
#endif // __BUYBACK

#ifdef __ACTION_SLOT_PAGES
#define MAX_ACTION_SLOT			3
#endif //__ACTION_SLOT_PAGES

#ifdef __SYS_REBIRTH
#define MAX_REBIRTH_LEVEL		10
#endif //__SYS_REBIRTH
...
#ifdef __AEGON_FIXES
	int		nAdjParamVal[6];	// 적용값1;
#else
	DWORD	nAdjParamVal[5];	// 적용값1;
#endif
...
#define ALTERNATE_PRICE_MAX 4

struct AlternatePrices {
	// We don't use a std::map because FlyFF does memset on ItemProp
private:
	DWORD currency[ALTERNATE_PRICE_MAX];
	int price[ALTERNATE_PRICE_MAX];
	int max;

	int GetIndex(DWORD currency) const {
		for (int i = 0; i != max; i++) {
			if (currency == this->currency[i])
				return i;
		}
		return -1;
	}
...
public:
	...
	bool AddPrice(DWORD currency, int price) {
		int index = GetIndex(currency);
		if (index == -1) {
			if (max == ALTERNATE_PRICE_MAX) {
				Error("AlternatePrices::AddPrice - Too Much Item (Currency %lu, Price %d)", currency, price);
				return false;
			}

			this->currency[max] = currency;
			this->price[max] = price;
			max++;
		}
		else if (this->price[index] != price) {
			Error("AlternatePrices::AddPrice - Price doesn't match (Currency %lu, Wanted price %d, Current Price %d)", currency, price, this->price[index]);
			return false;
		}
		else {
			return true;
	...
	int GetPrice(DWORD currency) const {
		int index = GetIndex(currency);
		return index == -1 ? -1 : price[index];
	}
};
...
struct ItemProp : CtrlProp
{
	...
	BOOL IsBaruna( void ) { return( dwReferStat1 >= BARUNA_D && dwReferStat1 <= BARUNA_S ); }
...
#ifdef __AEGON_BOW_FACTOR
	float	fBowFactor;				//Bow Atk factor
#endif // __AEGON_BOW_FACTOR
#ifdef __V19_FORMATS
	TCHAR	szJobIcon[64];
#endif //__V19_FORMATS
...
#define SHORTCUT_TELEPORT	12
#endif // __AEGON_TELEPORT_SHORTCUT
#ifdef __SWITCH_EQUIP
#define SHORTCUT_SWITCH_EQUIP 13
...
typedef struct tagSHORTCUT
{
	...
	DWORD     m_dwUserId   ; 
#ifdef _WIN64
	unsigned long long m_dwData;
#else
	unsigned long long m_dwData; // Make it support same size on 32 and 64bit rathwer than casting data.
	//DWORD     m_dwData     ; 
#endif
...
struct MOUNT_ANIMATION
{
	...
	DWORD		dwBindBone;
	map<int, CString> m_aMonsterAnimation;
	FLOAT		fGroundSpeed, fFlightSpeed;
	MOUNT_ANIMATION()
	{
		mType = 0;
		dwPlayerAni = 0;
		fGroundSpeed = fFlightSpeed = 0.0f;
		vRotation = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		vOffSet = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		dwBindBone = NULL;
		m_aMonsterAnimation.clear();
	}
};
#endif // __MOUNT
