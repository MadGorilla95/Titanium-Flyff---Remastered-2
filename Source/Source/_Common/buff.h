...
public:
	...
	void	SetTotal(DWORD tmTotal, bool bOverwrite = false);
...
#ifdef __ANARCHY_SYSTEM
class CBuffItem3 : public IBuffItemBase
{
public:
	CBuffItem3() {}
	virtual	~CBuffItem3() {}
...
#ifdef __INCREASE_SKILL_ID_VALUE
	void	RemoveBuff(WORD wType, DWORD dwId, BOOL bFake = TRUE);
#else
...
#ifdef __INCREASE_SKILL_ID_VALUE
	IBuff* GetBuff(WORD wType, DWORD dwId);
#else
	IBuff* GetBuff(WORD wType, WORD wId);
#endif
	IBuff*	GetBuffByIk3( DWORD dwIk3 );
#ifdef __ANARCHY_SYSTEM
	void	RemoveBuffs(DWORD dwFlags, DWORD dwParam = 0, bool bForceRemove = false);
#else // __ANARCHY_SYSTEM
...
#ifdef __ANARCHY_SYSTEM
#ifdef __INCREASE_SKILL_ID_VALUE
	BOOL	AddBuff(WORD wType, DWORD dwId, DWORD dwLevel, DWORD tmTotal, OBJID oiAttacker = 0xFFFFFFFF, bool bOverwrite = false);
#else
	BOOL	AddBuff(WORD wType, WORD wId, DWORD dwLevel, DWORD tmTotal, OBJID oiAttacker = 0xFFFFFFFF, bool bOverwrite = false);
#endif
#else // __ANARCHY_SYSTEM
...
protected:
#ifdef __ANARCHY_SYSTEM
	BOOL	AddBuff(IBuff* pBuff, bool bOverwrite = false);
#else // __ANARCHY_SYSTEM
