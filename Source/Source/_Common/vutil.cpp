	...
	)
{
	...
	if (!pDevice || !ppTexture)
	{
		if (bLogErrors) 
		{
			LPCTSTR szErr = Error("LoadTextureFromRes aegdebug pDevice: %p filename: %s ppTexture: %p", (void*)pDevice, pFileName, (void*)ppTexture);
			ADDERRORMSG(szErr);
		}
	...
	if (FAILED(hr)) 
	{
		...
		if (IsValidObj(g_pPlayer))
			strName = g_pPlayer->GetName();
		LPCTSTR szErr = Error("LoadTextureFromRes TestCooperativeLevel: %08x - char: %s", (int)hr, strName.GetString());
		ADDERRORMSG(szErr);
		...
		if (hr == D3DERR_DEVICENOTRESET)
		{
			...
			if (FAILED(hr))
			{
				szErr = Error("%s reset failure %08x - char: %s", pFileName, (int)hr, strName.GetString());
				ADDERRORMSG(szErr);
				return E_FAIL;
			}
			else
			{
				szErr = Error("D3D Reset successful - char: %s", strName.GetString());
				ADDERRORMSG(szErr);
			}
		}
		else
	...
	if( file.Read( pSrcData, nSrcDataSize ) >= 1 )
	{
		...
		try 
		{
			hr = D3DXGetImageInfoFromFileInMemory(pSrcData, nSrcDataSize, &imageInfo);

			if (hr == D3D_OK && imageInfo.Height > 0 && imageInfo.Width > 0)
			{
				hr = D3DXCreateTextureFromFileInMemoryEx(pDevice,
					pSrcData, nSrcDataSize, imageInfo.Width, imageInfo.Height, MipLevels, Usage, Format, Pool, Filter, MipFilter, ColorKey, pSrcInfo, pPalette, ppTexture);
			}
			else
			{
				hr = D3DXCreateTextureFromFileInMemoryEx(pDevice,
		...
		catch (const std::exception& e)
		{
			...
			LPCTSTR szErr = Error("::LoadTextureFromRes :D3DXCreateTextureFromFileInMemoryEx threw exception %s", e.what());
			ADDERRORMSG(szErr);
		}
#else // __AEGON_TEX_LOAD
			hr = D3DXCreateTextureFromFileInMemoryEx( pDevice,
				 pSrcData, nSrcDataSize, Width, Height, MipLevels, Usage, Format, Pool, Filter, MipFilter, ColorKey, pSrcInfo, pPalette, ppTexture );
#endif // __AEGON_TEX_LOAD
		if( FAILED( hr ) )
		{
			if (bLogErrors)
			{
...
BOOL LoadImage( LPCTSTR lpszFileName, LPIMAGE lpImage ) //LPBYTE* lppData, SIZE* pSize, int* pnImgBit )
{
...
#ifdef __CLIENT
	CString strTmp = "";
	strTmp.Format("%s", lpszFileName);
	if (strTmp.Find("ThemeSwitcher") != 0)
		bLogErrors = FALSE;
#endif // __AEGON_THEME_SWITCHER
#endif // __CLIENT
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		return LoadTGA(lpszFileName, lpImage, bLogErrors);
	...
	{
#ifdef __AEGON_THEME_SWITCHER
		return LoadBMP(lpszFileName, lpImage, bLogErrors);
#else // __AEGON_THEME_SWITCHER
...
BOOL LoadTGA( LPCTSTR lpszFileName, LPIMAGE lpImage, BOOL bLogErrors ) //LPBYTE* lppData, SIZE* pSize )
{
#if !defined(__SERVER)
	lpImage->nImgBit = IMGBIT_32;
	CTarga targa;
	return targa.Load( lpszFileName, &lpImage->lpData, &lpImage->size, bLogErrors );
#endif
	return FALSE;
}

BOOL LoadBMP( LPCTSTR lpszFileName, LPIMAGE lpImage, BOOL bLogErrors) //LPBYTE* lppData, SIZE* pSize )
{
	CResFile file;
