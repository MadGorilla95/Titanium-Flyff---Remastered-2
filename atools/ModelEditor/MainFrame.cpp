...
void CMainFrame::_connectWidgets()
{
	...
	connect(ui.actionExportFolder, SIGNAL(triggered()), this, SLOT(OpenFolderToO3d()));
}
...
void CMainFrame::SetReferenceModel()
{
	...
	models.push_back("Monster"); // SEX_MONSTER
	models.push_back("Cloak"); // SEX_CLOAK	
	models.push_back("Wing"); // SEX_WINGS
	models.push_back("Mask [M]"); // SEX_MASKM
	models.push_back("Mask [F]"); // SEX_MASKF
	models.push_back("2H Sword"); // SEX_2HSWORD
	models.push_back("1H Sword"); // SEX_1HSWORD	
	models.push_back("2H Axt"); // SEX_2AXT
	models.push_back("1H Axe"); // SEX_1AXE
	models.push_back("Knuckle"); // SEX_KNUCKLE
	models.push_back("Stick"); // SEX_STICK
	models.push_back("Wand"); // SEX_WAND
	models.push_back("Staff"); // SEX_STAFF
	models.push_back("Bow"); // SEX_BOW
	models.push_back("Yoyo"); // SEX_YOYO
	models.push_back("Shield"); // SEX_SHIELD
	models.push_back("Zem"); // SEX_ZEM
	models.push_back("Book"); // SEX_BOK

	bool ok = false;
	const int newModelID = models.indexOf(QInputDialog::getItem(this, tr("Model de référence"), tr("Model Reference:"), models, m_referenceModelID, false, &ok));
	if (ok)
	{
...
void CMainFrame::OpenFolderToO3d(const string& folderpath, const string& savefolderpath)
{
	bool resetView = true;
	...
	while (it.hasNext())
	{
		...
		if (extension == "dae")
		{
			...
			ChangeCW();
			SetOverallEffect(XE_OPACITY);
			SetOverallEffect(XE_2SIDE);

			if (filename.startsWith("mvr_", Qt::CaseInsensitive) || filename.startsWith("item_", Qt::CaseInsensitive))
			{
				if (m_mesh->m_motion)
					newextension = ".ani";
				else
			...
			filename.replace("." % extension, newextension, Qt::CaseInsensitive);
			_saveFile(savefolderpath % "/" % filename);
		}

		if (filename.startsWith("part_", Qt::CaseInsensitive))
			CloseFile();
	}
}
...
void CMainFrame::OpenFolderToO3d()
{
	string dir = ModelMng->GetModelPath();

	const QString foldername = QFileDialog::getExistingDirectory(this, tr("Choose a folder with .DAE models"), dir);
	const QString savefoldername = QFileDialog::getExistingDirectory(this, tr("Choose an output folder"), dir);

	if (!foldername.isEmpty() && !savefoldername.isEmpty())
		OpenFolderToO3d(foldername, savefoldername);
}
