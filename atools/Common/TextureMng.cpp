...
{
	TextureMng = this;
	m_modelTexturePath = "Data/Model/";
	m_sfxTexturePath = "Data/SFX/Texture/";
}
...
CTexture* CTextureMng::GetGUITexture(const string& filename)
{
	...
	if (!texture->Load("Data/Theme/" % filename, 0, 0xffff00ff, powerOfTwo))
	{
		qWarning(("Can't load GUI texture from file 'Data/Theme/" % filename % "'").toLocal8Bit().data());
		Delete(texture);
	}
...
CTexture* CTextureMng::GetWeatherTexture(const string& filename)
{
	...
	if (!texture->Load("Data/Weather/" % filename))
	{
